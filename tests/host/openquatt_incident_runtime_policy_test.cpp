...
using esphome::openquatt_incident_manager::
    all_hp_outputs_safe_for_fallback;
using esphome::openquatt_incident_manager::
...
void test_confirmation_audit_policy() {
  assert(thermal_command_for_expected_mode(1U) ==
         HpThermalCommand::COOLING);
  assert(thermal_command_for_expected_mode(3U) ==
         HpThermalCommand::COOLING);
  assert(thermal_command_for_expected_mode(2U) ==
         HpThermalCommand::HEATING);
  assert(thermal_command_for_expected_mode(0U) ==
         HpThermalCommand::UNKNOWN);
...
void test_start_failure_retry_production_policy() {
  ...
  assert(perform_start_failure_retry(nullptr, 0U) ==
         StartFailureResetResult::HP_NOT_CONFIGURED);

  oq_incidents::HpIncidentEngine engine;
  assert(perform_start_failure_retry(&engine, 1U) ==
  ...
  engine.tick(200102U);
  assert(perform_start_failure_retry(&engine, 200103U) ==
  ...
  assert(perform_start_failure_retry(&engine, 220105U) ==
         StartFailureResetResult::CLEARED);
  assert(!engine.outputs().start_timed_out);
  assert(perform_start_failure_retry(&engine, 220106U) ==
         StartFailureResetResult::NO_START_FAILURE);
}
...
void test_duo_fallback_coverage_fails_closed() {
  ...
  assert(!all_unavailable_hps_allow_fallback(outputs.data(),
                                             outputs.size()));

  outputs[1].fallback_cause_present = true;
  assert(all_unavailable_hps_allow_fallback(outputs.data(),
                                            outputs.size()));
  assert(!all_hp_outputs_safe_for_fallback(outputs.data(),
                                           outputs.size()));
...
void test_persistence_overlay_is_fail_closed_and_causally_consistent() {
  ...
  const oq_incidents::DerivedOutputs blocked =
      apply_persistence_safety_gate(outputs, true);
  assert(!blocked.available_for_start);
  assert(!blocked.fallback_eligible);
  assert(blocked.fault_active);
  assert(blocked.protection_state ==
         oq_incidents::ProtectionState::START_BLOCKED);
  assert(blocked.primary_incident_id ==
  ...
  const oq_incidents::DerivedOutputs unchanged =
      apply_persistence_safety_gate(outputs, false);
...
void test_first_urgent_flush_is_not_delayed_by_boot_interval() {
  ...
  assert(!policy.should_attempt(2999999ULL, kCoalesceUs,
                                kMinIntervalUs));
  assert(policy.should_attempt(3000000ULL, kCoalesceUs,
                               kMinIntervalUs));
}
...
void test_failed_flush_obeys_retry_window_and_sequence_wrap() {
  ...
  assert(!policy.should_attempt(32000099U, 2000000ULL,
                                15000000ULL));
...
void test_manual_reset_restart_restore_and_partial_clear() {
  ...
  assert(restored.load(true, stored_a, true, stored_b, true, marker,
                       oq_incidents::kManualResetAllHpMask) ==
         ManualResetLatchPersistencePolicy::LoadResult::RESTORED);
  assert(restored.persisted_mask() ==
         oq_incidents::kManualResetHp1Mask);

  oq_incidents::HpIncidentEngine engine;
  const oq_incidents::IncidentId manual_reset_id =
  ...
  assert(after_partial_clear.load(
             true, partially_cleared, true, stored_b, true, marker,
             oq_incidents::kManualResetAllHpMask) ==
         ManualResetLatchPersistencePolicy::LoadResult::RESTORED);
  assert(after_partial_clear.persisted_mask() ==
         oq_incidents::kManualResetHp1Mask);
...
void test_manual_reset_confirm_is_per_hp_and_write_failure_safe() {
  ...
  ManualResetLatchMarker marker{};
  ManualResetLatchPersistencePolicy policy;
  assert(policy.load(true, stored_a, true, stored_b, true, marker,
                     oq_incidents::kManualResetAllHpMask) ==
         ManualResetLatchPersistencePolicy::LoadResult::RESTORED);

  oq_incidents::HpIncidentEngine hp1;
  assert(hp1.restore_power_cycle_latch(
      oq_incidents::incident_id(2120U, 4U)));
  uint8_t target_mask = 0U;
  assert(policy.confirmation_target(
      1U, hp1.has_cleared_power_cycle_latch(), &target_mask));
  ...
  assert(!oq_incidents::redundant_manual_reset_state_matches(
      true, cleared_a, true, stored_b, true, marker, target_mask));
  policy.mark_confirmation_failure(1U, 100U);
  assert(hp1.has_power_cycle_latch());
  assert(policy.persisted_mask() ==
         oq_incidents::kManualResetAllHpMask);
  assert((policy.fault_mask() &
          oq_incidents::kManualResetHp1Mask) != 0U);
  ...
  assert(oq_incidents::redundant_manual_reset_state_matches(
      true, cleared_a, true, cleared_b, true, marker, target_mask));
...
void test_manual_reset_ambiguous_storage_repairs_conservatively() {
  ...
  ManualResetLatchMarker marker{};
  ManualResetLatchPersistencePolicy policy;
  assert(policy.load(true, invalid, true, valid, true, marker,
                     oq_incidents::kManualResetAllHpMask) ==
         ManualResetLatchPersistencePolicy::LoadResult::
             RECOVERY_REQUIRED);
  assert(!policy.ready());
  ...
  policy.mark_persist_success(
      oq_incidents::kManualResetAllHpMask, 101U);
  assert(policy.ready());
  assert(!policy.recovery_required());
...
void test_manual_reset_brownout_and_missing_slot_never_restore_empty() {
  ...
  assert(missing_slot.load(
             true, empty, false, empty, true, marker,
             oq_incidents::kManualResetAllHpMask) ==
         ManualResetLatchPersistencePolicy::LoadResult::
             RECOVERY_REQUIRED);
  assert(missing_slot.persisted_mask() ==
         oq_incidents::kManualResetAllHpMask);
  assert(missing_slot.should_attempt_persist(100U, 60000U));

  ManualResetLatchPersistencePolicy missing_marker;
  assert(missing_marker.load(
             true, empty, true, empty, false, marker,
             oq_incidents::kManualResetAllHpMask) ==
         ManualResetLatchPersistencePolicy::LoadResult::
             RECOVERY_REQUIRED);
  assert(missing_marker.persisted_mask() ==
         oq_incidents::kManualResetAllHpMask);
  assert(missing_marker.persistence_target_mask() ==
         oq_incidents::kManualResetAllHpMask);
}
