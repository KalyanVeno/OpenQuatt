...
static const uint32_t MANUAL_RESET_STORAGE_A_KEY =
    fnv1_hash("openquatt_incident_manual_reset_store_a");
static const uint32_t MANUAL_RESET_STORAGE_B_KEY =
    fnv1_hash("openquatt_incident_manual_reset_store_b");
static const uint32_t MANUAL_RESET_MARKER_KEY =
    fnv1_hash("openquatt_incident_manual_reset_marker");

bool url_path_matches(const char *url, const char *path) {
  if (url == nullptr || path == nullptr) return false;
  const size_t path_length = std::strlen(path);
  return std::strncmp(url, path, path_length) == 0 &&
...
bool header_matches_host(const std::string &header_value,
  ...
  const size_t authority_end =
      header_value.find_first_of("/?#", authority_start);
...
const char *start_failure_reset_result_name(
    oq_incidents::StartFailureResetResult result) {
  switch (result) {
    case oq_incidents::StartFailureResetResult::READY: return "ready";
...
bool write_raw(httpd_req_t *req, const char *value) {
  return value != nullptr &&
         httpd_resp_send_chunk(req, value, HTTPD_RESP_USE_STRLEN) == ESP_OK;
}
...
bool write_bool(httpd_req_t *req, bool value) {
  return write_raw(req, value ? "true" : "false");
}
...
const char *link_state_name(oq_incidents::LinkState state) {
  switch (state) {
    case oq_incidents::LinkState::BOOTSTRAP: return "bootstrap";
    case oq_incidents::LinkState::HEALTHY: return "healthy";
    case oq_incidents::LinkState::SUSPECT: return "suspect";
    case oq_incidents::LinkState::LOST: return "lost";
    case oq_incidents::LinkState::RECOVERING: return "recovering";
  }
  return "unknown";
}
...
const char *protection_state_name(oq_incidents::ProtectionState state) {
  switch (state) {
    case oq_incidents::ProtectionState::CLEAR: return "clear";
    case oq_incidents::ProtectionState::LIMITED: return "limited";
    case oq_incidents::ProtectionState::START_BLOCKED: return "start_blocked";
    case oq_incidents::ProtectionState::FAULT_ACTIVE: return "fault_active";
    case oq_incidents::ProtectionState::FAULT_RECOVERY: return "fault_recovery";
  }
  return "unknown";
}
...
const char *run_state_name(oq_incidents::RunState state) {
  switch (state) {
    case oq_incidents::RunState::UNKNOWN: return "unknown";
    case oq_incidents::RunState::STOPPED: return "stopped";
    case oq_incidents::RunState::START_REQUESTED: return "start_requested";
    case oq_incidents::RunState::WAIT_MODE: return "wait_mode";
    case oq_incidents::RunState::WAIT_COMPRESSOR: return "wait_compressor";
    case oq_incidents::RunState::RUNNING: return "running";
    case oq_incidents::RunState::STOPPING: return "stopping";
    case oq_incidents::RunState::STOP_UNCONFIRMED: return "stop_unconfirmed";
  }
  return "unknown";
}
...
const char *category_name(oq_incidents::IncidentCategory category) {
  switch (category) {
    case oq_incidents::IncidentCategory::STATUS: return "status";
    case oq_incidents::IncidentCategory::PROTECTION: return "protection";
    case oq_incidents::IncidentCategory::WARNING: return "warning";
    case oq_incidents::IncidentCategory::FAULT: return "fault";
  }
  return "unknown";
}
...
const char *severity_name(oq_incidents::IncidentSeverity severity) {
  switch (severity) {
    case oq_incidents::IncidentSeverity::INFO: return "info";
    case oq_incidents::IncidentSeverity::WARNING: return "warning";
    case oq_incidents::IncidentSeverity::FAULT: return "fault";
  }
  return "warning";
}
...
const char *user_action_name(oq_incidents::UserAction action) {
  switch (action) {
    case oq_incidents::UserAction::NONE: return "none";
...
const char *recovery_condition_name(
    oq_incidents::RecoveryCondition condition) {
  switch (condition) {
...
const char *incident_lifecycle_name(
  ...
  if (runtime.latched &&
      definition.clear_policy ==
          oq_incidents::ClearPolicy::AFTER_CONFIRMED_ODU_POWER_CYCLE) {
    return "latched";
  }
  return "cleared";
}

const char *availability_name(
  ...
  if (outputs.link_state == oq_incidents::LinkState::RECOVERING ||
      outputs.protection_state ==
          oq_incidents::ProtectionState::FAULT_RECOVERY) {
    return "recovering";
  }
  if (outputs.must_stop ||
      outputs.link_state == oq_incidents::LinkState::LOST) {
    return "unavailable";
  }
  if (outputs.protection_state ==
...
const char *system_action_name(uint8_t control_mode, bool fallback_requested,
                               bool fallback_active, uint8_t block_reason,
                               bool boiler_command_active) {
  if ((fallback_active || control_mode == 4U) && boiler_command_active) {
    return "boiler_fallback";
  }
  if ((fallback_requested || control_mode == 4U) &&
      (!boiler_command_active || block_reason != 0U)) {
...
bool write_effects(httpd_req_t *req, oq_incidents::EffectMask effects) {
  ...
  static constexpr NamedEffect kEffects[] = {
      ...
      {oq_incidents::IncidentEffect::REQUIRE_CONFIRMED_ODU_POWER_CYCLE,
  ...
  for (const NamedEffect &entry : kEffects) {
    ...
    if ((!first && !write_raw(req, ",")) ||
        !write_json_string(req, entry.name)) {
      return false;
    }
    first = false;
  }
...
class IncidentManagerRequestHandler : public AsyncWebHandler {
 public:
  explicit IncidentManagerRequestHandler(OpenQuattIncidentManager *parent)
      : parent_(parent) {}
  ...
  bool canHandle(AsyncWebServerRequest *request) const override {
    ...
    return url_path_matches(url_buf, "/openquatt/incidents") &&
  ...
  void handleRequest(AsyncWebServerRequest *request) override {
    ...
    const bool retry_start = url_path_matches(
        url_buf, "/openquatt/incidents/retry-start");
    const bool confirm_odu_power_cycle = url_path_matches(
        url_buf,
        "/openquatt/incidents/confirm-odu-power-cycle");
    if (retry_start || confirm_odu_power_cycle) {
      if (!this->passes_same_origin_(request) ||
          !this->passes_csrf_(request)) {
        request->send(
            403, "application/json",
            R"({"accepted":false,"result":"forbidden"})");
        return;
      }

      const std::string hp_arg = request->arg("hp");
      const uint8_t hp_index =
          hp_arg == "1" ? 1U : (hp_arg == "2" ? 2U : 0U);
      if (hp_index == 0U) {
        request->send(
            400, "application/json",
            R"({"accepted":false,"result":"invalid_hp"})");
        return;
      }
...
float OpenQuattIncidentManager::get_setup_priority() const {
...
void OpenQuattIncidentManager::loop() {
  ...
  for (size_t slot = 0U; slot < this->units_().size(); ++slot) {
    ...
    if (has_pending_fault_word &&
        elapsed_ms_(now_ms, unit.fault_pending_since_ms) >=
            PARTIAL_FAULT_SNAPSHOT_TIMEOUT_MS) {
      this->process_fault_snapshot_(unit, slot, now_ms, true);
    }

    if (link_round_timeout_elapsed(
            now_ms, unit.last_link_round_ms,
            LINK_ROUND_TIMEOUT_MS)) {
...
void OpenQuattIncidentManager::dump_config() {
  ...
  ESP_LOGCONFIG(TAG,
                "  Start retry: POST /openquatt/incidents/retry-start");
  ESP_LOGCONFIG(
      TAG,
...
void OpenQuattIncidentManager::record_action_result_(
    UnitState &unit, const char *action, const char *result, bool ok,
...
bool OpenQuattIncidentManager::valid_hp_index_(uint8_t hp_index) {
  return hp_index == 1U || hp_index == 2U;
}

size_t OpenQuattIncidentManager::hp_slot_(uint8_t hp_index) {
  return static_cast<size_t>(hp_index - 1U);
}

uint32_t OpenQuattIncidentManager::elapsed_ms_(uint32_t now_ms,
                                               uint32_t since_ms) {
...
void OpenQuattIncidentManager::observe_transport(uint8_t hp_index, bool online,
                                                 uint32_t now_ms) {
...
void OpenQuattIncidentManager::observe_working_mode(
    uint8_t hp_index, float working_mode, uint32_t now_ms) {
  UnitState *unit = this->unit_(hp_index);
...
void OpenQuattIncidentManager::observe_compressor_frequency(
    uint8_t hp_index, float frequency_hz, uint32_t now_ms) {
  UnitState *unit = this->unit_(hp_index);
  ...
  const bool waiting_for_start =
      run_state == oq_incidents::RunState::START_REQUESTED ||
      run_state == oq_incidents::RunState::WAIT_MODE ||
      run_state == oq_incidents::RunState::WAIT_COMPRESSOR;
  const bool waiting_for_stop =
      run_state == oq_incidents::RunState::STOPPING ||
      run_state == oq_incidents::RunState::STOP_UNCONFIRMED;
  ...
  observation.stop_mode_confirmed =
      unit->working_mode_valid && post_command_mode &&
      static_cast<uint8_t>(std::lround(unit->working_mode)) == 0U;
  unit->engine.observe_run(observation);
  this->publish_transitions_(*unit, hp_slot_(hp_index), now_ms);
  if (unit->engine.outputs().run_state ==
      oq_incidents::RunState::RUNNING) {
    unit->start_feedback_armed = false;
  }
  if (unit->engine.outputs().run_state ==
      oq_incidents::RunState::STOPPED) {
    unit->stop_feedback_armed = false;
    unit->active_command_mode = 0U;
    unit->stop_confirmation_reason =
        openquatt_decision_log::REASON_UNKNOWN;
  }
  this->publish_snapshot_(now_ms);
}

void OpenQuattIncidentManager::observe_fault_word(
    uint8_t hp_index, uint16_t register_address, uint16_t word,
    uint32_t now_ms) {
  UnitState *unit = this->unit_(hp_index);
  ...
  const size_t bank =
      static_cast<size_t>(register_address - oq_incidents::kFirstFaultRegister);
...
void OpenQuattIncidentManager::process_fault_snapshot_(
    UnitState &unit, size_t slot, uint32_t now_ms, bool force_partial) {
  const bool complete =
      unit.fault_pending[0] && unit.fault_pending[1] &&
      unit.fault_pending[2];
...
void OpenQuattIncidentManager::observe_complete_link_round_(
    UnitState &unit, uint32_t now_ms) {
  const bool complete =
      unit.transport_seen && unit.transport_online &&
      feedback_generation_is_newer(
          unit.working_mode_generation,
          unit.last_link_working_generation) &&
      feedback_generation_is_newer(
          unit.compressor_frequency_generation,
          unit.last_link_frequency_generation) &&
      feedback_generation_is_newer(
          unit.fault_snapshot_generation,
          unit.last_link_fault_generation);
  if (!complete) return;
...
bool OpenQuattIncidentManager::request_start(uint8_t hp_index,
                                             uint8_t expected_mode,
                                             uint32_t now_ms) {
  UnitState *unit = this->unit_(hp_index);
  const bool newly_armed =
      unit != nullptr &&
      unit->engine.outputs().run_state == oq_incidents::RunState::STOPPED;
...
void OpenQuattIncidentManager::request_stop(uint8_t hp_index,
                                            uint32_t now_ms) {
  UnitState *unit = this->unit_(hp_index);
  if (unit == nullptr) return;
  const oq_incidents::DerivedOutputs stop_cause_outputs =
      unit->engine.outputs();
  if (unit->engine.request_stop(now_ms)) {
    ...
    if (stop_cause_outputs.must_stop) {
      stop_reason = reason_for_incident_id_(
          stop_cause_outputs.primary_incident_id);
      if (stop_reason == openquatt_decision_log::REASON_UNKNOWN &&
          stop_cause_outputs.primary_incident_id !=
              oq_incidents::kNoIncident) {
        stop_reason = incident_reason_(oq_incidents::definition_for_id(
            stop_cause_outputs.primary_incident_id));
      }
    }
    if (stop_reason == openquatt_decision_log::REASON_UNKNOWN) {
...
oq_incidents::StartFailureResetResult
OpenQuattIncidentManager::retry_start_failure(uint8_t hp_index,
                                              uint32_t now_ms,
                                              uint32_t request_id) {
  UnitState *unit = this->unit_(hp_index);
  const oq_incidents::StartFailureResetResult result =
      perform_start_failure_retry(
          unit != nullptr ? &unit->engine : nullptr, now_ms);
  if (unit == nullptr) return result;
  const bool cleared =
      result == oq_incidents::StartFailureResetResult::CLEARED;
  record_action_result_(
      *unit, "start_failure_retry",
      start_failure_reset_result_name(result), cleared, now_ms,
      request_id);
  if (cleared) {
    this->publish_transitions_(*unit, hp_slot_(hp_index), now_ms);
  }
...
bool OpenQuattIncidentManager::acknowledge(
    uint8_t hp_index, oq_incidents::IncidentId incident_id) {
  UnitState *unit = this->unit_(hp_index);
  ...
  oq_incidents::IncidentRuntime &runtime =
      unit->synthetic_incidents[synthetic_slot];
  ...
  const oq_incidents::IncidentDefinition definition =
      synthetic_definition_(synthetic_slot);
  this->publish_synthetic_transition_(
      slot, definition, reason_for_incident_id_(definition.id), previous,
      runtime, now_ms);
  this->publish_snapshot_(now_ms);
  return true;
}
  ...
  for (size_t hp_slot = 0U; hp_slot < this->units_().size(); ++hp_slot) {
    ...
    for (size_t synthetic_slot = 0U;
      ...
      const oq_incidents::IncidentDefinition definition =
          synthetic_definition_(synthetic_slot);
      this->publish_synthetic_transition_(
          hp_slot, definition, reason_for_incident_id_(definition.id),
          previous, runtime, now_ms);
      ++acknowledged;
    }
  }
  this->publish_snapshot_(now_ms);
...
bool OpenQuattIncidentManager::confirm_odu_power_cycle(uint8_t hp_index,
                                                       uint32_t now_ms,
                                                       uint32_t request_id) {
  UnitState *unit = this->unit_(hp_index);
  if (unit == nullptr) {
  ...
  if (!unit->engine.has_cleared_power_cycle_latch()) {
    record_action_result_(
        *unit, "confirm_odu_power_cycle",
        "no_cleared_manual_reset_latch", false, now_ms,
        request_id);
    this->publish_snapshot_(now_ms);
    return false;
  }
  ...
  if (!this->manual_reset_persistence_.confirmation_target(
          hp_index, true, &target_mask)) {
    ESP_LOGE(TAG,
    ...
    record_action_result_(
        *unit, "confirm_odu_power_cycle",
        "persistence_unavailable", false, now_ms, request_id);
    this->publish_snapshot_(now_ms);
    return false;
  }
  if (!this->persist_manual_reset_mask_(target_mask)) {
    this->manual_reset_persistence_.mark_confirmation_failure(
        hp_index, now_ms);
    ESP_LOGE(TAG,
    ...
    record_action_result_(
        *unit, "confirm_odu_power_cycle",
        "persistence_write_failed", false, now_ms, request_id);
    this->publish_transitions_(*unit, hp_slot_(hp_index), now_ms);
    this->publish_snapshot_(now_ms);
    return false;
  }
  ...
  if (!unit->engine.confirm_odu_power_cycle(now_ms)) {
    ...
    this->manual_reset_persistence_.observe_runtime_latches(
        this->runtime_manual_reset_mask_());
    this->reconcile_manual_reset_persistence_(now_ms);
    ESP_LOGE(TAG,
             "HP%u ODU power-cycle confirmation raced with incident state",
             hp_index);
    record_action_result_(
        *unit, "confirm_odu_power_cycle",
        "incident_state_changed", false, now_ms, request_id);
    this->publish_transitions_(*unit, hp_slot_(hp_index), now_ms);
    this->publish_snapshot_(now_ms);
    return false;
  }
  record_action_result_(
      *unit, "confirm_odu_power_cycle",
      "odu_power_cycle_confirmed", true, now_ms, request_id);
  this->publish_transitions_(*unit, hp_slot_(hp_index), now_ms);
  this->publish_snapshot_(now_ms);
  return true;
}
...
uint8_t OpenQuattIncidentManager::availability_state_(
  ...
  if (outputs.protection_state ==
      oq_incidents::ProtectionState::START_BLOCKED) {
    return outputs.primary_incident_id ==
                   oq_incidents::incident_id(2119U, 6U)
               ? openquatt_decision_log::STATE_PREHEAT
...
uint8_t OpenQuattIncidentManager::incident_reason_(
    const oq_incidents::IncidentDefinition &definition) {
...
uint8_t OpenQuattIncidentManager::incident_severity_(
    const oq_incidents::IncidentDefinition &definition) {
...
uint8_t OpenQuattIncidentManager::event_subject_(size_t slot) {
  return slot == 0U ? openquatt_decision_log::SUBJECT_HP1
                    : openquatt_decision_log::SUBJECT_HP2;
}

uint8_t OpenQuattIncidentManager::incident_flags_(
    const oq_incidents::IncidentDefinition &definition,
    const oq_incidents::IncidentRuntime &runtime) {
  uint8_t flags = oq_incidents::valid_fault_location(
                      definition.register_address, definition.bit)
  ...
  if (definition.clear_policy ==
      oq_incidents::ClearPolicy::AFTER_STABLE_READS) {
...
uint16_t OpenQuattIncidentManager::duration_seconds_(uint32_t from_ms,
                                                     uint32_t to_ms) {
  const uint32_t seconds = elapsed_ms_(to_ms, from_ms) / 1000U;
  return seconds > std::numeric_limits<uint16_t>::max()
             ? std::numeric_limits<uint16_t>::max()
             : static_cast<uint16_t>(seconds);
}

size_t OpenQuattIncidentManager::synthetic_slot_(
    oq_incidents::IncidentId incident_id) {
  switch (incident_id) {
    case LINK_LOSS_INCIDENT_ID: return 0U;
    case START_FAILED_INCIDENT_ID: return 1U;
    case STOP_UNCONFIRMED_INCIDENT_ID: return 2U;
    case PERSISTENCE_FAILURE_INCIDENT_ID: return 3U;
    default: return SYNTHETIC_INCIDENT_COUNT;
  }
}

oq_incidents::IncidentDefinition
OpenQuattIncidentManager::synthetic_definition_(size_t slot) {
  ...
  const oq_incidents::EffectMask fallback_fault_effects =
      static_cast<oq_incidents::EffectMask>(
  ...
  switch (slot) {
    ...
    case 3U:
      return {
          PERSISTENCE_FAILURE_INCIDENT_ID,
          0U,
          0U,
...
uint8_t OpenQuattIncidentManager::reason_for_incident_id_(
    oq_incidents::IncidentId incident_id) {
  switch (incident_id) {
...
uint32_t OpenQuattIncidentManager::epoch_for_runtime_ms_(
    uint32_t generated_epoch_s, uint32_t generated_at_ms,
    uint32_t runtime_ms) {
...
void OpenQuattIncidentManager::publish_incident_transition_(
    size_t slot, const oq_incidents::IncidentDefinition &definition,
  ...
  const uint8_t control_mode =
      this->control_mode_code_ != nullptr
          ? static_cast<uint8_t>(this->control_mode_code_->value())
          : 0U;
  ...
  if (!previous.confirmed_active && current.confirmed_active) {
    this->decision_log_->emit(
        openquatt_decision_log::EVENT_INCIDENT_START, event_subject_(slot),
        incident_reason_(definition), incident_severity_(definition),
        control_mode, openquatt_decision_log::STATE_IDLE,
        openquatt_decision_log::STATE_ACTIVE,
        static_cast<int16_t>(definition.id), raw_word,
        static_cast<int16_t>(definition.trip_reads), 0U,
        incident_flags_(definition, current));
    ...
    const bool blocking_latch =
        ...
        definition.clear_policy ==
            oq_incidents::ClearPolicy::AFTER_CONFIRMED_ODU_POWER_CYCLE;
    this->decision_log_->emit(
        openquatt_decision_log::EVENT_INCIDENT_CLEAR, event_subject_(slot),
        incident_reason_(definition), incident_severity_(definition),
        control_mode, openquatt_decision_log::STATE_ACTIVE,
        blocking_latch ? openquatt_decision_log::STATE_BLOCKED
                       : openquatt_decision_log::STATE_IDLE,
        static_cast<int16_t>(definition.id), raw_word,
        static_cast<int16_t>(definition.clear_reads),
        duration_seconds_(previous.first_seen_ms, now_ms),
        incident_flags_(definition, current));
  } else if (!previous.acknowledged && current.acknowledged) {
    this->decision_log_->emit(
        openquatt_decision_log::EVENT_INCIDENT_ACKNOWLEDGED,
        event_subject_(slot), incident_reason_(definition),
        incident_severity_(definition), control_mode,
        previous.confirmed_active ? openquatt_decision_log::STATE_ACTIVE
                                  : openquatt_decision_log::STATE_IDLE,
        current.confirmed_active ? openquatt_decision_log::STATE_ACTIVE
                                 : openquatt_decision_log::STATE_IDLE,
        static_cast<int16_t>(definition.id), raw_word, 0, 0U,
        incident_flags_(definition, current));
  }
}

void OpenQuattIncidentManager::publish_synthetic_incident_(
    UnitState &unit, size_t hp_slot, size_t synthetic_slot,
  ...
  if (next_active) {
    ...
    runtime.occurrence_count =
        runtime.occurrence_count == std::numeric_limits<uint32_t>::max()
            ? runtime.occurrence_count
  ...
  const oq_incidents::IncidentDefinition definition =
      synthetic_definition_(synthetic_slot);
  this->publish_synthetic_transition_(
      hp_slot, definition, reason_for_incident_id_(definition.id), previous,
      runtime, now_ms);
}
...
void OpenQuattIncidentManager::publish_synthetic_transition_(
  ...
  if (!previous.confirmed_active && current.confirmed_active) {
    this->decision_log_->emit(
        openquatt_decision_log::EVENT_INCIDENT_START,
        event_subject_(hp_slot), reason,
        openquatt_decision_log::SEVERITY_FAULT, control_mode,
        openquatt_decision_log::STATE_IDLE,
        openquatt_decision_log::STATE_ACTIVE,
        static_cast<int16_t>(definition.id), 0,
        static_cast<int16_t>(definition.trip_reads), 0U, flags);
  } else if (previous.confirmed_active && !current.confirmed_active) {
    this->decision_log_->emit(
        openquatt_decision_log::EVENT_INCIDENT_CLEAR,
        event_subject_(hp_slot), reason,
        openquatt_decision_log::SEVERITY_FAULT, control_mode,
        openquatt_decision_log::STATE_ACTIVE,
        openquatt_decision_log::STATE_IDLE,
...
void OpenQuattIncidentManager::publish_transitions_(
    UnitState &unit, size_t slot, uint32_t now_ms) {
  this->reconcile_manual_reset_persistence_(now_ms);
  ...
  if (unit.restored_manual_reset_pending &&
    ...
    const oq_incidents::IncidentDefinition definition =
        oq_incidents::definition_for(2120U, 4U);
    const oq_incidents::IncidentRuntime runtime =
        unit.engine.incident(definition.id);
    const uint8_t control_mode =
        this->control_mode_code_ != nullptr
            ? static_cast<uint8_t>(this->control_mode_code_->value())
            : 0U;
  ...
  for (size_t incident_slot = 0U;
    ...
    this->publish_incident_transition_(
        slot, oq_incidents::definition_for_id(id),
        unit.previous_incidents[incident_slot], current, now_ms);
    unit.previous_incidents[incident_slot] = current;
  }

  const bool link_incident_active =
      unit.synthetic_incidents[0U].confirmed_active;
  const bool next_link_loss =
      link_incident_active
          ? current_outputs.link_state != oq_incidents::LinkState::HEALTHY
          : current_outputs.link_state == oq_incidents::LinkState::LOST;
  this->publish_synthetic_incident_(
      unit, slot, 0U, next_link_loss, now_ms);
  this->publish_synthetic_incident_(
      unit, slot, 1U, current_outputs.start_timed_out, now_ms);
  this->publish_synthetic_incident_(
  ...
  if (this->decision_log_ != nullptr) {
    const uint8_t control_mode =
        this->control_mode_code_ != nullptr
            ? static_cast<uint8_t>(this->control_mode_code_->value())
            : 0U;
    if (should_emit_start_confirmation(
            unit.start_feedback_armed,
            unit.previous_outputs.running_confirmed,
            current_outputs.running_confirmed)) {
      this->decision_log_->emit(
          openquatt_decision_log::EVENT_HP_START_CONFIRMED,
          event_subject_(slot),
  ...
  if (!unit.availability_reporting_initialized) {
    ...
    if (this->decision_log_ != nullptr) {
      ...
      this->decision_log_->emit(
          openquatt_decision_log::EVENT_HP_AVAILABILITY_CHANGE,
          event_subject_(slot), reason,
          availability == openquatt_decision_log::STATE_FAULTED ||
                  availability == openquatt_decision_log::STATE_OFFLINE
          ...
          this->control_mode_code_ != nullptr
              ...
              : 0U,
          unit.last_reported_availability, availability,
          static_cast<int16_t>(current_outputs.primary_incident_id),
...
bool OpenQuattIncidentManager::hp_configured(uint8_t hp_index) const {
  return this->unit_(hp_index) != nullptr;
}

uint8_t OpenQuattIncidentManager::configured_hp_count() const {
  return OQ_TOPOLOGY_DUO != 0 ? 2U : 1U;
}

uint8_t OpenQuattIncidentManager::available_hp_count() const {
  uint8_t count = 0U;
  for (uint8_t hp_index = 1U; hp_index <= this->configured_hp_count();
       ++hp_index) {
...
bool OpenQuattIncidentManager::availability_complete() const {
  ...
  if (!this->manual_reset_persistence_.ready() ||
      (this->manual_reset_persistence_.fault_mask() &
       configured_mask) != 0U) {
    return false;
  }
  for (uint8_t hp_index = 1U; hp_index <= this->configured_hp_count();
       ++hp_index) {
    const oq_incidents::LinkState state =
        this->get_outputs(hp_index).link_state;
    if (state == oq_incidents::LinkState::BOOTSTRAP ||
        state == oq_incidents::LinkState::SUSPECT ||
...
bool OpenQuattIncidentManager::all_unavailable_hps_allow_fallback() const {
  ...
  return openquatt_incident_manager::all_unavailable_hps_allow_fallback(
      outputs.data(), configured_count);
}
...
void OpenQuattIncidentManager::set_fallback_status(bool requested, bool active,
...
void OpenQuattIncidentManager::set_boiler_status(uint8_t role,
                                                 bool command_active,
                                                 bool output_continuous) {
...
void OpenQuattIncidentManager::setup_manual_reset_persistence_(
    uint32_t now_ms) {
  const uint8_t configured_mask =
      this->configured_hp_count() == 2U
          ? oq_incidents::kManualResetAllHpMask
  ...
  if (global_preferences != nullptr) {
    this->manual_reset_pref_a_ =
        global_preferences
            ->make_preference<oq_incidents::ManualResetLatchStorage>(
                MANUAL_RESET_STORAGE_A_KEY, true);
    this->manual_reset_pref_b_ =
        global_preferences
            ->make_preference<oq_incidents::ManualResetLatchStorage>(
                MANUAL_RESET_STORAGE_B_KEY, true);
    this->manual_reset_marker_pref_ =
        global_preferences
            ->make_preference<oq_incidents::ManualResetLatchMarker>(
                MANUAL_RESET_MARKER_KEY, true);
  ...
  const auto load_result = this->manual_reset_persistence_.load(
      storage_a_loaded, storage, storage_b_loaded, storage_b,
...
void OpenQuattIncidentManager::reconcile_manual_reset_persistence_(
    uint32_t now_ms) {
  const uint8_t persisted_mask =
      this->manual_reset_persistence_.persisted_mask();
  const oq_incidents::IncidentId power_cycle_incident =
      oq_incidents::incident_id(2120U, 4U);
  for (size_t slot = 0U; slot < this->units_().size(); ++slot) {
  ...
  this->manual_reset_persistence_.observe_runtime_latches(
      this->runtime_manual_reset_mask_());
  if (!this->manual_reset_persistence_.should_attempt_persist(
          now_ms, MANUAL_RESET_PERSIST_RETRY_MS)) {
    return;
  }

  const uint8_t target_mask =
      this->manual_reset_persistence_.persistence_target_mask();
  if (this->persist_manual_reset_mask_(target_mask)) {
    this->manual_reset_persistence_.mark_persist_success(target_mask,
                                                         now_ms);
    return;
  }
...
bool OpenQuattIncidentManager::persist_manual_reset_mask_(
    uint8_t latch_mask) {
  if (global_preferences == nullptr) return false;
  const oq_incidents::ManualResetLatchStorage storage{
      oq_incidents::kManualResetStorageMagic,
      oq_incidents::kManualResetStorageVersion,
      static_cast<uint8_t>(latch_mask &
                           oq_incidents::kManualResetAllHpMask),
      0U};
  const oq_incidents::ManualResetLatchMarker marker{
      oq_incidents::kManualResetMarkerMagic,
      oq_incidents::kManualResetStorageVersion,
      0U};
  ...
  const bool storage_a_queued =
      this->manual_reset_pref_a_.save(&storage);
  const bool storage_b_queued =
      this->manual_reset_pref_b_.save(&storage);
  const bool marker_queued =
      this->manual_reset_marker_pref_.save(&marker);
  ...
  const bool verify_a_loaded =
      this->manual_reset_pref_a_.load(&verify_a);
  const bool verify_b_loaded =
      this->manual_reset_pref_b_.load(&verify_b);
  const bool verify_marker_loaded =
      this->manual_reset_marker_pref_.load(&verify_marker);
  const bool verified = oq_incidents::redundant_manual_reset_state_matches(
      verify_a_loaded, verify_a, verify_b_loaded, verify_b,
      verify_marker_loaded, verify_marker, storage.latch_mask);
  if (verified && (!storage_a_queued || !storage_b_queued ||
                   !marker_queued || !sync_ok)) {
    ESP_LOGW(TAG,
...
void OpenQuattIncidentManager::write_snapshot(httpd_req_t *req) const {
  ...
  const uint32_t generated_epoch_s =
      snapshot.generated_at_epoch_s != 0U
          ? snapshot.generated_at_epoch_s
          : this->current_epoch_s_();
  bool ok =
      ...
      write_json_string(
          req, system_action_name(snapshot.control_mode,
                                  snapshot.fallback_requested,
                                  snapshot.fallback_active,
                                  snapshot.fallback_block_reason,
                                  snapshot.boiler_command_active)) &&
      write_raw(req, R"(,"boiler_role":)") &&
      write_json_string(req, boiler_role_name(snapshot.boiler_role)) &&
      write_raw(req, R"(,"previous_boiler_role":)") &&
      write_json_string(req, boiler_role_name(snapshot.previous_boiler_role)) &&
      write_raw(req, R"(,"boiler_command_active":)") &&
      write_bool(req, snapshot.boiler_command_active) &&
      write_raw(req, R"(,"boiler_output_continuous":)") &&
      write_bool(req, snapshot.boiler_output_continuous) &&
      write_raw(req, R"(,"fallback_block_reason":)") &&
      write_uint(req, snapshot.fallback_block_reason) &&
  ...
  for (size_t slot = 0U;
    ...
    ok = (first_hp || write_raw(req, ",")) &&
         write_raw(req, R"({"index":)") && write_uint(req, slot + 1U) &&
         write_raw(req, R"(,"link_state":)") &&
         write_json_string(req, link_state_name(outputs.link_state)) &&
         write_raw(req, R"(,"protection_state":)") &&
         write_json_string(req,
                           protection_state_name(outputs.protection_state)) &&
         write_raw(req, R"(,"run_state":)") &&
         write_json_string(req, run_state_name(outputs.run_state)) &&
         write_raw(req, R"(,"availability":)") &&
         write_json_string(req, availability_name(outputs)) &&
         write_raw(req, R"(,"available_for_start":)") &&
         write_bool(req, outputs.available_for_start) &&
         write_raw(req, R"(,"must_stop":)") &&
         write_bool(req, outputs.must_stop) &&
         write_raw(req, R"(,"fault_active":)") &&
         write_bool(req, outputs.fault_active) &&
         write_raw(req, R"(,"protection_active":)") &&
         write_bool(req, outputs.protection_active) &&
         write_raw(req, R"(,"running_confirmed":)") &&
         write_bool(req, outputs.running_confirmed) &&
         write_raw(req, R"(,"stop_confirmed":)") &&
         write_bool(req, outputs.stop_confirmed) &&
         write_raw(req, R"(,"stop_unconfirmed":)") &&
         write_bool(req, outputs.stop_unconfirmed) &&
         write_raw(req, R"(,"fallback_cause_present":)") &&
         write_bool(req, outputs.fallback_cause_present) &&
         write_raw(req, R"(,"fallback_eligible":)") &&
         write_bool(req, outputs.fallback_eligible) &&
         write_raw(req, R"(,"primary_incident_id":)") &&
         write_uint(req, outputs.primary_incident_id) &&
         write_raw(req, R"(,"last_action_result":)");
    if (ok && snapshot.units[slot].last_action_seq == 0U) {
      ok = write_raw(req, "null");
    } else if (ok) {
      ok = write_raw(req, R"({"sequence":)") &&
           write_uint(req, snapshot.units[slot].last_action_seq) &&
           write_raw(req, R"(,"request_id":)") &&
           write_uint(
               req, snapshot.units[slot].last_action_request_id) &&
           write_raw(req, R"(,"action":)") &&
           write_json_string(req, snapshot.units[slot].last_action) &&
           write_raw(req, R"(,"ok":)") &&
           write_bool(req, snapshot.units[slot].last_action_ok) &&
           write_raw(req, R"(,"result":)") &&
           write_json_string(
               req, snapshot.units[slot].last_action_result) &&
    ...
    const auto write_incident =
      ...
      const uint32_t first_seen_s = epoch_for_runtime_ms_(
          generated_epoch_s, snapshot.generated_at_ms,
          runtime.first_seen_ms);
      const uint32_t last_seen_s = epoch_for_runtime_ms_(
          generated_epoch_s, snapshot.generated_at_ms,
          runtime.last_seen_ms);
      const uint32_t cleared_at_s = epoch_for_runtime_ms_(
          generated_epoch_s, snapshot.generated_at_ms,
          runtime.cleared_at_ms);
      const bool item_ok =
          (first_incident || write_raw(req, ",")) &&
          write_raw(req, R"({"definition":{"id":)") &&
          write_uint(req, definition.id) &&
          write_raw(req, R"(,"key":)") &&
          write_json_string(req, definition.key) &&
          write_raw(req, R"(,"presentation_key":)") &&
          write_json_string(req, definition.presentation_key) &&
          write_raw(req, R"(,"category":)") &&
          write_json_string(req, category_name(definition.category)) &&
          write_raw(req, R"(,"severity":)") &&
          write_json_string(req, severity_name(definition.severity)) &&
          write_raw(req, R"(,"effects":)") &&
          write_effects(req, definition.effects) &&
          write_raw(req, R"(,"effect_mask":)") &&
          write_uint(req, definition.effects) &&
          write_raw(req, R"(,"user_action":)") &&
          write_json_string(req, user_action_name(definition.user_action)) &&
          write_raw(req, R"(,"recovery_condition":)") &&
          write_json_string(
              req, recovery_condition_name(definition.recovery_condition)) &&
          write_raw(req, R"(,"register_address":)") &&
          write_uint(req, definition.register_address) &&
          write_raw(req, R"(,"bit":)") &&
          write_uint(req, definition.bit) &&
          write_raw(req, R"(},"runtime":{"lifecycle":)") &&
          write_json_string(
              req, incident_lifecycle_name(definition, runtime)) &&
          write_raw(req, R"(,"raw_active":)") &&
          write_bool(req, runtime.raw_active) &&
          write_raw(req, R"(,"confirmed_active":)") &&
          write_bool(req, runtime.confirmed_active) &&
          write_raw(req, R"(,"latched":)") &&
          write_bool(req, runtime.latched) &&
          write_raw(req, R"(,"acknowledged":)") &&
          write_bool(req, runtime.acknowledged) &&
          write_raw(req, R"(,"first_seen_ms":)") &&
          write_uint(req, runtime.first_seen_ms) &&
          write_raw(req, R"(,"last_seen_ms":)") &&
          write_uint(req, runtime.last_seen_ms) &&
          write_raw(req, R"(,"cleared_at_ms":)") &&
          write_uint(req, runtime.cleared_at_ms) &&
          write_raw(req, R"(,"first_seen_s":)") &&
          write_uint(req, first_seen_s) &&
          write_raw(req, R"(,"last_seen_s":)") &&
          write_uint(req, last_seen_s) &&
          write_raw(req, R"(,"cleared_at_s":)") &&
          write_uint(req, cleared_at_s) &&
          write_raw(req, R"(,"occurrence_count":)") &&
          write_uint(req, runtime.occurrence_count) &&
          write_raw(req, "}}");
    ...
    for (size_t incident_slot = 0U;
         ok && incident_slot < oq_incidents::kRawIncidentSlotCount;
         ++incident_slot) {
      const oq_incidents::IncidentRuntime &runtime =
          snapshot.units[slot].incidents[incident_slot];
      const oq_incidents::IncidentDefinition definition =
          oq_incidents::definition_for_id(
              static_cast<oq_incidents::IncidentId>(incident_slot + 1U));
    ...
    for (size_t synthetic_slot = 0U;
         ok && synthetic_slot < SYNTHETIC_INCIDENT_COUNT;
         ++synthetic_slot) {
      ok = write_incident(
          synthetic_definition_(synthetic_slot),
