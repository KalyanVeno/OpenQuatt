...
inline Decision decide(const Inputs &inputs) {
  ...
  if (inputs.requested_level > 0 &&
      inputs.previous_applied_level <= 0 &&
      !inputs.available_for_start) {
    return Decision{Action::BLOCK_NEW_START, 0, false};
  }
  return Decision{Action::FOLLOW_REQUEST, inputs.requested_level, false};
}

inline bool safe_stop_write_retry_due(bool stop_confirmation_pending,
                                      uint32_t now_ms,
                                      uint32_t last_write_ms,
                                      uint32_t retry_interval_ms) {
  if (!stop_confirmation_pending) return false;
  if (last_write_ms == 0U) return true;
  return static_cast<uint32_t>(now_ms - last_write_ms) >=
         retry_interval_ms;
}
...
inline bool apply_start_gate_before_active_write(bool start_required,
...
inline void apply_stop_notification_before_safe_write(
    bool stop_notification_required,
    StopNotify stop_notify,
    SafeWrite safe_write) {
