...
inline bool is_assist_fallback_handover(BoilerRole previous_role, BoilerRole next_role) {
  return (previous_role == BoilerRole::ASSIST_CM3 && next_role == BoilerRole::FALLBACK_CM4) ||
         (previous_role == BoilerRole::FALLBACK_CM4 && next_role == BoilerRole::ASSIST_CM3);
}
...
inline BoilerOutputTransition plan_output_transition(BoilerRole previous_role, bool previous_heat_enable,
  ...
  transition.output_action = previous_heat_enable == next_heat_enable
                                 ? BoilerOutputAction::KEEP
                                 : (next_heat_enable ? BoilerOutputAction::ENABLE : BoilerOutputAction::DISABLE);
  transition.assist_fallback_handover_without_output_edge =
      previous_heat_enable && next_heat_enable && is_assist_fallback_handover(previous_role, next_command.role);
  return transition;
}
...
class BoilerOutputController {
 public:
  BoilerOutputTransition advance(const BoilerOutputCommand& command) {
    const BoilerOutputTransition transition = plan_output_transition(role_, heat_enable_, command);
