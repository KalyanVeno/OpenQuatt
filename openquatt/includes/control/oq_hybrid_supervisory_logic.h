...
inline bool fallback_availability_is_confirmed(
    ...
    bool every_unavailable_hp_has_fallback_cause,
    bool all_hp_outputs_safe) {
  ...
  return raw_availability_complete ||
         (every_unavailable_hp_has_fallback_cause && all_hp_outputs_safe);
}
...
class Cm4ResumeTracker {
 public:
  ...
  int resume_mode() const {
    return origin_valid_ && resume_mode_ == 3 ? 3 : 2;
  }

  void finish_after_decision(bool heating_demand,
                             bool competing_owner,
                             bool fallback_requested,
                             bool hp_available) {
    if (!heating_demand || competing_owner ||
        (!fallback_requested && hp_available)) {
      origin_valid_ = false;
      resume_mode_ = 2;
    }
...
inline int recovered_heating_mode(int stored_resume_mode,
                                  bool hp_available,
                                  bool power_house_active,
                                  bool boiler_assist_enabled) {
  if (!hp_available) return 1;
  return stored_resume_mode == 3 &&
                 power_house_active &&
...
inline FallbackEvaluation evaluate_fallback(
    const FallbackEvaluationInputs &inputs) {
  FallbackEvaluation evaluation;
  evaluation.availability_complete = fallback_availability_is_confirmed(
      inputs.raw_availability_complete,
      inputs.every_unavailable_hp_has_fallback_cause,
      inputs.all_hp_outputs_safe);
  evaluation.no_hp_available_confirmed =
      inputs.available_hp_count == 0 && evaluation.availability_complete;
  evaluation.fallback_requested =
      inputs.heating_demand &&
  ...
  fallback_inputs.hp_availability_complete =
      evaluation.availability_complete;
  fallback_inputs.confirmed_fallback_cause =
      inputs.every_unavailable_hp_has_fallback_cause;
  ...
  fallback_inputs.supply_temperature_valid =
      inputs.supply_temperature_valid;
...
inline HeatingModeDecision decide_heating_mode(
    const HeatingModeInputs &inputs) {
  ...
  if (inputs.base_target == 4) {
    if (inputs.current_mode == 4) {
      return HeatingModeDecision{
          4, -1, false, "boiler fallback already active"};
    }
    ...
    return HeatingModeDecision{
        1, 4, false, "boiler fallback waiting for CM1 preflow"};
  }
  ...
  if (inputs.base_target == 2) {
    ...
    if (inputs.current_mode == 4) {
      const int recovered_mode = recovered_heating_mode(
          inputs.cm4_resume_mode,
          inputs.hp_available,
          inputs.power_house_active,
          inputs.boiler_assist_enabled);
      return HeatingModeDecision{
          recovered_mode,
          -1,
          false,
          recovered_mode == 3
              ? "CM4 -> CM3 continuous boiler handback"
              : "CM4 -> CM2 HP recovery handback",
      };
    }
    return HeatingModeDecision{
        1, 2, false, "heating request waiting for CM1 hold"};
  }
