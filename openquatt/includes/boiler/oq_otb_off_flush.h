...
class OffFlushState {
 public:
  ...
  void record_request(uint8_t message_id, uint8_t message_type, uint8_t value_hb, uint8_t value_lb) {
    ...
    if (message_id == OFF_FLUSH_ID_STATUS && message_type == OFF_FLUSH_TYPE_READ_DATA && (value_hb & 0x01U) == 0U) {
      this->status_off_sent_ = true;
    } else if (message_id == OFF_FLUSH_ID_CH_SETPOINT && message_type == OFF_FLUSH_TYPE_WRITE_DATA && value_hb == 0U &&
               value_lb == 0U) {
  ...
  void record_response(uint8_t message_id, uint8_t message_type, uint8_t value_hb, uint8_t value_lb) {
    ...
    if (message_id == OFF_FLUSH_ID_STATUS && message_type == OFF_FLUSH_TYPE_READ_ACK && this->status_off_sent_) {
      this->status_off_acknowledged_ = true;
    } else if (message_id == OFF_FLUSH_ID_CH_SETPOINT && message_type == OFF_FLUSH_TYPE_WRITE_ACK && value_hb == 0U &&
               value_lb == 0U && this->setpoint_zero_sent_) {
      this->setpoint_zero_acknowledged_ = true;
    }
  }

  bool active() const { return this->active_; }
  bool complete() const { return this->status_off_acknowledged_ && this->setpoint_zero_acknowledged_; }
  bool status_off_acknowledged() const { return this->status_off_acknowledged_; }
  bool setpoint_zero_acknowledged() const { return this->setpoint_zero_acknowledged_; }
