...
inline bool is_cm100(int control_mode_code) { return control_mode_code == 100; }

inline bool task_selected(int task_code, TaskCode expected) { return task_code == static_cast<int>(expected); }
...
inline bool stop_routes_to_autotune_abort(bool commissioning_active, int task_code, bool flow_autotune_req) {
  return task_active(commissioning_active, task_code, TASK_FLOW_AUTOTUNE) ||
         task_selected(task_code, TASK_FLOW_AUTOTUNE) || flow_autotune_req;
}

inline bool stop_routes_to_commissioning_abort(bool commissioning_active, int task_code) {
  return task_running(commissioning_active, task_code) && !task_selected(task_code, TASK_FLOW_AUTOTUNE);
}

inline bool flow_autotune_mode_valid(int control_mode_code, int task_code) {
  return is_cm100(control_mode_code) && task_selected(task_code, TASK_FLOW_AUTOTUNE);
}

inline bool air_purge_mode_valid(int control_mode_code, int task_code) {
  return is_cm100(control_mode_code) && task_selected(task_code, TASK_AIR_PURGE);
}

inline bool hp_water_calibration_mode_valid(int control_mode_code, int task_code) {
  return is_cm100(control_mode_code) && task_selected(task_code, TASK_HP_WATER_CALIBRATION);
}
