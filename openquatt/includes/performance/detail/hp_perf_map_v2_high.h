...
static constexpr float T_amb_bp_w65[13] = {
    -15.00f,    -5.000001f, -5.00f,     -0.000001f, 0.00f,      4.999999f, 5.00f,
    10.999999f, 11.00f,     13.999999f, 14.00f,     18.999999f, 19.00f,
};
...
static inline float interp_ambient_level(const float (&data)[N][N_LVL], const float (&amb_bp)[N], int level,
                                         float Tamb) {
...
static inline float interp_power_th_w(int level, float Tamb, float Tsup) {
  switch (select_band(Tsup)) {
    case 55:
      return interp_ambient_level(P_TH_W55, T_amb_bp_w55, level, Tamb);
    case 65:
      return interp_ambient_level(P_TH_W65, T_amb_bp_w65, level, Tamb);
    default:
      return interp_ambient_level(P_TH_W70, T_amb_bp_w70, level, Tamb);
  }
}
...
static inline float interp_cop(int level, float Tamb, float Tsup) {
  switch (select_band(Tsup)) {
    case 55:
      return interp_ambient_level(COP_W55, T_amb_bp_w55, level, Tamb);
    case 65:
      return interp_ambient_level(COP_W65, T_amb_bp_w65, level, Tamb);
    default:
      return interp_ambient_level(COP_W70, T_amb_bp_w70, level, Tamb);
  }
}
