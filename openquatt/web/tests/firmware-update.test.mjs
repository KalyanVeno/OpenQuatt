...
test("a new OTA attempt replaces stale reboot progress before rendering", () => {
  state.entities = {
    firmwareUpdate: {
  ...
  assert.equal(state.entities.firmwareUpdateStatus.state, "Starting");
  assert.equal(state.entities.firmwareUpdateProgress.state, 0);
