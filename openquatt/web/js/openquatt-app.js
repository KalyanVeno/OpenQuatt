...
`)),r.toString()}function hl(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=my(l);if(!c)return;let u=[...a,c],d=yp(u,t);if(a.length&&d.length>ry){r.push({keys:n,body:i}),n=[l],a=[c],i=yp(a,t);return}n=[...n,l],a=u,i=d}),a.length&&r.push({keys:n,body:i}),r}async function gy(e){let t=gl(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await Sn(oy,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function fy(){let e=gl(),r=await Sn(ny,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function hy(){let e=gl(),r=await Sn(ay,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function by(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.hour_start_epoch_s??"",i.source??"",i.attention_count??"",e?.meta?.flash_write_count??""].join("|")}async function wy(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<iy)return!1;o.decisionLogFetchPromise=(async()=>{let n=await hy(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=by(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function yy(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{fn.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=Ap(r,o.entities[r],n))})}async function vy(e){if(!e.length)return{ok:!0,message:""};try{let t=await fy();return yy(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function K(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||fn.has(b)||!dy(b,n));if(!i.length)return;let l=i.filter(b=>fn.has(b)),c=i.filter(b=>!fn.has(b)),d=(await vy(l)).ok?c:[...c,...l];if(!d.length){vp(),An(Jt()),o.busyAction||(wp(),o.controlError="");return}let g=Number(r.concurrency),w=Number.isFinite(g)&&g>0?Math.floor(g):Kt,y=hl(d,t),f=Math.max(1,Math.min(w,Kt)),m=[];for(let b=0;b<y.length;b+=f){let S=y.slice(b,b+f),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await gy(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let p="";m.forEach(b=>{if(b.status!=="fulfilled"){let M=b.reason.message||String(b.reason);p||(p=M);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(M=>{if(Object.prototype.hasOwnProperty.call(E,M)){o.optionalMissingEntities&&delete o.optionalMissingEntities[M],o.entities[M]=Ap(M,o.entities[M],E[M]);return}let P=L[M];P?.optional?k.has(M)&&py(M,n):p||(p=`${P?.name||M} ontbreekt in bulk response`)})}),vp(),An(Jt()),p?($p(p),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${p}`):o.busyAction||(wp(),o.controlError="")}function Ap(e,t={},r={}){let n={...t||{},...r||{}};return L[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function vp(){o.complete=Na(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=sd(),o.appView==="diagnosis"&&!bt()&&Ht(yo(),{syncMode:"replace",forceSync:!0}),o.appView||Ht(Mn()||yo(),{syncMode:"replace",forceSync:!0})}function Sy(){let e=cl();return o.appView==="settings"?[...new Set([...e,...ul()])]:o.appView==="energy"||o.appView==="results"?Hn(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?Hn(o.appView,{forceFast:!0}):[...new Set(e)]}function qy(e=[]){let t=new Set(e);return(o.appView==="settings"?ul():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...Hn(o.appView,{includeBulk:!0}),...Ut])]:o.appView==="energy"||o.appView==="results"?[...new Set([...Hn(o.appView,{forceFast:!0}),...Ut])]:[...new Set(["setupComplete","strategy",...ot])]).filter(n=>!t.has(n))}function Sp(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=Vu}async function we(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||ja()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){Yw(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<wn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&Sp(t,e),d=t-Number(o.lastStaticEntitySyncAt||0)>=Bu||o.updateInstallBusy||o.updateInstallPhaseHint?Ut:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?ht:[],w=o.quickStartModalOpen&&o.currentStep==="setup"?Gt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Lo:[],f=Js()?Xt():[],m=n?[...fs,...ot,"setupComplete",...d]:i?[...Hn(a,{forceFast:l,includeBulk:c}),...d]:r==="settings"?[...new Set([...ul(),...f,...d])]:c?["setupComplete",...d,...ot,"strategy",...Oo,...tt,...ae()?fe.map(p=>p.key):xt]:["setupComplete",...ot,"strategy",...d];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let p=o.deviceReconnectMode,b=cy(t,e)?await uy():{ok:!0,message:""};if(!b.ok){$p(b.message),n||h();return}if(await K([...new Set([...m,...w,...g,...y])],n?"state":r==="settings"||w.length?"all":"state",{concurrency:l&&i?zt:Kt}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),d.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&kp();let S=p!==o.deviceReconnectMode,q=l&&i,E=q?!1:a==="overview"||a==="control"||a==="diagnosis"?await Xs():!1,k=q?!1:o.appView==="results"?await Va():!1,M=a==="control"?await wy({force:e.forceDecisionLog===!0}):!1,P=q?!1:Js()?(await Promise.all([Zd(),Va({metaOnly:!0})])).some(Boolean):!1,N=q||!vd()?!1:await Ko(),V=q||!Sd()?!1:await zo(),H=q||!pp()?!1:await Yo({force:o.systemModal==="mqtt-sensors"}),F=Ud();if(q&&!o.nativeOpen&&xp(dl(a)),S){h();return}if(E&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){h();return}if(k&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){h();return}if(M&&o.appView==="control"){el()||h();return}if(P&&o.appView==="settings"){h();return}if(N&&(o.systemModal==="login"||Yt())){h();return}if(V&&o.appView==="settings"){h();return}if(o.systemModal==="mqtt-sensors"){Ka()!==o.mqttSensorsModalRenderSignature&&h();return}if(H&&o.appView==="settings"){h();return}let A=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(F!==o.headerRenderSignature)if(A&&Gs())o.headerRenderSignature=F;else{h();return}else Gs();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){En()!==o.settingsRenderSignature&&h();return}if(A)return;if(o.appView==="settings"){let O=En();if(O!==o.settingsRenderSignature&&!o.focusedField){h();return}if(!Qd()){o.focusedField||h();return}o.focusedField&&(o.settingsRenderSignature=O);return}if(o.appView==="diagnosis"){op()||h();return}if(o.appView==="control"){el()||h();return}if(o.appView==="energy"){Xd()||h();return}if(o.appView==="results"){ep()||h();return}tp()||h()}catch(p){n||(o.controlError=`Helperstatus kon niet worden geladen. ${p.message}`,h())}finally{o.entitySyncInFlight=!1;let p=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,p&&!o.nativeOpen&&window.setTimeout(()=>{we(p)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!p&&Sp(Date.now())&&window.setTimeout(()=>{we({forceBulk:!0})},Gw)}}function Pt(){return{}}function ky(){let e=String(T("hpGeneration")||"").trim();return e||""}function Jo(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function Ey(){if(!Array.isArray(Cr))return"";if(Cr.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return Cr.every(t=>e[t])?"single":""}function bl(e){let t=Jo(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Ty(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function oe(){let e=Jo(T("installationTopology"));if(e==="single"||e==="duo")return bl(e);let t=Jo(Pt().installation);if(t==="single"||t==="duo")return bl(t);let r=Ey();return r?bl(r):Ty()}function Nt(){let e=oe(),t=ky();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Ge(){return"OpenQuatt"}function Rn(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function nt(e=at()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Xo(e=oe()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function Pr(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Pt().hardwareProfile||e).trim().toLowerCase()}function at(){return Rn(T("connectionText")||Pt().connection)}function Hp(){let e=at();return e==="wifi"?"eth":e==="eth"?"wifi":""}function Rp(){let e=oe();return e==="single"?"duo":e==="duo"?"single":""}function qo(e=oe(),t=at()){let r=Xo(e),n=Pr();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${nt(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${nt(t)}`:n==="waveshare"?`Waveshare ${r} ${nt(t)}`:`${Ge()} ${r} ${nt(t)}`}function wl(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function za(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${wl()}`}function Cy(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function xy(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Nr(){let e=xy(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Cy(o.entities.uptime);if(n==="d")return Bo(e*1440);if(n==="h")return Bo(e*60);if(n==="s")return Bo(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(Pt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Bo((Date.now()-r)/6e4)}function _r(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(Pt().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function ko({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let y=e();if(!y)return null;let f={scrollTop:y.scrollTop};if((i||l)&&(f.scrollHeight=y.scrollHeight),l){let m=y.scrollHeight-y.scrollTop-y.clientHeight;f.stickToBottom=m<c}return a&&(f.identity=a(y)),f}function d(y){if(!y)return;let f=e();if(!f||a&&a(f)!==y.identity)return;if(y.stickToBottom){f.scrollTop=f.scrollHeight;return}let m=i?f.scrollHeight-Number(y.scrollHeight||0):0;f.scrollTop=Math.max(0,Number(y.scrollTop||0)+m)}function g(y,f=!0){if(!y)return;let m=Number(t()||0)+1;r(m);let p=()=>{Number(t()||0)!==m||!n()||d(y)};if(f){window.requestAnimationFrame(p);return}p()}function w(){r(Number(t()||0)+1)}return{capture:u,invalidate:w,queue:g,restore:d}}function $e(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function Eo({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function or({active:e,startKey:t,stopKey:r,startLabel:n,stopLabel:a,startClass:i="oq-helper-button oq-helper-button--primary",stopClass:l="oq-helper-button oq-helper-button--ghost",startDisabled:c=!1,stopDisabled:u=!1}){return Qe(e?r:t,e?a:n,e?l:i,e?u:c)}function _p(e,t,r,n,a=""){if(!v(e))return"";let i=o.entities[e]||{},l=String(T(e)||""),c=to(i),u=o.loadingEntities||o.busyAction===`save-${e}`,d=`
    <div class="oq-settings-choice-grid">
      ${c.map(g=>{let w=n[g]||"",y=typeof w=="string"?w:w.copy||"",f=typeof w=="string"?"":w.image||"",m=typeof w=="string"?"":w.alt||"";return Dr({key:e,option:g,currentValue:l,busy:u,copy:y,image:f,imageAlt:m})}).join("")}
    </div>
  ...
  `}function Sl(e,t,r,n=""){if(!v(e))return"";let a=yn(T(e));return ye(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function re(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var Dp=250;function Op(){return[]}function Or(){return typeof window>"u"?!1:Op().length>0}function $y(){return`${Oe()}/events`}function Ay(){return`${Oe()}/openquatt/logs/recent`}function Qa(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Ya(){return o.nativeOpen?"Niet beschikbaar":Or()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function Hy(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function Lp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return Hy(t)}function Ry(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Py(){return o.nativeOpen?"Niet beschikbaar":Or()?Qa()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":Qa()?"Buffer aan":"Buffer uit"}function Ny(){return Qa()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function ql(){return o.entities?.debugLevel||null}function Ip(e=ql()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function _y(e=ql()){let t=String(e?.value??e?.state??"").trim(),r=Ip(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function Fp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function Dy(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function jp(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(Dy);let n=[];for(let a of r){let i=n[n.length-1]||null;Fp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-Dp)}function kl(e,t={}){let r=ov(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:rv(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function Oy(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function Ly(){let e=Op(),t=e.length;return e.map((r,n)=>kl(r,{receivedAt:Oy(n,t),seq:n+1}))}function Iy(){let e=Al();e&&(e.scrollTop=e.scrollHeight)}var El=ko({getScroller:Al,getToken:()=>o.webServerLogScrollRestoreToken,setToken:e=>{o.webServerLogScrollRestoreToken=e},isActive:()=>o.systemModal==="webserver-logs",preserveGrowth:!0,stickToBottom:!0}),ro=El.capture,no=El.queue;function Fy(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}var Wp=ko({getScroller:Fy,getToken:()=>o.cm100CommissioningScrollRestoreToken,setToken:e=>{o.cm100CommissioningScrollRestoreToken=e},isActive:()=>o.systemModal==="cm100-commissioning",preserveGrowth:!0,stickToBottom:!0}),Vp=Wp.capture,Tl=Wp.queue;function jy(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}var Bp=ko({getScroller:jy,getToken:()=>o.serviceTaskModalScrollRestoreToken,setToken:e=>{o.serviceTaskModalScrollRestoreToken=e},isActive:()=>String(o.systemModal||"").startsWith("service-task-")}),Kp=Bp.capture,Cl=Bp.queue;function Wy(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}var zp=ko({getScroller:Wy,getToken:()=>o.historyStorageModalScrollRestoreToken,setToken:e=>{o.historyStorageModalScrollRestoreToken=e},isActive:()=>o.systemModal==="history-storage"}),Up=zp.capture,xl=zp.queue;function Vy(){return o.root?o.root.querySelector("[data-oq-settings-backup-restore-scroller]"):null}var Gp=ko({getScroller:Vy,getToken:()=>o.settingsBackupRestoreScrollRestoreToken,setToken:e=>{o.settingsBackupRestoreScrollRestoreToken=e},isActive:()=>o.systemModal==="settings-backup-restore"}),Qp=Gp.capture,Ml=Gp.queue;async function Lr(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||ro(),r=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=r,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let n=await window.fetch(Ay(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==r)return;let i=Ky(a);o.webServerLogHistoryLoaded=!0,i.length>0&&(jp(i,{prepend:!0}),o.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),o.webServerLogRecentAnchorAt=Date.now())}catch(n){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(h(),no(t))}}function By(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?kl(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function Ky(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>By(r,n+1)).filter(r=>r!==null)}function zy(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function Uy(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>Fp(e,r))}function Gy(){Or()&&o.webServerLogEntries.length===0&&wt({webServerLogEntries:Ly()}),wt({webServerLogCopyMessage:"",webServerLogCopyError:""}),o.systemModal="webserver-logs",h(),K(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=ro();h(),no(e)}),Iy(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&Lr()}function Ir(){wt({webServerLogEntries:[],webServerLogError:"",webServerLogHistoryError:"",webServerLogHistoryLoading:!1,webServerLogHistoryLoaded:!1,webServerLogCopyMessage:"",webServerLogCopyError:"",webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1,webServerLogRecentTail:[],webServerLogRecentAnchorAt:0}),El.invalidate(),o.systemModal==="webserver-logs"&&h()}function Qy(){let e=ro();_t(),wt({webServerLogEnabled:null,webServerLogConnected:!1}),Ir(),o.systemModal==="webserver-logs"&&Lr({scrollState:e})}function $l(){if(Or()){_t();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs")){_t();return}if(o.webServerLogEnabled===!1){_t();return}o.webServerLogSource||Yy()}function Yy(){if(Or()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",h();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",h();return}try{let e=new window.EventSource($y());o.webServerLogSource=e,e.addEventListener("open",Zy),e.addEventListener("ping",Jy),e.addEventListener("log",ev),e.onerror=Xy}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",_t(),h()}}function _t(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function Zy(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?ro():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",h(),no(e)}rp({clearOutput:Ir,closeStream:_t,resetRecoveryState:Qy});function Jy(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?ro():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",h(),no(e))}function Xy(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?ro():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",_t(),h(),no(e)}function ev(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=ro(),r=tv(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>kl(u)).filter(u=>!zy(u)&&!Uy(u));if(i.length===0)return;jp(i);let l=av(),c=Al();nv(l),iv(i,l),o.webServerLogEnabled=!0,c&&t&&no(t,!1)}function tv(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).replace(/\r/g,`
`).trimEnd()}catch{}return t}function ov(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function rv(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function nv(e){for(;o.webServerLogEntries.length>Dp;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function av(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function Al(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function iv(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",Yp(r))}}function Yp(e){let t=Lp(e.receivedAt),r=Ry(e.receivedAt);return`
  ...
  `}function sv(e=o.webServerLogEntries){return e.length?e.map(t=>Yp(t)).join(""):`
      <p class="oq-webserver-log-empty">Nog geen logregels ontvangen. Open de log en wacht op een nieuwe melding.</p>
    `}function lv(){let e=[];return o.webServerLogHistoryLoading&&e.push('<p class="oq-helper-modal-note">Recente firmwarelogs worden opgehaald...</p>'),o.webServerLogCopyMessage&&e.push(`
  ...
  `}function uv(){let e=ql();if(!e)return"";let t=Ip(e),r=_y(e),n=o.loadingEntities||o.busyAction==="save-debugLevel";return`
...
`)}async function pv(){let e=dv();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",h();return}try{if(!await ho(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&h()}var mv={"open-webserver-log-modal":()=>Gy(),"clear-webserver-log-output":()=>Ir(),"copy-webserver-log-output":()=>pv()};function Zp(e){return ie(mv,e)}function Jp(){let e=Or();return te({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
    ...
    `})}function gv(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Hl(e){return gv().includes(e)}function Ja(){let e=Pr(),t=oe(),r=at(),n=Hp();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:v("firmwareUpdateTarget")&&Hl("alternate connection")&&v("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:nt(r),targetLabel:nt(n),currentBuildLabel:qo(t,r),targetBuildLabel:qo(t,n)}}function Xa(){let e=Pr(),t=oe(),r=Rp(),n=at(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:v("firmwareUpdateTarget")&&Hl("alternate topology")&&v("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Xo(t),targetLabel:Xo(r),currentBuildLabel:qo(t,n),targetBuildLabel:qo(r,n)}}function ei(e,t){let r=Pr(),n=oe(),a=at(),i=Jo(e),l=Rn(t),c=i&&i!==n,u=l&&l!==a,d=c&&u?"alternate topology and connection":c?"alternate topology":u?"alternate connection":"current build",g=r==="heatpump_controller_q"&&["single","duo"].includes(n)&&["single","duo"].includes(i)&&["wifi","eth"].includes(a)&&["wifi","eth"].includes(l),w=v("firmwareUpdateTarget"),y=Hl(d),f=v("installFirmwareUpdateTarget");return{available:g,canSwitch:g&&d!=="current build"&&w&&y&&f,targetEntityAvailable:w,targetOptionAvailable:y,installActionAvailable:f,currentTopology:n,currentConnection:a,targetTopology:i,targetConnection:l,targetOption:d,currentBuildLabel:qo(n,a),targetBuildLabel:qo(i,l)}}function nr(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function jr(){let e=Pr(),t=oe(),r=at(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${nt(r)}`}}function Pn(e=nr(),t=jr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function Mo(){if(st())return"Controleren";let e=St();if(e)return e.phaseLabel;if(Nl())return"Bijgewerkt";if(rr())return"Bezig";if(ir())return"Beschikbaar";let t=om();if(t!==null&&t<=0)return"Actueel";let r=Pt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Dl()?"Actueel":pe()?"Nog niet gecontroleerd":"\u2014"}function pe(){return o.entities.firmwareUpdate||null}function ti(){let e=pe();return e?String(e.state??e.value??"").trim().toLowerCase():""}function fv(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Nn(){return fv().toLowerCase()}function tm(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function Rl(){let e=String(o.updateInstallTargetVersion||"").trim(),t=se();return!e||!t?!1:_n(t,e)>=0}function hv(e=pe()||{}){let t=ao(e),r=se(e);return!t||!r?!1:_n(r,t)>=0}function Pl(){return(Rl()||hv())&&!st()&&!Fr()&&!ir()}function Nl(){return(o.updateInstallCompleted||Pl())&&!st()&&!St()&&!ir()}function ar(){Me({updateInstallBusy:!1,updateInstallTargetVersion:"",updateInstallPhaseHint:"",updateInstallProgressHint:Number.NaN,updateInstallStatusPollObserved:!1,updateInstallMode:"",updateInstallTargetConnection:"",updateInstallTargetTopology:""}),Ev()}function Dt(){o.updateInstallPhaseHint="starting",o.updateInstallProgressHint=0,o.updateInstallStatusPollObserved=!1}function Ee(){Me({updateManualUploadFile:null,updateManualUploadFileName:"",updateManualUploadError:""})}function Ye(e={}){Me({...e.clearPr?{updateTestFirmwarePr:""}:{},updateTestFirmwareConfirmed:!1,updateTestFirmwareError:"",updateTestFirmwareBuild:null})}function bv(){let e=Nn(),t=tm(),r=e!=="rebooting"||!o.updateInstallBusy||o.updateInstallStatusPollObserved;if((e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting")&&r){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(o.updateInstallMode==="normal"&&Rl()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function Fr(){let e=Nn();return e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting"}function wv(){let e=Nn();return e==="error"?"De firmware-installatie op het device is mislukt. Controleer de netwerkverbinding en probeer opnieuw.":e==="aborted"?"De firmware-installatie is door het device afgebroken. Probeer de installatie opnieuw.":""}function St(){bv();let e=Nn(),t=e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting"&&(!o.updateInstallBusy||o.updateInstallStatusPollObserved),r=t?e:o.updateInstallPhaseHint,n=tm(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!Fr()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="retrying"?{phaseLabel:"Opnieuw proberen",percent:0,copy:"De eerste verbinding voor de firmwaredownload mislukte. OpenQuatt probeert het automatisch nog \xE9\xE9n keer."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Ge()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${nt(o.updateInstallTargetConnection)}-build wordt nu naar ${Ge()} verzonden.`:o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?`De ${qo(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Ge()} verzonden.`:`Firmware wordt nu naar ${Ge()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Ge()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${nt(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?`Opstellingswissel naar ${Xo(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Ge()}.`}}function ao(e=pe()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function se(e=pe()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function yv(e=pe()||{}){let t=se(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:_n(t,r)>0}function _l(e=pe()||{},t=xo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=ao(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function Xp(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function _n(e,t){let r=Xp(e),n=Xp(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function rr(){if(Pl())return!1;let e=ti();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function st(){let e=ti();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function ir(){let e=ti();if(!_l())return!1;let t=om();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Pt().updateAvailable===!0}function Dl(){let e=ti();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||yv()}function Ol(){let e=pe()||{},t=se(e)||"\u2014",r=_l(e)?ao(e):"",n=r?_n(r,t):null;return!st()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function om(){let{current:e,latest:t}=Ol();return e==="\u2014"||t==="\u2014"?null:_n(t,e)}function rm(e=xo()){let t=String(e||"").trim().toLowerCase();return ns[t]||ns.main}function vv(){let e=String((pe()||{}).release_url||"").trim(),t=rm();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function Sv(){return Ge()}function xo(){return String(T("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function qv(){return Ol().latest!=="\u2014"}function em(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function kv(e,t=pe()||{}){let r=em(e);return r?em(`${t.title||""} ${t.summary||""}`).includes(r):!0}function nm(e){return new Promise(t=>window.setTimeout(t,e))}function Wr(e=fo){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,Nd(),typeof _t=="function"&&_t(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&Ds()},e)}function Ev(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||Ds()}function am(){return o.deviceReconnectMode?te({modalId:"reconnect",titleId:"oq-reconnect-modal-title",kicker:"Systeem",title:fd(),modalClass:"oq-helper-modal--reconnect",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `}):""}function Dn(e=xo()){let t=pe()||{},r=se(t);o.entities.firmwareUpdate={...t,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:rm(e),current_version:r}}async function On(e={}){let t=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await nm(r===0?900:1200),await K(Gt,"all",{forceMissing:!0});let n=_l(),a=!t||kv(t),i=qv(),l=st(),c=Mo();if(n&&a&&(i||!l&&c!=="Nog niet gecontroleerd"))return!0}return!1}async function sr(e={}){let t=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await nm(a===0?r:n);try{let i=o.entities.firmwareUpdateStatus;await K(Gt,"all",{forceMissing:!0});let l=Nn();o.entities.firmwareUpdateStatus!==i&&(o.updateInstallStatusPollObserved=!0);let c=wv();if(c){let u=new Error(c);throw u.firmwareInstallTerminal=!0,u}if(l==="rebooting"&&o.updateInstallStatusPollObserved&&wo("ota"),h(),o.updateInstallMode==="connection-switch"){let u=Rn(o.updateInstallTargetConnection);if(u&&at()===u&&!Fr()&&!rr())return!0}else if(o.updateInstallMode==="topology-switch"){let u=Jo(o.updateInstallTargetTopology);if(u&&oe()===u&&!Fr()&&!rr())return!0}else if(o.updateInstallMode==="build-switch"){let u=Jo(o.updateInstallTargetTopology),d=Rn(o.updateInstallTargetConnection);if(u&&d&&oe()===u&&at()===d&&!Fr()&&!rr())return!0}else if(Rl()||Pl()||Dl()&&!Fr()&&!rr())return!0}catch(i){if(i?.firmwareInstallTerminal)throw i;t||(o.controlNotice="Wachten tot het device opnieuw is opgestart...",h(),t=!0)}}return!1}function Tv(){let e=xo(),t=St();if(t)return t.copy;if(Nl()){let r=o.updateInstallCompletedVersion||se()||xo();return`${Ge()} draait nu op ${r}.`}return rr()?`OTA-update wordt voorbereid voor ${Ge()}. Het device kan kort herstarten.`:st()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:ir()?"Er staat een nieuwere firmware klaar.":Dl()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function im(){return!!(o.firmwareAdvancedOpen||o.firmwareConnectionSwitchOpen||o.firmwareTopologySwitchOpen||o.updateManualUploadOpen||o.updateTestFirmwareOpen)}function Za(e,t,r,n,a=!1){return`
      <button
    ...
    `}function Cv(e,t,r,n){if(!im())return"";let i=!!(St()||o.updateInstallBusy||st());return`
    ...
    `}function xv(){let e=Ja();if(!e||!o.firmwareConnectionSwitchOpen)return"";let r=!!(St()||o.updateInstallBusy||st()),n=!!o.firmwareConnectionSwitchConfirmed,a=e.targetConnection==="eth",i=!e.canSwitch,l=a?"Sluit eerst de netwerkkabel aan. Na de herstart verdwijnt Wi-Fi uit deze firmware.":"Na de herstart verdwijnt Ethernet uit deze firmware. Als er geen Wi-Fi-gegevens bekend zijn, start het OpenQuatt fallback access point.",c=i?'<p class="oq-helper-modal-note oq-helper-modal-note--muted">Verbindingswissel wordt geladen. Open deze modal opnieuw of wacht een moment als de knop disabled blijft.</p>':"";return`
    ...
    `}function $v(){if(!o.updateTestFirmwareOpen)return"";let t=!!(St()||o.updateInstallBusy||st()),r=nr(),n=jr(),a=Pn(r,n),i=!!(n.available&&v("firmwareTestOtaUrl")&&v("firmwareTestOtaMd5Url")&&v("installFirmwareTestOta")),l=!!(r&&i),c=o.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,d=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Av(){if(!o.updateManualUploadOpen)return"";let t=!!(St()||o.updateInstallBusy||st()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
  ...
  `}function Nv(){if(!o.root)return;if(o.interfacePanelOpen){h();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=Ll();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function _v(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Ln()),r&&(r.textContent=ni())}function Il(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function Dv(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${Il(e)}</span>`}function io(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${Il(e)}</span>`}function Ov(){return[...jo]}function Lv(){let e=[];return jo.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function Iv(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function ai(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function cr(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function $o(e){return`${Oe()}/openquatt/debug-recording/${e}`}function Vr(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Fl(){Vr({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function Fv(){let e=await window.fetch($o("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),t}function In(e=2e3){cr(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{ii({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function ii(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",h());try{await Fv(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),In()}catch(t){Fl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?h():(Nv(),_v())}}async function fm(){let e=hl(jo,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch($o(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==jo.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${jo.length})`);return t}async function jv(e){let t=Math.max(1,Number(e)||15);ai(),cr(),Da({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await fm();let r=await window.fetch($o(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Vr(n),In()}catch(r){Fl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,h()}}async function Wv(){ai(),cr(),Da({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await fm();let e=await window.fetch($o("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Vr(t),In()}catch(e){Fl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function Vv(){let e=await window.fetch($o("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),cr(),t}async function Bv(){ai(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{await Vv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function Kv(e={}){ai(),cr(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{let t=await window.fetch($o("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Vr(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,h()}}function lm(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(Lu/1e3),sample_count:Ot(),column_count:jo.length,storage:"browser"},columns:Ov(),units:Lv(),initial:Iv(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function oi(e){return JSON.stringify(e)}function zv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([oi(lm())]).size}catch{return oi(lm()).length}}function Uv(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function Gv(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function Qv(){if(Ot()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Lt();h();try{let t=await window.fetch($o("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,xs(Gv(r),oi(r),"application/json"),gm(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,h()}}async function Yv(){if(Ot()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Lt();h();try{let t=await window.fetch($o("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await ho(oi(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");gm(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,h()}}var Zv={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",h(),ii()),"start-debug-recording":e=>jv(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>Wv(),"select-debug-recording-duration":e=>Rv(e.dataset.debugMinutes||15),"stop-debug-recording":()=>Kv(),"freeze-debug-recording":()=>Bv(),"download-debug-recording":()=>Qv(),"copy-debug-recording":()=>Yv()};function hm(e,t){return ie(Zv,e,t)}function bm(){let e=o.debugRecordingActive,t=Lt(),r=ri(),n=Ot(),a=o.debugRecordingBusy,i=Uv(zv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Hv(),u=pm(),d=cm(),g=Pv(),w=n>0,y=`${g.toFixed(1)}%`,f=[{icon:"status",label:"Status",value:Ln()},{icon:"clock",label:t?"Retentie":"Duur",value:lr(t?d:um())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],m=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return te({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:ni(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
          ...
          ${e?`
            <div class="oq-debug-recording-progress">
              <div class="oq-debug-recording-progress-head">
                <span>${s(t?`Laatste ${lr(d)} bewaard`:`Nog ${lr(u)}`)}</span>
        ...
        <div class="oq-debug-recording-actions">
          ...
          `:""}
        </div>`})}function ym(e){o.overviewTheme=e==="dark"?"dark":"light";try{window.localStorage.setItem("oq-overview-theme",o.overviewTheme)}catch{}}function Fn(e){o.interfacePanelOpen=e===!0}function jl(e){try{window.localStorage.setItem("oq-active-surface",e==="native"?"native":"app")}catch{}}function si(e,t={}){o.settingsGroup=De.has(e)?e:ge[0].id;try{window.localStorage.setItem("oq-settings-group",o.settingsGroup)}catch{}t.syncUrl!==!1&&o.appView==="settings"&&Go(t.syncMode||"replace")}function vm(e){o.hpVisualMode=e==="compact"?"compact":"schematic";try{window.localStorage.setItem("oq-hp-visual-mode",o.hpVisualMode)}catch{}}function Sm(e){o.hpLayoutMode=e==="focus-hp1"||e==="focus-hp2"?e:"equal";try{window.localStorage.setItem("oq-hp-layout-mode",o.hpLayoutMode)}catch{}}function Jv(e){if(o.reducedMotion=!!e?.matches,o.reducedMotion){Cn();return}Uo()}function Xv(){let e=ws();!e||o.motionPreferenceMedia===e||(o.motionPreferenceMedia=e,o.motionPreferenceListener=Jv,typeof e.addEventListener=="function"?e.addEventListener("change",o.motionPreferenceListener):typeof e.addListener=="function"&&e.addListener(o.motionPreferenceListener),o.reducedMotion=!!e.matches)}function eS(){return Object.keys(o.entities).length>0}function tS(){return bs+Math.floor(Math.random()*(Qu-bs+1))}function oS(){return(document.hidden?wn:Wu)+tS()}function qm(e=oS()){o.pollTimer||o.nativeOpen||o.updateInstallBusy||ja()||(o.pollTimer=window.setTimeout(async()=>{o.pollTimer=null,await we(),qm()},e))}function Wl(){qm()}function Vl(){o.pollTimer&&(window.clearTimeout(o.pollTimer),o.pollTimer=null)}Pd({start:Wl,stop:Vl});function rS(){o.nativeOpen||(Vl(),Wl(),document.hidden||we({forceProbe:!0}))}function li(e={}){if(jn(),o.nativeOpen){Vl(),Cn(),o.nativeFrontendLoaded||ci();return}if(Uo(),Wl(),e.refresh!==!1){if(!eS()){Ep();return}we({forceFast:!0})}}function nS(){let e=Mn()||yo(),t=e==="settings"?Vs()||o.settingsGroup:"",r=e==="control"?Fs()||"status":o.controlReplayTab,n=e==="control"?js()||"last24":o.controlReplayWindow,a=e==="control"?Ws():null;if(!(e===o.appView&&(e!=="settings"||t===o.settingsGroup)&&(e!=="control"||r===o.controlReplayTab&&n===o.controlReplayWindow&&(!a||a.start===o.controlReplayCustomStart&&a.end===o.controlReplayCustomEnd)))){if(o.appView=e,e==="control"&&(o.controlReplayTab=r,o.controlReplayWindow=n,o.controlReplayCustomStart=a?.start||"",o.controlReplayCustomEnd=a?.end||"",o.controlReplayPeriodMenuOpen=!1,o.controlReplayCustomPeriodOpen=!1,o.controlReplayCustomPeriodError=""),e==="settings"&&t){o.settingsGroup=t;try{window.localStorage.setItem("oq-settings-group",o.settingsGroup)}catch{}}h(),we({forceFast:!0})}}function jn(){o.nativeApp&&(o.nativeApp.classList.add("oq-native-app"),o.nativeApp.classList.toggle("oq-native-app--collapsed",!o.nativeOpen),o.nativeApp.setAttribute("aria-hidden",o.nativeOpen?"false":"true"))}function km(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",wm,{once:!0}):wm(),window.addEventListener("pointermove",jd),window.addEventListener("pointerup",Wd),window.addEventListener("popstate",nS),document.addEventListener("visibilitychange",rS)}function wm(){aS();let e=document.querySelector("esp-app");e||(e=document.createElement("esp-app"),document.body.appendChild(e)),o.nativeApp=e,o.nativeFrontendLoaded=Array.from(document.scripts).some(t=>t.src===as),o.mounted||(iS(e),o.mounted=!0,li()),jn(),o.nativeOpen||(we(),ii({silent:!0}))}function aS(){if(!document.head)return;let e=document.head.querySelector('meta[name="viewport"]');e||(e=document.createElement("meta"),e.name="viewport",document.head.appendChild(e)),e.setAttribute("content","width=device-width, initial-scale=1")}function iS(e){let t=document.createElement("section");t.id="oq-helper-root",t.lang="nl-NL",document.documentElement&&!document.documentElement.lang&&(document.documentElement.lang="nl-NL"),e.parentNode.insertBefore(t,e),t.addEventListener("click",Od),t.addEventListener("change",Dd),t.addEventListener("input",Ld),t.addEventListener("keydown",Id),t.addEventListener("wheel",Kd,{passive:!1}),t.addEventListener("focusin",Os),t.addEventListener("focusout",Os),t.addEventListener("mouseover",Bd),t.addEventListener("mouseout",Vd),t.addEventListener("pointerdown",Fd),o.root=t,Xv();let r=Mn()||yo(),n=r==="settings"?Vs():"",a=r==="control"?Fs():"",i=r==="control"?js():"",l=r==="control"?Ws():null;n&&si(n,{syncUrl:!1}),a&&(o.controlReplayTab=a),i&&(o.controlReplayWindow=i,o.controlReplayCustomStart=l?.start||"",o.controlReplayCustomEnd=l?.end||""),Ht(r,{syncMode:"replace",forceSync:!0}),Tn(),h()}function sS(e){return new Promise((t,r)=>{if(!e){t();return}let n=Array.from(document.scripts).find(i=>i.src===e);if(n){if(n.dataset.loaded==="true"){t();return}n.addEventListener("load",()=>t(),{once:!0}),n.addEventListener("error",i=>r(i),{once:!0});return}let a=document.createElement("script");a.src=e,a.async=!1,a.addEventListener("load",()=>{a.dataset.loaded="true",t()},{once:!0}),a.addEventListener("error",i=>r(i),{once:!0}),document.head.appendChild(a)})}async function ci(){if(!(o.nativeFrontendLoaded||o.nativeFrontendLoading)){o.nativeFrontendLoading=!0,o.nativeOpen&&h();try{await sS(as),o.nativeFrontendLoaded=!0}catch(e){o.controlError=`ESPHome fallback kon niet worden geladen. ${e.message||e}`,o.nativeOpen=!1,jl("app"),h(),li()}finally{o.nativeFrontendLoading=!1,o.nativeOpen&&h()}}}function Wn(){let e=o.authStatus;return e?e.enabled?e.setup_window_active?"Instelvenster":"Beveiligd":"Onbeveiligd":"Laden..."}function lS(){return"Login"}function cS(){let e=o.authStatus;return e?e.enabled?"De web-app vraagt nu een login voordat beheer beschikbaar is. Je kunt die hier aanpassen of uitzetten.":"De web-app staat open op je netwerk. Houd de herstelknop 5 seconden vast om een login toe te voegen.":"We halen de huidige loginstatus op."}function Vn(){let e=o.authStatus;return e?e.enabled?e.setup_window_active?"Login actief. Tijdelijk instelvenster is open.":`Login actief${e.source?` via ${e.source}`:""}.`:e.setup_window_active?"Login uit. Tijdelijk instelvenster is open.":"Login uit. Webtoegang is open / onbeveiligd op het netwerk.":"Logingegevens laden..."}function Br(){let e=o.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?"Herstart nodig":e.transport_active===!0?"Aan":e.transport_active===!1?"Uit":e.enabled?"Aan":"Uit":"Laden..."}function Kr(){let e=o.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API beveiligd.":e.key?"Deze wijziging wordt actief na herstart. De sleutel blijft opgeslagen voor later gebruik.":"Deze wijziging wordt actief na herstart.":e.transport_active===!0?"API-encryptie staat aan. Gebruik dezelfde sleutel in Home Assistant.":"De native API staat nu open op je lokale netwerk.":"API-encryptie wordt geladen."}function uS(){return"ESPHome API encryption"}function dS(){let e=o.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu alvast voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API nog beveiligd.":"Deze wijziging wordt actief na herstart. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":e.transport_active===!0?"De native API is beveiligd. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":"API-encryptie staat uit. Schakel in om een sleutel te bekijken, kopi\xEBren of vernieuwen.":"We halen de huidige API-beveiliging op."}function pS(){let e=o.apiSecurityStatus;if(!e)return"Laden...";let t=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active);return t&&e.enabled===!0&&e.transport_active===!1?"Annuleer inschakelen":t&&e.enabled===!1&&e.transport_active===!0?"Annuleer uitschakelen":e.enabled?"Uitschakelen":"Inschakelen"}function mS(){let e=o.apiSecurityStatus;return e?e.key?"Vernieuw sleutel":"Genereer sleutel":"Laden..."}function Em(e,t,r="",n=!1){return`
    ...
    `}function Tm(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return te({id:"system",titleId:"oq-api-security-modal-title",kicker:"Toegang",title:uS(),copy:dS(),className:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit API-beveiliging popup",body:`
      ...
      `;return te({id:"system",titleId:"oq-login-modal-title",kicker:"Systeem",title:lS(),copy:cS(),closeAction:"close-system-modal",closeLabel:"Sluit login-popup",body:`
          ${i}
          ${l}
          <div class="oq-helper-modal-grid">
            ${Em("Beveiligingsstatus",Wn(),Vn())}
        ...
        ${n?`<button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="save-web-auth" ${o.authBusy?"disabled":""}>${t?"Opslaan":"Login opslaan"}</button>`:""}`})}function Bn(e,t,r){let n=R(t),a=R(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=R(e);return Number.isFinite(i)?i:NaN}function Bl(e="oq-settings-grid"){let t=gS();return`
      <div class="${s(e)}">
    ...
    `}function hS({status:e,running:t,resultReady:r,startDisabled:n,abortDisabled:a,applyDisabled:i,busy:l,controlsAvailable:c}){let u=String(e||"").toUpperCase(),d=u.includes("FAILED")||u.includes("REFUSED")||u.includes("ABORT"),g=u.includes("APPLIED"),w=v("hp2WaterIn")||v("hp2WaterOut")||v("hp2WaterInRaw")||v("hp2WaterOutRaw"),y=R("hpWaterCalibrationStableProgress"),f=R("hpWaterCalibrationStableRequired"),m=R("hpWaterCalibrationRemaining"),p=Math.round(R("hpWaterCalibrationPhase")),b=t&&(p===1||u.includes("MIXING")),S=t&&!b,q=300,E=60,k=Number.isFinite(m)?Math.max(0,q-m):NaN,M=Number.isFinite(k)?Math.max(0,E-k):NaN,P=b&&Number.isFinite(k)?Math.max(0,Math.min(100,k/E*100)):S&&Number.isFinite(y)&&Number.isFinite(f)&&f>0?Math.max(0,Math.min(100,y/f*100)):t&&Number.isFinite(m)?Math.max(0,Math.min(100,100-m/q*100)):r?100:0,N=r&&v("hpWaterCalibrationResultSpreadBefore")?ke("hpWaterCalibrationResultSpreadBefore",2):ke("hpWaterCalibrationSpread",2),V=b?"Water mengen":Number.isFinite(y)&&Number.isFinite(f)&&f>0?y>0?`${Math.round(Math.max(0,y))} / ${Math.round(f)} s binnen grenzen`:"Nog niet binnen grenzen":"Wachten op stabiel venster",H=r?3:t?2:1,F=g?"Offsets toegepast":r?`Meting klaar - spreiding ${N}`:t?b?`Water mengen${Number.isFinite(M)&&M>0?` - meting start over ${Math.round(M)} s`:""}`:`Meting bezig - ${Number.isFinite(m)&&m>0?`max. ${Math.round(m)} s resterend`:V}`:d?"Meting niet voltooid":"Voorbereiding",A=g?"De voorgestelde offsets zijn opgeslagen.":r?"Controleer de voorgestelde offsets en pas ze toe.":t?b?"De waterpomp circuleert zonder compressor zodat de watertemperaturen eerst kunnen mengen.":"De firmware stopt zodra het laatste meetvenster binnen de spreiding- en driftgrenzen valt.":d?er("hpWaterCalibrationStatus","Controleer de voorwaarden en start opnieuw."):w?"Start alleen wanneer compressor en boiler uit zijn. HP1 en HP2 water in/out worden samen naar een relatieve referentie gebracht.":"Start alleen wanneer compressor en boiler uit zijn. Bij single setup wordt HP1 water in/out onderling gelijkgetrokken; supply blijft diagnose.",O=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",liveKey:"hp1WaterIn",resultRawKey:"hpWaterCalibrationResultHp1InRawAvg",offsetKey:"hp1WaterInOffset",suggestedKey:"hp1WaterInOffsetSuggested"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",liveKey:"hp1WaterOut",resultRawKey:"hpWaterCalibrationResultHp1OutRawAvg",offsetKey:"hp1WaterOutOffset",suggestedKey:"hp1WaterOutOffsetSuggested"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",liveKey:"hp2WaterIn",resultRawKey:"hpWaterCalibrationResultHp2InRawAvg",offsetKey:"hp2WaterInOffset",suggestedKey:"hp2WaterInOffsetSuggested"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",liveKey:"hp2WaterOut",resultRawKey:"hpWaterCalibrationResultHp2OutRawAvg",offsetKey:"hp2WaterOutOffset",suggestedKey:"hp2WaterOutOffsetSuggested"}].filter(I=>v(I.liveKey)||v(I.rawKey)||v(I.offsetKey)),D=(I,W)=>{let ee=H>I;return`
      ...
      `,J=I=>{let W=R(I.resultRawKey),ee=Number.isFinite(W)?W:Bn(I.rawKey,I.liveKey,I.offsetKey),Ne=R(I.suggestedKey),Et=Number.isFinite(ee)&&Number.isFinite(Ne)?eo(ee+Ne,o.entities[I.suggestedKey]?.uom||"\xB0C",2):"\u2014";return`
        <tr>
    ...
    `}function $m(){let e=v("boilerCvAssistEnabled")&&C("boilerCvAssistEnabled"),t=Ua(),r=C("cm100Active"),n=String(t||"").trim().toUpperCase(),a=ur(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,u=!!o.pendingCommissioningCm100Start,d=c||i||a,g=c||!i,w=tr("boilerPowerTestStatus","IDLE"),y=ui(w,"boiler"),f=C("boilerPowerTestActive"),m=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",p=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),b=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=ur(w),k=!Te(w)&&(f||b||S||zr(w))&&!q,M=Ae("boilerRatedHeatPower"),P=Ae("boilerHeatPower"),N=R("boilerHeatPower"),V=N>0?P:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:P;N>0&&(o.commissioningBoilerHeatPowerDisplay=P);let H=tr("flowAutotuneStatus","IDLE"),F=ui(H,"autotune"),A=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",O=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),D=!!o.pendingFlowAutotuneStart,_=o.commissioningTaskLock==="autotune",J=ur(H),W=!Te(H)&&(D||_||zr(H))&&!J,ee=tr("airPurgeStatus","IDLE"),Ne=ui(ee,"purge"),Et=C("airPurgeActive"),pt=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",Bt=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),Je=!!o.pendingAirPurgeStart,_e=o.commissioningTaskLock==="purge",x=!Te(ee)&&(Et||Je||_e||zr(ee)),B=/DONE/.test(String(ee||"").toUpperCase()),Q=!!(Bt||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),G=Ae("airPurgeRemaining",{decimals:0}),le=R("airPurgePhase"),ve=le===1?"Rustig":le===2?"Pulsen":le===3?"Stabiliseren":Ne.phase,ue=tr("manualFlowStatus","IDLE"),Ce=C("manualFlowActive"),me=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",mt=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),Tt=!!o.pendingManualFlowStart,de=o.commissioningTaskLock==="manual-flow",We=!Te(ue)&&(Ce||Tt||de||zr(ue)),Xe=tr("manualHpStatus","IDLE"),gt=C("manualHpActive"),ft=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",_o=!!(o.entities.manualHpStart||o.entities.manualHpAbort),Se=!!o.pendingManualHpStart,Ct=o.commissioningTaskLock==="manual-hp",et=!Te(Xe)&&(gt||Se||Ct||zr(Xe)),ku=/SAFETY STOP/.test(String(Xe||"").toUpperCase()),Eu=/STOPPING/.test(String(Xe||"").toUpperCase()),po=tr("hpWaterCalibrationStatus","IDLE"),db=ui(po,"hp-water-calibration"),Tu=C("hpWaterCalibrationActive"),ba=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",Cu=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),wa=!!o.pendingHpWaterCalibrationStart,mo=o.commissioningTaskLock==="hp-water-calibration",ze=!Te(po)&&(Tu||wa||mo||zr(po)),ya=/DONE|APPLIED/.test(String(po||"").toUpperCase()),xu=/APPLIED/.test(String(po||"").toUpperCase()),pb=Ae("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),mb=Ae("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),Mu=/DONE|APPLIED/.test(String(w||"").toUpperCase()),$u=/DONE|APPLIED/.test(String(H||"").toUpperCase()),ts=i?q?"Wachten op CM100":k?y.phase:Mu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",os=i?J?"Wachten op CM100":W?F.phase:$u?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",rs=i?x?Ne.phase:B?"Klaar":"Klaar om te starten":"Wachten op CM100",Au=i?We?"Actief":"Klaar om te starten":"Wachten op CM100",Hu=i?et?Eu?"Bezig met stoppen":ku?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",Ru=i?ze?db.phase:xu?"Offsets toegepast":ya?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Pu=!i||m||!p||W||x||We||et||ze||k||_||_e||de||Ct||mo||b,gb=m||!(k||S||b),fb=m||Pu||!Mu||W||x||ze,Nu=!i||A||!O||k||x||We||et||ze||W||S||_e||de||Ct||mo||D,hb=A||!(W||_||D),bb=A||Nu||!$u||k||x||ze,wb=!i||pt||!Bt||k||W||We||et||ze||x||S||_||de||Ct||mo||Je,yb=pt||!(x||_e||Je),vb=!i||me||!mt||k||W||x||et||ze||We||S||_||_e||Ct||mo||Tt,Sb=me||!(We||de||Tt),qb=!i||ft||!_o||k||W||x||We||ze||et||S||_||_e||de||mo||Se,kb=ft||!(et||Ct||Se),Eb=!i||ba||!Cu||k||W||x||We||et||ze||S||_||_e||de||Ct||wa,Tb=ba||!(ze||mo||wa),Cb=ba||ze||!ya||xu;u&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),b&&(f||Te(w))&&(o.pendingBoilerPowerTestStart=!1),S&&Te(w)&&(o.commissioningTaskLock=""),D&&Te(H)&&(o.pendingFlowAutotuneStart=!1),_&&Te(H)&&(o.commissioningTaskLock=""),Je&&(Et||Te(ee))&&(o.pendingAirPurgeStart=!1),_e&&Te(ee)&&(o.commissioningTaskLock=""),Tt&&(Ce||Te(ue))&&(o.pendingManualFlowStart=!1),de&&(Ce||Te(ue))&&(o.commissioningTaskLock=""),Se&&(gt||Te(Xe))&&(o.pendingManualHpStart=!1),Ct&&(gt||Te(Xe))&&(o.commissioningTaskLock=""),wa&&(Tu||Te(po))&&(o.pendingHpWaterCalibrationStart=!1),mo&&Te(po)&&(o.commissioningTaskLock="");let xb=a?"Wachten op CM100":t,Mb=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",$b=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:Ru,available:!!(Cu||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:Ru,statusCopy:ze?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":ya?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:hS({status:po,running:ze,resultReady:ya,startDisabled:Eb,abortDisabled:Tb,applyDisabled:Cb,busy:ba,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:Au,available:!!(mt||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:Au,statusCopy:We?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?or({active:We,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:me||vb,stopDisabled:me||Sb}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:Hu,available:!!(_o||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:Hu,statusCopy:et?Eu?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":ku?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `,metrics:`
            ...
            <div class="oq-settings-manual-hp-results">
              ${Pe("flowSelected","Gemeten flow","Actuele doorstroming in het watercircuit.",Ae("flowSelected"),"oq-settings-field--compact")}
              ${Pe("hp1Compressor","Warmtepomp 1 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",Mm("hp1Compressor","hp1Freq"),"oq-settings-field--compact")}
              ${v("hp2Compressor")?Pe("hp2Compressor","Warmtepomp 2 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",Mm("hp2Compressor","hp2Freq"),"oq-settings-field--compact"):""}
            </div>
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:os,available:!0,openDisabled:ur(os),cardMarkup:Ur({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:os,statusCopy:J?"Wacht totdat CM100 actief is voordat je autotune start.":W?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort?or({active:W,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:A||Nu,stopDisabled:A||hb}):""}
            ${o.entities.flowAutotuneApply?Qe("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",A||bb):""}
          `,metrics:`
            ${Pe("flowKpSuggested","Voorgestelde Kp","Kp bepaalt hoe sterk de regeling meteen corrigeert.",pb,"oq-settings-field--compact")}
            ${Pe("flowKiSuggested","Voorgestelde Ki","Ki corrigeert kleine afwijkingen langzaam weg.",mb,"oq-settings-field--compact")}
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:ts,available:e,openDisabled:ur(ts),cardMarkup:Ur({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s(M)}`,status:ts,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":k?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
          ...
          `})}].filter(Ab=>Ab.available);return{cm100Status:xb,cm100StartDisabled:d,cm100StopDisabled:g,serviceStatusCopy:Mb,tasks:$b}}function bS(e){return Le({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:Xt(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,Ho(),o.appView==="results"&&mr({force:!0}).then(()=>h())})},"select-energy-history-import-file":()=>GS(),"clear-energy-history-import-file":()=>{Zm(),h()},"import-energy-history-file":()=>QS(),"export-energy-history":()=>NS(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",h();let e=qi({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&h()});return Ho([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",h()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",h()},"open-storage-energy":()=>{o.settingsStoragePage="energy",h()},"back-storage-overview":()=>{o.settingsStoragePage="overview",h()},"download-settings-backup":()=>oq(),"open-settings-backup-import":()=>{o.settingsBackupMqttPassword="",o.settingsBackupRestoreResult=null,o.settingsBackupError="",o.systemModal="settings-backup-import",h()},"confirm-settings-backup-restore":()=>iq()};function lg(e,t){return ie(sq,e,t)}function ac(e,t,r="",n=!1){return`
    ...
    `}function lq(e,t,r,n=""){if(!v(e))return"";let a=o.entities[e],i=Array.isArray(a?.option)?a.option:[],l=String(T(e)||"");return i.length?`
    ...
    `:""}function cq(e){return v(e)||!!L[e]&&!o.optionalMissingEntities?.[e]}function Yn(e,t,r,n={}){if(!cq(e))return"";let a=v(e),i=a&&(o.loadingEntities||o.busyAction===e),l=n.disabled===!0||!a,c=n.buttonClass||"oq-helper-button oq-helper-button--ghost";return`
      <button
        class="${s(c)}"
        type="button"
        data-oq-action="${s(r)}"
    ...
    `}function gq(){if(!v("lifetimeEnergyHistoryEnabled"))return"";let e=Jr(o.energyHistoryExportMode),t=Gn.map(n=>`
    ...
    `}zd({getSignature:xi,patch:bq});function Cg(){if(o.systemModal==="login")return Cm();if(o.systemModal==="api-security")return Tm();if(o.systemModal==="mqtt")return sp();if(o.systemModal==="mqtt-sensors")return lp();if(o.systemModal==="connectivity"){let e=hq();return te({modalId:"system",titleId:"oq-system-modal-title",kicker:"Systeem",title:"Connectiviteit",closeAction:"close-system-modal",closeLabel:"Sluit systeem-popup",bodyMarkup:`
      ...
      `:"";return te({modalId:"system",titleId:"oq-backup-success-modal-title",kicker:"Beheer",title:"Backup hersteld",closeAction:"close-system-modal",closeLabel:"Sluit bevestiging",className:"oq-helper-modal--wide oq-helper-modal--scrollable",bodyMarkup:`
        ...
        `});if(o.systemModal==="webserver-logs")return Jp();if(o.systemModal==="debug-recording")return bm();if(o.systemModal==="openquatt-pause"){let e=C("openquattEnabled"),t=o.busyAction==="openquatt-regulation",r=v("openquattResumeAt"),n=o.loadingEntities||o.entitySyncInFlight,a=r||!n,i=vn(),l=Mr(T("openquattResumeAt")),c=Ra();return te({modalId:"system",titleId:"oq-openquatt-pause-modal-title",kicker:"Bediening",title:"Openquatt regeling",modalClass:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit regeling-popup",bodyMarkup:`
        ...
        `})}return""}function yq(e=!1){let t=Pa();return t?`
    ...
    `:""}function uc(){return`
      <div class="oq-settings-curve-grid">
        ...
        ${ce("curveFallbackSupply","Fallback-aanvoertemperatuur zonder buitentemperatuur","Aanvoertemperatuur die gebruikt wordt als de buitentemperatuursensor niet beschikbaar is.","oq-settings-field--curve-fallback-card",{footerMarkup:yq()})}
      </div>
    ...
    `}function vq(e="oq-settings-grid"){let t=[ce("flowKp","Flow PI Kp","Hoe sterk de regeling direct reageert op een afwijking."),ce("flowKi","Flow PI Ki","Hoe snel de regeling kleine restfouten wegwerkt.")].filter(Boolean);return t.length?`
      <div class="${s(e)}">
        ${t.join("")}
      </div>
    ...
    `}function Sq(){if(!v("phResponseProfile"))return"";let e=String(T("phResponseProfile")||""),t=o.loadingEntities||o.busyAction==="save-phResponseProfile",n=`
    ...
    `:""}function xg(){let e=vq();return re("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
        ${pc()}
        ${e?`
          <div class="oq-settings-subpanel oq-settings-subpanel--nested">
            <div class="oq-settings-subpanel-head">
    ...
    `}function Pg(){let e=kq();return e.length?`
    ...
    `}function Hg(e,t){return v(`${t}CompressorStarts2h`)?`
      <div class="oq-settings-monitoring-compressor-unit">
        <div>
          <p>${s(e)}</p>
          <span>Laatste start: ${s(Aq(`${t}CompressorLastStartAge`))}</span>
        </div>
        <dl>
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=to(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function _g(){let e=Nt(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&to(t).length>0;return!e&&!r?"":re("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Lg(){let e=Mo(),t=za(),r=o.busyAction==="restartAction";return re("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ${Le({dataValue:"uptime",label:"Uptime",value:Nr()})}
          ${Le({dataValue:"ip",label:"IP-adres",value:_r()})}
    ...
    `}function Wg(e="wizard"){return e==="picker"?`
        <section class="oq-helper-panel">
          ${yc()}
    ...
    `}function Dq(){let e=Ai(),t=o.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function Bq(){let e=Gg().id;return e==="setup"?Pq():e==="generation"?Wg():e==="boiler"?v("boilerCvAssistEnabled")?Lq():jg():e==="flow-source"?_q():e==="thermostat-source"?Dq():e==="flow"?Iq():e==="heating"?Fq():e==="water"?jq():e==="silent"?Wq():e==="confirm"?Vq():jg()}function gr(){let e=Sc().isQEdition;return kr.filter(t=>(t.id!=="setup"||e)&&(!t.optionalEntity||v(t.optionalEntity)))}function kt(e){let t=gr().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function Kq(e){let t=Hi(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function zq(e=!1){return gr().map((t,r)=>{let n=Kq(r);return`
        <button
      ...
      `}).join("")}function Gg(){let e=gr();return e.find(t=>t.id===o.currentStep)||e[0]||kr[0]}function Hi(){return Math.max(0,gr().findIndex(e=>e.id===o.currentStep))}function Qg(e){let t=gr(),r=Math.min(t.length-1,Math.max(0,Hi()+e));o.currentStep=t[r]?.id||kr[0].id}function Ft(e={}){let t=Hi(),r=gr(),n=t>0?r[t-1]:null,a=t<r.length-1?r[t+1]:null;return`
    ...
    `}function Uq(){let e=Hi(),t=gr();return`
    ...
    `:"";return re("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
      ...
      `},D=($,x={})=>{if(!v($))return{markup:"",warning:""};let B=o.entities[$]||{},Q=String(T($)||""),G=to(B),le=new Set(x.hiddenOptions||[]),ve=Q&&le.has(Q),ue=G.filter(de=>!le.has(de)&&w(de,x)),Ce=Q&&!w(Q,x),me=Q==="HA input"&&x.keepUnavailableCurrent!==!0,Tt=(ve&&!ue.includes(Q)?[Q,...ue]:Ce&&!me&&!ue.includes(Q)?[Q,...ue]:ue).map(de=>{let qr=m(de,x);return`<option value="${s(de)}" ${de===Q?"selected":""}>${s(qr)}</option>`}).join("");return{markup:`
        ...
        `,warning:ve?"Huidige bron is legacy; kies een nieuwe bron.":Ce?`Huidige bron niet beschikbaar: ${y(Q,x)}`:""}},_=({key:$,title:x,icon:B="",select:Q,secondarySelect:G=null,secondarySelects:le=null,activeRows:ve=[],measurementRows:ue=[],activeCopy:Ce="",rows:me=[]})=>{let mt=Q&&Q.when!==!1?D(Q.key,Q):{markup:"",warning:""},de=(Array.isArray(le)?le:G?[G]:[]).filter(Se=>Se&&Se.when!==!1).map(Se=>D(Se.key,Se)).filter(Se=>Se.markup),qr=de.map(Se=>Se.markup).join(""),We=de.map(Se=>Se.warning).find(Boolean)||"",Xe=me.filter(Boolean).join(""),gt=`${mt.markup}${qr}`,ft=mt.warning||We,_o=[O({title:"Configuratie",icon:"settings",className:"oq-settings-source-group--config",content:gt?`
    ...
    `:"";return re("Integratie","MQTT inputbronnen","Beheer de brokerverbinding voor externe MQTT-bronwaarden.",`
      ...
      `)}function ef(){let e=[["login","Login",Wn(),Vn(),"open-login-modal"],["api","ESPHome API encryption",Br(),Kr(),"open-api-security-modal"]];return re("Toegang","Toegang & Beveiliging","Pas hier de web-login of de ESPHome API-sleutel aan. Deze wijziging wordt actief na herstart.",`
    ...
    `}function of(){let e=De.has(o.settingsGroup)?o.settingsGroup:ge[0].id;return`
    ...
    `:""}function Jq(e){let t=e.groups.map(Zq).filter(Boolean).join("");return t?`
    ...
    `:""}function Xq(e){let t=e.categories.map(Jq).filter(Boolean).join("");return t?`
    ...
    `:""}function Tc(){let e=$a.map(Xq).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function af(e=Tc()){return be(e)}function sf(e=Tc()){return`
    ...
    `}var lf=[{id:"day",label:"Dag"},{id:"week",label:"Week"},{id:"month",label:"Maand"},{id:"year",label:"Jaar"},{id:"all",label:"Alles"}],ek=new Set(["day","week","month","year"]);function je(e){let t=String(e||"").trim();return lf.some(r=>r.id===t)?t:"day"}function cf(e){let t=je(e);o.energyHistoryView!==t&&(At({energyHistoryView:t,energyHistoryLastFetchAt:0}),h(),uf())}function uf(){typeof mr=="function"&&mr({force:!0}).then(e=>{e&&h()})}function an(e){return ek.has(je(e))}function Ni(){let e=new Date;return e.getFullYear()*1e4+(e.getMonth()+1)*100+e.getDate()}function df(){return mi(o.energyHistoryRaw)}function Cc(){let e=String(o.energyHistoryRaw||""),t=null;return e.split(/\r?\n/).forEach(r=>{let n=Xl(r);n&&(t=n.dateKey)}),t}function tk(e=[],t=!0){let r=Cc(),n=df(),a=(Array.isArray(e)?e:[]).map(i=>Number(i?.dateKey)).filter(Number.isFinite);return Number.isFinite(Number(n.newestDateKey))&&a.push(Number(n.newestDateKey)),Number.isFinite(Number(r))&&a.push(Number(r)),t&&_i().forEach(i=>{let l=Number(i?.dateKey);Number.isFinite(l)&&a.push(l)}),a.length?Math.max(...a):Ni()}function fr(e){let t=R(e);return!Number.isFinite(t)||t<0?null:Math.round(t*1e3)}function ok(){let e=Cc()||Ni(),t=ne(e);if(!t)return null;let r={sequence:Number.MAX_SAFE_INTEGER-1,dateKey:e,year:t.year,month:t.month,day:t.day,partial:!0,source:"sensors",electricalInputWh:fr("electricalEnergyDaily"),heatingInputWh:fr("heatingElectricalEnergyDaily"),coolingInputWh:fr("coolingElectricalEnergyDaily"),heatpumpHeatOutputWh:fr("heatpumpThermalEnergyDaily"),heatpumpCoolingOutputWh:fr("heatpumpCoolingEnergyDaily"),boilerHeatOutputWh:fr("boilerThermalEnergyDaily"),systemHeatOutputWh:fr("systemThermalEnergyDaily")};return pi.some(n=>Number.isFinite(r[n]))?r:null}function ea(){let e=new Map;String(o.energyHistoryRaw||"").split(/\r?\n/).forEach(i=>{let l=Im(i)||Xl(i);if(!l)return;let c=e.get(l.dateKey);(!c||l.sequence>=c.sequence)&&e.set(l.dateKey,l)});let r=ok();r&&!e.has(r.dateKey)&&e.set(r.dateKey,r);let n=new Set(e.keys()),a=new Map;return _i().forEach(i=>{if(n.has(i.dateKey))return;let l=a.get(i.dateKey);if(!l){let c=ne(i.dateKey);if(!c)return;l=hr({dateKey:c.key,year:c.year,month:c.month,day:c.day,label:It(c.key),sortKey:c.key,source:"hour-summary"}),l.tooltipLabel=`${It(i.dateKey)} \xB7 uurdata sinds herstart`,a.set(i.dateKey,l)}br(l,i)}),a.forEach((i,l)=>{e.set(l,i)}),[...e.values()].sort((i,l)=>i.dateKey-l.dateKey)}function _i(){let e=new Map;return String(o.energyHistoryRaw||"").split(/\r?\n/).forEach(r=>{let n=Fm(r);if(!n)return;let a=`${n.dateKey}:${n.hour}`,i=e.get(a);(!i||n.sequence>=i.sequence)&&e.set(a,n)}),[...e.values()].sort((r,n)=>r.sortKey-n.sortKey)}function rk(e){return _i().filter(t=>t.dateKey===Number(e))}function nn(e,t){return e.reduce((r,n)=>r+Qr(n,t),0)}function nk(e){return["heatpumpHeatOutputWh","heatpumpCoolingOutputWh","boilerHeatOutputWh"].reduce((t,r)=>t+Qr(e,r),0)}function ak(e){return Qr(e,"electricalInputWh")+nk(e)}function Pi(e,t){let r=Number(e),n=Number(t);return!Number.isFinite(r)||!Number.isFinite(n)||n<=0?"\u2014":(r/n).toFixed(2)}function Ke(e,t=1){let r=Number(e);return Number.isFinite(r)?Math.abs(r)>=999500?`${(r/1e6).toFixed(2)} MWh`:Math.abs(r)<1e3?`${Math.round(r)} Wh`:`${(r/1e3).toFixed(t)} kWh`:"\u2014"}function hr({dateKey:e,year:t,month:r,day:n,hour:a=null,label:i,tooltipLabel:l="",sortKey:c,source:u="bucket"}){return{sequence:0,dateKey:e,year:t,month:r,day:n,hour:a,label:i,tooltipLabel:l,sortKey:c??e,partial:!1,source:u,electricalInputWh:0,heatingInputWh:0,coolingInputWh:0,heatpumpHeatOutputWh:0,heatpumpCoolingOutputWh:0,boilerHeatOutputWh:0,systemHeatOutputWh:0}}function br(e,t){return pi.forEach(r=>{e[r]+=Qr(t,r)}),e.partial=e.partial||!!t?.partial,e.sequence=Math.max(Number(e.sequence||0),Number(t?.sequence||0)),e}function ik(e){let t=new Map;return e.forEach(r=>{t.set(r.dateKey,r)}),t}function pf(e,t){let r=je(e);if(r==="day"){let n=Ul(t)||ne(t);return n?String(n.key):""}if(r==="week"){let n=Lm(t);return n?String(n.key):""}if(r==="month"){let n=Om(t);return n?String(n.key):""}if(r==="year"){let n=Number(t);return Number.isInteger(n)&&n>=2020&&n<=2200?String(n):""}return""}function Di(e,t){let r=je(t),n=ne(tk(e,!0)),a=df(),i=_i(),l=[...e.map(f=>f.dateKey),...i.map(f=>f.dateKey)].filter(f=>Number.isFinite(Number(f)));Number.isFinite(Number(a.oldestDateKey))&&l.push(Number(a.oldestDateKey)),Number.isFinite(Number(a.newestDateKey))&&l.push(Number(a.newestDateKey));let c=l.length?Math.min(...l.map(Number)):n?.key,d=(c?ne(c):n)?.date||n?.date||new Date,g=n?.date||new Date,w=n?.key||Ni(),y=w;return r==="week"?(w=Yl(d),y=Yl(g)):r==="month"?(w=fi(d),y=fi(g)):r==="year"?(w=d.getFullYear(),y=g.getFullYear()):(w=Ie(d),y=Ie(g)),Number(w)>Number(y)&&(w=y),{min:String(w),max:String(y)}}function mf(e,t){let r=Number(e);return Number.isFinite(r)?r<Number(t.min)?String(t.min):r>Number(t.max)?String(t.max):String(e):String(t.max)}function sk(e,t,r=Di(e,t)){let n=je(t),a=o.energyHistoryPeriodSelection?.[n],i=pf(n,a);return mf(i||r.max,r)}function lk(e,t){let r=je(e),n=[],a=0;if(r==="week"){let i=String(t.max);for(;Number(i)>=Number(t.min)&&a<6e3;){let l=ne(i);n.push({value:i,label:Zl(i),group:l?String(Ql(l.date).year):""}),i=l?String(Ie(pr(l.date,-7))):"",a+=1}return n}if(r==="month"){let i=String(t.max);for(;Number(i)>=Number(t.min)&&a<1200;){let l=dr(i);if(!l)break;n.push({value:i,label:l.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"}),group:String(l.year)}),i=Gl(i,-1),a+=1}return n}if(r==="year")for(let i=Number(t.max);i>=Number(t.min);i-=1)n.push({value:String(i),label:String(i)});return n}function Oi(e,t){let r=je(t);if(!an(r))return{view:r,selectedValue:"",minValue:"",maxValue:"",canPrevious:!1,canNext:!1,isNow:!0,options:[]};let n=Di(e,r),a=sk(e,r,n);return{view:r,selectedValue:a,minValue:n.min,maxValue:n.max,canPrevious:Number(a)>Number(n.min),canNext:Number(a)<Number(n.max),isNow:Number(a)===Number(n.max),options:lk(r,n)}}function ck(e,t){let r=je(t);if(!an(r))return{from:"",to:"",hours:"0"};let n=Oi(e,r);if(r==="day")return{from:n.selectedValue,to:n.selectedValue,hours:"1"};if(r==="week"){let a=ne(n.selectedValue);if(!a)return{from:"",to:"",hours:"0"};let i=Gr(a.date),l=pr(i,6);return{from:String(Ie(i)),to:String(Ie(l)),hours:"0"}}if(r==="month"){let a=dr(n.selectedValue);return a?{from:String(a.year*1e4+a.month*100+1),to:String(a.year*1e4+a.month*100+zl(a.year,a.month)),hours:"0"}:{from:"",to:"",hours:"0"}}if(r==="year"){let a=Number(n.selectedValue);return Number.isInteger(a)?{from:`${a}0101`,to:`${a}1231`,hours:"0"}:{from:"",to:"",hours:"0"}}return{from:"",to:"",hours:"0"}}function uk(){if(!String(o.energyHistoryRaw||"").trim())return"?meta=1";let e=ea(),t=ck(e,o.energyHistoryView||"day"),r=new URLSearchParams;t.from&&r.set("from",t.from),t.to&&r.set("to",t.to),r.set("hours",t.hours);let n=r.toString();return n?`?${n}`:""}Km(uk);function ta(e,t){let r=je(e);if(!an(r))return;let n=ea(),a=Di(n,r),i=pf(r,t),l=mf(i||a.max,a);At({energyHistoryPeriodSelection:{...o.energyHistoryPeriodSelection,[r]:l},energyHistoryLastFetchAt:0}),h(),uf()}function gf(e,t){let r=je(e);if(!an(r))return;let n=ea(),a=Oi(n,r),i=Number(t)<0?-1:1,l=a.selectedValue;if(r==="day"){let c=ne(a.selectedValue);l=c?String(Ie(pr(c.date,i))):l}else if(r==="week"){let c=ne(a.selectedValue);l=c?String(Ie(pr(c.date,i*7))):l}else r==="month"?l=Gl(a.selectedValue,i):r==="year"&&(l=String(Number(a.selectedValue)+i));ta(r,l)}function ff(e){let t=je(e);if(!an(t))return;let r=ea(),n=Di(r,t);ta(t,n.max)}function dk(e,t,r=Oi(e,t)){let n=je(t),a=ik(e);if(!e.length&&n==="all")return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};if(n==="day"){let c=ne(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=rk(c.key);if(u.length){let f=new Map(u.map(p=>[p.hour,p])),m=[];for(let p=0;p<24;p+=1){let b=String(p),S=`${c.date.toLocaleDateString("nl-NL",{day:"numeric",month:"long"})} \xB7 ${String(p).padStart(2,"0")}:00 - ${String((p+1)%24).padStart(2,"0")}:00`,q=hr({dateKey:c.key,year:c.year,month:c.month,day:c.day,hour:p,label:b,tooltipLabel:S,sortKey:p,source:"hour"}),E=f.get(p);E&&br(q,E),m.push(q)}return{buckets:m,title:"Dag",detail:`${c.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 uurdata sinds herstart`}}let d=a.get(c.key),g=Cc()||Ni(),w=c.key===g?"Vandaag":It(c.key),y=hr({dateKey:c.key,year:c.year,month:c.month,day:c.day,label:w,sortKey:c.key,source:"day"});return d&&br(y,d),{buckets:[y],title:"Dag",detail:`${c.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 dagtotaal`}}if(n==="week"){let c=ne(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=Gr(c.date),d=[];for(let g=0;g<7;g+=1){let w=pr(u,g),y=Ie(w),f=ne(y),m=hr({dateKey:y,year:f.year,month:f.month,day:f.day,label:It(y,"weekday"),sortKey:y}),p=a.get(y);p&&br(m,p),d.push(m)}return{buckets:d,title:"Week",detail:Zl(r.selectedValue)}}if(n==="month"){let c=dr(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=zl(c.year,c.month),d=[];for(let g=1;g<=u;g+=1){let w=Kl(c.year,c.month,g),y=Ie(w),f=hr({dateKey:y,year:c.year,month:c.month,day:g,label:String(g),sortKey:y}),m=a.get(y);m&&br(f,m),d.push(f)}return{buckets:d,title:"Maand",detail:c.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"})}}if(n==="year"){let c=Number(r.selectedValue);if(!Number.isInteger(c))return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=[];for(let d=1;d<=12;d+=1){let g=c*1e4+d*100+1,w=hr({dateKey:g,year:c,month:d,day:1,label:It(g,"month"),sortKey:d,source:"month"});e.filter(y=>y.year===c&&y.month===d).forEach(y=>br(w,y)),u.push(w)}return{buckets:u,title:"Jaar",detail:String(c)}}let i=new Map;e.forEach(c=>{i.has(c.year)||i.set(c.year,hr({dateKey:c.year*1e4+101,year:c.year,month:1,day:1,label:String(c.year),sortKey:c.year,source:"year"})),br(i.get(c.year),c)});let l=[...i.values()].sort((c,u)=>c.sortKey-u.sortKey);return{buckets:l,title:"Alles",detail:l.length?`${l[0].label} - ${l[l.length-1].label}`:"Geen data"}}function pk(e){let t=nn(e,"heatpumpHeatOutputWh"),r=nn(e,"heatpumpCoolingOutputWh"),n=nn(e,"boilerHeatOutputWh");return{electricalInputWh:nn(e,"electricalInputWh"),heatingInputWh:nn(e,"heatingInputWh"),coolingInputWh:nn(e,"coolingInputWh"),heatOutputWh:t,coolingOutputWh:r,boilerOutputWh:n,outputWh:t+r+n}}function mk(e){let t=Number(e.heatOutputWh||0)+Number(e.coolingOutputWh||0),r=Number(e.boilerOutputWh||0),n=t+r;return!Number.isFinite(n)||n<=0?Number.NaN:t/n*100}function gk(e){let t=Pi(e.heatOutputWh,e.heatingInputWh),r=Pi(e.coolingOutputWh,e.coolingInputWh),n=Number(e.heatOutputWh||0)>0&&t!=="\u2014",a=Number(e.coolingOutputWh||0)>0&&r!=="\u2014";return n&&a?{label:"COP / EER",value:`${t} / ${r}`}:a?{label:"Gemiddelde EER",value:r}:{label:"Gemiddelde COP",value:t}}function Ri(e,t,r=""){return`
    ...
    `}function fk(e){return an(e.view)?`
    ...
    `:`
      ...
      `}function hk(){let e=document.activeElement;return!!(e&&e.closest&&e.closest(".oq-energy-history-period"))}function bk(e){return`
    ...
    `}function wk(e){let t=Number(e.electricalInputWh||0),r=Number(e.heatOutputWh||0),n=Number(e.coolingOutputWh||0),a=Number(e.boilerOutputWh||0),i=a>0?"boiler":"boiler-zero",l=Math.max(1,t+r+n+a),c=d=>`${Math.max(0,Number(d||0)/l*100).toFixed(2)}%`,u=mk(e);return`
    ...
    `}function yk(e){let t=Math.max(1,Number(e||0)/1e3),r=Math.pow(10,Math.floor(Math.log10(t))),n=t/r;return(n<=1.5?1.5:n<=3?3:n<=6?6:10)*r*1e3}function vk(e){let t=Number(e);return Number.isFinite(t)?t>=999500?`${Number((t/1e6).toFixed(1))}`:`${Number((t/1e3).toFixed(1))}`:""}function Sk(e){return e>=999500?"MWh":"kWh"}function qk(e){let d=Math.max(1e3,...e.map(ak)),g=yk(d),w=e.length?1218/e.length:1218,y=Math.max(6,Math.min(38,w*.68));return{width:1280,height:260,left:44,right:18,top:26,bottom:38,plotWidth:1218,plotHeight:196,axisMax:g,barSlot:w,barWidth:y,yOf:m=>26+(1-Math.min(1,Math.max(0,Number(m||0)/g)))*196}}function kk(e){let t=Pi(e.heatpumpHeatOutputWh,e.heatingInputWh),r=Pi(e.heatpumpCoolingOutputWh,e.coolingInputWh);return[e.tooltipLabel||e.label||It(e.dateKey),`Elektrisch totaal: ${Ke(e.electricalInputWh,1)}`,`Elektrisch verwarmen: ${Ke(e.heatingInputWh,1)}`,`Elektrisch koelen: ${Ke(e.coolingInputWh,1)}`,`Warmtepomp warmte: ${Ke(e.heatpumpHeatOutputWh,1)}`,`Warmtepomp koeling: ${Ke(e.heatpumpCoolingOutputWh,1)}`,`Cv-ketel warmte: ${Ke(e.boilerHeatOutputWh,1)}`,`COP verwarmen: ${t}`,`EER koelen: ${r}`].join(`
`)}function Ek(e,t=""){if(!e.length)return`
      ...
      `;let r=qk(e),n=Sk(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,c)=>{let u=r.left+r.barSlot*c+r.barSlot/2,d=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],g=r.height-r.bottom,w=d.map(p=>{let b=Qr(l,p.key);if(b<=0)return"";let S=b/r.axisMax*r.plotHeight;return g-=S,`
        ...
        `}).join(""),f=e.length<=12||c===0||c===e.length-1||c%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||It(l.dateKey))}</text>`:"",m=kk(l);return`
      ...
      <svg class="oq-energy-history-chart oq-energy-history-chart--${s(je(t))}" viewBox="0 0 ${r.width} ${r.height}" role="img" aria-label="Energiehistorie">
        ...
        ${a.map(l=>{let c=r.yOf(l);return`
            ...
            <text x="${r.left-10}" y="${c.toFixed(1)}" text-anchor="end" dominant-baseline="middle" class="oq-energy-history-axis-label">${s(vk(l))}</text>
          `}).join("")}
        ${i}
      </svg>
    `}function Tk(e=null){let t=Number(e?.boilerOutputWh||0)>0?"boiler":"boiler-zero";return`
    ...
    `}function Ck(e,t,r,n){let a=je(r)==="day"?Number(n):Number.NaN,i=Number.isFinite(a)?e.find(l=>l.dateKey===a):null;return i?[i]:t}function xc(){let e=ea(),t=je(o.energyHistoryView),r=Oi(e,t),n=dk(e,t,r),a=Ck(e,n.buckets,t,r.selectedValue),i=pk(a);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:i,activeView:t}}function hf(e=xc()){return be({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,summary:e.summary})}function bf(e=xc()){let t=e.summary,r=gk(t),n=e.buckets[0]?.dateKey?It(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?It(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
    ...
    `}function xk(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=Tc();return Ze(t,af(n),sf(n))}function Mk(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=xc(),a=hk();return(a?!1:Ze(t,hf(n),bf(n)))||a}So({patchEnergyDom:xk,patchResultsDom:Mk});function $k({label:e,value:t,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(t)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function na(e,t=!1){return e.map(r=>$k({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?rt(r.key):r.value,status:t})).join("")}function Rc(e){return`
    ...
    `}function Pk(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}var Nk=Bo;function _k(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function Dk(e){let t=_k(j("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Ok(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=Nk(r),a=v("timeValid")&&C("timeValid")?Dk(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Lk(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Tf(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>v(e))||""}function Ik(){return["hp1WaterIn","hp2WaterIn"].find(e=>v(e))||""}function Cf(e=j("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function Wt(){return Cf()}function xf(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function Dc(){return Wt()?"Koeling":ae()?"Stooklijn":"Power House"}function Fk(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=R(t);if(!Number.isNaN(r))return r}return Number.NaN}function Mf(){let e=Fk(),t=R("phouseHouse"),r=R("totalHeat"),n=R("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:z(e,0,"W"),houseText:z(t,0,"W"),correctionText:Lk(a),capacityText:rt("hpCapacity"),statusTitle:i,statusCopy:l}}function $f(){let e=R("curveSupplyTarget"),t=R("supplyTemp"),r=Tf(),n=r?R(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:rt("curveSupplyTarget"),supplyText:rt("supplyTemp"),deltaText:Pk(a),capacityText:rt("hpCapacity"),statusTitle:l,statusCopy:c}}function Af(){let e=R("supplyTemp"),t=j("coolingGuardMode",""),r=j("coolingFallbackNightMinOutdoorTemp","\u2014"),n=R("coolingSupplyError"),a=R("coolingDemandRaw"),i=C("coolingPermitted"),l=C("coolingRequestActive"),c=j("coolingBlockReason","Onbekend"),u=kc(c),d=xf(c,l),g="Wacht op koelvraag",w="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return d?(g="Wacht op koelvraag",w="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(g="Houdt doel vast",w="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(g="Trekt aanvoer omlaag",w="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(g="Benadert koeldoel",w="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(g="Koelt rustig door",w="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(g="Koeling gereed",w="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(g="Koeling geblokkeerd",w=`Blokkade: ${u}.`),{targetText:rt("coolingSupplyTarget"),supplyText:rt("supplyTemp"),safeFloorText:rt("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:rt("coolingDemandRaw"),statusTitle:g,statusCopy:w,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:d}}function Ii(){if(Wt()){let t=Af(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(ae()){let t=$f();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=Mf();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function Oc(e){return kf({className:"oq-overview-system",title:e.title,copy:e.copy,signature:be(e),body:`
        ...
        <div class="oq-overview-metrics oq-overview-metrics--three-column">
          ${e.metrics.map(t=>Rk(t.label,t.value,t.tone,t.note)).join("")}
        </div>
      `})}function jk(){if(!C("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(Wt()){let n=Af(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(rf())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(ae()?$f():Mf()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Wk(){return C("openquattEnabled")?Wt()?C("coolingPermitted")?C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:j("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:C("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Lc(e,t){let r=jk(),n=Wk();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Ic(e,t){let r=Lc(e,t);return`
    ...
    `}function Fc(){let e=Wt();return[{key:"totalPower",label:"Elektrisch vermogen",tone:"blue",note:"hele systeem"},{key:e?"totalCoolingPower":"totalHeat",label:e?"Koelvermogen":"Verwarmingsvermogen",tone:"orange",note:"thermisch vermogen"},{key:e?"totalEer":"totalCop",label:e?"COP (EER)":"COP",tone:"green",note:"rendement"},{key:"flowSelected",label:"Flow",tone:"sky",note:"watercircuit"}]}function Vk(){let e=C("openquattEnabled"),t=T("openquattResumeAt"),r=vn(t),n=(o.loadingEntities||o.entitySyncInFlight)&&!v("openquattResumeAt"),a=C("manualCoolingEnable"),i=v("coolingEnableSelected")?C("coolingEnableSelected"):a,l=vf(j("coolingEnableEffectiveSource","")),c=String(T("coolingEnableSource")||"").trim(),u=vf(T("coolingEnableSource")),d=String(T("silentModeOverride")||"Schedule"),g=!C("coolingPermitted"),w=C("coolingRequestActive"),y=Cf(),f=j("coolingBlockReason",""),m=xf(f,w),p="Uit",b=c==="Disabled"?"Koeling is niet toegestaan: handmatig staat uit.":u&&u!=="geen bron"?`Koeling is niet toegestaan: ${u} geeft geen toestemming en handmatig staat uit.`:"Koeling is niet toegestaan.";i&&y?(p="Actief",b=oa("Koeling draait nu.",l)):i&&m?(p="Aan",b=oa("Koeling is toegestaan en wacht op kamertemperatuur boven het koel-setpoint.",l)):i&&g?(p="Geblokkeerd",b=oa(kc(f||"Koeling wacht nog op veilige condities."),l)):i&&w?(p="Start bijna",b=oa("Er is koelvraag. Koeling start zodra dat kan.",l)):i&&(p="Aan",b=oa("Koeling is toegestaan en wacht op koelvraag.",l));let S="Uit",q="Stille modus staat uit.",E="neutral";return d==="On"?(S="Aan",q="Stille modus staat geforceerd aan, ook buiten het tijdvenster.",E="orange"):d==="Schedule"&&(S="Schema",C("silentActive")?(q="Stille modus staat nu aan via het tijdvenster.",E="violet"):q="Stille modus volgt het tijdvenster."),[{key:"openquattEnabled",label:"Openquatt regeling",status:e?"Actief":"Tijdelijk uit",copy:e?"Verwarmen en koelen worden automatisch geregeld.":r?"Verwarming en koeling zijn tijdelijk uitgeschakeld. Beveiligingen blijven actief.":"Verwarming en koeling zijn uitgeschakeld. Beveiligingen blijven actief.",tone:e?"green":"orange",kind:"openquatt-control",meta:e?[]:[n?{label:"Hervatten",value:"Laden\u2026",tone:"neutral",loading:!0}:{label:r?"Hervat automatisch":"Hervatten",value:r?Mr(t,!0):"Handmatig",tone:r?"orange":"neutral"}]},{key:"manualCoolingEnable",label:"Koeling",status:p,copy:b,buttonLabel:a?"Handmatig uit":"Handmatig aan",nextState:a?"off":"on",tone:i?y?"blue":"sky":"neutral"},{key:"silentModeOverride",label:"Stille modus",status:S,copy:q,tone:E,kind:"select",selectedOption:d,settingsAction:!0,options:[{value:"Off",label:"Uit"},{value:"On",label:"Aan"},{value:"Schedule",label:"Schema"}]}].filter(k=>v(k.key))}function Bk(e=[]){return e.length?`
    ...
    `:""}function sn({className:e,action:t,label:r,busy:n=!1,loading:a=!1,attrs:i=""}){return`
      ...
      `:s(n?"Bezig...":r)}</button>
    `}function Kk(e){if(e.kind==="openquatt-control"){let t=o.busyAction==="openquatt-regulation",r=(o.loadingEntities||o.entitySyncInFlight)&&!v("openquattResumeAt");return C("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${sn({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:t})}</div>`:`
    ...
    `}function jc(){let e=Vk();return e.length?`
    ...
    `:""}function Hf(e){let t=j("controlModeLabel");return`
    ...
    `}function r1(e){let t=e.samples[e.samples.length-1]||null,r=Bc(e.windowHours);return`
      <article class="oq-overview-trendcard oq-overview-trendcard--${s(e.tone)}" data-oq-trend-card="${s(e.id)}" data-render-signature="${s(Qk(e))}">
    ...
    `}function a1(){let e=lo(),t=tc();return`
    ...
    `}function s1(){if(!o.root||o.appView!=="diagnosis")return!1;let e=o.root.querySelector(".oq-overview-board");if(!e)return!1;let t=e.querySelector(".oq-overview-trends");return t?(Ze(t,ji(),Wi()),zc(e),aa(e),!0):!1}function Uc(e){if(!e)return null;let t=e.__oqTrendHoverNodes;if(t&&t.chart&&t.hoverLayer&&t.hoverPanel)return t;let r=e.querySelector("[data-oq-trend-hover-layer]"),n={chart:e.querySelector(".oq-overview-trend-chart"),hoverLayer:r,hoverPanel:e.querySelector("[data-oq-trend-hover]"),hoverTime:e.querySelector("[data-oq-trend-hover-time]"),hoverNote:e.querySelector("[data-oq-trend-hover-note]"),hoverValues:e.querySelector("[data-oq-trend-hover-values]"),hoverLine:r?r.querySelector(".oq-overview-trend-hover-line"):null,hoverDots:{}};return r&&r.querySelectorAll("[data-oq-trend-hover-dot]").forEach(a=>{n.hoverDots[a.getAttribute("data-oq-trend-hover-dot")]=a}),e.__oqTrendHoverNodes=n,n}function qf(e,t,r,n=Uc(e)){if(!e||!t||!Array.isArray(t.points)||t.points.length===0)return;let a=Math.max(0,Math.min(t.points.length-1,r)),i=t.points[a];if(!i||!n||!n.chart||!n.hoverLayer||!n.hoverPanel||!n.hoverTime||!n.hoverNote||!n.hoverValues)return;let l=String(a);if(!n.hoverPanel.hidden&&e.dataset.oqTrendHoverIndex===l)return;let c=Ok(i.sample.t,t.endTime);n.hoverPanel.hidden=!1,n.hoverLayer.removeAttribute("hidden"),n.hoverTime.textContent=c.value,n.hoverNote.textContent=c.note,n.hoverLine&&(n.hoverLine.setAttribute("x1",i.x.toFixed(1)),n.hoverLine.setAttribute("x2",i.x.toFixed(1)));let u=[];t.series.forEach(d=>{let g=ra(d,i.sample),w=d.id||d.sampleKey||d.label,y=n.hoverDots[w];if(!Number.isFinite(g)){y&&y.setAttribute("display","none");return}let f=i.values.find(m=>m.seriesId===w);y&&f&&(y.removeAttribute("display"),y.setAttribute("cx",f.x.toFixed(1)),y.setAttribute("cy",f.y.toFixed(1))),u.push(`
        <div class="oq-overview-trend-hover-row oq-overview-trend-hover-row--${s(d.tone)}">
          <span>${s(d.label)}</span>
          <strong>${s(z(g,d.decimals,d.unit))}</strong>
        </div>
      `)}),n.hoverValues.innerHTML=u.join(""),e.dataset.oqTrendHoverIndex=l}function l1(e){if(!e)return;let t=Uc(e);t?.hoverPanel&&(t.hoverPanel.hidden=!0),t?.hoverLayer&&t.hoverLayer.setAttribute("hidden",""),delete e.dataset.oqTrendHoverIndex}function aa(e=o.root){if(!e)return;e.querySelectorAll("[data-oq-trend-card]").forEach(r=>{let n=r.dataset.renderSignature||"";if(r.__oqTrendBoundSignature===n)return;typeof r.__oqTrendCleanup=="function"&&r.__oqTrendCleanup(),r.__oqTrendHoverNodes=null,r.__oqTrendBoundSignature=n;let a=r.querySelector(".oq-overview-trend-chart");if(!a)return;let i=Xk(r.dataset.oqTrendCard);if(!i)return;let l=Nf(i.samples,i.series,{mockData:i.mock});r.__oqTrendModel=l;let c=Uc(r),u=0,d=null,g=()=>{let f=d;d=null,u=0;let m=a.getBoundingClientRect();if(!m.width||!m.height)return;let p=Number(f?.clientX);if(!Number.isFinite(p)){qf(r,l,l.points.length-1,c);return}let S=Math.min(m.width,Math.max(0,p-m.left))/m.width*l.width,q=e1(l,S);qf(r,l,q,c)},w=f=>{d=f,u||(u=window.requestAnimationFrame(g))},y=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,l1(r)};a.addEventListener("pointermove",w),a.addEventListener("pointerenter",w),a.addEventListener("pointerleave",y),a.addEventListener("focus",w),a.addEventListener("blur",y),a.addEventListener("touchstart",w,{passive:!0}),r.__oqTrendCleanup=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,a.removeEventListener("pointermove",w),a.removeEventListener("pointerenter",w),a.removeEventListener("pointerleave",y),a.removeEventListener("focus",w),a.removeEventListener("blur",y),a.removeEventListener("touchstart",w)}})}So({patchDiagnosisDom:s1});function eu(e,t,r){let n=Bi(j(t.mode,"Unknown")),a=C(t.defrost),i=vo(j(t.failures,"None")),l=Wa(i),c=n==="Verwarmen"||n==="Koelen"||a;return{mode:n,defrostActive:a,failures:i,warningFailures:l,running:c,thermalKey:n==="Koelen"?t.cooling:t.heat,schematic:w1(e,t,r,n,a,l,c)}}function Lf(e,t=null){return`<h3>${s(e)}</h3>${t?`<button class="oq-overview-hp-card-action" type="button" data-oq-action="select-hp-layout" data-hp-layout="${s(t.layout)}">${E1(t.layout==="equal"?"minus":"plus")}<span>${s(t.label)}</span></button>`:""}`}function Qc(e,t,r,n){return`<div class="oq-overview-hp-status">${_c(e,t,r,n)}</div>`}function c1(e,t){return t?"ontdooit":e==="Verwarmen"?"verwarmt":e==="Koelen"?"koelt":e==="Stand-by"?"stand-by":"onbekend"}function If(e){return!Array.isArray(e)||e.length===0?"":`<p class="oq-overview-hp-summary">${s(e.map(t=>`${t.title} ${c1(Bi(j(t.keys.mode,"Unknown")),C(t.keys.defrost))}`).join(", "))}</p>`}function u1(e){let t=o.entities[e];if(!t)return"Positie: \u2014";let r=R(e);return Number.isNaN(r)?`Positie: ${j(e)}`:`Positie: ${z(r,0,t.uom||"")}`}function d1(e){return v(e)?`Positie: ${C(e)?"Koelen/Defrost":"Verwarmen"}`:"Positie: \u2014"}function Bi(e){let t=String(e||"").trim();return!t||t==="Unknown"?"Onbekend":t==="Standby"?"Stand-by":t==="Heating"?"Verwarmen":t==="Cooling"?"Koelen":t}function Yc(e,t,r,n=!0,a="default"){return`
    ...
    `}function m1({tooltip:e,...t}){return`${Zc(t)}${ia({bind:t.bind,...e})}`}function g1({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
    ...
    `}function Jc(){if(!tu())return"";let e=Ki();return o.hpVisualMode!=="schematic"?k1(e):`
    ...
    `}function Wf(e){return!Array.isArray(e)||e.length===0?"":`
    ...
    `}function T1(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(jt(e,Wf(t)),!0):(jt(r,`
      <h3>Warmtepompen</h3>
      ${If(t)}
    `),n.classList.toggle("is-active",o.hpVisualMode==="schematic"),a.classList.toggle("is-active",o.hpVisualMode==="compact"),!0)}function Vf(){let e=Dc(),t=la(),r=ru(t),n=jf(t),a=Wf(t);return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
          ${Pc()}
          ${Hf(e)}
    ...
    `}function bE(){let e=dt(),t=pa(),r=W1(),n=No().filter(g=>!g.quick&&!g.custom),a=V1(),i=B1(a),l=Qf(a.start),c=Qf(a.end),u=o.controlReplayPeriodMenuOpen,d=e==="custom"?"Eigen periode":r.some(g=>g.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function wE(e){let t=pu(e.severity);return`
      <section class="oq-working-now oq-working-now--${s(t.tone)}">
        <div class="oq-working-now-main">
          ...
          <div class="oq-working-pill-row">
            ${vr(t.label,t.tone,"shield")}
            ${vr(e.reasonLabel,"info","target")}
            ${vr(e.sinceLabel,"context")}
          </div>
        </div>
    ...
    `}function yE(e,t){let r=pu(e.severity),n=t&&t.id===e.id,a=ih(e.kind),i=ah(e);return`
      <button
    ...
    `}function hh(e){if(!e)return"";let t=pu(e.severity),r=yr(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=sh(e),l=ah(e),c=Array.isArray(e.checks)?e.checks:r.checks;return`
    ...
    `}function SE(e,t){let r=pa(),a=e.filter(c=>!c.timelineHidden).slice(0,80),i=String(o.decisionLogError||"").trim(),l=!a.length&&!o.decisionLog&&!i;return`
    ...
    `}function xE(){if(!o.root||o.appView!=="control")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-working"):null;if(!e||!t)return!1;let r=document.activeElement;if(r&&r.closest("[data-oq-control-replay-period-menu]")&&r.matches("[data-oq-control-replay-custom-input]"))return!0;let n=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==n&&(e.className=n);let a=la();return Ze(t,wh(a),yh(a))||!0}So({patchControlReplayDom:xE});function ME(){let e=document.activeElement;return o.appView!=="settings"||!o.root?.contains(e)||!e?.dataset?.oqField?null:{field:e.dataset.oqField,modalId:e.closest("[data-oq-modal]")?.dataset.oqModal||"",selectionStart:e.selectionStart,selectionEnd:e.selectionEnd}}function $E(e){if(!e||!o.root)return;let t=document.activeElement.closest("[data-oq-modal]");if((t?.dataset.oqModal||"")!==e.modalId)return;let r=(t||o.root).querySelector(`[data-oq-field="${e.field}"]`);!r||r.disabled||(r.focus({preventScroll:!0}),typeof e.selectionStart=="number"&&typeof r.setSelectionRange=="function"&&r.setSelectionRange(e.selectionStart,e.selectionEnd))}function AE(){return`
      <section class="oq-helper-panel">
    ...
    `}function HE(){return te({modalId:"initial-load",titleId:"oq-loading-modal-title",kicker:"OpenQuatt",title:"OpenQuatt laden",backdropClass:"oq-helper-modal-backdrop--loading",modalClass:"oq-helper-modal--reconnect oq-helper-modal--loading",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `})}function RE(){return o.appView==="overview"?Vf():o.appView==="control"?vh():o.appView==="energy"?wf():o.appView==="diagnosis"?Df():o.appView==="results"?yf():AE()}function PE(){let e=lc();return`
    ...
    `}function NE(){let e=typeof document<"u"?document.activeElement:null;return!e||typeof e.matches!="function"?null:e.matches("select[data-oq-dev-control]")?e:null}function _E(e){if(!e||o.deferDevControlSelectRender)return;o.deferDevControlSelectRender=!0;let t=()=>{e.removeEventListener("blur",t),e.removeEventListener("change",t),o.deferDevControlSelectRender=!1,window.setTimeout(()=>Sh(),0)};e.addEventListener("blur",t,{once:!0}),e.addEventListener("change",t,{once:!0})}function DE(){if(o.nativeOpen||o.appView!=="settings"||o.renderedAppView!=="settings"||o.renderedSettingsGroup!==o.settingsGroup)return null;let e=document.scrollingElement||document.documentElement,t=Number(window.scrollY||e?.scrollTop||0);return!Number.isFinite(t)||t<=0?null:{group:o.settingsGroup,left:Number(window.scrollX||e?.scrollLeft||0),top:t}}function OE(e){if(!e)return;let t=(o.settingsPageScrollRestoreToken||0)+1;o.settingsPageScrollRestoreToken=t;let r=()=>{if(t!==o.settingsPageScrollRestoreToken||o.nativeOpen||o.appView!=="settings"||o.settingsGroup!==e.group)return;let n=document.scrollingElement||document.documentElement;if(!n)return;let a=Math.max(0,n.scrollHeight-n.clientHeight),i=Math.min(e.top,a);window.scrollTo({left:e.left,top:i,behavior:"auto"})};window.requestAnimationFrame(()=>{r(),window.requestAnimationFrame(r),window.setTimeout(r,80)})}function Sh(){if(!o.root)return;let e=NE();if(e){_E(e);return}let t=ME(),r=xd(o.root),n=o.systemModal==="webserver-logs"?ro():null,a=o.systemModal==="cm100-commissioning"?Vp():null,i=String(o.systemModal||"").startsWith("service-task-")?Kp():null,l=o.systemModal==="history-storage"?Up():null,c=o.systemModal==="settings-backup-restore"?Qp():null,u=o.quickStartModalOpen?Ug():null,d=DE();if(o.nativeOpen){o.root.innerHTML=`
        ${cc()}
        ${Tg()}
      `,Ps(o.root),_s(o.root,r),o.renderedAppView="native",o.renderedSettingsGroup="",o.settingsRenderSignature="",o.headerRenderSignature=xi(),Ar({mqttSensorsModalRenderSignature:""}),Cn(),jn(),$l(),ks(),qs(),no(n),Tl(a),Cl(i),xl(l),Ml(c),qc(u);return}let g=RE(),w=o.loadingEntities?`${g}${HE()}`:g,y=o.appView==="overview"||o.appView==="control"||o.appView==="energy"||o.appView==="diagnosis"||o.appView==="results";o.root.innerHTML=`
      ${cc()}
