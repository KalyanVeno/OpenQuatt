...
`)),r.toString()}function rl(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=Nw(l);if(!c)return;let u=[...a,c],d=Xd(u,t);if(a.length&&d.length>Ew){r.push({keys:n,body:i}),n=[l],a=[c],i=Xd(a,t);return}n=[...n,l],a=u,i=d}),a.length&&r.push({keys:n,body:i}),r}async function Dw(e){let t=tl(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await gn(kw,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function _w(){let e=tl(),r=await gn(Tw,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function Lw(){let e=tl(),r=await gn(Cw,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function Ow(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.hour_start_epoch_s??"",i.source??"",i.attention_count??"",e?.meta?.flash_write_count??""].join("|")}async function Iw(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<xw)return!1;o.decisionLogFetchPromise=(async()=>{let n=await Lw(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=Ow(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function Fw(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{an.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=up(r,o.entities[r],n))})}async function Ww(e){if(!e.length)return{ok:!0,message:""};try{let t=await _w();return Fw(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function K(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||an.has(b)||!Rw(b,n));if(!i.length)return;let l=i.filter(b=>an.has(b)),c=i.filter(b=>!an.has(b)),d=(await Ww(l)).ok?c:[...c,...l];if(!d.length){ep(),kn(Kt()),o.busyAction||(Zd(),o.controlError="");return}let g=Number(r.concurrency),y=Number.isFinite(g)&&g>0?Math.floor(g):Ft,w=rl(d,t),h=Math.max(1,Math.min(y,Ft)),m=[];for(let b=0;b<w.length;b+=h){let S=w.slice(b,b+h),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await Dw(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let p="";m.forEach(b=>{if(b.status!=="fulfilled"){let $=b.reason.message||String(b.reason);p||(p=$);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach($=>{if(Object.prototype.hasOwnProperty.call(E,$)){o.optionalMissingEntities&&delete o.optionalMissingEntities[$],o.entities[$]=up($,o.entities[$],E[$]);return}let P=O[$];P?.optional?k.has($)&&Pw($,n):p||(p=`${P?.name||$} ontbreekt in bulk response`)})}),ep(),kn(Kt()),p?(cp(p),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${p}`):o.busyAction||(Zd(),o.controlError="")}function up(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function ep(){o.complete=$a(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=Ku(),o.appView==="diagnosis"&&!ft()&&xt(po(),{syncMode:"replace",forceSync:!0}),o.appView||xt(Sn()||po(),{syncMode:"replace",forceSync:!0})}function jw(){let e=Ys();return o.appView==="settings"?[...new Set([...e,...Js()])]:o.appView==="energy"||o.appView==="results"?En(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?En(o.appView,{forceFast:!0}):[...new Set(e)]}function Vw(e=[]){let t=new Set(e);return(o.appView==="settings"?Js():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...En(o.appView,{includeBulk:!0}),...jt])]:o.appView==="energy"||o.appView==="results"?[...new Set([...En(o.appView,{forceFast:!0}),...jt])]:[...new Set(["setupComplete","strategy",...tt])]).filter(n=>!t.has(n))}function tp(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=xu}async function be(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||Da()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){bw(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<dn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&tp(t,e),d=t-Number(o.lastStaticEntitySyncAt||0)>=$u||o.updateInstallBusy||o.updateInstallPhaseHint?jt:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?gt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Ro:[],w=Is()?zt():[],h=n?[...ns,...tt,"setupComplete",...d]:i?[...En(a,{forceFast:l,includeBulk:c}),...d]:r==="settings"?[...new Set([...Js(),...w,...d])]:c?["setupComplete",...d,...tt,"strategy",...Ao,...et,...ae()?ge.map(m=>m.key):kt]:["setupComplete",...tt,"strategy",...d];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let m=o.deviceReconnectMode,p=Hw(t,e)?await Aw():{ok:!0,message:""};if(!p.ok){cp(p.message),n||f();return}if(await K([...new Set([...h,...g,...y])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?Wt:Ft}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),d.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&rp();let b=m!==o.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="control"||a==="diagnosis"?await Fs():!1,E=S?!1:o.appView==="results"?await La():!1,k=a==="control"?await Iw({force:e.forceDecisionLog===!0}):!1,$=S?!1:Is()?(await Promise.all([Ad(),La({metaOnly:!0})])).some(Boolean):!1,P=S||!nd()?!1:await Fo(),N=S||!ad()?!1:await Wo(),V=S||!zd()?!1:await Ko({force:o.systemModal==="mqtt-sensors"}),A=xd();if(S&&!o.nativeOpen&&sp(Zs(a)),b){f();return}if(q&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){f();return}if(E&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){f();return}if(k&&o.appView==="control"){Ws()||f();return}if($&&o.appView==="settings"){f();return}if(P&&(o.systemModal==="login"||Vt())){f();return}if(N&&o.appView==="settings"){f();return}if(o.systemModal==="mqtt-sensors"){Ia()!==o.mqttSensorsModalRenderSignature&&f();return}if(V&&o.appView==="settings"){f();return}let F=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(A!==o.headerRenderSignature)if(F&&Ds())o.headerRenderSignature=A;else{f();return}else Ds();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){bn()!==o.settingsRenderSignature&&f();return}if(F)return;if(o.appView==="settings"){let H=bn();if(H!==o.settingsRenderSignature&&!o.focusedField){f();return}if(!Md()){o.focusedField||f();return}o.focusedField&&(o.settingsRenderSignature=H);return}if(o.appView==="diagnosis"){_d()||f();return}if(o.appView==="control"){Ws()||f();return}if(o.appView==="energy"){Pd()||f();return}if(o.appView==="results"){Nd()||f();return}Dd()||f()}catch(m){n||(o.controlError=`Helperstatus kon niet worden geladen. ${m.message}`,f())}finally{o.entitySyncInFlight=!1;let m=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,m&&!o.nativeOpen&&window.setTimeout(()=>{be(m)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!m&&tp(Date.now())&&window.setTimeout(()=>{be({forceBulk:!0})},fw)}}function Mt(){return{}}function Bw(){let e=String(T("hpGeneration")||"").trim();return e||""}function Tn(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function Kw(){if(!Array.isArray(wr))return"";if(wr.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return wr.every(t=>e[t])?"single":""}function nl(e){let t=Tn(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function zw(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function ne(){let e=Tn(T("installationTopology"));if(e==="single"||e==="duo")return nl(e);let t=Tn(Mt().installation);if(t==="single"||t==="duo")return nl(t);let r=Kw();return r?nl(r):zw()}function Ht(){let e=ne(),t=Bw();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Ue(){return"OpenQuatt"}function al(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function rt(e=fo()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Uo(e=ne()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function Cn(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Mt().hardwareProfile||e).trim().toLowerCase()}function fo(){return al(T("connectionText")||Mt().connection)}function dp(){let e=fo();return e==="wifi"?"eth":e==="eth"?"wifi":""}function pp(){let e=ne();return e==="single"?"duo":e==="duo"?"single":""}function Er(e=ne(),t=fo()){let r=Uo(e),n=Cn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${rt(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${rt(t)}`:n==="waveshare"?`Waveshare ${r} ${rt(t)}`:`${Ue()} ${r} ${rt(t)}`}function il(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function Fa(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${il()}`}function Uw(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Gw(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Tr(){let e=Gw(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Uw(o.entities.uptime);if(n==="d")return Io(e*1440);if(n==="h")return Io(e*60);if(n==="s")return Io(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(Mt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Io((Date.now()-r)/6e4)}function Cr(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(Mt().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function Go({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let w=e();if(!w)return null;let h={scrollTop:w.scrollTop};if((i||l)&&(h.scrollHeight=w.scrollHeight),l){let m=w.scrollHeight-w.scrollTop-w.clientHeight;h.stickToBottom=m<c}return a&&(h.identity=a(w)),h}function d(w){if(!w)return;let h=e();if(!h||a&&a(h)!==w.identity)return;if(w.stickToBottom){h.scrollTop=h.scrollHeight;return}let m=i?h.scrollHeight-Number(w.scrollHeight||0):0;h.scrollTop=Math.max(0,Number(w.scrollTop||0)+m)}function g(w,h=!0){if(!w)return;let m=Number(t()||0)+1;r(m);let p=()=>{Number(t()||0)!==m||!n()||d(w)};if(h){window.requestAnimationFrame(p);return}p()}function y(){r(Number(t()||0)+1)}return{capture:u,invalidate:y,queue:g,restore:d}}function $e(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function ho({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function sl(e,t,r="",n=""){let a=t?r:n;return a?`<p data-oq-switch-copy="${s(e)}" data-on-copy="${s(r)}" data-off-copy="${s(n)}">${s(a)}</p>`:""}function mp(e,t,r,n="",a="",i=""){if(!v(e))return"";let l=!!T(e),c=o.loadingEntities||o.busyAction===`switch-${e}`;return we(e,t,r,`
    ...
    `,a)}function ja(e,t,r){if(!v(e))return"";let n=!!T(e),a=o.loadingEntities||o.busyAction===`switch-${e}`;return`
...
`)}async function Ry(){let e=Ay();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",f();return}try{if(!await lo(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&f()}var Py={"open-webserver-log-modal":()=>gy(),"clear-webserver-log-output":()=>Hr(),"copy-webserver-log-output":()=>Ry()};function Mp(e){return ie(Py,e)}function Hp(){let e=$r();return te({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
    ...
    `})}function Ny(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Pp(e){return Ny().includes(e)}function za(){let e=Cn(),t=ne(),r=fo(),n=dp();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:v("firmwareUpdateTarget")&&Pp("alternate connection")&&v("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:rt(r),targetLabel:rt(n),currentBuildLabel:Er(t,r),targetBuildLabel:Er(t,n)}}function Ua(){let e=Cn(),t=ne(),r=pp(),n=fo(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:v("firmwareUpdateTarget")&&Pp("alternate topology")&&v("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Uo(t),targetLabel:Uo(r),currentBuildLabel:Er(t,n),targetBuildLabel:Er(r,n)}}function Zo(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function Rr(){let e=Cn(),t=ne(),r=fo(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${rt(r)}`}}function $n(e=Zo(),t=Rr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function vo(){if(at())return"Controleren";let e=Zt();if(e)return e.phaseLabel;if(vl())return"Bijgewerkt";if(Ar())return"Bezig";if(Xo())return"Beschikbaar";let t=Dp();if(t!==null&&t<=0)return"Actueel";let r=Mt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||ql()?"Actueel":ye()?"Nog niet gecontroleerd":"\u2014"}function ye(){return o.entities.firmwareUpdate||null}function Ga(){let e=ye();return e?String(e.state??e.value??"").trim().toLowerCase():""}function Dy(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Qa(){return Dy().toLowerCase()}function Np(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function wl(){let e=String(o.updateInstallTargetVersion||"").trim(),t=ue();return!e||!t?!1:Mn(t,e)>=0}function _y(e=ye()||{}){let t=So(e),r=ue(e);return!t||!r?!1:Mn(r,t)>=0}function yl(){return(wl()||_y())&&!at()&&!xn()&&!Xo()}function vl(){return(o.updateInstallCompleted||yl())&&!at()&&!Zt()&&!Xo()}function Pr(){xe({updateInstallBusy:!1,updateInstallTargetVersion:"",updateInstallPhaseHint:"",updateInstallProgressHint:Number.NaN,updateInstallMode:"",updateInstallTargetConnection:"",updateInstallTargetTopology:""}),Vy()}function He(){xe({updateManualUploadFile:null,updateManualUploadFileName:"",updateManualUploadError:""})}function Qe(e={}){xe({...e.clearPr?{updateTestFirmwarePr:""}:{},updateTestFirmwareConfirmed:!1,updateTestFirmwareError:"",updateTestFirmwareBuild:null})}function Ly(){let e=Qa(),t=Np();if(e==="starting"||e==="uploading"||e==="rebooting"){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(wl()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function xn(){let e=Qa();return e==="starting"||e==="uploading"||e==="rebooting"}function Zt(){Ly();let e=Qa(),t=e==="starting"||e==="uploading"||e==="rebooting",r=t?e:o.updateInstallPhaseHint,n=Np(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!xn()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Ue()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${rt(o.updateInstallTargetConnection)}-build wordt nu naar ${Ue()} verzonden.`:o.updateInstallMode==="topology-switch"?`De ${Er(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Ue()} verzonden.`:`Firmware wordt nu naar ${Ue()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Ue()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${rt(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${Uo(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Ue()}.`}}function So(e=ye()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function ue(e=ye()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function Oy(e=ye()||{}){let t=ue(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:Mn(t,r)>0}function Sl(e=ye()||{},t=yo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=So(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function Ap(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function Mn(e,t){let r=Ap(e),n=Ap(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function Ar(){if(yl())return!1;let e=Ga();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function at(){let e=Ga();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Xo(){let e=Ga();if(!Sl())return!1;let t=Dp();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Mt().updateAvailable===!0}function ql(){let e=Ga();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Oy()}function kl(){let e=ye()||{},t=ue(e)||"\u2014",r=Sl(e)?So(e):"",n=r?Mn(r,t):null;return!at()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function Dp(){let{current:e,latest:t}=kl();return e==="\u2014"||t==="\u2014"?null:Mn(t,e)}function _p(e=yo()){let t=String(e||"").trim().toLowerCase();return Ui[t]||Ui.main}function Iy(){let e=String((ye()||{}).release_url||"").trim(),t=_p();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function Fy(){return Ue()}function yo(){return String(T("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function Wy(){return kl().latest!=="\u2014"}function Rp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function jy(e,t=ye()||{}){let r=Rp(e);return r?Rp(`${t.title||""} ${t.summary||""}`).includes(r):!0}function Lp(e){return new Promise(t=>window.setTimeout(t,e))}function Hn(e=_o){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,gd(),typeof At=="function"&&At(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&ks()},e)}function Vy(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||ks()}function Op(){return o.deviceReconnectMode?te({modalId:"reconnect",titleId:"oq-reconnect-modal-title",kicker:"Systeem",title:Xu(),modalClass:"oq-helper-modal--reconnect",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `}):""}function An(e=yo()){let t=ye()||{},r=ue(t);o.entities.firmwareUpdate={...t,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:_p(e),current_version:r}}async function Rn(e={}){let t=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await Lp(r===0?900:1200),await K(ln,"all",{forceMissing:!0});let n=Sl(),a=!t||jy(t),i=Wy(),l=at(),c=vo();if(n&&a&&(i||!l&&c!=="Nog niet gecontroleerd"))return!0}return!1}async function Nr(e={}){let t=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await Lp(a===0?r:n);try{if(await K(ln,"all",{forceMissing:!0}),Qa()==="rebooting"&&uo("ota"),f(),o.updateInstallMode==="connection-switch"){let i=al(o.updateInstallTargetConnection);if(i&&fo()===i&&!xn()&&!Ar())return!0}else if(o.updateInstallMode==="topology-switch"){let i=Tn(o.updateInstallTargetTopology);if(i&&ne()===i&&!xn()&&!Ar())return!0}else if(wl()||yl()||ql()&&!xn()&&!Ar())return!0}catch{t||(o.controlNotice="Wachten tot het device opnieuw is opgestart...",f(),t=!0)}}return!1}function By(){let e=yo(),t=Zt();if(t)return t.copy;if(vl()){let r=o.updateInstallCompletedVersion||ue()||yo();return`${Ue()} draait nu op ${r}.`}return Ar()?`OTA-update wordt voorbereid voor ${Ue()}. Het device kan kort herstarten.`:at()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:Xo()?"Er staat een nieuwere firmware klaar.":ql()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function Ip(){return!!(o.firmwareAdvancedOpen||o.firmwareConnectionSwitchOpen||o.firmwareTopologySwitchOpen||o.updateManualUploadOpen||o.updateTestFirmwareOpen)}function Ka(e,t,r,n,a=!1){return`
      <button
    ...
    `}function zy(){let e=za();if(!e||!o.firmwareConnectionSwitchOpen)return"";let r=!!(Zt()||o.updateInstallBusy||at()),n=!!o.firmwareConnectionSwitchConfirmed,a=e.targetConnection==="eth",i=!e.canSwitch,l=a?"Sluit eerst de netwerkkabel aan. Na de herstart verdwijnt Wi-Fi uit deze firmware.":"Na de herstart verdwijnt Ethernet uit deze firmware. Als er geen Wi-Fi-gegevens bekend zijn, start het OpenQuatt fallback access point.",c=i?'<p class="oq-helper-modal-note oq-helper-modal-note--muted">Verbindingswissel wordt geladen. Open deze modal opnieuw of wacht een moment als de knop disabled blijft.</p>':"";return`
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:zt(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,To(),o.appView==="results"&&ar({force:!0}).then(()=>f())})},"select-energy-history-import-file":()=>dS(),"clear-energy-history-import-file":()=>{Cm(),f()},"import-energy-history-file":()=>pS(),"export-energy-history":()=>Jv(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",f();let e=di({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&f()});return To([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",f()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",f()},"open-storage-energy":()=>{o.settingsStoragePage="energy",f()},"back-storage-overview":()=>{o.settingsStoragePage="overview",f()},"download-settings-backup":()=>vS(),"open-settings-backup-import":()=>{o.systemModal="settings-backup-import",f()},"confirm-settings-backup-restore":()=>SS()};function _m(e,t){return ie(qS,e,t)}function Bl(e,t,r="",n=!1){return`
    ...
    `}function TS(){return o.trendHistoryMetadata&&typeof o.trendHistoryMetadata=="object"?o.trendHistoryMetadata:{}}function Kr(){return!!o.trendHistoryMetadataSignature}function zr(){return!!(o.energyHistoryRaw||o.energyHistorySignature)}function it(e){return e?"Niet geladen":"Laden..."}function Fe(e,t="\u2014"){if(v(e))return Me(e);let r=CS(e);return r==null||r===""?t:r}function CS(e){let t=TS();if(e==="trendHistoryFlashAvailable")return Kr()?t.available||"Alleen live":it(o.trendHistoryMetadataError);if(e==="trendHistoryFlashOldest")return Kr()?t.oldest||"Geen data":it(o.trendHistoryMetadataError);if(e==="trendHistoryFlashNewest")return Kr()?t.newest||"Geen data":it(o.trendHistoryMetadataError);if(e==="trendHistoryFlashLastFlush")return Kr()?t.lastFlush||"Geen data":it(o.trendHistoryMetadataError);if(e==="trendHistoryFlashSize")return Kr()?fi(t.sizeKb):it(o.trendHistoryMetadataError);if(e==="trendHistoryFlashWrites")return Kr()?zn(t.writes):it(o.trendHistoryMetadataError);let r=jm(),a=String(o.energyHistoryRaw||"").includes("@day_retention|");return e==="lifetimeEnergyHistoryAvailable"?zr()?a&&!r.dayPartitionAvailable?"Niet beschikbaar":Im(r.storedDayCount,"Geen data"):it(o.energyHistoryError):e==="lifetimeEnergyHistoryOldest"?zr()?Lm(r.oldestDateKey):it(o.energyHistoryError):e==="lifetimeEnergyHistoryNewest"?zr()?Lm(r.newestDateKey):it(o.energyHistoryError):e==="lifetimeEnergyHistoryLastWrite"?zr()?gi(r.dayLastWriteTimestampS):it(o.energyHistoryError):e==="lifetimeEnergyHistorySize"?zr()?fi(r.dayStorageKb):it(o.energyHistoryError):e==="lifetimeEnergyHistoryWrites"?zr()?zn(r.dayWriteCount):it(o.energyHistoryError):null}function Im(e,t="Geen data"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:`${Math.round(r)} ${Math.round(r)===1?"dag":"dagen"}`}function Fm(e,t="Nog geen historie"){let r=Math.max(0,Math.round(Number(e)||0));return r<=0?t:`${r} ${r===1?"gebeurtenis":"gebeurtenissen"}`}function Wm(){return o.decisionLogStorageMetadata&&typeof o.decisionLogStorageMetadata=="object"?o.decisionLogStorageMetadata:{}}function fi(e,t="\u2014"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:`${Math.round(r)} kB`}function zn(e,t="0"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:String(Math.round(r))}function Lm(e){let t=re(e);return t?t.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit",year:"numeric"}):"Geen data"}function gi(e,t="Geen data"){let r=Number(e);if(!Number.isFinite(r)||r<=0)return t;let n=new Date(r*1e3),a=n.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit"}),i=n.toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"});return`${a} ${i}`}function jm(){return ii(o.energyHistoryRaw)}function xS(e){let t=Array.isArray(e.items)?e.items:[];return`
    ...
    `}function Bm(){let e=["diagnosis","decision-log","energy"].includes(o.settingsStoragePage)?o.settingsStoragePage:"overview",t=v("trendHistoryEnabled")&&C("trendHistoryEnabled"),r=t&&v("trendHistoryFlashEnabled")&&C("trendHistoryFlashEnabled"),n=v("decisionLogHistoryEnabled"),a=n&&C("decisionLogHistoryEnabled"),i=Wm(),l=Fm(i.storedEvents),c=v("lifetimeEnergyHistoryEnabled"),u=c&&C("lifetimeEnergyHistoryEnabled"),d=c?Fe("lifetimeEnergyHistoryAvailable","Geen data"):"Geen data",g=Om(d),y=v("lifetimeEnergyHistoryClear")&&!["Geen data","\u2014"].includes(d),w=t&&v("trendHistoryFlush"),h=a&&v("decisionLogHistoryFlush"),m=v("lifetimeEnergyHistoryCapture"),p=jm(),b=String(o.energyHistoryRaw||"").includes("@hour_retention|"),S=b&&!p.hourPartitionAvailable,q=b?S?"Alleen live":Im(p.hourStoredDayCount,"Geen uurdata"):"Laden...",E=b&&!S?fi(p.hourStorageKb):"\u2014",k=b&&!S?zn(p.hourWriteCount):"\u2014",$=b&&!S?gi(p.hourLastWriteTimestampS):"Geen data",P=e==="overview"?"":`
      ...
      <p class="oq-settings-storage-footnote"><strong>Goed om te weten:</strong> gegevens die worden bewaard, blijven beschikbaar na een herstart. Tijdelijke gegevens bestaan alleen zolang de controller online is.</p>`;return e==="diagnosis"?(H="Diagnosegegevens",L="Beheer technische meetreeksen voor diagnosegrafieken en hulp bij problemen.",_=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
          ${mi("trendHistoryEnabled","Recente diagnosegegevens","Bewaar de laatste meetpunten zolang de controller online is.","Deze gegevens zijn tijdelijk en verdwijnen na een herstart.","Nieuwe tijdelijke diagnosegegevens worden niet bijgehouden.","Tijdelijk")}
          ${mi("trendHistoryFlashEnabled","Diagnosegeschiedenis bewaren","Bewaar recente diagnosegegevens ook na een herstart of update.","OpenQuatt slaat ongeveer ieder uur een blok op.","Bestaande geschiedenis blijft staan.","Blijft bewaard na herstart")}
        ...
        <div class="oq-settings-storage-domain-rows">
          ${mi("decisionLogHistoryEnabled","Beslisloghistorie bewaren","Bewaar exacte momenten en redenen uit de beslislog.","De laatste zeven dagen blijven beschikbaar na een herstart of update.","De actuele beslislog blijft tijdelijk beschikbaar; bestaande historie blijft staan.","Blijft bewaard na herstart")}
      ...
      ${v("decisionLogHistoryClear")?`<details class="oq-settings-storage-advanced"><summary>Geavanceerd</summary><div class="oq-settings-storage-inline-action oq-settings-storage-inline-action--danger"><div><h4>Beslisloghistorie wissen</h4><p>Verwijder alle bewaarde gebeurtenissen. De actuele beslislog blijft staan.</p></div>${Kn("decisionLogHistoryClear","Historie wissen","clear-decision-log-history",{disabled:Number(i.storedEvents||0)<=0,buttonClass:"oq-helper-button oq-helper-button--warning",busyLabel:"Wissen..."})}</div></details>`:""}`):e==="energy"&&(H="Energiehistorie",L="Beheer dagtotalen en uurdetail voor de Resultatenpagina.",_=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
          ${mi("lifetimeEnergyHistoryEnabled","Dagtotalen bewaren","Bewaar elke dag een samenvatting van je energiegegevens.","Resultaten blijven beschikbaar na een herstart of update.","Nieuwe dagtotalen worden niet bewaard; bestaande historie blijft staan.","Blijft bewaard na herstart")}
    ...
    `}Cd({getSignature:hi,patch:RS});function eg(){if(o.systemModal==="login")return am();if(o.systemModal==="api-security")return nm();if(o.systemModal==="mqtt")return Wd();if(o.systemModal==="mqtt-sensors")return jd();if(o.systemModal==="connectivity"){let e=AS();return te({modalId:"system",titleId:"oq-system-modal-title",kicker:"Systeem",title:"Connectiviteit",closeAction:"close-system-modal",closeLabel:"Sluit systeem-popup",bodyMarkup:`
        ...
        `})}return""}function NS(e=!1){let t=xa();return t?`
    ...
    `:"";return oe("Bewaking","Installatiebewaking","Lokale diagnose voor compressorstarts, hydrauliek en verbindingen. Hiervoor is geen Home Assistant nodig.",`
      ...
      `)}function nc(){if(!v("hpGeneration"))return"";let e={V1:{copy:"Voor Quatt V1 en Quatt V1 + V1.5 combinaties.",image:Ma,alt:"Quatt Hybrid V1 en V1.5",infoTitle:"V1",infoCopy:`Model: AMM4
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=Gt(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function cg(){let e=Ht(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&Gt(t).length>0;return!e&&!r?"":oe("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function pg(){let e=vo(),t=Fa(),r=o.busyAction==="restartAction";return oe("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ...
          ${Oe({dataValue:"webserverLog",label:"Logboek",value:Ba(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
          ...
          ${Oe({dataValue:"debugRecording",label:"Debugopname",value:Pn(),note:Za(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
              data-oq-action="open-debug-recording-modal"
            >
    ...
    `:""}function cc(){let e=ka.map(mq).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function Pg(e=cc()){return he(e)}function Ng(e=cc()){return`
...
`)}function Lq(e,t=""){if(!e.length)return`
    ...
    `}function ea(e,t=!1){return e.map(r=>Wq({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?ot(r.key):r.value,status:t})).join("")}function hc(e){return`
    ...
    `}function oa({bind:e,modifier:t,x:r,y:n,width:a,kicker:i,detail:l,detailBind:c="",icon:u="heater",direction:d="down"}){let y=r+26,w=n+22,h=c?` data-oq-bind="${s(c)}"`:"",m="";if(d==="up"){let p=r+Math.round(a*.52);m=`M${p-6} ${n} L${p} ${n-8} L${p+6} ${n} Z`}else if(d==="left"){let p=n+Math.round(22);m=`M${r} ${p-6} L${r-8} ${p} L${r} ${p+6} Z`}else if(d==="right"){let p=n+Math.round(22);m=`M${r+a} ${p-6} L${r+a+8} ${p} L${r+a} ${p+6} Z`}else{let p=r+Math.round(a*.52);m=`M${p-6} ${n+44} L${p} ${n+44+8} L${p+6} ${n+44} Z`}return`
      <g
    ...
    `}function Tk({tooltip:e,...t}){return`${Dc(t)}${oa({bind:t.bind,...e})}`}function Ck({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
    ...
    `}function ff(e,t,r,n="normal",a=null){if(!v(t.power))return"";let i=Oc(e,t,r),{mode:l,defrostActive:c,running:u,thermalKey:d}=i,g=i.schematic;return o.hpVisualMode==="schematic"?`
      ...
      <section class="oq-overview-hp oq-overview-hp--${s(r)} oq-overview-hp--${s(n)}" data-oq-hp-panel="${s(e)}">
        ...
        <div class="oq-overview-hp-stats">
          ${ea([{key:t.power,label:"Stroomverbruik",tone:"blue",note:"elektrisch verbruik"},{key:d,label:g.heatLabel,tone:"orange",note:g.heatDescription},{label:g.efficiencyLabel,value:g.efficiencyText,tone:"green",note:"actueel"}])}
        </div>
    ...
    `}function Lk(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(_t(e,bf(t)),!0):(_t(r,`
      <h3>Warmtepompen</h3>
      ${gf(t)}
    `),n.classList.toggle("is-active",o.hpVisualMode==="schematic"),a.classList.toggle("is-active",o.hpVisualMode==="compact"),!0)}function wf(){let e=vc(),t=na(),r=Wc(t),n=hf(t),a=bf(t);return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
          ${bc()}
          ${af(e)}
    ...
    `}function M1(){let e=ct(),t=la(),r=e1(),n=$o().filter(g=>!g.quick&&!g.custom),a=t1(),i=o1(a),l=Ef(a.start),c=Ef(a.end),u=o.controlReplayPeriodMenuOpen,d=e==="custom"?"Eigen periode":r.some(g=>g.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function P1(e,t){let r=la(),a=e.filter(c=>!c.timelineHidden).slice(0,80),i=String(o.decisionLogError||"").trim(),l=!a.length&&!o.decisionLog&&!i;return`
