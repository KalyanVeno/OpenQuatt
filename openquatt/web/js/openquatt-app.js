...
`)),r.toString()}function hl(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=my(l);if(!c)return;let u=[...a,c],d=yp(u,t);if(a.length&&d.length>ry){r.push({keys:n,body:i}),n=[l],a=[c],i=yp(a,t);return}n=[...n,l],a=u,i=d}),a.length&&r.push({keys:n,body:i}),r}async function gy(e){let t=gl(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await Sn(oy,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function fy(){let e=gl(),r=await Sn(ny,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function hy(){let e=gl(),r=await Sn(ay,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function by(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.hour_start_epoch_s??"",i.source??"",i.attention_count??"",e?.meta?.flash_write_count??""].join("|")}async function wy(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<iy)return!1;o.decisionLogFetchPromise=(async()=>{let n=await hy(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=by(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function yy(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{fn.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=Ap(r,o.entities[r],n))})}async function vy(e){if(!e.length)return{ok:!0,message:""};try{let t=await fy();return yy(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function K(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||fn.has(b)||!dy(b,n));if(!i.length)return;let l=i.filter(b=>fn.has(b)),c=i.filter(b=>!fn.has(b)),d=(await vy(l)).ok?c:[...c,...l];if(!d.length){vp(),An(Jt()),o.busyAction||(wp(),o.controlError="");return}let g=Number(r.concurrency),w=Number.isFinite(g)&&g>0?Math.floor(g):Kt,y=hl(d,t),f=Math.max(1,Math.min(w,Kt)),m=[];for(let b=0;b<y.length;b+=f){let S=y.slice(b,b+f),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await gy(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let p="";m.forEach(b=>{if(b.status!=="fulfilled"){let M=b.reason.message||String(b.reason);p||(p=M);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(M=>{if(Object.prototype.hasOwnProperty.call(E,M)){o.optionalMissingEntities&&delete o.optionalMissingEntities[M],o.entities[M]=Ap(M,o.entities[M],E[M]);return}let P=L[M];P?.optional?k.has(M)&&py(M,n):p||(p=`${P?.name||M} ontbreekt in bulk response`)})}),vp(),An(Jt()),p?($p(p),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${p}`):o.busyAction||(wp(),o.controlError="")}function Ap(e,t={},r={}){let n={...t||{},...r||{}};return L[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function vp(){o.complete=Na(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=sd(),o.appView==="diagnosis"&&!bt()&&Ht(yo(),{syncMode:"replace",forceSync:!0}),o.appView||Ht(Mn()||yo(),{syncMode:"replace",forceSync:!0})}function Sy(){let e=cl();return o.appView==="settings"?[...new Set([...e,...ul()])]:o.appView==="energy"||o.appView==="results"?Hn(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?Hn(o.appView,{forceFast:!0}):[...new Set(e)]}function qy(e=[]){let t=new Set(e);return(o.appView==="settings"?ul():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...Hn(o.appView,{includeBulk:!0}),...Ut])]:o.appView==="energy"||o.appView==="results"?[...new Set([...Hn(o.appView,{forceFast:!0}),...Ut])]:[...new Set(["setupComplete","strategy",...ot])]).filter(n=>!t.has(n))}function Sp(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=Vu}async function we(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||Wa()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){Yw(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<wn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&Sp(t,e),d=t-Number(o.lastStaticEntitySyncAt||0)>=Bu||o.updateInstallBusy||o.updateInstallPhaseHint?Ut:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?ht:[],w=o.quickStartModalOpen&&o.currentStep==="setup"?Gt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Lo:[],f=Js()?Xt():[],m=n?[...fs,...ot,"setupComplete",...d]:i?[...Hn(a,{forceFast:l,includeBulk:c}),...d]:r==="settings"?[...new Set([...ul(),...f,...d])]:c?["setupComplete",...d,...ot,"strategy",...Oo,...tt,...ae()?fe.map(p=>p.key):xt]:["setupComplete",...ot,"strategy",...d];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let p=o.deviceReconnectMode,b=cy(t,e)?await uy():{ok:!0,message:""};if(!b.ok){$p(b.message),n||h();return}if(await K([...new Set([...m,...w,...g,...y])],n?"state":r==="settings"||w.length?"all":"state",{concurrency:l&&i?zt:Kt}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),d.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&kp();let S=p!==o.deviceReconnectMode,q=l&&i,E=q?!1:a==="overview"||a==="control"||a==="diagnosis"?await Xs():!1,k=q?!1:o.appView==="results"?await Va():!1,M=a==="control"?await wy({force:e.forceDecisionLog===!0}):!1,P=q?!1:Js()?(await Promise.all([Zd(),Va({metaOnly:!0})])).some(Boolean):!1,N=q||!vd()?!1:await Ko(),V=q||!Sd()?!1:await zo(),H=q||!pp()?!1:await Yo({force:o.systemModal==="mqtt-sensors"}),F=Ud();if(q&&!o.nativeOpen&&xp(dl(a)),S){h();return}if(E&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){h();return}if(k&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){h();return}if(M&&o.appView==="control"){el()||h();return}if(P&&o.appView==="settings"){h();return}if(N&&(o.systemModal==="login"||Yt())){h();return}if(V&&o.appView==="settings"){h();return}if(o.systemModal==="mqtt-sensors"){Ka()!==o.mqttSensorsModalRenderSignature&&h();return}if(H&&o.appView==="settings"){h();return}let A=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(F!==o.headerRenderSignature)if(A&&Gs())o.headerRenderSignature=F;else{h();return}else Gs();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){En()!==o.settingsRenderSignature&&h();return}if(A)return;if(o.appView==="settings"){let O=En();if(O!==o.settingsRenderSignature&&!o.focusedField){h();return}if(!Qd()){o.focusedField||h();return}o.focusedField&&(o.settingsRenderSignature=O);return}if(o.appView==="diagnosis"){op()||h();return}if(o.appView==="control"){el()||h();return}if(o.appView==="energy"){Xd()||h();return}if(o.appView==="results"){ep()||h();return}tp()||h()}catch(p){n||(o.controlError=`Helperstatus kon niet worden geladen. ${p.message}`,h())}finally{o.entitySyncInFlight=!1;let p=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,p&&!o.nativeOpen&&window.setTimeout(()=>{we(p)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!p&&Sp(Date.now())&&window.setTimeout(()=>{we({forceBulk:!0})},Gw)}}function Pt(){return{}}function ky(){let e=String(T("hpGeneration")||"").trim();return e||""}function Jo(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function Ey(){if(!Array.isArray(Mr))return"";if(Mr.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return Mr.every(t=>e[t])?"single":""}function bl(e){let t=Jo(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Ty(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function oe(){let e=Jo(T("installationTopology"));if(e==="single"||e==="duo")return bl(e);let t=Jo(Pt().installation);if(t==="single"||t==="duo")return bl(t);let r=Ey();return r?bl(r):Ty()}function Nt(){let e=oe(),t=ky();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Ge(){return"OpenQuatt"}function Rn(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function nt(e=at()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Xo(e=oe()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function _r(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Pt().hardwareProfile||e).trim().toLowerCase()}function at(){return Rn(T("connectionText")||Pt().connection)}function Hp(){let e=at();return e==="wifi"?"eth":e==="eth"?"wifi":""}function Rp(){let e=oe();return e==="single"?"duo":e==="duo"?"single":""}function qo(e=oe(),t=at()){let r=Xo(e),n=_r();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${nt(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${nt(t)}`:n==="waveshare"?`Waveshare ${r} ${nt(t)}`:`${Ge()} ${r} ${nt(t)}`}function wl(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function za(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${wl()}`}function Cy(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function xy(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Dr(){let e=xy(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Cy(o.entities.uptime);if(n==="d")return Bo(e*1440);if(n==="h")return Bo(e*60);if(n==="s")return Bo(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(Pt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Bo((Date.now()-r)/6e4)}function Or(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(Pt().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function ko({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let y=e();if(!y)return null;let f={scrollTop:y.scrollTop};if((i||l)&&(f.scrollHeight=y.scrollHeight),l){let m=y.scrollHeight-y.scrollTop-y.clientHeight;f.stickToBottom=m<c}return a&&(f.identity=a(y)),f}function d(y){if(!y)return;let f=e();if(!f||a&&a(f)!==y.identity)return;if(y.stickToBottom){f.scrollTop=f.scrollHeight;return}let m=i?f.scrollHeight-Number(y.scrollHeight||0):0;f.scrollTop=Math.max(0,Number(y.scrollTop||0)+m)}function g(y,f=!0){if(!y)return;let m=Number(t()||0)+1;r(m);let p=()=>{Number(t()||0)!==m||!n()||d(y)};if(f){window.requestAnimationFrame(p);return}p()}function w(){r(Number(t()||0)+1)}return{capture:u,invalidate:w,queue:g,restore:d}}function $e(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function Eo({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function yl(e,t,r="",n=""){let a=t?r:n;return a?`<p data-oq-switch-copy="${s(e)}" data-on-copy="${s(r)}" data-off-copy="${s(n)}">${s(a)}</p>`:""}function Pp(e,t,r,n="",a="",i=""){if(!v(e))return"";let l=!!T(e),c=o.loadingEntities||o.busyAction===`switch-${e}`;return ye(e,t,r,`
  ...
  `}function Sl(e,t,r,n=""){if(!v(e))return"";let a=yn(T(e));return ye(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function re(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var Dp=250;function Op(){return[]}function nr(){return typeof window>"u"?!1:Op().length>0}function $y(){return`${Pe()}/events`}function Ay(){return`${Pe()}/openquatt/logs/recent`}function Hy(){return`${Pe()}/openquatt/logs/clear`}function Qa(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Ya(){return o.nativeOpen?"Niet beschikbaar":nr()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function Ry(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function Lp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return Ry(t)}function Py(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Ny(){return o.nativeOpen?"Niet beschikbaar":nr()?Qa()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":Qa()?"Buffer aan":"Buffer uit"}function _y(){return Qa()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function ql(){return o.entities?.debugLevel||null}function Ip(e=ql()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function Dy(e=ql()){let t=String(e?.value??e?.state??"").trim(),r=Ip(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function Fp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function Oy(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function Wp(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(Oy);let n=[];for(let a of r){let i=n[n.length-1]||null;Fp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-Dp)}function kl(e,t={}){let r=nv(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:av(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function Ly(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function Iy(){let e=Op(),t=e.length;return e.map((r,n)=>kl(r,{receivedAt:Ly(n,t),seq:n+1}))}function Fy(){let e=Al();e&&(e.scrollTop=e.scrollHeight)}var El=ko({getScroller:Al,getToken:()=>o.webServerLogScrollRestoreToken,setToken:e=>{o.webServerLogScrollRestoreToken=e},isActive:()=>o.systemModal==="webserver-logs",preserveGrowth:!0,stickToBottom:!0}),ro=El.capture,no=El.queue;function Wy(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}var jp=ko({getScroller:Wy,getToken:()=>o.cm100CommissioningScrollRestoreToken,setToken:e=>{o.cm100CommissioningScrollRestoreToken=e},isActive:()=>o.systemModal==="cm100-commissioning",preserveGrowth:!0,stickToBottom:!0}),Vp=jp.capture,Tl=jp.queue;function jy(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}var Bp=ko({getScroller:jy,getToken:()=>o.serviceTaskModalScrollRestoreToken,setToken:e=>{o.serviceTaskModalScrollRestoreToken=e},isActive:()=>String(o.systemModal||"").startsWith("service-task-")}),Kp=Bp.capture,Cl=Bp.queue;function Vy(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}var zp=ko({getScroller:Vy,getToken:()=>o.historyStorageModalScrollRestoreToken,setToken:e=>{o.historyStorageModalScrollRestoreToken=e},isActive:()=>o.systemModal==="history-storage"}),Up=zp.capture,xl=zp.queue;function By(){return o.root?o.root.querySelector("[data-oq-settings-backup-restore-scroller]"):null}var Gp=ko({getScroller:By,getToken:()=>o.settingsBackupRestoreScrollRestoreToken,setToken:e=>{o.settingsBackupRestoreScrollRestoreToken=e},isActive:()=>o.systemModal==="settings-backup-restore"}),Qp=Gp.capture,Ml=Gp.queue;async function Ir(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||ro(),r=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=r,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let n=await window.fetch(Ay(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==r)return;o.webServerLogCsrfToken=String(a.csrf_token||"");let i=zy(a);o.webServerLogHistoryLoaded=!0,i.length>0&&(Wp(i,{prepend:!0}),o.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),o.webServerLogRecentAnchorAt=Date.now())}catch(n){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(h(),no(t))}}function Ky(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?kl(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function zy(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>Ky(r,n+1)).filter(r=>r!==null)}function Uy(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function Gy(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>Fp(e,r))}function Qy(){nr()&&o.webServerLogEntries.length===0&&wt({webServerLogEntries:Iy()}),wt({webServerLogCopyMessage:"",webServerLogCopyError:""}),o.systemModal="webserver-logs",h(),K(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=ro();h(),no(e)}),Fy(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&Ir()}function rr(){wt({webServerLogEntries:[],webServerLogError:"",webServerLogHistoryError:"",webServerLogHistoryLoading:!1,webServerLogHistoryLoaded:!1,webServerLogCopyMessage:"",webServerLogCopyError:"",webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1,webServerLogRecentTail:[],webServerLogRecentAnchorAt:0}),El.invalidate(),o.systemModal==="webserver-logs"&&h()}async function Yy(){if(o.busyAction)return!1;if(o.nativeOpen||nr())return rr(),!0;if(typeof window.fetch!="function")return o.webServerLogHistoryError="De RAM-logbuffer kan niet vanuit deze browser worden geleegd.",h(),!1;let e=String(o.webServerLogCsrfToken||"");if(!e)return o.webServerLogHistoryError="De beveiligingstoken voor de RAM-logbuffer ontbreekt. Open het logboek opnieuw.",h(),!1;o.busyAction="clear-webserver-log-history",o.webServerLogHistoryError="",h();try{let t=new URLSearchParams;t.set("csrf_token",e);let r=await window.fetch(Hy(),{method:"POST",body:t});if(!r.ok)throw new Error(`HTTP ${r.status}`);return rr(),!0}catch(t){return o.webServerLogHistoryError=`De RAM-logbuffer kon niet worden geleegd (${t instanceof Error?t.message:"onbekende fout"}).`,!1}finally{o.busyAction="",o.systemModal==="webserver-logs"&&h()}}function Zy(){let e=ro();_t(),wt({webServerLogEnabled:null,webServerLogConnected:!1,webServerLogCsrfToken:""}),rr(),o.systemModal==="webserver-logs"&&Ir({scrollState:e})}function $l(){if(nr()){_t();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs")){_t();return}if(o.webServerLogEnabled===!1){_t();return}o.webServerLogSource||Jy()}function Jy(){if(nr()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",h();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",h();return}try{let e=new window.EventSource($y());o.webServerLogSource=e,e.addEventListener("open",Xy),e.addEventListener("ping",ev),e.addEventListener("log",ov),e.onerror=tv}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",_t(),h()}}function _t(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function Xy(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?ro():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",h(),no(e)}rp({clearOutput:rr,closeStream:_t,resetRecoveryState:Zy});function ev(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?ro():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",h(),no(e))}function tv(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?ro():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",_t(),h(),no(e)}function ov(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=ro(),r=rv(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>kl(u)).filter(u=>!Uy(u)&&!Gy(u));if(i.length===0)return;Wp(i);let l=sv(),c=Al();iv(l),lv(i,l),o.webServerLogEnabled=!0,c&&t&&no(t,!1)}function rv(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).trimEnd()}catch{}return t}function nv(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function av(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function iv(e){for(;o.webServerLogEntries.length>Dp;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function sv(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function Al(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function lv(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",Yp(r))}}function Yp(e){let t=Lp(e.receivedAt),r=Py(e.receivedAt);return`
  ...
  `}function pv(){let e=ql();if(!e)return"";let t=Ip(e),r=Dy(e),n=o.loadingEntities||!!o.busyAction;return`
...
`)}async function gv(){let e=mv();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",h();return}try{if(!await ho(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&h()}var fv={"open-webserver-log-modal":()=>Qy(),"clear-webserver-log-output":()=>Yy(),"copy-webserver-log-output":()=>gv()};function Zp(e){return ie(fv,e)}function Jp(){let e=nr(),t=o.busyAction==="clear-webserver-log-history",r=!!o.busyAction||o.webServerLogHistoryLoading||!e&&!o.nativeOpen&&!o.webServerLogCsrfToken;return te({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
      ...
      <button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="close-system-modal">Gereed</button>
    `})}function hv(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Hl(e){return hv().includes(e)}function Ja(){let e=_r(),t=oe(),r=at(),n=Hp();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:v("firmwareUpdateTarget")&&Hl("alternate connection")&&v("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:nt(r),targetLabel:nt(n),currentBuildLabel:qo(t,r),targetBuildLabel:qo(t,n)}}function Xa(){let e=_r(),t=oe(),r=Rp(),n=at(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:v("firmwareUpdateTarget")&&Hl("alternate topology")&&v("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Xo(t),targetLabel:Xo(r),currentBuildLabel:qo(t,n),targetBuildLabel:qo(r,n)}}function ei(e,t){let r=_r(),n=oe(),a=at(),i=Jo(e),l=Rn(t),c=i&&i!==n,u=l&&l!==a,d=c&&u?"alternate topology and connection":c?"alternate topology":u?"alternate connection":"current build",g=r==="heatpump_controller_q"&&["single","duo"].includes(n)&&["single","duo"].includes(i)&&["wifi","eth"].includes(a)&&["wifi","eth"].includes(l),w=v("firmwareUpdateTarget"),y=Hl(d),f=v("installFirmwareUpdateTarget");return{available:g,canSwitch:g&&d!=="current build"&&w&&y&&f,targetEntityAvailable:w,targetOptionAvailable:y,installActionAvailable:f,currentTopology:n,currentConnection:a,targetTopology:i,targetConnection:l,targetOption:d,currentBuildLabel:qo(n,a),targetBuildLabel:qo(i,l)}}function ir(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function Wr(){let e=_r(),t=oe(),r=at(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${nt(r)}`}}function Pn(e=ir(),t=Wr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function Mo(){if(st())return"Controleren";let e=St();if(e)return e.phaseLabel;if(Nl())return"Bijgewerkt";if(ar())return"Bezig";if(lr())return"Beschikbaar";let t=om();if(t!==null&&t<=0)return"Actueel";let r=Pt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Dl()?"Actueel":pe()?"Nog niet gecontroleerd":"\u2014"}function pe(){return o.entities.firmwareUpdate||null}function ti(){let e=pe();return e?String(e.state??e.value??"").trim().toLowerCase():""}function bv(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Nn(){return bv().toLowerCase()}function tm(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function Rl(){let e=String(o.updateInstallTargetVersion||"").trim(),t=se();return!e||!t?!1:_n(t,e)>=0}function wv(e=pe()||{}){let t=ao(e),r=se(e);return!t||!r?!1:_n(r,t)>=0}function Pl(){return(Rl()||wv())&&!st()&&!Fr()&&!lr()}function Nl(){return(o.updateInstallCompleted||Pl())&&!st()&&!St()&&!lr()}function sr(){Me({updateInstallBusy:!1,updateInstallTargetVersion:"",updateInstallPhaseHint:"",updateInstallProgressHint:Number.NaN,updateInstallStatusPollObserved:!1,updateInstallMode:"",updateInstallTargetConnection:"",updateInstallTargetTopology:""}),Cv()}function Dt(){o.updateInstallPhaseHint="starting",o.updateInstallProgressHint=0,o.updateInstallStatusPollObserved=!1}function Ee(){Me({updateManualUploadFile:null,updateManualUploadFileName:"",updateManualUploadError:""})}function Ye(e={}){Me({...e.clearPr?{updateTestFirmwarePr:""}:{},updateTestFirmwareConfirmed:!1,updateTestFirmwareError:"",updateTestFirmwareBuild:null})}function yv(){let e=Nn(),t=tm(),r=e!=="rebooting"||!o.updateInstallBusy||o.updateInstallStatusPollObserved;if((e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting")&&r){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(o.updateInstallMode==="normal"&&Rl()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function Fr(){let e=Nn();return e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting"}function vv(){let e=Nn();return e==="error"?"De firmware-installatie op het device is mislukt. Controleer de netwerkverbinding en probeer opnieuw.":e==="aborted"?"De firmware-installatie is door het device afgebroken. Probeer de installatie opnieuw.":""}function St(){yv();let e=Nn(),t=e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting"&&(!o.updateInstallBusy||o.updateInstallStatusPollObserved),r=t?e:o.updateInstallPhaseHint,n=tm(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!Fr()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="retrying"?{phaseLabel:"Opnieuw proberen",percent:0,copy:"De eerste verbinding voor de firmwaredownload mislukte. OpenQuatt probeert het automatisch nog \xE9\xE9n keer."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Ge()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${nt(o.updateInstallTargetConnection)}-build wordt nu naar ${Ge()} verzonden.`:o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?`De ${qo(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Ge()} verzonden.`:`Firmware wordt nu naar ${Ge()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Ge()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${nt(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?`Opstellingswissel naar ${Xo(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Ge()}.`}}function ao(e=pe()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function se(e=pe()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function Sv(e=pe()||{}){let t=se(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:_n(t,r)>0}function _l(e=pe()||{},t=xo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=ao(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function Xp(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function _n(e,t){let r=Xp(e),n=Xp(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function ar(){if(Pl())return!1;let e=ti();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function st(){let e=ti();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function lr(){let e=ti();if(!_l())return!1;let t=om();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Pt().updateAvailable===!0}function Dl(){let e=ti();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Sv()}function Ol(){let e=pe()||{},t=se(e)||"\u2014",r=_l(e)?ao(e):"",n=r?_n(r,t):null;return!st()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function om(){let{current:e,latest:t}=Ol();return e==="\u2014"||t==="\u2014"?null:_n(t,e)}function rm(e=xo()){let t=String(e||"").trim().toLowerCase();return ns[t]||ns.main}function qv(){let e=String((pe()||{}).release_url||"").trim(),t=rm();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function kv(){return Ge()}function xo(){return String(T("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function Ev(){return Ol().latest!=="\u2014"}function em(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Tv(e,t=pe()||{}){let r=em(e);return r?em(`${t.title||""} ${t.summary||""}`).includes(r):!0}function nm(e){return new Promise(t=>window.setTimeout(t,e))}function jr(e=fo){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,Nd(),typeof _t=="function"&&_t(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&Ds()},e)}function Cv(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||Ds()}function am(){return o.deviceReconnectMode?te({modalId:"reconnect",titleId:"oq-reconnect-modal-title",kicker:"Systeem",title:fd(),modalClass:"oq-helper-modal--reconnect",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `}):""}function Dn(e=xo()){let t=pe()||{},r=se(t);o.entities.firmwareUpdate={...t,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:rm(e),current_version:r}}async function On(e={}){let t=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await nm(r===0?900:1200),await K(Gt,"all",{forceMissing:!0});let n=_l(),a=!t||Tv(t),i=Ev(),l=st(),c=Mo();if(n&&a&&(i||!l&&c!=="Nog niet gecontroleerd"))return!0}return!1}async function cr(e={}){let t=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await nm(a===0?r:n);try{let i=o.entities.firmwareUpdateStatus;await K(Gt,"all",{forceMissing:!0});let l=Nn();o.entities.firmwareUpdateStatus!==i&&(o.updateInstallStatusPollObserved=!0);let c=vv();if(c){let u=new Error(c);throw u.firmwareInstallTerminal=!0,u}if(l==="rebooting"&&o.updateInstallStatusPollObserved&&wo("ota"),h(),o.updateInstallMode==="connection-switch"){let u=Rn(o.updateInstallTargetConnection);if(u&&at()===u&&!Fr()&&!ar())return!0}else if(o.updateInstallMode==="topology-switch"){let u=Jo(o.updateInstallTargetTopology);if(u&&oe()===u&&!Fr()&&!ar())return!0}else if(o.updateInstallMode==="build-switch"){let u=Jo(o.updateInstallTargetTopology),d=Rn(o.updateInstallTargetConnection);if(u&&d&&oe()===u&&at()===d&&!Fr()&&!ar())return!0}else if(Rl()||Pl()||Dl()&&!Fr()&&!ar())return!0}catch(i){if(i?.firmwareInstallTerminal)throw i;t||(o.controlNotice="Wachten tot het device opnieuw is opgestart...",h(),t=!0)}}return!1}function xv(){let e=xo(),t=St();if(t)return t.copy;if(Nl()){let r=o.updateInstallCompletedVersion||se()||xo();return`${Ge()} draait nu op ${r}.`}return ar()?`OTA-update wordt voorbereid voor ${Ge()}. Het device kan kort herstarten.`:st()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:lr()?"Er staat een nieuwere firmware klaar.":Dl()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function im(){return!!(o.firmwareAdvancedOpen||o.firmwareConnectionSwitchOpen||o.firmwareTopologySwitchOpen||o.updateManualUploadOpen||o.updateTestFirmwareOpen)}function Za(e,t,r,n,a=!1){return`
      <button
    ...
    `}function Mv(e,t,r,n){if(!im())return"";let i=!!(St()||o.updateInstallBusy||st());return`
    ...
    `}function $v(){let e=Ja();if(!e||!o.firmwareConnectionSwitchOpen)return"";let r=!!(St()||o.updateInstallBusy||st()),n=!!o.firmwareConnectionSwitchConfirmed,a=e.targetConnection==="eth",i=!e.canSwitch,l=a?"Sluit eerst de netwerkkabel aan. Na de herstart verdwijnt Wi-Fi uit deze firmware.":"Na de herstart verdwijnt Ethernet uit deze firmware. Als er geen Wi-Fi-gegevens bekend zijn, start het OpenQuatt fallback access point.",c=i?'<p class="oq-helper-modal-note oq-helper-modal-note--muted">Verbindingswissel wordt geladen. Open deze modal opnieuw of wacht een moment als de knop disabled blijft.</p>':"";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
          <strong>Verbinding wisselen</strong>
          <span>Installeer dezelfde ${s(xo())}-build voor de andere netwerkverbinding.</span>
        </div>
    ...
    `}function Hv(){if(!o.updateTestFirmwareOpen)return"";let t=!!(St()||o.updateInstallBusy||st()),r=ir(),n=Wr(),a=Pn(r,n),i=!!(n.available&&v("firmwareTestOtaUrl")&&v("firmwareTestOtaMd5Url")&&v("installFirmwareTestOta")),l=!!(r&&i),c=o.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,d=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Rv(){if(!o.updateManualUploadOpen)return"";let t=!!(St()||o.updateInstallBusy||st()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function sm(){if(!o.updateModalOpen)return"";let e=pe(),t=o.entities.firmwareUpdateChannel||null,{current:r,latest:n}=Ol(),a=st(),i=ar(),l=lr(),c=xv(),u=St(),d=Nl(),g=qv(),w=d?"Firmware-update afgerond":u||i?"Firmware-update bezig":a?"Controleren op firmware-update":kv(),y=t?Array.isArray(t.option)?t.option:Array.isArray(t.options)?t.options:[]:[],f=Ja(),m=Xa(),p=!!(f&&!d),b=!!(m&&!d);return te({id:"firmware-update",titleId:"oq-update-modal-title",kicker:"OTA-update",title:w,copy:c,backdropClass:a||i||u?"is-busy":"",className:"oq-helper-modal--firmware oq-helper-modal--scrollable",closeAction:"close-update-modal",closeLabel:"Sluit update-popup",body:`
          ...
          ${Mv(p,f,b,m)}`})}function Ot(){return o.debugRecordingDeviceStatus?Math.max(0,Number(o.debugRecordingDeviceStatus.sample_count||0)):Array.isArray(o.debugRecordingSamples)?o.debugRecordingSamples.length:0}function Lt(e=o.debugRecordingDeviceStatus){return e?.rolling===!0||String(e?.mode||"").toLowerCase()==="rolling"}function ri(e=o.debugRecordingDeviceStatus){return Lt(e)&&e?.frozen===!0&&!e?.active}function ur(e){let t=Math.max(0,Math.round(Number(e||0)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return r>0?`${r}u ${String(n).padStart(2,"0")}m`:n>0?`${n}m ${String(a).padStart(2,"0")}s`:`${a}s`}function cm(){return o.debugRecordingDeviceStatus?Math.max(0,Number(o.debugRecordingDeviceStatus.retained_duration_s||0)*1e3):um()}function um(){if(o.debugRecordingDeviceStatus)return Math.max(0,Number(o.debugRecordingDeviceStatus.elapsed_s||0)*1e3);if(!o.debugRecordingStartedAt)return 0;let e=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return Math.max(0,e-Number(o.debugRecordingStartedAt||e))}function Ln(){return o.debugRecordingDeviceStatus&&o.debugRecordingDeviceStatus.available===!1?"Niet beschikbaar":ri()?"Rolling gestopt":o.debugRecordingActive&&Lt()?"Rolling actief":o.debugRecordingActive?"Bezig met opnemen":Ot()>0?"Voltooid":"Niet gestart"}function ni(){return ri()?"Rolling debug is gestopt. De recente samples blijven bewaard tot je downloadt, kopieert, hervat of een nieuwe opname start.":o.debugRecordingActive&&Lt()?"Rolling debug bewaart continu de recente samples. Download of kopieer maakt een momentopname; rolling blijft daarna doorlopen.":o.debugRecordingActive?"De opname loopt in apparaatgeheugen. Je kunt deze pagina sluiten en later het bestand downloaden.":Ot()>0?"De opname is klaar. Download het supportbestand en voeg dit toe aan je supportverzoek.":o.debugRecordingDeviceStatus&&o.debugRecordingDeviceStatus.available===!1?"Debugopname in apparaatgeheugen is niet beschikbaar op deze firmware.":"Neem tijdelijk supportgegevens op voor analyse. De opname wordt lokaal in het apparaatgeheugen opgeslagen. Er wordt niets automatisch verzonden."}function dm(){return ri()?"Gestopt":o.debugRecordingActive&&Lt()?`Rolling \xB7 ${ur(cm())}`:o.debugRecordingActive?`Loopt \xB7 ${ur(pm())}`:Ot()>0?"Klaar":Ln()}function Pv(){let e=Number(o.debugRecordingSelectedMinutes||15);return Ma.map(r=>Number(r.minutes)).includes(e)?e:Number(Ma[0]?.minutes||15)}function Nv(e){o.debugRecordingActive||(Da({debugRecordingSelectedMinutes:Math.max(1,Number(e)||15),debugRecordingNotice:"",debugRecordingError:""}),h())}function pm(){return Lt()?0:o.debugRecordingDeviceStatus?Math.max(0,Number(o.debugRecordingDeviceStatus.remaining_s||0)*1e3):o.debugRecordingActive?Math.max(0,Number(o.debugRecordingEndsAt||0)-Date.now()):0}function _v(){if(o.debugRecordingDeviceStatus){if(Lt()){let a=Math.max(1,Number(o.debugRecordingDeviceStatus.sample_capacity||0));return Math.max(0,Math.min(100,Ot()/a*100))}let r=Math.max(1,Number(o.debugRecordingDeviceStatus.duration_s||0)),n=Math.max(0,Number(o.debugRecordingDeviceStatus.elapsed_s||0));return!o.debugRecordingActive&&Ot()>0?100:Math.max(0,Math.min(100,n/r*100))}if(!o.debugRecordingActive||!o.debugRecordingStartedAt||!o.debugRecordingEndsAt)return Ot()>0?100:0;let e=Math.max(1,Number(o.debugRecordingEndsAt)-Number(o.debugRecordingStartedAt)),t=Math.max(0,Date.now()-Number(o.debugRecordingStartedAt));return Math.max(0,Math.min(100,t/e*100))}function mm(e=o.debugRecordingDeviceStatus){return String(e?.recording_id??e?.recording?.recording_id??"").trim()}function gm(e){if(e?.recording?.active)return;let t=mm(e);if(t){o.debugRecordingAcknowledgedId=t;try{window.localStorage.setItem("oq-debug-recording-acknowledged-id",t)}catch{}}}function Ll(){let e=o.debugRecordingDeviceStatus,t=Math.max(0,Number(e?.sample_count||0));if(!e||e.available===!1||!e.active&&t===0)return"";let r=!!e.active;if(!r&&mm(e)===o.debugRecordingAcknowledgedId)return"";let n=Lt(e),a=ur(Math.max(0,Number(e.retained_duration_s||0))*1e3),i=ur(Math.max(0,Number(e.remaining_s||0))*1e3),l=r?n?`Rolling debug \xB7 ${a}`:`Debug loopt \xB7 ${i}`:n?"Rolling gestopt":"Debug klaar",c=r?n?`Rolling debug loopt, laatste ${a} beschikbaar`:`Debugopname loopt, nog ${i}`:n?"Rolling debug gestopt; recente buffer klaar om te downloaden":"Debugopname klaar om te downloaden";return`
    <button
  ...
  `}function Dv(){if(!o.root)return;if(o.interfacePanelOpen){h();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=Ll();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function Ov(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Ln()),r&&(r.textContent=ni())}function Il(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function Lv(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${Il(e)}</span>`}function io(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${Il(e)}</span>`}function Iv(){return[...Wo]}function Fv(){let e=[];return Wo.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function Wv(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function ai(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function dr(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function $o(e){return`${Pe()}/openquatt/debug-recording/${e}`}function Vr(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Fl(){Vr({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function jv(){let e=await window.fetch($o("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),t}function In(e=2e3){dr(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{ii({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function ii(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",h());try{await jv(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),In()}catch(t){Fl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?h():(Dv(),Ov())}}async function fm(){let e=hl(Wo,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch($o(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==Wo.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${Wo.length})`);return t}async function Vv(e){let t=Math.max(1,Number(e)||15);ai(),dr(),Da({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await fm();let r=await window.fetch($o(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Vr(n),In()}catch(r){Fl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,h()}}async function Bv(){ai(),dr(),Da({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await fm();let e=await window.fetch($o("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Vr(t),In()}catch(e){Fl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function Kv(){let e=await window.fetch($o("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),dr(),t}async function zv(){ai(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{await Kv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function Uv(e={}){ai(),dr(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{let t=await window.fetch($o("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Vr(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,h()}}function lm(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(Lu/1e3),sample_count:Ot(),column_count:Wo.length,storage:"browser"},columns:Iv(),units:Fv(),initial:Wv(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function oi(e){return JSON.stringify(e)}function Gv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([oi(lm())]).size}catch{return oi(lm()).length}}function Qv(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function Yv(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function Zv(){if(Ot()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Lt();h();try{let t=await window.fetch($o("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,xs(Yv(r),oi(r),"application/json"),gm(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,h()}}async function Jv(){if(Ot()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Lt();h();try{let t=await window.fetch($o("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await ho(oi(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");gm(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,h()}}var Xv={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",h(),ii()),"start-debug-recording":e=>Vv(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>Bv(),"select-debug-recording-duration":e=>Nv(e.dataset.debugMinutes||15),"stop-debug-recording":()=>Uv(),"freeze-debug-recording":()=>zv(),"download-debug-recording":()=>Zv(),"copy-debug-recording":()=>Jv()};function hm(e,t){return ie(Xv,e,t)}function bm(){let e=o.debugRecordingActive,t=Lt(),r=ri(),n=Ot(),a=o.debugRecordingBusy,i=Qv(Gv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Pv(),u=pm(),d=cm(),g=_v(),w=n>0,y=`${g.toFixed(1)}%`,f=[{icon:"status",label:"Status",value:Ln()},{icon:"clock",label:t?"Retentie":"Duur",value:ur(t?d:um())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],m=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return te({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:ni(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
    ...
    `}function Tm(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return te({id:"system",titleId:"oq-api-security-modal-title",kicker:"Toegang",title:pS(),copy:mS(),className:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit API-beveiliging popup",body:`
      ...
      `;return te({id:"system",titleId:"oq-login-modal-title",kicker:"Systeem",title:uS(),copy:dS(),closeAction:"close-system-modal",closeLabel:"Sluit login-popup",body:`
        ...
        ${n?`<button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="save-web-auth" ${o.authBusy?"disabled":""}>${t?"Opslaan":"Login opslaan"}</button>`:""}`})}function Bn(e,t,r){let n=R(t),a=R(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=R(e);return Number.isFinite(i)?i:NaN}function Bl(e="oq-settings-grid"){let t=hS();return`
      <div class="${s(e)}">
    ...
    `}function wS({status:e,running:t,resultReady:r,startDisabled:n,abortDisabled:a,applyDisabled:i,busy:l,controlsAvailable:c}){let u=String(e||"").toUpperCase(),d=u.includes("FAILED")||u.includes("REFUSED")||u.includes("ABORT"),g=u.includes("APPLIED"),w=v("hp2WaterIn")||v("hp2WaterOut")||v("hp2WaterInRaw")||v("hp2WaterOutRaw"),y=R("hpWaterCalibrationStableProgress"),f=R("hpWaterCalibrationStableRequired"),m=R("hpWaterCalibrationRemaining"),p=Math.round(R("hpWaterCalibrationPhase")),b=t&&(p===1||u.includes("MIXING")),S=t&&!b,q=300,E=60,k=Number.isFinite(m)?Math.max(0,q-m):NaN,M=Number.isFinite(k)?Math.max(0,E-k):NaN,P=b&&Number.isFinite(k)?Math.max(0,Math.min(100,k/E*100)):S&&Number.isFinite(y)&&Number.isFinite(f)&&f>0?Math.max(0,Math.min(100,y/f*100)):t&&Number.isFinite(m)?Math.max(0,Math.min(100,100-m/q*100)):r?100:0,N=r&&v("hpWaterCalibrationResultSpreadBefore")?ke("hpWaterCalibrationResultSpreadBefore",2):ke("hpWaterCalibrationSpread",2),V=b?"Water mengen":Number.isFinite(y)&&Number.isFinite(f)&&f>0?y>0?`${Math.round(Math.max(0,y))} / ${Math.round(f)} s binnen grenzen`:"Nog niet binnen grenzen":"Wachten op stabiel venster",H=r?3:t?2:1,F=g?"Offsets toegepast":r?`Meting klaar - spreiding ${N}`:t?b?`Water mengen${Number.isFinite(M)&&M>0?` - meting start over ${Math.round(M)} s`:""}`:`Meting bezig - ${Number.isFinite(m)&&m>0?`max. ${Math.round(m)} s resterend`:V}`:d?"Meting niet voltooid":"Voorbereiding",A=g?"De voorgestelde offsets zijn opgeslagen.":r?"Controleer de voorgestelde offsets en pas ze toe.":t?b?"De waterpomp circuleert zonder compressor zodat de watertemperaturen eerst kunnen mengen.":"De firmware stopt zodra het laatste meetvenster binnen de spreiding- en driftgrenzen valt.":d?er("hpWaterCalibrationStatus","Controleer de voorwaarden en start opnieuw."):w?"Start alleen wanneer compressor en boiler uit zijn. HP1 en HP2 water in/out worden samen naar een relatieve referentie gebracht.":"Start alleen wanneer compressor en boiler uit zijn. Bij single setup wordt HP1 water in/out onderling gelijkgetrokken; supply blijft diagnose.",O=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",liveKey:"hp1WaterIn",resultRawKey:"hpWaterCalibrationResultHp1InRawAvg",offsetKey:"hp1WaterInOffset",suggestedKey:"hp1WaterInOffsetSuggested"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",liveKey:"hp1WaterOut",resultRawKey:"hpWaterCalibrationResultHp1OutRawAvg",offsetKey:"hp1WaterOutOffset",suggestedKey:"hp1WaterOutOffsetSuggested"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",liveKey:"hp2WaterIn",resultRawKey:"hpWaterCalibrationResultHp2InRawAvg",offsetKey:"hp2WaterInOffset",suggestedKey:"hp2WaterInOffsetSuggested"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",liveKey:"hp2WaterOut",resultRawKey:"hpWaterCalibrationResultHp2OutRawAvg",offsetKey:"hp2WaterOutOffset",suggestedKey:"hp2WaterOutOffsetSuggested"}].filter(I=>v(I.liveKey)||v(I.rawKey)||v(I.offsetKey)),D=(I,j)=>{let ee=H>I;return`
      ...
      `,J=I=>{let j=R(I.resultRawKey),ee=Number.isFinite(j)?j:Bn(I.rawKey,I.liveKey,I.offsetKey),_e=R(I.suggestedKey),Et=Number.isFinite(ee)&&Number.isFinite(_e)?eo(ee+_e,o.entities[I.suggestedKey]?.uom||"\xB0C",2):"\u2014";return`
        <tr>
    ...
    `}function $m(){let e=v("boilerCvAssistEnabled")&&C("boilerCvAssistEnabled"),t=Ua(),r=C("cm100Active"),n=String(t||"").trim().toUpperCase(),a=pr(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,u=!!o.pendingCommissioningCm100Start,d=c||i||a,g=c||!i,w=tr("boilerPowerTestStatus","IDLE"),y=ui(w,"boiler"),f=C("boilerPowerTestActive"),m=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",p=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),b=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=pr(w),k=!Te(w)&&(f||b||S||zr(w))&&!q,M=Ae("boilerRatedHeatPower"),P=Ae("boilerHeatPower"),N=R("boilerHeatPower"),V=N>0?P:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:P;N>0&&(o.commissioningBoilerHeatPowerDisplay=P);let H=tr("flowAutotuneStatus","IDLE"),F=ui(H,"autotune"),A=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",O=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),D=!!o.pendingFlowAutotuneStart,_=o.commissioningTaskLock==="autotune",J=pr(H),j=!Te(H)&&(D||_||zr(H))&&!J,ee=tr("airPurgeStatus","IDLE"),_e=ui(ee,"purge"),Et=C("airPurgeActive"),pt=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",Bt=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),Je=!!o.pendingAirPurgeStart,De=o.commissioningTaskLock==="purge",x=!Te(ee)&&(Et||Je||De||zr(ee)),B=/DONE/.test(String(ee||"").toUpperCase()),Q=!!(Bt||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),G=Ae("airPurgeRemaining",{decimals:0}),le=R("airPurgePhase"),ve=le===1?"Rustig":le===2?"Pulsen":le===3?"Stabiliseren":_e.phase,ue=tr("manualFlowStatus","IDLE"),Ce=C("manualFlowActive"),me=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",mt=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),Tt=!!o.pendingManualFlowStart,de=o.commissioningTaskLock==="manual-flow",je=!Te(ue)&&(Ce||Tt||de||zr(ue)),Xe=tr("manualHpStatus","IDLE"),gt=C("manualHpActive"),ft=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",_o=!!(o.entities.manualHpStart||o.entities.manualHpAbort),Se=!!o.pendingManualHpStart,Ct=o.commissioningTaskLock==="manual-hp",et=!Te(Xe)&&(gt||Se||Ct||zr(Xe)),ku=/SAFETY STOP/.test(String(Xe||"").toUpperCase()),Eu=/STOPPING/.test(String(Xe||"").toUpperCase()),po=tr("hpWaterCalibrationStatus","IDLE"),db=ui(po,"hp-water-calibration"),Tu=C("hpWaterCalibrationActive"),ba=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",Cu=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),wa=!!o.pendingHpWaterCalibrationStart,mo=o.commissioningTaskLock==="hp-water-calibration",ze=!Te(po)&&(Tu||wa||mo||zr(po)),ya=/DONE|APPLIED/.test(String(po||"").toUpperCase()),xu=/APPLIED/.test(String(po||"").toUpperCase()),pb=Ae("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),mb=Ae("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),Mu=/DONE|APPLIED/.test(String(w||"").toUpperCase()),$u=/DONE|APPLIED/.test(String(H||"").toUpperCase()),ts=i?q?"Wachten op CM100":k?y.phase:Mu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",os=i?J?"Wachten op CM100":j?F.phase:$u?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",rs=i?x?_e.phase:B?"Klaar":"Klaar om te starten":"Wachten op CM100",Au=i?je?"Actief":"Klaar om te starten":"Wachten op CM100",Hu=i?et?Eu?"Bezig met stoppen":ku?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",Ru=i?ze?db.phase:xu?"Offsets toegepast":ya?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Pu=!i||m||!p||j||x||je||et||ze||k||_||De||de||Ct||mo||b,gb=m||!(k||S||b),fb=m||Pu||!Mu||j||x||ze,Nu=!i||A||!O||k||x||je||et||ze||j||S||De||de||Ct||mo||D,hb=A||!(j||_||D),bb=A||Nu||!$u||k||x||ze,wb=!i||pt||!Bt||k||j||je||et||ze||x||S||_||de||Ct||mo||Je,yb=pt||!(x||De||Je),vb=!i||me||!mt||k||j||x||et||ze||je||S||_||De||Ct||mo||Tt,Sb=me||!(je||de||Tt),qb=!i||ft||!_o||k||j||x||je||ze||et||S||_||De||de||mo||Se,kb=ft||!(et||Ct||Se),Eb=!i||ba||!Cu||k||j||x||je||et||ze||S||_||De||de||Ct||wa,Tb=ba||!(ze||mo||wa),Cb=ba||ze||!ya||xu;u&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),b&&(f||Te(w))&&(o.pendingBoilerPowerTestStart=!1),S&&Te(w)&&(o.commissioningTaskLock=""),D&&Te(H)&&(o.pendingFlowAutotuneStart=!1),_&&Te(H)&&(o.commissioningTaskLock=""),Je&&(Et||Te(ee))&&(o.pendingAirPurgeStart=!1),De&&Te(ee)&&(o.commissioningTaskLock=""),Tt&&(Ce||Te(ue))&&(o.pendingManualFlowStart=!1),de&&(Ce||Te(ue))&&(o.commissioningTaskLock=""),Se&&(gt||Te(Xe))&&(o.pendingManualHpStart=!1),Ct&&(gt||Te(Xe))&&(o.commissioningTaskLock=""),wa&&(Tu||Te(po))&&(o.pendingHpWaterCalibrationStart=!1),mo&&Te(po)&&(o.commissioningTaskLock="");let xb=a?"Wachten op CM100":t,Mb=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",$b=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:Ru,available:!!(Cu||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:Ru,statusCopy:ze?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":ya?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:wS({status:po,running:ze,resultReady:ya,startDisabled:Eb,abortDisabled:Tb,applyDisabled:Cb,busy:ba,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:Au,available:!!(mt||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:Au,statusCopy:je?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?or({active:je,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:me||vb,stopDisabled:me||Sb}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:Hu,available:!!(_o||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:Hu,statusCopy:et?Eu?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":ku?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:os,available:!0,openDisabled:pr(os),cardMarkup:Ur({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:os,statusCopy:J?"Wacht totdat CM100 actief is voordat je autotune start.":j?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort?or({active:j,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:A||Nu,stopDisabled:A||hb}):""}
            ${o.entities.flowAutotuneApply?Qe("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",A||bb):""}
          `,metrics:`
          ...
          `})}].filter(Ab=>Ab.available);return{cm100Status:xb,cm100StartDisabled:d,cm100StopDisabled:g,serviceStatusCopy:Mb,tasks:$b}}function yS(e){return Le({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:Xt(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,Ho(),o.appView==="results"&&fr({force:!0}).then(()=>h())})},"select-energy-history-import-file":()=>YS(),"clear-energy-history-import-file":()=>{Zm(),h()},"import-energy-history-file":()=>ZS(),"export-energy-history":()=>DS(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",h();let e=qi({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&h()});return Ho([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",h()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",h()},"open-storage-energy":()=>{o.settingsStoragePage="energy",h()},"back-storage-overview":()=>{o.settingsStoragePage="overview",h()},"download-settings-backup":()=>nq(),"open-settings-backup-import":()=>{o.settingsBackupMqttPassword="",o.settingsBackupRestoreResult=null,o.settingsBackupError="",o.systemModal="settings-backup-import",h()},"confirm-settings-backup-restore":()=>lq()};function lg(e,t){return ie(cq,e,t)}function ac(e,t,r="",n=!1){return`
    ...
    `:""}function dq(e){return v(e)||!!L[e]&&!o.optionalMissingEntities?.[e]}function Yn(e,t,r,n={}){if(!dq(e))return"";let a=v(e),i=a&&(o.loadingEntities||o.busyAction===e),l=n.disabled===!0||!a,c=n.buttonClass||"oq-helper-button oq-helper-button--ghost";return`
      <button
        class="${s(c)}"
        type="button"
        data-oq-action="${s(r)}"
    ...
    `:""}function fg(){if(!v("trendHistoryEnabled")&&!v("decisionLogHistoryEnabled")&&!v("lifetimeEnergyHistoryEnabled"))return"";let e=C("trendHistoryEnabled"),t=e&&C("trendHistoryFlashEnabled"),r=v("lifetimeEnergyHistoryEnabled"),n=r&&C("lifetimeEnergyHistoryEnabled"),a=v("decisionLogHistoryEnabled"),i=a&&C("decisionLogHistoryEnabled"),l=mg(),c=t?Fe("trendHistoryFlashAvailable","Alleen live"):"Alleen live",u=r?ug(Fe("lifetimeEnergyHistoryAvailable","Geen data")):"Geen data";return re("Diagnose","Gegevens bewaren","Bepaal welke gegevens OpenQuatt bewaart voor grafieken, resultaten en hulp bij problemen.",`
        <article class="oq-settings-storage-summary">
          ...
          <div class="oq-settings-storage-summary-metrics" aria-label="Opslagstatus">
            ...
            ${a?ac("Beslislog",i?pg(l.storedEvents):"Alleen sinds herstart",i?"Maximaal 7 dagen":"Tijdelijk",i):""}
    ...
    `}function hg(){let e=["diagnosis","decision-log","energy"].includes(o.settingsStoragePage)?o.settingsStoragePage:"overview",t=v("trendHistoryEnabled")&&C("trendHistoryEnabled"),r=t&&v("trendHistoryFlashEnabled")&&C("trendHistoryFlashEnabled"),n=v("decisionLogHistoryEnabled"),a=n&&C("decisionLogHistoryEnabled"),i=mg(),l=pg(i.storedEvents),c=v("lifetimeEnergyHistoryEnabled"),u=c&&C("lifetimeEnergyHistoryEnabled"),d=c?Fe("lifetimeEnergyHistoryAvailable","Geen data"):"Geen data",g=ug(d),w=v("lifetimeEnergyHistoryClear")&&!["Geen data","\u2014"].includes(d),y=t&&v("trendHistoryFlush"),f=a&&v("decisionLogHistoryFlush"),m=v("lifetimeEnergyHistoryCapture"),p=gg(),b=String(o.energyHistoryRaw||"").includes("@hour_retention|"),S=b&&!p.hourPartitionAvailable,q=b?S?"Alleen live":dg(p.hourStoredDayCount,"Geen uurdata"):"Laden...",E=b&&!S?Ci(p.hourStorageKb):"\u2014",k=b&&!S?Zn(p.hourWriteCount):"\u2014",M=b&&!S?Ti(p.hourLastWriteTimestampS):"Geen data",P=e==="overview"?"":`
      ...
      </section>${ic([V])}`):e==="decision-log"?(A="Beslisloghistorie",O="Bewaar exacte controllerkeuzes en gebeurtenissen, maximaal zeven dagen.",D=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
          ...
          ${f?`<div class="oq-settings-storage-inline-action"><div><h4>Beslislog nu opslaan</h4><p>Sla nieuwe gebeurtenissen alvast op v\xF3\xF3r een update of herstart.</p></div>${Yn("decisionLogHistoryFlush","Nu opslaan","flush-decision-log-history",{disabled:!a,busyLabel:"Opslaan..."})}</div>`:""}
        </div>
      </section>${ic([H])}
      ${v("decisionLogHistoryClear")?`<details class="oq-settings-storage-advanced"${o.settingsStorageAdvancedOpen?" open":""}><summary data-oq-action="toggle-storage-advanced">Geavanceerd</summary><div class="oq-settings-storage-inline-action oq-settings-storage-inline-action--danger"><div><h4>Beslisloghistorie wissen</h4><p>Verwijder alle bewaarde gebeurtenissen. De actuele beslislog blijft staan.</p></div>${Yn("decisionLogHistoryClear","Historie wissen","clear-decision-log-history",{disabled:Number(i.storedEvents||0)<=0,buttonClass:"oq-helper-button oq-helper-button--warning",busyLabel:"Wissen..."})}</div></details>`:""}`):e==="energy"&&(A="Energiehistorie",O="Beheer dagtotalen en uurdetail voor de Resultatenpagina.",D=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
    ...
    `}function yq(){if(!o.root)return!1;let e=o.root.querySelector(".oq-helper-status-grid");if(!e)return!!o.root.querySelector(".oq-helper-hub");let t=Sg();if(e.querySelectorAll("[data-oq-header-status]").length!==t.length)return e.outerHTML=Jn(),!0;for(let[n,a,i,l]of t){let c=e.querySelector(`[data-oq-header-status="${n}"]`);if(!c)return e.outerHTML=Jn(),!0;let u=vg(n),d=!!(l||u);if(c.tagName.toLowerCase()!==(d?"button":"div"))return e.outerHTML=Jn(),!0;let g=c.querySelector(".oq-helper-status-label"),w=c.querySelector(".oq-helper-status-value");if(!g||!w)return e.outerHTML=Jn(),!0;g.textContent!==a&&(g.textContent=a);let y=kg(n),f=y?`<span class="oq-helper-status-value-text">${s(i)}</span><span class="oq-helper-status-badge" aria-label="Update beschikbaar" title="Update beschikbaar"></span>`:s(i);w.innerHTML!==f&&(w.innerHTML=f),d?c.setAttribute("data-oq-action",u):c.removeAttribute("data-oq-action"),c.classList.toggle("oq-helper-status-item--button",d),c.classList.toggle("oq-helper-status-item--attention",y)}return!0}function vq(){return""}function cc(){return""}function Eg(){let e=o.nativeOpen?"native":"app",t=qg();return o.interfacePanelOpen?`
    ...
    `}zd({getSignature:xi,patch:yq});function Cg(){if(o.systemModal==="login")return Cm();if(o.systemModal==="api-security")return Tm();if(o.systemModal==="mqtt")return sp();if(o.systemModal==="mqtt-sensors")return lp();if(o.systemModal==="connectivity"){let e=wq();return te({modalId:"system",titleId:"oq-system-modal-title",kicker:"Systeem",title:"Connectiviteit",closeAction:"close-system-modal",closeLabel:"Sluit systeem-popup",bodyMarkup:`
      ...
      `:"";return te({modalId:"system",titleId:"oq-backup-success-modal-title",kicker:"Beheer",title:"Backup hersteld",closeAction:"close-system-modal",closeLabel:"Sluit bevestiging",className:"oq-helper-modal--wide oq-helper-modal--scrollable",bodyMarkup:`
        ...
        `});if(o.systemModal==="webserver-logs")return Jp();if(o.systemModal==="debug-recording")return bm();if(o.systemModal==="openquatt-pause"){let e=C("openquattEnabled"),t=o.busyAction==="openquatt-regulation",r=v("openquattResumeAt"),n=o.loadingEntities||o.entitySyncInFlight,a=r||!n,i=vn(),l=Ar(T("openquattResumeAt")),c=Ra();return te({modalId:"system",titleId:"oq-openquatt-pause-modal-title",kicker:"Bediening",title:"Openquatt regeling",modalClass:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit regeling-popup",bodyMarkup:`
    ...
    `}function kq(){if(!v("phResponseProfile"))return"";let e=String(T("phResponseProfile")||""),t=o.loadingEntities||o.busyAction==="save-phResponseProfile",n=`
      <div class="oq-settings-choice-grid oq-settings-choice-grid--response">
        ${[{value:"Calm",label:"Rustig",rise:"12 min",fall:"5 min",meta:"Opbouw 12 min \xB7 Afbouw 5 min",copy:"Reageert minder snel op schommelingen. Fijn voor vloerverwarming of een woning die traag opwarmt en afkoelt."},{value:"Balanced",label:"Gebalanceerd",rise:"8 min",fall:"3 min",meta:"Opbouw 8 min \xB7 Afbouw 3 min",copy:"Goede middenweg tussen comfort en rust. Meestal het beste startpunt voor dagelijks gebruik."},{value:"Responsive",label:"Direct",rise:"5 min",fall:"2 min",meta:"Opbouw 5 min \xB7 Afbouw 2 min",copy:"Reageert sneller op veranderende warmtevraag. Handig als je woning snel afkoelt of je sneller effect wilt zien."},{value:"Custom",label:"Aangepast",rise:"Vrij",fall:"Instelbaar",meta:"Opbouw en afbouw instelbaar",copy:"Stel zelf in hoe snel de regeling op- en afbouwt. Handig als de standaardprofielen net niet goed passen."}].map(a=>{let i=a.value===e;return a.value==="Custom"&&i?`
            ...
            `:Lr({key:"phResponseProfile",option:a.value,currentValue:e,busy:t,copy:a.copy,meta:a.meta})}).join("")}
      </div>
    ...
    `:""}function xg(){let e=qq();return re("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
      ...
      `;return re("Regeling","Verwarmingsstrategie","Kies hier hoe OpenQuatt je verwarming regelt. De instellingen hieronder passen zich automatisch aan.",`
        ...
        ${gc()}
        ${e}
      `)}function wc(){let r={top:22,right:18,bottom:38,left:34},n=560-r.left-r.right,a=240-r.top-r.bottom,i=fe[0].outdoor,l=fe[fe.length-1].outdoor,c=f=>r.left+(f-i)/(l-i)*n,u=f=>r.top+(70-f)/50*a,d=[20,30,40,50,60,70].map(f=>{let m=u(f);return`
      ...
      <div class="oq-settings-odu-runtime-table" role="table" aria-label="${s(`HP${e} ODU runtime frequentietabel`)}">
        ...
        ${Cr.map(r=>`
          <div class="oq-settings-odu-runtime-row" role="row">
            ...
            <div role="cell">${$g(Do(e,"cooling",r),r)}</div>
            <div role="cell">${$g(Do(e,"heating",r),t+r)}</div>
          </div>
        `).join("")}
      </div>
    ...
    `}function Ro(e){let t=R(e);return Number.isNaN(t)?"\u2014":String(Math.max(0,Math.round(t)))}function Rq(e){let t=R(e);if(Number.isNaN(t))return"Nog niet gemeten";if(t<1)return"Zojuist";if(t<60)return`${Math.round(t)} min geleden`;let r=Math.floor(t/60),n=Math.round(t%60);return`${r}u ${n}m geleden`}function Ag(e){let t=R(e);return Number.isNaN(t)||t<=0?"Tijdstip onbekend":new Intl.DateTimeFormat("nl-NL",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}).format(new Date(t*1e3))}function Pq(e){if(!e.cyclingAlertLatched)return"";let t=X("compressorCyclingAlertAlternating"),r=Ro("compressorCyclingAlertHp1Peak2h"),n=Ro("compressorCyclingAlertHp1Peak72h"),a=v("compressorCyclingAlertHp2Peak2h")?Ro("compressorCyclingAlertHp2Peak2h"):"",i=v("compressorCyclingAlertHp2Peak72h")?Ro("compressorCyclingAlertHp2Peak72h"):"";return`
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=to(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
      <div class="oq-settings-generation-field oq-settings-field--span-2">
        <div class="oq-settings-generation-grid">
          ${n.map(i=>{let l=e[i]||{};return Lr({key:"hpGeneration",option:i,currentValue:r,busy:a,copy:l.copy||"",image:l.image||"",imageAlt:l.alt||"",infoTitle:l.infoTitle||"",infoCopy:l.infoCopy||"",infoId:`hp-generation-${String(i).toLowerCase().replace(/[^a-z0-9]+/g,"-")}`})}).join("")}
        </div>
      </div>
    `}function _g(){let e=Nt(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&to(t).length>0;return!e&&!r?"":re("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Lg(){let e=Mo(),t=za(),r=o.busyAction==="restartAction";return re("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ${Le({dataValue:"uptime",label:"Uptime",value:Dr()})}
          ${Le({dataValue:"ip",label:"IP-adres",value:Or()})}
          ...
          ${Le({dataValue:"webserverLog",label:"Logboek",value:Ya(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
          ...
          ${Le({dataValue:"debugRecording",label:"Debugopname",value:Ln(),note:ni(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
              data-oq-action="open-debug-recording-modal"
            >
          ...
          ${Le({dataValue:"espTemp",label:"ESP-temp",value:Mi()})}
          ...
          ${v("statusLedsEnabled")?`
            ${Le({dataValue:"statusLeds",label:"Status-LEDs",value:C("statusLedsEnabled")?"Aan":"Uit",note:"Schakelt de gele netwerk-LED en rode storings-LED op de Q-edition controller.",action:oo("statusLedsEnabled","Status-LEDs",C("statusLedsEnabled"),o.loadingEntities||o.busyAction==="switch-statusLedsEnabled")})}
          `:""}
        </div>
      `)}function Ig(){let e=[bc("Warmtepomp 1","hp1ExcludedA","hp1ExcludedB"),bc("Warmtepomp 2","hp2ExcludedA","hp2ExcludedB")].filter(Boolean).join("");return re("Installatie","Compressorinstellingen","Stel hier de minimale draaitijd in en bepaal per warmtepomp welke compressorstanden je wilt overslaan.",`
        <div class="oq-settings-subpanel">
          <div class="oq-settings-subpanel-head">
    ...
    `}function Lq(){let e=Ai(),t=o.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function zq(){let e=Gg().id;return e==="setup"?_q():e==="generation"?jg():e==="boiler"?v("boilerCvAssistEnabled")?Fq():Wg():e==="flow-source"?Oq():e==="thermostat-source"?Lq():e==="flow"?Wq():e==="heating"?jq():e==="water"?Vq():e==="silent"?Bq():e==="confirm"?Kq():Wg()}function hr(){let e=Sc().isQEdition;return Tr.filter(t=>(t.id!=="setup"||e)&&(!t.optionalEntity||v(t.optionalEntity)))}function kt(e){let t=hr().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function Uq(e){let t=Hi(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function Gq(e=!1){return hr().map((t,r)=>{let n=Uq(r);return`
        <button
      ...
      `}).join("")}function Gg(){let e=hr();return e.find(t=>t.id===o.currentStep)||e[0]||Tr[0]}function Hi(){return Math.max(0,hr().findIndex(e=>e.id===o.currentStep))}function Qg(e){let t=hr(),r=Math.min(t.length-1,Math.max(0,Hi()+e));o.currentStep=t[r]?.id||Tr[0].id}function Ft(e={}){let t=Hi(),r=hr(),n=t>0?r[t-1]:null,a=t<r.length-1?r[t+1]:null;return`
    ...
    `}function Qq(){let e=Hi(),t=hr();return`
    ...
    `}function kc(e){let t=String(e||"").trim();return t?{Ready:"Gereed","Waiting for room request":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","Cooling enabled, waiting for room temperature above cooling setpoint":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","No dew point source":"Geen dauwpuntbron","OpenQuatt paused":"OpenQuatt gepauzeerd","Cooling disabled":"Koeling uitgeschakeld","Cooling minimum unavailable":"Minimale koel-aanvoer onbekend","Flow too low":"Flow te laag","Fallback active":"Dauwpuntsbenadering actief","Fallback active (+0.5\xB0C warm night)":"Dauwpuntsbenadering actief (+0,5\xB0C warme nacht)","Fallback active (+1.0\xB0C very warm night)":"Dauwpuntsbenadering actief (+1,0\xB0C zeer warme nacht)","Fallback active (+1.5\xB0C tropical night)":"Dauwpuntsbenadering actief (+1,5\xB0C tropische nacht)","User responsibility (no dew point or fallback)":"Expliciet toegestaan (geen dauwpunt of benadering)","Fallback cooling active":"Dauwpuntsbenadering actief","Fallback corrected by warm night":"Dauwpuntsbenadering gecorrigeerd door warme nacht","Fallback blocked by tropical night":"Dauwpuntsbenadering geblokkeerd door tropische nacht"}[t]||t:""}function Yg(){let e=!v("coolingRoomRequestRequired")||C("coolingRoomRequestRequired"),t=[ce("coolingMinimumSupplyTemp","Minimale koel-aanvoer","Ondergrens voor het koeldoel. OpenQuatt gebruikt de hoogste waarde van deze instelling en de dauwpuntveilige grens."),it("coolingDemandMax","Maximale koelsterkte","Bepaalt hoe krachtig OpenQuatt mag koelen. Lager geeft langere, rustigere runs; hoger geeft meer koelvermogen bij warm weer.","",{minLabel:"Rustig",maxLabel:"Krachtig",valueLabel:`${Y("coolingDemandMax")} max`}),ce("coolingRestartDelta","Herstartmarge watertemperatuur","Na het bereiken van het koel-aanvoerdoel start de watercyclus pas opnieuw zodra de aanvoer deze marge boven het doel ligt."),ce("coolingSafetyMargin","Dauwpunt veiligheidsmarge","Extra marge boven het geselecteerde dauwpunt voor de minimale veilige watertemperatuur.")].filter(Boolean),r=[v("coolingRoomRequestRequired")?Pp("coolingRoomRequestRequired","Koelvraag via kamerthermostaat","Aan: OpenQuatt wacht op echte koelvraag vanuit de kamer. Uit: koeltoestemming geldt direct als koelvraag.","Koelvraag start en stopt met de marges hieronder.","Koeltoestemming geldt direct als koelvraag. De start- en stopmarge worden nu niet gebruikt.","oq-settings-field--span-2"):"",e?ce("coolingRequestOnDelta","Koelvraag start boven setpoint","Koelvraag wordt actief zodra de kamer warmer is dan setpoint plus deze marge."):"",e?ce("coolingRequestOffDelta","Koelvraag stopt boven setpoint","Koelvraag valt weer af zodra de kamer koeler is dan setpoint plus deze marge."):""].filter(Boolean),n=r.length>0,a=v("coolingWithoutDewPointMode"),i=[v("coolingGuardMode")?Xn("Route",qe(W("coolingGuardMode","Onbekend"))):"",v("coolingEffectiveMinSupplyTemp")?Xn("Actieve ondergrens",W("coolingEffectiveMinSupplyTemp","\u2014")):""].filter(Boolean),l=i.length?ye("coolingGuardStatus","Actuele beveiliging","Laat zien welke route koeling nu begrenst en welke ondergrens daadwerkelijk geldt.",`<div class="oq-settings-cooling-facts">${i.join("")}</div>`,"oq-settings-field--span-2 oq-settings-field--cooling-status"):"",c=[v("outsideTempSelected")?Xn("Actuele buitentemperatuur",W("outsideTempSelected","\u2014")):"",v("coolingFallbackNightMinOutdoorTemp")?Xn("Nachtminimum buitentemperatuur",W("coolingFallbackNightMinOutdoorTemp","\u2014")):"",v("coolingFallbackMinSupplyTemp")?Xn("Berekende minimum watertemperatuur",W("coolingFallbackMinSupplyTemp","\u2014")):""].filter(Boolean),u=c.length?`<div class="oq-settings-cooling-fallback-metrics">${c.join("")}</div>`:"",d=a||c.length>0,w=W("coolingGuardMode","").toLowerCase().includes("fallback");if(!t.length&&!n&&!a&&!l&&!d)return"";let y={"Dew point required":"Gebruik alleen een betrouwbare dauwpuntmeting. Zonder meting blijft koeling uit.","Allow without dew point":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use fallback":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use dew point approximation":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, user responsibility":"Negeer dauwpunt en benadering; alleen de ingestelde minimale koel-aanvoer geldt."};return re("Koeling","Koelingsinstellingen","Stel hier in wanneer koelvraag ontstaat, hoe koud het water mag worden en hoeveel het water mag opwarmen voor herstart.",`
        ...
        `:""}
      `)}function Zg(){let e=v("otEnabled"),t=v("cicPollingEnabled")||v("cicFeedUrl"),r=v("cicCompatibilityMode"),n=v("otLinkProblem")||v("cicDataStale")||v("cicJsonFeedOk");if(!e&&!t&&!r&&!n)return"";let a=Rt("cicPollingEnabled"),i=Rt("otEnabled"),l=({label:m,value:p,active:b=!1})=>`
    ...
    `:"";return re("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
      ...
      `},D=($,x={})=>{if(!v($))return{markup:"",warning:""};let B=o.entities[$]||{},Q=String(T($)||""),G=to(B),le=new Set(x.hiddenOptions||[]),ve=Q&&le.has(Q),ue=G.filter(de=>!le.has(de)&&w(de,x)),Ce=Q&&!w(Q,x),me=Q==="HA input"&&x.keepUnavailableCurrent!==!0,Tt=(ve&&!ue.includes(Q)?[Q,...ue]:Ce&&!me&&!ue.includes(Q)?[Q,...ue]:ue).map(de=>{let Er=m(de,x);return`<option value="${s(de)}" ${de===Q?"selected":""}>${s(Er)}</option>`}).join("");return{markup:`
        ...
        `,warning:ve?"Huidige bron is legacy; kies een nieuwe bron.":Ce?`Huidige bron niet beschikbaar: ${y(Q,x)}`:""}},_=({key:$,title:x,icon:B="",select:Q,secondarySelect:G=null,secondarySelects:le=null,activeRows:ve=[],measurementRows:ue=[],activeCopy:Ce="",rows:me=[]})=>{let mt=Q&&Q.when!==!1?D(Q.key,Q):{markup:"",warning:""},de=(Array.isArray(le)?le:G?[G]:[]).filter(Se=>Se&&Se.when!==!1).map(Se=>D(Se.key,Se)).filter(Se=>Se.markup),Er=de.map(Se=>Se.markup).join(""),je=de.map(Se=>Se.warning).find(Boolean)||"",Xe=me.filter(Boolean).join(""),gt=`${mt.markup}${Er}`,ft=mt.warning||je,_o=[O({title:"Configuratie",icon:"settings",className:"oq-settings-source-group--config",content:gt?`
            <div class="oq-settings-source-controls">
              ${gt}
            </div>
            ${ft?`<p class="oq-settings-source-warning">${s(ft)}</p>`:""}
          `:""}),O({title:"Actief",icon:"target",rows:ve,copy:Ce,className:"oq-settings-source-group--active"}),O({title:"Metingen",icon:"activity",rows:ue,className:"oq-settings-source-group--measurements"})].filter(Boolean).join("");return!_o&&!gt&&!Xe?"":`
    ...
    `:"";return re("Integratie","MQTT inputbronnen","Beheer de brokerverbinding voor externe MQTT-bronwaarden.",`
      ...
      `)}function tf(){let e=Oe.has(o.settingsGroup)?o.settingsGroup:ge[0].id;return`
    ...
    `:""}function Tc(){let e=$a.map(tk).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function af(e=Tc()){return be(e)}function sf(e=Tc()){return`
...
`)}function Ck(e,t=""){if(!e.length)return`
      ...
      `;let r=Ek(e),n=kk(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,c)=>{let u=r.left+r.barSlot*c+r.barSlot/2,d=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],g=r.height-r.bottom,w=d.map(p=>{let b=Qr(l,p.key);if(b<=0)return"";let S=b/r.axisMax*r.plotHeight;return g-=S,`
        ...
        `}).join(""),f=e.length<=12||c===0||c===e.length-1||c%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||It(l.dateKey))}</text>`:"",m=Tk(l);return`
    ...
    `}function Mk(e,t,r,n){let a=We(r)==="day"?Number(n):Number.NaN,i=Number.isFinite(a)?e.find(l=>l.dateKey===a):null;return i?[i]:t}function xc(){let e=ea(),t=We(o.energyHistoryView),r=Oi(e,t),n=mk(e,t,r),a=Mk(e,n.buckets,t,r.selectedValue),i=gk(a);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:i,activeView:t}}function hf(e=xc()){return be({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,summary:e.summary})}function bf(e=xc()){let t=e.summary,r=hk(t),n=e.buckets[0]?.dateKey?It(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?It(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
    ...
    `}function $k(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=Tc();return Ze(t,af(n),sf(n))}function Ak(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=xc(),a=wk();return(a?!1:Ze(t,hf(n),bf(n)))||a}So({patchEnergyDom:$k,patchResultsDom:Ak});function Hk({label:e,value:t,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(t)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function na(e,t=!1){return e.map(r=>Hk({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?rt(r.key):r.value,status:t})).join("")}function Rc(e){return`
    ...
    `}function _c(e,t,r,n){return`${r?Pk(n):""}${Rk(e,t)}`}function Ef(e,t,r,n,a){if(!e)return;let i=be({mode:t,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&(Wt(e,_c(t,r,n,a)),e.dataset.renderSignature=i)}function Li(e,t,r=""){return`
    ...
    `}function _k(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}var Dk=Bo;function Ok(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function Lk(e){let t=Ok(W("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Ik(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=Dk(r),a=v("timeValid")&&C("timeValid")?Lk(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Fk(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Tf(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>v(e))||""}function Wk(){return["hp1WaterIn","hp2WaterIn"].find(e=>v(e))||""}function Cf(e=W("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function jt(){return Cf()}function xf(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function Dc(){return jt()?"Koeling":ae()?"Stooklijn":"Power House"}function jk(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=R(t);if(!Number.isNaN(r))return r}return Number.NaN}function Mf(){let e=jk(),t=R("phouseHouse"),r=R("totalHeat"),n=R("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:z(e,0,"W"),houseText:z(t,0,"W"),correctionText:Fk(a),capacityText:rt("hpCapacity"),statusTitle:i,statusCopy:l}}function $f(){let e=R("curveSupplyTarget"),t=R("supplyTemp"),r=Tf(),n=r?R(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:rt("curveSupplyTarget"),supplyText:rt("supplyTemp"),deltaText:_k(a),capacityText:rt("hpCapacity"),statusTitle:l,statusCopy:c}}function Af(){let e=R("supplyTemp"),t=W("coolingGuardMode",""),r=W("coolingFallbackNightMinOutdoorTemp","\u2014"),n=R("coolingSupplyError"),a=R("coolingDemandRaw"),i=C("coolingPermitted"),l=C("coolingRequestActive"),c=W("coolingBlockReason","Onbekend"),u=kc(c),d=xf(c,l),g="Wacht op koelvraag",w="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return d?(g="Wacht op koelvraag",w="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(g="Houdt doel vast",w="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(g="Trekt aanvoer omlaag",w="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(g="Benadert koeldoel",w="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(g="Koelt rustig door",w="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(g="Koeling gereed",w="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(g="Koeling geblokkeerd",w=`Blokkade: ${u}.`),{targetText:rt("coolingSupplyTarget"),supplyText:rt("supplyTemp"),safeFloorText:rt("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:rt("coolingDemandRaw"),statusTitle:g,statusCopy:w,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:d}}function Ii(){if(jt()){let t=Af(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(ae()){let t=$f();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=Mf();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function Oc(e){return kf({className:"oq-overview-system",title:e.title,copy:e.copy,signature:be(e),body:`
      ...
      `})}function Vk(){if(!C("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(jt()){let n=Af(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(rf())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(ae()?$f():Mf()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Bk(){return C("openquattEnabled")?jt()?C("coolingPermitted")?C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:W("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:C("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Lc(e,t){let r=Vk(),n=Bk();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Ic(e,t){let r=Lc(e,t);return`
    ...
    `}function Wc(){let e=Kk();return e.length?`
    ...
    `}function aE(e){let t=e.samples[e.samples.length-1]||null,r=Bc(e.windowHours);return`
    ...
    `}function sE(){let e=lo(),t=tc();return`
    ...
    `}function Df(){let e=bt(),r=Pf().length>0;return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
    ...
    `}function cE(){if(!o.root||o.appView!=="diagnosis")return!1;let e=o.root.querySelector(".oq-overview-board");if(!e)return!1;let t=e.querySelector(".oq-overview-trends");return t?(Ze(t,Wi(),ji()),zc(e),aa(e),!0):!1}function Uc(e){if(!e)return null;let t=e.__oqTrendHoverNodes;if(t&&t.chart&&t.hoverLayer&&t.hoverPanel)return t;let r=e.querySelector("[data-oq-trend-hover-layer]"),n={chart:e.querySelector(".oq-overview-trend-chart"),hoverLayer:r,hoverPanel:e.querySelector("[data-oq-trend-hover]"),hoverTime:e.querySelector("[data-oq-trend-hover-time]"),hoverNote:e.querySelector("[data-oq-trend-hover-note]"),hoverValues:e.querySelector("[data-oq-trend-hover-values]"),hoverLine:r?r.querySelector(".oq-overview-trend-hover-line"):null,hoverDots:{}};return r&&r.querySelectorAll("[data-oq-trend-hover-dot]").forEach(a=>{n.hoverDots[a.getAttribute("data-oq-trend-hover-dot")]=a}),e.__oqTrendHoverNodes=n,n}function qf(e,t,r,n=Uc(e)){if(!e||!t||!Array.isArray(t.points)||t.points.length===0)return;let a=Math.max(0,Math.min(t.points.length-1,r)),i=t.points[a];if(!i||!n||!n.chart||!n.hoverLayer||!n.hoverPanel||!n.hoverTime||!n.hoverNote||!n.hoverValues)return;let l=String(a);if(!n.hoverPanel.hidden&&e.dataset.oqTrendHoverIndex===l)return;let c=Ik(i.sample.t,t.endTime);n.hoverPanel.hidden=!1,n.hoverLayer.removeAttribute("hidden"),n.hoverTime.textContent=c.value,n.hoverNote.textContent=c.note,n.hoverLine&&(n.hoverLine.setAttribute("x1",i.x.toFixed(1)),n.hoverLine.setAttribute("x2",i.x.toFixed(1)));let u=[];t.series.forEach(d=>{let g=ra(d,i.sample),w=d.id||d.sampleKey||d.label,y=n.hoverDots[w];if(!Number.isFinite(g)){y&&y.setAttribute("display","none");return}let f=i.values.find(m=>m.seriesId===w);y&&f&&(y.removeAttribute("display"),y.setAttribute("cx",f.x.toFixed(1)),y.setAttribute("cy",f.y.toFixed(1))),u.push(`
        <div class="oq-overview-trend-hover-row oq-overview-trend-hover-row--${s(d.tone)}">
          <span>${s(d.label)}</span>
          <strong>${s(z(g,d.decimals,d.unit))}</strong>
        </div>
      `)}),n.hoverValues.innerHTML=u.join(""),e.dataset.oqTrendHoverIndex=l}function uE(e){if(!e)return;let t=Uc(e);t?.hoverPanel&&(t.hoverPanel.hidden=!0),t?.hoverLayer&&t.hoverLayer.setAttribute("hidden",""),delete e.dataset.oqTrendHoverIndex}function aa(e=o.root){if(!e)return;e.querySelectorAll("[data-oq-trend-card]").forEach(r=>{let n=r.dataset.renderSignature||"";if(r.__oqTrendBoundSignature===n)return;typeof r.__oqTrendCleanup=="function"&&r.__oqTrendCleanup(),r.__oqTrendHoverNodes=null,r.__oqTrendBoundSignature=n;let a=r.querySelector(".oq-overview-trend-chart");if(!a)return;let i=tE(r.dataset.oqTrendCard);if(!i)return;let l=Nf(i.samples,i.series,{mockData:i.mock});r.__oqTrendModel=l;let c=Uc(r),u=0,d=null,g=()=>{let f=d;d=null,u=0;let m=a.getBoundingClientRect();if(!m.width||!m.height)return;let p=Number(f?.clientX);if(!Number.isFinite(p)){qf(r,l,l.points.length-1,c);return}let S=Math.min(m.width,Math.max(0,p-m.left))/m.width*l.width,q=oE(l,S);qf(r,l,q,c)},w=f=>{d=f,u||(u=window.requestAnimationFrame(g))},y=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,uE(r)};a.addEventListener("pointermove",w),a.addEventListener("pointerenter",w),a.addEventListener("pointerleave",y),a.addEventListener("focus",w),a.addEventListener("blur",y),a.addEventListener("touchstart",w,{passive:!0}),r.__oqTrendCleanup=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,a.removeEventListener("pointermove",w),a.removeEventListener("pointerenter",w),a.removeEventListener("pointerleave",y),a.removeEventListener("focus",w),a.removeEventListener("blur",y),a.removeEventListener("touchstart",w)}})}So({patchDiagnosisDom:cE});function eu(e,t,r){let n=Bi(W(t.mode,"Unknown")),a=C(t.defrost),i=vo(W(t.failures,"None")),l=ja(i),c=n==="Verwarmen"||n==="Koelen"||a;return{mode:n,defrostActive:a,failures:i,warningFailures:l,running:c,thermalKey:n==="Koelen"?t.cooling:t.heat,schematic:vE(e,t,r,n,a,l,c)}}function Lf(e,t=null){return`<h3>${s(e)}</h3>${t?`<button class="oq-overview-hp-card-action" type="button" data-oq-action="select-hp-layout" data-hp-layout="${s(t.layout)}">${CE(t.layout==="equal"?"minus":"plus")}<span>${s(t.label)}</span></button>`:""}`}function Qc(e,t,r,n){return`<div class="oq-overview-hp-status">${_c(e,t,r,n)}</div>`}function dE(e,t){return t?"ontdooit":e==="Verwarmen"?"verwarmt":e==="Koelen"?"koelt":e==="Stand-by"?"stand-by":"onbekend"}function If(e){return!Array.isArray(e)||e.length===0?"":`<p class="oq-overview-hp-summary">${s(e.map(t=>`${t.title} ${dE(Bi(W(t.keys.mode,"Unknown")),C(t.keys.defrost))}`).join(", "))}</p>`}function pE(e){let t=o.entities[e];if(!t)return"Positie: \u2014";let r=R(e);return Number.isNaN(r)?`Positie: ${W(e)}`:`Positie: ${z(r,0,t.uom||"")}`}function mE(e){return v(e)?`Positie: ${C(e)?"Koelen/Defrost":"Verwarmen"}`:"Positie: \u2014"}function Bi(e){let t=String(e||"").trim();return!t||t==="Unknown"?"Onbekend":t==="Standby"?"Stand-by":t==="Heating"?"Verwarmen":t==="Cooling"?"Koelen":t}function Yc(e,t,r,n=!0,a="default"){return`
    ...
    `}function fE({tooltip:e,...t}){return`${Zc(t)}${ia({bind:t.bind,...e})}`}function hE({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
    ...
    `}function Ff(e,t,r,n="normal",a=null){if(!v(t.power))return"";let i=eu(e,t,r),{mode:l,defrostActive:c,running:u,thermalKey:d}=i,g=i.schematic;return o.hpVisualMode==="schematic"?`
    ...
    `}function Jc(){if(!tu())return"";let e=Ki();return o.hpVisualMode!=="schematic"?TE(e):`
    ...
    `}function xE(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(Wt(e,jf(t)),!0):(Wt(r,`
      <h3>Warmtepompen</h3>
      ${If(t)}
    `),n.classList.toggle("is-active",o.hpVisualMode==="schematic"),a.classList.toggle("is-active",o.hpVisualMode==="compact"),!0)}function Vf(){let e=Dc(),t=la(),r=ru(t),n=Wf(t),a=jf(t);return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
          ${Pc()}
          ${Hf(e)}
    ...
    `}function y1(){let e=dt(),t=pa(),r=BE(),n=No().filter(g=>!g.quick&&!g.custom),a=KE(),i=zE(a),l=Qf(a.start),c=Qf(a.end),u=o.controlReplayPeriodMenuOpen,d=e==="custom"?"Eigen periode":r.some(g=>g.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function v1(e){let t=pu(e.severity);return`
    ...
    `}function S1(e,t){let r=pu(e.severity),n=t&&t.id===e.id,a=ih(e.kind),i=ah(e);return`
      <button
    ...
    `}function hh(e){if(!e)return"";let t=pu(e.severity),r=Sr(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=sh(e),l=ah(e),c=Array.isArray(e.checks)?e.checks:r.checks;return`
      ...
      <div class="oq-working-status">
        ...
        <div class="oq-working-status-grid">
          ...
          <section class="oq-working-source-grid" aria-label="Bronnen">
            ...
            ${nu("CV",e.cvStatus,"","",e.cvStatus==="Actief",f?"Geen rol bij koelen.":"Tijdelijke ondersteuning bij extra warmtevraag.")}
          </section>
    ...
    `}function P1(){return te({modalId:"initial-load",titleId:"oq-loading-modal-title",kicker:"OpenQuatt",title:"OpenQuatt laden",backdropClass:"oq-helper-modal-backdrop--loading",modalClass:"oq-helper-modal--reconnect oq-helper-modal--loading",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `})}function N1(){return o.appView==="overview"?Vf():o.appView==="control"?vh():o.appView==="energy"?wf():o.appView==="diagnosis"?Df():o.appView==="results"?yf():R1()}function _1(){let e=lc();return`
    ...
    `}function D1(){let e=typeof document<"u"?document.activeElement:null;return!e||typeof e.matches!="function"?null:e.matches("select[data-oq-dev-control]")?e:null}function O1(e){if(!e||o.deferDevControlSelectRender)return;o.deferDevControlSelectRender=!0;let t=()=>{e.removeEventListener("blur",t),e.removeEventListener("change",t),o.deferDevControlSelectRender=!1,window.setTimeout(()=>Sh(),0)};e.addEventListener("blur",t,{once:!0}),e.addEventListener("change",t,{once:!0})}function L1(){if(o.nativeOpen||o.appView!=="settings"||o.renderedAppView!=="settings"||o.renderedSettingsGroup!==o.settingsGroup)return null;let e=document.scrollingElement||document.documentElement,t=Number(window.scrollY||e?.scrollTop||0);return!Number.isFinite(t)||t<=0?null:{group:o.settingsGroup,left:Number(window.scrollX||e?.scrollLeft||0),top:t}}function I1(e){if(!e)return;let t=(o.settingsPageScrollRestoreToken||0)+1;o.settingsPageScrollRestoreToken=t;let r=()=>{if(t!==o.settingsPageScrollRestoreToken||o.nativeOpen||o.appView!=="settings"||o.settingsGroup!==e.group)return;let n=document.scrollingElement||document.documentElement;if(!n)return;let a=Math.max(0,n.scrollHeight-n.clientHeight),i=Math.min(e.top,a);window.scrollTo({left:e.left,top:i,behavior:"auto"})};window.requestAnimationFrame(()=>{r(),window.requestAnimationFrame(r),window.setTimeout(r,80)})}function Sh(){if(!o.root)return;let e=D1();if(e){O1(e);return}let t=A1(),r=xd(o.root),n=o.systemModal==="webserver-logs"?ro():null,a=o.systemModal==="cm100-commissioning"?Vp():null,i=String(o.systemModal||"").startsWith("service-task-")?Kp():null,l=o.systemModal==="history-storage"?Up():null,c=o.systemModal==="settings-backup-restore"?Qp():null,u=o.quickStartModalOpen?Ug():null,d=L1();if(o.nativeOpen){o.root.innerHTML=`
        ${cc()}
        ${Tg()}
      `,Ps(o.root),_s(o.root,r),o.renderedAppView="native",o.renderedSettingsGroup="",o.settingsRenderSignature="",o.headerRenderSignature=xi(),Rr({mqttSensorsModalRenderSignature:""}),Cn(),Wn(),$l(),ks(),qs(),no(n),Tl(a),Cl(i),xl(l),Ml(c),qc(u);return}let g=N1(),w=o.loadingEntities?`${g}${P1()}`:g,y=o.appView==="overview"||o.appView==="control"||o.appView==="energy"||o.appView==="diagnosis"||o.appView==="results";o.root.innerHTML=`
      ${cc()}
