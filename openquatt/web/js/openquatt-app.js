...
`)),r.toString()}function gs(e,o){let r=[],n=[],a=[],i="";return e.forEach(l=>{let p=Hf(l);if(!p)return;let u=[...a,p],c=Uu(u,o);if(a.length&&c.length>Tf){r.push({keys:n,body:i}),n=[l],a=[p],i=Uu(a,o);return}n=[...n,l],a=u,i=c}),a.length&&r.push({keys:n,body:i}),r}async function Mf(e){let o=op(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body};if(typeof AbortController=="function"){let a=new AbortController,i=window.setTimeout(()=>a.abort(),o);try{let l=await fetch(Bu,{...r,signal:a.signal});if(!l.ok)throw new Error(`bulk entities HTTP ${l.status}`);return l.json()}catch(l){throw a.signal.aborted?new Error(`bulk entities request timed out after ${o}ms`):l}finally{window.clearTimeout(i)}}let n=await fetch(Bu,r);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function Rf(){let e=op(),o={cache:"no-store",headers:{"Cache-Control":"no-store"}};if(typeof AbortController=="function"){let n=new AbortController,a=window.setTimeout(()=>n.abort(),e);try{let i=await fetch(Ku,{...o,signal:n.signal});if(!i.ok)throw new Error(`service status HTTP ${i.status}`);return i.json()}catch(i){throw n.signal.aborted?new Error(`service status request timed out after ${e}ms`):i}finally{window.clearTimeout(a)}}let r=await fetch(Ku,o);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}function Pf(e={}){let o=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(o).forEach(([r,n])=>{Dr.has(r)&&(t.optionalMissingEntities&&delete t.optionalMissingEntities[r],t.entities[r]=np(r,t.entities[r],n))})}async function Nf(e){if(!e.length)return{ok:!0,message:""};try{let o=await Rf();return Pf(o),t.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(o){return{ok:!1,message:o.message||String(o)}}}async function L(e,o="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(v=>a||Dr.has(v)||!$f(v,n));if(!i.length)return;let l=i.filter(v=>Dr.has(v)),p=i.filter(v=>!Dr.has(v)),c=(await Nf(l)).ok?p:[...p,...l];if(!c.length){Gu(),Jr(Ot()),t.busyAction||(ju(),t.controlError="");return}let m=Number(r.concurrency),b=Number.isFinite(m)&&m>0?Math.floor(m):Mt,y=gs(c,o),w=Math.max(1,Math.min(b,Mt)),g=[];for(let v=0;v<y.length;v+=w){let S=y.slice(v,v+w),q=await Promise.allSettled(S.map(async C=>({chunk:C,payload:await Mf(C)})));g.push(...q)}g.some(v=>v.status==="fulfilled")&&(t.lastEntityResponseAt=Date.now());let f="";g.forEach(v=>{if(v.status!=="fulfilled"){let M=v.reason.message||String(v.reason);f||(f=M);return}let{chunk:S,payload:q}=v.value,C=q?.entities&&typeof q.entities=="object"?q.entities:{},$=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(M=>{if(Object.prototype.hasOwnProperty.call(C,M)){t.optionalMissingEntities&&delete t.optionalMissingEntities[M],t.entities[M]=np(M,t.entities[M],C[M]);return}let V=R[M];V?.optional?$.has(M)&&Af(M,n):f||(f=`${V?.name||M} ontbreekt in bulk response`)})}),Gu(),Jr(Ot()),f?(rp(f),t.deviceReconnectMode?t.controlError="":t.controlError=`Niet alle helpervelden konden worden ververst. ${f}`):t.busyAction||(ju(),t.controlError="")}function np(e,o={},r={}){let n={...o||{},...r||{}};return R[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(o?.state??"").trim()&&(n.state=o.state),!String(r.value??"").trim()&&String(o?.value??"").trim()&&(n.value=o.value),!Array.isArray(r.option)&&Array.isArray(o?.option)&&(n.option=o.option),!Array.isArray(r.options)&&Array.isArray(o?.options)&&(n.options=o.options)),n}function Gu(){t.complete=Xn(),t.stage=t.complete===!0?"Gereed":t.complete===!1?"Quick Start":"Laden...",t.summary=Pc(),t.appView==="diagnosis"&&!ct()&&ut(eo(),{syncMode:"replace",forceSync:!0}),t.appView||ut(Yr()||eo(),{syncMode:"replace",forceSync:!0})}function Of(){let e=ls();return t.appView==="settings"?[...new Set([...e,...cs()])]:t.appView==="energy"||t.appView==="results"?Xr(t.appView,{forceFast:!0}):t.appView==="overview"||t.appView==="diagnosis"?Xr(t.appView,{forceFast:!0}):[...new Set(e)]}function Df(e=[]){let o=new Set(e);return(t.appView==="settings"?cs():t.appView==="overview"||t.appView==="diagnosis"?[...new Set([...Xr(t.appView,{includeBulk:!0}),...Pt])]:t.appView==="energy"||t.appView==="results"?[...new Set([...Xr(t.appView,{forceFast:!0}),...Pt])]:[...new Set(["setupComplete","strategy",...Ze])]).filter(n=>!o.has(n))}function zu(e=Date.now(),o={}){return o.forceBulk===!0?!0:e-Number(t.lastBulkEntitySyncAt||0)>=Sc}async function ce(e={}){if(t.nativeOpen||t.loadingEntities||t.draggingCurveKey||t.busyAction||t.settingsInteractionLock)return;if(t.updateInstallBusy||aa()){t.pendingEntitySyncOptions=null;return}if(t.focusedField&&t.appView!=="settings")return;if(t.entitySyncInFlight){bf(e);return}let o=Date.now();if(document.hidden&&o-Number(t.lastEntitySyncAttemptAt||0)<Wr)return;let r=t.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,p=!l&&!n&&zu(o,e),c=o-Number(t.lastStaticEntitySyncAt||0)>=qc||t.updateInstallBusy||t.updateInstallPhaseHint?Pt:[],m=t.quickStartModalOpen&&t.currentStep==="flow-source"?lt:[],b=t.quickStartModalOpen&&t.currentStep==="thermostat-source"?So:[],y=Ji()?oo():[],w=n?[...Ei,...Ze,"setupComplete",...c]:i?[...Xr(a,{forceFast:l,includeBulk:p}),...c]:r==="settings"?[...new Set([...cs(),...y,...c])]:p?["setupComplete",...c,...Ze,"strategy",...wo,...Ye,...oe()?ge.map(g=>g.key):vt]:["setupComplete",...Ze,"strategy",...c];t.entitySyncInFlight=!0,t.lastEntitySyncAttemptAt=o;try{let g=t.deviceReconnectMode,f=kf(o,e)?await xf():{ok:!0,message:""};if(!f.ok){rp(f.message),n||d();return}if(await L([...new Set([...w,...m,...b])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?Rt:Mt}),t.lastFastEntitySyncAt=Date.now(),p&&(a==="overview"||a==="diagnosis")&&!n&&(t.lastBulkEntitySyncAt=t.lastFastEntitySyncAt),c.length&&(t.lastStaticEntitySyncAt=t.lastFastEntitySyncAt),n)return;i&&!t.overviewMetadataHydrated&&!t.overviewMetadataHydrating&&Yu();let v=g!==t.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="diagnosis"?await Xi():!1,C=S?!1:t.appView==="results"?await sa():!1,$=S?!1:Ji()?(await Promise.all([qu(),sa({metaOnly:!0})])).some(Boolean):!1,M=S||!Bc()?!1:await $o(),V=S||!Kc()?!1:await Xt(),F=S||!Ou()?!1:await Mo(),ee=yu();if(S&&!t.nativeOpen&&ep(us(a)),v){d();return}if(q&&t.appView==="diagnosis"&&!t.root?.querySelector(".oq-overview-trends")){d();return}if(C&&t.appView==="results"&&!t.root?.querySelector(".oq-energy-history")){d();return}if($&&t.appView==="settings"){d();return}if(M&&(t.systemModal==="login"||Nt())){d();return}if(V&&t.appView==="settings"){d();return}if(t.systemModal==="mqtt-sensors"){ca()!==t.mqttSensorsModalRenderSignature&&d();return}if(F&&t.appView==="settings"){d();return}let H=!!(t.updateModalOpen||t.systemModal||t.interfacePanelOpen);if(ee!==t.headerRenderSignature)if(H&&zi())t.headerRenderSignature=ee;else{d();return}else zi();if(t.appView==="settings"&&String(t.systemModal||"").startsWith("service-task-")){jr()!==t.settingsRenderSignature&&d();return}if(H)return;if(t.appView==="settings"){let G=jr();if(G!==t.settingsRenderSignature&&!t.focusedField){d();return}if(!wu()){t.focusedField||d();return}t.focusedField&&(t.settingsRenderSignature=G);return}if(t.appView==="diagnosis"){xu()||d();return}if(t.appView==="energy"){Eu()||d();return}if(t.appView==="results"){Cu()||d();return}ku()||d()}catch(g){n||(t.controlError=`Helperstatus kon niet worden geladen. ${g.message}`,d())}finally{t.entitySyncInFlight=!1;let g=t.pendingEntitySyncOptions;t.pendingEntitySyncOptions=null,g&&!t.nativeOpen&&window.setTimeout(()=>{ce(g)},0),l&&(a==="overview"||a==="diagnosis")&&!n&&!t.nativeOpen&&!g&&zu(Date.now())&&window.setTimeout(()=>{ce({forceBulk:!0})},hf)}}function Tt(){return typeof window<"u"&&window.__OQ_DEV_META&&typeof window.__OQ_DEV_META=="object"?window.__OQ_DEV_META:{}}function Lf(){let e=String(T("hpGeneration")||"").trim();return e||""}function en(e){let o=String(e||"").trim().toLowerCase();return o==="single"||o.includes("quatt single")||o.includes("openquatt single")?"single":o==="duo"||o.includes("quatt duo")||o.includes("openquatt duo")?"duo":""}function Ff(){if(!Array.isArray(Jo))return"";if(Jo.some(o=>h(o)))return"duo";let e=t.optionalMissingEntities||{};return Jo.every(o=>e[o])?"single":""}function hs(e){let o=en(e);return(o==="single"||o==="duo")&&typeof t<"u"&&t&&typeof t=="object"&&(t.lastKnownInstallationTopology=o),o}function If(){if(typeof t<"u"&&t&&typeof t=="object"){let e=String(t.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function Z(){let e=en(T("installationTopology"));if(e==="single"||e==="duo")return hs(e);let o=en(Tt().installation);if(o==="single"||o==="duo")return hs(o);let r=Ff();return r?hs(r):If()}function dt(){let e=Z(),o=Lf();return e==="single"?o?`Quatt Single ${o}`:"Quatt Single":e==="duo"?o?`Quatt Duo ${o}`:"Quatt Duo":o?`Quatt Hybrid ${o}`:"Quatt Hybrid"}function Fe(){return"OpenQuatt"}function fs(e){let o=String(e||"").trim().toLowerCase();return o==="wifi"||o==="wi-fi"||o.includes("wifi")||o.includes("wi-fi")?"wifi":o==="eth"||o==="ethernet"||o.includes("ethernet")?"eth":""}function Xe(e=ro()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Po(e=Z()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function tn(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Tt().hardwareProfile||e).trim().toLowerCase()}function ro(){return fs(T("connectionText")||Tt().connection)}function ap(){let e=ro();return e==="wifi"?"eth":e==="eth"?"wifi":""}function ip(){let e=Z();return e==="single"?"duo":e==="duo"?"single":""}function rr(e=Z(),o=ro()){let r=Po(e),n=tn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${Xe(o)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${Xe(o)}`:n==="waveshare"?`Waveshare ${r} ${Xe(o)}`:`${Fe()} ${r} ${Xe(o)}`}function bs(){let e=k("timeValid"),o=String(T("timeNowHhmm")||"").trim();if(o&&o!=="invalid")return o;if(h("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function pa(){return h("timeValid")&&!k("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${bs()}`}function ua(e){if(!Number.isFinite(e)||e<0)return"\u2014";let o=Math.floor(e),r=Math.floor(o/1440),n=Math.floor(o%1440/60),a=o%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function _f(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Wf(e){let o=e?.state;if(o!==""&&o!==null&&o!==void 0){let a=Number(o);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function nr(){let e=Wf(t.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=_f(t.entities.uptime);if(n==="d")return ua(e*1440);if(n==="h")return ua(e*60);if(n==="s")return ua(e/60)}let o=String(t.entities.uptimeReadable?.state??t.entities.uptimeReadable?.value??"").trim();if(o&&o.toLowerCase()!=="unknown")return o;let r=Number(Tt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":ua((Date.now()-r)/6e4)}function ar(){let e=String(t.entities.ipAddress?.state??t.entities.ipAddress?.value??"").trim();if(e)return e;let o=String(Tt().ipAddress||"").trim();return o||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function no({key:e,value:o,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `;if(!c)return y;let w=u||we(o),g=m||`${e}-${o}`;return`
  ...
  `}function Do({active:e,startKey:o,stopKey:r,startLabel:n,stopLabel:a,startClass:i="oq-helper-button oq-helper-button--primary",stopClass:l="oq-helper-button oq-helper-button--ghost",startDisabled:p=!1,stopDisabled:u=!1}){return je(e?r:o,e?a:n,e?l:i,e?u:p)}function cp(e,o,r,n,a=""){if(!h(e))return"";let i=t.entities[e]||{},l=String(T(e)||""),p=Ft(i),u=t.loadingEntities||t.busyAction===`save-${e}`,c=`
    <div class="oq-settings-choice-grid">
      ${p.map(m=>{let b=n[m]||"",y=typeof b=="string"?b:b.copy||"",w=typeof b=="string"?"":b.image||"",g=typeof b=="string"?"":b.alt||"";return ir({key:e,option:m,currentValue:l,busy:u,copy:y,image:w,imageAlt:g})}).join("")}
    </div>
  `;return pe(e,o,r,c,a)}function ae(e,o,r,n="",a={}){if(!h(e))return"";let i=Oe(e),l=Ee(e),p=a.unitOverride||i.uom||"",u=a.showUnit!==!1&&!!p,c=u&&a.unitMode!=="outside",m=no({key:e,value:l,meta:i,controlClass:`oq-helper-control${u&&!c?" oq-helper-control--split":""}${c?" oq-helper-control--suffix":""}`,unitMarkup:u?c?`<span class="oq-helper-unit-chip">${s(p)}</span>`:`<span class="oq-helper-unit">${s(p)}</span>`:""});return pe(e,o,r,m,n,a.footerMarkup||"")}function et(e,o,r,n="",a={}){if(!h(e))return"";let i=Oe(e),l=he(e,T(e)),p=a.minLabel||`${i.min}${i.uom||""}`,u=a.maxLabel||`${i.max}${i.uom||""}`,c=a.valueLabel||K(e,l);return pe(e,o,r,`<label class="oq-helper-slider-field"><div class="oq-helper-slider-meta"><span>${s(p)}</span><strong>${s(c)}</strong><span>${s(u)}</span></div><input class="oq-helper-range" type="range" data-oq-field="${s(e)}" min="${i.min}" max="${i.max}" step="${i.step}" value="${l}" ${t.loadingEntities?"disabled":""}></label>`,n)}function vs(e,o,r,n={}){if(!h(e))return"";let a=Oe(e),i=Ee(e),l=n.compact===!0,p=n.embedded===!0,u=n.infoId||e,c=n.showCopy!==!1;return`
...
`)}async function Ep(){let e=Eb();if(t.webServerLogCopyMessage="",t.webServerLogCopyError="",!e){t.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",d();return}try{if(!await Yt(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");t.webServerLogCopyMessage=`${t.webServerLogEntries.length} logregel${t.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(o){t.webServerLogCopyError=o instanceof Error?o.message:"Kopi\xEBren naar het klembord is niet gelukt."}t.systemModal==="webserver-logs"&&d()}function Cp(){let e=sr();return`
  ...
  `}function Cb(){let e=t.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function $p(e){return Cb().includes(e)}function va(){let e=tn(),o=Z(),r=ro(),n=ap();return e!=="heatpump_controller_q"||o!=="single"&&o!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:h("firmwareUpdateTarget")&&$p("alternate connection")&&h("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:Xe(r),targetLabel:Xe(n),currentBuildLabel:rr(o,r),targetBuildLabel:rr(o,n)}}function wa(){let e=tn(),o=Z(),r=ip(),n=ro(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||o!=="single"&&o!=="duo"||!r||!a.includes(n)?null:{canSwitch:h("firmwareUpdateTarget")&&$p("alternate topology")&&h("installFirmwareUpdateTarget"),currentTopology:o,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Po(o),targetLabel:Po(r),currentBuildLabel:rr(o,n),targetBuildLabel:rr(r,n)}}function Fo(e=t.updateTestFirmwarePr){let o=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(o)?o:""}function ur(){let e=tn(),o=Z(),r=ro(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||o!=="single"&&o!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${o}-${r}`,l=o==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${Xe(r)}`}}function rn(e=Fo(),o=ur()){if(!e||!o.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${o.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function lo(){if(ot())return"Controleren";let e=Vt();if(e)return e.phaseLabel;if(As())return"Bijgewerkt";if(cr())return"Bezig";if(Io())return"Beschikbaar";let o=Hp();if(o!==null&&o<=0)return"Actueel";let r=Tt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Ms()?"Actueel":fe()?"Nog niet gecontroleerd":"\u2014"}function fe(){return t.entities.firmwareUpdate||null}function Sa(){let e=fe();return e?String(e.state??e.value??"").trim().toLowerCase():""}function kb(){let e=t.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function qa(){return kb().toLowerCase()}function Ap(){let e=t.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let o=Number(e.value??e.state);return Number.isNaN(o)?Number.NaN:Math.max(0,Math.min(100,o))}function xs(){let e=String(t.updateInstallTargetVersion||"").trim(),o=ie();return!e||!o?!1:nn(o,e)>=0}function xb(e=fe()||{}){let o=co(e),r=ie(e);return!o||!r?!1:nn(r,o)>=0}function $s(){return(xs()||xb())&&!ot()&&!on()&&!Io()}function As(){return(t.updateInstallCompleted||$s())&&!ot()&&!Vt()&&!Io()}function pr(){t.updateInstallBusy=!1,t.updateInstallTargetVersion="",t.updateInstallPhaseHint="",t.updateInstallProgressHint=Number.NaN,t.updateInstallMode="",t.updateInstallTargetConnection="",t.updateInstallTargetTopology="",Nb()}function He(){t.updateManualUploadFile=null,t.updateManualUploadFileName="",t.updateManualUploadError=""}function tt(e={}){e.clearPr&&(t.updateTestFirmwarePr=""),t.updateTestFirmwareConfirmed=!1,t.updateTestFirmwareError="",t.updateTestFirmwareBuild=null}function $b(){let e=qa(),o=Ap();if(e==="starting"||e==="uploading"||e==="rebooting"){t.updateInstallPhaseHint=e,Number.isNaN(o)||(t.updateInstallProgressHint=e==="rebooting"?Math.max(o,100):o);return}if(t.updateInstallBusy){if(xs()){t.updateInstallPhaseHint="rebooting",t.updateInstallProgressHint=100;return}t.controlNotice.includes("opnieuw is opgestart")&&(t.updateInstallPhaseHint="rebooting",t.updateInstallProgressHint=100)}}function on(){let e=qa();return e==="starting"||e==="uploading"||e==="rebooting"}function Vt(){$b();let e=qa(),o=e==="starting"||e==="uploading"||e==="rebooting",r=o?e:t.updateInstallPhaseHint,n=Ap(),a=Number.isNaN(t.updateInstallProgressHint)?0:Math.round(t.updateInstallProgressHint),i=o&&!Number.isNaN(n)?Math.round(n):a;return!on()&&!t.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:t.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":t.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":t.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:t.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Fe()} gedownload en ge\xEFnstalleerd.`:t.updateInstallMode==="connection-switch"?`De ${Xe(t.updateInstallTargetConnection)}-build wordt nu naar ${Fe()} verzonden.`:t.updateInstallMode==="topology-switch"?`De ${rr(t.updateInstallTargetTopology,t.updateInstallTargetConnection)}-build wordt nu naar ${Fe()} verzonden.`:`Firmware wordt nu naar ${Fe()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:t.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Fe()}.`:t.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${Xe(t.updateInstallTargetConnection)} is gestart.`:t.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${Po(t.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Fe()}.`}}function co(e=fe()||{}){let o=String(e.latest_version||"").trim();if(o)return o;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function ie(e=fe()||{}){let o=String(t.entities.projectVersionText?.state||t.entities.projectVersionText?.value||"").trim();return o||String(e.current_version||"").trim()}function Ab(e=fe()||{}){let o=ie(e),r=String(e.current_version||"").trim();return!o||!r||o===r?!1:nn(o,r)>0}function Hs(e=fe()||{},o=so()){let r=String(o||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=co(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function kp(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function nn(e,o){let r=kp(e),n=kp(o);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function cr(){if($s())return!1;let e=Sa();return t.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function ot(){let e=Sa();return t.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Io(){let e=Sa();if(!Hs())return!1;let o=Hp();return o!==null?o>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Tt().updateAvailable===!0}function Ms(){let e=Sa();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Ab()}function Rs(){let e=fe()||{},o=ie(e)||"\u2014",r=Hs(e)?co(e):"",n=r?nn(r,o):null;return!ot()&&n!==null&&n<=0&&(r=""),{current:o,latest:r||"\u2014"}}function Hp(){let{current:e,latest:o}=Rs();return e==="\u2014"||o==="\u2014"?null:nn(o,e)}function Mp(e=so()){let o=String(e||"").trim().toLowerCase();return mi[o]||mi.main}function Hb(){let e=String((fe()||{}).release_url||"").trim(),o=Mp();return!e||o.includes("/dev-latest")&&!e.includes("/dev-latest")||!o.includes("/dev-latest")&&e.includes("/dev-latest")?o:e}function Mb(){return Fe()}function so(){return String(T("firmwareUpdateChannel")||t.entities.releaseChannelText?.state||t.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function Rb(){return Rs().latest!=="\u2014"}function xp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Pb(e,o=fe()||{}){let r=xp(e);return r?xp(`${o.title||""} ${o.summary||""}`).includes(r):!0}function Rp(e){return new Promise(o=>window.setTimeout(o,e))}function an(e=Co){let r=Date.now()+e;t.firmwareOtaQuietUntil=Math.max(Number(t.firmwareOtaQuietUntil||0),r),t.pendingEntitySyncOptions=null,au(),typeof Et=="function"&&Et(),t.firmwareOtaQuietTimer&&window.clearTimeout(t.firmwareOtaQuietTimer),t.firmwareOtaQuietTimer=window.setTimeout(()=>{t.firmwareOtaQuietTimer=null,t.firmwareOtaQuietUntil=0,!t.updateInstallBusy&&!t.nativeOpen&&_i()},e)}function Nb(){t.firmwareOtaQuietTimer&&(window.clearTimeout(t.firmwareOtaQuietTimer),t.firmwareOtaQuietTimer=null),t.firmwareOtaQuietUntil=0,t.nativeOpen||_i()}function Pp(){return t.deviceReconnectMode?`
    ...
    `:""}function sn(e=so()){let o=fe()||{},r=ie(o);t.entities.firmwareUpdate={...o,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:Mp(e),current_version:r}}async function ln(e={}){let o=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await Rp(r===0?900:1200),await L(Fr,"all",{forceMissing:!0});let n=Hs(),a=!o||Pb(o),i=Rb(),l=ot(),p=lo();if(n&&a&&(i||!l&&p!=="Nog niet gecontroleerd"))return!0}return!1}async function dr(e={}){let o=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await Rp(a===0?r:n);try{if(await L(Fr,"all",{forceMissing:!0}),qa()==="rebooting"&&Jt("ota"),d(),t.updateInstallMode==="connection-switch"){let i=fs(t.updateInstallTargetConnection);if(i&&ro()===i&&!on()&&!cr())return!0}else if(t.updateInstallMode==="topology-switch"){let i=en(t.updateInstallTargetTopology);if(i&&Z()===i&&!on()&&!cr())return!0}else if(xs()||$s()||Ms()&&!on()&&!cr())return!0}catch{o||(t.controlNotice="Wachten tot het device opnieuw is opgestart...",d(),o=!0)}}return!1}function Ob(){let e=so(),o=Vt();if(o)return o.copy;if(As()){let r=t.updateInstallCompletedVersion||ie()||so();return`${Fe()} draait nu op ${r}.`}return cr()?`OTA-update wordt voorbereid voor ${Fe()}. Het device kan kort herstarten.`:ot()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:Io()?"Er staat een nieuwere firmware klaar.":Ms()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function Np(){return!!(t.firmwareAdvancedOpen||t.firmwareConnectionSwitchOpen||t.firmwareTopologySwitchOpen||t.updateManualUploadOpen||t.updateTestFirmwareOpen)}function ya(e,o,r,n,a=!1){return`
      <button
    ...
    `}function Db(e,o,r,n){if(!Np())return"";let i=!!(Vt()||t.updateInstallBusy||ot());return`
    ...
    `}function Ib(){if(!t.updateTestFirmwareOpen)return"";let o=!!(Vt()||t.updateInstallBusy||ot()),r=Fo(),n=ur(),a=rn(r,n),i=!!(n.available&&h("firmwareTestOtaUrl")&&h("firmwareTestOtaMd5Url")&&h("installFirmwareTestOta")),l=!!(r&&i),p=t.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,c=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function _b(){if(!t.updateManualUploadOpen)return"";let o=!!(Vt()||t.updateInstallBusy||ot()),r=String(t.updateManualUploadFileName||t.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
  ...
  `}function Xp(e){t.overviewTheme=e==="dark"?"dark":"light";try{window.localStorage.setItem("oq-overview-theme",t.overviewTheme)}catch{}}function xa(e){t.interfacePanelOpen=e===!0}function Ds(e){try{window.localStorage.setItem("oq-active-surface",e==="native"?"native":"app")}catch{}}function dn(e,o={}){t.settingsGroup=Pe.has(e)?e:me[0].id;try{window.localStorage.setItem("oq-settings-group",t.settingsGroup)}catch{}o.syncUrl!==!1&&t.appView==="settings"&&Bi(o.syncMode||"replace")}function ed(e){t.devPanelOpen=e===!0;try{window.localStorage.setItem("oq-dev-panel-open",t.devPanelOpen?"true":"false")}catch{}}function td(e){t.hpVisualMode=e==="compact"?"compact":"schematic";try{window.localStorage.setItem("oq-hp-visual-mode",t.hpVisualMode)}catch{}}function od(e){t.hpLayoutMode=e==="focus-hp1"||e==="focus-hp2"?e:"equal";try{window.localStorage.setItem("oq-hp-layout-mode",t.hpLayoutMode)}catch{}}function ey(e){if(t.reducedMotion=!!e?.matches,t.reducedMotion){zr();return}Gr()}function ty(){let e=xi();!e||t.motionPreferenceMedia===e||(t.motionPreferenceMedia=e,t.motionPreferenceListener=ey,typeof e.addEventListener=="function"?e.addEventListener("change",t.motionPreferenceListener):typeof e.addListener=="function"&&e.addListener(t.motionPreferenceListener),t.reducedMotion=!!e.matches)}function oy(){return Object.keys(t.entities).length>0}function ry(){return ki+Math.floor(Math.random()*(xc-ki+1))}function ny(){return(document.hidden?Wr:wc)+ry()}function rd(e=ny()){t.pollTimer||t.nativeOpen||t.updateInstallBusy||aa()||(t.pollTimer=window.setTimeout(async()=>{t.pollTimer=null,await ce(),rd()},e))}function Ls(){rd()}function Fs(){t.pollTimer&&(window.clearTimeout(t.pollTimer),t.pollTimer=null)}nu({start:Ls,stop:Fs});function ay(){t.nativeOpen||(Fs(),Ls(),document.hidden||ce({forceProbe:!0}))}function $a(e={}){if(mn(),t.nativeOpen){Fs(),zr(),t.nativeFrontendLoaded||Aa();return}if(Gr(),Ls(),e.refresh!==!1){if(!oy()){Zu();return}ce({forceFast:!0})}}function iy(){let e=Yr()||eo(),o=e==="settings"?Vi()||t.settingsGroup:"";if(!(e===t.appView&&(e!=="settings"||o===t.settingsGroup))){if(t.appView=e,e==="settings"&&o){t.settingsGroup=o;try{window.localStorage.setItem("oq-settings-group",t.settingsGroup)}catch{}}d(),ce({forceFast:!0})}}function mn(){t.nativeApp&&(t.nativeApp.classList.add("oq-native-app"),t.nativeApp.classList.toggle("oq-native-app--collapsed",!t.nativeOpen),t.nativeApp.setAttribute("aria-hidden",t.nativeOpen?"false":"true"))}function nd(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Jp,{once:!0}):Jp(),window.addEventListener("pointermove",du),window.addEventListener("pointerup",mu),window.addEventListener("popstate",iy),window.addEventListener("oq-mock-updated",sy),window.addEventListener("oq-dev-controls-changed",ly),document.addEventListener("visibilitychange",ay)}function sy(){t.mounted&&ce()}function ly(){t.mounted&&d()}function Jp(){cy();let e=document.querySelector("esp-app");e||(e=document.createElement("esp-app"),document.body.appendChild(e)),t.nativeApp=e,t.nativeFrontendLoaded=Array.from(document.scripts).some(o=>o.src===gi),t.mounted||(uy(e),t.mounted=!0,$a()),mn(),t.nativeOpen||(ce(),pn({silent:!0}))}function cy(){if(!document.head)return;let e=document.head.querySelector('meta[name="viewport"]');e||(e=document.createElement("meta"),e.name="viewport",document.head.appendChild(e)),e.setAttribute("content","width=device-width, initial-scale=1")}function uy(e){let o=document.createElement("section");o.id="oq-helper-root",o.lang="nl-NL",document.documentElement&&!document.documentElement.lang&&(document.documentElement.lang="nl-NL"),e.parentNode.insertBefore(o,e),o.addEventListener("click",lu),o.addEventListener("change",su),o.addEventListener("input",cu),o.addEventListener("keydown",uu),o.addEventListener("wheel",fu,{passive:!1}),o.addEventListener("focusin",Wi),o.addEventListener("focusout",Wi),o.addEventListener("mouseover",hu),o.addEventListener("mouseout",gu),o.addEventListener("pointerdown",pu),t.root=o,ty();let r=Yr()||eo(),n=r==="settings"?Vi():"";n&&dn(n,{syncUrl:!1}),ut(r,{syncMode:"replace",forceSync:!0}),Ur(),d()}function py(e){return new Promise((o,r)=>{if(!e){o();return}let n=Array.from(document.scripts).find(i=>i.src===e);if(n){if(n.dataset.loaded==="true"){o();return}n.addEventListener("load",()=>o(),{once:!0}),n.addEventListener("error",i=>r(i),{once:!0});return}let a=document.createElement("script");a.src=e,a.async=!1,a.addEventListener("load",()=>{a.dataset.loaded="true",o()},{once:!0}),a.addEventListener("error",i=>r(i),{once:!0}),document.head.appendChild(a)})}async function Aa(){if(!(t.nativeFrontendLoaded||t.nativeFrontendLoading)){t.nativeFrontendLoading=!0,t.nativeOpen&&d();try{await py(gi),t.nativeFrontendLoaded=!0}catch(e){t.controlError=`ESPHome fallback kon niet worden geladen. ${e.message||e}`,t.nativeOpen=!1,Ds("app"),d(),$a()}finally{t.nativeFrontendLoading=!1,t.nativeOpen&&d()}}}function Is(){if(!t.root)return;let e=typeof window<"u"?window.__OQ_DEV_CONTROLS__:null;!e||typeof e.bind!="function"||e.bind(t.root)}function gn(){let e=t.authStatus;return e?e.enabled?e.setup_window_active?"Instelvenster":"Beveiligd":"Onbeveiligd":"Laden..."}function dy(){return"Login"}function my(){let e=t.authStatus;return e?e.enabled?"De web-app vraagt nu een login voordat beheer beschikbaar is. Je kunt die hier aanpassen of uitzetten.":"De web-app staat open op je netwerk. Houd de herstelknop 5 seconden vast om een login toe te voegen.":"We halen de huidige loginstatus op."}function hn(){let e=t.authStatus;return e?e.enabled?e.setup_window_active?"Login actief. Tijdelijk instelvenster is open.":`Login actief${e.source?` via ${e.source}`:""}.`:e.setup_window_active?"Login uit. Tijdelijk instelvenster is open.":"Login uit. Webtoegang is open / onbeveiligd op het netwerk.":"Logingegevens laden..."}function gr(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?"Herstart nodig":e.transport_active===!0?"Aan":e.transport_active===!1?"Uit":e.enabled?"Aan":"Uit":"Laden..."}function hr(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API beveiligd.":e.key?"Deze wijziging wordt actief na herstart. De sleutel blijft opgeslagen voor later gebruik.":"Deze wijziging wordt actief na herstart.":e.transport_active===!0?"API-encryptie staat aan. Gebruik dezelfde sleutel in Home Assistant.":"De native API staat nu open op je lokale netwerk.":"API-encryptie wordt geladen."}function gy(){return"ESPHome API encryption"}function hy(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu alvast voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API nog beveiligd.":"Deze wijziging wordt actief na herstart. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":e.transport_active===!0?"De native API is beveiligd. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":"API-encryptie staat uit. Schakel in om een sleutel te bekijken, kopi\xEBren of vernieuwen.":"We halen de huidige API-beveiliging op."}function fy(){let e=t.apiSecurityStatus;if(!e)return"Laden...";let o=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active);return o&&e.enabled===!0&&e.transport_active===!1?"Annuleer inschakelen":o&&e.enabled===!1&&e.transport_active===!0?"Annuleer uitschakelen":e.enabled?"Uitschakelen":"Inschakelen"}function by(){let e=t.apiSecurityStatus;return e?e.key?"Vernieuw sleutel":"Genereer sleutel":"Laden..."}function ad(e,o,r="",n=!1){return`
    ...
    `}function ud(){let e=h("boilerCvAssistEnabled")&&k("boilerCvAssistEnabled"),o=da(),r=k("cm100Active"),n=String(o||"").trim().toUpperCase(),a=Vo(o),i=!a&&(r||n==="CM100 READY"),l=t.commissioningTaskLock==="cm100",p=t.loadingEntities||t.busyAction==="commissioningCm100Start"||t.busyAction==="commissioningCm100Stop"||l,u=!!t.pendingCommissioningCm100Start,c=p||i||a,m=p||!i,b=Oo("boilerPowerTestStatus","IDLE"),y=Ha(b,"boiler"),w=k("boilerPowerTestActive"),g=t.loadingEntities||t.busyAction==="boilerPowerTestStart"||t.busyAction==="boilerPowerTestAbort"||t.busyAction==="boilerPowerTestApply",f=!!(t.entities.boilerPowerTestStart||t.entities.boilerPowerTestAbort||t.entities.boilerPowerTestApply),v=!!t.pendingBoilerPowerTestStart,S=t.commissioningTaskLock==="boiler",q=Vo(b),$=!qe(b)&&(w||v||S||fr(b))&&!q,M=ke("boilerRatedHeatPower"),V=ke("boilerHeatPower"),F=A("boilerHeatPower"),ee=F>0?V:$&&t.commissioningBoilerHeatPowerDisplay?t.commissioningBoilerHeatPowerDisplay:V;F>0&&(t.commissioningBoilerHeatPowerDisplay=V);let H=Oo("flowAutotuneStatus","IDLE"),G=Ha(H,"autotune"),X=t.loadingEntities||t.busyAction==="flowAutotuneStart"||t.busyAction==="flowAutotuneAbort"||t.busyAction==="flowAutotuneApply",de=!!(t.entities.flowAutotuneStart||t.entities.flowAutotuneAbort||t.entities.flowAutotuneApply),se=!!t.pendingFlowAutotuneStart,N=t.commissioningTaskLock==="autotune",Y=Vo(H),I=!qe(H)&&(se||N||fr(H))&&!Y,z=Oo("airPurgeStatus","IDLE"),We=Ha(z,"purge"),ft=k("airPurgeActive"),it=t.loadingEntities||t.busyAction==="airPurgeStart"||t.busyAction==="airPurgeAbort",At=!!(t.entities.airPurgeStart||t.entities.airPurgeAbort),Ue=!!t.pendingAirPurgeStart,Me=t.commissioningTaskLock==="purge",E=!qe(z)&&(ft||Ue||Me||fr(z)),D=/DONE/.test(String(z||"").toUpperCase()),B=!!(At||t.entities.airPurgeStatus||t.entities.airPurgeReturnToAuto),Q=ke("airPurgeRemaining",{decimals:0}),ne=A("airPurgePhase"),be=ne===1?"Rustig":ne===2?"Pulsen":ne===3?"Stabiliseren":We.phase,te=Oo("manualFlowStatus","IDLE"),xe=k("manualFlowActive"),ye=t.loadingEntities||t.busyAction==="manualFlowStart"||t.busyAction==="manualFlowAbort",le=!!(t.entities.manualFlowStart||t.entities.manualFlowAbort),Ht=!!t.pendingManualFlowStart,ue=t.commissioningTaskLock==="manual-flow",Re=!qe(te)&&(xe||Ht||ue||fr(te)),Ve=Oo("manualHpStatus","IDLE"),Ge=k("manualHpActive"),ze=t.loadingEntities||t.busyAction==="manualHpStart"||t.busyAction==="manualHpAbort",yo=!!(t.entities.manualHpStart||t.entities.manualHpAbort),ve=!!t.pendingManualHpStart,bt=t.commissioningTaskLock==="manual-hp",Qe=!qe(Ve)&&(Ge||ve||bt||fr(Ve)),Nn=/SAFETY STOP/.test(String(Ve||"").toUpperCase()),On=/STOPPING/.test(String(Ve||"").toUpperCase()),yt=Oo("hpWaterCalibrationStatus","IDLE"),ci=Ha(yt,"hp-water-calibration"),Dn=k("hpWaterCalibrationActive"),zo=t.loadingEntities||t.busyAction==="hpWaterCalibrationStart"||t.busyAction==="hpWaterCalibrationAbort"||t.busyAction==="hpWaterCalibrationApply",nc=!!(t.entities.hpWaterCalibrationStart||t.entities.hpWaterCalibrationAbort||t.entities.hpWaterCalibrationApply),Ln=!!t.pendingHpWaterCalibrationStart,Gt=t.commissioningTaskLock==="hp-water-calibration",Be=!qe(yt)&&(Dn||Ln||Gt||fr(yt)),Fn=/DONE|APPLIED/.test(String(yt||"").toUpperCase()),ac=/APPLIED/.test(String(yt||"").toUpperCase()),ah=ke("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),ih=ke("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),ic=/DONE|APPLIED/.test(String(b||"").toUpperCase()),sc=/DONE|APPLIED/.test(String(H||"").toUpperCase()),ui=i?q?"Wachten op CM100":$?y.phase:ic?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",pi=i?Y?"Wachten op CM100":I?G.phase:sc?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",di=i?E?We.phase:D?"Klaar":"Klaar om te starten":"Wachten op CM100",lc=i?Re?"Actief":"Klaar om te starten":"Wachten op CM100",cc=i?Qe?On?"Bezig met stoppen":Nn?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",uc=i?Be?ci.phase:ac?"Offsets toegepast":Fn?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",pc=!i||g||!f||I||E||Re||Qe||Be||$||N||Me||ue||bt||Gt||v,sh=g||!($||S||v),lh=g||pc||!ic||I||E||Be,dc=!i||X||!de||$||E||Re||Qe||Be||I||S||Me||ue||bt||Gt||se,ch=X||!(I||N||se),uh=X||dc||!sc||$||E||Be,ph=!i||it||!At||$||I||Re||Qe||Be||E||S||N||ue||bt||Gt||Ue,dh=it||!(E||Me||Ue),mh=!i||ye||!le||$||I||E||Qe||Be||Re||S||N||Me||bt||Gt||Ht,gh=ye||!(Re||ue||Ht),hh=!i||ze||!yo||$||I||E||Re||Be||Qe||S||N||Me||ue||Gt||ve,fh=ze||!(Qe||bt||ve),bh=!i||zo||!nc||$||I||E||Re||Qe||Be||S||N||Me||ue||bt||Ln,yh=zo||!(Be||Gt||Ln),vh=zo||Be||!Fn||ac;u&&i&&(t.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(t.commissioningTaskLock=""),v&&(w||qe(b))&&(t.pendingBoilerPowerTestStart=!1),S&&qe(b)&&(t.commissioningTaskLock=""),se&&qe(H)&&(t.pendingFlowAutotuneStart=!1),N&&qe(H)&&(t.commissioningTaskLock=""),Ue&&(ft||qe(z))&&(t.pendingAirPurgeStart=!1),Me&&qe(z)&&(t.commissioningTaskLock=""),Ht&&(xe||qe(te))&&(t.pendingManualFlowStart=!1),ue&&(xe||qe(te))&&(t.commissioningTaskLock=""),ve&&(Ge||qe(Ve))&&(t.pendingManualHpStart=!1),bt&&(Ge||qe(Ve))&&(t.commissioningTaskLock=""),Ln&&(Dn||qe(yt))&&(t.pendingHpWaterCalibrationStart=!1),Gt&&qe(yt)&&(t.commissioningTaskLock="");let wh=a?"Wachten op CM100":o,Sh=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",qh=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:uc,available:!!(nc||t.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:br({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:uc,statusCopy:Be?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":Fn?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:wy({status:yt,running:Be,resultReady:Fn,startDisabled:bh,abortDisabled:yh,applyDisabled:vh,busy:zo,controlsAvailable:!!(t.entities.hpWaterCalibrationStart||t.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:lc,available:!!(le||t.entities.manualFlowStatus),openDisabled:!i,cardMarkup:br({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:lc,statusCopy:Re?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:cc,available:!!(yo||t.entities.manualHpStatus),openDisabled:!i,cardMarkup:br({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:cc,statusCopy:Qe?On?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":Nn?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:ui,available:e,openDisabled:Vo(ui),cardMarkup:br({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s(M)}`,status:ui,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":$?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ${t.entities.boilerPowerTestStart||t.entities.boilerPowerTestAbort?Do({active:$,startKey:"boilerPowerTestStart",stopKey:"boilerPowerTestAbort",startLabel:"Boiler test starten",stopLabel:"Boiler test stoppen",startDisabled:g||pc,stopDisabled:g||sh}):""}
            ${t.entities.boilerPowerTestApply?je("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",g||lh):""}
          `,metrics:`
          ...
          `})}].filter(Th=>Th.available);return{cm100Status:wh,cm100StartDisabled:c,cm100StopDisabled:m,serviceStatusCopy:Sh,tasks:qh}}function Sy(e){return Ne({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
`));let c=`${n}|${u.length}|${u.slice(0,120)}|${u.slice(-120)}`,m=Number.isFinite(t.trendHistoryNowMs),y=Number.isFinite(p)?!m||t.trendHistoryNowMs!==p:m,w=u!==t.trendHistoryRaw||t.trendHistoryError!==""||t.trendHistorySignature!==c||y;return t.trendHistoryRaw=u,t.trendHistoryError="",t.trendHistorySignature=c,t.trendHistoryNowMs=Number.isFinite(p)?p:Number.NaN,t.trendHistoryLastFetchAt=Date.now(),w})();try{return await t.trendHistoryFetchPromise}catch(n){let a=`Trendhistorie kon niet worden geladen. ${n.message}`,i=t.trendHistoryError!==a;return t.trendHistoryError=a,t.trendHistoryRaw="",t.trendHistorySignature="",t.trendHistoryNowMs=Number.NaN,t.trendHistoryLastFetchAt=Date.now(),i}finally{t.trendHistoryFetchPromise=null}}async function go(e={}){if(!h("lifetimeEnergyHistoryEnabled")&&!wr()){let a=!!(t.energyHistoryRaw||t.energyHistoryError);return t.energyHistoryRaw="",t.energyHistoryError="",t.energyHistorySignature="",t.energyHistoryNowMs=Number.NaN,t.energyHistoryLastFetchAt=0,t.energyHistoryRequestQuery="",t.energyHistoryFetchQuery="",t.energyHistoryCsrfToken="",a}let o=e.force===!0,r=Date.now(),n=e.metaOnly===!0?"?meta=1":typeof bn=="function"?bn():"";if(!o&&t.energyHistoryFetchPromise&&t.energyHistoryFetchQuery===n)return t.energyHistoryFetchPromise;if(!o&&t.energyHistoryRequestQuery===n&&(t.energyHistoryRaw||t.energyHistoryError)&&r-Number(t.energyHistoryLastFetchAt||0)<In)return!1;t.energyHistoryFetchQuery=n,t.energyHistoryFetchPromise=(async()=>{let a=async f=>{let v=await fetch(`${De()}/energy/history${f}`,{cache:"no-store"});if(!v.ok)throw new Error(`HTTP ${v.status}`);return v.text()},i=n,l=await a(i);if(e.metaOnly!==!0&&i.includes("meta=1")&&typeof bn=="function"){let f=t.energyHistoryRaw;t.energyHistoryRaw=l;let v=bn();t.energyHistoryRaw=f,v&&v!==i&&(i=v,l=await a(i))}let p=l.split(/\r?\n/),u=xd(l)||t.energyHistoryCsrfToken,c=Number.NaN;p.forEach(f=>{f.startsWith("@now|")&&(c=Number(f.slice(5)))});let m=`${l.length}|${l.slice(0,120)}|${l.slice(-120)}`,b=Number.isFinite(t.energyHistoryNowMs),w=Number.isFinite(c)?!b||t.energyHistoryNowMs!==c:b,g=l!==t.energyHistoryRaw||t.energyHistoryError!==""||t.energyHistorySignature!==m||w||u!==t.energyHistoryCsrfToken;return t.energyHistoryRaw=l,t.energyHistoryError="",t.energyHistorySignature=m,t.energyHistoryNowMs=Number.isFinite(c)?c:Number.NaN,t.energyHistoryLastFetchAt=Date.now(),t.energyHistoryRequestQuery=i,t.energyHistoryCsrfToken=u,g})();try{return await t.energyHistoryFetchPromise}catch(a){let i=`Energiehistorie kon niet worden geladen. ${a.message}`,l=t.energyHistoryError!==i;return t.energyHistoryError=i,t.energyHistoryRaw="",t.energyHistorySignature="",t.energyHistoryNowMs=Number.NaN,t.energyHistoryLastFetchAt=Date.now(),t.energyHistoryRequestQuery=n,t.energyHistoryCsrfToken="",l}finally{t.energyHistoryFetchPromise=null}}Su({refreshEnergyHistoryData:go,refreshSettingsStorageState:wn,refreshTrendHistoryData:js,refreshTrendHistoryMetadata:vd,shouldRefreshSettingsStorageForCurrentSurface:jy});function Od(){return O("controlModeLabel","").toLowerCase().includes("standby")}function xt(e,o){e&&e.innerHTML!==o&&(e.innerHTML=o)}function gt(e,o,r){return!e||e.dataset.renderSignature===o?!1:(e.outerHTML=r,!0)}function Jy([e,o]){let r=Hi(o);if(!h(o)&&Number.isNaN(r))return"";let n=Ai(o)?Je(o):O(o);return`
    ...
    `:""}function Qs(){let e=zn.map(tv).filter(Boolean),o=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:o}}function _d(e=Qs()){return Ce(e)}function Wd(e=Qs()){return`
    ...
    `}function Sv(e){return Er(e.view)?`
...
`)}function Hv(e,o=""){if(!e.length)return`
      ...
      `;let r=$v(e),n=xv(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,p)=>{let u=r.left+r.barSlot*p+r.barSlot/2,c=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],m=r.height-r.bottom,b=c.map(f=>{let v=Tn(l,f.key);if(v<=0)return"";let S=v/r.axisMax*r.plotHeight;return m-=S,`
          ...
          >
            <title>${s(`${l.label} \xB7 ${f.label}: ${rt(v,1)}`)}</title>
          </rect>
    ...
    `}function Rv(){if(!t.root||t.appView!=="energy")return!1;let e=t.root.querySelector(".oq-overview-board"),o=e?e.querySelector(".oq-overview-energy"):null;if(!e||!o)return!1;let r=`oq-overview-board oq-overview-board--${t.overviewTheme}`;e.className!==r&&(e.className=r);let n=Qs();return gt(o,_d(n),Wd(n))}function Pv(){if(!t.root||t.appView!=="results")return!1;let e=t.root.querySelector(".oq-overview-board"),o=e?e.querySelector(".oq-energy-history"):null;if(!e||!o)return!1;let r=`oq-overview-board oq-overview-board--${t.overviewTheme}`;e.className!==r&&(e.className=r);let n=tl(),a=qv();return(a?!1:gt(o,tm(n),om(n)))||a}or({patchEnergyDom:Rv,patchResultsDom:Pv});function rl(e,o,r="",n=!1){return`
    ...
    `}function Wv(){if(!h("lifetimeEnergyHistoryEnabled"))return"";let e=vr(t.energyHistoryExportMode),o=vn.map(n=>`
    ...
    `}function mm(){let e=t.settingsBackupDraft;if(!e)return"";let o=e.summary||Ks(e),r=String(e.source?.installation||e.source?.device||"Onbekend"),n=dt(),a=String(e.source?.firmware_version||"Onbekend"),i=String(e.source?.firmware_channel||"").trim()||"Onbekend",l=String(e.source?.topology||"").trim()||"Onbekend",p=ie(),u=typeof Z=="function"?Z():"",c=l!=="Onbekend"&&u&&l!==u,m=r!=="Onbekend"&&r!==n,b=c||m?"De backup lijkt van een andere installatie te komen. Je kunt nog steeds doorzetten, maar controleer de secties even goed.":o.requiredMissing?"Ontbrekende velden houden hun firmware-default.":"Velden zonder waarde worden overgeslagen.";return`
      <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
    ...
    `}bu({getSignature:Ka,patch:Kv});function wm(){if(t.systemModal==="login")return sd();if(t.systemModal==="api-security")return id();if(t.systemModal==="mqtt")return Ru();if(t.systemModal==="mqtt-sensors")return Pu();if(t.systemModal==="connectivity"){let e=Bv();return`
      ...
      `;if(t.systemModal==="webserver-logs")return Cp();if(t.systemModal==="debug-recording")return Zp();if(t.systemModal==="openquatt-pause"){let e=k("openquattEnabled"),o=t.busyAction==="openquatt-regulation",r=h("openquattResumeAt"),n=t.loadingEntities||t.entitySyncInFlight,a=r||!n,i=Br(),l=Xo(T("openquattResumeAt")),p=Zn();return`
        <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
    ...
    `:""}function pl(){return`
      <div class="oq-settings-curve-grid">
        ...
        ${ae("curveFallbackSupply","Fallback-aanvoertemperatuur zonder buitentemperatuur","Aanvoertemperatuur die gebruikt wordt als de buitentemperatuursensor niet beschikbaar is.","oq-settings-field--curve-fallback-card",{footerMarkup:Uv()})}
      </div>
    ...
    `}function Gv(e="oq-settings-grid"){let o=[ae("flowKp","Flow PI Kp","Hoe sterk de regeling direct reageert op een afwijking."),ae("flowKi","Flow PI Ki","Hoe snel de regeling kleine restfouten wegwerkt.")].filter(Boolean);return o.length?`
      <div class="${s(e)}">
        ${o.join("")}
      </div>
    ...
    `;return pe("curveControlProfile","Regelprofiel","Kies of de stooklijn vooral comfortabel, gebalanceerd of rustig moet reageren.",n,"oq-settings-field--span-2")}function Qv(){let e=(N,Y=0)=>{let P=A(N);return Number.isNaN(P)?Y:Math.max(0,P)},r=e("phComfortBelow",.1),n=e("phComfortAbove",.3),a=e("phKp",3e3),i=20-r,l=20+n,p=620,u=184,c=46,m=24,b=18,y=40,w=96,g=p-c-m,f=Math.min(20-1.2,i-.35),v=Math.max(20+1.2,l+.35),S=N=>c+(N-f)/Math.max(.01,v-f)*g,q=S(f),C=S(v),$=S(i),M=S(20),V=S(l),F=Math.abs(i-20)>.001,ee=Math.abs(l-20)>.001,H=b+24,G=u-y,X=w-44,de=(N,Y,P,I="")=>{let ft=Math.max(q+4,Math.min(C-110-4,N-55)),it=N-14,At=X,Ue=28,Me=w-X+16;return`
        <g class="oq-ph-concept-hotspot" tabindex="0" role="img" aria-label="${s(`${Y} ${P}`)}">
          ...
          <circle class="oq-ph-concept-hit" cx="${N}" cy="${w}" r="14"></circle>
      ...
      `},se=[`M ${q.toFixed(1)} ${H.toFixed(1)}`,`L ${$.toFixed(1)} ${w.toFixed(1)}`,`L ${V.toFixed(1)} ${w.toFixed(1)}`,`L ${C.toFixed(1)} ${G.toFixed(1)}`].join(" ");return`
      <div class="oq-ph-concept-card">
        <div class="oq-ph-concept-visual">
          ...
          <svg class="oq-ph-concept-svg" viewBox="0 0 ${p} ${u}" role="img" aria-label="Grafiek voor Power House tuning">
            ...
            ${F?de($,"Comfort onder setpoint",_(i,1,"\xB0C"),"below"):""}
    ...
    `:""}function Sm(){let e=Gv();return J("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
      ...
      `;return J("Regeling","Verwarmingsstrategie","Kies hier hoe OpenQuatt je verwarming regelt. De instellingen hieronder passen zich automatisch aan.",`
        ...
        ${hl()}
        ${e}
      `)}function vl(){let r={top:22,right:18,bottom:38,left:34},n=560-r.left-r.right,a=240-r.top-r.bottom,i=ge[0].outdoor,l=ge[ge.length-1].outdoor,p=w=>r.left+(w-i)/(l-i)*n,u=w=>r.top+(70-w)/50*a,c=[20,30,40,50,60,70].map(w=>{let g=u(w);return`
      ...
      `).join(""),b=ge.map(w=>`${p(w.outdoor)},${u(he(w.key,T(w.key)))}`).join(" "),y=ge.map(w=>{let g=he(w.key,T(w.key));return`
          <g>
            <circle
              cx="${p(w.outdoor)}"
              cy="${u(g)}"
    ...
    `}function km(e){if(e.key!=="Tab"||e.altKey||e.ctrlKey||e.metaKey)return;let o=e.target&&e.target.closest?e.target.closest("input[data-oq-odu-runtime-tab-index]"):null,r=o?o.closest(".oq-settings-odu-runtime-table"):null;if(!o||!r)return;let n=Array.from(r.querySelectorAll("input[data-oq-odu-runtime-tab-index]:not(:disabled)")).sort((l,p)=>Number(l.dataset.oqOduRuntimeTabIndex||0)-Number(p.dataset.oqOduRuntimeTabIndex||0)),a=n.indexOf(o),i=n[a+(e.shiftKey?-1:1)];a<0||!i||(e.preventDefault(),i.focus(),typeof i.select=="function"&&i.select())}function ow(e){let o=Te(e,"Enable"),r=Te(e,"Load"),n=Te(e,"Apply"),a=Te(e,"Status"),i=String(T(a)||"").trim()||"Nog niet geladen",l=Jv(e),p=Xv(e),u=!!T(o),c=t.loadingEntities||t.busyAction===r||t.busyAction===n,m=c||!u||!l.valid||!p.safe||!h(n),b=l.valid?"Waarden zijn 0-120 Hz en per tabel oplopend.":`Controleer ${l.invalid.slice(0,5).join(", ")}${l.invalid.length>5?"...":""}.`;return`
      ...
      ${h(o)?It(o,`HP${e} writes vrijgeven`,u,c,"Enable","Locked"):""}
            ${h(n)?je(n,t.busyAction===n?"Schrijven...":"Runtime toepassen","oq-helper-button oq-helper-button--warning",m):""}
          </div>
        </div>
    ...
    `:""}function $m(){let e=Ot();Jr(e);let o=qt("cicPollingEnabled"),r=qt("otEnabled"),n=[h("lowflowFaultActive")?xr({label:"Flow",value:U("lowflowFaultActive")?"Te lage flow gemeld":"Geen lage-flowmelding",active:U("lowflowFaultActive")}):"",h("flowMismatch")?xr({label:"Flowvergelijking duo",value:U("flowMismatch")?"Afwijking tussen warmtepompen":"Geen afwijking gemeld",active:U("flowMismatch")}):""].filter(Boolean).join(""),a=[h("cicDataStale")?xr({label:"CIC-data",value:o?U("cicDataStale")?"Verouderd":"Geen probleem gemeld":"Polling uitgeschakeld",active:o&&U("cicDataStale")}):"",h("otLinkProblem")?xr({label:"OpenTherm",value:r?U("otLinkProblem")?"Verbindingsprobleem":"Geen probleem gemeld":"Uitgeschakeld",active:r&&U("otLinkProblem")}):""].filter(Boolean).join(""),i=[h("hp1Failures")?xr({label:"Warmtepomp 1",value:ji("hp1Failures"),active:Zr("hp1Failures")}):"",h("hp2Failures")?xr({label:"Warmtepomp 2",value:ji("hp2Failures"),active:Zr("hp2Failures")}):""].filter(Boolean).join(""),l=A("compressorStarts2hWarningLimit"),p=A("compressorStarts72hWarningLimit"),u=U("compressorCyclingWarning2h")||U("compressorCyclingWarning72h")||U("alternatingCompressorStartsWarning")||e.cyclingAlertLatched,c=n?`
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},o=t.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=Ft(o),a=t.loadingEntities||t.busyAction==="save-hpGeneration";return`
    ...
    `}function Am(){let e=dt(),o=t.entities.hpGeneration||{},r=h("hpGeneration")&&Ft(o).length>0;return!e&&!r?"":J("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Rm(){let e=lo(),o=pa(),r=t.busyAction==="restartAction";return J("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ...
          ${h("statusLedsEnabled")?`
            ${Ne({dataValue:"statusLeds",label:"Status-LEDs",value:k("statusLedsEnabled")?"Aan":"Uit",note:"Schakelt de gele netwerk-LED en rode storings-LED op de Q-edition controller.",action:It("statusLedsEnabled","Status-LEDs",k("statusLedsEnabled"),t.loadingEntities||t.busyAction==="switch-statusLedsEnabled")})}
          `:""}
        </div>
    ...
    `}function hw(){return t.currentStep==="generation"?Dm():t.currentStep==="boiler"?h("boilerCvAssistEnabled")?cw():Om():t.currentStep==="flow-source"?iw():t.currentStep==="thermostat-source"?sw():t.currentStep==="flow"?uw():t.currentStep==="heating"?pw():t.currentStep==="water"?dw():t.currentStep==="silent"?mw():t.currentStep==="confirm"?gw():Om()}function Go(){return Qo.filter(e=>!e.optionalEntity||h(e.optionalEntity))}function $t(e){let o=Go().findIndex(r=>r.id===e);return`Stap ${Math.max(0,o)+1}`}function fw(e){let o=Qa(),r=e===o,n=t.complete===!0||e<o;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function bw(e=!1){return Go().map((o,r)=>{let n=fw(r);return`
      ...
      `}).join("")}function yw(){let e=Go();return e.find(o=>o.id===t.currentStep)||e[0]||Qo[0]}function Qa(){return Math.max(0,Go().findIndex(e=>e.id===t.currentStep))}function El(e){let o=Go(),r=Math.min(o.length-1,Math.max(0,Qa()+e));t.currentStep=o[r]?.id||Qo[0].id}function jt(e={}){let o=Qa(),r=Go(),n=o>0?r[o-1]:null,a=o<r.length-1?r[o+1]:null;return`
    ...
    `}function vw(){let e=Qa(),o=Go();return`
    ...
    `}function Cl(e){let o=String(e||"").trim();return o?{Ready:"Gereed","Waiting for room request":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","Cooling enabled, waiting for room temperature above cooling setpoint":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","No dew point source":"Geen dauwpuntbron","OpenQuatt paused":"OpenQuatt gepauzeerd","Cooling disabled":"Koeling uitgeschakeld","Cooling minimum unavailable":"Minimale koel-aanvoer onbekend","Flow too low":"Flow te laag","Fallback active":"Dauwpuntsbenadering actief","Fallback active (+0.5\xB0C warm night)":"Dauwpuntsbenadering actief (+0,5\xB0C warme nacht)","Fallback active (+1.0\xB0C very warm night)":"Dauwpuntsbenadering actief (+1,0\xB0C zeer warme nacht)","Fallback active (+1.5\xB0C tropical night)":"Dauwpuntsbenadering actief (+1,5\xB0C tropische nacht)","User responsibility (no dew point or fallback)":"Expliciet toegestaan (geen dauwpunt of benadering)","Fallback cooling active":"Dauwpuntsbenadering actief","Fallback corrected by warm night":"Dauwpuntsbenadering gecorrigeerd door warme nacht","Fallback blocked by tropical night":"Dauwpuntsbenadering geblokkeerd door tropische nacht"}[o]||o:""}function Bm(){let e=!h("coolingRoomRequestRequired")||k("coolingRoomRequestRequired"),o=[ae("coolingMinimumSupplyTemp","Minimale koel-aanvoer","Ondergrens voor het koeldoel. OpenQuatt gebruikt de hoogste waarde van deze instelling en de dauwpuntveilige grens."),et("coolingDemandMax","Maximale koelsterkte","Bepaalt hoe krachtig OpenQuatt mag koelen. Lager geeft langere, rustigere runs; hoger geeft meer koelvermogen bij warm weer.","",{minLabel:"Rustig",maxLabel:"Krachtig",valueLabel:`${K("coolingDemandMax")} max`}),ae("coolingRestartDelta","Herstartmarge watertemperatuur","Na het bereiken van het koel-aanvoerdoel start de watercyclus pas opnieuw zodra de aanvoer deze marge boven het doel ligt."),ae("coolingSafetyMargin","Dauwpunt veiligheidsmarge","Extra marge boven het geselecteerde dauwpunt voor de minimale veilige watertemperatuur.")].filter(Boolean),r=[h("coolingRoomRequestRequired")?sp("coolingRoomRequestRequired","Koelvraag via kamerthermostaat","Aan: OpenQuatt wacht op echte koelvraag vanuit de kamer. Uit: koeltoestemming geldt direct als koelvraag.","Koelvraag start en stopt met de marges hieronder.","Koeltoestemming geldt direct als koelvraag. De start- en stopmarge worden nu niet gebruikt.","oq-settings-field--span-2"):"",e?ae("coolingRequestOnDelta","Koelvraag start boven setpoint","Koelvraag wordt actief zodra de kamer warmer is dan setpoint plus deze marge."):"",e?ae("coolingRequestOffDelta","Koelvraag stopt boven setpoint","Koelvraag valt weer af zodra de kamer koeler is dan setpoint plus deze marge."):""].filter(Boolean),n=r.length>0,a=h("coolingWithoutDewPointMode"),i=[h("coolingGuardMode")?xn("Route",we(O("coolingGuardMode","Onbekend"))):"",h("coolingEffectiveMinSupplyTemp")?xn("Actieve ondergrens",O("coolingEffectiveMinSupplyTemp","\u2014")):""].filter(Boolean),l=i.length?pe("coolingGuardStatus","Actuele beveiliging","Laat zien welke route koeling nu begrenst en welke ondergrens daadwerkelijk geldt.",`<div class="oq-settings-cooling-facts">${i.join("")}</div>`,"oq-settings-field--span-2 oq-settings-field--cooling-status"):"",p=[h("outsideTempSelected")?xn("Actuele buitentemperatuur",O("outsideTempSelected","\u2014")):"",h("coolingFallbackNightMinOutdoorTemp")?xn("Nachtminimum buitentemperatuur",O("coolingFallbackNightMinOutdoorTemp","\u2014")):"",h("coolingFallbackMinSupplyTemp")?xn("Berekende minimum watertemperatuur",O("coolingFallbackMinSupplyTemp","\u2014")):""].filter(Boolean),u=p.length?`<div class="oq-settings-cooling-fallback-metrics">${p.join("")}</div>`:"",c=a||p.length>0,b=O("coolingGuardMode","").toLowerCase().includes("fallback");if(!o.length&&!n&&!a&&!l&&!c)return"";let y={"Dew point required":"Gebruik alleen een betrouwbare dauwpuntmeting. Zonder meting blijft koeling uit.","Allow without dew point":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use fallback":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use dew point approximation":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, user responsibility":"Negeer dauwpunt en benadering; alleen de ingestelde minimale koel-aanvoer geldt."};return J("Koeling","Koelingsinstellingen","Stel hier in wanneer koelvraag ontstaat, hoe koud het water mag worden en hoeveel het water mag opwarmen voor herstart.",`
        ...
        `:""}
      `)}function Km(){let e=h("otEnabled"),o=h("cicPollingEnabled")||h("cicFeedUrl"),r=h("cicCompatibilityMode"),n=h("otLinkProblem")||h("cicDataStale")||h("cicJsonFeedOk");if(!e&&!o&&!r&&!n)return"";let a=qt("cicPollingEnabled"),i=qt("otEnabled"),l=({label:g,value:f,active:v=!1})=>`
      <div class="oq-settings-integration-diagnostic-item${v?" is-warning":""}">
        <dt>${s(g)}</dt>
        <dd>${s(f)}</dd>
      </div>
    `,p=(g,f,v="Actief",S="Normaal",q={})=>{if(!h(g))return"";let C=U(g);return l({label:f,value:C?v:S,active:q.warningWhenActive?C:!1})},u=(g,f,v={})=>{let S=v.fallbackKey||"";return!h(g)&&!(S&&h(S))?"":l({label:f,value:ke(h(g)?g:S,v)})},c=(g,f)=>{let v=f.filter(Boolean).join("");return v?`
    ...
    `:"";return J("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
        ...
        ${w}
      `)}function jm(){if(!Lr.some(x=>h(x)))return"";let o=qt("cicPollingEnabled"),r=qt("otEnabled"),n=(x={})=>x.haValueKey||(x.haKeys||[]).find(E=>!/valid$/i.test(E))||"",a=(x={})=>x.haValidKey||(x.haKeys||[]).find(E=>/valid$/i.test(E))||"",i=(x="",E="")=>!!x&&!!E&&h(x)&&h(E)&&U(E),l=(x={})=>i(n(x),a(x)),p={mqttCoolingDewPoint:"cooling_dew_point",mqttOutsideTemperature:"outside_temperature",mqttRoomTemperature:"room_temperature",mqttRoomSetpoint:"room_setpoint",mqttHeatingEnable:"heating_enable",mqttCoolingEnable:"cooling_enable"},u=(x={})=>x.mqttTopicKey||p[x.valueKey]||"",c=(x="")=>{if(!x)return!0;let E=t.mqttStatus?.input_enabled;return E&&typeof E=="object"&&Object.prototype.hasOwnProperty.call(E,x)?E[x]!==!1:!0},m=x=>/\bMQTT\b/i.test(String(x||"")),b=(x,E={})=>x==="CIC"?o:x==="OT thermostat"?r:x==="HA input"?l(E):x==="CIC or HA input"?o||l(E):m(x)?c(u(E)):x==="Flowmeter HP2"?h("hp2Flow"):x==="Local aggregate HP1/HP2"?h("flowLocal")||h("hp2Flow"):!0,y=(x,E={})=>x==="CIC"&&!o?"CIC-polling staat uit":x==="OT thermostat"&&!r?"OpenTherm staat uit":x==="HA input"&&!l(E)?"HA-bron ongeldig":x==="CIC or HA input"&&!o&&!l(E)?"CIC en HA ontbreken":m(x)&&!c(u(E))?"MQTT-topic staat uit":x==="Flowmeter HP2"&&!h("hp2Flow")?"HP2-flow ontbreekt":x==="Local aggregate HP1/HP2"&&!h("flowLocal")&&!h("hp2Flow")?"Lokale flow ontbreekt":"",w=(x,E="Actief",D="Normaal")=>h(x)?U(x)?E:D:"",g=(x,E={})=>{let D=String(x||"").trim();return D?E.optionLabels?.[D]||we(D):""},f=(x,E={})=>{let D=String(T(x)||"").trim();return D?g(D,E):""},v=x=>{let E=No(x,"");return E?we(E):""},S=x=>{let E=String(No(x,"")||"").trim();return!E||E==="None"?"":we(E)},q=(...x)=>x.find(E=>String(E||"").trim())||"",C=()=>{let x=f("waterSupplySource");if(String(T("waterSupplySource")||"")==="Local"&&h("localWaterSupplyTempSource")){let E=f("localWaterSupplyTempSource");return E?`${x} - ${E}`:x}return x},$=()=>{let x=String(T("flowSource")||"").trim();if(x==="Outdoor unit"){if(h("qFlowSource")){let E=String(T("qFlowSource")||"").trim(),D=String(T("hpGeneration")||"").trim();return E==="Local"||E==="Auto"&&D==="V1"?E==="Auto"?"Lokaal (auto)":"Lokaal":q(f("outdoorUnitFlowMode"),E==="Auto"?"Buitenunit (auto)":"Buitenunit")}return q(f("outdoorUnitFlowMode"),"Quatt-flow")}return we(x)},M=()=>{let x=String(T("outsideTempSource")||"").trim();if(x==="MQTT"&&!c("outside_temperature"))return"MQTT-topic staat uit";if(x!=="Auto")return we(x);let E=A("outsideTempLocalAggregated"),D=A("outsideTempHa"),B=A("mqttOutsideTemperature"),Q=!Number.isNaN(E),ne=h("outsideTempHaValid")?U("outsideTempHaValid")&&!Number.isNaN(D):!Number.isNaN(D),be=c("outside_temperature")&&h("mqttOutsideTemperatureValid")&&U("mqttOutsideTemperatureValid")&&!Number.isNaN(B),te=[Q?{label:"Buitenunit",value:E}:null,ne?{label:"HA-invoer",value:D}:null,be?{label:"MQTT",value:B}:null].filter(Boolean);return te.length?te.reduce((xe,ye)=>ye.value<xe.value?ye:xe,te[0]).label:"Auto"},V=x=>{if(!h(x))return NaN;let E=Number(T(x));if(Number.isFinite(E))return E;let B=String(t.entities[x]?.state??"").trim().replace(",",".").match(/-?\d+(?:\.\d+)?/);return B?Number(B[0]):NaN},F=(x,E="")=>{if(!h(x))return!1;let D=V(x);return(E?U(E):!0)&&Number.isFinite(D)},ee=()=>{let x=String(T("coolingDewPointSource")||"").trim();if(x==="Home Assistant")return F("coolingDewPointHa","coolingDewPointHaValid")?"HA-invoer":"HA-invoer ontbreekt";if(x==="MQTT")return c("cooling_dew_point")?F("mqttCoolingDewPoint","mqttCoolingDewPointValid")?"MQTT":"MQTT ontbreekt of verouderd":"MQTT-topic staat uit";let E=F("coolingDewPointHa","coolingDewPointHaValid"),D=c("cooling_dew_point")&&F("mqttCoolingDewPoint","mqttCoolingDewPointValid");if(E&&D){let B=V("coolingDewPointHa");return V("mqttCoolingDewPoint")>B?"MQTT":"HA-invoer"}return E?"HA-invoer":D?"MQTT":x?we(x):"Auto"},H=({label:x,value:E="",key:D="",active:B=!1,status:Q="",statusTone:ne="",statusTitle:be=""})=>{let te=E||(D?ke(D):"");if(!te&&!Q)return"";let xe=String(ne||"").replace(/[^a-z0-9_-]/gi,""),ye=be||Q,le=Q?`<em class="oq-settings-source-status${xe?` oq-settings-source-status--${s(xe)}`:""}" title="${s(ye)}" aria-label="${s(`${Q}: ${ye}`)}">${s(Q)}</em>`:"";return`
        <div class="oq-settings-source-row${B?" is-warning":""}${Q?" has-status":""}">
          <span>${s(x)}${le}</span>
          <strong>${s(te)}</strong>
        </div>
      `},G=({label:x="HA-invoer",valueKey:E="",validKey:D="",value:B=""})=>{if(!E||!D||!h(E)||!h(D))return[];let Q=U(D);return[H({label:x,key:E,value:B,status:Q?"Geldig":"Ongeldig",statusTone:Q?"valid":"invalid",statusTitle:Q?"Home Assistant geeft dit signaal geldig door. OpenQuatt mag deze HA-invoer gebruiken.":"Home Assistant geeft dit signaal niet geldig door. OpenQuatt gebruikt deze HA-invoer dan niet als bron."})]},X=({label:x="MQTT",valueKey:E="",validKey:D="",value:B="",topicKey:Q=""})=>{if(!E||!D||!h(E)||!h(D))return[];if(!c(Q||p[E]))return[];let ne=U(D),be=ne?"MQTT heeft een geldige, recente waarde ontvangen. OpenQuatt mag deze MQTT-invoer gebruiken.":"MQTT heeft nog geen geldige recente waarde ontvangen. OpenQuatt gebruikt deze MQTT-invoer dan niet als bron.";return[H({label:x,key:E,value:ne?B:"\u2014",status:rs(D),statusTone:ne?"valid":"invalid",statusTitle:be})]},de=({title:x,icon:E="",content:D="",rows:B=[],copy:Q="",className:ne=""})=>{let be=B.filter(Boolean).join("");return!D&&!be&&!Q?"":`
        <section class="oq-settings-source-group${ne?` ${s(ne)}`:""}">
          <h5>
            ${E?`<span class="oq-settings-source-group-icon">${st(E,"oq-settings-source-group-icon-svg")}</span>`:""}
            <span>${s(x)}</span>
          </h5>
      ...
      `},se=(x,E={})=>{if(!h(x))return{markup:"",warning:""};let D=t.entities[x]||{},B=String(T(x)||""),Q=Ft(D),ne=new Set(E.hiddenOptions||[]),be=B&&ne.has(B),te=Q.filter(ue=>!ne.has(ue)&&b(ue,E)),xe=B&&!b(B,E),ye=B==="HA input"&&E.keepUnavailableCurrent!==!0,Ht=(be&&!te.includes(B)?[B,...te]:xe&&!ye&&!te.includes(B)?[B,...te]:te).map(ue=>{let bo=g(ue,E);return`<option value="${s(ue)}" ${ue===B?"selected":""}>${s(bo)}</option>`}).join("");return{markup:`
        ...
        <article class="oq-settings-source-card" data-oq-settings-field="${s(x||B.key)}">
    ...
    `}function Sw(){if(!t.root||t.appView!=="settings")return!1;let e=t.root.querySelector(".oq-settings-group-nav"),o=t.root.querySelector(".oq-settings-group-stack");if(!e||!o)return!1;let r=Pe.has(t.settingsGroup)?t.settingsGroup:me[0].id;if(r==="service")return!1;let n=e.querySelectorAll(".oq-settings-group-button");if(n.length!==me.length)return!1;n.forEach(g=>{let v=String(g.dataset.groupId||"")===r;g.classList.toggle("is-active",v),g.setAttribute("aria-pressed",v?"true":"false")}),o.querySelectorAll(".oq-settings-info").forEach(g=>{let f=String(g.dataset.oqSettingsInfo||""),v=t.settingsInfoOpen===f;g.classList.toggle("is-open",v);let S=g.querySelector(".oq-settings-info-popover");S&&(S.hidden=!v);let q=g.querySelector(".oq-settings-info-button");q&&q.setAttribute("aria-expanded",v?"true":"false")}),o.querySelectorAll("[data-oq-settings-field]").forEach(g=>{let f=String(g.dataset.oqSettingsField||"");if(!f)return;let v=g.querySelector(".oq-settings-static-value");if(v){let C=O(f);v.textContent!==C&&(v.textContent=C)}g.querySelectorAll("select[data-oq-field]").forEach(C=>{let $=String(C.dataset.oqField||f),M=String(T($)||"");C.value!==M&&(C.value=M)}),g.querySelectorAll("input[data-oq-field]").forEach(C=>{let $=String(C.dataset.oqField||f),M=String(Ee($)||"");C.value!==M&&(C.value=M)});let S=g.querySelector(".oq-helper-slider-meta strong"),q=g.querySelector('input[type="range"][data-oq-field]');if(S&&q){let C=K(f,he(f,T(f)));S.textContent!==C&&(S.textContent=C)}}),o.querySelectorAll("[data-select-key]").forEach(g=>{let f=String(g.dataset.selectKey||""),v=String(g.dataset.selectOption||""),S=String(T(f)||""),q=v===S;g.classList.toggle("is-active",q),g.setAttribute("aria-pressed",q?"true":"false"),f==="strategy"?g.disabled=t.loadingEntities||t.busyAction==="save-strategy":f==="hpGeneration"?g.disabled=t.loadingEntities||t.busyAction==="save-hpGeneration":f==="curveControlProfile"?g.disabled=t.loadingEntities||t.busyAction==="save-curveControlProfile":f==="phResponseProfile"&&(g.disabled=t.loadingEntities||t.busyAction==="save-phResponseProfile");let C=g.closest(".oq-settings-choice-card-shell");C&&C.classList.toggle("is-active",q)});let a=o.querySelector(".oq-settings-choice-card--static.oq-settings-choice-card--custom");if(a){let g=String(T("phResponseProfile")||"")==="Custom";a.classList.toggle("is-active",g),a.querySelectorAll("input[data-oq-field]").forEach(v=>{let S=String(v.dataset.oqField||""),q=String(Ee(S)||"");v.value!==q&&(v.value=q)})}o.querySelectorAll('[data-oq-action="toggle-overview-control"][data-control-key]').forEach(g=>{let f=String(g.dataset.controlKey||""),v=!!T(f),S=String(g.dataset.onLabel||"Aan"),q=String(g.dataset.offLabel||"Uit"),C=String(g.dataset.switchTitle||f),$=v?S:q;g.dataset.controlState=v?"off":"on",g.classList.toggle("is-on",v),g.setAttribute("aria-checked",v?"true":"false"),g.setAttribute("aria-label",`${C}: ${$}`),g.disabled=t.loadingEntities||t.busyAction===`switch-${f}`}),o.querySelectorAll("[data-oq-switch-pill]").forEach(g=>{let f=String(g.dataset.oqSwitchPill||""),v=!!T(f),S=String(g.dataset.onLabel||"Aan"),q=String(g.dataset.offLabel||"Uit"),C=v?S:q;g.classList.toggle("is-on",v),g.textContent!==C&&(g.textContent=C)}),o.querySelectorAll("[data-oq-switch-copy]").forEach(g=>{let f=String(g.dataset.oqSwitchCopy||""),v=!!T(f),S=String(g.dataset.onCopy||""),q=String(g.dataset.offCopy||""),C=v?S:q;g.hidden=!C,g.textContent!==C&&(g.textContent=C)});let i=o.querySelector('button[data-oq-action="open-generation-modal"]')?.closest(".oq-settings-quickstart-status");if(i){let g=i.querySelector(".oq-settings-quickstart-status-value"),f=i.querySelector(".oq-settings-quickstart-status-copy"),v=i.querySelector('button[data-oq-action="open-generation-modal"]'),S=dt(),q=t.entities.hpGeneration||{},C=h("hpGeneration")&&Ft(q).length>0;if(g){let $=S||"Onbekend";g.textContent!==$&&(g.textContent=$)}if(f){let $="Pas dit aan als je een andere Quatt Hybrid hebt.";f.textContent!==$&&(f.textContent=$)}v&&(v.disabled=!C||t.loadingEntities||t.busyAction==="save-hpGeneration")}let l=o.querySelector('button[data-oq-action="open-cm100-commissioning-modal"]')?.closest(".oq-settings-quickstart-status");if(l){let g=l.querySelector(".oq-settings-quickstart-status-value"),f=l.querySelector(".oq-settings-quickstart-status-copy"),v=l.querySelector('button[data-oq-action="open-cm100-commissioning-modal"]'),S=da(),q=k("cm100Active");g&&g.textContent!==S&&(g.textContent=S);let C=q?"CM100 is actief en klaar voor commissioning.":"Open de modal om CM100 te starten en de taken hieronder te ontgrendelen.";f&&f.textContent!==C&&(f.textContent=C),v&&(v.disabled=t.loadingEntities)}let p=o.querySelector('button[data-oq-action="reset"]')?.closest(".oq-settings-quickstart-status");if(p){let g=p.querySelector(".oq-settings-quickstart-status-value"),f=p.querySelector(".oq-settings-quickstart-status-copy"),v=p.querySelector('button[data-oq-action="reset"]'),S=t.complete===!0?"Afgerond":t.complete===!1?"Open":"Laden...",q=t.complete===!0?"Quick Start is afgerond. Je kunt de status hier altijd weer openen met een reset.":t.complete===!1?"Quick Start staat nog open. Gebruik de resetknop om opnieuw te beginnen.":"De status van Quick Start wordt nog geladen.";g&&g.textContent!==S&&(g.textContent=S),f&&f.textContent!==q&&(f.textContent=q),v&&(v.disabled=t.busyAction==="reset")}let u=o.querySelectorAll("[data-oq-access-security-item]");u.length&&u.forEach(g=>{let f=String(g.dataset.oqAccessSecurityItem||""),v=g.querySelector(".oq-settings-quickstart-status-value"),S=g.querySelector(".oq-settings-quickstart-status-copy"),q=g.querySelector("button[data-oq-action]");if(f==="login"){let C=gn(),$=hn();v&&v.textContent!==C&&(v.textContent=C),S&&S.textContent!==$&&(S.textContent=$)}else if(f==="api"){let C=gr(),$=hr();v&&v.textContent!==C&&(v.textContent=C),S&&S.textContent!==$&&(S.textContent=$)}q&&(q.disabled=!1)});let c=o.querySelector(".oq-settings-system-summary");if(c){let g=c.querySelectorAll(".oq-settings-system-row"),f={uptime:nr(),ip:ar(),updates:lo(),datetime:pa(),espTemp:ja(),restart:"Opnieuw opstarten"};g.forEach(q=>{let C=q.querySelector(".oq-settings-system-row-value"),$=q.dataset.oqDiagnosticsRow||"";if(C&&Object.prototype.hasOwnProperty.call(f,$)){let M=f[$];C.textContent!==M&&(C.textContent=M)}});let v=c.querySelector('button[data-oq-action="open-update-modal"]');v&&(v.disabled=!1);let S=c.querySelector('button[data-oq-action="open-restart-confirm"]');if(S){let q=t.busyAction==="restartAction";S.disabled=q,S.textContent=q?"Herstarten...":"Herstarten"}}o.querySelectorAll(".oq-settings-hp-offset-row").forEach(g=>{let f=String(g.dataset.oqSettingsField||""),v=String(g.dataset.oqHpOffsetRawKey||""),S=String(g.dataset.oqHpOffsetFinalKey||"");if(!f||!v||!S)return;let q=Oe(f),C=fn(v,S,f),$=$e(Ee(f)),M=Number.isFinite(C)&&Number.isFinite($)?Lt(C+$,q.uom||"\xB0C",2):Se(S,2),V=g.querySelector("[data-oq-hp-offset-active]");if(V){let H=`${Se(S,2)} actief`;V.textContent!==H&&(V.textContent=H)}let F=g.querySelector("[data-oq-hp-offset-raw]");if(F){let H=Number.isFinite(C)?Lt(C,q.uom||"\xB0C",2):Se(v,2);F.textContent!==H&&(F.textContent=H)}let ee=g.querySelector("[data-oq-hp-offset-final]");ee&&ee.textContent!==M&&(ee.textContent=M)});let m=o.querySelector(".oq-settings-curve-shell"),b=oe();if(!!m!==b)return!1;let y=!!a,w=String(T("phResponseProfile")||"")==="Custom";return y===w}vu({patch:Sw});function qw({label:e,value:o,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(o)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function Hn(e,o=!1){return e.map(r=>qw({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?Je(r.key):r.value,status:o})).join("")}function Al(e){return`
    ...
    `}function Rl(e,o,r,n){return`${r?Ew(n):""}${Tw(e,o)}`}function eg(e,o,r,n,a){if(!e)return;let i=Ce({mode:o,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&(xt(e,Rl(o,r,n,a)),e.dataset.renderSignature=i)}function Ya(e,o,r=""){return`
    ...
    `:""}function Ar({className:e,action:o,label:r,busy:n=!1,loading:a=!1,attrs:i=""}){return`
      ...
      `:s(n?"Bezig...":r)}</button>
    `}function Fw(e){if(e.kind==="openquatt-control"){let o=t.busyAction==="openquatt-regulation",r=(t.loadingEntities||t.entitySyncInFlight)&&!h("openquattResumeAt");return k("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${Ar({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:o})}</div>`:`
    ...
    `}function yS(e,o){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(xt(e,wg(o)),!0):(xt(r,`
      <h3>Warmtepompen</h3>
      ${hg(o)}
