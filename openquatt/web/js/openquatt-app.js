...
`)),r.toString()}function gs(e,o){let r=[],n=[],a=[],i="";return e.forEach(l=>{let p=Hf(l);if(!p)return;let u=[...a,p],c=Uu(u,o);if(a.length&&c.length>Tf){r.push({keys:n,body:i}),n=[l],a=[p],i=Uu(a,o);return}n=[...n,l],a=u,i=c}),a.length&&r.push({keys:n,body:i}),r}async function Mf(e){let o=op(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body};if(typeof AbortController=="function"){let a=new AbortController,i=window.setTimeout(()=>a.abort(),o);try{let l=await fetch(Bu,{...r,signal:a.signal});if(!l.ok)throw new Error(`bulk entities HTTP ${l.status}`);return l.json()}catch(l){throw a.signal.aborted?new Error(`bulk entities request timed out after ${o}ms`):l}finally{window.clearTimeout(i)}}let n=await fetch(Bu,r);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function Rf(){let e=op(),o={cache:"no-store",headers:{"Cache-Control":"no-store"}};if(typeof AbortController=="function"){let n=new AbortController,a=window.setTimeout(()=>n.abort(),e);try{let i=await fetch(Ku,{...o,signal:n.signal});if(!i.ok)throw new Error(`service status HTTP ${i.status}`);return i.json()}catch(i){throw n.signal.aborted?new Error(`service status request timed out after ${e}ms`):i}finally{window.clearTimeout(a)}}let r=await fetch(Ku,o);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}function Pf(e={}){let o=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(o).forEach(([r,n])=>{Nr.has(r)&&(t.optionalMissingEntities&&delete t.optionalMissingEntities[r],t.entities[r]=np(r,t.entities[r],n))})}async function Nf(e){if(!e.length)return{ok:!0,message:""};try{let o=await Rf();return Pf(o),t.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(o){return{ok:!1,message:o.message||String(o)}}}async function L(e,o="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(v=>a||Nr.has(v)||!$f(v,n));if(!i.length)return;let l=i.filter(v=>Nr.has(v)),p=i.filter(v=>!Nr.has(v)),c=(await Nf(l)).ok?p:[...p,...l];if(!c.length){Gu(),Yr(At()),t.busyAction||(ju(),t.controlError="");return}let m=Number(r.concurrency),b=Number.isFinite(m)&&m>0?Math.floor(m):Ct,y=gs(c,o),w=Math.max(1,Math.min(b,Ct)),g=[];for(let v=0;v<y.length;v+=w){let S=y.slice(v,v+w),q=await Promise.allSettled(S.map(async C=>({chunk:C,payload:await Mf(C)})));g.push(...q)}g.some(v=>v.status==="fulfilled")&&(t.lastEntityResponseAt=Date.now());let f="";g.forEach(v=>{if(v.status!=="fulfilled"){let H=v.reason.message||String(v.reason);f||(f=H);return}let{chunk:S,payload:q}=v.value,C=q?.entities&&typeof q.entities=="object"?q.entities:{},x=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(H=>{if(Object.prototype.hasOwnProperty.call(C,H)){t.optionalMissingEntities&&delete t.optionalMissingEntities[H],t.entities[H]=np(H,t.entities[H],C[H]);return}let V=R[H];V?.optional?x.has(H)&&Af(H,n):f||(f=`${V?.name||H} ontbreekt in bulk response`)})}),Gu(),Yr(At()),f?(rp(f),t.deviceReconnectMode?t.controlError="":t.controlError=`Niet alle helpervelden konden worden ververst. ${f}`):t.busyAction||(ju(),t.controlError="")}function np(e,o={},r={}){let n={...o||{},...r||{}};return R[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(o?.state??"").trim()&&(n.state=o.state),!String(r.value??"").trim()&&String(o?.value??"").trim()&&(n.value=o.value),!Array.isArray(r.option)&&Array.isArray(o?.option)&&(n.option=o.option),!Array.isArray(r.options)&&Array.isArray(o?.options)&&(n.options=o.options)),n}function Gu(){t.complete=Jn(),t.stage=t.complete===!0?"Gereed":t.complete===!1?"Quick Start":"Laden...",t.summary=Pc(),t.appView==="diagnosis"&&!rt()&&nt(Qt(),{syncMode:"replace",forceSync:!0}),t.appView||nt(zr()||Qt(),{syncMode:"replace",forceSync:!0})}function Of(){let e=ls();return t.appView==="settings"?[...new Set([...e,...cs()])]:t.appView==="energy"||t.appView==="results"?Zr(t.appView,{forceFast:!0}):t.appView==="overview"||t.appView==="diagnosis"?Zr(t.appView,{forceFast:!0}):[...new Set(e)]}function Df(e=[]){let o=new Set(e);return(t.appView==="settings"?cs():t.appView==="overview"||t.appView==="diagnosis"?[...new Set([...Zr(t.appView,{includeBulk:!0}),...xt])]:t.appView==="energy"||t.appView==="results"?[...new Set([...Zr(t.appView,{forceFast:!0}),...xt])]:[...new Set(["setupComplete","strategy",...je])]).filter(n=>!o.has(n))}function zu(e=Date.now(),o={}){return o.forceBulk===!0?!0:e-Number(t.lastBulkEntitySyncAt||0)>=Sc}async function ce(e={}){if(t.nativeOpen||t.loadingEntities||t.draggingCurveKey||t.busyAction||t.settingsInteractionLock)return;if(t.updateInstallBusy||na()){t.pendingEntitySyncOptions=null;return}if(t.focusedField&&t.appView!=="settings")return;if(t.entitySyncInFlight){bf(e);return}let o=Date.now();if(document.hidden&&o-Number(t.lastEntitySyncAttemptAt||0)<Ir)return;let r=t.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,p=!l&&!n&&zu(o,e),c=o-Number(t.lastStaticEntitySyncAt||0)>=qc||t.updateInstallBusy||t.updateInstallPhaseHint?xt:[],m=t.quickStartModalOpen&&t.currentStep==="flow-source"?ot:[],b=t.quickStartModalOpen&&t.currentStep==="thermostat-source"?ho:[],y=Ji()?Zt():[],w=n?[...Ei,...je,"setupComplete",...c]:i?[...Zr(a,{forceFast:l,includeBulk:p}),...c]:r==="settings"?[...new Set([...cs(),...y,...c])]:p?["setupComplete",...c,...je,"strategy",...go,...Ke,...oe()?me.map(g=>g.key):dt]:["setupComplete",...je,"strategy",...c];t.entitySyncInFlight=!0,t.lastEntitySyncAttemptAt=o;try{let g=t.deviceReconnectMode,f=kf(o,e)?await xf():{ok:!0,message:""};if(!f.ok){rp(f.message),n||d();return}if(await L([...new Set([...w,...m,...b])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?kt:Ct}),t.lastFastEntitySyncAt=Date.now(),p&&(a==="overview"||a==="diagnosis")&&!n&&(t.lastBulkEntitySyncAt=t.lastFastEntitySyncAt),c.length&&(t.lastStaticEntitySyncAt=t.lastFastEntitySyncAt),n)return;i&&!t.overviewMetadataHydrated&&!t.overviewMetadataHydrating&&Yu();let v=g!==t.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="diagnosis"?await Xi():!1,C=S?!1:t.appView==="results"?await ia():!1,x=S?!1:Ji()?(await Promise.all([qu(),ia({metaOnly:!0})])).some(Boolean):!1,H=S||!Bc()?!1:await qo(),V=S||!Kc()?!1:await zt(),K=S||!Ou()?!1:await Co(),M=yu();if(S&&!t.nativeOpen&&ep(us(a)),v){d();return}if(q&&t.appView==="diagnosis"&&!t.root?.querySelector(".oq-overview-trends")){d();return}if(C&&t.appView==="results"&&!t.root?.querySelector(".oq-energy-history")){d();return}if(x&&t.appView==="settings"){d();return}if(H&&(t.systemModal==="login"||$t())){d();return}if(V&&t.appView==="settings"){d();return}if(t.systemModal==="mqtt-sensors"){la()!==t.mqttSensorsModalRenderSignature&&d();return}if(K&&t.appView==="settings"){d();return}let I=!!(t.updateModalOpen||t.systemModal||t.interfacePanelOpen);if(M!==t.headerRenderSignature)if(I&&zi())t.headerRenderSignature=M;else{d();return}else zi();if(t.appView==="settings"&&String(t.systemModal||"").startsWith("service-task-")){Br()!==t.settingsRenderSignature&&d();return}if(I)return;if(t.appView==="settings"){let z=Br();if(z!==t.settingsRenderSignature&&!t.focusedField){d();return}if(!wu()){t.focusedField||d();return}t.focusedField&&(t.settingsRenderSignature=z);return}if(t.appView==="diagnosis"){xu()||d();return}if(t.appView==="energy"){Eu()||d();return}if(t.appView==="results"){Cu()||d();return}ku()||d()}catch(g){n||(t.controlError=`Helperstatus kon niet worden geladen. ${g.message}`,d())}finally{t.entitySyncInFlight=!1;let g=t.pendingEntitySyncOptions;t.pendingEntitySyncOptions=null,g&&!t.nativeOpen&&window.setTimeout(()=>{ce(g)},0),l&&(a==="overview"||a==="diagnosis")&&!n&&!t.nativeOpen&&!g&&zu(Date.now())&&window.setTimeout(()=>{ce({forceBulk:!0})},hf)}}function ft(){return typeof window<"u"&&window.__OQ_DEV_META&&typeof window.__OQ_DEV_META=="object"?window.__OQ_DEV_META:{}}function Lf(){let e=String(E("hpGeneration")||"").trim();return e||""}function Jr(e){let o=String(e||"").trim().toLowerCase();return o==="single"||o.includes("quatt single")||o.includes("openquatt single")?"single":o==="duo"||o.includes("quatt duo")||o.includes("openquatt duo")?"duo":""}function Ff(){if(!Array.isArray(zo))return"";if(zo.some(o=>h(o)))return"duo";let e=t.optionalMissingEntities||{};return zo.every(o=>e[o])?"single":""}function hs(e){let o=Jr(e);return(o==="single"||o==="duo")&&typeof t<"u"&&t&&typeof t=="object"&&(t.lastKnownInstallationTopology=o),o}function If(){if(typeof t<"u"&&t&&typeof t=="object"){let e=String(t.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function ee(){let e=Jr(E("installationTopology"));if(e==="single"||e==="duo")return hs(e);let o=Jr(ft().installation);if(o==="single"||o==="duo")return hs(o);let r=Ff();return r?hs(r):If()}function it(){let e=ee(),o=Lf();return e==="single"?o?`Quatt Single ${o}`:"Quatt Single":e==="duo"?o?`Quatt Duo ${o}`:"Quatt Duo":o?`Quatt Hybrid ${o}`:"Quatt Hybrid"}function Ne(){return"OpenQuatt"}function fs(e){let o=String(e||"").trim().toLowerCase();return o==="wifi"||o==="wi-fi"||o.includes("wifi")||o.includes("wi-fi")?"wifi":o==="eth"||o==="ethernet"||o.includes("ethernet")?"eth":""}function Ge(e=Jt()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function xo(e=ee()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function Xr(){let e=String(E("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(ft().hardwareProfile||e).trim().toLowerCase()}function Jt(){return fs(E("connectionText")||ft().connection)}function ap(){let e=Jt();return e==="wifi"?"eth":e==="eth"?"wifi":""}function ip(){let e=ee();return e==="single"?"duo":e==="duo"?"single":""}function Xo(e=ee(),o=Jt()){let r=xo(e),n=Xr();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${Ge(o)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${Ge(o)}`:n==="waveshare"?`Waveshare ${r} ${Ge(o)}`:`${Ne()} ${r} ${Ge(o)}`}function bs(){let e=k("timeValid"),o=String(E("timeNowHhmm")||"").trim();if(o&&o!=="invalid")return o;if(h("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function ua(){return h("timeValid")&&!k("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${bs()}`}function ca(e){if(!Number.isFinite(e)||e<0)return"\u2014";let o=Math.floor(e),r=Math.floor(o/1440),n=Math.floor(o%1440/60),a=o%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function _f(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Wf(e){let o=e?.state;if(o!==""&&o!==null&&o!==void 0){let a=Number(o);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function er(){let e=Wf(t.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=_f(t.entities.uptime);if(n==="d")return ca(e*1440);if(n==="h")return ca(e*60);if(n==="s")return ca(e/60)}let o=String(t.entities.uptimeReadable?.state??t.entities.uptimeReadable?.value??"").trim();if(o&&o.toLowerCase()!=="unknown")return o;let r=Number(ft().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":ca((Date.now()-r)/6e4)}function tr(){let e=String(t.entities.ipAddress?.state??t.entities.ipAddress?.value??"").trim();if(e)return e;let o=String(ft().ipAddress||"").trim();return o||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function Xt({key:e,value:o,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `;if(!c)return y;let w=u||qe(o),g=m||`${e}-${o}`;return`
  ...
  `}function ys(e,o,r="",n=""){let a=o?r:n;return a?`<p data-oq-switch-copy="${s(e)}" data-on-copy="${s(r)}" data-off-copy="${s(n)}">${s(a)}</p>`:""}function sp(e,o,r,n="",a="",i=""){if(!h(e))return"";let l=!!E(e),p=t.loadingEntities||t.busyAction===`switch-${e}`;return pe(e,o,r,`
      <div class="oq-settings-compact-switch-field">
        ${Pt(e,o,l,p)}
        ${ys(e,l,n,a)}
      </div>
    `,i)}function lp(e,o,r,n,a=""){if(!h(e))return"";let i=!!E(e),l=t.loadingEntities||t.busyAction===`switch-${e}`;return pe(e,o,r,`
      <div class="oq-settings-compact-switch-field">
        ${Pt(e,o,i,l)}
        ${n?`<p>${s(n)}</p>`:""}
      </div>
    `,a)}function da(e,o,r){if(!h(e))return"";let n=!!E(e),a=t.loadingEntities||t.busyAction===`switch-${e}`;return`
  ...
  `}function ws(e,o,r,n=""){if(!h(e))return"";let a=_r(E(e));return pe(e,o,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${t.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function te(e,o,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(o)}</h3><p>${s(r)}</p></div>${n}</section>`}var up=250;function pp(){if(typeof window>"u")return[];let e=window.__OQ_DEV_WEBSERVER_LOGS__,o=typeof e=="function"?e():e;return Array.isArray(o)?o.map(r=>String(r||"")).filter(r=>r.trim()!==""):[]}function nr(){return typeof window>"u"?!1:pp().length>0}function Bf(){return`${Re()}/events`}function Kf(){return`${Re()}/openquatt/logs/recent`}function ma(){let e=t.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let o=String(e.state??e.value??"").toLowerCase();return o==="on"||o==="true"||o==="1"}function ga(){return t.nativeOpen?"Niet beschikbaar":nr()?"Voorbeeld":t.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function jf(e){let o=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(o/3600),n=Math.floor(o%3600/60),a=o%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function dp(e){let o=Number(e)||0;if(o>9466848e5){let r=e instanceof Date?e:new Date(o),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return jf(o)}function Uf(e){let o=Number(e)||0;if(o>9466848e5)return new Date(o).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(o/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Gf(){return t.nativeOpen?"Niet beschikbaar":nr()?ma()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":ma()?"Buffer aan":"Buffer uit"}function zf(){return ma()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function Ss(){return t.entities?.debugLevel||null}function mp(e=Ss()){let o=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return o.length?o:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function Qf(e=Ss()){let o=String(e?.value??e?.state??"").trim(),r=mp(e);return r.includes(o)?o:r.includes("INFO")?"INFO":r[0]||""}function gp(e,o=null){if(!e||!o)return!1;let r=Number(e.seq),n=Number(o.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(o.raw??o.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),p=Number(o.receivedAt??o.ts??0);return Math.abs(l-p)<=2e3}function Yf(e,o){let r=Number(e.receivedAt??e.ts??0),n=Number(o.receivedAt??o.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(o.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(o.raw??""))}function hp(e,{prepend:o=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=o?[...e,...t.webServerLogEntries]:[...t.webServerLogEntries,...e];r.sort(Yf);let n=[];for(let a of r){let i=n[n.length-1]||null;gp(a,i)||n.push(a)}t.webServerLogEntries=n.slice(-up)}function qs(e,o={}){let r=hb(e).trimEnd(),n=Number(o.receivedAt),a=Number(o.seq);return{raw:e,text:r,tone:fb(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function Zf(e,o){let n=Math.max(0,o-e-1)*9e4;return Date.now()-n}function Jf(){let e=pp(),o=e.length;return e.map((r,n)=>qs(r,{receivedAt:Zf(n,o),seq:n+1}))}function Xf(){let e=ha();e&&(e.scrollTop=e.scrollHeight)}function Nt(){let e=ha();return e?{scrollHeight:e.scrollHeight,scrollTop:e.scrollTop,stickToBottom:fa(e)}:null}function eb(e){if(!e)return;let o=ha();if(!o)return;if(e.stickToBottom){o.scrollTop=o.scrollHeight;return}let r=e.scrollTop+(o.scrollHeight-e.scrollHeight);o.scrollTop=Math.max(0,r)}function Ot(e,o=!0){if(!e)return;let r=Number(t.webServerLogScrollRestoreToken||0)+1;t.webServerLogScrollRestoreToken=r;let n=()=>{t.webServerLogScrollRestoreToken!==r||t.systemModal!=="webserver-logs"||eb(e)};if(o){window.requestAnimationFrame(n);return}n()}function fp(){return t.root?t.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}function bp(){let e=fp();return e?{scrollHeight:e.scrollHeight,scrollTop:e.scrollTop,stickToBottom:fa(e)}:null}function tb(e){if(!e)return;let o=fp();if(!o)return;if(e.stickToBottom){o.scrollTop=o.scrollHeight;return}let r=e.scrollTop+(o.scrollHeight-e.scrollHeight);o.scrollTop=Math.max(0,r)}function Ts(e,o=!0){if(!e)return;let r=Number(t.cm100CommissioningScrollRestoreToken||0)+1;t.cm100CommissioningScrollRestoreToken=r;let n=()=>{t.cm100CommissioningScrollRestoreToken!==r||t.systemModal!=="cm100-commissioning"||tb(e)};if(o){window.requestAnimationFrame(n);return}n()}function yp(){return t.root?t.root.querySelector("[data-oq-service-task-scroller]"):null}function vp(){let e=yp();return e?{scrollTop:e.scrollTop}:null}function ob(e){if(!e)return;let o=yp();o&&(o.scrollTop=Math.max(0,e.scrollTop))}function Es(e,o=!0){if(!e)return;let r=Number(t.serviceTaskModalScrollRestoreToken||0)+1;t.serviceTaskModalScrollRestoreToken=r;let n=()=>{t.serviceTaskModalScrollRestoreToken!==r||!String(t.systemModal||"").startsWith("service-task-")||ob(e)};if(o){window.requestAnimationFrame(n);return}n()}function wp(){return t.root?t.root.querySelector("[data-oq-history-storage-scroller]"):null}function Sp(){let e=wp();return e?{scrollTop:e.scrollTop}:null}function rb(e){if(!e)return;let o=wp();o&&(o.scrollTop=Math.max(0,e.scrollTop))}function Cs(e,o=!0){if(!e)return;let r=Number(t.historyStorageModalScrollRestoreToken||0)+1;t.historyStorageModalScrollRestoreToken=r;let n=()=>{t.historyStorageModalScrollRestoreToken!==r||t.systemModal!=="history-storage"||rb(e)};if(o){window.requestAnimationFrame(n);return}n()}async function ar(e={}){if(t.nativeOpen||typeof window.fetch!="function")return;let o=e.scrollState||Nt(),r=Number(t.webServerLogHistoryRequestToken||0)+1;t.webServerLogHistoryRequestToken=r,t.webServerLogHistoryLoading=!0,t.webServerLogHistoryError="";try{let n=await window.fetch(Kf(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(t.systemModal!=="webserver-logs"||t.webServerLogHistoryRequestToken!==r)return;let i=ab(a);t.webServerLogHistoryLoaded=!0,i.length>0&&(hp(i,{prepend:!0}),t.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),t.webServerLogRecentAnchorAt=Date.now())}catch(n){t.systemModal==="webserver-logs"&&t.webServerLogHistoryRequestToken===r&&(t.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{t.webServerLogHistoryRequestToken===r&&(t.webServerLogHistoryLoading=!1),t.systemModal==="webserver-logs"&&t.webServerLogHistoryRequestToken===r&&(d(),Ot(o))}}function nb(e,o=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?qs(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??o)}):null}function ab(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>nb(r,n+1)).filter(r=>r!==null)}function ib(e){if(!e||!Array.isArray(t.webServerLogRecentTail)||t.webServerLogRecentTail.length===0||Date.now()-Number(t.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?t.webServerLogRecentTail.includes(r):!1}function sb(e,o=t.webServerLogEntries){return!e||!Array.isArray(o)||o.length===0?!1:o.some(r=>gp(e,r))}function qp(){nr()&&t.webServerLogEntries.length===0&&(t.webServerLogEntries=Jf()),t.webServerLogCopyMessage="",t.webServerLogCopyError="",t.systemModal="webserver-logs",d(),L(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(t.systemModal!=="webserver-logs")return;let e=Nt();d(),Ot(e)}),Xf(),(!t.webServerLogHistoryLoaded||t.webServerLogEntries.length===0)&&ar()}function Ho(){t.webServerLogEntries=[],t.webServerLogError="",t.webServerLogHistoryError="",t.webServerLogHistoryLoading=!1,t.webServerLogHistoryLoaded=!1,t.webServerLogScrollRestoreToken=Number(t.webServerLogScrollRestoreToken||0)+1,t.webServerLogCopyMessage="",t.webServerLogCopyError="",t.webServerLogHistoryRequestToken+=1,t.webServerLogRecentTail=[],t.webServerLogRecentAnchorAt=0,t.systemModal==="webserver-logs"&&d()}function lb(){let e=Nt();bt(),t.webServerLogEnabled=null,t.webServerLogConnected=!1,Ho(),t.systemModal==="webserver-logs"&&ar({scrollState:e})}function ks(){if(nr()){bt();return}if(!(t.mounted&&!t.nativeOpen&&t.systemModal==="webserver-logs")){bt();return}if(t.webServerLogEnabled===!1){bt();return}t.webServerLogSource||cb()}function cb(){if(nr()){t.webServerLogEnabled=!0,t.webServerLogConnected=!1,t.webServerLogError="",d();return}if(typeof window.EventSource!="function"){t.webServerLogEnabled=!1,t.webServerLogConnected=!1,t.webServerLogError="Deze browser ondersteunt geen live logstream.",d();return}try{let e=new window.EventSource(Bf());t.webServerLogSource=e,e.addEventListener("open",ub),e.addEventListener("ping",pb),e.addEventListener("log",mb),e.onerror=db}catch(e){t.webServerLogEnabled=!1,t.webServerLogConnected=!1,t.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",bt(),d()}}function bt(){let e=t.webServerLogSource;if(e)try{e.close()}catch{}t.webServerLogSource=null,t.webServerLogConnected=!1}function ub(){if(!t.webServerLogSource||t.nativeOpen)return;let e=t.systemModal==="webserver-logs"?Nt():null;t.webServerLogEnabled=!0,t.webServerLogConnected=!0,t.webServerLogError="",d(),Ot(e)}$u({clearOutput:Ho,closeStream:bt,resetRecoveryState:lb});function pb(){if(!t.webServerLogSource||t.nativeOpen)return;let e=t.systemModal==="webserver-logs"?Nt():null;t.webServerLogEnabled=!0,t.webServerLogConnected||(t.webServerLogConnected=!0,t.webServerLogError="",d(),Ot(e))}function db(){if(!t.webServerLogSource)return;let e=t.systemModal==="webserver-logs"?Nt():null;t.webServerLogEnabled=!1,t.webServerLogConnected=!1,t.webServerLogError="De live logstream kon niet worden geopend.",bt(),d(),Ot(e)}function mb(e){if(!t.webServerLogSource||!e||typeof e.data!="string")return;let o=Nt(),r=gb(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>qs(u)).filter(u=>!ib(u)&&!sb(u));if(i.length===0)return;hp(i);let l=yb(),p=ha();bb(l),vb(i,l),t.webServerLogEnabled=!0,p&&o&&Ot(o,!1)}function gb(e){let o=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).trimEnd()}catch{}return o}function hb(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function fb(e){let o=String(e??""),r=Array.from(o.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(p=>Number(p)).filter(p=>Number.isFinite(p));for(let p=l.length-1;p>=0;p-=1){let u=l[p];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=o.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function bb(e){for(;t.webServerLogEntries.length>up;)t.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function yb(){return t.root?t.root.querySelector("[data-oq-webserver-log-output]"):null}function ha(){return t.root?t.root.querySelector("[data-oq-webserver-log-scroller]"):null}function fa(e){return e?e.scrollHeight-e.scrollTop-e.clientHeight<48:!1}function vb(e,o){if(!(!o||e.length===0)){o.dataset.webServerLogEmpty==="true"&&(o.dataset.webServerLogEmpty="false",o.innerHTML="");for(let r of e)o.insertAdjacentHTML("beforeend",Tp(r))}}function Tp(e){let o=dp(e.receivedAt),r=Uf(e.receivedAt);return`
  ...
  `}function wb(e=t.webServerLogEntries){return e.length?e.map(o=>Tp(o)).join(""):`
    ...
    `),t.webServerLogCopyError&&e.push(`<p class="oq-helper-modal-note oq-helper-modal-note--error">${s(t.webServerLogCopyError)}</p>`),t.webServerLogHistoryError&&e.push(`<p class="oq-helper-modal-note oq-helper-modal-note--error">${s(t.webServerLogHistoryError)}</p>`),t.webServerLogError&&e.push(`<p class="oq-helper-modal-note oq-helper-modal-note--error">${s(t.webServerLogError)}</p>`),e.length?e.join(""):""}function qb(){let e=ma(),o=t.loadingEntities||t.busyAction==="switch-webServerLogHistoryEnabled",r=Gf(),n=zf(),a=Tb();return`
...
`)}async function Ep(){let e=Eb();if(t.webServerLogCopyMessage="",t.webServerLogCopyError="",!e){t.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",d();return}try{if(!await jt(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");t.webServerLogCopyMessage=`${t.webServerLogEntries.length} logregel${t.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(o){t.webServerLogCopyError=o instanceof Error?o.message:"Kopi\xEBren naar het klembord is niet gelukt."}t.systemModal==="webserver-logs"&&d()}function Cp(){let e=nr();return`
    <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
  ...
  `}function Cb(){let e=t.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function $p(e){return Cb().includes(e)}function ya(){let e=Xr(),o=ee(),r=Jt(),n=ap();return e!=="heatpump_controller_q"||o!=="single"&&o!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:h("firmwareUpdateTarget")&&$p("alternate connection")&&h("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:Ge(r),targetLabel:Ge(n),currentBuildLabel:Xo(o,r),targetBuildLabel:Xo(o,n)}}function va(){let e=Xr(),o=ee(),r=ip(),n=Jt(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||o!=="single"&&o!=="duo"||!r||!a.includes(n)?null:{canSwitch:h("firmwareUpdateTarget")&&$p("alternate topology")&&h("installFirmwareUpdateTarget"),currentTopology:o,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:xo(o),targetLabel:xo(r),currentBuildLabel:Xo(o,n),targetBuildLabel:Xo(r,n)}}function Mo(e=t.updateTestFirmwarePr){let o=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(o)?o:""}function sr(){let e=Xr(),o=ee(),r=Jt(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||o!=="single"&&o!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${o}-${r}`,l=o==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${Ge(r)}`}}function tn(e=Mo(),o=sr()){if(!e||!o.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${o.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function ro(){if(Ye())return"Controleren";let e=Dt();if(e)return e.phaseLabel;if(As())return"Bijgewerkt";if(ir())return"Bezig";if(Ro())return"Beschikbaar";let o=Hp();if(o!==null&&o<=0)return"Actueel";let r=ft();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Ms()?"Actueel":he()?"Nog niet gecontroleerd":"\u2014"}function he(){return t.entities.firmwareUpdate||null}function wa(){let e=he();return e?String(e.state??e.value??"").trim().toLowerCase():""}function kb(){let e=t.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Sa(){return kb().toLowerCase()}function Ap(){let e=t.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let o=Number(e.value??e.state);return Number.isNaN(o)?Number.NaN:Math.max(0,Math.min(100,o))}function xs(){let e=String(t.updateInstallTargetVersion||"").trim(),o=le();return!e||!o?!1:on(o,e)>=0}function xb(e=he()||{}){let o=no(e),r=le(e);return!o||!r?!1:on(r,o)>=0}function $s(){return(xs()||xb())&&!Ye()&&!en()&&!Ro()}function As(){return(t.updateInstallCompleted||$s())&&!Ye()&&!Dt()&&!Ro()}function lr(){t.updateInstallBusy=!1,t.updateInstallTargetVersion="",t.updateInstallPhaseHint="",t.updateInstallProgressHint=Number.NaN,t.updateInstallMode="",t.updateInstallTargetConnection="",t.updateInstallTargetTopology="",Nb()}function ke(){t.updateManualUploadFile=null,t.updateManualUploadFileName="",t.updateManualUploadError=""}function Qe(e={}){e.clearPr&&(t.updateTestFirmwarePr=""),t.updateTestFirmwareConfirmed=!1,t.updateTestFirmwareError="",t.updateTestFirmwareBuild=null}function $b(){let e=Sa(),o=Ap();if(e==="starting"||e==="uploading"||e==="rebooting"){t.updateInstallPhaseHint=e,Number.isNaN(o)||(t.updateInstallProgressHint=e==="rebooting"?Math.max(o,100):o);return}if(t.updateInstallBusy){if(xs()){t.updateInstallPhaseHint="rebooting",t.updateInstallProgressHint=100;return}t.controlNotice.includes("opnieuw is opgestart")&&(t.updateInstallPhaseHint="rebooting",t.updateInstallProgressHint=100)}}function en(){let e=Sa();return e==="starting"||e==="uploading"||e==="rebooting"}function Dt(){$b();let e=Sa(),o=e==="starting"||e==="uploading"||e==="rebooting",r=o?e:t.updateInstallPhaseHint,n=Ap(),a=Number.isNaN(t.updateInstallProgressHint)?0:Math.round(t.updateInstallProgressHint),i=o&&!Number.isNaN(n)?Math.round(n):a;return!en()&&!t.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:t.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":t.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":t.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:t.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Ne()} gedownload en ge\xEFnstalleerd.`:t.updateInstallMode==="connection-switch"?`De ${Ge(t.updateInstallTargetConnection)}-build wordt nu naar ${Ne()} verzonden.`:t.updateInstallMode==="topology-switch"?`De ${Xo(t.updateInstallTargetTopology,t.updateInstallTargetConnection)}-build wordt nu naar ${Ne()} verzonden.`:`Firmware wordt nu naar ${Ne()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:t.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Ne()}.`:t.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${Ge(t.updateInstallTargetConnection)} is gestart.`:t.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${xo(t.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Ne()}.`}}function no(e=he()||{}){let o=String(e.latest_version||"").trim();if(o)return o;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function le(e=he()||{}){let o=String(t.entities.projectVersionText?.state||t.entities.projectVersionText?.value||"").trim();return o||String(e.current_version||"").trim()}function Ab(e=he()||{}){let o=le(e),r=String(e.current_version||"").trim();return!o||!r||o===r?!1:on(o,r)>0}function Hs(e=he()||{},o=oo()){let r=String(o||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=no(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function kp(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function on(e,o){let r=kp(e),n=kp(o);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function ir(){if($s())return!1;let e=wa();return t.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function Ye(){let e=wa();return t.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Ro(){let e=wa();if(!Hs())return!1;let o=Hp();return o!==null?o>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:ft().updateAvailable===!0}function Ms(){let e=wa();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Ab()}function Rs(){let e=he()||{},o=le(e)||"\u2014",r=Hs(e)?no(e):"",n=r?on(r,o):null;return!Ye()&&n!==null&&n<=0&&(r=""),{current:o,latest:r||"\u2014"}}function Hp(){let{current:e,latest:o}=Rs();return e==="\u2014"||o==="\u2014"?null:on(o,e)}function Mp(e=oo()){let o=String(e||"").trim().toLowerCase();return mi[o]||mi.main}function Hb(){let e=String((he()||{}).release_url||"").trim(),o=Mp();return!e||o.includes("/dev-latest")&&!e.includes("/dev-latest")||!o.includes("/dev-latest")&&e.includes("/dev-latest")?o:e}function Mb(){return Ne()}function oo(){return String(E("firmwareUpdateChannel")||t.entities.releaseChannelText?.state||t.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function Rb(){return Rs().latest!=="\u2014"}function xp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Pb(e,o=he()||{}){let r=xp(e);return r?xp(`${o.title||""} ${o.summary||""}`).includes(r):!0}function Rp(e){return new Promise(o=>window.setTimeout(o,e))}function rn(e=vo){let r=Date.now()+e;t.firmwareOtaQuietUntil=Math.max(Number(t.firmwareOtaQuietUntil||0),r),t.pendingEntitySyncOptions=null,au(),typeof bt=="function"&&bt(),t.firmwareOtaQuietTimer&&window.clearTimeout(t.firmwareOtaQuietTimer),t.firmwareOtaQuietTimer=window.setTimeout(()=>{t.firmwareOtaQuietTimer=null,t.firmwareOtaQuietUntil=0,!t.updateInstallBusy&&!t.nativeOpen&&_i()},e)}function Nb(){t.firmwareOtaQuietTimer&&(window.clearTimeout(t.firmwareOtaQuietTimer),t.firmwareOtaQuietTimer=null),t.firmwareOtaQuietUntil=0,t.nativeOpen||_i()}function Pp(){return t.deviceReconnectMode?`
    ...
    `:""}function nn(e=oo()){let o=he()||{},r=le(o);t.entities.firmwareUpdate={...o,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:Mp(e),current_version:r}}async function an(e={}){let o=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await Rp(r===0?900:1200),await L(Dr,"all",{forceMissing:!0});let n=Hs(),a=!o||Pb(o),i=Rb(),l=Ye(),p=ro();if(n&&a&&(i||!l&&p!=="Nog niet gecontroleerd"))return!0}return!1}async function cr(e={}){let o=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await Rp(a===0?r:n);try{if(await L(Dr,"all",{forceMissing:!0}),Sa()==="rebooting"&&Gt("ota"),d(),t.updateInstallMode==="connection-switch"){let i=fs(t.updateInstallTargetConnection);if(i&&Jt()===i&&!en()&&!ir())return!0}else if(t.updateInstallMode==="topology-switch"){let i=Jr(t.updateInstallTargetTopology);if(i&&ee()===i&&!en()&&!ir())return!0}else if(xs()||$s()||Ms()&&!en()&&!ir())return!0}catch{o||(t.controlNotice="Wachten tot het device opnieuw is opgestart...",d(),o=!0)}}return!1}function Ob(){let e=oo(),o=Dt();if(o)return o.copy;if(As()){let r=t.updateInstallCompletedVersion||le()||oo();return`${Ne()} draait nu op ${r}.`}return ir()?`OTA-update wordt voorbereid voor ${Ne()}. Het device kan kort herstarten.`:Ye()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:Ro()?"Er staat een nieuwere firmware klaar.":Ms()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function Np(){return!!(t.firmwareAdvancedOpen||t.firmwareConnectionSwitchOpen||t.firmwareTopologySwitchOpen||t.updateManualUploadOpen||t.updateTestFirmwareOpen)}function ba(e,o,r,n,a=!1){return`
      <button
    ...
    `}function Db(e,o,r,n){if(!Np())return"";let i=!!(Dt()||t.updateInstallBusy||Ye());return`
      <div class="oq-helper-modal-callout oq-helper-modal-callout--subtle oq-firmware-advanced-panel">
        ...
        <div class="oq-firmware-advanced-options">
          ${e?ba("toggle-firmware-connection-switch","Verbinding wisselen",`Naar ${o.targetLabel}`,t.firmwareConnectionSwitchOpen,i):""}
          ${r?ba("toggle-firmware-topology-switch","Opstelling wisselen",`Naar ${n.targetLabel}`,t.firmwareTopologySwitchOpen,i):""}
          ${ba("toggle-firmware-upload","Handmatige upload","Lokaal OTA-bestand",t.updateManualUploadOpen,i)}
          ${ba("toggle-firmware-test","Testfirmware","PR-release installeren",t.updateTestFirmwareOpen,i)}
        </div>
        ${Lb()}
        ${Fb()}
        ${_b()}
    ...
    `}function Ib(){if(!t.updateTestFirmwareOpen)return"";let o=!!(Dt()||t.updateInstallBusy||Ye()),r=Mo(),n=sr(),a=tn(r,n),i=!!(n.available&&h("firmwareTestOtaUrl")&&h("firmwareTestOtaMd5Url")&&h("installFirmwareTestOta")),l=!!(r&&i),p=t.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,c=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function _b(){if(!t.updateManualUploadOpen)return"";let o=!!(Dt()||t.updateInstallBusy||Ye()),r=String(t.updateManualUploadFileName||t.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Op(){if(!t.updateModalOpen)return"";let e=he(),o=t.entities.firmwareUpdateChannel||null,{current:r,latest:n}=Rs(),a=Ye(),i=ir(),l=Ro(),p=Ob(),u=Dt(),c=As(),m=Hb(),b=c?"Firmware-update afgerond":u||i?"Firmware-update bezig":a?"Controleren op firmware-update":Mb(),y=o?Array.isArray(o.option)?o.option:Array.isArray(o.options)?o.options:[]:[],w=ya(),g=va(),f=!!(w&&!c),v=!!(g&&!c);return`
        ...
        `:`
          <section class="oq-debug-recording-duration" aria-label="Duur">
            ...
            <div class="oq-debug-recording-segments" role="group" aria-label="Kies opnameduur">
              ${Un.map(f=>{let v=Number(f.minutes)===p;return`
                  <button
  ...
  `}function Xp(e){t.overviewTheme=e==="dark"?"dark":"light";try{window.localStorage.setItem("oq-overview-theme",t.overviewTheme)}catch{}}function ka(e){t.interfacePanelOpen=e===!0}function Ds(e){try{window.localStorage.setItem("oq-active-surface",e==="native"?"native":"app")}catch{}}function un(e,o={}){t.settingsGroup=xe.has(e)?e:de[0].id;try{window.localStorage.setItem("oq-settings-group",t.settingsGroup)}catch{}o.syncUrl!==!1&&t.appView==="settings"&&Bi(o.syncMode||"replace")}function ed(e){t.devPanelOpen=e===!0;try{window.localStorage.setItem("oq-dev-panel-open",t.devPanelOpen?"true":"false")}catch{}}function td(e){t.hpVisualMode=e==="compact"?"compact":"schematic";try{window.localStorage.setItem("oq-hp-visual-mode",t.hpVisualMode)}catch{}}function od(e){t.hpLayoutMode=e==="focus-hp1"||e==="focus-hp2"?e:"equal";try{window.localStorage.setItem("oq-hp-layout-mode",t.hpLayoutMode)}catch{}}function ey(e){if(t.reducedMotion=!!e?.matches,t.reducedMotion){Ur();return}jr()}function ty(){let e=xi();!e||t.motionPreferenceMedia===e||(t.motionPreferenceMedia=e,t.motionPreferenceListener=ey,typeof e.addEventListener=="function"?e.addEventListener("change",t.motionPreferenceListener):typeof e.addListener=="function"&&e.addListener(t.motionPreferenceListener),t.reducedMotion=!!e.matches)}function oy(){return Object.keys(t.entities).length>0}function ry(){return ki+Math.floor(Math.random()*(xc-ki+1))}function ny(){return(document.hidden?Ir:wc)+ry()}function rd(e=ny()){t.pollTimer||t.nativeOpen||t.updateInstallBusy||na()||(t.pollTimer=window.setTimeout(async()=>{t.pollTimer=null,await ce(),rd()},e))}function Ls(){rd()}function Fs(){t.pollTimer&&(window.clearTimeout(t.pollTimer),t.pollTimer=null)}nu({start:Ls,stop:Fs});function ay(){t.nativeOpen||(Fs(),Ls(),document.hidden||ce({forceProbe:!0}))}function xa(e={}){if(pn(),t.nativeOpen){Fs(),Ur(),t.nativeFrontendLoaded||$a();return}if(jr(),Ls(),e.refresh!==!1){if(!oy()){Zu();return}ce({forceFast:!0})}}function iy(){let e=zr()||Qt(),o=e==="settings"?Vi()||t.settingsGroup:"";if(!(e===t.appView&&(e!=="settings"||o===t.settingsGroup))){if(t.appView=e,e==="settings"&&o){t.settingsGroup=o;try{window.localStorage.setItem("oq-settings-group",t.settingsGroup)}catch{}}d(),ce({forceFast:!0})}}function pn(){t.nativeApp&&(t.nativeApp.classList.add("oq-native-app"),t.nativeApp.classList.toggle("oq-native-app--collapsed",!t.nativeOpen),t.nativeApp.setAttribute("aria-hidden",t.nativeOpen?"false":"true"))}function nd(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Jp,{once:!0}):Jp(),window.addEventListener("pointermove",du),window.addEventListener("pointerup",mu),window.addEventListener("popstate",iy),window.addEventListener("oq-mock-updated",sy),window.addEventListener("oq-dev-controls-changed",ly),document.addEventListener("visibilitychange",ay)}function sy(){t.mounted&&ce()}function ly(){t.mounted&&d()}function Jp(){cy();let e=document.querySelector("esp-app");e||(e=document.createElement("esp-app"),document.body.appendChild(e)),t.nativeApp=e,t.nativeFrontendLoaded=Array.from(document.scripts).some(o=>o.src===gi),t.mounted||(uy(e),t.mounted=!0,xa()),pn(),t.nativeOpen||(ce(),cn({silent:!0}))}function cy(){if(!document.head)return;let e=document.head.querySelector('meta[name="viewport"]');e||(e=document.createElement("meta"),e.name="viewport",document.head.appendChild(e)),e.setAttribute("content","width=device-width, initial-scale=1")}function uy(e){let o=document.createElement("section");o.id="oq-helper-root",o.lang="nl-NL",document.documentElement&&!document.documentElement.lang&&(document.documentElement.lang="nl-NL"),e.parentNode.insertBefore(o,e),o.addEventListener("click",lu),o.addEventListener("change",su),o.addEventListener("input",cu),o.addEventListener("keydown",uu),o.addEventListener("wheel",fu,{passive:!1}),o.addEventListener("focusin",Wi),o.addEventListener("focusout",Wi),o.addEventListener("mouseover",hu),o.addEventListener("mouseout",gu),o.addEventListener("pointerdown",pu),t.root=o,ty();let r=zr()||Qt(),n=r==="settings"?Vi():"";n&&un(n,{syncUrl:!1}),nt(r,{syncMode:"replace",forceSync:!0}),Kr(),d()}function py(e){return new Promise((o,r)=>{if(!e){o();return}let n=Array.from(document.scripts).find(i=>i.src===e);if(n){if(n.dataset.loaded==="true"){o();return}n.addEventListener("load",()=>o(),{once:!0}),n.addEventListener("error",i=>r(i),{once:!0});return}let a=document.createElement("script");a.src=e,a.async=!1,a.addEventListener("load",()=>{a.dataset.loaded="true",o()},{once:!0}),a.addEventListener("error",i=>r(i),{once:!0}),document.head.appendChild(a)})}async function $a(){if(!(t.nativeFrontendLoaded||t.nativeFrontendLoading)){t.nativeFrontendLoading=!0,t.nativeOpen&&d();try{await py(gi),t.nativeFrontendLoaded=!0}catch(e){t.controlError=`ESPHome fallback kon niet worden geladen. ${e.message||e}`,t.nativeOpen=!1,Ds("app"),d(),xa()}finally{t.nativeFrontendLoading=!1,t.nativeOpen&&d()}}}function Is(){if(!t.root)return;let e=typeof window<"u"?window.__OQ_DEV_CONTROLS__:null;!e||typeof e.bind!="function"||e.bind(t.root)}function dn(){let e=t.authStatus;return e?e.enabled?e.setup_window_active?"Instelvenster":"Beveiligd":"Onbeveiligd":"Laden..."}function dy(){return"Login"}function my(){let e=t.authStatus;return e?e.enabled?"De web-app vraagt nu een login voordat beheer beschikbaar is. Je kunt die hier aanpassen of uitzetten.":"De web-app staat open op je netwerk. Houd de herstelknop 5 seconden vast om een login toe te voegen.":"We halen de huidige loginstatus op."}function mn(){let e=t.authStatus;return e?e.enabled?e.setup_window_active?"Login actief. Tijdelijk instelvenster is open.":`Login actief${e.source?` via ${e.source}`:""}.`:e.setup_window_active?"Login uit. Tijdelijk instelvenster is open.":"Login uit. Webtoegang is open / onbeveiligd op het netwerk.":"Logingegevens laden..."}function pr(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?"Herstart nodig":e.transport_active===!0?"Aan":e.transport_active===!1?"Uit":e.enabled?"Aan":"Uit":"Laden..."}function dr(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API beveiligd.":e.key?"Deze wijziging wordt actief na herstart. De sleutel blijft opgeslagen voor later gebruik.":"Deze wijziging wordt actief na herstart.":e.transport_active===!0?"API-encryptie staat aan. Gebruik dezelfde sleutel in Home Assistant.":"De native API staat nu open op je lokale netwerk.":"API-encryptie wordt geladen."}function gy(){return"ESPHome API encryption"}function hy(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu alvast voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API nog beveiligd.":"Deze wijziging wordt actief na herstart. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":e.transport_active===!0?"De native API is beveiligd. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":"API-encryptie staat uit. Schakel in om een sleutel te bekijken, kopi\xEBren of vernieuwen.":"We halen de huidige API-beveiliging op."}function fy(){let e=t.apiSecurityStatus;if(!e)return"Laden...";let o=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active);return o&&e.enabled===!0&&e.transport_active===!1?"Annuleer inschakelen":o&&e.enabled===!1&&e.transport_active===!0?"Annuleer uitschakelen":e.enabled?"Uitschakelen":"Inschakelen"}function by(){let e=t.apiSecurityStatus;return e?e.key?"Vernieuw sleutel":"Genereer sleutel":"Laden..."}function ad(e,o,r="",n=!1){return`
    ...
    `}function gn(e,o,r){let n=A(o),a=A(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=A(e);return Number.isFinite(i)?i:NaN}function _s(e="oq-settings-grid"){let o=yy();return`
      <div class="${s(e)}">
    ...
    `}function wy({status:e,running:o,resultReady:r,startDisabled:n,abortDisabled:a,applyDisabled:i,busy:l,controlsAvailable:p}){let u=String(e||"").toUpperCase(),c=u.includes("FAILED")||u.includes("REFUSED")||u.includes("ABORT"),m=u.includes("APPLIED"),b=h("hp2WaterIn")||h("hp2WaterOut")||h("hp2WaterInRaw")||h("hp2WaterOutRaw"),y=A("hpWaterCalibrationStableProgress"),w=A("hpWaterCalibrationStableRequired"),g=A("hpWaterCalibrationRemaining"),f=Math.round(A("hpWaterCalibrationPhase")),v=o&&(f===1||u.includes("MIXING")),S=o&&!v,q=300,C=60,x=Number.isFinite(g)?Math.max(0,q-g):NaN,H=Number.isFinite(x)?Math.max(0,C-x):NaN,V=v&&Number.isFinite(x)?Math.max(0,Math.min(100,x/C*100)):S&&Number.isFinite(y)&&Number.isFinite(w)&&w>0?Math.max(0,Math.min(100,y/w*100)):o&&Number.isFinite(g)?Math.max(0,Math.min(100,100-g/q*100)):r?100:0,K=r&&h("hpWaterCalibrationResultSpreadBefore")?be("hpWaterCalibrationResultSpreadBefore",2):be("hpWaterCalibrationSpread",2),M=v?"Water mengen":Number.isFinite(y)&&Number.isFinite(w)&&w>0?y>0?`${Math.round(Math.max(0,y))} / ${Math.round(w)} s binnen grenzen`:"Nog niet binnen grenzen":"Wachten op stabiel venster",I=r?3:o?2:1,z=m?"Offsets toegepast":r?`Meting klaar - spreiding ${K}`:o?v?`Water mengen${Number.isFinite(H)&&H>0?` - meting start over ${Math.round(H)} s`:""}`:`Meting bezig - ${Number.isFinite(g)&&g>0?`max. ${Math.round(g)} s resterend`:M}`:c?"Meting niet voltooid":"Voorbereiding",ne=m?"De voorgestelde offsets zijn opgeslagen.":r?"Controleer de voorgestelde offsets en pas ze toe.":o?v?"De waterpomp circuleert zonder compressor zodat de watertemperaturen eerst kunnen mengen.":"De firmware stopt zodra het laatste meetvenster binnen de spreiding- en driftgrenzen valt.":c?or("hpWaterCalibrationStatus","Controleer de voorwaarden en start opnieuw."):b?"Start alleen wanneer compressor en boiler uit zijn. HP1 en HP2 water in/out worden samen naar een relatieve referentie gebracht.":"Start alleen wanneer compressor en boiler uit zijn. Bij single setup wordt HP1 water in/out onderling gelijkgetrokken; supply blijft diagnose.",fe=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",liveKey:"hp1WaterIn",resultRawKey:"hpWaterCalibrationResultHp1InRawAvg",offsetKey:"hp1WaterInOffset",suggestedKey:"hp1WaterInOffsetSuggested"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",liveKey:"hp1WaterOut",resultRawKey:"hpWaterCalibrationResultHp1OutRawAvg",offsetKey:"hp1WaterOutOffset",suggestedKey:"hp1WaterOutOffsetSuggested"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",liveKey:"hp2WaterIn",resultRawKey:"hpWaterCalibrationResultHp2InRawAvg",offsetKey:"hp2WaterInOffset",suggestedKey:"hp2WaterInOffsetSuggested"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",liveKey:"hp2WaterOut",resultRawKey:"hpWaterCalibrationResultHp2OutRawAvg",offsetKey:"hp2WaterOutOffset",suggestedKey:"hp2WaterOutOffsetSuggested"}].filter(N=>h(N.liveKey)||h(N.rawKey)||h(N.offsetKey)),Z=(N,F)=>{let T=I>N;return`
      ...
      `,Y=N=>{let F=A(N.resultRawKey),T=Number.isFinite(F)?F:gn(N.rawKey,N.liveKey,N.offsetKey),$=A(N.suggestedKey),P=Number.isFinite(T)&&Number.isFinite($)?Mt(T+$,t.entities[N.suggestedKey]?.uom||"\xB0C",2):"\u2014";return`
        <tr>
    ...
    `}function ud(){let e=h("boilerCvAssistEnabled")&&k("boilerCvAssistEnabled"),o=pa(),r=k("cm100Active"),n=String(o||"").trim().toUpperCase(),a=Oo(o),i=!a&&(r||n==="CM100 READY"),l=t.commissioningTaskLock==="cm100",p=t.loadingEntities||t.busyAction==="commissioningCm100Start"||t.busyAction==="commissioningCm100Stop"||l,u=!!t.pendingCommissioningCm100Start,c=p||i||a,m=p||!i,b=$o("boilerPowerTestStatus","IDLE"),y=Aa(b,"boiler"),w=k("boilerPowerTestActive"),g=t.loadingEntities||t.busyAction==="boilerPowerTestStart"||t.busyAction==="boilerPowerTestAbort"||t.busyAction==="boilerPowerTestApply",f=!!(t.entities.boilerPowerTestStart||t.entities.boilerPowerTestAbort||t.entities.boilerPowerTestApply),v=!!t.pendingBoilerPowerTestStart,S=t.commissioningTaskLock==="boiler",q=Oo(b),x=!ye(b)&&(w||v||S||mr(b))&&!q,H=Te("boilerRatedHeatPower"),V=Te("boilerHeatPower"),K=A("boilerHeatPower"),M=K>0?V:x&&t.commissioningBoilerHeatPowerDisplay?t.commissioningBoilerHeatPowerDisplay:V;K>0&&(t.commissioningBoilerHeatPowerDisplay=V);let I=$o("flowAutotuneStatus","IDLE"),z=Aa(I,"autotune"),ne=t.loadingEntities||t.busyAction==="flowAutotuneStart"||t.busyAction==="flowAutotuneAbort"||t.busyAction==="flowAutotuneApply",fe=!!(t.entities.flowAutotuneStart||t.entities.flowAutotuneAbort||t.entities.flowAutotuneApply),Z=!!t.pendingFlowAutotuneStart,D=t.commissioningTaskLock==="autotune",Y=Oo(I),F=!ye(I)&&(Z||D||mr(I))&&!Y,T=$o("airPurgeStatus","IDLE"),$=Aa(T,"purge"),P=k("airPurgeActive"),B=t.loadingEntities||t.busyAction==="airPurgeStart"||t.busyAction==="airPurgeAbort",Q=!!(t.entities.airPurgeStart||t.entities.airPurgeAbort),J=!!t.pendingAirPurgeStart,X=t.commissioningTaskLock==="purge",ae=!ye(T)&&(P||J||X||mr(T)),Fe=/DONE/.test(String(T||"").toUpperCase()),Ae=!!(Q||t.entities.airPurgeStatus||t.entities.airPurgeReturnToAuto),Wo=Te("airPurgeRemaining",{decimals:0}),Wt=A("airPurgePhase"),Ar=Wt===1?"Rustig":Wt===2?"Pulsen":Wt===3?"Stabiliseren":$.phase,et=$o("manualFlowStatus","IDLE"),qt=k("manualFlowActive"),He=t.loadingEntities||t.busyAction==="manualFlowStart"||t.busyAction==="manualFlowAbort",ue=!!(t.entities.manualFlowStart||t.entities.manualFlowAbort),Tt=!!t.pendingManualFlowStart,se=t.commissioningTaskLock==="manual-flow",Ie=!ye(et)&&(qt||Tt||se||mr(et)),Et=$o("manualHpStatus","IDLE"),Vo=k("manualHpActive"),po=t.loadingEntities||t.busyAction==="manualHpStart"||t.busyAction==="manualHpAbort",Rn=!!(t.entities.manualHpStart||t.entities.manualHpAbort),Bo=!!t.pendingManualHpStart,ut=t.commissioningTaskLock==="manual-hp",Be=!ye(Et)&&(Vo||Bo||ut||mr(Et)),Pn=/SAFETY STOP/.test(String(Et||"").toUpperCase()),Nn=/STOPPING/.test(String(Et||"").toUpperCase()),pt=$o("hpWaterCalibrationStatus","IDLE"),ci=Aa(pt,"hp-water-calibration"),On=k("hpWaterCalibrationActive"),Ko=t.loadingEntities||t.busyAction==="hpWaterCalibrationStart"||t.busyAction==="hpWaterCalibrationAbort"||t.busyAction==="hpWaterCalibrationApply",nc=!!(t.entities.hpWaterCalibrationStart||t.entities.hpWaterCalibrationAbort||t.entities.hpWaterCalibrationApply),Dn=!!t.pendingHpWaterCalibrationStart,Vt=t.commissioningTaskLock==="hp-water-calibration",_e=!ye(pt)&&(On||Dn||Vt||mr(pt)),Ln=/DONE|APPLIED/.test(String(pt||"").toUpperCase()),ac=/APPLIED/.test(String(pt||"").toUpperCase()),ah=Te("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),ih=Te("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),ic=/DONE|APPLIED/.test(String(b||"").toUpperCase()),sc=/DONE|APPLIED/.test(String(I||"").toUpperCase()),ui=i?q?"Wachten op CM100":x?y.phase:ic?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",pi=i?Y?"Wachten op CM100":F?z.phase:sc?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",di=i?ae?$.phase:Fe?"Klaar":"Klaar om te starten":"Wachten op CM100",lc=i?Ie?"Actief":"Klaar om te starten":"Wachten op CM100",cc=i?Be?Nn?"Bezig met stoppen":Pn?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",uc=i?_e?ci.phase:ac?"Offsets toegepast":Ln?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",pc=!i||g||!f||F||ae||Ie||Be||_e||x||D||X||se||ut||Vt||v,sh=g||!(x||S||v),lh=g||pc||!ic||F||ae||_e,dc=!i||ne||!fe||x||ae||Ie||Be||_e||F||S||X||se||ut||Vt||Z,ch=ne||!(F||D||Z),uh=ne||dc||!sc||x||ae||_e,ph=!i||B||!Q||x||F||Ie||Be||_e||ae||S||D||se||ut||Vt||J,dh=B||!(ae||X||J),mh=!i||He||!ue||x||F||ae||Be||_e||Ie||S||D||X||ut||Vt||Tt,gh=He||!(Ie||se||Tt),hh=!i||po||!Rn||x||F||ae||Ie||_e||Be||S||D||X||se||Vt||Bo,fh=po||!(Be||ut||Bo),bh=!i||Ko||!nc||x||F||ae||Ie||Be||_e||S||D||X||se||ut||Dn,yh=Ko||!(_e||Vt||Dn),vh=Ko||_e||!Ln||ac;u&&i&&(t.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(t.commissioningTaskLock=""),v&&(w||ye(b))&&(t.pendingBoilerPowerTestStart=!1),S&&ye(b)&&(t.commissioningTaskLock=""),Z&&ye(I)&&(t.pendingFlowAutotuneStart=!1),D&&ye(I)&&(t.commissioningTaskLock=""),J&&(P||ye(T))&&(t.pendingAirPurgeStart=!1),X&&ye(T)&&(t.commissioningTaskLock=""),Tt&&(qt||ye(et))&&(t.pendingManualFlowStart=!1),se&&(qt||ye(et))&&(t.commissioningTaskLock=""),Bo&&(Vo||ye(Et))&&(t.pendingManualHpStart=!1),ut&&(Vo||ye(Et))&&(t.commissioningTaskLock=""),Dn&&(On||ye(pt))&&(t.pendingHpWaterCalibrationStart=!1),Vt&&ye(pt)&&(t.commissioningTaskLock="");let wh=a?"Wachten op CM100":o,Sh=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",qh=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:uc,available:!!(nc||t.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:gr({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:uc,statusCopy:_e?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":Ln?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:wy({status:pt,running:_e,resultReady:Ln,startDisabled:bh,abortDisabled:yh,applyDisabled:vh,busy:Ko,controlsAvailable:!!(t.entities.hpWaterCalibrationStart||t.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:lc,available:!!(ue||t.entities.manualFlowStatus),openDisabled:!i,cardMarkup:gr({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:lc,statusCopy:Ie?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${t.entities.manualFlowStart||t.entities.manualFlowAbort?Ao({active:Ie,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:He||mh,stopDisabled:He||gh}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:cc,available:!!(Rn||t.entities.manualHpStatus),openDisabled:!i,cardMarkup:gr({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:cc,statusCopy:Be?Nn?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":Pn?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `,metrics:`
            ...
            <div class="oq-settings-manual-hp-results">
              ${Ce("flowSelected","Gemeten flow","Actuele doorstroming in het watercircuit.",Te("flowSelected"),"oq-settings-field--compact")}
              ${Ce("hp1Compressor","Warmtepomp 1 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",cd("hp1Compressor","hp1Freq"),"oq-settings-field--compact")}
              ${h("hp2Compressor")?Ce("hp2Compressor","Warmtepomp 2 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",cd("hp2Compressor","hp2Freq"),"oq-settings-field--compact"):""}
            </div>
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:pi,available:!0,openDisabled:Oo(pi),cardMarkup:gr({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:pi,statusCopy:Y?"Wacht totdat CM100 actief is voordat je autotune start.":F?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${t.entities.flowAutotuneStart||t.entities.flowAutotuneAbort?Ao({active:F,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:ne||dc,stopDisabled:ne||ch}):""}
            ${t.entities.flowAutotuneApply?Ve("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",ne||uh):""}
          `,metrics:`
            ${Ce("flowKpSuggested","Voorgestelde Kp","Kp bepaalt hoe sterk de regeling meteen corrigeert.",ah,"oq-settings-field--compact")}
            ${Ce("flowKiSuggested","Voorgestelde Ki","Ki corrigeert kleine afwijkingen langzaam weg.",ih,"oq-settings-field--compact")}
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:ui,available:e,openDisabled:Oo(ui),cardMarkup:gr({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s(H)}`,status:ui,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":x?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ${t.entities.boilerPowerTestStart||t.entities.boilerPowerTestAbort?Ao({active:x,startKey:"boilerPowerTestStart",stopKey:"boilerPowerTestAbort",startLabel:"Boiler test starten",stopLabel:"Boiler test stoppen",startDisabled:g||pc,stopDisabled:g||sh}):""}
            ${t.entities.boilerPowerTestApply?Ve("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",g||lh):""}
          `,metrics:`
          ...
          `})},{key:"purge",title:"Ontluchten",label:"Ontluchten",summary:"Draait een vaste ontluchtingsrun van 5 minuten met rustige flow, pomp-pulsen en stabilisatie.",status:di,available:Ae,openDisabled:Oo(di),cardMarkup:gr({taskKey:"purge",title:"Ontluchten",copy:"Draait 5 minuten met rustige doorstroming, korte pomp-pulsen en een stabilisatiefase.",subcopy:"Na afloop kan OpenQuatt de service mode (CM100) afsluiten of actief laten.",status:di,statusCopy:ae?"Ontluchten loopt vast 5 minuten door en stopt daarna automatisch.":i?"CM100 staat klaar. Start ontluchten wanneer het circuit open staat.":"Start CM100 eerst en voer daarna ontluchten uit.",progressTask:"purge",className:"oq-settings-commissioning-card--air-purge",actions:`
            ${t.entities.airPurgeStart||t.entities.airPurgeAbort?Ao({active:ae,startKey:"airPurgeStart",stopKey:"airPurgeAbort",startLabel:"Ontluchten starten",stopLabel:"Ontluchten stoppen",startDisabled:B||ph,stopDisabled:B||dh}):""}
          `,metrics:`
          ...
          `})}].filter(Th=>Th.available);return{cm100Status:wh,cm100StartDisabled:c,cm100StopDisabled:m,serviceStatusCopy:Sh,tasks:qh}}function Sy(e){return $e({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
        ...
        </button>`})}function pd(){let e=ud();return te("Service","Service & commissioning","Gebruik de service-stand (controlmode CM100) voor testen, afstelling en onderhoudstaken.",`
    ...
    `:""}function Ha(e="oq-settings-grid"){return`
      <div class="${s(e)}">
...
`));let c=`${n}|${u.length}|${u.slice(0,120)}|${u.slice(-120)}`,m=Number.isFinite(t.trendHistoryNowMs),y=Number.isFinite(p)?!m||t.trendHistoryNowMs!==p:m,w=u!==t.trendHistoryRaw||t.trendHistoryError!==""||t.trendHistorySignature!==c||y;return t.trendHistoryRaw=u,t.trendHistoryError="",t.trendHistorySignature=c,t.trendHistoryNowMs=Number.isFinite(p)?p:Number.NaN,t.trendHistoryLastFetchAt=Date.now(),w})();try{return await t.trendHistoryFetchPromise}catch(n){let a=`Trendhistorie kon niet worden geladen. ${n.message}`,i=t.trendHistoryError!==a;return t.trendHistoryError=a,t.trendHistoryRaw="",t.trendHistorySignature="",t.trendHistoryNowMs=Number.NaN,t.trendHistoryLastFetchAt=Date.now(),i}finally{t.trendHistoryFetchPromise=null}}async function lo(e={}){if(!h("lifetimeEnergyHistoryEnabled")&&!br()){let a=!!(t.energyHistoryRaw||t.energyHistoryError);return t.energyHistoryRaw="",t.energyHistoryError="",t.energyHistorySignature="",t.energyHistoryNowMs=Number.NaN,t.energyHistoryLastFetchAt=0,t.energyHistoryRequestQuery="",t.energyHistoryFetchQuery="",t.energyHistoryCsrfToken="",a}let o=e.force===!0,r=Date.now(),n=e.metaOnly===!0?"?meta=1":typeof hn=="function"?hn():"";if(!o&&t.energyHistoryFetchPromise&&t.energyHistoryFetchQuery===n)return t.energyHistoryFetchPromise;if(!o&&t.energyHistoryRequestQuery===n&&(t.energyHistoryRaw||t.energyHistoryError)&&r-Number(t.energyHistoryLastFetchAt||0)<Fn)return!1;t.energyHistoryFetchQuery=n,t.energyHistoryFetchPromise=(async()=>{let a=async f=>{let v=await fetch(`${Re()}/energy/history${f}`,{cache:"no-store"});if(!v.ok)throw new Error(`HTTP ${v.status}`);return v.text()},i=n,l=await a(i);if(e.metaOnly!==!0&&i.includes("meta=1")&&typeof hn=="function"){let f=t.energyHistoryRaw;t.energyHistoryRaw=l;let v=hn();t.energyHistoryRaw=f,v&&v!==i&&(i=v,l=await a(i))}let p=l.split(/\r?\n/),u=xd(l)||t.energyHistoryCsrfToken,c=Number.NaN;p.forEach(f=>{f.startsWith("@now|")&&(c=Number(f.slice(5)))});let m=`${l.length}|${l.slice(0,120)}|${l.slice(-120)}`,b=Number.isFinite(t.energyHistoryNowMs),w=Number.isFinite(c)?!b||t.energyHistoryNowMs!==c:b,g=l!==t.energyHistoryRaw||t.energyHistoryError!==""||t.energyHistorySignature!==m||w||u!==t.energyHistoryCsrfToken;return t.energyHistoryRaw=l,t.energyHistoryError="",t.energyHistorySignature=m,t.energyHistoryNowMs=Number.isFinite(c)?c:Number.NaN,t.energyHistoryLastFetchAt=Date.now(),t.energyHistoryRequestQuery=i,t.energyHistoryCsrfToken=u,g})();try{return await t.energyHistoryFetchPromise}catch(a){let i=`Energiehistorie kon niet worden geladen. ${a.message}`,l=t.energyHistoryError!==i;return t.energyHistoryError=i,t.energyHistoryRaw="",t.energyHistorySignature="",t.energyHistoryNowMs=Number.NaN,t.energyHistoryLastFetchAt=Date.now(),t.energyHistoryRequestQuery=n,t.energyHistoryCsrfToken="",l}finally{t.energyHistoryFetchPromise=null}}Su({refreshEnergyHistoryData:lo,refreshSettingsStorageState:yn,refreshTrendHistoryData:js,refreshTrendHistoryMetadata:vd,shouldRefreshSettingsStorageForCurrentSurface:jy});function Od(){return O("controlModeLabel","").toLowerCase().includes("standby")}function wt(e,o){e&&e.innerHTML!==o&&(e.innerHTML=o)}function lt(e,o,r){return!e||e.dataset.renderSignature===o?!1:(e.outerHTML=r,!0)}function Jy([e,o]){let r=Hi(o);if(!h(o)&&Number.isNaN(r))return"";let n=Ai(o)?Ue(o):O(o);return`
    ...
    `:""}function Qs(){let e=Gn.map(tv).filter(Boolean),o=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:o}}function _d(e=Qs()){return Se(e)}function Wd(e=Qs()){return`
    ...
    `}var La=["electricalInputWh","heatingInputWh","coolingInputWh","heatpumpHeatOutputWh","heatpumpCoolingOutputWh","boilerHeatOutputWh","systemHeatOutputWh"],Vd=[{id:"day",label:"Dag"},{id:"week",label:"Week"},{id:"month",label:"Maand"},{id:"year",label:"Jaar"},{id:"all",label:"Alles"}],ov=new Set(["day","week","month","year"]),rv=["Zo","Ma","Di","Wo","Do","Vr","Za"];function Oe(e){let o=String(e||"").trim();return Vd.some(r=>r.id===o)?o:"day"}function Ys(e){let o=Oe(e);t.energyHistoryView!==o&&(t.energyHistoryView=o,t.energyHistoryLastFetchAt=0,d(),Bd())}function Bd(){typeof lo=="function"&&lo({force:!0}).then(e=>{e&&d()})}function Sr(e){return ov.has(Oe(e))}function Fa(){let e=new Date;return e.getFullYear()*1e4+(e.getMonth()+1)*100+e.getDate()}function vn(){let e=String(t.energyHistoryRaw||""),o={storedDayCount:0,oldestDateKey:null,newestDateKey:null,hourStoredDayCount:0,hourOldestDateKey:null,hourNewestDateKey:null,hourRequestedRetentionDays:0,hourSlotCount:0,hourPartitionAvailable:!1,hourRecordCount:0,hourWriteCount:0,hourStorageKb:0,hourLastWriteTimestampS:0,dayPartitionAvailable:!1,dayStorageKb:0,dayWriteCount:0,dayLastWriteTimestampS:0};return e.split(/\r?\n/).forEach(r=>{if(!r.startsWith("@bounds|")&&!r.startsWith("@day_retention|")&&!r.startsWith("@hour_retention|"))return;let n=r.split("|");r.startsWith("@bounds|")?(o.storedDayCount=Number(n[1])||0,o.oldestDateKey=Number(n[2])||null,o.newestDateKey=Number(n[3])||null,o.hourStoredDayCount=Number(n[4])||0,o.hourOldestDateKey=Number(n[5])||null,o.hourNewestDateKey=Number(n[6])||null):r.startsWith("@day_retention|")?(o.dayPartitionAvailable=Number(n[1])===1,o.dayStorageKb=Number(n[2])||0,o.dayWriteCount=Number(n[3])||0,o.dayLastWriteTimestampS=Number(n[4])||0):r.startsWith("@hour_retention|")&&(o.hourRequestedRetentionDays=Number(n[1])||0,o.hourSlotCount=Number(n[2])||0,o.hourPartitionAvailable=Number(n[3])===1,o.hourRecordCount=Number(n[4])||0,o.hourWriteCount=Number(n[5])||0,o.hourStorageKb=Number(n[6])||0,o.hourLastWriteTimestampS=Number(n[7])||0)}),o}function Zs(){let e=String(t.energyHistoryRaw||""),o=null;return e.split(/\r?\n/).forEach(r=>{let n=Zd(r);n&&(o=n.dateKey)}),o}function nv(e=[],o=!0){let r=Zs(),n=vn(),a=(Array.isArray(e)?e:[]).map(i=>Number(i?.dateKey)).filter(Number.isFinite);return Number.isFinite(Number(n.newestDateKey))&&a.push(Number(n.newestDateKey)),Number.isFinite(Number(r))&&a.push(Number(r)),o&&Ia().forEach(i=>{let l=Number(i?.dateKey);Number.isFinite(l)&&a.push(l)}),a.length?Math.max(...a):Fa()}function Je(e){return e.getFullYear()*1e4+(e.getMonth()+1)*100+e.getDate()}function Kd(e,o,r){return new Date(e,o-1,r,12,0,0)}function jd(e,o){return new Date(e,o,0).getDate()}function Dd(e){return String(e).padStart(2,"0")}function Us(e){let o=re(e);return o?`${o.year}-${Dd(o.month)}-${Dd(o.day)}`:""}function Ud(e){let o=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(e||"").trim());if(!o)return null;let r=Number(o[1]),n=Number(o[2]),a=Number(o[3]),i=Kd(r,n,a);return i.getFullYear()!==r||i.getMonth()+1!==n||i.getDate()!==a?null:re(Je(i))}function Gd(e,o){return Number(e)*100+Number(o)}function zs(e){return Gd(e.getFullYear(),e.getMonth()+1)}function vr(e){let o=Number(e);if(!Number.isFinite(o)||o<=0)return null;let r=Math.floor(o/100),n=o%100;return r<2020||n<1||n>12?null:{key:o,year:r,month:n,date:new Date(r,n-1,1,12,0,0)}}function av(e){let o=String(e||"").trim(),r=/^(\d{4})-(\d{2})$/.exec(o);return vr(r?Gd(Number(r[1]),Number(r[2])):o)}function zd(e,o){let r=vr(e);if(!r)return"";let n=new Date(r.year,r.month-1+Number(o||0),1,12,0,0);return String(zs(n))}function wn(e){let o=new Date(e.getTime()),r=o.getDay(),n=r===0?-6:1-r;return o.setDate(o.getDate()+n),o.setHours(12,0,0,0),o}function wr(e,o){let r=new Date(e.getTime());return r.setDate(r.getDate()+o),r.setHours(12,0,0,0),r}function Ld(e){return e.toLocaleDateString("nl-NL",{day:"numeric",month:"short"}).replace(/\./g,"")}function Qd(e){let o=new Date(Date.UTC(e.getFullYear(),e.getMonth(),e.getDate())),r=(o.getUTCDay()+6)%7;o.setUTCDate(o.getUTCDate()-r+3);let n=o.getUTCFullYear(),a=new Date(Date.UTC(n,0,4)),i=(a.getUTCDay()+6)%7;return a.setUTCDate(a.getUTCDate()-i+3),{week:1+Math.round((o-a)/(10080*60*1e3)),year:n}}function Fd(e){return Je(wn(e))}function iv(e){let r=Ud(e)||re(e);if(!r)return null;let n=wn(r.date);return re(Je(n))}function Yd(e){let o=re(e);if(!o)return"Week";let r=wn(o.date),n=wr(r,6);return`Week ${Qd(r).week} (${Ld(r)} - ${Ld(n)})`}function re(e){let o=Number(e);if(!Number.isFinite(o)||o<=0)return null;let r=Math.floor(o/1e4),n=Math.floor(o/100)%100,a=o%100;return r<2020||n<1||n>12||a<1||a>31?null:{key:o,year:r,month:n,day:a,date:new Date(r,n-1,a,12,0,0)}}function Ft(e,o="day"){let r=re(e);return r?o==="weekday"?rv[r.date.getDay()]||"":o==="month"?r.date.toLocaleDateString("nl-NL",{month:"short"}):o==="year"?String(r.year):r.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"short"}):"\u2014"}function Lo(e){let o=A(e);return!Number.isFinite(o)||o<0?null:Math.round(o*1e3)}function sv(e){let o=Number(e);return Number.isFinite(o)&&o>=0?o:null}function Js(e,o=0){let r={};return La.forEach((n,a)=>{r[n]=sv(e[o+a])}),r}function lv(e){let o=String(e||"").trim();if(!o||o.startsWith("@"))return null;let r=o.split("|");if(r.length<10)return null;let n=Number(r[0]),a=Number(r[1]),i=Number(r[2]),l=re(a);return!Number.isFinite(n)||!l?null:{sequence:n,dateKey:a,year:l.year,month:l.month,day:l.day,partial:!!(i&1),source:"flash",...Js(r,3)}}function Zd(e){let o=String(e||"").trim();if(!o.startsWith("@current|"))return null;let r=o.split("|");if(r.length<9)return null;let n=Number(r[1]),a=re(n);return a?{sequence:Number.MAX_SAFE_INTEGER,dateKey:n,year:a.year,month:a.month,day:a.day,partial:!0,source:"current",...Js(r,2)}:null}function cv(e){let o=String(e||"").trim();if(!o.startsWith("@hour|"))return null;let r=o.split("|");if(r.length<11)return null;let n=Number(r[1]),a=Number(r[2]),i=Number(r[3]),l=re(a);return!Number.isFinite(n)||!l||!Number.isInteger(i)||i<0||i>23?null:{sequence:n,dateKey:a,year:l.year,month:l.month,day:l.day,hour:i,partial:!0,source:"hour",label:String(i),tooltipLabel:`${String(i).padStart(2,"0")}:00 - ${String((i+1)%24).padStart(2,"0")}:00`,sortKey:a*100+i,...Js(r,4)}}function uv(){let e=Zs()||Fa(),o=re(e);if(!o)return null;let r={sequence:Number.MAX_SAFE_INTEGER-1,dateKey:e,year:o.year,month:o.month,day:o.day,partial:!0,source:"sensors",electricalInputWh:Lo("electricalEnergyDaily"),heatingInputWh:Lo("heatingElectricalEnergyDaily"),coolingInputWh:Lo("coolingElectricalEnergyDaily"),heatpumpHeatOutputWh:Lo("heatpumpThermalEnergyDaily"),heatpumpCoolingOutputWh:Lo("heatpumpCoolingEnergyDaily"),boilerHeatOutputWh:Lo("boilerThermalEnergyDaily"),systemHeatOutputWh:Lo("systemThermalEnergyDaily")};return La.some(n=>Number.isFinite(r[n]))?r:null}function Sn(e,o){let r=Number(e?.[o]);return Number.isFinite(r)&&r>=0?r:0}function qn(){let e=new Map;String(t.energyHistoryRaw||"").split(/\r?\n/).forEach(i=>{let l=lv(i)||Zd(i);if(!l)return;let p=e.get(l.dateKey);(!p||l.sequence>=p.sequence)&&e.set(l.dateKey,l)});let r=uv();r&&e.set(r.dateKey,r);let n=new Set(e.keys()),a=new Map;return Ia().forEach(i=>{if(n.has(i.dateKey))return;let l=a.get(i.dateKey);if(!l){let p=re(i.dateKey);if(!p)return;l=Fo({dateKey:p.key,year:p.year,month:p.month,day:p.day,label:Ft(p.key),sortKey:p.key,source:"hour-summary"}),l.tooltipLabel=`${Ft(i.dateKey)} \xB7 uurdata sinds herstart`,a.set(i.dateKey,l)}Io(l,i)}),a.forEach((i,l)=>{e.set(l,i)}),[...e.values()].sort((i,l)=>i.dateKey-l.dateKey)}function Ia(){let e=new Map;return String(t.energyHistoryRaw||"").split(/\r?\n/).forEach(r=>{let n=cv(r);if(!n)return;let a=`${n.dateKey}:${n.hour}`,i=e.get(a);(!i||n.sequence>=i.sequence)&&e.set(a,n)}),[...e.values()].sort((r,n)=>r.sortKey-n.sortKey)}function pv(e){return Ia().filter(o=>o.dateKey===Number(e))}function yr(e,o){return e.reduce((r,n)=>r+Sn(n,o),0)}function Jd(e){return["heatpumpHeatOutputWh","heatpumpCoolingOutputWh","boilerHeatOutputWh"].reduce((o,r)=>o+Sn(e,r),0)}function dv(e){return Sn(e,"electricalInputWh")+Jd(e)}function Da(e,o){let r=Number(e),n=Number(o);return!Number.isFinite(r)||!Number.isFinite(n)||n<=0?"\u2014":(r/n).toFixed(2)}function Ze(e,o=1){let r=Number(e);return Number.isFinite(r)?Math.abs(r)>=999500?`${(r/1e6).toFixed(2)} MWh`:Math.abs(r)<1e3?`${Math.round(r)} Wh`:`${(r/1e3).toFixed(o)} kWh`:"\u2014"}function Fo({dateKey:e,year:o,month:r,day:n,hour:a=null,label:i,tooltipLabel:l="",sortKey:p,source:u="bucket"}){return{sequence:0,dateKey:e,year:o,month:r,day:n,hour:a,label:i,tooltipLabel:l,sortKey:p??e,partial:!1,source:u,electricalInputWh:0,heatingInputWh:0,coolingInputWh:0,heatpumpHeatOutputWh:0,heatpumpCoolingOutputWh:0,boilerHeatOutputWh:0,systemHeatOutputWh:0}}function Io(e,o){return La.forEach(r=>{e[r]+=Sn(o,r)}),e.partial=e.partial||!!o?.partial,e.sequence=Math.max(Number(e.sequence||0),Number(o?.sequence||0)),e}function mv(e){let o=new Map;return e.forEach(r=>{o.set(r.dateKey,r)}),o}function Xd(e,o){let r=Oe(e);if(r==="day"){let n=Ud(o)||re(o);return n?String(n.key):""}if(r==="week"){let n=iv(o);return n?String(n.key):""}if(r==="month"){let n=av(o);return n?String(n.key):""}if(r==="year"){let n=Number(o);return Number.isInteger(n)&&n>=2020&&n<=2200?String(n):""}return""}function _a(e,o){let r=Oe(o),n=re(nv(e,!0)),a=vn(),i=Ia(),l=[...e.map(w=>w.dateKey),...i.map(w=>w.dateKey)].filter(w=>Number.isFinite(Number(w)));Number.isFinite(Number(a.oldestDateKey))&&l.push(Number(a.oldestDateKey)),Number.isFinite(Number(a.newestDateKey))&&l.push(Number(a.newestDateKey));let p=l.length?Math.min(...l.map(Number)):n?.key,c=(p?re(p):n)?.date||n?.date||new Date,m=n?.date||new Date,b=n?.key||Fa(),y=b;return r==="week"?(b=Fd(c),y=Fd(m)):r==="month"?(b=zs(c),y=zs(m)):r==="year"?(b=c.getFullYear(),y=m.getFullYear()):(b=Je(c),y=Je(m)),Number(b)>Number(y)&&(b=y),{min:String(b),max:String(y)}}function em(e,o){let r=Number(e);return Number.isFinite(r)?r<Number(o.min)?String(o.min):r>Number(o.max)?String(o.max):String(e):String(o.max)}function gv(e,o,r=_a(e,o)){let n=Oe(o),a=t.energyHistoryPeriodSelection?.[n],i=Xd(n,a);return em(i||r.max,r)}function hv(e,o){let r=Oe(e),n=[],a=0;if(r==="week"){let i=String(o.max);for(;Number(i)>=Number(o.min)&&a<6e3;){let l=re(i);n.push({value:i,label:Yd(i),group:l?String(Qd(l.date).year):""}),i=l?String(Je(wr(l.date,-7))):"",a+=1}return n}if(r==="month"){let i=String(o.max);for(;Number(i)>=Number(o.min)&&a<1200;){let l=vr(i);if(!l)break;n.push({value:i,label:l.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"}),group:String(l.year)}),i=zd(i,-1),a+=1}return n}if(r==="year")for(let i=Number(o.max);i>=Number(o.min);i-=1)n.push({value:String(i),label:String(i)});return n}function Wa(e,o){let r=Oe(o);if(!Sr(r))return{view:r,selectedValue:"",minValue:"",maxValue:"",canPrevious:!1,canNext:!1,isNow:!0,options:[]};let n=_a(e,r),a=gv(e,r,n);return{view:r,selectedValue:a,minValue:n.min,maxValue:n.max,canPrevious:Number(a)>Number(n.min),canNext:Number(a)<Number(n.max),isNow:Number(a)===Number(n.max),options:hv(r,n)}}function fv(e,o){let r=Oe(o);if(!Sr(r))return{from:"",to:"",hours:"0"};let n=Wa(e,r);if(r==="day")return{from:n.selectedValue,to:n.selectedValue,hours:"1"};if(r==="week"){let a=re(n.selectedValue);if(!a)return{from:"",to:"",hours:"0"};let i=wn(a.date),l=wr(i,6);return{from:String(Je(i)),to:String(Je(l)),hours:"0"}}if(r==="month"){let a=vr(n.selectedValue);return a?{from:String(a.year*1e4+a.month*100+1),to:String(a.year*1e4+a.month*100+jd(a.year,a.month)),hours:"0"}:{from:"",to:"",hours:"0"}}if(r==="year"){let a=Number(n.selectedValue);return Number.isInteger(a)?{from:`${a}0101`,to:`${a}1231`,hours:"0"}:{from:"",to:"",hours:"0"}}return{from:"",to:"",hours:"0"}}function bv(){if(!String(t.energyHistoryRaw||"").trim())return"?meta=1";let e=qn(),o=fv(e,t.energyHistoryView||"day"),r=new URLSearchParams;o.from&&r.set("from",o.from),o.to&&r.set("to",o.to),r.set("hours",o.hours);let n=r.toString();return n?`?${n}`:""}hd(bv);function Tn(e,o){let r=Oe(e);if(!Sr(r))return;let n=qn(),a=_a(n,r),i=Xd(r,o),l=em(i||a.max,a);t.energyHistoryPeriodSelection={...t.energyHistoryPeriodSelection,[r]:l},t.energyHistoryLastFetchAt=0,d(),Bd()}function Xs(e,o){let r=Oe(e);if(!Sr(r))return;let n=qn(),a=Wa(n,r),i=Number(o)<0?-1:1,l=a.selectedValue;if(r==="day"){let p=re(a.selectedValue);l=p?String(Je(wr(p.date,i))):l}else if(r==="week"){let p=re(a.selectedValue);l=p?String(Je(wr(p.date,i*7))):l}else r==="month"?l=zd(a.selectedValue,i):r==="year"&&(l=String(Number(a.selectedValue)+i));Tn(r,l)}function el(e){let o=Oe(e);if(!Sr(o))return;let r=qn(),n=_a(r,o);Tn(o,n.max)}function yv(e,o,r=Wa(e,o)){let n=Oe(o),a=mv(e);if(!e.length&&n==="all")return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};if(n==="day"){let p=re(r.selectedValue);if(!p)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=pv(p.key);if(u.length){let w=new Map(u.map(f=>[f.hour,f])),g=[];for(let f=0;f<24;f+=1){let v=String(f),S=`${p.date.toLocaleDateString("nl-NL",{day:"numeric",month:"long"})} \xB7 ${String(f).padStart(2,"0")}:00 - ${String((f+1)%24).padStart(2,"0")}:00`,q=Fo({dateKey:p.key,year:p.year,month:p.month,day:p.day,hour:f,label:v,tooltipLabel:S,sortKey:f,source:"hour"}),C=w.get(f);C&&Io(q,C),g.push(q)}return{buckets:g,title:"Dag",detail:`${p.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 uurdata sinds herstart`}}let c=a.get(p.key),m=Zs()||Fa(),b=p.key===m?"Vandaag":Ft(p.key),y=Fo({dateKey:p.key,year:p.year,month:p.month,day:p.day,label:b,sortKey:p.key,source:"day"});return c&&Io(y,c),{buckets:[y],title:"Dag",detail:`${p.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 dagtotaal`}}if(n==="week"){let p=re(r.selectedValue);if(!p)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=wn(p.date),c=[];for(let m=0;m<7;m+=1){let b=wr(u,m),y=Je(b),w=re(y),g=Fo({dateKey:y,year:w.year,month:w.month,day:w.day,label:Ft(y,"weekday"),sortKey:y}),f=a.get(y);f&&Io(g,f),c.push(g)}return{buckets:c,title:"Week",detail:Yd(r.selectedValue)}}if(n==="month"){let p=vr(r.selectedValue);if(!p)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=jd(p.year,p.month),c=[];for(let m=1;m<=u;m+=1){let b=Kd(p.year,p.month,m),y=Je(b),w=Fo({dateKey:y,year:p.year,month:p.month,day:m,label:String(m),sortKey:y}),g=a.get(y);g&&Io(w,g),c.push(w)}return{buckets:c,title:"Maand",detail:p.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"})}}if(n==="year"){let p=Number(r.selectedValue);if(!Number.isInteger(p))return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=[];for(let c=1;c<=12;c+=1){let m=p*1e4+c*100+1,b=Fo({dateKey:m,year:p,month:c,day:1,label:Ft(m,"month"),sortKey:c,source:"month"});e.filter(y=>y.year===p&&y.month===c).forEach(y=>Io(b,y)),u.push(b)}return{buckets:u,title:"Jaar",detail:String(p)}}let i=new Map;e.forEach(p=>{i.has(p.year)||i.set(p.year,Fo({dateKey:p.year*1e4+101,year:p.year,month:1,day:1,label:String(p.year),sortKey:p.year,source:"year"})),Io(i.get(p.year),p)});let l=[...i.values()].sort((p,u)=>p.sortKey-u.sortKey);return{buckets:l,title:"Alles",detail:l.length?`${l[0].label} - ${l[l.length-1].label}`:"Geen data"}}function vv(e){let o=yr(e,"heatpumpHeatOutputWh"),r=yr(e,"heatpumpCoolingOutputWh"),n=yr(e,"boilerHeatOutputWh");return{electricalInputWh:yr(e,"electricalInputWh"),heatingInputWh:yr(e,"heatingInputWh"),coolingInputWh:yr(e,"coolingInputWh"),heatOutputWh:o,coolingOutputWh:r,boilerOutputWh:n,outputWh:o+r+n}}function wv(e){let o=Number(e.heatOutputWh||0)+Number(e.coolingOutputWh||0),r=Number(e.boilerOutputWh||0),n=o+r;return!Number.isFinite(n)||n<=0?Number.NaN:o/n*100}function Oa(e,o,r=""){return`
    ...
    `}function Sv(e){return Sr(e.view)?`
...
`)}function Hv(e,o=""){if(!e.length)return`
      ...
      `;let r=$v(e),n=xv(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,p)=>{let u=r.left+r.barSlot*p+r.barSlot/2,c=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],m=r.height-r.bottom,b=c.map(f=>{let v=Sn(l,f.key);if(v<=0)return"";let S=v/r.axisMax*r.plotHeight;return m-=S,`
          ...
          >
            <title>${s(`${l.label} \xB7 ${f.label}: ${Ze(v,1)}`)}</title>
          </rect>
        `}).join(""),w=e.length<=12||p===0||p===e.length-1||p%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||Ft(l.dateKey))}</text>`:"",g=Av(l);return`
    ...
    `}function tl(){let e=qn(),o=Oe(t.energyHistoryView),r=Wa(e,o),n=yv(e,o,r),a=vv(n.buckets);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:a,activeView:o}}function tm(e=tl()){return Se({energyHistorySignature:t.energyHistorySignature||"",energyHistoryError:t.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,currentValues:La.map(o=>e.buckets[e.buckets.length-1]?.[o]??null)})}function om(e=tl()){let o=e.summary,r=e.buckets[0]?.dateKey?Ft(e.buckets[0].dateKey):"\u2014",n=e.buckets[e.buckets.length-1]?.dateKey?Ft(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
      <section class="oq-energy-history" data-render-signature="${s(tm(e))}">
        ...
        <div class="oq-energy-history-stats">
          ${Oa("Gemiddelde COP",Da(o.heatOutputWh,o.heatingInputWh),`${s(r)} - ${s(n)}`)}
          ${Oa("Elektrisch",Ze(o.electricalInputWh,1),"verbruikt")}
          ${Oa("Warmtepomp",Ze(o.heatOutputWh+o.coolingOutputWh,1),"warmte en koeling")}
          ${Oa("Cv-ketel",Ze(o.boilerOutputWh,1),"thermisch")}
        </div>
        ${Ev(o)}
    ...
    `}function Rv(){if(!t.root||t.appView!=="energy")return!1;let e=t.root.querySelector(".oq-overview-board"),o=e?e.querySelector(".oq-overview-energy"):null;if(!e||!o)return!1;let r=`oq-overview-board oq-overview-board--${t.overviewTheme}`;e.className!==r&&(e.className=r);let n=Qs();return lt(o,_d(n),Wd(n))}function Pv(){if(!t.root||t.appView!=="results")return!1;let e=t.root.querySelector(".oq-overview-board"),o=e?e.querySelector(".oq-energy-history"):null;if(!e||!o)return!1;let r=`oq-overview-board oq-overview-board--${t.overviewTheme}`;e.className!==r&&(e.className=r);let n=tl(),a=qv();return(a?!1:lt(o,tm(n),om(n)))||a}Jo({patchEnergyDom:Rv,patchResultsDom:Pv});function rl(e,o,r="",n=!1){return`
      <div class="oq-settings-storage-summary-metric${n?" is-on":""}">
        <span>${s(e)}</span>
        <strong>${s(o)}</strong>
        ${r?`<em>${s(r)}</em>`:""}
      </div>
    `}function am(e){let o=String(e||"").trim(),r=o.match(/^(\d+(?:[.,]\d+)?)\s+records?$/i);return r?`${r[1]} ${r[1]==="1"?"dag":"dagen"}`:o}function nl(e,o,r,n="",a="",i=""){if(!h(e))return"";let l=!!E(e),p=t.loadingEntities||t.busyAction===`switch-${e}`;return`
    ...
    `}function Nv(e,o,r,n=""){if(!h(e))return"";let a=t.entities[e],i=Array.isArray(a?.option)?a.option:[],l=String(E(e)||"");return i.length?`
    ...
    `}function Dv(){return t.trendHistoryMetadata&&typeof t.trendHistoryMetadata=="object"?t.trendHistoryMetadata:{}}function qr(){return!!t.trendHistoryMetadataSignature}function Tr(){return!!(t.energyHistoryRaw||t.energyHistorySignature)}function Xe(e){return e?"Niet geladen":"Laden..."}function De(e,o="\u2014"){if(h(e))return Te(e);let r=Lv(e);return r==null||r===""?o:r}function Lv(e){let o=Dv();if(e==="trendHistoryFlashAvailable")return qr()?o.available||"Alleen live":Xe(t.trendHistoryMetadataError);if(e==="trendHistoryFlashOldest")return qr()?o.oldest||"Geen data":Xe(t.trendHistoryMetadataError);if(e==="trendHistoryFlashNewest")return qr()?o.newest||"Geen data":Xe(t.trendHistoryMetadataError);if(e==="trendHistoryFlashLastFlush")return qr()?o.lastFlush||"Geen data":Xe(t.trendHistoryMetadataError);if(e==="trendHistoryFlashSize")return qr()?il(o.sizeKb):Xe(t.trendHistoryMetadataError);if(e==="trendHistoryFlashWrites")return qr()?sl(o.writes):Xe(t.trendHistoryMetadataError);let r=lm(),a=String(t.energyHistoryRaw||"").includes("@day_retention|");return e==="lifetimeEnergyHistoryAvailable"?Tr()?a&&!r.dayPartitionAvailable?"Niet beschikbaar":im(r.storedDayCount,"Geen data"):Xe(t.energyHistoryError):e==="lifetimeEnergyHistoryOldest"?Tr()?Va(r.oldestDateKey):Xe(t.energyHistoryError):e==="lifetimeEnergyHistoryNewest"?Tr()?Va(r.newestDateKey):Xe(t.energyHistoryError):e==="lifetimeEnergyHistoryLastWrite"?Tr()?sm(r.dayLastWriteTimestampS):Xe(t.energyHistoryError):e==="lifetimeEnergyHistorySize"?Tr()?il(r.dayStorageKb):Xe(t.energyHistoryError):e==="lifetimeEnergyHistoryWrites"?Tr()?sl(r.dayWriteCount):Xe(t.energyHistoryError):null}function im(e,o="Geen data"){let r=Number(e);return!Number.isFinite(r)||r<=0?o:`${Math.round(r)} ${Math.round(r)===1?"dag":"dagen"}`}function il(e,o="\u2014"){let r=Number(e);return!Number.isFinite(r)||r<=0?o:`${Math.round(r)} kB`}function sl(e,o="0"){let r=Number(e);return!Number.isFinite(r)||r<=0?o:String(Math.round(r))}function Va(e){let o=typeof re=="function"?re(e):null;return o?o.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit",year:"numeric"}):"Geen data"}function sm(e,o="Geen data"){let r=Number(e);if(!Number.isFinite(r)||r<=0)return o;let n=new Date(r*1e3),a=n.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit"}),i=n.toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"});return`${a} ${i}`}function lm(){return typeof vn!="function"?{}:vn()}function Fv(e){let o=Array.isArray(e.items)?e.items:[];return`
    ...
    `}function Wv(){if(!h("lifetimeEnergyHistoryEnabled"))return"";let e=fr(t.energyHistoryExportMode),o=bn.map(n=>`
    ...
    `}function pm(){let e=t.settingsBackupBusy,o=Kt.length,r=Bt.length;return te("Beheer","Backup en restore","Sla een JSON-backup op van de instellingen die OpenQuatt in deze web-app beheert, en zet die later weer terug na een factory-bin update.",`
    ...
    `}function mm(){let e=t.settingsBackupDraft;if(!e)return"";let o=e.summary||Ks(e),r=String(e.source?.installation||e.source?.device||"Onbekend"),n=it(),a=String(e.source?.firmware_version||"Onbekend"),i=String(e.source?.firmware_channel||"").trim()||"Onbekend",l=String(e.source?.topology||"").trim()||"Onbekend",p=le(),u=typeof ee=="function"?ee():"",c=l!=="Onbekend"&&u&&l!==u,m=r!=="Onbekend"&&r!==n,b=c||m?"De backup lijkt van een andere installatie te komen. Je kunt nog steeds doorzetten, maar controleer de secties even goed.":o.requiredMissing?"Ontbrekende velden houden hun firmware-default.":"Velden zonder waarde worden overgeslagen.";return`
      <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
    ...
    `}function Kv(){if(!t.root)return!1;let e=t.root.querySelector(".oq-helper-status-grid");if(!e)return!!t.root.querySelector(".oq-helper-hub");let o=hm();if(e.querySelectorAll("[data-oq-header-status]").length!==o.length)return e.outerHTML=En(),!0;for(let[n,a,i,l]of o){let p=e.querySelector(`[data-oq-header-status="${n}"]`);if(!p)return e.outerHTML=En(),!0;let u=gm(n),c=!!(l||u);if(p.tagName.toLowerCase()!==(c?"button":"div"))return e.outerHTML=En(),!0;let m=p.querySelector(".oq-helper-status-label"),b=p.querySelector(".oq-helper-status-value");if(!m||!b)return e.outerHTML=En(),!0;m.textContent!==a&&(m.textContent=a);let y=bm(n),w=y?`<span class="oq-helper-status-value-text">${s(i)}</span><span class="oq-helper-status-badge" aria-label="Update beschikbaar" title="Update beschikbaar"></span>`:s(i);b.innerHTML!==w&&(b.innerHTML=w),c?p.setAttribute("data-oq-action",u):p.removeAttribute("data-oq-action"),p.classList.toggle("oq-helper-status-item--button",c),p.classList.toggle("oq-helper-status-item--attention",y)}return!0}function jv(){let e=typeof window<"u"?window.__OQ_DEV_CONTROLS__:null;return!e||typeof e.render!="function"?"":e.render()}function ul(){let e=jv();return e?t.devPanelOpen?`
    ...
    `}bu({getSignature:Ba,patch:Kv});function wm(){if(t.systemModal==="login")return sd();if(t.systemModal==="api-security")return id();if(t.systemModal==="mqtt")return Ru();if(t.systemModal==="mqtt-sensors")return Pu();if(t.systemModal==="connectivity"){let e=Bv();return`
        <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
      ...
      `;if(t.systemModal==="webserver-logs")return Cp();if(t.systemModal==="debug-recording")return Zp();if(t.systemModal==="openquatt-pause"){let e=k("openquattEnabled"),o=t.busyAction==="openquatt-regulation",r=h("openquattResumeAt"),n=t.loadingEntities||t.entitySyncInFlight,a=r||!n,i=Wr(),l=Qo(E("openquattResumeAt")),p=Yn();return`
        <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
      ...
      `}return""}function Uv(e=!1){let o=Zn();return o?`
    ...
    `:""}function pl(){return`
      <div class="oq-settings-curve-grid">
        ...
        ${ie("curveFallbackSupply","Fallback-aanvoertemperatuur zonder buitentemperatuur","Aanvoertemperatuur die gebruikt wordt als de buitentemperatuursensor niet beschikbaar is.","oq-settings-field--curve-fallback-card",{footerMarkup:Uv()})}
      </div>
    ...
    `}function ml(e="oq-settings-grid"){let o=[ie("flowSetpoint","Gewenste flow verwarmen","De flow die OpenQuatt zoveel mogelijk probeert vast te houden buiten koeling."),ie("coolingFlowSetpoint","Gewenste flow koelen","De flow die OpenQuatt gebruikt tijdens actieve koeling.")].filter(Boolean).join("");return`
      <div class="${s(e)}">
        ${to("flowControlMode","Regelmodus","Kies tussen automatische flowregeling en een vaste pompstand.")}
        ${Rc()?ie("manualIpwm","Vaste pompstand","Deze pompstand wordt gebruikt zolang de regeling op handmatig staat."):o}
      </div>
    `}function Gv(e="oq-settings-grid"){let o=[ie("flowKp","Flow PI Kp","Hoe sterk de regeling direct reageert op een afwijking."),ie("flowKi","Flow PI Ki","Hoe snel de regeling kleine restfouten wegwerkt.")].filter(Boolean);return o.length?`
      <div class="${s(e)}">
        ${o.join("")}
      </div>
    ...
    `}function zv(){if(!h("phResponseProfile"))return"";let e=String(E("phResponseProfile")||""),o=t.loadingEntities||t.busyAction==="save-phResponseProfile",n=`
    ...
    `;return pe("phResponseProfile","Power House responsprofiel","Kies hoe rustig of direct Power House mag reageren op veranderingen in je woning.",n,"oq-settings-field--span-2")}function fl(){if(!h("curveControlProfile"))return"";let e=String(E("curveControlProfile")||""),o=t.loadingEntities||t.busyAction==="save-curveControlProfile",n=`
    ...
    `;return pe("curveControlProfile","Regelprofiel","Kies of de stooklijn vooral comfortabel, gebalanceerd of rustig moet reageren.",n,"oq-settings-field--span-2")}function Qv(){let e=(D,Y=0)=>{let N=A(D);return Number.isNaN(N)?Y:Math.max(0,N)},r=e("phComfortBelow",.1),n=e("phComfortAbove",.3),a=e("phKp",3e3),i=20-r,l=20+n,p=620,u=184,c=46,m=24,b=18,y=40,w=96,g=p-c-m,f=Math.min(20-1.2,i-.35),v=Math.max(20+1.2,l+.35),S=D=>c+(D-f)/Math.max(.01,v-f)*g,q=S(f),C=S(v),x=S(i),H=S(20),V=S(l),K=Math.abs(i-20)>.001,M=Math.abs(l-20)>.001,I=b+24,z=u-y,ne=w-44,fe=(D,Y,N,F="")=>{let P=Math.max(q+4,Math.min(C-110-4,D-55)),B=D-14,Q=ne,J=28,X=w-ne+16;return`
        <g class="oq-ph-concept-hotspot" tabindex="0" role="img" aria-label="${s(`${Y} ${N}`)}">
          <rect class="oq-ph-concept-hit" x="${B}" y="${Q}" width="${J}" height="${X}" rx="10"></rect>
          <circle class="oq-ph-concept-hit" cx="${D}" cy="${w}" r="14"></circle>
      ...
      `},Z=[`M ${q.toFixed(1)} ${I.toFixed(1)}`,`L ${x.toFixed(1)} ${w.toFixed(1)}`,`L ${V.toFixed(1)} ${w.toFixed(1)}`,`L ${C.toFixed(1)} ${z.toFixed(1)}`].join(" ");return`
      <div class="oq-ph-concept-card">
        <div class="oq-ph-concept-visual">
          ...
          <svg class="oq-ph-concept-svg" viewBox="0 0 ${p} ${u}" role="img" aria-label="Grafiek voor Power House tuning">
            ...
            ${K?`<line x1="${x}" y1="${w-12}" x2="${x}" y2="${w+12}" class="oq-ph-concept-marker oq-ph-concept-marker--below"></line>`:""}
            <line x1="${H}" y1="${w-14}" x2="${H}" y2="${w+14}" class="oq-ph-concept-marker oq-ph-concept-marker--setpoint"></line>
            ${M?`<line x1="${V}" y1="${w-12}" x2="${V}" y2="${w+12}" class="oq-ph-concept-marker oq-ph-concept-marker--above"></line>`:""}
            ${K?`<circle cx="${x}" cy="${w}" r="5" class="oq-ph-concept-point oq-ph-concept-point--below"></circle>`:""}
            <circle cx="${H}" cy="${w}" r="6" class="oq-ph-concept-point oq-ph-concept-point--setpoint"></circle>
            ${M?`<circle cx="${V}" cy="${w}" r="5" class="oq-ph-concept-point oq-ph-concept-point--above"></circle>`:""}
            ${K?fe(x,"Comfort onder setpoint",_(i,1,"\xB0C"),"below"):""}
            ${fe(H,"Setpoint",_(20,1,"\xB0C"),"setpoint")}
            ${M?fe(V,"Comfort boven setpoint",_(l,1,"\xB0C"),"above"):""}
            ...
            ${K?`<text x="${x-5}" y="${u-14}" text-anchor="end" class="oq-ph-concept-tick-value">${s(_(i,1,"\xB0C"))}</text>`:""}
            <text x="${H}" y="${u-14}" text-anchor="middle" class="oq-ph-concept-tick-value oq-ph-concept-tick-value--setpoint">${s(_(20,1,"\xB0C"))}</text>
            ${M?`<text x="${V+5}" y="${u-14}" text-anchor="start" class="oq-ph-concept-tick-value">${s(_(l,1,"\xB0C"))}</text>`:""}
          </svg>
        </div>
    ...
    `:""}function yl(e,o,r){let n=[to(o,"Stand A","Kies hier welke compressorstand je wilt uitsluiten."),to(r,"Stand B","Kies hier nog een compressorstand die je wilt overslaan.")].filter(Boolean).join("");return n?`
    ...
    `:""}function Sm(){let e=Gv();return te("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
        ${ml()}
        ${e?`
          <div class="oq-settings-subpanel oq-settings-subpanel--nested">
            <div class="oq-settings-subpanel-head">
      ...
      `).join(""),b=me.map(w=>`${p(w.outdoor)},${u(ge(w.key,E(w.key)))}`).join(" "),y=me.map(w=>{let g=ge(w.key,E(w.key));return`
          <g>
            <circle
              cx="${p(w.outdoor)}"
              cy="${u(g)}"
      ...
      <div class="oq-settings-odu-runtime-table" role="table" aria-label="${s(`HP${e} ODU runtime frequentietabel`)}">
        ...
        ${Uo.map(r=>`
          <div class="oq-settings-odu-runtime-row" role="row">
            ...
            <div role="cell">${Tm(mo(e,"cooling",r),r)}</div>
            <div role="cell">${Tm(mo(e,"heating",r),o+r)}</div>
          </div>
        `).join("")}
      </div>
    `}function km(e){if(e.key!=="Tab"||e.altKey||e.ctrlKey||e.metaKey)return;let o=e.target&&e.target.closest?e.target.closest("input[data-oq-odu-runtime-tab-index]"):null,r=o?o.closest(".oq-settings-odu-runtime-table"):null;if(!o||!r)return;let n=Array.from(r.querySelectorAll("input[data-oq-odu-runtime-tab-index]:not(:disabled)")).sort((l,p)=>Number(l.dataset.oqOduRuntimeTabIndex||0)-Number(p.dataset.oqOduRuntimeTabIndex||0)),a=n.indexOf(o),i=n[a+(e.shiftKey?-1:1)];a<0||!i||(e.preventDefault(),i.focus(),typeof i.select=="function"&&i.select())}function ow(e){let o=ve(e,"Enable"),r=ve(e,"Load"),n=ve(e,"Apply"),a=ve(e,"Status"),i=String(E(a)||"").trim()||"Nog niet geladen",l=Jv(e),p=Xv(e),u=!!E(o),c=t.loadingEntities||t.busyAction===r||t.busyAction===n,m=c||!u||!l.valid||!p.safe||!h(n),b=l.valid?"Waarden zijn 0-120 Hz en per tabel oplopend.":`Controleer ${l.invalid.slice(0,5).join(", ")}${l.invalid.length>5?"...":""}.`;return`
    ...
    `:""}function Ua(e,o="Aandacht",r="OK"){return`<span class="oq-settings-monitoring-badge${e?" is-warning":" is-clear"}">${s(e?o:r)}</span>`}function Er({label:e,value:o,note:r="",active:n=!1}){return`
    ...
    `}function co(e){let o=A(e);return Number.isNaN(o)?"\u2014":String(Math.max(0,Math.round(o)))}function rw(e){let o=A(e);if(Number.isNaN(o))return"Nog niet gemeten";if(o<1)return"Zojuist";if(o<60)return`${Math.round(o)} min geleden`;let r=Math.floor(o/60),n=Math.round(o%60);return`${r}u ${n}m geleden`}function Em(e){let o=A(e);return Number.isNaN(o)||o<=0?"Tijdstip onbekend":new Intl.DateTimeFormat("nl-NL",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}).format(new Date(o*1e3))}function nw(e){if(!e.cyclingAlertLatched)return"";let o=G("compressorCyclingAlertAlternating"),r=co("compressorCyclingAlertHp1Peak2h"),n=co("compressorCyclingAlertHp1Peak72h"),a=h("compressorCyclingAlertHp2Peak2h")?co("compressorCyclingAlertHp2Peak2h"):"",i=h("compressorCyclingAlertHp2Peak72h")?co("compressorCyclingAlertHp2Peak72h"):"";return`
      <div class="oq-settings-monitoring-incident${e.cyclingAlertActive?" is-active":" is-recovered"}">
        <div class="oq-settings-monitoring-incident-head">
          ...
          ${Ua(e.cyclingAlertActive,"Actief","Hersteld")}
        </div>
    ...
    `:""}function $m(){let e=At();Yr(e);let o=ht("cicPollingEnabled"),r=ht("otEnabled"),n=[h("lowflowFaultActive")?Er({label:"Flow",value:G("lowflowFaultActive")?"Te lage flow gemeld":"Geen lage-flowmelding",active:G("lowflowFaultActive")}):"",h("flowMismatch")?Er({label:"Flowvergelijking duo",value:G("flowMismatch")?"Afwijking tussen warmtepompen":"Geen afwijking gemeld",active:G("flowMismatch")}):""].filter(Boolean).join(""),a=[h("cicDataStale")?Er({label:"CIC-data",value:o?G("cicDataStale")?"Verouderd":"Geen probleem gemeld":"Polling uitgeschakeld",active:o&&G("cicDataStale")}):"",h("otLinkProblem")?Er({label:"OpenTherm",value:r?G("otLinkProblem")?"Verbindingsprobleem":"Geen probleem gemeld":"Uitgeschakeld",active:r&&G("otLinkProblem")}):""].filter(Boolean).join(""),i=[h("hp1Failures")?Er({label:"Warmtepomp 1",value:ji("hp1Failures"),active:Qr("hp1Failures")}):"",h("hp2Failures")?Er({label:"Warmtepomp 2",value:ji("hp2Failures"),active:Qr("hp2Failures")}):""].filter(Boolean).join(""),l=A("compressorStarts2hWarningLimit"),p=A("compressorStarts72hWarningLimit"),u=G("compressorCyclingWarning2h")||G("compressorCyclingWarning72h")||G("alternatingCompressorStartsWarning")||e.cyclingAlertLatched,c=n?`
    ...
    `:"";return te("Bewaking","Installatiebewaking","Lokale diagnose voor compressorstarts, hydrauliek en verbindingen. Hiervoor is geen Home Assistant nodig.",`
        <div class="oq-settings-monitoring-summary${e.active?" is-warning":" is-clear"}">
          ...
          ${Ua(e.active,"Aandacht nodig","Alles rustig")}
        </div>
      ...
      `)}function Sl(){if(!h("hpGeneration"))return"";let e={V1:{copy:"Voor Quatt V1 en Quatt V1 + V1.5 combinaties.",image:ja,alt:"Quatt Hybrid V1 en V1.5",infoTitle:"V1",infoCopy:`Model: AMM4
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},o=t.entities.hpGeneration||{},r=String(E("hpGeneration")||""),n=Rt(o),a=t.loadingEntities||t.busyAction==="save-hpGeneration";return`
    ...
    `}function Am(){let e=it(),o=t.entities.hpGeneration||{},r=h("hpGeneration")&&Rt(o).length>0;return!e&&!r?"":te("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Rm(){let e=ro(),o=ua(),r=t.busyAction==="restartAction";return te("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ...
          ${$e({dataValue:"webserverLog",label:"Logboek",value:ga(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
          ...
          ${$e({dataValue:"debugRecording",label:"Debugopname",value:sn(),note:Ea(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
              data-oq-action="open-debug-recording-modal"
            >
          ...
          ${$e({dataValue:"espTemp",label:"ESP-temp",value:Ka()})}
      ...
      `)}function Pm(){let e=[yl("Warmtepomp 1","hp1ExcludedA","hp1ExcludedB"),yl("Warmtepomp 2","hp2ExcludedA","hp2ExcludedB")].filter(Boolean).join("");return te("Installatie","Compressorinstellingen","Stel hier de minimale draaitijd in en bepaal per warmtepomp welke compressorstanden je wilt overslaan.",`
        <div class="oq-settings-subpanel">
          <div class="oq-settings-subpanel-head">
    ...
    `}function sw(){let e=Ga(),o=t.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function mw(){return`
      <section class="oq-helper-panel">
        ...
        ${Ha("oq-settings-grid oq-settings-grid--quickstart")}
        ${It()}
      </section>
    `}function gw(){return`
      <section class="oq-helper-panel">
    ...
    `}function hw(){return t.currentStep==="generation"?Dm():t.currentStep==="boiler"?h("boilerCvAssistEnabled")?cw():Om():t.currentStep==="flow-source"?iw():t.currentStep==="thermostat-source"?sw():t.currentStep==="flow"?uw():t.currentStep==="heating"?pw():t.currentStep==="water"?dw():t.currentStep==="silent"?mw():t.currentStep==="confirm"?gw():Om()}function _o(){return jo.filter(e=>!e.optionalEntity||h(e.optionalEntity))}function St(e){let o=_o().findIndex(r=>r.id===e);return`Stap ${Math.max(0,o)+1}`}function fw(e){let o=za(),r=e===o,n=t.complete===!0||e<o;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function bw(e=!1){return _o().map((o,r)=>{let n=fw(r);return`
      ...
      `}).join("")}function yw(){let e=_o();return e.find(o=>o.id===t.currentStep)||e[0]||jo[0]}function za(){return Math.max(0,_o().findIndex(e=>e.id===t.currentStep))}function El(e){let o=_o(),r=Math.min(o.length-1,Math.max(0,za()+e));t.currentStep=o[r]?.id||jo[0].id}function It(e={}){let o=za(),r=_o(),n=o>0?r[o-1]:null,a=o<r.length-1?r[o+1]:null;return`
    ...
    `}function vw(){let e=za(),o=_o();return`
    ...
    `}function Cl(e){let o=String(e||"").trim();return o?{Ready:"Gereed","Waiting for room request":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","Cooling enabled, waiting for room temperature above cooling setpoint":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","No dew point source":"Geen dauwpuntbron","OpenQuatt paused":"OpenQuatt gepauzeerd","Cooling disabled":"Koeling uitgeschakeld","Cooling minimum unavailable":"Minimale koel-aanvoer onbekend","Flow too low":"Flow te laag","Fallback active":"Dauwpuntsbenadering actief","Fallback active (+0.5\xB0C warm night)":"Dauwpuntsbenadering actief (+0,5\xB0C warme nacht)","Fallback active (+1.0\xB0C very warm night)":"Dauwpuntsbenadering actief (+1,0\xB0C zeer warme nacht)","Fallback active (+1.5\xB0C tropical night)":"Dauwpuntsbenadering actief (+1,5\xB0C tropische nacht)","User responsibility (no dew point or fallback)":"Expliciet toegestaan (geen dauwpunt of benadering)","Fallback cooling active":"Dauwpuntsbenadering actief","Fallback corrected by warm night":"Dauwpuntsbenadering gecorrigeerd door warme nacht","Fallback blocked by tropical night":"Dauwpuntsbenadering geblokkeerd door tropische nacht"}[o]||o:""}function Bm(){let e=!h("coolingRoomRequestRequired")||k("coolingRoomRequestRequired"),o=[ie("coolingMinimumSupplyTemp","Minimale koel-aanvoer","Ondergrens voor het koeldoel. OpenQuatt gebruikt de hoogste waarde van deze instelling en de dauwpuntveilige grens."),ze("coolingDemandMax","Maximale koelsterkte","Bepaalt hoe krachtig OpenQuatt mag koelen. Lager geeft langere, rustigere runs; hoger geeft meer koelvermogen bij warm weer.","",{minLabel:"Rustig",maxLabel:"Krachtig",valueLabel:`${j("coolingDemandMax")} max`}),ie("coolingRestartDelta","Herstartmarge watertemperatuur","Na het bereiken van het koel-aanvoerdoel start de watercyclus pas opnieuw zodra de aanvoer deze marge boven het doel ligt."),ie("coolingSafetyMargin","Dauwpunt veiligheidsmarge","Extra marge boven het geselecteerde dauwpunt voor de minimale veilige watertemperatuur.")].filter(Boolean),r=[h("coolingRoomRequestRequired")?sp("coolingRoomRequestRequired","Koelvraag via kamerthermostaat","Aan: OpenQuatt wacht op echte koelvraag vanuit de kamer. Uit: koeltoestemming geldt direct als koelvraag.","Koelvraag start en stopt met de marges hieronder.","Koeltoestemming geldt direct als koelvraag. De start- en stopmarge worden nu niet gebruikt.","oq-settings-field--span-2"):"",e?ie("coolingRequestOnDelta","Koelvraag start boven setpoint","Koelvraag wordt actief zodra de kamer warmer is dan setpoint plus deze marge."):"",e?ie("coolingRequestOffDelta","Koelvraag stopt boven setpoint","Koelvraag valt weer af zodra de kamer koeler is dan setpoint plus deze marge."):""].filter(Boolean),n=r.length>0,a=h("coolingWithoutDewPointMode"),i=[h("coolingGuardMode")?Cn("Route",qe(O("coolingGuardMode","Onbekend"))):"",h("coolingEffectiveMinSupplyTemp")?Cn("Actieve ondergrens",O("coolingEffectiveMinSupplyTemp","\u2014")):""].filter(Boolean),l=i.length?pe("coolingGuardStatus","Actuele beveiliging","Laat zien welke route koeling nu begrenst en welke ondergrens daadwerkelijk geldt.",`<div class="oq-settings-cooling-facts">${i.join("")}</div>`,"oq-settings-field--span-2 oq-settings-field--cooling-status"):"",p=[h("outsideTempSelected")?Cn("Actuele buitentemperatuur",O("outsideTempSelected","\u2014")):"",h("coolingFallbackNightMinOutdoorTemp")?Cn("Nachtminimum buitentemperatuur",O("coolingFallbackNightMinOutdoorTemp","\u2014")):"",h("coolingFallbackMinSupplyTemp")?Cn("Berekende minimum watertemperatuur",O("coolingFallbackMinSupplyTemp","\u2014")):""].filter(Boolean),u=p.length?`<div class="oq-settings-cooling-fallback-metrics">${p.join("")}</div>`:"",c=a||p.length>0,b=O("coolingGuardMode","").toLowerCase().includes("fallback");if(!o.length&&!n&&!a&&!l&&!c)return"";let y={"Dew point required":"Gebruik alleen een betrouwbare dauwpuntmeting. Zonder meting blijft koeling uit.","Allow without dew point":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use fallback":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use dew point approximation":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, user responsibility":"Negeer dauwpunt en benadering; alleen de ingestelde minimale koel-aanvoer geldt."};return te("Koeling","Koelingsinstellingen","Stel hier in wanneer koelvraag ontstaat, hoe koud het water mag worden en hoeveel het water mag opwarmen voor herstart.",`
        ...
        `:""}
      `)}function Km(){let e=h("otEnabled"),o=h("cicPollingEnabled")||h("cicFeedUrl"),r=h("cicCompatibilityMode"),n=h("otLinkProblem")||h("cicDataStale")||h("cicJsonFeedOk");if(!e&&!o&&!r&&!n)return"";let a=ht("cicPollingEnabled"),i=ht("otEnabled"),l=({label:g,value:f,active:v=!1})=>`
      <div class="oq-settings-integration-diagnostic-item${v?" is-warning":""}">
        <dt>${s(g)}</dt>
        <dd>${s(f)}</dd>
      </div>
    `,p=(g,f,v="Actief",S="Normaal",q={})=>{if(!h(g))return"";let C=G(g);return l({label:f,value:C?v:S,active:q.warningWhenActive?C:!1})},u=(g,f,v={})=>{let S=v.fallbackKey||"";return!h(g)&&!(S&&h(S))?"":l({label:f,value:Te(h(g)?g:S,v)})},c=(g,f)=>{let v=f.filter(Boolean).join("");return v?`
    ...
    `:"";return te("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
        <div class="oq-settings-integration-grid">
          ${da("otEnabled","OpenTherm","Thermostaatbus voor warmtevraag en kamerwaarden.")}
          ${da("cicPollingEnabled","CIC-polling","JSON-feed uitlezen voor setpoint, kamerwaarden en flow.")}
          ${da("cicCompatibilityMode","CiC-compatibiliteit","Gegevens doorgeven zodat de Quatt app kan blijven meekijken.")}
          ${m}
        </div>
        ${w}
      `)}function jm(){if(!Or.some(T=>h(T)))return"";let o=ht("cicPollingEnabled"),r=ht("otEnabled"),n=(T={})=>T.haValueKey||(T.haKeys||[]).find($=>!/valid$/i.test($))||"",a=(T={})=>T.haValidKey||(T.haKeys||[]).find($=>/valid$/i.test($))||"",i=(T="",$="")=>!!T&&!!$&&h(T)&&h($)&&G($),l=(T={})=>i(n(T),a(T)),p={mqttCoolingDewPoint:"cooling_dew_point",mqttOutsideTemperature:"outside_temperature",mqttRoomTemperature:"room_temperature",mqttRoomSetpoint:"room_setpoint",mqttHeatingEnable:"heating_enable",mqttCoolingEnable:"cooling_enable"},u=(T={})=>T.mqttTopicKey||p[T.valueKey]||"",c=(T="")=>{if(!T)return!0;let $=t.mqttStatus?.input_enabled;return $&&typeof $=="object"&&Object.prototype.hasOwnProperty.call($,T)?$[T]!==!1:!0},m=T=>/\bMQTT\b/i.test(String(T||"")),b=(T,$={})=>T==="CIC"?o:T==="OT thermostat"?r:T==="HA input"?l($):T==="CIC or HA input"?o||l($):m(T)?c(u($)):T==="Flowmeter HP2"?h("hp2Flow"):T==="Local aggregate HP1/HP2"?h("flowLocal")||h("hp2Flow"):!0,y=(T,$={})=>T==="CIC"&&!o?"CIC-polling staat uit":T==="OT thermostat"&&!r?"OpenTherm staat uit":T==="HA input"&&!l($)?"HA-bron ongeldig":T==="CIC or HA input"&&!o&&!l($)?"CIC en HA ontbreken":m(T)&&!c(u($))?"MQTT-topic staat uit":T==="Flowmeter HP2"&&!h("hp2Flow")?"HP2-flow ontbreekt":T==="Local aggregate HP1/HP2"&&!h("flowLocal")&&!h("hp2Flow")?"Lokale flow ontbreekt":"",w=(T,$="Actief",P="Normaal")=>h(T)?G(T)?$:P:"",g=(T,$={})=>{let P=String(T||"").trim();return P?$.optionLabels?.[P]||qe(P):""},f=(T,$={})=>{let P=String(E(T)||"").trim();return P?g(P,$):""},v=T=>{let $=or(T,"");return $?qe($):""},S=(...T)=>T.find($=>String($||"").trim())||"",q=()=>{let T=f("waterSupplySource");if(String(E("waterSupplySource")||"")==="Local"&&h("localWaterSupplyTempSource")){let $=f("localWaterSupplyTempSource");return $?`${T} - ${$}`:T}return T},C=()=>{let T=String(E("flowSource")||"").trim();if(T==="Outdoor unit"){if(h("qFlowSource")){let $=String(E("qFlowSource")||"").trim(),P=String(E("hpGeneration")||"").trim();return $==="Local"||$==="Auto"&&P==="V1"?$==="Auto"?"Lokaal (auto)":"Lokaal":S(f("outdoorUnitFlowMode"),$==="Auto"?"Buitenunit (auto)":"Buitenunit")}return S(f("outdoorUnitFlowMode"),"Quatt-flow")}return qe(T)},x=()=>{let T=String(E("outsideTempSource")||"").trim();if(T==="MQTT"&&!c("outside_temperature"))return"MQTT-topic staat uit";if(T!=="Auto")return qe(T);let $=A("outsideTempLocalAggregated"),P=A("outsideTempHa"),B=A("mqttOutsideTemperature"),Q=!Number.isNaN($),J=h("outsideTempHaValid")?G("outsideTempHaValid")&&!Number.isNaN(P):!Number.isNaN(P),X=c("outside_temperature")&&h("mqttOutsideTemperatureValid")&&G("mqttOutsideTemperatureValid")&&!Number.isNaN(B),Le=[Q?{label:"Buitenunit",value:$}:null,J?{label:"HA-invoer",value:P}:null,X?{label:"MQTT",value:B}:null].filter(Boolean);return Le.length?Le.reduce((ae,Fe)=>Fe.value<ae.value?Fe:ae,Le[0]).label:"Auto"},H=T=>{if(!h(T))return NaN;let $=Number(E(T));if(Number.isFinite($))return $;let B=String(t.entities[T]?.state??"").trim().replace(",",".").match(/-?\d+(?:\.\d+)?/);return B?Number(B[0]):NaN},V=(T,$="")=>{if(!h(T))return!1;let P=H(T);return($?G($):!0)&&Number.isFinite(P)},K=()=>{let T=String(E("coolingDewPointSource")||"").trim();if(T==="Home Assistant")return V("coolingDewPointHa","coolingDewPointHaValid")?"HA-invoer":"HA-invoer ontbreekt";if(T==="MQTT")return c("cooling_dew_point")?V("mqttCoolingDewPoint","mqttCoolingDewPointValid")?"MQTT":"MQTT ontbreekt of verouderd":"MQTT-topic staat uit";let $=V("coolingDewPointHa","coolingDewPointHaValid"),P=c("cooling_dew_point")&&V("mqttCoolingDewPoint","mqttCoolingDewPointValid");if($&&P){let B=H("coolingDewPointHa");return H("mqttCoolingDewPoint")>B?"MQTT":"HA-invoer"}return $?"HA-invoer":P?"MQTT":T?qe(T):"Auto"},M=({label:T,value:$="",key:P="",active:B=!1,status:Q="",statusTone:J="",statusTitle:X=""})=>{let Le=$||(P?Te(P):"");if(!Le&&!Q)return"";let ae=String(J||"").replace(/[^a-z0-9_-]/gi,""),Fe=X||Q,Ae=Q?`<em class="oq-settings-source-status${ae?` oq-settings-source-status--${s(ae)}`:""}" title="${s(Fe)}" aria-label="${s(`${Q}: ${Fe}`)}">${s(Q)}</em>`:"";return`
        <div class="oq-settings-source-row${B?" is-warning":""}${Q?" has-status":""}">
          <span>${s(T)}${Ae}</span>
          <strong>${s(Le)}</strong>
        </div>
      `},I=({label:T="HA-invoer",valueKey:$="",validKey:P="",value:B=""})=>{if(!$||!P||!h($)||!h(P))return[];let Q=G(P);return[M({label:T,key:$,value:B,status:Q?"Geldig":"Ongeldig",statusTone:Q?"valid":"invalid",statusTitle:Q?"Home Assistant geeft dit signaal geldig door. OpenQuatt mag deze HA-invoer gebruiken.":"Home Assistant geeft dit signaal niet geldig door. OpenQuatt gebruikt deze HA-invoer dan niet als bron."})]},z=({label:T="MQTT",valueKey:$="",validKey:P="",value:B="",topicKey:Q=""})=>{if(!$||!P||!h($)||!h(P))return[];if(!c(Q||p[$]))return[];let J=G(P),X=J?"MQTT heeft een geldige, recente waarde ontvangen. OpenQuatt mag deze MQTT-invoer gebruiken.":"MQTT heeft nog geen geldige recente waarde ontvangen. OpenQuatt gebruikt deze MQTT-invoer dan niet als bron.";return[M({label:T,key:$,value:J?B:"\u2014",status:rs(P),statusTone:J?"valid":"invalid",statusTitle:X})]},ne=({title:T,icon:$="",content:P="",rows:B=[],copy:Q="",className:J=""})=>{let X=B.filter(Boolean).join("");return!P&&!X&&!Q?"":`
        <section class="oq-settings-source-group${J?` ${s(J)}`:""}">
          <h5>
            ${$?`<span class="oq-settings-source-group-icon">${tt($,"oq-settings-source-group-icon-svg")}</span>`:""}
            <span>${s(T)}</span>
          </h5>
      ...
      `},fe=(T,$={})=>{if(!h(T))return{markup:"",warning:""};let P=t.entities[T]||{},B=String(E(T)||""),J=Rt(P).filter(Ae=>b(Ae,$)),X=B&&!b(B,$),Le=B==="HA input"&&$.keepUnavailableCurrent!==!0,Fe=(X&&!Le&&!J.includes(B)?[B,...J]:J).map(Ae=>{let Wo=g(Ae,$);return`<option value="${s(Ae)}" ${Ae===B?"selected":""}>${s(Wo)}</option>`}).join("");return{markup:`
          <label class="oq-settings-source-select">
            <span class="oq-settings-source-select-head">
              <span>${s($.label||"Bron")}</span>
              ${$.infoCopy?eo($.infoId||T,$.infoTitle||$.label||"Bron",$.infoCopy):""}
            </span>
            <select class="oq-helper-select" data-oq-field="${s(T)}" ${t.loadingEntities?"disabled":""}>
              ${Fe}
            </select>
          </label>
        `,warning:X?`Huidige bron niet beschikbaar: ${y(B,$)}`:""}},Z=({key:T,title:$,icon:P="",select:B,secondarySelect:Q=null,secondarySelects:J=null,activeRows:X=[],measurementRows:Le=[],activeCopy:ae="",rows:Fe=[]})=>{let Ae=B&&B.when!==!1?fe(B.key,B):{markup:"",warning:""},Wt=(Array.isArray(J)?J:Q?[Q]:[]).filter(se=>se&&se.when!==!1).map(se=>fe(se.key,se)).filter(se=>se.markup),Ar=Wt.map(se=>se.markup).join(""),et=Wt.map(se=>se.warning).find(Boolean)||"",qt=Fe.filter(Boolean).join(""),He=`${Ae.markup}${Ar}`,ue=Ae.warning||et,Tt=[ne({title:"Configuratie",icon:"settings",className:"oq-settings-source-group--config",content:He?`
        ...
        <article class="oq-settings-source-card" data-oq-settings-field="${s(T||B.key)}">
    ...
    `}function Qm(){let e=xe.has(t.settingsGroup)?t.settingsGroup:de[0].id;return`
    ...
    `}function Sw(){if(!t.root||t.appView!=="settings")return!1;let e=t.root.querySelector(".oq-settings-group-nav"),o=t.root.querySelector(".oq-settings-group-stack");if(!e||!o)return!1;let r=xe.has(t.settingsGroup)?t.settingsGroup:de[0].id;if(r==="service")return!1;let n=e.querySelectorAll(".oq-settings-group-button");if(n.length!==de.length)return!1;n.forEach(g=>{let v=String(g.dataset.groupId||"")===r;g.classList.toggle("is-active",v),g.setAttribute("aria-pressed",v?"true":"false")}),o.querySelectorAll(".oq-settings-info").forEach(g=>{let f=String(g.dataset.oqSettingsInfo||""),v=t.settingsInfoOpen===f;g.classList.toggle("is-open",v);let S=g.querySelector(".oq-settings-info-popover");S&&(S.hidden=!v);let q=g.querySelector(".oq-settings-info-button");q&&q.setAttribute("aria-expanded",v?"true":"false")}),o.querySelectorAll("[data-oq-settings-field]").forEach(g=>{let f=String(g.dataset.oqSettingsField||"");if(!f)return;let v=g.querySelector(".oq-settings-static-value");if(v){let C=O(f);v.textContent!==C&&(v.textContent=C)}g.querySelectorAll("select[data-oq-field]").forEach(C=>{let x=String(C.dataset.oqField||f),H=String(E(x)||"");C.value!==H&&(C.value=H)}),g.querySelectorAll("input[data-oq-field]").forEach(C=>{let x=String(C.dataset.oqField||f),H=String(we(x)||"");C.value!==H&&(C.value=H)});let S=g.querySelector(".oq-helper-slider-meta strong"),q=g.querySelector('input[type="range"][data-oq-field]');if(S&&q){let C=j(f,ge(f,E(f)));S.textContent!==C&&(S.textContent=C)}}),o.querySelectorAll("[data-select-key]").forEach(g=>{let f=String(g.dataset.selectKey||""),v=String(g.dataset.selectOption||""),S=String(E(f)||""),q=v===S;g.classList.toggle("is-active",q),g.setAttribute("aria-pressed",q?"true":"false"),f==="strategy"?g.disabled=t.loadingEntities||t.busyAction==="save-strategy":f==="hpGeneration"?g.disabled=t.loadingEntities||t.busyAction==="save-hpGeneration":f==="curveControlProfile"?g.disabled=t.loadingEntities||t.busyAction==="save-curveControlProfile":f==="phResponseProfile"&&(g.disabled=t.loadingEntities||t.busyAction==="save-phResponseProfile");let C=g.closest(".oq-settings-choice-card-shell");C&&C.classList.toggle("is-active",q)});let a=o.querySelector(".oq-settings-choice-card--static.oq-settings-choice-card--custom");if(a){let g=String(E("phResponseProfile")||"")==="Custom";a.classList.toggle("is-active",g),a.querySelectorAll("input[data-oq-field]").forEach(v=>{let S=String(v.dataset.oqField||""),q=String(we(S)||"");v.value!==q&&(v.value=q)})}o.querySelectorAll('[data-oq-action="toggle-overview-control"][data-control-key]').forEach(g=>{let f=String(g.dataset.controlKey||""),v=!!E(f),S=String(g.dataset.onLabel||"Aan"),q=String(g.dataset.offLabel||"Uit"),C=String(g.dataset.switchTitle||f),x=v?S:q;g.dataset.controlState=v?"off":"on",g.classList.toggle("is-on",v),g.setAttribute("aria-checked",v?"true":"false"),g.setAttribute("aria-label",`${C}: ${x}`),g.disabled=t.loadingEntities||t.busyAction===`switch-${f}`}),o.querySelectorAll("[data-oq-switch-pill]").forEach(g=>{let f=String(g.dataset.oqSwitchPill||""),v=!!E(f),S=String(g.dataset.onLabel||"Aan"),q=String(g.dataset.offLabel||"Uit"),C=v?S:q;g.classList.toggle("is-on",v),g.textContent!==C&&(g.textContent=C)}),o.querySelectorAll("[data-oq-switch-copy]").forEach(g=>{let f=String(g.dataset.oqSwitchCopy||""),v=!!E(f),S=String(g.dataset.onCopy||""),q=String(g.dataset.offCopy||""),C=v?S:q;g.hidden=!C,g.textContent!==C&&(g.textContent=C)});let i=o.querySelector('button[data-oq-action="open-generation-modal"]')?.closest(".oq-settings-quickstart-status");if(i){let g=i.querySelector(".oq-settings-quickstart-status-value"),f=i.querySelector(".oq-settings-quickstart-status-copy"),v=i.querySelector('button[data-oq-action="open-generation-modal"]'),S=it(),q=t.entities.hpGeneration||{},C=h("hpGeneration")&&Rt(q).length>0;if(g){let x=S||"Onbekend";g.textContent!==x&&(g.textContent=x)}if(f){let x="Pas dit aan als je een andere Quatt Hybrid hebt.";f.textContent!==x&&(f.textContent=x)}v&&(v.disabled=!C||t.loadingEntities||t.busyAction==="save-hpGeneration")}let l=o.querySelector('button[data-oq-action="open-cm100-commissioning-modal"]')?.closest(".oq-settings-quickstart-status");if(l){let g=l.querySelector(".oq-settings-quickstart-status-value"),f=l.querySelector(".oq-settings-quickstart-status-copy"),v=l.querySelector('button[data-oq-action="open-cm100-commissioning-modal"]'),S=pa(),q=k("cm100Active");g&&g.textContent!==S&&(g.textContent=S);let C=q?"CM100 is actief en klaar voor commissioning.":"Open de modal om CM100 te starten en de taken hieronder te ontgrendelen.";f&&f.textContent!==C&&(f.textContent=C),v&&(v.disabled=t.loadingEntities)}let p=o.querySelector('button[data-oq-action="reset"]')?.closest(".oq-settings-quickstart-status");if(p){let g=p.querySelector(".oq-settings-quickstart-status-value"),f=p.querySelector(".oq-settings-quickstart-status-copy"),v=p.querySelector('button[data-oq-action="reset"]'),S=t.complete===!0?"Afgerond":t.complete===!1?"Open":"Laden...",q=t.complete===!0?"Quick Start is afgerond. Je kunt de status hier altijd weer openen met een reset.":t.complete===!1?"Quick Start staat nog open. Gebruik de resetknop om opnieuw te beginnen.":"De status van Quick Start wordt nog geladen.";g&&g.textContent!==S&&(g.textContent=S),f&&f.textContent!==q&&(f.textContent=q),v&&(v.disabled=t.busyAction==="reset")}let u=o.querySelectorAll("[data-oq-access-security-item]");u.length&&u.forEach(g=>{let f=String(g.dataset.oqAccessSecurityItem||""),v=g.querySelector(".oq-settings-quickstart-status-value"),S=g.querySelector(".oq-settings-quickstart-status-copy"),q=g.querySelector("button[data-oq-action]");if(f==="login"){let C=dn(),x=mn();v&&v.textContent!==C&&(v.textContent=C),S&&S.textContent!==x&&(S.textContent=x)}else if(f==="api"){let C=pr(),x=dr();v&&v.textContent!==C&&(v.textContent=C),S&&S.textContent!==x&&(S.textContent=x)}q&&(q.disabled=!1)});let c=o.querySelector(".oq-settings-system-summary");if(c){let g=c.querySelectorAll(".oq-settings-system-row"),f={uptime:er(),ip:tr(),updates:ro(),datetime:ua(),espTemp:Ka(),restart:"Opnieuw opstarten"};g.forEach(q=>{let C=q.querySelector(".oq-settings-system-row-value"),x=q.dataset.oqDiagnosticsRow||"";if(C&&Object.prototype.hasOwnProperty.call(f,x)){let H=f[x];C.textContent!==H&&(C.textContent=H)}});let v=c.querySelector('button[data-oq-action="open-update-modal"]');v&&(v.disabled=!1);let S=c.querySelector('button[data-oq-action="open-restart-confirm"]');if(S){let q=t.busyAction==="restartAction";S.disabled=q,S.textContent=q?"Herstarten...":"Herstarten"}}o.querySelectorAll(".oq-settings-hp-offset-row").forEach(g=>{let f=String(g.dataset.oqSettingsField||""),v=String(g.dataset.oqHpOffsetRawKey||""),S=String(g.dataset.oqHpOffsetFinalKey||"");if(!f||!v||!S)return;let q=Me(f),C=gn(v,S,f),x=Ee(we(f)),H=Number.isFinite(C)&&Number.isFinite(x)?Mt(C+x,q.uom||"\xB0C",2):be(S,2),V=g.querySelector("[data-oq-hp-offset-active]");if(V){let I=`${be(S,2)} actief`;V.textContent!==I&&(V.textContent=I)}let K=g.querySelector("[data-oq-hp-offset-raw]");if(K){let I=Number.isFinite(C)?Mt(C,q.uom||"\xB0C",2):be(v,2);K.textContent!==I&&(K.textContent=I)}let M=g.querySelector("[data-oq-hp-offset-final]");M&&M.textContent!==H&&(M.textContent=H)});let m=o.querySelector(".oq-settings-curve-shell"),b=oe();if(!!m!==b)return!1;let y=!!a,w=String(E("phResponseProfile")||"")==="Custom";return y===w}vu({patch:Sw});function qw({label:e,value:o,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(o)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function $n(e,o=!1){return e.map(r=>qw({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?Ue(r.key):r.value,status:o})).join("")}function Al(e){return`
      <div class="oq-overview-sectionhead">
        <h3>${s(e)}</h3>
      </div>
    `}function Hl(){let e=At();return`
    ...
    `}function Rl(e,o,r,n){return`${r?Ew(n):""}${Tw(e,o)}`}function eg(e,o,r,n,a){if(!e)return;let i=Se({mode:o,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&(wt(e,Rl(o,r,n,a)),e.dataset.renderSignature=i)}function Qa(e,o,r=""){return`
    ...
    `}function kw(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}function xw(e){if(!Number.isFinite(e)||e<0)return"\u2014";let o=Math.floor(e),r=Math.floor(o/1440),n=Math.floor(o%1440/60),a=o%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function $w(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function Aw(e){let o=$w(O("timeNowHhmm",""));if(!Number.isFinite(o))return"";let r=Math.round(e),n=((o-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Hw(e,o){let r=Math.max(0,(Number(o)-Number(e))/6e4),n=xw(r),a=h("timeValid")&&k("timeValid")?Aw(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Mw(e){let o=Number(e);return Number.isNaN(o)?"\u2014":`${o>0?"+":o<0?"-":""}${Math.abs(o).toFixed(0)} W`}function tg(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>h(e))||""}function Rw(){return["hp1WaterIn","hp2WaterIn"].find(e=>h(e))||""}function og(e=O("controlModeLabel","")){let o=String(e||"").toLowerCase();return o.includes("cm5")||o.includes("cooling")||o.includes("koeling")}function uo(){return og()}function rg(e,o){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:o?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function Pl(){return uo()?"Koeling":oe()?"Stooklijn":"Power House"}function Pw(){let e=["phouseReq","strategyRequestedPower"];for(let o of e){let r=A(o);if(!Number.isNaN(r))return r}return Number.NaN}function ng(){let e=Pw(),o=A("phouseHouse"),r=A("totalHeat"),n=A("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(o)?Number.NaN:e-o,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:_(e,0,"W"),houseText:_(o,0,"W"),correctionText:Mw(a),capacityText:Ue("hpCapacity"),statusTitle:i,statusCopy:l}}function ag(){let e=A("curveSupplyTarget"),o=A("supplyTemp"),r=tg(),n=r?A(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(o)?Number.NaN:o-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",p="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",p="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",p="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",p="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",p="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:Ue("curveSupplyTarget"),supplyText:Ue("supplyTemp"),deltaText:kw(a),capacityText:Ue("hpCapacity"),statusTitle:l,statusCopy:p}}function ig(){let e=A("supplyTemp"),o=O("coolingGuardMode",""),r=O("coolingFallbackNightMinOutdoorTemp","\u2014"),n=A("coolingSupplyError"),a=A("coolingDemandRaw"),i=k("coolingPermitted"),l=k("coolingRequestActive"),p=O("coolingBlockReason","Onbekend"),u=Cl(p),c=rg(p,l),m="Wacht op koelvraag",b="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return c?(m="Wacht op koelvraag",b="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(m="Houdt doel vast",b="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(m="Trekt aanvoer omlaag",b="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(m="Benadert koeldoel",b="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(m="Koelt rustig door",b="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(m="Koeling gereed",b="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(m="Koeling geblokkeerd",b=`Blokkade: ${u}.`),{targetText:Ue("coolingSupplyTarget"),supplyText:Ue("supplyTemp"),safeFloorText:Ue("coolingEffectiveMinSupplyTemp"),guardMode:o,fallbackNightMin:r,demandText:Ue("coolingDemandRaw"),statusTitle:m,statusCopy:b,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:c}}function Ya(){if(uo()){let o=ig(),r=o.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:o.targetText,focusCopy:o.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:o.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:o.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${o.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:o.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(oe()){let o=ag();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:o.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:o.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:o.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:o.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=ng();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function Nl(e){return Xm({className:"oq-overview-system",title:e.title,copy:e.copy,signature:Se(e),body:`
      ...
      `})}function Nw(){if(!k("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(uo()){let n=ig(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(Od())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let o=(oe()?ag():ng()).statusTitle;return{label:"Regeling nu",value:o,tone:o==="In balans"||o==="Dicht bij doel"?"green":o==="Nog aan het opbouwen"||o==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Ow(){return k("openquattEnabled")?uo()?k("coolingPermitted")?k("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:O("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:k("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:k("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Ol(e,o){let r=Nw(),n=Ow();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:o,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Dl(e,o){let r=Ol(e,o);return`
    ...
    `}function Ll(){let e=uo();return[{key:"totalPower",label:"Elektrisch vermogen",tone:"blue",note:"hele systeem"},{key:e?"totalCoolingPower":"totalHeat",label:e?"Koelvermogen":"Verwarmingsvermogen",tone:"orange",note:"thermisch vermogen"},{key:e?"totalEer":"totalCop",label:e?"COP (EER)":"COP",tone:"green",note:"rendement"},{key:"flowSelected",label:"Flow",tone:"sky",note:"watercircuit"}]}function Dw(){let e=k("openquattEnabled"),o=E("openquattResumeAt"),r=Wr(o),n=(t.loadingEntities||t.entitySyncInFlight)&&!h("openquattResumeAt"),a=k("manualCoolingEnable"),i=h("coolingEnableSelected")?k("coolingEnableSelected"):a,l=Ym(O("coolingEnableEffectiveSource","")),p=String(E("coolingEnableSource")||"").trim(),u=Ym(E("coolingEnableSource")),c=String(E("silentModeOverride")||"Schedule"),m=!k("coolingPermitted"),b=k("coolingRequestActive"),y=og(),w=O("coolingBlockReason",""),g=rg(w,b),f="Uit",v=p==="Disabled"?"Koeling is niet toegestaan: handmatig staat uit.":u&&u!=="geen bron"?`Koeling is niet toegestaan: ${u} geeft geen toestemming en handmatig staat uit.`:"Koeling is niet toegestaan.";i&&y?(f="Actief",v=kn("Koeling draait nu.",l)):i&&g?(f="Aan",v=kn("Koeling is toegestaan en wacht op kamertemperatuur boven het koel-setpoint.",l)):i&&m?(f="Geblokkeerd",v=kn(Cl(w||"Koeling wacht nog op veilige condities."),l)):i&&b?(f="Start bijna",v=kn("Er is koelvraag. Koeling start zodra dat kan.",l)):i&&(f="Aan",v=kn("Koeling is toegestaan en wacht op koelvraag.",l));let S="Uit",q="Stille modus staat uit.",C="neutral";return c==="On"?(S="Aan",q="Stille modus staat geforceerd aan, ook buiten het tijdvenster.",C="orange"):c==="Schedule"&&(S="Schema",k("silentActive")?(q="Stille modus staat nu aan via het tijdvenster.",C="violet"):q="Stille modus volgt het tijdvenster."),[{key:"openquattEnabled",label:"Openquatt regeling",status:e?"Actief":"Tijdelijk uit",copy:e?"Verwarmen en koelen worden automatisch geregeld.":r?"Verwarming en koeling zijn tijdelijk uitgeschakeld. Beveiligingen blijven actief.":"Verwarming en koeling zijn uitgeschakeld. Beveiligingen blijven actief.",tone:e?"green":"orange",kind:"openquatt-control",meta:e?[]:[n?{label:"Hervatten",value:"Laden\u2026",tone:"neutral",loading:!0}:{label:r?"Hervat automatisch":"Hervatten",value:r?Qo(o,!0):"Handmatig",tone:r?"orange":"neutral"}]},{key:"manualCoolingEnable",label:"Koeling",status:f,copy:v,buttonLabel:a?"Handmatig uit":"Handmatig aan",nextState:a?"off":"on",tone:i?y?"blue":"sky":"neutral"},{key:"silentModeOverride",label:"Stille modus",status:S,copy:q,tone:C,kind:"select",selectedOption:c,settingsAction:!0,options:[{value:"Off",label:"Uit"},{value:"On",label:"Aan"},{value:"Schedule",label:"Schema"}]}].filter(x=>h(x.key))}function Lw(e=[]){return e.length?`
    ...
    `}function Jw(){let e=_t(),o=Vs();return`
    ...
    `}function dg(){let e=rt(),r=cg().length>0;return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(t.overviewTheme)}">
    ...
    `}function eS(){if(!t.root||t.appView!=="diagnosis")return!1;let e=t.root.querySelector(".oq-overview-board");if(!e)return!1;let o=e.querySelector(".oq-overview-trends");return o?(lt(o,Ja(),Xa()),Bl(e),An(e),!0):!1}function Kl(e){if(!e)return null;let o=e.__oqTrendHoverNodes;if(o&&o.chart&&o.hoverLayer&&o.hoverPanel)return o;let r=e.querySelector("[data-oq-trend-hover-layer]"),n={chart:e.querySelector(".oq-overview-trend-chart"),hoverLayer:r,hoverPanel:e.querySelector("[data-oq-trend-hover]"),hoverTime:e.querySelector("[data-oq-trend-hover-time]"),hoverNote:e.querySelector("[data-oq-trend-hover-note]"),hoverValues:e.querySelector("[data-oq-trend-hover-values]"),hoverLine:r?r.querySelector(".oq-overview-trend-hover-line"):null,hoverDots:{}};return r&&r.querySelectorAll("[data-oq-trend-hover-dot]").forEach(a=>{n.hoverDots[a.getAttribute("data-oq-trend-hover-dot")]=a}),e.__oqTrendHoverNodes=n,n}function Jm(e,o,r,n=Kl(e)){if(!e||!o||!Array.isArray(o.points)||o.points.length===0)return;let a=Math.max(0,Math.min(o.points.length-1,r)),i=o.points[a];if(!i||!n||!n.chart||!n.hoverLayer||!n.hoverPanel||!n.hoverTime||!n.hoverNote||!n.hoverValues)return;let l=String(a);if(!n.hoverPanel.hidden&&e.dataset.oqTrendHoverIndex===l)return;let p=Hw(i.sample.t,o.endTime);n.hoverPanel.hidden=!1,n.hoverLayer.removeAttribute("hidden"),n.hoverTime.textContent=p.value,n.hoverNote.textContent=p.note,n.hoverLine&&(n.hoverLine.setAttribute("x1",i.x.toFixed(1)),n.hoverLine.setAttribute("x2",i.x.toFixed(1)));let u=[];o.series.forEach(c=>{let m=xn(c,i.sample),b=c.id||c.sampleKey||c.label,y=n.hoverDots[b];if(!Number.isFinite(m)){y&&y.setAttribute("display","none");return}let w=i.values.find(g=>g.seriesId===b);y&&w&&(y.removeAttribute("display"),y.setAttribute("cx",w.x.toFixed(1)),y.setAttribute("cy",w.y.toFixed(1))),u.push(`
        <div class="oq-overview-trend-hover-row oq-overview-trend-hover-row--${s(c.tone)}">
          <span>${s(c.label)}</span>
          <strong>${s(_(m,c.decimals,c.unit))}</strong>
        </div>
      `)}),n.hoverValues.innerHTML=u.join(""),e.dataset.oqTrendHoverIndex=l}function tS(e){if(!e)return;let o=Kl(e);o?.hoverPanel&&(o.hoverPanel.hidden=!0),o?.hoverLayer&&o.hoverLayer.setAttribute("hidden",""),delete e.dataset.oqTrendHoverIndex}function An(e=t.root){if(!e)return;e.querySelectorAll("[data-oq-trend-card]").forEach(r=>{let n=r.dataset.renderSignature||"";if(r.__oqTrendBoundSignature===n)return;typeof r.__oqTrendCleanup=="function"&&r.__oqTrendCleanup(),r.__oqTrendHoverNodes=null,r.__oqTrendBoundSignature=n;let a=r.querySelector(".oq-overview-trend-chart");if(!a)return;let i=Uw(r.dataset.oqTrendCard);if(!i)return;let l=ug(i.samples,i.series,{mockData:i.mock});r.__oqTrendModel=l;let p=Kl(r),u=0,c=null,m=()=>{let w=c;c=null,u=0;let g=a.getBoundingClientRect();if(!g.width||!g.height)return;let f=Number(w?.clientX);if(!Number.isFinite(f)){Jm(r,l,l.points.length-1,p);return}let S=Math.min(g.width,Math.max(0,f-g.left))/g.width*l.width,q=Gw(l,S);Jm(r,l,q,p)},b=w=>{c=w,u||(u=window.requestAnimationFrame(m))},y=()=>{u&&(window.cancelAnimationFrame(u),u=0),c=null,tS(r)};a.addEventListener("pointermove",b),a.addEventListener("pointerenter",b),a.addEventListener("pointerleave",y),a.addEventListener("focus",b),a.addEventListener("blur",y),a.addEventListener("touchstart",b,{passive:!0}),r.__oqTrendCleanup=()=>{u&&(window.cancelAnimationFrame(u),u=0),c=null,a.removeEventListener("pointermove",b),a.removeEventListener("pointerenter",b),a.removeEventListener("pointerleave",y),a.removeEventListener("focus",b),a.removeEventListener("blur",y),a.removeEventListener("touchstart",b)}})}Jo({patchDiagnosisDom:eS});function Zl(e,o,r){let n=fg(O(o.mode,"Unknown")),a=k(o.defrost),i=Yt(O(o.failures,"None")),l=aa(i),p=n==="Verwarmen"||n==="Koelen"||a;return{mode:n,defrostActive:a,failures:i,warningFailures:l,running:p,thermalKey:n==="Koelen"?o.cooling:o.heat,schematic:pS(e,o,r,n,a,l,p)}}function gg(e,o=null){return`<h3>${s(e)}</h3>${o?`<button class="oq-overview-hp-card-action" type="button" data-oq-action="select-hp-layout" data-hp-layout="${s(o.layout)}">${bS(o.layout==="equal"?"minus":"plus")}<span>${s(o.label)}</span></button>`:""}`}function Ul(e,o,r,n){return`<div class="oq-overview-hp-status">${Rl(e,o,r,n)}</div>`}function oS(e,o){return o?"ontdooit":e==="Verwarmen"?"verwarmt":e==="Koelen"?"koelt":e==="Stand-by"?"stand-by":"onbekend"}function hg(e){return!Array.isArray(e)||e.length===0?"":`<p class="oq-overview-hp-summary">${s(e.map(o=>`${o.title} ${oS(fg(O(o.keys.mode,"Unknown")),k(o.keys.defrost))}`).join(", "))}</p>`}function rS(e){let o=t.entities[e];if(!o)return"Positie: \xE2\u20AC\u201D";let r=A(e);return Number.isNaN(r)?`Positie: ${O(e)}`:`Positie: ${_(r,0,o.uom||"")}`}function nS(e){return h(e)?`Positie: ${k(e)?"Koelen/Defrost":"Verwarmen"}`:"Positie: \xE2\u20AC\u201D"}function fg(e){let o=String(e||"").trim();return!o||o==="Unknown"?"Onbekend":o==="Standby"?"Stand-by":o==="Heating"?"Verwarmen":o==="Cooling"?"Koelen":o}function Gl(e,o,r,n=!0,a="default"){return`
    ...
    `}function Hn({bind:e,modifier:o,x:r,y:n,width:a,kicker:i,detail:l,detailBind:p="",icon:u="heater",direction:c="down"}){let b=r+26,y=n+22,w=p?` data-oq-bind="${s(p)}"`:"",g="";if(c==="up"){let f=r+Math.round(a*.52);g=`M${f-6} ${n} L${f} ${n-8} L${f+6} ${n} Z`}else if(c==="left"){let f=n+Math.round(22);g=`M${r} ${f-6} L${r-8} ${f} L${r} ${f+6} Z`}else if(c==="right"){let f=n+Math.round(22);g=`M${r+a} ${f-6} L${r+a+8} ${f} L${r+a} ${f+6} Z`}else{let f=r+Math.round(a*.52);g=`M${f-6} ${n+44} L${f} ${n+44+8} L${f+6} ${n+44} Z`}return`
      <g
    ...
    `}function iS({tooltip:e,...o}){return`${zl(o)}${Hn({bind:o.bind,...e})}`}function sS({bind:e,ariaLabel:o,x:r,y:n,width:a,height:i,rx:l,tooltip:p}){return`
    ...
    `}function bg(e,o,r,n="normal",a=null){if(!h(o.power))return"";let i=Zl(e,o,r),{mode:l,defrostActive:p,running:u,thermalKey:c}=i,m=i.schematic;return t.hpVisualMode==="schematic"?`
      ...
      <section class="oq-overview-hp oq-overview-hp--${s(r)} oq-overview-hp--${s(n)}" data-oq-hp-panel="${s(e)}">
        ...
        <div class="oq-overview-hp-stats">
          ${$n([{key:o.power,label:"Stroomverbruik",tone:"blue",note:"elektrisch verbruik"},{key:c,label:m.heatLabel,tone:"orange",note:m.heatDescription},{label:m.efficiencyLabel,value:m.efficiencyText,tone:"green",note:"actueel"}])}
        </div>
    ...
    `}function Ql(){if(!Jl())return"";let e=ti();return t.hpVisualMode!=="schematic"?fS(e):`
          ...
          <div class="oq-boiler-card-main">
            ...
            <div class="oq-boiler-mini-schematic">
              <svg class="oq-boiler-mini-svg" viewBox="0 0 420 132" role="img" aria-label="Schematische weergave CV-ketel of boiler">
                ...
                ${Hn({bind:"boiler-return",modifier:"return",icon:"temperature",x:82,y:70,width:124,kicker:"Temperatuur",detail:"Retour",direction:"left"})}
                ...
                ${Hn({bind:"boiler-supply",modifier:"supply",icon:"temperature",x:294,y:14,width:124,kicker:"Temperatuur",detail:"Aanvoer",direction:"right"})}
              </svg>
    ...
    `}function yS(e,o){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(wt(e,wg(o)),!0):(wt(r,`
      <h3>Warmtepompen</h3>
      ${hg(o)}
      ...
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(t.overviewTheme)}">
          ...
          <div class="oq-overview-hp-grid oq-overview-hp-grid--${s(n)}">
            ${o.map((i,l)=>bg(i.title,i.keys,i.accent,ei(l,o,r),Yl(l,o,r))).join("")}
            ${Ql()}
          </div>
        </div>
      </section>
    `}function vS(e,o,r){if(!e)return;let n=e.querySelector(o);n&&n.textContent!==r&&(n.textContent=r)}function xr(e,o,r){e&&e.getAttribute(o)!==r&&e.setAttribute(o,r)}function wS(e,o){o.forEach(([r,n])=>{vS(e,`[data-oq-bind="${r}"]`,n)})}function SS(e,o){o.forEach(([r,n])=>{xr(e.querySelector(`[data-oq-bind="${r}"]`),"aria-label",n)})}function qS(e,o,r,n=""){let a=e.querySelector(`[data-oq-bind="${o}"]`);a&&(a.classList.toggle("is-active",r),xr(a,"tabindex",r?"0":"-1"),!r&&n&&oi(e.querySelector(`[data-oq-bind="${n}"]`)))}function TS(e,o,r){xr(e.querySelector(`[data-oq-bind="${o}"]`),"fill",r)}function Mn(e,o,r,n){if(!e)return;let a=`${o}${r}`;n.map(l=>`${o}${l}`).find(l=>e.classList.contains(l))!==a&&(n.forEach(l=>e.classList.remove(`${o}${l}`)),e.classList.add(a))}function ES(e,o,r,n){let a=e.querySelector(`[data-oq-pipe="${o}"]`);a&&(Mn(a,"oq-hp-tech-pipe--",r,["supply","return","hotgas","liquid","expansion","suction"]),a.querySelectorAll("path").forEach(i=>{i.getAttribute("d")!==n&&i.setAttribute("d",n)}))}function oi(e){e&&(e.classList.remove("is-active"),e.setAttribute("aria-hidden","true"))}function mg(e,o,r){!e||!o||!r||(e.querySelectorAll(".oq-hp-tech-tooltip.is-active").forEach(n=>{n!==r&&oi(n)}),o.appendChild(r),r.classList.add("is-active"),r.setAttribute("aria-hidden","false"))}function CS(e,o,r,n){if(!e||!o||!r||!n||r.dataset.oqTooltipWired==="true")return;r.dataset.oqTooltipWired="true";let a=()=>{r.matches(":hover")||document.activeElement===r||oi(n)};r.addEventListener("mouseenter",()=>mg(e,o,n)),r.addEventListener("mouseleave",a),r.addEventListener("focus",()=>mg(e,o,n)),r.addEventListener("blur",a)}function qg(e){if(!e)return;let o=e.querySelector(".oq-hp-tech-svg");if(!o)return;let r=o.querySelector(".oq-hp-tech-tooltip-layer");r||(r=document.createElementNS("http://www.w3.org/2000/svg","g"),r.setAttribute("class","oq-hp-tech-tooltip-layer"),o.appendChild(r)),Array.from(o.querySelectorAll(".oq-hp-tech-tooltip")).forEach(n=>{r.appendChild(n)}),e.querySelectorAll("[data-oq-tooltip-target]").forEach(n=>{let a=n.getAttribute("data-oq-tooltip-target");if(!a)return;let i=e.querySelector(`[data-oq-bind="${a}-tooltip"]`);CS(e,r,n,i)})}function Tg(e=t.root){e&&e.querySelectorAll("[data-oq-hp-board]").forEach(o=>{qg(o)})}function kS(e,o,r,n,a=null,i=null){if(!e)return;let l=i||Zl(o,r,n),{mode:p,running:u}=l,c=l.schematic,m=e.querySelector(".oq-overview-hp-head-title");m&&wt(m,gg(o,a));let b=e.querySelector(".oq-overview-hp-head-side");if(b){let v=b.querySelector(".oq-overview-hp-status");v||(wt(b,Ul(p,u,c.warningActive,c.failureText)),v=b.querySelector(".oq-overview-hp-status")),eg(v,p,u,c.warningActive,c.failureText)}let y=e.querySelector("[data-oq-hp-board]");if(!y)return;y.className!==c.boardClass&&(y.className=c.boardClass),wS(y,[["status",c.statusText],["left-exchanger-title",c.leftExchangerTitle],["right-exchanger-title",c.rightExchangerTitle],["compressor-freq",c.compressorFreqText],["flow-value",c.flowText],["inner-coil-temp-value",c.innerCoilTempText],["evaporator-temp-value",c.evaporatorCoilTempText],["outside-temp-value",c.outsideTempText],["discharge-pressure-value",c.dischargePressureText],["discharge-temp-value",c.dischargeTempText],["suction-pressure-value",c.suctionPressureText],["suction-temp-value",c.suctionTempText],["supply-value",c.waterOutText],["return-value",c.waterInText],["footer-mode",c.mode],["footer-power",c.powerText],["footer-heat",c.heatText],["footer-efficiency-label",c.efficiencyLabel],["footer-efficiency",c.efficiencyText],["fan-speed-value",c.fanRpmText],["fourway-detail",c.fourWayPositionText],["eev-detail",c.eevPositionText]]);let w=y.querySelector('[data-oq-bind="footer-heat-label"]');if(w){xr(w,"aria-label",c.heatLabel);let v=c.heatLabel==="Koelafgifte"?"Koel<br>afgifte":"Warmte<br>afgifte";w.innerHTML!==v&&(w.innerHTML=v)}[["bottom-heater",c.bottomPlateActive],["crankcase-heater",c.crankcaseActive]].forEach(([v,S])=>{qS(y,v,S,`${v}-tooltip`)});let g=y.querySelector('[data-oq-bind="defrost-badge"]');g&&(xr(g,"tabindex",c.defrostActive?"0":"-1"),xr(g,"aria-label",c.defrostActive?"Defrost actief":"Defrost uit"),c.defrostActive||oi(y.querySelector('[data-oq-bind="defrost-badge-tooltip"]'))),[["supply-tooltip",c.supplyLineTone],["return-tooltip",c.returnLineTone]].forEach(([v,S])=>{Mn(y.querySelector(`[data-oq-bind="${v}"]`),"oq-hp-tech-tooltip--",S,["warm","supply","return"])}),SS(y,[["supply-reading",`Aanvoer temperatuur ${c.waterOutText}`],["flow-reading",`Flow ${c.flowText}`],["inner-coil-temp-reading",`Inner coil temperatuur ${c.innerCoilTempText}`],["evaporator-temp-reading",`Verdampertemperatuur ${c.evaporatorCoilTempText}`],["outside-temp-reading",`Buitentemperatuur ${c.outsideTempText}`],["compressor-freq-trigger",`Compressorfrequentie ${c.compressorFreqText}`],["fan-speed-reading",`Ventilatorsnelheid ${c.fanRpmText}`],["discharge-pressure-reading",`Persdruk ${c.dischargePressureText}`],["discharge-temp-reading",`Perstemperatuur ${c.dischargeTempText}`],["return-reading",`Retour temperatuur ${c.waterInText}`],["suction-pressure-reading",`Zuigdruk ${c.suctionPressureText}`],["suction-temp-reading",`Zuigtemperatuur ${c.suctionTempText}`],["fourway-trigger",`4-wegklep, ${c.fourWayPositionText}`],["eev-trigger",`Expansieventiel, ${c.eevPositionText}`]]),Mn(y.querySelector(".oq-hp-tech-pump"),"oq-hp-tech-pump--",c.returnLineTone,["supply","return"]);let f=String(c.title||"hp").toLowerCase().replace(/[^a-z0-9]+/g,"-");TS(y,"cond-water",`url(#${c.reverseCycle?`${f}-cond-water-cool`:`${f}-cond-water-heat`})`),Object.entries(c.pipes).forEach(([v,S])=>{ES(y,v.replace(/[A-Z]/g,q=>`-${q.toLowerCase()}`),S.tone,S.d)}),qg(y),oa()}function xS(){if(!t.root||t.appView!=="overview")return!1;let e=t.root.querySelector(".oq-overview-board");if(!e)return!1;let o=`oq-overview-board oq-overview-board--${t.overviewTheme}`;e.className!==o&&(e.className=o);let r=Pl(),n=e.querySelector("[data-oq-monitoring-notice]"),a=e.querySelector(".oq-overview-summary-shell"),i=e.querySelector(".oq-overview-system"),l=e.querySelector(".oq-overview-temps"),p=e.querySelector(".oq-overview-trends"),u=e.querySelector(".oq-overview-hp-tools"),c=e.querySelector(".oq-overview-hp-grid"),m=e.querySelector("[data-oq-boiler-panel]"),b=yg();if(n){let q=At();lt(n,Se(q),Hl())}if(a){let q=a.querySelector(".oq-overview-top");q&&wt(q,$n(Ll()));let C=a.querySelector(".oq-overview-statuspanel");if(C){let H=O("controlModeLabel");lt(C,Se(Ol(r,H)),Dl(r,H))}let x=a.querySelector(".oq-overview-summary-side");if(x){let H=ta();x.dataset.renderSignature!==H&&(wt(x,Fl()),x.dataset.renderSignature=H)}}if(i&&lt(i,Se(Ya()),Nl(Ya())),l){let q=Za();lt(l,Il(q),_l())}if(p&&t.appView==="overview"&&(lt(p,Ja(),Xa()),Bl(e)),An(e),!u||!c)return!1;let y=ec(b),w=vg(b);if(yS(u,b),Mn(c,"oq-overview-hp-grid--",w,["single","equal","focus-hp1","focus-hp2"]),t.hpVisualMode!=="schematic"){let q=[...b.map((x,H)=>bg(x.title,x.keys,x.accent,ei(H,b,y),Yl(H,b,y))),Ql()].join(""),C=Se({visualMode:t.hpVisualMode,layout:w,markup:q});return c.dataset.renderSignature!==C&&(wt(c,q),c.dataset.renderSignature=C),!0}let g=Jl()?ti():null,f=g?Ql():"",v=g?Xl(g):"";return!!m!=!!f||(m&&m.dataset.renderSignature!==v?m.outerHTML=f:m&&g&&hS(m,g),c.querySelectorAll("[data-oq-hp-panel]").length!==b.length)?!1:(b.forEach((q,C)=>{let x=e.querySelector(`[data-oq-hp-panel="${q.title}"]`);if(x){let H=Zl(q.title,q.keys,q.accent);Mn(x,"oq-overview-hp--",ei(C,b,y),["normal","focus","muted"]),kS(x,q.title,q.keys,q.accent,Yl(C,b,y),H)}}),!0)}Jo({patchOverviewDom:xS});function $S(){return`
      <section class="oq-helper-panel">
