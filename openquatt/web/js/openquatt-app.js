...
`)),r.toString()}function rl(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=My(l);if(!c)return;let u=[...a,c],d=Yd(u,t);if(a.length&&d.length>wy){r.push({keys:n,body:i}),n=[l],a=[c],i=Yd(a,t);return}n=[...n,l],a=u,i=d}),a.length&&r.push({keys:n,body:i}),r}async function Ay(e){let t=tl(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await mn(yy,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function Hy(){let e=tl(),r=await mn(vy,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function Ry(){let e=tl(),r=await mn(Sy,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function Py(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.attention_count??""].join("|")}async function Ny(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<qy)return!1;o.decisionLogFetchPromise=(async()=>{let n=await Ry(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=Py(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function Dy(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{nn.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=sp(r,o.entities[r],n))})}async function _y(e){if(!e.length)return{ok:!0,message:""};try{let t=await Hy();return Dy(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function B(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||nn.has(b)||!xy(b,n));if(!i.length)return;let l=i.filter(b=>nn.has(b)),c=i.filter(b=>!nn.has(b)),d=(await _y(l)).ok?c:[...c,...l];if(!d.length){Jd(),kn(Ut()),o.busyAction||(Qd(),o.controlError="");return}let g=Number(r.concurrency),w=Number.isFinite(g)&&g>0?Math.floor(g):Vt,y=rl(d,t),f=Math.max(1,Math.min(w,Vt)),m=[];for(let b=0;b<y.length;b+=f){let S=y.slice(b,b+f),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await Ay(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let p="";m.forEach(b=>{if(b.status!=="fulfilled"){let x=b.reason.message||String(b.reason);p||(p=x);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(x=>{if(Object.prototype.hasOwnProperty.call(E,x)){o.optionalMissingEntities&&delete o.optionalMissingEntities[x],o.entities[x]=sp(x,o.entities[x],E[x]);return}let N=O[x];N?.optional?k.has(x)&&$y(x,n):p||(p=`${N?.name||x} ontbreekt in bulk response`)})}),Jd(),kn(Ut()),p?(ip(p),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${p}`):o.busyAction||(Qd(),o.controlError="")}function sp(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function Jd(){o.complete=$a(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=ju(),o.appView==="diagnosis"&&!gt()&&$t(co(),{syncMode:"replace",forceSync:!0}),o.appView||$t(Sn()||co(),{syncMode:"replace",forceSync:!0})}function Oy(){let e=Ys();return o.appView==="settings"?[...new Set([...e,...Js()])]:o.appView==="energy"||o.appView==="results"?En(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?En(o.appView,{forceFast:!0}):[...new Set(e)]}function Ly(e=[]){let t=new Set(e);return(o.appView==="settings"?Js():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...En(o.appView,{includeBulk:!0}),...Bt])]:o.appView==="energy"||o.appView==="results"?[...new Set([...En(o.appView,{forceFast:!0}),...Bt])]:[...new Set(["setupComplete","strategy",...nt])]).filter(n=>!t.has(n))}function Zd(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=Tu}async function be(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||_a()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){py(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<un)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&Zd(t,e),d=t-Number(o.lastStaticEntitySyncAt||0)>=Cu||o.updateInstallBusy||o.updateInstallPhaseHint?Bt:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?mt:[],w=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Ao:[],y=Is()?Wo():[],f=n?[...ns,...nt,"setupComplete",...d]:i?[...En(a,{forceFast:l,includeBulk:c}),...d]:r==="settings"?[...new Set([...Js(),...y,...d])]:c?["setupComplete",...d,...nt,"strategy",...Mo,...rt,...ie()?ge.map(m=>m.key):Et]:["setupComplete",...nt,"strategy",...d];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let m=o.deviceReconnectMode,p=Ty(t,e)?await Cy():{ok:!0,message:""};if(!p.ok){ip(p.message),n||h();return}if(await B([...new Set([...f,...g,...w])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?jt:Vt}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),d.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&ep();let b=m!==o.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="control"||a==="diagnosis"?await Fs():!1,E=S?!1:o.appView==="results"?await La():!1,k=a==="control"?await Ny({force:e.forceDecisionLog===!0}):!1,x=S?!1:Is()?(await Promise.all([$d(),La({metaOnly:!0})])).some(Boolean):!1,N=S||!od()?!1:await Lo(),D=S||!rd()?!1:await Io(),V=S||!jd()?!1:await jo({force:o.systemModal==="mqtt-sensors"}),A=Ed();if(S&&!o.nativeOpen&&np(Zs(a)),b){h();return}if(q&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){h();return}if(E&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){h();return}if(k&&o.appView==="control"){Ws()||h();return}if(x&&o.appView==="settings"){h();return}if(N&&(o.systemModal==="login"||Kt())){h();return}if(D&&o.appView==="settings"){h();return}if(o.systemModal==="mqtt-sensors"){Fa()!==o.mqttSensorsModalRenderSignature&&h();return}if(V&&o.appView==="settings"){h();return}let K=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(A!==o.headerRenderSignature)if(K&&Ds())o.headerRenderSignature=A;else{h();return}else Ds();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){hn()!==o.settingsRenderSignature&&h();return}if(K)return;if(o.appView==="settings"){let H=hn();if(H!==o.settingsRenderSignature&&!o.focusedField){h();return}if(!Cd()){o.focusedField||h();return}o.focusedField&&(o.settingsRenderSignature=H);return}if(o.appView==="diagnosis"){Pd()||h();return}if(o.appView==="control"){Ws()||h();return}if(o.appView==="energy"){Ad()||h();return}if(o.appView==="results"){Hd()||h();return}Rd()||h()}catch(m){n||(o.controlError=`Helperstatus kon niet worden geladen. ${m.message}`,h())}finally{o.entitySyncInFlight=!1;let m=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,m&&!o.nativeOpen&&window.setTimeout(()=>{be(m)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!m&&Zd(Date.now())&&window.setTimeout(()=>{be({forceBulk:!0})},uy)}}function At(){return{}}function Iy(){let e=String(T("hpGeneration")||"").trim();return e||""}function Tn(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function Fy(){if(!Array.isArray(br))return"";if(br.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return br.every(t=>e[t])?"single":""}function nl(e){let t=Tn(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Wy(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function ne(){let e=Tn(T("installationTopology"));if(e==="single"||e==="duo")return nl(e);let t=Tn(At().installation);if(t==="single"||t==="duo")return nl(t);let r=Fy();return r?nl(r):Wy()}function Ht(){let e=ne(),t=Iy();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Qe(){return"OpenQuatt"}function al(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function it(e=mo()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Ko(e=ne()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function Cn(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(At().hardwareProfile||e).trim().toLowerCase()}function mo(){return al(T("connectionText")||At().connection)}function lp(){let e=mo();return e==="wifi"?"eth":e==="eth"?"wifi":""}function cp(){let e=ne();return e==="single"?"duo":e==="duo"?"single":""}function kr(e=ne(),t=mo()){let r=Ko(e),n=Cn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${it(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${it(t)}`:n==="waveshare"?`Waveshare ${r} ${it(t)}`:`${Qe()} ${r} ${it(t)}`}function il(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function Wa(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${il()}`}function Vy(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function jy(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Er(){let e=jy(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Vy(o.entities.uptime);if(n==="d")return Oo(e*1440);if(n==="h")return Oo(e*60);if(n==="s")return Oo(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(At().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Oo((Date.now()-r)/6e4)}function Tr(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(At().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function zo({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let y=e();if(!y)return null;let f={scrollTop:y.scrollTop};if((i||l)&&(f.scrollHeight=y.scrollHeight),l){let m=y.scrollHeight-y.scrollTop-y.clientHeight;f.stickToBottom=m<c}return a&&(f.identity=a(y)),f}function d(y){if(!y)return;let f=e();if(!f||a&&a(f)!==y.identity)return;if(y.stickToBottom){f.scrollTop=f.scrollHeight;return}let m=i?f.scrollHeight-Number(y.scrollHeight||0):0;f.scrollTop=Math.max(0,Number(y.scrollTop||0)+m)}function g(y,f=!0){if(!y)return;let m=Number(t()||0)+1;r(m);let p=()=>{Number(t()||0)!==m||!n()||d(y)};if(f){window.requestAnimationFrame(p);return}p()}function w(){r(Number(t()||0)+1)}return{capture:u,invalidate:w,queue:g,restore:d}}function $e(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function go({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function sl(e,t,r="",n=""){let a=t?r:n;return a?`<p data-oq-switch-copy="${s(e)}" data-on-copy="${s(r)}" data-off-copy="${s(n)}">${s(a)}</p>`:""}function up(e,t,r,n="",a="",i=""){if(!v(e))return"";let l=!!T(e),c=o.loadingEntities||o.busyAction===`switch-${e}`;return ye(e,t,r,`
  ...
  `}function cl(e,t,r,n=""){if(!v(e))return"";let a=dn(T(e));return ye(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function oe(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var mp=250;function gp(){return[]}function xr(){return typeof window>"u"?!1:gp().length>0}function Ky(){return`${We()}/events`}function zy(){return`${We()}/openquatt/logs/recent`}function Ba(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Ka(){return o.nativeOpen?"Niet beschikbaar":xr()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function Uy(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function fp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return Uy(t)}function Gy(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Qy(){return o.nativeOpen?"Niet beschikbaar":xr()?Ba()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":Ba()?"Buffer aan":"Buffer uit"}function Yy(){return Ba()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function ul(){return o.entities?.debugLevel||null}function hp(e=ul()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function Jy(e=ul()){let t=String(e?.value??e?.state??"").trim(),r=hp(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function bp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function Zy(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function yp(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(Zy);let n=[];for(let a of r){let i=n[n.length-1]||null;bp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-mp)}function dl(e,t={}){let r=bw(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:yw(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function Xy(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function ew(){let e=gp(),t=e.length;return e.map((r,n)=>dl(r,{receivedAt:Xy(n,t),seq:n+1}))}function tw(){let e=bl();e&&(e.scrollTop=e.scrollHeight)}var pl=zo({getScroller:bl,getToken:()=>o.webServerLogScrollRestoreToken,setToken:e=>{o.webServerLogScrollRestoreToken=e},isActive:()=>o.systemModal==="webserver-logs",preserveGrowth:!0,stickToBottom:!0}),Jt=pl.capture,Zt=pl.queue;function ow(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}var wp=zo({getScroller:ow,getToken:()=>o.cm100CommissioningScrollRestoreToken,setToken:e=>{o.cm100CommissioningScrollRestoreToken=e},isActive:()=>o.systemModal==="cm100-commissioning",preserveGrowth:!0,stickToBottom:!0}),vp=wp.capture,ml=wp.queue;function rw(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}var Sp=zo({getScroller:rw,getToken:()=>o.serviceTaskModalScrollRestoreToken,setToken:e=>{o.serviceTaskModalScrollRestoreToken=e},isActive:()=>String(o.systemModal||"").startsWith("service-task-")}),qp=Sp.capture,gl=Sp.queue;function nw(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}var kp=zo({getScroller:nw,getToken:()=>o.historyStorageModalScrollRestoreToken,setToken:e=>{o.historyStorageModalScrollRestoreToken=e},isActive:()=>o.systemModal==="history-storage"}),Ep=kp.capture,fl=kp.queue;async function $r(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||Jt(),r=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=r,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let n=await window.fetch(zy(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==r)return;let i=iw(a);o.webServerLogHistoryLoaded=!0,i.length>0&&(yp(i,{prepend:!0}),o.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),o.webServerLogRecentAnchorAt=Date.now())}catch(n){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(h(),Zt(t))}}function aw(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?dl(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function iw(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>aw(r,n+1)).filter(r=>r!==null)}function sw(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function lw(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>bp(e,r))}function cw(){xr()&&o.webServerLogEntries.length===0&&ft({webServerLogEntries:ew()}),ft({webServerLogCopyMessage:"",webServerLogCopyError:""}),o.systemModal="webserver-logs",h(),B(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=Jt();h(),Zt(e)}),tw(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&$r()}function Mr(){ft({webServerLogEntries:[],webServerLogError:"",webServerLogHistoryError:"",webServerLogHistoryLoading:!1,webServerLogHistoryLoaded:!1,webServerLogCopyMessage:"",webServerLogCopyError:"",webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1,webServerLogRecentTail:[],webServerLogRecentAnchorAt:0}),pl.invalidate(),o.systemModal==="webserver-logs"&&h()}function uw(){let e=Jt();Rt(),ft({webServerLogEnabled:null,webServerLogConnected:!1}),Mr(),o.systemModal==="webserver-logs"&&$r({scrollState:e})}function hl(){if(xr()){Rt();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs")){Rt();return}if(o.webServerLogEnabled===!1){Rt();return}o.webServerLogSource||dw()}function dw(){if(xr()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",h();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",h();return}try{let e=new window.EventSource(Ky());o.webServerLogSource=e,e.addEventListener("open",pw),e.addEventListener("ping",mw),e.addEventListener("log",fw),e.onerror=gw}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",Rt(),h()}}function Rt(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function pw(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Jt():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",h(),Zt(e)}Nd({clearOutput:Mr,closeStream:Rt,resetRecoveryState:uw});function mw(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Jt():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",h(),Zt(e))}function gw(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?Jt():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",Rt(),h(),Zt(e)}function fw(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=Jt(),r=hw(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>dl(u)).filter(u=>!sw(u)&&!lw(u));if(i.length===0)return;yp(i);let l=vw(),c=bl();ww(l),Sw(i,l),o.webServerLogEnabled=!0,c&&t&&Zt(t,!1)}function hw(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).trimEnd()}catch{}return t}function bw(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function yw(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function ww(e){for(;o.webServerLogEntries.length>mp;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function vw(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function bl(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function Sw(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",Tp(r))}}function Tp(e){let t=fp(e.receivedAt),r=Gy(e.receivedAt);return`
  ...
  `}function Tw(){let e=ul();if(!e)return"";let t=hp(e),r=Jy(e),n=o.loadingEntities||o.busyAction==="save-debugLevel";return`
...
`)}async function xw(){let e=Cw();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",h();return}try{if(!await io(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&h()}var $w={"open-webserver-log-modal":()=>cw(),"clear-webserver-log-output":()=>Mr(),"copy-webserver-log-output":()=>xw()};function Cp(e){return se($w,e)}function xp(){let e=xr();return ee({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
      ...
      <button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="close-system-modal">Gereed</button>
    `})}function Mw(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Ap(e){return Mw().includes(e)}function Ua(){let e=Cn(),t=ne(),r=mo(),n=lp();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:v("firmwareUpdateTarget")&&Ap("alternate connection")&&v("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:it(r),targetLabel:it(n),currentBuildLabel:kr(t,r),targetBuildLabel:kr(t,n)}}function Ga(){let e=Cn(),t=ne(),r=cp(),n=mo(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:v("firmwareUpdateTarget")&&Ap("alternate topology")&&v("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Ko(t),targetLabel:Ko(r),currentBuildLabel:kr(t,n),targetBuildLabel:kr(r,n)}}function Yo(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function Hr(){let e=Cn(),t=ne(),r=mo(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${it(r)}`}}function $n(e=Yo(),t=Hr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function yo(){if(lt())return"Controleren";let e=Xt();if(e)return e.phaseLabel;if(vl())return"Bijgewerkt";if(Ar())return"Bezig";if(Jo())return"Beschikbaar";let t=Rp();if(t!==null&&t<=0)return"Actueel";let r=At();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||ql()?"Actueel":we()?"Nog niet gecontroleerd":"\u2014"}function we(){return o.entities.firmwareUpdate||null}function Qa(){let e=we();return e?String(e.state??e.value??"").trim().toLowerCase():""}function Aw(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Ya(){return Aw().toLowerCase()}function Hp(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function yl(){let e=String(o.updateInstallTargetVersion||"").trim(),t=de();return!e||!t?!1:Mn(t,e)>=0}function Hw(e=we()||{}){let t=wo(e),r=de(e);return!t||!r?!1:Mn(r,t)>=0}function wl(){return(yl()||Hw())&&!lt()&&!xn()&&!Jo()}function vl(){return(o.updateInstallCompleted||wl())&&!lt()&&!Xt()&&!Jo()}function Rr(){xe({updateInstallBusy:!1,updateInstallTargetVersion:"",updateInstallPhaseHint:"",updateInstallProgressHint:Number.NaN,updateInstallMode:"",updateInstallTargetConnection:"",updateInstallTargetTopology:""}),Lw()}function Ae(){xe({updateManualUploadFile:null,updateManualUploadFileName:"",updateManualUploadError:""})}function Je(e={}){xe({...e.clearPr?{updateTestFirmwarePr:""}:{},updateTestFirmwareConfirmed:!1,updateTestFirmwareError:"",updateTestFirmwareBuild:null})}function Rw(){let e=Ya(),t=Hp();if(e==="starting"||e==="uploading"||e==="rebooting"){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(yl()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function xn(){let e=Ya();return e==="starting"||e==="uploading"||e==="rebooting"}function Xt(){Rw();let e=Ya(),t=e==="starting"||e==="uploading"||e==="rebooting",r=t?e:o.updateInstallPhaseHint,n=Hp(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!xn()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Qe()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${it(o.updateInstallTargetConnection)}-build wordt nu naar ${Qe()} verzonden.`:o.updateInstallMode==="topology-switch"?`De ${kr(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Qe()} verzonden.`:`Firmware wordt nu naar ${Qe()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Qe()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${it(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${Ko(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Qe()}.`}}function wo(e=we()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function de(e=we()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function Pw(e=we()||{}){let t=de(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:Mn(t,r)>0}function Sl(e=we()||{},t=bo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=wo(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function $p(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function Mn(e,t){let r=$p(e),n=$p(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function Ar(){if(wl())return!1;let e=Qa();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function lt(){let e=Qa();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Jo(){let e=Qa();if(!Sl())return!1;let t=Rp();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:At().updateAvailable===!0}function ql(){let e=Qa();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Pw()}function kl(){let e=we()||{},t=de(e)||"\u2014",r=Sl(e)?wo(e):"",n=r?Mn(r,t):null;return!lt()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function Rp(){let{current:e,latest:t}=kl();return e==="\u2014"||t==="\u2014"?null:Mn(t,e)}function Pp(e=bo()){let t=String(e||"").trim().toLowerCase();return Ui[t]||Ui.main}function Nw(){let e=String((we()||{}).release_url||"").trim(),t=Pp();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function Dw(){return Qe()}function bo(){return String(T("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function _w(){return kl().latest!=="\u2014"}function Mp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Ow(e,t=we()||{}){let r=Mp(e);return r?Mp(`${t.title||""} ${t.summary||""}`).includes(r):!0}function Np(e){return new Promise(t=>window.setTimeout(t,e))}function An(e=No){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,dd(),typeof Rt=="function"&&Rt(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&ks()},e)}function Lw(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||ks()}function Dp(){return o.deviceReconnectMode?ee({modalId:"reconnect",titleId:"oq-reconnect-modal-title",kicker:"Systeem",title:Ju(),modalClass:"oq-helper-modal--reconnect",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `}):""}function Hn(e=bo()){let t=we()||{},r=de(t);o.entities.firmwareUpdate={...t,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:Pp(e),current_version:r}}async function Rn(e={}){let t=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await Np(r===0?900:1200),await B(sn,"all",{forceMissing:!0});let n=Sl(),a=!t||Ow(t),i=_w(),l=lt(),c=yo();if(n&&a&&(i||!l&&c!=="Nog niet gecontroleerd"))return!0}return!1}async function Pr(e={}){let t=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await Np(a===0?r:n);try{if(await B(sn,"all",{forceMissing:!0}),Ya()==="rebooting"&&lo("ota"),h(),o.updateInstallMode==="connection-switch"){let i=al(o.updateInstallTargetConnection);if(i&&mo()===i&&!xn()&&!Ar())return!0}else if(o.updateInstallMode==="topology-switch"){let i=Tn(o.updateInstallTargetTopology);if(i&&ne()===i&&!xn()&&!Ar())return!0}else if(yl()||wl()||ql()&&!xn()&&!Ar())return!0}catch{t||(o.controlNotice="Wachten tot het device opnieuw is opgestart...",h(),t=!0)}}return!1}function Iw(){let e=bo(),t=Xt();if(t)return t.copy;if(vl()){let r=o.updateInstallCompletedVersion||de()||bo();return`${Qe()} draait nu op ${r}.`}return Ar()?`OTA-update wordt voorbereid voor ${Qe()}. Het device kan kort herstarten.`:lt()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:Jo()?"Er staat een nieuwere firmware klaar.":ql()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function _p(){return!!(o.firmwareAdvancedOpen||o.firmwareConnectionSwitchOpen||o.firmwareTopologySwitchOpen||o.updateManualUploadOpen||o.updateTestFirmwareOpen)}function za(e,t,r,n,a=!1){return`
      <button
    ...
    `}function jw(){if(!o.updateTestFirmwareOpen)return"";let t=!!(Xt()||o.updateInstallBusy||lt()),r=Yo(),n=Hr(),a=$n(r,n),i=!!(n.available&&v("firmwareTestOtaUrl")&&v("firmwareTestOtaMd5Url")&&v("installFirmwareTestOta")),l=!!(r&&i),c=o.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,d=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Op(){if(!o.updateModalOpen)return"";let e=we(),t=o.entities.firmwareUpdateChannel||null,{current:r,latest:n}=kl(),a=lt(),i=Ar(),l=Jo(),c=Iw(),u=Xt(),d=vl(),g=Nw(),w=d?"Firmware-update afgerond":u||i?"Firmware-update bezig":a?"Controleren op firmware-update":Dw(),y=t?Array.isArray(t.option)?t.option:Array.isArray(t.options)?t.options:[]:[],f=Ua(),m=Ga(),p=!!(f&&!d),b=!!(m&&!d);return ee({id:"firmware-update",titleId:"oq-update-modal-title",kicker:"OTA-update",title:w,copy:c,backdropClass:a||i||u?"is-busy":"",className:"oq-helper-modal--firmware oq-helper-modal--scrollable",closeAction:"close-update-modal",closeLabel:"Sluit update-popup",body:`
          ${d?`
  ...
  `}function Gw(){if(!o.root)return;if(o.interfacePanelOpen){h();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=El();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function Qw(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Pn()),r&&(r.textContent=Xa())}function Tl(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function Yw(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${Tl(e)}</span>`}function eo(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${Tl(e)}</span>`}function Jw(){return[...Po]}function Zw(){let e=[];return Po.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function Xw(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function ei(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function Xo(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function vo(e){return`${We()}/openquatt/debug-recording/${e}`}function Nr(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Cl(){Nr({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function ev(){let e=await window.fetch(vo("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Nr(t),t}function Nn(e=2e3){Xo(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{ti({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function ti(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",h());try{await ev(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),Nn()}catch(t){Cl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?h():(Gw(),Qw())}}async function Kp(){let e=rl(Po,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch(vo(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==Po.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${Po.length})`);return t}async function tv(e){let t=Math.max(1,Number(e)||15);ei(),Xo(),Aa({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await Kp();let r=await window.fetch(vo(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Nr(n),Nn()}catch(r){Cl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,h()}}async function ov(){ei(),Xo(),Aa({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await Kp();let e=await window.fetch(vo("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Nr(t),Nn()}catch(e){Cl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function rv(){let e=await window.fetch(vo("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Nr(t),Xo(),t}async function nv(){ei(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{await rv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function av(e={}){ei(),Xo(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{let t=await window.fetch(vo("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Nr(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,h()}}function Lp(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(Su/1e3),sample_count:Pt(),column_count:Po.length,storage:"browser"},columns:Jw(),units:Zw(),initial:Xw(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function Ja(e){return JSON.stringify(e)}function iv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([Ja(Lp())]).size}catch{return Ja(Lp()).length}}function sv(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function lv(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function cv(){if(Pt()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Nt();h();try{let t=await window.fetch(vo("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,hs(lv(r),Ja(r),"application/json"),Bp(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,h()}}async function uv(){if(Pt()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Nt();h();try{let t=await window.fetch(vo("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await io(Ja(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");Bp(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,h()}}var dv={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",h(),ti()),"start-debug-recording":e=>tv(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>ov(),"select-debug-recording-duration":e=>zw(e.dataset.debugMinutes||15),"stop-debug-recording":()=>av(),"freeze-debug-recording":()=>nv(),"download-debug-recording":()=>cv(),"copy-debug-recording":()=>uv()};function zp(e,t){return se(dv,e,t)}function Up(){let e=o.debugRecordingActive,t=Nt(),r=Za(),n=Pt(),a=o.debugRecordingBusy,i=sv(iv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Kw(),u=Vp(),d=Ip(),g=Uw(),w=n>0,y=`${g.toFixed(1)}%`,f=[{icon:"status",label:"Status",value:Pn()},{icon:"clock",label:t?"Retentie":"Duur",value:Zo(t?d:Fp())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],m=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return ee({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:Xa(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
    ...
    `}function tm(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return ee({id:"system",titleId:"oq-api-security-modal-title",kicker:"Toegang",title:Ev(),copy:Tv(),className:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit API-beveiliging popup",body:`
      ...
      `;return ee({id:"system",titleId:"oq-login-modal-title",kicker:"Systeem",title:qv(),copy:kv(),closeAction:"close-system-modal",closeLabel:"Sluit login-popup",body:`
        ...
        ${n?`<button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="save-web-auth" ${o.authBusy?"disabled":""}>${t?"Opslaan":"Login opslaan"}</button>`:""}`})}function In(e,t,r){let n=R(t),a=R(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=R(e);return Number.isFinite(i)?i:NaN}function Al(e="oq-settings-grid"){let t=$v();return`
      <div class="${s(e)}">
    ...
    `}function am(){let e=v("boilerCvAssistEnabled")&&C("boilerCvAssistEnabled"),t=Va(),r=C("cm100Active"),n=String(t||"").trim().toUpperCase(),a=er(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,u=!!o.pendingCommissioningCm100Start,d=c||i||a,g=c||!i,w=Go("boilerPowerTestStatus","IDLE"),y=ai(w,"boiler"),f=C("boilerPowerTestActive"),m=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",p=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),b=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=er(w),k=!Te(w)&&(f||b||S||Or(w))&&!q,x=Me("boilerRatedHeatPower"),N=Me("boilerHeatPower"),D=R("boilerHeatPower"),V=D>0?N:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:N;D>0&&(o.commissioningBoilerHeatPowerDisplay=N);let A=Go("flowAutotuneStatus","IDLE"),K=ai(A,"autotune"),H=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",F=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),_=!!o.pendingFlowAutotuneStart,P=o.commissioningTaskLock==="autotune",Z=er(A),W=!Te(A)&&(_||P||Or(A))&&!Z,X=Go("airPurgeStatus","IDLE"),Ke=ai(X,"purge"),St=C("airPurgeActive"),pt=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",Ft=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),Xe=!!o.pendingAirPurgeStart,De=o.commissioningTaskLock==="purge",$=!Te(X)&&(St||Xe||De||Or(X)),j=/DONE/.test(String(X||"").toUpperCase()),G=!!(Ft||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),te=Me("airPurgeRemaining",{decimals:0}),le=R("airPurgePhase"),ve=le===1?"Rustig":le===2?"Pulsen":le===3?"Stabiliseren":Ke.phase,ae=Go("manualFlowStatus","IDLE"),He=C("manualFlowActive"),Se=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",ue=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),Wt=!!o.pendingManualFlowStart,pe=o.commissioningTaskLock==="manual-flow",_e=!Te(ae)&&(He||Wt||pe||Or(ae)),ze=Go("manualHpStatus","IDLE"),et=C("manualHpActive"),tt=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",xo=!!(o.entities.manualHpStart||o.entities.manualHpAbort),qe=!!o.pendingManualHpStart,qt=o.commissioningTaskLock==="manual-hp",ot=!Te(ze)&&(et||qe||qt||Or(ze)),la=/SAFETY STOP/.test(String(ze||"").toUpperCase()),ca=/STOPPING/.test(String(ze||"").toUpperCase()),kt=Go("hpWaterCalibrationStatus","IDLE"),ji=ai(kt,"hp-water-calibration"),ua=C("hpWaterCalibrationActive"),mr=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",cu=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),da=!!o.pendingHpWaterCalibrationStart,ro=o.commissioningTaskLock==="hp-water-calibration",Ue=!Te(kt)&&(ua||da||ro||Or(kt)),pa=/DONE|APPLIED/.test(String(kt||"").toUpperCase()),uu=/APPLIED/.test(String(kt||"").toUpperCase()),Ph=Me("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),Nh=Me("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),du=/DONE|APPLIED/.test(String(w||"").toUpperCase()),pu=/DONE|APPLIED/.test(String(A||"").toUpperCase()),Bi=i?q?"Wachten op CM100":k?y.phase:du?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Ki=i?Z?"Wachten op CM100":W?K.phase:pu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",zi=i?$?Ke.phase:j?"Klaar":"Klaar om te starten":"Wachten op CM100",mu=i?_e?"Actief":"Klaar om te starten":"Wachten op CM100",gu=i?ot?ca?"Bezig met stoppen":la?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",fu=i?Ue?ji.phase:uu?"Offsets toegepast":pa?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",hu=!i||m||!p||W||$||_e||ot||Ue||k||P||De||pe||qt||ro||b,Dh=m||!(k||S||b),_h=m||hu||!du||W||$||Ue,bu=!i||H||!F||k||$||_e||ot||Ue||W||S||De||pe||qt||ro||_,Oh=H||!(W||P||_),Lh=H||bu||!pu||k||$||Ue,Ih=!i||pt||!Ft||k||W||_e||ot||Ue||$||S||P||pe||qt||ro||Xe,Fh=pt||!($||De||Xe),Wh=!i||Se||!ue||k||W||$||ot||Ue||_e||S||P||De||qt||ro||Wt,Vh=Se||!(_e||pe||Wt),jh=!i||tt||!xo||k||W||$||_e||Ue||ot||S||P||De||pe||ro||qe,Bh=tt||!(ot||qt||qe),Kh=!i||mr||!cu||k||W||$||_e||ot||Ue||S||P||De||pe||qt||da,zh=mr||!(Ue||ro||da),Uh=mr||Ue||!pa||uu;u&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),b&&(f||Te(w))&&(o.pendingBoilerPowerTestStart=!1),S&&Te(w)&&(o.commissioningTaskLock=""),_&&Te(A)&&(o.pendingFlowAutotuneStart=!1),P&&Te(A)&&(o.commissioningTaskLock=""),Xe&&(St||Te(X))&&(o.pendingAirPurgeStart=!1),De&&Te(X)&&(o.commissioningTaskLock=""),Wt&&(He||Te(ae))&&(o.pendingManualFlowStart=!1),pe&&(He||Te(ae))&&(o.commissioningTaskLock=""),qe&&(et||Te(ze))&&(o.pendingManualHpStart=!1),qt&&(et||Te(ze))&&(o.commissioningTaskLock=""),da&&(ua||Te(kt))&&(o.pendingHpWaterCalibrationStart=!1),ro&&Te(kt)&&(o.commissioningTaskLock="");let Gh=a?"Wachten op CM100":t,Qh=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",Yh=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:fu,available:!!(cu||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Lr({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:fu,statusCopy:Ue?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":pa?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:Av({status:kt,running:Ue,resultReady:pa,startDisabled:Kh,abortDisabled:zh,applyDisabled:Uh,busy:mr,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:mu,available:!!(ue||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Lr({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:mu,statusCopy:_e?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?Qo({active:_e,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:Se||Wh,stopDisabled:Se||Vh}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:gu,available:!!(xo||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Lr({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:gu,statusCopy:ot?ca?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":la?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:Ki,available:!0,openDisabled:er(Ki),cardMarkup:Lr({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:Ki,statusCopy:Z?"Wacht totdat CM100 actief is voordat je autotune start.":W?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort?Qo({active:W,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:H||bu,stopDisabled:H||Oh}):""}
          ...
          `})},{key:"purge",title:"Ontluchten",label:"Ontluchten",summary:"Draait een vaste ontluchtingsrun van 5 minuten met rustige flow, pomp-pulsen en stabilisatie.",status:zi,available:G,openDisabled:er(zi),cardMarkup:Lr({taskKey:"purge",title:"Ontluchten",copy:"Draait 5 minuten met rustige doorstroming, korte pomp-pulsen en een stabilisatiefase.",subcopy:"Na afloop kan OpenQuatt de service mode (CM100) afsluiten of actief laten.",status:zi,statusCopy:$?"Ontluchten loopt vast 5 minuten door en stopt daarna automatisch.":i?"CM100 staat klaar. Start ontluchten wanneer het circuit open staat.":"Start CM100 eerst en voer daarna ontluchten uit.",progressTask:"purge",className:"oq-settings-commissioning-card--air-purge",actions:`
            ${o.entities.airPurgeStart||o.entities.airPurgeAbort?Qo({active:$,startKey:"airPurgeStart",stopKey:"airPurgeAbort",startLabel:"Ontluchten starten",stopLabel:"Ontluchten stoppen",startDisabled:pt||Ih,stopDisabled:pt||Fh}):""}
          `,metrics:`
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:Wo(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,Vr(),o.appView==="results"&&rr({force:!0}).then(()=>h())})},"select-energy-history-import-file":()=>nS(),"clear-energy-history-import-file":()=>{km(),h()},"import-energy-history-file":()=>aS(),"export-energy-history":()=>Bv(),"open-history-storage-modal":()=>{o.systemModal="history-storage",h();let e=pi({forceMissing:!0,forceTrendHistory:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&h()});return Vr([1e3,3e3,7e3]),e},"download-settings-backup":()=>mS(),"open-settings-backup-import":()=>{o.systemModal="settings-backup-import",h()},"confirm-settings-backup-restore":()=>gS()};function Pm(e,t){return se(fS,e,t)}function Bl(e,t,r="",n=!1){return`
    ...
    `:""}function bS(e){return v(e)||!!O[e]&&!o.optionalMissingEntities?.[e]}function zl(e,t,r,n={}){if(!bS(e))return"";let a=v(e),i=a&&(o.loadingEntities||o.busyAction===e),l=n.disabled===!0||!a,c=n.buttonClass||"oq-helper-button oq-helper-button--ghost";return`
      <button
        class="${s(c)}"
        type="button"
        data-oq-action="${s(r)}"
    ...
    `}function yS(){return o.trendHistoryMetadata&&typeof o.trendHistoryMetadata=="object"?o.trendHistoryMetadata:{}}function Kr(){return!!o.trendHistoryMetadataSignature}function zr(){return!!(o.energyHistoryRaw||o.energyHistorySignature)}function ct(e){return e?"Niet geladen":"Laden..."}function je(e,t="\u2014"){if(v(e))return Me(e);let r=wS(e);return r==null||r===""?t:r}function wS(e){let t=yS();if(e==="trendHistoryFlashAvailable")return Kr()?t.available||"Alleen live":ct(o.trendHistoryMetadataError);if(e==="trendHistoryFlashOldest")return Kr()?t.oldest||"Geen data":ct(o.trendHistoryMetadataError);if(e==="trendHistoryFlashNewest")return Kr()?t.newest||"Geen data":ct(o.trendHistoryMetadataError);if(e==="trendHistoryFlashLastFlush")return Kr()?t.lastFlush||"Geen data":ct(o.trendHistoryMetadataError);if(e==="trendHistoryFlashSize")return Kr()?Ul(t.sizeKb):ct(o.trendHistoryMetadataError);if(e==="trendHistoryFlashWrites")return Kr()?Gl(t.writes):ct(o.trendHistoryMetadataError);let r=Om(),a=String(o.energyHistoryRaw||"").includes("@day_retention|");return e==="lifetimeEnergyHistoryAvailable"?zr()?a&&!r.dayPartitionAvailable?"Niet beschikbaar":Dm(r.storedDayCount,"Geen data"):ct(o.energyHistoryError):e==="lifetimeEnergyHistoryOldest"?zr()?gi(r.oldestDateKey):ct(o.energyHistoryError):e==="lifetimeEnergyHistoryNewest"?zr()?gi(r.newestDateKey):ct(o.energyHistoryError):e==="lifetimeEnergyHistoryLastWrite"?zr()?_m(r.dayLastWriteTimestampS):ct(o.energyHistoryError):e==="lifetimeEnergyHistorySize"?zr()?Ul(r.dayStorageKb):ct(o.energyHistoryError):e==="lifetimeEnergyHistoryWrites"?zr()?Gl(r.dayWriteCount):ct(o.energyHistoryError):null}function Dm(e,t="Geen data"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:`${Math.round(r)} ${Math.round(r)===1?"dag":"dagen"}`}function Ul(e,t="\u2014"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:`${Math.round(r)} kB`}function Gl(e,t="0"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:String(Math.round(r))}function gi(e){let t=re(e);return t?t.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit",year:"numeric"}):"Geen data"}function _m(e,t="Geen data"){let r=Number(e);if(!Number.isFinite(r)||r<=0)return t;let n=new Date(r*1e3),a=n.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit"}),i=n.toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"});return`${a} ${i}`}function Om(){return si(o.energyHistoryRaw)}function vS(e){let t=Array.isArray(e.items)?e.items:[];return`
    ...
    `:""}function Lm(){if(!v("trendHistoryEnabled")&&!v("lifetimeEnergyHistoryEnabled"))return"";let e=C("trendHistoryEnabled"),t=e&&C("trendHistoryFlashEnabled"),r=v("lifetimeEnergyHistoryEnabled"),n=r&&C("lifetimeEnergyHistoryEnabled"),a=t?je("trendHistoryFlashAvailable","Alleen live"):"Alleen live",i=r?Nm(je("lifetimeEnergyHistoryAvailable","Geen data")):"Geen data";return oe("Diagnose","Gegevens bewaren","Bepaal welke gegevens OpenQuatt bewaart voor grafieken, resultaten en hulp bij problemen.",`
        <article class="oq-settings-storage-summary">
          ...
          <div class="oq-settings-storage-summary-metrics" aria-label="Opslagstatus">
            ${v("trendHistoryEnabled")?Bl("Live diagnose",e?"Aan":"Uit","Live",e):""}
            ${v("trendHistoryFlashEnabled")?Bl("Diagnosehistorie",a,t?"Permanent geheugen":"Uit",t):""}
    ...
    `}function kS(){if(!v("lifetimeEnergyHistoryEnabled"))return"";let e=jr(o.energyHistoryExportMode),t=jn.map(n=>`
          ...
          <div class="oq-settings-storage-domain-grid">
            <section class="oq-settings-storage-domain oq-settings-storage-domain--trend">
              ...
              <div class="oq-settings-storage-domain-rows">
                ${Kl("trendHistoryEnabled","Recente diagnosegegevens","Bewaar de laatste meetpunten zolang de controller online is.","Na een herstart of update zijn deze tijdelijke gegevens niet meer beschikbaar.","Nieuwe tijdelijke diagnosegegevens worden niet bijgehouden.","Tijdelijk")}
                ${Kl("trendHistoryFlashEnabled","Diagnosegeschiedenis bewaren","Bewaar recente diagnosegegevens ook na een herstart of update.","OpenQuatt slaat deze gegevens ongeveer elk uur op.","Alleen recente diagnosegegevens; bestaande geschiedenis blijft staan.","Permanent geheugen")}
                ${c?`
                  <div class="oq-settings-storage-inline-action">
                    ...
                    ${zl("trendHistoryFlush","Nu opslaan","flush-trend-history",{disabled:!t,busyLabel:"Opslaan..."})}
                  </div>
                `:""}
              </div>
            </section>
              ...
              <div class="oq-settings-storage-domain-rows">
                ...
                ${u?`
                  <div class="oq-settings-storage-inline-action oq-settings-storage-inline-action--split">
                    ...
                    <div class="oq-settings-storage-action-stack">
                      ${zl("lifetimeEnergyHistoryCapture","Vandaag opslaan","save-lifetime-energy-history",{disabled:!n,busyLabel:"Opslaan..."})}
                    </div>
                  </div>
                `:""}
                ${kS()}
                ...
                ${v("lifetimeEnergyHistoryClear")?`
                  <div class="oq-settings-storage-inline-action oq-settings-storage-inline-action--danger">
                    ...
                    <div class="oq-settings-storage-action-stack">
                      ${zl("lifetimeEnergyHistoryClear","Historie wissen","clear-lifetime-energy-history",{disabled:!i,buttonClass:"oq-helper-button oq-helper-button--warning",busyLabel:"Wissen..."})}
                    </div>
                  </div>
                `:""}
              </div>
    ...
    `}kd({getSignature:fi,patch:CS});function Qm(){if(o.systemModal==="login")return om();if(o.systemModal==="api-security")return tm();if(o.systemModal==="mqtt")return Ld();if(o.systemModal==="mqtt-sensors")return Id();if(o.systemModal==="connectivity"){let e=TS();return ee({modalId:"system",titleId:"oq-system-modal-title",kicker:"Systeem",title:"Connectiviteit",closeAction:"close-system-modal",closeLabel:"Sluit systeem-popup",bodyMarkup:`
        ...
        `});if(o.systemModal==="webserver-logs")return xp();if(o.systemModal==="debug-recording")return Up();if(o.systemModal==="openquatt-pause"){let e=C("openquattEnabled"),t=o.busyAction==="openquatt-regulation",r=v("openquattResumeAt"),n=o.loadingEntities||o.entitySyncInFlight,a=r||!n,i=pn(),l=yr(T("openquattResumeAt")),c=Ca();return ee({modalId:"system",titleId:"oq-openquatt-pause-modal-title",kicker:"Bediening",title:"Openquatt regeling",modalClass:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit regeling-popup",bodyMarkup:`
    ...
    `}function ec(e="oq-settings-grid"){let t=[ce("flowSetpoint","Gewenste flow verwarmen","De flow die OpenQuatt zoveel mogelijk probeert vast te houden buiten koeling."),ce("coolingFlowSetpoint","Gewenste flow koelen","De flow die OpenQuatt gebruikt tijdens actieve koeling.")].filter(Boolean).join("");return`
      <div class="${s(e)}">
        ...
        ${Vu()?ce("manualIpwm","Vaste pompstand","Deze pompstand wordt gebruikt zolang de regeling op handmatig staat."):t}
      </div>
    ...
    `:""}function ac(e,t,r){let n=[ho(t,"Stand A","Kies hier welke compressorstand je wilt uitsluiten."),ho(r,"Stand B","Kies hier nog een compressorstand die je wilt overslaan.")].filter(Boolean).join("");return n?`
    ...
    `:""}function Ym(){let e=MS();return oe("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
      ...
      `;return oe("Regeling","Verwarmingsstrategie","Kies hier hoe OpenQuatt je verwarming regelt. De instellingen hieronder passen zich automatisch aan.",`
        ...
        ${oc()}
        ${e}
      `)}function ic(){let r={top:22,right:18,bottom:38,left:34},n=560-r.left-r.right,a=240-r.top-r.bottom,i=ge[0].outdoor,l=ge[ge.length-1].outdoor,c=f=>r.left+(f-i)/(l-i)*n,u=f=>r.top+(70-f)/50*a,d=[20,30,40,50,60,70].map(f=>{let m=u(f);return`
    ...
    `:"";return oe("Bewaking","Installatiebewaking","Lokale diagnose voor compressorstarts, hydrauliek en verbindingen. Hiervoor is geen Home Assistant nodig.",`
      ...
      `)}function sc(){if(!v("hpGeneration"))return"";let e={V1:{copy:"Voor Quatt V1 en Quatt V1 + V1.5 combinaties.",image:Ma,alt:"Quatt Hybrid V1 en V1.5",infoTitle:"V1",infoCopy:`Model: AMM4
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=Qt(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function ng(){let e=Ht(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&Qt(t).length>0;return!e&&!r?"":oe("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function sg(){let e=yo(),t=Wa(),r=o.busyAction==="restartAction";return oe("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
      ...
      `)}function lg(){let e=[ac("Warmtepomp 1","hp1ExcludedA","hp1ExcludedB"),ac("Warmtepomp 2","hp2ExcludedA","hp2ExcludedB")].filter(Boolean).join("");return oe("Installatie","Compressorinstellingen","Stel hier de minimale draaitijd in en bepaal per warmtepomp welke compressorstanden je wilt overslaan.",`
        <div class="oq-settings-subpanel">
          <div class="oq-settings-subpanel-head">
    ...
    `}function jS(){let e=yi(),t=o.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function JS(){return o.currentStep==="generation"?dg():o.currentStep==="boiler"?v("boilerCvAssistEnabled")?KS():ug():o.currentStep==="flow-source"?VS():o.currentStep==="thermostat-source"?jS():o.currentStep==="flow"?zS():o.currentStep==="heating"?US():o.currentStep==="water"?GS():o.currentStep==="silent"?QS():o.currentStep==="confirm"?YS():ug()}function nr(){return gr.filter(e=>!e.optionalEntity||v(e.optionalEntity))}function _t(e){let t=nr().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function ZS(e){let t=wi(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function XS(e=!1){return nr().map((t,r)=>{let n=ZS(r);return`
        <button
      ...
      `}).join("")}function eq(){let e=nr();return e.find(t=>t.id===o.currentStep)||e[0]||gr[0]}function wi(){return Math.max(0,nr().findIndex(e=>e.id===o.currentStep))}function yg(e){let t=nr(),r=Math.min(t.length-1,Math.max(0,wi()+e));o.currentStep=t[r]?.id||gr[0].id}function to(e={}){let t=wi(),r=nr(),n=t>0?r[t-1]:null,a=t<r.length-1?r[t+1]:null;return`
    ...
    `}function tq(){let e=wi(),t=nr();return`
    ...
    `:"";return oe("Integratie","MQTT inputbronnen","Beheer de brokerverbinding voor externe MQTT-bronwaarden.",`
      ...
      `)}function kg(){let e=[["login","Login",On(),Ln(),"open-login-modal"],["api","ESPHome API encryption",Dr(),_r(),"open-api-security-modal"]];return oe("Toegang","Toegang & Beveiliging","Pas hier de web-login of de ESPHome API-sleutel aan. Deze wijziging wordt actief na herstart.",`
    ...
    `:""}function pc(){let e=ka.map(sq).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function $g(e=pc()){return he(e)}function Mg(e=pc()){return`
    ...
    `}var Ag=[{id:"day",label:"Dag"},{id:"week",label:"Week"},{id:"month",label:"Maand"},{id:"year",label:"Jaar"},{id:"all",label:"Alles"}],lq=new Set(["day","week","month","year"]);function Be(e){let t=String(e||"").trim();return Ag.some(r=>r.id===t)?t:"day"}function Hg(e){let t=Be(e);o.energyHistoryView!==t&&(xt({energyHistoryView:t,energyHistoryLastFetchAt:0}),h(),Rg())}function Rg(){typeof rr=="function"&&rr({force:!0}).then(e=>{e&&h()})}function Yr(e){return lq.has(Be(e))}function Si(){let e=new Date;return e.getFullYear()*1e4+(e.getMonth()+1)*100+e.getDate()}function Pg(){return si(o.energyHistoryRaw)}function mc(){let e=String(o.energyHistoryRaw||""),t=null;return e.split(/\r?\n/).forEach(r=>{let n=Il(r);n&&(t=n.dateKey)}),t}function cq(e=[],t=!0){let r=mc(),n=Pg(),a=(Array.isArray(e)?e:[]).map(i=>Number(i?.dateKey)).filter(Number.isFinite);return Number.isFinite(Number(n.newestDateKey))&&a.push(Number(n.newestDateKey)),Number.isFinite(Number(r))&&a.push(Number(r)),t&&qi().forEach(i=>{let l=Number(i?.dateKey);Number.isFinite(l)&&a.push(l)}),a.length?Math.max(...a):Si()}function ar(e){let t=R(e);return!Number.isFinite(t)||t<0?null:Math.round(t*1e3)}function uq(){let e=mc()||Si(),t=re(e);if(!t)return null;let r={sequence:Number.MAX_SAFE_INTEGER-1,dateKey:e,year:t.year,month:t.month,day:t.day,partial:!0,source:"sensors",electricalInputWh:ar("electricalEnergyDaily"),heatingInputWh:ar("heatingElectricalEnergyDaily"),coolingInputWh:ar("coolingElectricalEnergyDaily"),heatpumpHeatOutputWh:ar("heatpumpThermalEnergyDaily"),heatpumpCoolingOutputWh:ar("heatpumpCoolingEnergyDaily"),boilerHeatOutputWh:ar("boilerThermalEnergyDaily"),systemHeatOutputWh:ar("systemThermalEnergyDaily")};return Fn.some(n=>Number.isFinite(r[n]))?r:null}function Gn(){let e=new Map;String(o.energyHistoryRaw||"").split(/\r?\n/).forEach(i=>{let l=fm(i)||Il(i);if(!l)return;let c=e.get(l.dateKey);(!c||l.sequence>=c.sequence)&&e.set(l.dateKey,l)});let r=uq();r&&e.set(r.dateKey,r);let n=new Set(e.keys()),a=new Map;return qi().forEach(i=>{if(n.has(i.dateKey))return;let l=a.get(i.dateKey);if(!l){let c=re(i.dateKey);if(!c)return;l=ir({dateKey:c.key,year:c.year,month:c.month,day:c.day,label:Dt(c.key),sortKey:c.key,source:"hour-summary"}),l.tooltipLabel=`${Dt(i.dateKey)} \xB7 uurdata sinds herstart`,a.set(i.dateKey,l)}sr(l,i)}),a.forEach((i,l)=>{e.set(l,i)}),[...e.values()].sort((i,l)=>i.dateKey-l.dateKey)}function qi(){let e=new Map;return String(o.energyHistoryRaw||"").split(/\r?\n/).forEach(r=>{let n=hm(r);if(!n)return;let a=`${n.dateKey}:${n.hour}`,i=e.get(a);(!i||n.sequence>=i.sequence)&&e.set(a,n)}),[...e.values()].sort((r,n)=>r.sortKey-n.sortKey)}function dq(e){return qi().filter(t=>t.dateKey===Number(e))}function Qr(e,t){return e.reduce((r,n)=>r+Fr(n,t),0)}function Ng(e){return["heatpumpHeatOutputWh","heatpumpCoolingOutputWh","boilerHeatOutputWh"].reduce((t,r)=>t+Fr(e,r),0)}function pq(e){return Fr(e,"electricalInputWh")+Ng(e)}function Un(e,t){let r=Number(e),n=Number(t);return!Number.isFinite(r)||!Number.isFinite(n)||n<=0?"\u2014":(r/n).toFixed(2)}function ut(e,t=1){let r=Number(e);return Number.isFinite(r)?Math.abs(r)>=999500?`${(r/1e6).toFixed(2)} MWh`:Math.abs(r)<1e3?`${Math.round(r)} Wh`:`${(r/1e3).toFixed(t)} kWh`:"\u2014"}function ir({dateKey:e,year:t,month:r,day:n,hour:a=null,label:i,tooltipLabel:l="",sortKey:c,source:u="bucket"}){return{sequence:0,dateKey:e,year:t,month:r,day:n,hour:a,label:i,tooltipLabel:l,sortKey:c??e,partial:!1,source:u,electricalInputWh:0,heatingInputWh:0,coolingInputWh:0,heatpumpHeatOutputWh:0,heatpumpCoolingOutputWh:0,boilerHeatOutputWh:0,systemHeatOutputWh:0}}function sr(e,t){return Fn.forEach(r=>{e[r]+=Fr(t,r)}),e.partial=e.partial||!!t?.partial,e.sequence=Math.max(Number(e.sequence||0),Number(t?.sequence||0)),e}function mq(e){let t=new Map;return e.forEach(r=>{t.set(r.dateKey,r)}),t}function Dg(e,t){let r=Be(e);if(r==="day"){let n=Pl(t)||re(t);return n?String(n.key):""}if(r==="week"){let n=gm(t);return n?String(n.key):""}if(r==="month"){let n=mm(t);return n?String(n.key):""}if(r==="year"){let n=Number(t);return Number.isInteger(n)&&n>=2020&&n<=2200?String(n):""}return""}function ki(e,t){let r=Be(t),n=re(cq(e,!0)),a=Pg(),i=qi(),l=[...e.map(f=>f.dateKey),...i.map(f=>f.dateKey)].filter(f=>Number.isFinite(Number(f)));Number.isFinite(Number(a.oldestDateKey))&&l.push(Number(a.oldestDateKey)),Number.isFinite(Number(a.newestDateKey))&&l.push(Number(a.newestDateKey));let c=l.length?Math.min(...l.map(Number)):n?.key,d=(c?re(c):n)?.date||n?.date||new Date,g=n?.date||new Date,w=n?.key||Si(),y=w;return r==="week"?(w=_l(d),y=_l(g)):r==="month"?(w=ci(d),y=ci(g)):r==="year"?(w=d.getFullYear(),y=g.getFullYear()):(w=Ie(d),y=Ie(g)),Number(w)>Number(y)&&(w=y),{min:String(w),max:String(y)}}function _g(e,t){let r=Number(e);return Number.isFinite(r)?r<Number(t.min)?String(t.min):r>Number(t.max)?String(t.max):String(e):String(t.max)}function gq(e,t,r=ki(e,t)){let n=Be(t),a=o.energyHistoryPeriodSelection?.[n],i=Dg(n,a);return _g(i||r.max,r)}function fq(e,t){let r=Be(e),n=[],a=0;if(r==="week"){let i=String(t.max);for(;Number(i)>=Number(t.min)&&a<6e3;){let l=re(i);n.push({value:i,label:Ol(i),group:l?String(Dl(l.date).year):""}),i=l?String(Ie(or(l.date,-7))):"",a+=1}return n}if(r==="month"){let i=String(t.max);for(;Number(i)>=Number(t.min)&&a<1200;){let l=tr(i);if(!l)break;n.push({value:i,label:l.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"}),group:String(l.year)}),i=Nl(i,-1),a+=1}return n}if(r==="year")for(let i=Number(t.max);i>=Number(t.min);i-=1)n.push({value:String(i),label:String(i)});return n}function Ei(e,t){let r=Be(t);if(!Yr(r))return{view:r,selectedValue:"",minValue:"",maxValue:"",canPrevious:!1,canNext:!1,isNow:!0,options:[]};let n=ki(e,r),a=gq(e,r,n);return{view:r,selectedValue:a,minValue:n.min,maxValue:n.max,canPrevious:Number(a)>Number(n.min),canNext:Number(a)<Number(n.max),isNow:Number(a)===Number(n.max),options:fq(r,n)}}function hq(e,t){let r=Be(t);if(!Yr(r))return{from:"",to:"",hours:"0"};let n=Ei(e,r);if(r==="day")return{from:n.selectedValue,to:n.selectedValue,hours:"1"};if(r==="week"){let a=re(n.selectedValue);if(!a)return{from:"",to:"",hours:"0"};let i=Ir(a.date),l=or(i,6);return{from:String(Ie(i)),to:String(Ie(l)),hours:"0"}}if(r==="month"){let a=tr(n.selectedValue);return a?{from:String(a.year*1e4+a.month*100+1),to:String(a.year*1e4+a.month*100+Rl(a.year,a.month)),hours:"0"}:{from:"",to:"",hours:"0"}}if(r==="year"){let a=Number(n.selectedValue);return Number.isInteger(a)?{from:`${a}0101`,to:`${a}1231`,hours:"0"}:{from:"",to:"",hours:"0"}}return{from:"",to:"",hours:"0"}}function bq(){if(!String(o.energyHistoryRaw||"").trim())return"?meta=1";let e=Gn(),t=hq(e,o.energyHistoryView||"day"),r=new URLSearchParams;t.from&&r.set("from",t.from),t.to&&r.set("to",t.to),r.set("hours",t.hours);let n=r.toString();return n?`?${n}`:""}bm(bq);function Qn(e,t){let r=Be(e);if(!Yr(r))return;let n=Gn(),a=ki(n,r),i=Dg(r,t),l=_g(i||a.max,a);xt({energyHistoryPeriodSelection:{...o.energyHistoryPeriodSelection,[r]:l},energyHistoryLastFetchAt:0}),h(),Rg()}function Og(e,t){let r=Be(e);if(!Yr(r))return;let n=Gn(),a=Ei(n,r),i=Number(t)<0?-1:1,l=a.selectedValue;if(r==="day"){let c=re(a.selectedValue);l=c?String(Ie(or(c.date,i))):l}else if(r==="week"){let c=re(a.selectedValue);l=c?String(Ie(or(c.date,i*7))):l}else r==="month"?l=Nl(a.selectedValue,i):r==="year"&&(l=String(Number(a.selectedValue)+i));Qn(r,l)}function Lg(e){let t=Be(e);if(!Yr(t))return;let r=Gn(),n=ki(r,t);Qn(t,n.max)}function yq(e,t,r=Ei(e,t)){let n=Be(t),a=mq(e);if(!e.length&&n==="all")return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};if(n==="day"){let c=re(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=dq(c.key);if(u.length){let f=new Map(u.map(p=>[p.hour,p])),m=[];for(let p=0;p<24;p+=1){let b=String(p),S=`${c.date.toLocaleDateString("nl-NL",{day:"numeric",month:"long"})} \xB7 ${String(p).padStart(2,"0")}:00 - ${String((p+1)%24).padStart(2,"0")}:00`,q=ir({dateKey:c.key,year:c.year,month:c.month,day:c.day,hour:p,label:b,tooltipLabel:S,sortKey:p,source:"hour"}),E=f.get(p);E&&sr(q,E),m.push(q)}return{buckets:m,title:"Dag",detail:`${c.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 uurdata sinds herstart`}}let d=a.get(c.key),g=mc()||Si(),w=c.key===g?"Vandaag":Dt(c.key),y=ir({dateKey:c.key,year:c.year,month:c.month,day:c.day,label:w,sortKey:c.key,source:"day"});return d&&sr(y,d),{buckets:[y],title:"Dag",detail:`${c.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 dagtotaal`}}if(n==="week"){let c=re(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=Ir(c.date),d=[];for(let g=0;g<7;g+=1){let w=or(u,g),y=Ie(w),f=re(y),m=ir({dateKey:y,year:f.year,month:f.month,day:f.day,label:Dt(y,"weekday"),sortKey:y}),p=a.get(y);p&&sr(m,p),d.push(m)}return{buckets:d,title:"Week",detail:Ol(r.selectedValue)}}if(n==="month"){let c=tr(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=Rl(c.year,c.month),d=[];for(let g=1;g<=u;g+=1){let w=Hl(c.year,c.month,g),y=Ie(w),f=ir({dateKey:y,year:c.year,month:c.month,day:g,label:String(g),sortKey:y}),m=a.get(y);m&&sr(f,m),d.push(f)}return{buckets:d,title:"Maand",detail:c.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"})}}if(n==="year"){let c=Number(r.selectedValue);if(!Number.isInteger(c))return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=[];for(let d=1;d<=12;d+=1){let g=c*1e4+d*100+1,w=ir({dateKey:g,year:c,month:d,day:1,label:Dt(g,"month"),sortKey:d,source:"month"});e.filter(y=>y.year===c&&y.month===d).forEach(y=>sr(w,y)),u.push(w)}return{buckets:u,title:"Jaar",detail:String(c)}}let i=new Map;e.forEach(c=>{i.has(c.year)||i.set(c.year,ir({dateKey:c.year*1e4+101,year:c.year,month:1,day:1,label:String(c.year),sortKey:c.year,source:"year"})),sr(i.get(c.year),c)});let l=[...i.values()].sort((c,u)=>c.sortKey-u.sortKey);return{buckets:l,title:"Alles",detail:l.length?`${l[0].label} - ${l[l.length-1].label}`:"Geen data"}}function wq(e){let t=Qr(e,"heatpumpHeatOutputWh"),r=Qr(e,"heatpumpCoolingOutputWh"),n=Qr(e,"boilerHeatOutputWh");return{electricalInputWh:Qr(e,"electricalInputWh"),heatingInputWh:Qr(e,"heatingInputWh"),coolingInputWh:Qr(e,"coolingInputWh"),heatOutputWh:t,coolingOutputWh:r,boilerOutputWh:n,outputWh:t+r+n}}function vq(e){let t=Number(e.heatOutputWh||0)+Number(e.coolingOutputWh||0),r=Number(e.boilerOutputWh||0),n=t+r;return!Number.isFinite(n)||n<=0?Number.NaN:t/n*100}function Sq(e){let t=Un(e.heatOutputWh,e.heatingInputWh),r=Un(e.coolingOutputWh,e.coolingInputWh),n=Number(e.heatOutputWh||0)>0&&t!=="\u2014",a=Number(e.coolingOutputWh||0)>0&&r!=="\u2014";return n&&a?{label:"COP / EER",value:`${t} / ${r}`}:a?{label:"Gemiddelde EER",value:r}:{label:"Gemiddelde COP",value:t}}function vi(e,t,r=""){return`
...
`)}function Hq(e,t=""){if(!e.length)return`
    ...
    `}function gc(){let e=Gn(),t=Be(o.energyHistoryView),r=Ei(e,t),n=yq(e,t,r),a=wq(n.buckets);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:a,activeView:t}}function Ig(e=gc()){return he({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,currentValues:Fn.map(t=>e.buckets[e.buckets.length-1]?.[t]??null)})}function Fg(e=gc()){let t=e.summary,r=Sq(t),n=e.buckets[0]?.dateKey?Dt(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?Dt(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
      <section class="oq-energy-history" data-render-signature="${s(Ig(e))}">
        ...
        <div class="oq-energy-history-stats">
          ...
          ${vi("Elektrisch",ut(t.electricalInputWh,1),"verbruikt")}
          ${vi("Warmtepomp",ut(t.heatOutputWh+t.coolingOutputWh,1),"warmte en koeling")}
          ${vi("Cv-ketel",ut(t.boilerOutputWh,1),"thermisch")}
        </div>
        ${Tq(t)}
    ...
    `}function Pq(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=pc();return Ze(t,$g(n),Mg(n))}function Nq(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=gc(),a=kq();return(a?!1:Ze(t,Ig(n),Fg(n)))||a}po({patchEnergyDom:Pq,patchResultsDom:Nq});function Dq({label:e,value:t,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(t)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function Zn(e,t=!1){return e.map(r=>Dq({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?at(r.key):r.value,status:t})).join("")}function wc(e){return`
    ...
    `}function jg(e){let t=String(e||"").trim();return{None:"geen bron",Manual:"handmatig",Disabled:"handmatig","HA input":"HA-invoer",MQTT:"MQTT","OT thermostat":"OpenTherm","HA input + Manual":"HA-invoer + handmatig","MQTT + Manual":"MQTT + handmatig","OT thermostat + Manual":"OpenTherm + handmatig"}[t]||t}function Yn(e,t){return!t||t==="geen bron"?e:`${e} Toestemming: ${t}.`}function Sc(e,t){return t?"Actief":e==="Stand-by"?"Stand-by":e==="Onbekend"?"Onbekend":"Niet actief"}function _q(e,t){let r=t?"active":"neutral",n=Sc(e,t);return`<span class="oq-overview-chip oq-overview-chip--${s(r)}" data-oq-bind="panel-status">${s(n)}</span>`}function Oq(e){return`
      <span
    ...
    `}function qc(e,t,r,n){return`${r?Oq(n):""}${_q(e,t)}`}function Ug(e,t,r,n,a){if(!e)return;let i=he({mode:t,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&(Ot(e,qc(t,r,n,a)),e.dataset.renderSignature=i)}function Ti(e,t,r=""){return`
    ...
    `}function Iq(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}var Fq=Oo;function Wq(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function Vq(e){let t=Wq(L("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function jq(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=Fq(r),a=v("timeValid")&&C("timeValid")?Vq(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Bq(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Gg(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>v(e))||""}function Kq(){return["hp1WaterIn","hp2WaterIn"].find(e=>v(e))||""}function Qg(e=L("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function Lt(){return Qg()}function Yg(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function kc(){return Lt()?"Koeling":ie()?"Stooklijn":"Power House"}function zq(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=R(t);if(!Number.isNaN(r))return r}return Number.NaN}function Jg(){let e=zq(),t=R("phouseHouse"),r=R("totalHeat"),n=R("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:z(e,0,"W"),houseText:z(t,0,"W"),correctionText:Bq(a),capacityText:at("hpCapacity"),statusTitle:i,statusCopy:l}}function Zg(){let e=R("curveSupplyTarget"),t=R("supplyTemp"),r=Gg(),n=r?R(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:at("curveSupplyTarget"),supplyText:at("supplyTemp"),deltaText:Iq(a),capacityText:at("hpCapacity"),statusTitle:l,statusCopy:c}}function Xg(){let e=R("supplyTemp"),t=L("coolingGuardMode",""),r=L("coolingFallbackNightMinOutdoorTemp","\u2014"),n=R("coolingSupplyError"),a=R("coolingDemandRaw"),i=C("coolingPermitted"),l=C("coolingRequestActive"),c=L("coolingBlockReason","Onbekend"),u=uc(c),d=Yg(c,l),g="Wacht op koelvraag",w="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return d?(g="Wacht op koelvraag",w="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(g="Houdt doel vast",w="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(g="Trekt aanvoer omlaag",w="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(g="Benadert koeldoel",w="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(g="Koelt rustig door",w="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(g="Koeling gereed",w="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(g="Koeling geblokkeerd",w=`Blokkade: ${u}.`),{targetText:at("coolingSupplyTarget"),supplyText:at("supplyTemp"),safeFloorText:at("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:at("coolingDemandRaw"),statusTitle:g,statusCopy:w,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:d}}function Ci(){if(Lt()){let t=Xg(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(ie()){let t=Zg();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=Jg();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function Ec(e){return zg({className:"oq-overview-system",title:e.title,copy:e.copy,signature:he(e),body:`
      ...
      `})}function Uq(){if(!C("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(Lt()){let n=Xg(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(Cg())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(ie()?Zg():Jg()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Gq(){return C("openquattEnabled")?Lt()?C("coolingPermitted")?C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:L("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:C("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Tc(e,t){let r=Uq(),n=Gq();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Cc(e,t){let r=Tc(e,t);return`
    ...
    `:""}function Jr({className:e,action:t,label:r,busy:n=!1,loading:a=!1,attrs:i=""}){return`
      ...
      `:s(n?"Bezig...":r)}</button>
    `}function Jq(e){if(e.kind==="openquatt-control"){let t=o.busyAction==="openquatt-regulation",r=(o.loadingEntities||o.entitySyncInFlight)&&!v("openquattResumeAt");return C("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${Jr({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:t})}</div>`:`
    ...
    `}function $c(){let e=Qq();return e.length?`
    ...
    `}function nf(e,t){if(e?.currentKey&&v(e.currentKey)){let r=R(e.currentKey);if(Number.isFinite(r))return r}return Jn(e,t)}function Pc(e){e&&Rc().forEach(t=>{let r=e.querySelector(`[data-oq-trend-card="${t.id}"]`),n=t.samples[t.samples.length-1]||null;r&&t.series.forEach(a=>{let l=r.querySelector(`[data-oq-trend-current="${a.id}"]`)?.querySelector("strong"),c=z(nf(a,n),a.decimals,a.unit);l&&l.textContent!==c&&(l.textContent=c)})})}function l1(e,t,r=!1,n=oo()){let a=rf(e,t,{mockData:r,windowHours:n}),i=Hc(n),l=hc(a.startTime),c=hc(a.startTime+a.span/2),u=hc(a.endTime),d=a.tracks.flatMap(g=>{if(g.points.length<2){let w=g.points[0];return w?`
          <circle
            cx="${w.x.toFixed(1)}"
            cy="${w.y.toFixed(1)}"
            r="3.8"
    ...
    `}function c1(e){let t=e.samples[e.samples.length-1]||null,r=Hc(e.windowHours);return`
    ...
    `}function d1(){let e=oo(),t=Wl();return`
    ...
    `}function m1(){if(!o.root||o.appView!=="diagnosis")return!1;let e=o.root.querySelector(".oq-overview-board");if(!e)return!1;let t=e.querySelector(".oq-overview-trends");return t?(Ze(t,$i(),Mi()),Pc(e),Xn(e),!0):!1}function Nc(e){if(!e)return null;let t=e.__oqTrendHoverNodes;if(t&&t.chart&&t.hoverLayer&&t.hoverPanel)return t;let r=e.querySelector("[data-oq-trend-hover-layer]"),n={chart:e.querySelector(".oq-overview-trend-chart"),hoverLayer:r,hoverPanel:e.querySelector("[data-oq-trend-hover]"),hoverTime:e.querySelector("[data-oq-trend-hover-time]"),hoverNote:e.querySelector("[data-oq-trend-hover-note]"),hoverValues:e.querySelector("[data-oq-trend-hover-values]"),hoverLine:r?r.querySelector(".oq-overview-trend-hover-line"):null,hoverDots:{}};return r&&r.querySelectorAll("[data-oq-trend-hover-dot]").forEach(a=>{n.hoverDots[a.getAttribute("data-oq-trend-hover-dot")]=a}),e.__oqTrendHoverNodes=n,n}function Kg(e,t,r,n=Nc(e)){if(!e||!t||!Array.isArray(t.points)||t.points.length===0)return;let a=Math.max(0,Math.min(t.points.length-1,r)),i=t.points[a];if(!i||!n||!n.chart||!n.hoverLayer||!n.hoverPanel||!n.hoverTime||!n.hoverNote||!n.hoverValues)return;let l=String(a);if(!n.hoverPanel.hidden&&e.dataset.oqTrendHoverIndex===l)return;let c=jq(i.sample.t,t.endTime);n.hoverPanel.hidden=!1,n.hoverLayer.removeAttribute("hidden"),n.hoverTime.textContent=c.value,n.hoverNote.textContent=c.note,n.hoverLine&&(n.hoverLine.setAttribute("x1",i.x.toFixed(1)),n.hoverLine.setAttribute("x2",i.x.toFixed(1)));let u=[];t.series.forEach(d=>{let g=Jn(d,i.sample),w=d.id||d.sampleKey||d.label,y=n.hoverDots[w];if(!Number.isFinite(g)){y&&y.setAttribute("display","none");return}let f=i.values.find(m=>m.seriesId===w);y&&f&&(y.removeAttribute("display"),y.setAttribute("cx",f.x.toFixed(1)),y.setAttribute("cy",f.y.toFixed(1))),u.push(`
        <div class="oq-overview-trend-hover-row oq-overview-trend-hover-row--${s(d.tone)}">
          <span>${s(d.label)}</span>
          <strong>${s(z(g,d.decimals,d.unit))}</strong>
        </div>
      `)}),n.hoverValues.innerHTML=u.join(""),e.dataset.oqTrendHoverIndex=l}function g1(e){if(!e)return;let t=Nc(e);t?.hoverPanel&&(t.hoverPanel.hidden=!0),t?.hoverLayer&&t.hoverLayer.setAttribute("hidden",""),delete e.dataset.oqTrendHoverIndex}function Xn(e=o.root){if(!e)return;e.querySelectorAll("[data-oq-trend-card]").forEach(r=>{let n=r.dataset.renderSignature||"";if(r.__oqTrendBoundSignature===n)return;typeof r.__oqTrendCleanup=="function"&&r.__oqTrendCleanup(),r.__oqTrendHoverNodes=null,r.__oqTrendBoundSignature=n;let a=r.querySelector(".oq-overview-trend-chart");if(!a)return;let i=a1(r.dataset.oqTrendCard);if(!i)return;let l=rf(i.samples,i.series,{mockData:i.mock});r.__oqTrendModel=l;let c=Nc(r),u=0,d=null,g=()=>{let f=d;d=null,u=0;let m=a.getBoundingClientRect();if(!m.width||!m.height)return;let p=Number(f?.clientX);if(!Number.isFinite(p)){Kg(r,l,l.points.length-1,c);return}let S=Math.min(m.width,Math.max(0,p-m.left))/m.width*l.width,q=i1(l,S);Kg(r,l,q,c)},w=f=>{d=f,u||(u=window.requestAnimationFrame(g))},y=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,g1(r)};a.addEventListener("pointermove",w),a.addEventListener("pointerenter",w),a.addEventListener("pointerleave",y),a.addEventListener("focus",w),a.addEventListener("blur",y),a.addEventListener("touchstart",w,{passive:!0}),r.__oqTrendCleanup=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,a.removeEventListener("pointermove",w),a.removeEventListener("pointerenter",w),a.removeEventListener("pointerleave",y),a.removeEventListener("focus",w),a.removeEventListener("blur",y),a.removeEventListener("touchstart",w)}})}po({patchDiagnosisDom:m1});function Wc(e,t,r){let n=Hi(L(t.mode,"Unknown")),a=C(t.defrost),i=uo(L(t.failures,"None")),l=Oa(i),c=n==="Verwarmen"||n==="Koelen"||a;return{mode:n,defrostActive:a,failures:i,warningFailures:l,running:c,thermalKey:n==="Koelen"?t.cooling:t.heat,schematic:E1(e,t,r,n,a,l,c)}}function lf(e,t=null){return`<h3>${s(e)}</h3>${t?`<button class="oq-overview-hp-card-action" type="button" data-oq-action="select-hp-layout" data-hp-layout="${s(t.layout)}">${A1(t.layout==="equal"?"minus":"plus")}<span>${s(t.label)}</span></button>`:""}`}function _c(e,t,r,n){return`<div class="oq-overview-hp-status">${qc(e,t,r,n)}</div>`}function f1(e,t){return t?"ontdooit":e==="Verwarmen"?"verwarmt":e==="Koelen"?"koelt":e==="Stand-by"?"stand-by":"onbekend"}function cf(e){return!Array.isArray(e)||e.length===0?"":`<p class="oq-overview-hp-summary">${s(e.map(t=>`${t.title} ${f1(Hi(L(t.keys.mode,"Unknown")),C(t.keys.defrost))}`).join(", "))}</p>`}function h1(e){let t=o.entities[e];if(!t)return"Positie: \u2014";let r=R(e);return Number.isNaN(r)?`Positie: ${L(e)}`:`Positie: ${z(r,0,t.uom||"")}`}function b1(e){return v(e)?`Positie: ${C(e)?"Koelen/Defrost":"Verwarmen"}`:"Positie: \u2014"}function Hi(e){let t=String(e||"").trim();return!t||t==="Unknown"?"Onbekend":t==="Standby"?"Stand-by":t==="Heating"?"Verwarmen":t==="Cooling"?"Koelen":t}function Oc(e,t,r,n=!0,a="default"){return`
    ...
    `}function Lc({bind:e,x:t,y:r,width:n,value:a,label:i,ariaLabel:l="",align:c="start"}){let u=l||`${i} temperatuur ${a}`,d=c==="end",g=c==="center",w=g?"middle":d?"end":"start",y=g?t+n/2:d?t+n-2:t+2;return`
      <g
    ...
    `}function w1({tooltip:e,...t}){return`${Lc(t)}${ea({bind:t.bind,...e})}`}function v1({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
      ...
      ${ea({bind:e,...c})}
    `}function Dc({bind:e,className:t,active:r,ariaLabel:n,attrs:a="",activeClass:i="is-active",content:l,tooltip:c}){return`
      ...
      <div class="${s(e.boardClass)}" data-oq-hp-board="${s(e.title)}">
        <div class="oq-hp-tech-shell">
          <div class="oq-hp-tech-visual">
            ...
            ${Object.entries(e.pipes).map(([d,g])=>Oc(d.replace(/[A-Z]/g,w=>`-${w.toLowerCase()}`),g.tone,g.d,g.animated,g.flowVariant)).join("")}
            ...
            ${c.map(w1).join("")}
            ${Dc({bind:"bottom-heater",className:"oq-hp-tech-bottom-heater",active:e.bottomPlateActive,ariaLabel:"Bottom plate heater actief",content:`
            ...
            ${Dc({bind:"crankcase-heater",className:"oq-hp-tech-crankcase-heater",active:e.crankcaseActive,ariaLabel:"Crank case heater actief",content:`
            ...
            ${Dc({bind:"defrost-badge",className:"oq-hp-tech-defrost-badge",active:e.defrostActive,activeClass:"",ariaLabel:e.defrostActive?"Defrost actief":"Defrost uit",attrs:'transform="translate(532 288)"',content:`
    ...
    `}function uf(e,t,r,n="normal",a=null){if(!v(t.power))return"";let i=Wc(e,t,r),{mode:l,defrostActive:c,running:u,thermalKey:d}=i,g=i.schematic;return o.hpVisualMode==="schematic"?`
        <section class="oq-overview-hp oq-overview-hp--${s(r)} oq-overview-hp--${s(n)}" data-oq-hp-panel="${s(e)}">
          <div class="oq-overview-hp-head">
            ...
            <div class="oq-overview-hp-head-side">
              ${_c(l,u,g.warningActive,g.failureText)}
            </div>
          </div>
          ${T1(g)}
        </section>
    ...
    `}function Ic(){if(!Vc())return"";let e=Ri();return o.hpVisualMode!=="schematic"?M1(e):`
          ...
          <div class="oq-boiler-card-main">
            ...
            <div class="oq-boiler-mini-schematic">
              <svg class="oq-boiler-mini-svg" viewBox="0 0 420 132" role="img" aria-label="Schematische weergave CV-ketel of boiler">
                ...
                ${Lc({bind:"boiler-return",x:22,y:70,width:78,value:e.returnTempText,label:"Retour",ariaLabel:`Retour ${e.returnTempText}`,align:"start"})}
                ...
                ${Lc({bind:"boiler-supply",x:320,y:16,width:76,value:e.supplyTempText,label:"Aanvoer",ariaLabel:`Aanvoer ${e.supplyTempText}`,align:"end"})}
    ...
    `}function oa(){let e=typeof ne=="function"?ne():"";return va.filter(t=>e==="single"&&t.title==="HP2"?!1:v(t.keys.power))}function Bc(e){return!Array.isArray(e)||e.length<2||o.hpVisualMode!=="schematic"?"equal":o.hpLayoutMode==="focus-hp1"||o.hpLayoutMode==="focus-hp2"?o.hpLayoutMode:"equal"}function Ai(e,t,r){return!Array.isArray(t)||t.length<2?"normal":r==="focus-hp1"?e===0?"focus":"muted":r==="focus-hp2"?e===1?"focus":"muted":"normal"}function Fc(e,t,r){return!Array.isArray(t)||t.length<2||o.hpVisualMode!=="schematic"?null:Ai(e,t,r)==="focus"?{layout:"equal",label:"Toon beide"}:{layout:e===0?"focus-hp1":"focus-hp2",label:"Vergroot"}}function df(e){return!Array.isArray(e)||e.length!==1?Bc(e):Vc()?"equal":"single"}function A1(e="plus"){return`
    ...
    `}function H1(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(Ot(e,pf(t)),!0):(Ot(r,`
      <h3>Warmtepompen</h3>
      ${cf(t)}
    `),n.classList.toggle("is-active",o.hpVisualMode==="schematic"),a.classList.toggle("is-active",o.hpVisualMode==="compact"),!0)}function mf(){let e=kc(),t=oa(),r=Bc(t),n=df(t),a=pf(t);return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
          ${vc()}
          ${ef(e)}
    ...
    `}function kk(e){let t=Xc(e.severity);return`
    ...
    `}function Ek(e,t){let r=Xc(e.severity),n=t&&t.id===e.id,a=Af(e.kind),i=Mf(e);return`
      <button
    ...
    `}function Wf(e){if(!e)return"";let t=Xc(e.severity),r=cr(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=Hf(e),l=Mf(e);return`
      ...
      <div class="oq-working-split">
        <section class="oq-working-list" aria-label="${s(r.eyebrow)}">
          ...
          ${a.length?`<div class="oq-working-timeline">
                ${a.map(c=>Ek(c,t)).join("")}
              </div>`:i?Oi("Beslislog niet beschikbaar",`De firmwarelog kon niet worden geladen (${i}). Dit betekent niet dat deze periode leeg is.`):l?Oi("Beslislog laden","De controllerkeuzes worden opgehaald. Dit duurt meestal maar heel kort."):Oi("Nog geen gebeurtenissen","De beslislog is leeg voor deze periode. Nieuwe controllerkeuzes verschijnen hier zodra de firmware ze vastlegt.")}
        </section>
        ${t?Wf(t):""}
      </div>
    `}function Kc(e,t,r,n,a){return`
      ...
      <div class="oq-working-status">
        ...
        <div class="oq-working-status-grid">
          ...
          <section class="oq-working-source-grid" aria-label="Bronnen">
            ${Kc("HP1",e.hp1Status,e.hp1Starts,e.hp1Hours,e.hp1Running)}
            ${Kc("HP2",e.hp2Status,e.hp2Starts,e.hp2Hours,e.hp2Running)}
            ${Kc("CV",e.cvStatus,"\u2014","\u2014",e.cvStatus==="Actief")}
          </section>
    ...
    `}function Nk(){return ee({modalId:"initial-load",titleId:"oq-loading-modal-title",kicker:"OpenQuatt",title:"OpenQuatt laden",backdropClass:"oq-helper-modal-backdrop--loading",modalClass:"oq-helper-modal--reconnect oq-helper-modal--loading",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `})}function Dk(){return o.appView==="overview"?mf():o.appView==="control"?Kf():o.appView==="energy"?Wg():o.appView==="diagnosis"?af():o.appView==="results"?Vg():Pk()}function _k(){let e=Yl();return`
    ...
    `}function Ok(){let e=typeof document<"u"?document.activeElement:null;return!e||typeof e.matches!="function"?null:e.matches("select[data-oq-dev-control]")?e:null}function Lk(e){if(!e||o.deferDevControlSelectRender)return;o.deferDevControlSelectRender=!0;let t=()=>{e.removeEventListener("blur",t),e.removeEventListener("change",t),o.deferDevControlSelectRender=!1,window.setTimeout(()=>zf(),0)};e.addEventListener("blur",t,{once:!0}),e.addEventListener("change",t,{once:!0})}function Ik(){if(o.nativeOpen||o.appView!=="settings"||o.renderedAppView!=="settings"||o.renderedSettingsGroup!==o.settingsGroup)return null;let e=document.scrollingElement||document.documentElement,t=Number(window.scrollY||e?.scrollTop||0);return!Number.isFinite(t)||t<=0?null:{group:o.settingsGroup,left:Number(window.scrollX||e?.scrollLeft||0),top:t}}function Fk(e){if(!e)return;let t=(o.settingsPageScrollRestoreToken||0)+1;o.settingsPageScrollRestoreToken=t;let r=()=>{if(t!==o.settingsPageScrollRestoreToken||o.nativeOpen||o.appView!=="settings"||o.settingsGroup!==e.group)return;let n=document.scrollingElement||document.documentElement;if(!n)return;let a=Math.max(0,n.scrollHeight-n.clientHeight),i=Math.min(e.top,a);window.scrollTo({left:e.left,top:i,behavior:"auto"})};window.requestAnimationFrame(()=>{r(),window.requestAnimationFrame(r),window.setTimeout(r,80)})}function zf(){if(!o.root)return;let e=Ok();if(e){Lk(e);return}let t=o.systemModal==="webserver-logs"?Jt():null,r=o.systemModal==="cm100-commissioning"?vp():null,n=String(o.systemModal||"").startsWith("service-task-")?qp():null,a=o.systemModal==="history-storage"?Ep():null,i=o.quickStartModalOpen?bg():null,l=Ik();if(o.nativeOpen){o.root.innerHTML=`
        ${Jl()}
        ${Gm()}
      `,qs(o.root),o.renderedAppView="native",o.renderedSettingsGroup="",o.settingsRenderSignature="",o.headerRenderSignature=fi(),vr({mqttSensorsModalRenderSignature:""}),wn(),_n(),hl(),ps(),ds(),Zt(t),ml(r),gl(n),fl(a),cc(i);return}let c=Dk(),u=o.loadingEntities?`${c}${Nk()}`:c,d=o.appView==="overview"||o.appView==="control"||o.appView==="energy"||o.appView==="diagnosis"||o.appView==="results";o.root.innerHTML=`
      ${Jl()}
