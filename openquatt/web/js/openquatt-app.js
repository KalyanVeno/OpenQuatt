        ...
        <article class="oq-settings-mqtt-sensor-row${d?" is-open":""}${g?"":" is-disabled"}">
          ...
          >
            ...
            <span class="oq-settings-mqtt-sensor-value">
              ${g?Hw(l):'<span class="oq-settings-mqtt-sensor-value-missing">\u2014</span>'}
            </span>
...
`)),r.toString()}function wl(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=wy(l);if(!c)return;let u=[...a,c],p=qp(u,t);if(a.length&&p.length>ly){r.push({keys:n,body:i}),n=[l],a=[c],i=qp(a,t);return}n=[...n,l],a=u,i=p}),a.length&&r.push({keys:n,body:i}),r}async function yy(e){let t=hl(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await qn(sy,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function vy(){let e=hl(),r=await qn(cy,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function Sy(){let e=hl(),r=await qn(uy,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function qy(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.hour_start_epoch_s??"",i.source??"",i.attention_count??"",e?.meta?.flash_write_count??""].join("|")}async function ky(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<dy)return!1;o.decisionLogFetchPromise=(async()=>{let n=await Sy(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=qy(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function Ey(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{hn.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=Pp(r,o.entities[r],n))})}async function Ty(e){if(!e.length)return{ok:!0,message:""};try{let t=await vy();return Ey(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function K(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||hn.has(b)||!hy(b,n));if(!i.length)return;let l=i.filter(b=>hn.has(b)),c=i.filter(b=>!hn.has(b)),p=(await Ty(l)).ok?c:[...c,...l];if(!p.length){kp(),Hn(eo()),o.busyAction||(Sp(),o.controlError="");return}let g=Number(r.concurrency),w=Number.isFinite(g)&&g>0?Math.floor(g):Ut,y=wl(p,t),h=Math.max(1,Math.min(w,Ut)),m=[];for(let b=0;b<y.length;b+=h){let S=y.slice(b,b+h),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await yy(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let d="";m.forEach(b=>{if(b.status!=="fulfilled"){let M=b.reason.message||String(b.reason);d||(d=M);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(M=>{if(Object.prototype.hasOwnProperty.call(E,M)){o.optionalMissingEntities&&delete o.optionalMissingEntities[M],o.entities[M]=Pp(M,o.entities[M],E[M]);return}let P=O[M];P?.optional?k.has(M)&&by(M,n):d||(d=`${P?.name||M} ontbreekt in bulk response`)})}),kp(),Hn(eo()),d?(Rp(d),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${d}`):o.busyAction||(Sp(),o.controlError="")}function Pp(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function kp(){o.complete=Na(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=ud(),o.appView==="diagnosis"&&!yt()&&Nt(yo(),{syncMode:"replace",forceSync:!0}),o.appView||Nt($n()||yo(),{syncMode:"replace",forceSync:!0})}function Cy(){let e=dl();return o.appView==="settings"?[...new Set([...e,...pl()])]:o.appView==="energy"||o.appView==="results"?Rn(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?Rn(o.appView,{forceFast:!0}):[...new Set(e)]}function xy(e=[]){let t=new Set(e);return(o.appView==="settings"?pl():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...Rn(o.appView,{includeBulk:!0}),...Qt])]:o.appView==="energy"||o.appView==="results"?[...new Set([...Rn(o.appView,{forceFast:!0}),...Qt])]:[...new Set(["setupComplete","strategy",...nt])]).filter(n=>!t.has(n))}function Ep(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=zu}async function ye(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||ja()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){ty(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<yn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&Ep(t,e),p=t-Number(o.lastStaticEntitySyncAt||0)>=Uu||o.updateInstallBusy||o.updateInstallPhaseHint?Qt:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?wt:[],w=o.quickStartModalOpen&&o.currentStep==="setup"?Yt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Io:[],h=el()?to():[],m=n?[...bs,...nt,"setupComplete",...p]:i?[...Rn(a,{forceFast:l,includeBulk:c}),...p]:r==="settings"?[...new Set([...pl(),...h,...p])]:c?["setupComplete",...p,...nt,"strategy",...Oo,...rt,...ae()?he.map(d=>d.key):At]:["setupComplete",...nt,"strategy",...p];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let d=o.deviceReconnectMode,b=gy(t,e)?await fy():{ok:!0,message:""};if(!b.ok){Rp(b.message),n||f();return}if(await K([...new Set([...m,...w,...g,...y])],n?"state":r==="settings"||w.length?"all":"state",{concurrency:l&&i?Gt:Ut}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),p.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&Cp();let S=d!==o.deviceReconnectMode,q=l&&i,E=q?!1:a==="overview"||a==="control"||a==="diagnosis"?await tl():!1,k=q?!1:o.appView==="results"?await Va():!1,M=a==="control"?await ky({force:e.forceDecisionLog===!0}):!1,P=q?!1:el()?(await Promise.all([ep(),Va({metaOnly:!0})])).some(Boolean):!1,N=q||!kd()?!1:await zo(),V=q||!Ed()?!1:await Uo(),H=q||!fp()?!1:await Jo({force:o.systemModal==="mqtt-sensors"}),F=Yd();if(q&&!o.nativeOpen&&Ap(ml(a)),S){f();return}if(E&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){f();return}if(k&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){f();return}if(M&&o.appView==="control"){ol()||f();return}if(P&&o.appView==="settings"){f();return}if(N&&(o.systemModal==="login"||Zt())){f();return}if(V&&o.appView==="settings"){f();return}if(o.systemModal==="mqtt-sensors"){Ka()!==o.mqttSensorsModalRenderSignature&&f();return}if(H&&o.appView==="settings"){f();return}let A=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(F!==o.headerRenderSignature)if(A&&Ys())o.headerRenderSignature=F;else{f();return}else Ys();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){Tn()!==o.settingsRenderSignature&&f();return}if(A)return;if(o.appView==="settings"){let L=Tn();if(L!==o.settingsRenderSignature&&!o.focusedField){f();return}if(!Zd()){o.focusedField||f();return}o.focusedField&&(o.settingsRenderSignature=L);return}if(o.appView==="diagnosis"){ap()||f();return}if(o.appView==="control"){ol()||f();return}if(o.appView==="energy"){op()||f();return}if(o.appView==="results"){rp()||f();return}np()||f()}catch(d){n||(o.controlError=`Helperstatus kon niet worden geladen. ${d.message}`,f())}finally{o.entitySyncInFlight=!1;let d=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,d&&!o.nativeOpen&&window.setTimeout(()=>{ye(d)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!d&&Ep(Date.now())&&window.setTimeout(()=>{ye({forceBulk:!0})},Xw)}}function _t(){return{}}function My(){let e=String(T("hpGeneration")||"").trim();return e||""}function Xo(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function $y(){if(!Array.isArray($r))return"";if($r.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return $r.every(t=>e[t])?"single":""}function yl(e){let t=Xo(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Ay(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function oe(){let e=Xo(T("installationTopology"));if(e==="single"||e==="duo")return yl(e);let t=Xo(_t().installation);if(t==="single"||t==="duo")return yl(t);let r=$y();return r?yl(r):Ay()}function Lt(){let e=oe(),t=My();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Ye(){return"OpenQuatt"}function Pn(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function it(e=st()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function er(e=oe()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function _r(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(_t().hardwareProfile||e).trim().toLowerCase()}function st(){return Pn(T("connectionText")||_t().connection)}function Np(){let e=st();return e==="wifi"?"eth":e==="eth"?"wifi":""}function Dp(){let e=oe();return e==="single"?"duo":e==="duo"?"single":""}function qo(e=oe(),t=st()){let r=er(e),n=_r();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${it(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${it(t)}`:n==="waveshare"?`Waveshare ${r} ${it(t)}`:`${Ye()} ${r} ${it(t)}`}function vl(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function za(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${vl()}`}function Hy(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Ry(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Lr(){let e=Ry(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Hy(o.entities.uptime);if(n==="d")return Ko(e*1440);if(n==="h")return Ko(e*60);if(n==="s")return Ko(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(_t().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Ko((Date.now()-r)/6e4)}function Or(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(_t().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function ko({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let y=e();if(!y)return null;let h={scrollTop:y.scrollTop};if((i||l)&&(h.scrollHeight=y.scrollHeight),l){let m=y.scrollHeight-y.scrollTop-y.clientHeight;h.stickToBottom=m<c}return a&&(h.identity=a(y)),h}function p(y){if(!y)return;let h=e();if(!h||a&&a(h)!==y.identity)return;if(y.stickToBottom){h.scrollTop=h.scrollHeight;return}let m=i?h.scrollHeight-Number(y.scrollHeight||0):0;h.scrollTop=Math.max(0,Number(y.scrollTop||0)+m)}function g(y,h=!0){if(!y)return;let m=Number(t()||0)+1;r(m);let d=()=>{Number(t()||0)!==m||!n()||p(y)};if(h){window.requestAnimationFrame(d);return}d()}function w(){r(Number(t()||0)+1)}return{capture:u,invalidate:w,queue:g,restore:p}}function Ae(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function Eo({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function kl(e,t,r,n=""){if(!v(e))return"";let a=vn(T(e));return ve(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function re(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var Ip=250;function Fp(){return[]}function ar(){return typeof window>"u"?!1:Fp().length>0}function Ny(){return`${Pe()}/events`}function Dy(){return`${Pe()}/openquatt/logs/recent`}function _y(){return`${Pe()}/openquatt/logs/clear`}function Qa(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Ya(){return o.nativeOpen?"Niet beschikbaar":ar()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function Ly(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function jp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return Ly(t)}function Oy(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Iy(){return o.nativeOpen?"Niet beschikbaar":ar()?Qa()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":Qa()?"Buffer aan":"Buffer uit"}function Fy(){return Qa()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function Tl(){return o.entities?.debugLevel||null}function Wp(e=Tl()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function jy(e=Tl()){let t=String(e?.value??e?.state??"").trim(),r=Wp(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function Vp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function Wy(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function El(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(Wy);let n=[];for(let a of r){let i=n[n.length-1]||null;Vp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-Ip)}function Cl(e,t={}){let r=cv(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:uv(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function Vy(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function By(){let e=Fp(),t=e.length;return e.map((r,n)=>Cl(r,{receivedAt:Vy(n,t),seq:n+1}))}function Ky(){let e=Pl();e&&(e.scrollTop=e.scrollHeight)}var xl=ko({getScroller:Pl,getToken:()=>o.webServerLogScrollRestoreToken,setToken:e=>{o.webServerLogScrollRestoreToken=e},isActive:()=>o.systemModal==="webserver-logs",preserveGrowth:!0,stickToBottom:!0}),no=xl.capture,ao=xl.queue;function zy(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}var Bp=ko({getScroller:zy,getToken:()=>o.cm100CommissioningScrollRestoreToken,setToken:e=>{o.cm100CommissioningScrollRestoreToken=e},isActive:()=>o.systemModal==="cm100-commissioning",preserveGrowth:!0,stickToBottom:!0}),Kp=Bp.capture,Ml=Bp.queue;function Uy(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}var zp=ko({getScroller:Uy,getToken:()=>o.serviceTaskModalScrollRestoreToken,setToken:e=>{o.serviceTaskModalScrollRestoreToken=e},isActive:()=>String(o.systemModal||"").startsWith("service-task-")}),Up=zp.capture,$l=zp.queue;function Gy(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}var Gp=ko({getScroller:Gy,getToken:()=>o.historyStorageModalScrollRestoreToken,setToken:e=>{o.historyStorageModalScrollRestoreToken=e},isActive:()=>o.systemModal==="history-storage"}),Qp=Gp.capture,Al=Gp.queue;function Qy(){return o.root?o.root.querySelector("[data-oq-settings-backup-restore-scroller]"):null}var Yp=ko({getScroller:Qy,getToken:()=>o.settingsBackupRestoreScrollRestoreToken,setToken:e=>{o.settingsBackupRestoreScrollRestoreToken=e},isActive:()=>o.systemModal==="settings-backup-restore"}),Jp=Yp.capture,Hl=Yp.queue;async function xo(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||no(),r=e.replaceEntries===!0||o.webServerLogHistoryNeedsReconcile===!0,n=r?new Set(o.webServerLogEntries):null,a=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=a,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let i=await window.fetch(Dy(),{headers:{"Cache-Control":"no-store"}});if(!i.ok)throw new Error(`HTTP ${i.status}`);let l=await i.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==a)return;o.webServerLogCsrfToken=String(l.csrf_token||"");let c=Jy(l),u=r?o.webServerLogEntries.filter(p=>!n.has(p)):[];r&&(o.webServerLogEntries=[],o.webServerLogRecentTail=[],o.webServerLogRecentAnchorAt=0),o.webServerLogHistoryLoaded=!0,o.webServerLogHistoryNeedsReconcile=!1,c.length>0&&(El(c,{prepend:!0}),o.webServerLogRecentTail=c.slice(-4).map(p=>String(p.raw??p.text??"")),o.webServerLogRecentAnchorAt=Date.now()),u.length>0&&El(u)}catch(i){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===a&&(o.webServerLogHistoryError=i instanceof Error?i.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===a&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===a&&(f(),ao(t))}}function Yy(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?Cl(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function Jy(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>Yy(r,n+1)).filter(r=>r!==null)}function Zy(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function Xy(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>Vp(e,r))}function ev(){ar()&&o.webServerLogEntries.length===0&&Qe({webServerLogEntries:By()}),Qe({webServerLogCopyMessage:"",webServerLogCopyError:""}),o.systemModal="webserver-logs",f(),K(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=no();f(),ao(e)}),Ky(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&xo()}function nr(){Qe({webServerLogEntries:[],webServerLogError:"",webServerLogHistoryError:"",webServerLogHistoryLoading:!1,webServerLogHistoryLoaded:!1,webServerLogHistoryNeedsReconcile:!1,webServerLogCopyMessage:"",webServerLogCopyError:"",webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1,webServerLogRecentTail:[],webServerLogRecentAnchorAt:0}),xl.invalidate(),o.systemModal==="webserver-logs"&&f()}async function tv(){if(o.busyAction)return!1;if(o.nativeOpen||ar())return nr(),!0;if(typeof window.fetch!="function")return o.webServerLogHistoryError="De RAM-logbuffer kan niet vanuit deze browser worden geleegd.",f(),!1;let e=String(o.webServerLogCsrfToken||"");if(!e)return o.webServerLogHistoryError="De beveiligingstoken voor de RAM-logbuffer ontbreekt. Open het logboek opnieuw.",f(),!1;o.busyAction="clear-webserver-log-history",o.webServerLogHistoryError="",kt(),f();let t=!1;try{let n=e,a=!1;for(;;){let i=new URLSearchParams;i.set("csrf_token",n);let l=await window.fetch(_y(),{method:"POST",body:i});if(!(l.status===403&&!a&&(a=!0,Qe({webServerLogCsrfToken:"",webServerLogHistoryLoaded:!1,webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1}),await xo(),n=String(o.webServerLogCsrfToken||""),n))){if(!l.ok)throw new Error(`HTTP ${l.status}`);break}}nr(),t=!0}catch(n){Qe({webServerLogHistoryLoaded:!1,webServerLogHistoryNeedsReconcile:!0}),o.webServerLogHistoryError=`De RAM-logbuffer kon niet worden geleegd (${n instanceof Error?n.message:"onbekende fout"}).`}finally{o.busyAction="",o.systemModal==="webserver-logs"&&f()}let r=o.webServerLogHistoryError;return o.systemModal==="webserver-logs"&&(await xo(),r&&(o.webServerLogHistoryError=r,f())),t}function ov(){let e=no();kt(),Qe({webServerLogEnabled:null,webServerLogConnected:!1,webServerLogCsrfToken:""}),nr(),o.systemModal==="webserver-logs"&&xo({scrollState:e})}function Rl(){if(ar()){kt();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs"&&o.busyAction!=="clear-webserver-log-history")){kt();return}if(o.webServerLogEnabled===!1){kt();return}o.webServerLogSource||rv()}function rv(){if(ar()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",f();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",f();return}try{let e=new window.EventSource(Ny());o.webServerLogSource=e,e.addEventListener("open",nv),e.addEventListener("ping",av),e.addEventListener("log",sv),e.onerror=iv}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",kt(),f()}}function kt(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function nv(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?no():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",f(),ao(e)}ip({clearOutput:nr,closeStream:kt,resetRecoveryState:ov});function av(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?no():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",f(),ao(e))}function iv(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?no():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",kt(),f(),ao(e)}function sv(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=no(),r=lv(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>Cl(u)).filter(u=>!Zy(u)&&!Xy(u));if(i.length===0)return;El(i);let l=pv(),c=Pl();dv(l),mv(i,l),o.webServerLogEnabled=!0,c&&t&&ao(t,!1)}function lv(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).trimEnd()}catch{}return t}function cv(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function uv(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function dv(e){for(;o.webServerLogEntries.length>Ip;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function pv(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function Pl(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function mv(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",Zp(r))}}function Zp(e){let t=jp(e.receivedAt),r=Oy(e.receivedAt);return`
  ...
  `}function bv(){let e=Tl();if(!e)return"";let t=Wp(e),r=jy(e),n=o.loadingEntities||!!o.busyAction;return`
...
`)}async function yv(){let e=wv();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",f();return}try{if(!await ho(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&f()}var vv={"open-webserver-log-modal":()=>ev(),"clear-webserver-log-output":()=>tv(),"copy-webserver-log-output":()=>yv()};function Xp(e){return ie(vv,e)}function em(){let e=ar(),t=o.busyAction==="clear-webserver-log-history",r=!!o.busyAction||o.webServerLogHistoryLoading||!e&&!o.nativeOpen&&!o.webServerLogCsrfToken;return te({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
    ...
    `}function Pv(e,t,r,n){if(!lm())return"";let i=!!(Et()||o.updateInstallBusy||ct());return`
    ...
    `}function Nv(){let e=Za();if(!e||!o.firmwareConnectionSwitchOpen)return"";let r=!!(Et()||o.updateInstallBusy||ct()),n=!!o.firmwareConnectionSwitchConfirmed,a=e.targetConnection==="eth",i=!e.canSwitch,l=a?"Sluit eerst de netwerkkabel aan. Na de herstart verdwijnt Wi-Fi uit deze firmware.":"Na de herstart verdwijnt Ethernet uit deze firmware. Als er geen Wi-Fi-gegevens bekend zijn, start het OpenQuatt fallback access point.",c=i?'<p class="oq-helper-modal-note oq-helper-modal-note--muted">Verbindingswissel wordt geladen. Open deze modal opnieuw of wacht een moment als de knop disabled blijft.</p>':"";return`
    ...
    `}function _v(){if(!o.updateTestFirmwareOpen)return"";let t=!!(Et()||o.updateInstallBusy||ct()),r=sr(),n=jr(),a=Nn(r,n),i=!!(n.available&&v("firmwareTestOtaUrl")&&v("firmwareTestOtaMd5Url")&&v("installFirmwareTestOta")),l=!!(r&&i),c=o.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,p=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Lv(){if(!o.updateManualUploadOpen)return"";let t=!!(Et()||o.updateInstallBusy||ct()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
  ...
  `}function jv(){if(!o.root)return;if(o.interfacePanelOpen){f();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=jl();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function Wv(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=In()),r&&(r.textContent=ni())}function Wl(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function Vv(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${Wl(e)}</span>`}function so(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${Wl(e)}</span>`}function Bv(){return[...Wo]}function Kv(){let e=[];return Wo.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function zv(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function ai(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function pr(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function Ao(e){return`${Pe()}/openquatt/debug-recording/${e}`}function Vr(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Vl(){Vr({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function Uv(){let e=await window.fetch(Ao("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),t}function Fn(e=2e3){pr(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{ii({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function ii(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",f());try{await Uv(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),Fn()}catch(t){Vl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?f():(jv(),Wv())}}async function bm(){let e=wl(Wo,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch(Ao(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==Wo.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${Wo.length})`);return t}async function Gv(e){let t=Math.max(1,Number(e)||15);ai(),pr(),_a({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),f();try{await bm();let r=await window.fetch(Ao(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Vr(n),Fn()}catch(r){Vl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,f()}}async function Qv(){ai(),pr(),_a({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),f();try{await bm();let e=await window.fetch(Ao("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Vr(t),Fn()}catch(e){Vl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,f()}}async function Yv(){let e=await window.fetch(Ao("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),pr(),t}async function Jv(){ai(),o.debugRecordingBusy=!0,o.debugRecordingError="",f();try{await Yv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,f()}}async function Zv(e={}){ai(),pr(),o.debugRecordingBusy=!0,o.debugRecordingError="",f();try{let t=await window.fetch(Ao("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Vr(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,f()}}function um(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(ju/1e3),sample_count:It(),column_count:Wo.length,storage:"browser"},columns:Bv(),units:Kv(),initial:zv(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function oi(e){return JSON.stringify(e)}function Xv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([oi(um())]).size}catch{return oi(um()).length}}function eS(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function tS(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function oS(){if(It()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",f();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Ft();f();try{let t=await window.fetch(Ao("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,$s(tS(r),oi(r),"application/json"),hm(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,f()}}async function rS(){if(It()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",f();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Ft();f();try{let t=await window.fetch(Ao("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await ho(oi(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");hm(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,f()}}var nS={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",f(),ii()),"start-debug-recording":e=>Gv(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>Qv(),"select-debug-recording-duration":e=>Iv(e.dataset.debugMinutes||15),"stop-debug-recording":()=>Zv(),"freeze-debug-recording":()=>Jv(),"download-debug-recording":()=>oS(),"copy-debug-recording":()=>rS()};function wm(e,t){return ie(nS,e,t)}function ym(){let e=o.debugRecordingActive,t=Ft(),r=ri(),n=It(),a=o.debugRecordingBusy,i=eS(Xv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Ov(),u=gm(),p=dm(),g=Fv(),w=n>0,y=`${g.toFixed(1)}%`,h=[{icon:"status",label:"Status",value:In()},{icon:"clock",label:t?"Retentie":"Duur",value:dr(t?p:pm())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],m=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return te({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:ni(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
    ...
    `}function xm(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return te({id:"system",titleId:"oq-api-security-modal-title",kicker:"Toegang",title:bS(),copy:wS(),className:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit API-beveiliging popup",body:`
      ...
      `;return te({id:"system",titleId:"oq-login-modal-title",kicker:"Systeem",title:fS(),copy:hS(),closeAction:"close-system-modal",closeLabel:"Sluit login-popup",body:`
        ...
        ${n?`<button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="save-web-auth" ${o.authBusy?"disabled":""}>${t?"Opslaan":"Login opslaan"}</button>`:""}`})}function Kn(e,t,r){let n=R(t),a=R(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=R(e);return Number.isFinite(i)?i:NaN}function Ul(e="oq-settings-grid"){let t=SS();return`
      <div class="${s(e)}">
    ...
    `}function Hm(){let e=v("boilerCvAssistEnabled")&&C("boilerCvAssistEnabled"),t=Ua(),r=C("cm100Active"),n=String(t||"").trim().toUpperCase(),a=mr(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,u=!!o.pendingCommissioningCm100Start,p=c||i||a,g=c||!i,w=or("boilerPowerTestStatus","IDLE"),y=ui(w,"boiler"),h=C("boilerPowerTestActive"),m=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",d=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),b=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=mr(w),k=!Ce(w)&&(h||b||S||zr(w))&&!q,M=He("boilerRatedHeatPower"),P=He("boilerHeatPower"),N=R("boilerHeatPower"),V=N>0?P:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:P;N>0&&(o.commissioningBoilerHeatPowerDisplay=P);let H=or("flowAutotuneStatus","IDLE"),F=ui(H,"autotune"),A=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",L=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),_=!!o.pendingFlowAutotuneStart,D=o.commissioningTaskLock==="autotune",Z=mr(H),W=!Ce(H)&&(_||D||zr(H))&&!Z,ee=or("airPurgeStatus","IDLE"),De=ui(ee,"purge"),xt=C("airPurgeActive"),gt=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",zt=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),et=!!o.pendingAirPurgeStart,_e=o.commissioningTaskLock==="purge",x=!Ce(ee)&&(xt||et||_e||zr(ee)),B=/DONE/.test(String(ee||"").toUpperCase()),Q=!!(zt||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),G=He("airPurgeRemaining",{decimals:0}),le=R("airPurgePhase"),Se=le===1?"Rustig":le===2?"Pulsen":le===3?"Stabiliseren":De.phase,ue=or("manualFlowStatus","IDLE"),xe=C("manualFlowActive"),ge=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",ft=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),Mt=!!o.pendingManualFlowStart,pe=o.commissioningTaskLock==="manual-flow",Ve=!Ce(ue)&&(xe||Mt||pe||zr(ue)),tt=or("manualHpStatus","IDLE"),ht=C("manualHpActive"),bt=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",_o=!!(o.entities.manualHpStart||o.entities.manualHpAbort),qe=!!o.pendingManualHpStart,$t=o.commissioningTaskLock==="manual-hp",ot=!Ce(tt)&&(ht||qe||$t||zr(tt)),Cu=/SAFETY STOP/.test(String(tt||"").toUpperCase()),xu=/STOPPING/.test(String(tt||"").toUpperCase()),po=or("hpWaterCalibrationStatus","IDLE"),hb=ui(po,"hp-water-calibration"),Mu=C("hpWaterCalibrationActive"),ba=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",$u=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),wa=!!o.pendingHpWaterCalibrationStart,mo=o.commissioningTaskLock==="hp-water-calibration",Ue=!Ce(po)&&(Mu||wa||mo||zr(po)),ya=/DONE|APPLIED/.test(String(po||"").toUpperCase()),Au=/APPLIED/.test(String(po||"").toUpperCase()),bb=He("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),wb=He("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),Hu=/DONE|APPLIED/.test(String(w||"").toUpperCase()),Ru=/DONE|APPLIED/.test(String(H||"").toUpperCase()),rs=i?q?"Wachten op CM100":k?y.phase:Hu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",ns=i?Z?"Wachten op CM100":W?F.phase:Ru?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",as=i?x?De.phase:B?"Klaar":"Klaar om te starten":"Wachten op CM100",Pu=i?Ve?"Actief":"Klaar om te starten":"Wachten op CM100",Nu=i?ot?xu?"Bezig met stoppen":Cu?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",Du=i?Ue?hb.phase:Au?"Offsets toegepast":ya?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",_u=!i||m||!d||W||x||Ve||ot||Ue||k||D||_e||pe||$t||mo||b,yb=m||!(k||S||b),vb=m||_u||!Hu||W||x||Ue,Lu=!i||A||!L||k||x||Ve||ot||Ue||W||S||_e||pe||$t||mo||_,Sb=A||!(W||D||_),qb=A||Lu||!Ru||k||x||Ue,kb=!i||gt||!zt||k||W||Ve||ot||Ue||x||S||D||pe||$t||mo||et,Eb=gt||!(x||_e||et),Tb=!i||ge||!ft||k||W||x||ot||Ue||Ve||S||D||_e||$t||mo||Mt,Cb=ge||!(Ve||pe||Mt),xb=!i||bt||!_o||k||W||x||Ve||Ue||ot||S||D||_e||pe||mo||qe,Mb=bt||!(ot||$t||qe),$b=!i||ba||!$u||k||W||x||Ve||ot||Ue||S||D||_e||pe||$t||wa,Ab=ba||!(Ue||mo||wa),Hb=ba||Ue||!ya||Au;u&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),b&&(h||Ce(w))&&(o.pendingBoilerPowerTestStart=!1),S&&Ce(w)&&(o.commissioningTaskLock=""),_&&Ce(H)&&(o.pendingFlowAutotuneStart=!1),D&&Ce(H)&&(o.commissioningTaskLock=""),et&&(xt||Ce(ee))&&(o.pendingAirPurgeStart=!1),_e&&Ce(ee)&&(o.commissioningTaskLock=""),Mt&&(xe||Ce(ue))&&(o.pendingManualFlowStart=!1),pe&&(xe||Ce(ue))&&(o.commissioningTaskLock=""),qe&&(ht||Ce(tt))&&(o.pendingManualHpStart=!1),$t&&(ht||Ce(tt))&&(o.commissioningTaskLock=""),wa&&(Mu||Ce(po))&&(o.pendingHpWaterCalibrationStart=!1),mo&&Ce(po)&&(o.commissioningTaskLock="");let Rb=a?"Wachten op CM100":t,Pb=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",Nb=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:Du,available:!!($u||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:Du,statusCopy:Ue?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":ya?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:kS({status:po,running:Ue,resultReady:ya,startDisabled:$b,abortDisabled:Ab,applyDisabled:Hb,busy:ba,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:Pu,available:!!(ft||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:Pu,statusCopy:Ve?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?rr({active:Ve,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:ge||Tb,stopDisabled:ge||Cb}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:Nu,available:!!(_o||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:Nu,statusCopy:ot?xu?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":Cu?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
            ${o.entities.manualHpStart||o.entities.manualHpAbort?rr({active:ot,startKey:"manualHpStart",stopKey:"manualHpAbort",startLabel:"Bediening starten",stopLabel:"Bediening stoppen",startDisabled:bt||xb,stopDisabled:bt||Mb}):""}
          `,controls:`
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:ns,available:!0,openDisabled:mr(ns),cardMarkup:Ur({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:ns,statusCopy:Z?"Wacht totdat CM100 actief is voordat je autotune start.":W?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort?rr({active:W,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:A||Lu,stopDisabled:A||Sb}):""}
            ${o.entities.flowAutotuneApply?Je("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",A||qb):""}
          `,metrics:`
          ...
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:rs,available:e,openDisabled:mr(rs),cardMarkup:Ur({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s(M)}`,status:rs,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":k?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ${o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort?rr({active:k,startKey:"boilerPowerTestStart",stopKey:"boilerPowerTestAbort",startLabel:"Boiler test starten",stopLabel:"Boiler test stoppen",startDisabled:m||_u,stopDisabled:m||yb}):""}
            ${o.entities.boilerPowerTestApply?Je("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",m||vb):""}
          `,metrics:`
          ...
          `})},{key:"purge",title:"Ontluchten",label:"Ontluchten",summary:"Draait een vaste ontluchtingsrun van 5 minuten met rustige flow, pomp-pulsen en stabilisatie.",status:as,available:Q,openDisabled:mr(as),cardMarkup:Ur({taskKey:"purge",title:"Ontluchten",copy:"Draait 5 minuten met rustige doorstroming, korte pomp-pulsen en een stabilisatiefase.",subcopy:"Na afloop kan OpenQuatt de service mode (CM100) afsluiten of actief laten.",status:as,statusCopy:x?"Ontluchten loopt vast 5 minuten door en stopt daarna automatisch.":i?"CM100 staat klaar. Start ontluchten wanneer het circuit open staat.":"Start CM100 eerst en voer daarna ontluchten uit.",progressTask:"purge",className:"oq-settings-commissioning-card--air-purge",actions:`
            ${o.entities.airPurgeStart||o.entities.airPurgeAbort?rr({active:x,startKey:"airPurgeStart",stopKey:"airPurgeAbort",startLabel:"Ontluchten starten",stopLabel:"Ontluchten stoppen",startDisabled:gt||kb,stopDisabled:gt||Eb}):""}
          `,metrics:`
          ...
          `})}].filter(Db=>Db.available);return{cm100Status:Rb,cm100StartDisabled:p,cm100StopDisabled:g,serviceStatusCopy:Pb,tasks:Nb}}function ES(e){return Oe({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:to(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,Ro(),o.appView==="results"&&hr({force:!0}).then(()=>f())})},"select-energy-history-import-file":()=>oq(),"clear-energy-history-import-file":()=>{eg(),f()},"import-energy-history-file":()=>rq(),"export-energy-history":()=>WS(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",f();let e=qi({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&f()});return Ro([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",f()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",f()},"open-storage-energy":()=>{o.settingsStoragePage="energy",f()},"back-storage-overview":()=>{o.settingsStoragePage="overview",f()},"download-settings-backup":()=>uq(),"open-settings-backup-import":()=>{o.settingsBackupMqttPassword="",o.settingsBackupRestoreResult=null,o.settingsBackupError="",o.systemModal="settings-backup-import",f()},"confirm-settings-backup-restore":()=>fq()};function pg(e,t){return ie(hq,e,t)}function lc(e,t,r="",n=!1){return`
      <div class="oq-settings-storage-summary-metric${n?" is-on":""}">
        <span>${s(e)}</span>
        <strong>${s(t)}</strong>
        ${r?`<em>${s(r)}</em>`:""}
      </div>
    `}function gg(e){let t=String(e||"").trim(),r=t.match(/^(\d+(?:[.,]\d+)?)\s+records?$/i);return r?`${r[1]} ${r[1]==="1"?"dag":"dagen"}`:t}function Ei(e,t,r,n="",a="",i=""){if(!v(e))return"";let l=!!T(e),c=o.loadingEntities||o.busyAction===`switch-${e}`;return`
    ...
    `}function bq(e,t,r,n=""){if(!v(e))return"";let a=o.entities[e],i=Array.isArray(a?.option)?a.option:[],l=String(T(e)||"");return i.length?`
    ...
    `:""}function wq(e){return v(e)||!!O[e]&&!o.optionalMissingEntities?.[e]}function Jn(e,t,r,n={}){if(!wq(e))return"";let a=v(e),i=a&&(o.loadingEntities||o.busyAction===e),l=n.disabled===!0||!a,c=n.buttonClass||"oq-helper-button oq-helper-button--ghost";return`
      <button
        class="${s(c)}"
        type="button"
        data-oq-action="${s(r)}"
    ...
    `}function yq(){return o.trendHistoryMetadata&&typeof o.trendHistoryMetadata=="object"?o.trendHistoryMetadata:{}}function en(){return!!o.trendHistoryMetadataSignature}function tn(){return!!(o.energyHistoryRaw||o.energyHistorySignature)}function ut(e){return e?"Niet geladen":"Laden..."}function je(e,t="\u2014"){if(v(e))return He(e);let r=vq(e);return r==null||r===""?t:r}function vq(e){let t=yq();if(e==="trendHistoryFlashAvailable")return en()?t.available||"Alleen live":ut(o.trendHistoryMetadataError);if(e==="trendHistoryFlashOldest")return en()?t.oldest||"Geen data":ut(o.trendHistoryMetadataError);if(e==="trendHistoryFlashNewest")return en()?t.newest||"Geen data":ut(o.trendHistoryMetadataError);if(e==="trendHistoryFlashLastFlush")return en()?t.lastFlush||"Geen data":ut(o.trendHistoryMetadataError);if(e==="trendHistoryFlashSize")return en()?Ci(t.sizeKb):ut(o.trendHistoryMetadataError);if(e==="trendHistoryFlashWrites")return en()?Zn(t.writes):ut(o.trendHistoryMetadataError);let r=wg(),a=String(o.energyHistoryRaw||"").includes("@day_retention|");return e==="lifetimeEnergyHistoryAvailable"?tn()?a&&!r.dayPartitionAvailable?"Niet beschikbaar":fg(r.storedDayCount,"Geen data"):ut(o.energyHistoryError):e==="lifetimeEnergyHistoryOldest"?tn()?mg(r.oldestDateKey):ut(o.energyHistoryError):e==="lifetimeEnergyHistoryNewest"?tn()?mg(r.newestDateKey):ut(o.energyHistoryError):e==="lifetimeEnergyHistoryLastWrite"?tn()?Ti(r.dayLastWriteTimestampS):ut(o.energyHistoryError):e==="lifetimeEnergyHistorySize"?tn()?Ci(r.dayStorageKb):ut(o.energyHistoryError):e==="lifetimeEnergyHistoryWrites"?tn()?Zn(r.dayWriteCount):ut(o.energyHistoryError):null}function fg(e,t="Geen data"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:`${Math.round(r)} ${Math.round(r)===1?"dag":"dagen"}`}function hg(e,t="Nog geen historie"){let r=Math.max(0,Math.round(Number(e)||0));return r<=0?t:`${r} ${r===1?"gebeurtenis":"gebeurtenissen"}`}function bg(){return o.decisionLogStorageMetadata&&typeof o.decisionLogStorageMetadata=="object"?o.decisionLogStorageMetadata:{}}function Ci(e,t="\u2014"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:`${Math.round(r)} kB`}function Zn(e,t="0"){let r=Number(e);return!Number.isFinite(r)||r<=0?t:String(Math.round(r))}function mg(e){let t=ne(e);return t?t.date.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit",year:"numeric"}):"Geen data"}function Ti(e,t="Geen data"){let r=Number(e);if(!Number.isFinite(r)||r<=0)return t;let n=new Date(r*1e3),a=n.toLocaleDateString("nl-NL",{day:"2-digit",month:"2-digit"}),i=n.toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"});return`${a} ${i}`}function wg(){return mi(o.energyHistoryRaw)}function Sq(e){let t=Array.isArray(e.items)?e.items:[];return`
    ...
    `:""}function yg(){if(!v("trendHistoryEnabled")&&!v("decisionLogHistoryEnabled")&&!v("lifetimeEnergyHistoryEnabled"))return"";let e=C("trendHistoryEnabled"),t=e&&C("trendHistoryFlashEnabled"),r=v("lifetimeEnergyHistoryEnabled"),n=r&&C("lifetimeEnergyHistoryEnabled"),a=v("decisionLogHistoryEnabled"),i=a&&C("decisionLogHistoryEnabled"),l=bg(),c=t?je("trendHistoryFlashAvailable","Alleen live"):"Alleen live",u=r?gg(je("lifetimeEnergyHistoryAvailable","Geen data")):"Geen data";return re("Diagnose","Gegevens bewaren","Bepaal welke gegevens OpenQuatt bewaart voor grafieken, resultaten en hulp bij problemen.",`
        <article class="oq-settings-storage-summary">
          ...
          <div class="oq-settings-storage-summary-metrics" aria-label="Opslagstatus">
            ...
            ${a?lc("Beslislog",i?hg(l.storedEvents):"Alleen sinds herstart",i?"Maximaal 7 dagen":"Tijdelijk",i):""}
      ...
      `)}function qq(){if(!v("lifetimeEnergyHistoryEnabled"))return"";let e=o.energyHistoryImportRecords.length,t=new Set(o.energyHistoryImportHourRecords.map(c=>c.dateKey)).size,r=[];e>0&&r.push(`${e} dagrecords`),t>0&&r.push(`${t} uurdagen`),o.energyHistoryImportRange&&r.push(o.energyHistoryImportRange),o.energyHistoryImportSource&&r.push(o.energyHistoryImportSource),o.energyHistoryImportInvalidCount>0&&r.push(`${o.energyHistoryImportInvalidCount} regels niet gebruikt`);let n=!!o.energyHistoryImportFileName,a=e>0||t>0,i=Number(o.energyHistoryImportProgressPercent||0),l=o.energyHistoryImportBusy?`Importeren...${i>0?` (${i}%)`:""}`:"Importeren";return`
    ...
    `}function kq(){if(!v("lifetimeEnergyHistoryEnabled"))return"";let e=Zr(o.energyHistoryExportMode),t=Qn.map(n=>`
    ...
    `}function vg(){let e=["diagnosis","decision-log","energy"].includes(o.settingsStoragePage)?o.settingsStoragePage:"overview",t=v("trendHistoryEnabled")&&C("trendHistoryEnabled"),r=t&&v("trendHistoryFlashEnabled")&&C("trendHistoryFlashEnabled"),n=v("decisionLogHistoryEnabled"),a=n&&C("decisionLogHistoryEnabled"),i=bg(),l=hg(i.storedEvents),c=v("lifetimeEnergyHistoryEnabled"),u=c&&C("lifetimeEnergyHistoryEnabled"),p=c?je("lifetimeEnergyHistoryAvailable","Geen data"):"Geen data",g=gg(p),w=v("lifetimeEnergyHistoryClear")&&!["Geen data","\u2014"].includes(p),y=t&&v("trendHistoryFlush"),h=a&&v("decisionLogHistoryFlush"),m=v("lifetimeEnergyHistoryCapture"),d=wg(),b=String(o.energyHistoryRaw||"").includes("@hour_retention|"),S=b&&!d.hourPartitionAvailable,q=b?S?"Alleen live":fg(d.hourStoredDayCount,"Geen uurdata"):"Laden...",E=b&&!S?Ci(d.hourStorageKb):"\u2014",k=b&&!S?Zn(d.hourWriteCount):"\u2014",M=b&&!S?Ti(d.hourLastWriteTimestampS):"Geen data",P=e==="overview"?"":`
    ...
    `}function Cq(){if(!o.root)return!1;let e=o.root.querySelector(".oq-helper-status-grid");if(!e)return!!o.root.querySelector(".oq-helper-hub");let t=Tg();if(e.querySelectorAll("[data-oq-header-status]").length!==t.length)return e.outerHTML=Xn(),!0;for(let[n,a,i,l]of t){let c=e.querySelector(`[data-oq-header-status="${n}"]`);if(!c)return e.outerHTML=Xn(),!0;let u=Eg(n),p=!!(l||u);if(c.tagName.toLowerCase()!==(p?"button":"div"))return e.outerHTML=Xn(),!0;let g=c.querySelector(".oq-helper-status-label"),w=c.querySelector(".oq-helper-status-value");if(!g||!w)return e.outerHTML=Xn(),!0;g.textContent!==a&&(g.textContent=a);let y=xg(n),h=y?`<span class="oq-helper-status-value-text">${s(i)}</span><span class="oq-helper-status-badge" aria-label="Update beschikbaar" title="Update beschikbaar"></span>`:s(i);w.innerHTML!==h&&(w.innerHTML=h),p?c.setAttribute("data-oq-action",u):c.removeAttribute("data-oq-action"),c.classList.toggle("oq-helper-status-item--button",p),c.classList.toggle("oq-helper-status-item--attention",y)}return!0}function xq(){return""}function pc(){return""}function Mg(){let e=o.nativeOpen?"native":"app",t=Cg();return o.interfacePanelOpen?`
    ...
    `}Qd({getSignature:xi,patch:Cq});function Ag(){if(o.systemModal==="login")return Mm();if(o.systemModal==="api-security")return xm();if(o.systemModal==="mqtt")return up();if(o.systemModal==="mqtt-sensors")return dp();if(o.systemModal==="connectivity"){let e=Tq();return te({modalId:"system",titleId:"oq-system-modal-title",kicker:"Systeem",title:"Connectiviteit",closeAction:"close-system-modal",closeLabel:"Sluit systeem-popup",bodyMarkup:`
      ...
      `:"";return te({modalId:"system",titleId:"oq-backup-success-modal-title",kicker:"Beheer",title:"Backup hersteld",closeAction:"close-system-modal",closeLabel:"Sluit bevestiging",className:"oq-helper-modal--wide oq-helper-modal--scrollable",bodyMarkup:`
        ...
        `})}return""}function Mq(e=!1){let t=Pa();return t?`
    ...
    `:""}function mc(){return`
      <div class="oq-settings-curve-grid">
        ...
        ${ce("curveFallbackSupply","Fallback-aanvoertemperatuur zonder buitentemperatuur","Aanvoertemperatuur die gebruikt wordt als de buitentemperatuursensor niet beschikbaar is.","oq-settings-field--curve-fallback-card",{footerMarkup:Mq()})}
      </div>
    ...
    `}function $q(e="oq-settings-grid"){let t=[ce("flowKp","Flow PI Kp","Hoe sterk de regeling direct reageert op een afwijking."),ce("flowKi","Flow PI Ki","Hoe snel de regeling kleine restfouten wegwerkt.")].filter(Boolean);return t.length?`
      <div class="${s(e)}">
        ${t.join("")}
      </div>
    ...
    `}function Aq(){if(!v("phResponseProfile"))return"";let e=String(T("phResponseProfile")||""),t=o.loadingEntities||o.busyAction==="save-phResponseProfile",n=`
    ...
    `}function Rq(){return pn.filter(e=>v(Me(e,"Status"))||v(Me(e,"Load"))||v(Lo(e,"cooling",0)))}function Pq(e){return Re(Ae(e))}function Nq(e){let t=[];return qa.forEach(r=>{let n=-1/0;xr.forEach(a=>{let i=Lo(e,r,a),l=Pq(i);(!Number.isFinite(l)||l<0||l>120||l<n)&&t.push(`${r==="cooling"?"C":"H"}F${a}`),Number.isFinite(l)&&(n=l)})}),{valid:t.length===0,invalid:t}}function Dq(e){let t=String(T(`hp${e}Mode`)||"").trim(),r=Re(T(`hp${e}Freq`)),n=t&&t!=="Onbekend"&&t!=="Unknown",a=Number.isFinite(r),i=n&&/standby|stand-by/i.test(t),l=a&&r<=.5,c=n?i?a?l?"Standby en compressor uit.":`Compressor draait op ${r.toFixed(0)} Hz.`:"Compressorfrequentie is onbekend.":`ODU staat in ${t}.`:"ODU status is onbekend.";return{mode:n?t:"Onbekend",freq:Number.isFinite(r)?`${r.toFixed(0)} Hz`:"Onbekend",safe:i&&l,reason:c}}function _q(e){let t=String(e||"").toUpperCase();return!e||t==="UNKNOWN"||t==="UNAVAILABLE"?"Nog geen readback of apply-status ontvangen.":t.includes("APPLIED")?"Runtime registers zijn geschreven en via readback bevestigd. Een ODU powercycle zet de originele tabel terug.":t.includes("GUARD_READ_REQUESTED")?"Firmware leest actuele ODU mode en compressorfrequentie voordat er geschreven wordt.":t.includes("WRITE_QUEUED")||t.includes("WRITE_CONFIRMED")?"Runtime write loopt; wacht op bevestigde readback voordat je de waarden vertrouwt.":t.includes("FAILED")?"Firmware kon de runtime tabel niet volledig bevestigen. Laad opnieuw voordat je verder test.":t.includes("LOADED")?"Readback is in de velden geladen. Controleer de waarden voordat je schrijft.":t.includes("BLOCKED")?"Firmware heeft de actie geblokkeerd; controleer enable, standby en compressorstatus.":t.includes("LOAD_REQUESTED")?"Readback is aangevraagd bij de ODU.":"Laatste status van de experimentele runtime tabel."}function Pg(e,t){return v(e)?Eo({key:e,value:Ae(e),meta:Be(e),controlClass:"oq-helper-control oq-helper-control--suffix oq-settings-odu-runtime-control",inputClass:"oq-helper-input oq-helper-input--compact-number oq-settings-odu-runtime-input",inputAttributes:`data-oq-odu-runtime-tab-index="${t}"`,unitMarkup:'<span class="oq-helper-unit-chip">Hz</span>'}):'<span class="oq-settings-odu-runtime-missing">-</span>'}function Lq(e){let t=xr.length;return`
    ...
    `}function Po(e){let t=R(e);return Number.isNaN(t)?"\u2014":String(Math.max(0,Math.round(t)))}function Iq(e){let t=R(e);if(Number.isNaN(t))return"Nog niet gemeten";if(t<1)return"Zojuist";if(t<60)return`${Math.round(t)} min geleden`;let r=Math.floor(t/60),n=Math.round(t%60);return`${r}u ${n}m geleden`}function Ng(e){let t=R(e);return Number.isNaN(t)||t<=0?"Tijdstip onbekend":new Intl.DateTimeFormat("nl-NL",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}).format(new Date(t*1e3))}function Fq(e){if(!e.cyclingAlertLatched)return"";let t=X("compressorCyclingAlertAlternating"),r=Po("compressorCyclingAlertHp1Peak2h"),n=Po("compressorCyclingAlertHp1Peak72h"),a=v("compressorCyclingAlertHp2Peak2h")?Po("compressorCyclingAlertHp2Peak2h"):"",i=v("compressorCyclingAlertHp2Peak72h")?Po("compressorCyclingAlertHp2Peak72h"):"";return`
    ...
    `}function Dg(e,t){return v(`${t}CompressorStarts2h`)?`
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=ro(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function Ig(){let e=Lt(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&ro(t).length>0;return!e&&!r?"":re("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Wg(){let e=$o(),t=za(),r=o.busyAction==="restartAction";return re("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
      ...
      `)}function Vg(){let e=[vc("Warmtepomp 1","hp1ExcludedA","hp1ExcludedB"),vc("Warmtepomp 2","hp2ExcludedA","hp2ExcludedB")].filter(Boolean).join("");return re("Installatie","Compressorinstellingen","Stel hier de minimale draaitijd in en bepaal per warmtepomp welke compressorstanden je wilt overslaan.",`
        <div class="oq-settings-subpanel">
          <div class="oq-settings-subpanel-head">
  ...
  `}function jq(){let e=oe(),t=st(),r=`${e}:${t}`,n=o.quickStartSetupDraft||r,[a,i]=n.split(":");return{...ei(a,i),currentKey:r,selectedKey:n,changes:n!==r,targetIsDuo:a==="duo",targetIsEthernet:i==="eth"}}function Wq(){let e=jq(),t=Et(),r=!!(t||o.updateInstallBusy),n=[["single:wifi","Single \xB7 Wi-Fi","E\xE9n warmtepomp via het draadloze netwerk."],["single:eth","Single \xB7 Ethernet","E\xE9n warmtepomp via een vaste netwerkkabel."],["duo:wifi","Duo \xB7 Wi-Fi","Twee warmtepompen via het draadloze netwerk."],["duo:eth","Duo \xB7 Ethernet","Twee warmtepompen via een vaste netwerkkabel."]],a=[e.targetIsDuo?"De tweede warmtepomp is aangesloten en hoort bij deze controller.":"Deze controller wordt voor \xE9\xE9n warmtepomp gebruikt.",e.targetIsEthernet?"De netwerkkabel is aangesloten.":"De Wi-Fi-gegevens zijn beschikbaar op de controller."];return`
    ...
    `}function Kq(){let e=Ri(),t=o.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function Zq(){let e=C("usageTelemetryEnabled"),t=o.loadingEntities||o.busyAction==="switch-usageTelemetryEnabled";return`
      <section class="oq-helper-panel">
    ...
    `}function ek(){let e=Zg().id;return e==="setup"?Wq():e==="generation"?zg():e==="boiler"?v("boilerCvAssistEnabled")?Uq():Kg():e==="flow-source"?Bq():e==="thermostat-source"?Kq():e==="flow"?Gq():e==="heating"?Qq():e==="water"?Yq():e==="silent"?Jq():e==="usage-telemetry"?Zq():e==="confirm"?Xq():Kg()}function br(){let e=Ec().isQEdition;return Cr.filter(t=>(t.id!=="setup"||e)&&(!t.optionalEntity||v(t.optionalEntity)))}function dt(e){let t=br().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function tk(e){let t=Pi(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function ok(e=!1){return br().map((t,r)=>{let n=tk(r);return`
        <button
      ...
      `}).join("")}function Zg(){let e=br();return e.find(t=>t.id===o.currentStep)||e[0]||Cr[0]}function Pi(){return Math.max(0,br().findIndex(e=>e.id===o.currentStep))}function Xg(e){let t=br(),r=Math.min(t.length-1,Math.max(0,Pi()+e));o.currentStep=t[r]?.id||Cr[0].id}function Ct(e={}){let t=Pi(),r=br(),n=t>0?r[t-1]:null,a=t<r.length-1?r[t+1]:null;return`
    ...
    `}function rk(){let e=Pi(),t=br();return`
    ...
    `}function nk(){let e=ke(j("hpGeneration")),t=ae()?"Stooklijn":"Power House",r=S=>ke(j(S)),n=[],a=ae()?[["Regelprofiel",r("curveControlProfile")],["Aanvoer bij -20\xB0C",J("curveM20")],["Aanvoer bij -10\xB0C",J("curveM10")],["Aanvoer bij 0\xB0C",J("curve0")],["Aanvoer bij 5\xB0C",J("curve5")],["Aanvoer bij 10\xB0C",J("curve10")],["Aanvoer bij 15\xB0C",J("curve15")],["Fallback-aanvoer",J("curveFallbackSupply")]]:[["Profiel",r("phResponseProfile")],["Rated maximum house power",J("housePower")],["Maximum heating outdoor temperature",J("houseOutdoorMax")],["Temperatuurreactie",J("phKp")],["Comfort onder setpoint",J("phComfortBelow")],["Comfort boven setpoint",J("phComfortAbove")]],i=String(T("flowControlMode")||""),l=rn(),c=[["Status",l.status],["Actuele flow",l.flowAvailable?`${Math.round(l.flowValue)} L/h`:"Nog geen actuele waarde"]],u=Ri(),p=[["Status",u.status],["Kamertemperatuur",Number.isFinite(u.roomTempValue)?`${u.roomTempValue.toFixed(1)} \xB0C`:"Nog geen actuele waarde"],["Kamer-setpoint",Number.isFinite(u.roomSetpointValue)?`${u.roomSetpointValue.toFixed(1)} \xB0C`:"Nog geen actuele waarde"]],g=[["Flowregeling",i==="Manual PWM"?"Vaste pompstand":"Gewenste flow"],i==="Manual PWM"?["Vaste pompstand",J("manualIpwm")]:["Gewenste flow",J("flowSetpoint")]],w=v("boilerCvAssistEnabled")?[["CV-ketel/boiler aanwezig",C("boilerCvAssistEnabled")?"Ja":"Nee"],...C("boilerCvAssistEnabled")?[["Boiler rated heat power",J("boilerRatedHeatPower")]]:[]]:[],y=[["Maximale watertemperatuur",J("maxWater")]],h=[["Start stille uren",vn(T("silentStartTime"))||"\u2014"],["Einde stille uren",vn(T("silentEndTime"))||"\u2014"],["Maximaal niveau tijdens stille uren",J("silentMax")],["Maximaal niveau overdag",J("dayMax")]],m=v("usageTelemetryEnabled")?[["Technische gebruiksstatistieken",C("usageTelemetryEnabled")?"Delen":"Niet delen"]]:[],d=S=>`
    ...
    `}function Cc(e){let t=String(e||"").trim();return t?{Ready:"Gereed","Waiting for room request":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","Cooling enabled, waiting for room temperature above cooling setpoint":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","No dew point source":"Geen dauwpuntbron","OpenQuatt paused":"OpenQuatt gepauzeerd","Cooling disabled":"Koeling uitgeschakeld","Cooling minimum unavailable":"Minimale koel-aanvoer onbekend","Flow too low":"Flow te laag","Fallback active":"Dauwpuntsbenadering actief","Fallback active (+0.5\xB0C warm night)":"Dauwpuntsbenadering actief (+0,5\xB0C warme nacht)","Fallback active (+1.0\xB0C very warm night)":"Dauwpuntsbenadering actief (+1,0\xB0C zeer warme nacht)","Fallback active (+1.5\xB0C tropical night)":"Dauwpuntsbenadering actief (+1,5\xB0C tropische nacht)","User responsibility (no dew point or fallback)":"Expliciet toegestaan (geen dauwpunt of benadering)","Fallback cooling active":"Dauwpuntsbenadering actief","Fallback corrected by warm night":"Dauwpuntsbenadering gecorrigeerd door warme nacht","Fallback blocked by tropical night":"Dauwpuntsbenadering geblokkeerd door tropische nacht"}[t]||t:""}function ef(){let e=!v("coolingRoomRequestRequired")||C("coolingRoomRequestRequired"),t=[ce("coolingMinimumSupplyTemp","Minimale koel-aanvoer","Ondergrens voor het koeldoel. OpenQuatt gebruikt de hoogste waarde van deze instelling en de dauwpuntveilige grens."),lt("coolingDemandMax","Maximale koelsterkte","Bepaalt hoe krachtig OpenQuatt mag koelen. Lager geeft langere, rustigere runs; hoger geeft meer koelvermogen bij warm weer.","",{minLabel:"Rustig",maxLabel:"Krachtig",valueLabel:`${J("coolingDemandMax")} max`}),ce("coolingRestartDelta","Herstartmarge watertemperatuur","Na het bereiken van het koel-aanvoerdoel start de watercyclus pas opnieuw zodra de aanvoer deze marge boven het doel ligt."),ce("coolingSafetyMargin","Dauwpunt veiligheidsmarge","Extra marge boven het geselecteerde dauwpunt voor de minimale veilige watertemperatuur.")].filter(Boolean),r=[v("coolingRoomRequestRequired")?_p("coolingRoomRequestRequired","Koelvraag via kamerthermostaat","Aan: OpenQuatt wacht op echte koelvraag vanuit de kamer. Uit: koeltoestemming geldt direct als koelvraag.","Koelvraag start en stopt met de marges hieronder.","Koeltoestemming geldt direct als koelvraag. De start- en stopmarge worden nu niet gebruikt.","oq-settings-field--span-2"):"",e?ce("coolingRequestOnDelta","Koelvraag start boven setpoint","Koelvraag wordt actief zodra de kamer warmer is dan setpoint plus deze marge."):"",e?ce("coolingRequestOffDelta","Koelvraag stopt boven setpoint","Koelvraag valt weer af zodra de kamer koeler is dan setpoint plus deze marge."):""].filter(Boolean),n=r.length>0,a=v("coolingWithoutDewPointMode"),i=[v("coolingGuardMode")?ea("Route",ke(j("coolingGuardMode","Onbekend"))):"",v("coolingEffectiveMinSupplyTemp")?ea("Actieve ondergrens",j("coolingEffectiveMinSupplyTemp","\u2014")):""].filter(Boolean),l=i.length?ve("coolingGuardStatus","Actuele beveiliging","Laat zien welke route koeling nu begrenst en welke ondergrens daadwerkelijk geldt.",`<div class="oq-settings-cooling-facts">${i.join("")}</div>`,"oq-settings-field--span-2 oq-settings-field--cooling-status"):"",c=[v("outsideTempSelected")?ea("Actuele buitentemperatuur",j("outsideTempSelected","\u2014")):"",v("coolingFallbackNightMinOutdoorTemp")?ea("Nachtminimum buitentemperatuur",j("coolingFallbackNightMinOutdoorTemp","\u2014")):"",v("coolingFallbackMinSupplyTemp")?ea("Berekende minimum watertemperatuur",j("coolingFallbackMinSupplyTemp","\u2014")):""].filter(Boolean),u=c.length?`<div class="oq-settings-cooling-fallback-metrics">${c.join("")}</div>`:"",p=a||c.length>0,w=j("coolingGuardMode","").toLowerCase().includes("fallback");if(!t.length&&!n&&!a&&!l&&!p)return"";let y={"Dew point required":"Gebruik alleen een betrouwbare dauwpuntmeting. Zonder meting blijft koeling uit.","Allow without dew point":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use fallback":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use dew point approximation":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, user responsibility":"Negeer dauwpunt en benadering; alleen de ingestelde minimale koel-aanvoer geldt."};return re("Koeling","Koelingsinstellingen","Stel hier in wanneer koelvraag ontstaat, hoe koud het water mag worden en hoeveel het water mag opwarmen voor herstart.",`
        ...
        `:""}
      `)}function tf(){let e=v("otEnabled"),t=v("cicPollingEnabled")||v("cicFeedUrl"),r=v("cicCompatibilityMode"),n=v("otLinkProblem")||v("cicDataStale")||v("cicJsonFeedOk");if(!e&&!t&&!r&&!n)return"";let a=Dt("cicPollingEnabled"),i=Dt("otEnabled"),l=({label:m,value:d,active:b=!1})=>`
    ...
    `}function lf(){let e=Le.has(o.settingsGroup)?o.settingsGroup:fe[0].id;return`
    ...
    `:""}function Mc(){let e=$a.map(ck).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function df(e=Mc()){return we(e)}function pf(e=Mc()){return`
    ...
    `}function $k(e){let t=Math.max(1,Number(e||0)/1e3),r=Math.pow(10,Math.floor(Math.log10(t))),n=t/r;return(n<=1.5?1.5:n<=3?3:n<=6?6:10)*r*1e3}function Ak(e){let t=Number(e);return Number.isFinite(t)?t>=999500?`${Number((t/1e6).toFixed(1))}`:`${Number((t/1e3).toFixed(1))}`:""}function Hk(e){return e>=999500?"MWh":"kWh"}function Rk(e){let p=Math.max(1e3,...e.map(fk)),g=$k(p),w=e.length?1218/e.length:1218,y=Math.max(6,Math.min(38,w*.68));return{width:1280,height:260,left:44,right:18,top:26,bottom:38,plotWidth:1218,plotHeight:196,axisMax:g,barSlot:w,barWidth:y,yOf:m=>26+(1-Math.min(1,Math.max(0,Number(m||0)/g)))*196}}function Pk(e){let t=Di(e.heatpumpHeatOutputWh,e.heatingInputWh),r=Di(e.heatpumpCoolingOutputWh,e.coolingInputWh);return[e.tooltipLabel||e.label||jt(e.dateKey),`Elektrisch totaal: ${ze(e.electricalInputWh,1)}`,`Elektrisch verwarmen: ${ze(e.heatingInputWh,1)}`,`Elektrisch koelen: ${ze(e.coolingInputWh,1)}`,`Warmtepomp warmte: ${ze(e.heatpumpHeatOutputWh,1)}`,`Warmtepomp koeling: ${ze(e.heatpumpCoolingOutputWh,1)}`,`Cv-ketel warmte: ${ze(e.boilerHeatOutputWh,1)}`,`COP verwarmen: ${t}`,`EER koelen: ${r}`].join(`
`)}function Nk(e,t=""){if(!e.length)return`
      ...
      `;let r=Rk(e),n=Hk(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,c)=>{let u=r.left+r.barSlot*c+r.barSlot/2,p=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],g=r.height-r.bottom,w=p.map(d=>{let b=Qr(l,d.key);if(b<=0)return"";let S=b/r.axisMax*r.plotHeight;return g-=S,`
        ...
        `}).join(""),h=e.length<=12||c===0||c===e.length-1||c%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||jt(l.dateKey))}</text>`:"",m=Pk(l);return`
    ...
    `}function _k(e,t,r,n){let a=We(r)==="day"?Number(n):Number.NaN,i=Number.isFinite(a)?e.find(l=>l.dateKey===a):null;return i?[i]:t}function Ac(){let e=ta(),t=We(o.energyHistoryView),r=Ii(e,t),n=Sk(e,t,r),a=_k(e,n.buckets,t,r.selectedValue),i=qk(a);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:i,activeView:t}}function Sf(e=Ac()){return we({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,summary:e.summary})}function qf(e=Ac()){let t=e.summary,r=Ek(t),n=e.buckets[0]?.dateKey?jt(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?jt(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
    ...
    `}function Lk(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=Mc();return Xe(t,df(n),pf(n))}function Ok(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=Ac(),a=Ck();return(a?!1:Xe(t,Sf(n),qf(n)))||a}So({patchEnergyDom:Lk,patchResultsDom:Ok});function Ik({label:e,value:t,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(t)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function aa(e,t=!1){return e.map(r=>Ik({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?at(r.key):r.value,status:t})).join("")}function Dc(e){return`
    ...
    `}function Oc(e,t,r,n){return`${r?jk(n):""}${Fk(e,t)}`}function $f(e,t,r,n,a){if(!e)return;let i=we({mode:t,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&(Vt(e,Oc(t,r,n,a)),e.dataset.renderSignature=i)}function Fi(e,t,r=""){return`
    ...
    `}function Vk(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}var Bk=Ko;function Kk(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function zk(e){let t=Kk(j("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Uk(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=Bk(r),a=v("timeValid")&&C("timeValid")?zk(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Gk(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Af(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>v(e))||""}function Qk(){return["hp1WaterIn","hp2WaterIn"].find(e=>v(e))||""}function Hf(e=j("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function Bt(){return Hf()}function Rf(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function Ic(){return Bt()?"Koeling":ae()?"Stooklijn":"Power House"}function Yk(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=R(t);if(!Number.isNaN(r))return r}return Number.NaN}function Pf(){let e=Yk(),t=R("phouseHouse"),r=R("totalHeat"),n=R("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:z(e,0,"W"),houseText:z(t,0,"W"),correctionText:Gk(a),capacityText:at("hpCapacity"),statusTitle:i,statusCopy:l}}function Nf(){let e=R("curveSupplyTarget"),t=R("supplyTemp"),r=Af(),n=r?R(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:at("curveSupplyTarget"),supplyText:at("supplyTemp"),deltaText:Vk(a),capacityText:at("hpCapacity"),statusTitle:l,statusCopy:c}}function Df(){let e=R("supplyTemp"),t=j("coolingGuardMode",""),r=j("coolingFallbackNightMinOutdoorTemp","\u2014"),n=R("coolingSupplyError"),a=R("coolingDemandRaw"),i=C("coolingPermitted"),l=C("coolingRequestActive"),c=j("coolingBlockReason","Onbekend"),u=Cc(c),p=Rf(c,l),g="Wacht op koelvraag",w="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return p?(g="Wacht op koelvraag",w="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(g="Houdt doel vast",w="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(g="Trekt aanvoer omlaag",w="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(g="Benadert koeldoel",w="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(g="Koelt rustig door",w="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(g="Koeling gereed",w="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(g="Koeling geblokkeerd",w=`Blokkade: ${u}.`),{targetText:at("coolingSupplyTarget"),supplyText:at("supplyTemp"),safeFloorText:at("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:at("coolingDemandRaw"),statusTitle:g,statusCopy:w,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:p}}function ji(){if(Bt()){let t=Df(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(ae()){let t=Nf();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=Pf();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function Fc(e){return Mf({className:"oq-overview-system",title:e.title,copy:e.copy,signature:we(e),body:`
      ...
      `})}function Jk(){if(!C("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(Bt()){let n=Df(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(cf())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(ae()?Nf():Pf()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Zk(){return C("openquattEnabled")?Bt()?C("coolingPermitted")?C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:j("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:C("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function jc(e,t){let r=Jk(),n=Zk();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Wc(e,t){let r=jc(e,t);return`
    ...
    `:""}function sn({className:e,action:t,label:r,busy:n=!1,loading:a=!1,attrs:i=""}){return`
      ...
      `:s(n?"Bezig...":r)}</button>
    `}function tE(e){if(e.kind==="openquatt-control"){let t=o.busyAction==="openquatt-regulation",r=(o.loadingEntities||o.entitySyncInFlight)&&!v("openquattResumeAt");return C("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${sn({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:t})}</div>`:`
    ...
    `}function Bc(){let e=Xk();return e.length?`
    ...
    `:""}function _f(e){let t=j("controlModeLabel");return`
    ...
    `}function Ff(e,t){if(e?.currentKey&&v(e.currentKey)){let r=R(e.currentKey);if(Number.isFinite(r))return r}return na(e,t)}function Qc(e){e&&Gc().forEach(t=>{let r=e.querySelector(`[data-oq-trend-card="${t.id}"]`),n=t.samples[t.samples.length-1]||null;r&&t.series.forEach(a=>{let l=r.querySelector(`[data-oq-trend-current="${a.id}"]`)?.querySelector("strong"),c=z(Ff(a,n),a.decimals,a.unit);l&&l.textContent!==c&&(l.textContent=c)})})}function pE(e,t,r=!1,n=lo()){let a=If(e,t,{mockData:r,windowHours:n}),i=Uc(n),l=Rc(a.startTime),c=Rc(a.startTime+a.span/2),u=Rc(a.endTime),p=a.tracks.flatMap(g=>{if(g.points.length<2){let w=g.points[0];return w?`
          <circle
            cx="${w.x.toFixed(1)}"
            cy="${w.y.toFixed(1)}"
            r="3.8"
    ...
    `}function mE(e){let t=e.samples[e.samples.length-1]||null,r=Uc(e.windowHours);return`
    ...
    `}function fE(){let e=lo(),t=nc();return`
    ...
    `}function kE({tooltip:e,...t}){return`${eu(t)}${sa({bind:t.bind,...e})}`}function EE({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
    ...
    `}function Kf(e,t,r,n="normal",a=null){if(!v(t.power))return"";let i=ru(e,t,r),{mode:l,defrostActive:c,running:u,thermalKey:p}=i,g=i.schematic;return o.hpVisualMode==="schematic"?`
    ...
    `}function tu(){if(!nu())return"";let e=Ui();return o.hpVisualMode!=="schematic"?PE(e):`
    ...
    `}function DE(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(Vt(e,Uf(t)),!0):(Vt(r,`
      <h3>Warmtepompen</h3>
      ${Bf(t)}
    `),n.classList.toggle("is-active",o.hpVisualMode==="schematic"),a.classList.toggle("is-active",o.hpVisualMode==="compact"),!0)}function Gf(){let e=Ic(),t=ca(),r=iu(t),n=zf(t),a=Uf(t);return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
          ${_c()}
          ${_f(e)}
    ...
    `}function x1(){let e=mt(),t=ma(),r=ZE(),n=Do().filter(g=>!g.quick&&!g.custom),a=XE(),i=e1(a),l=eh(a.start),c=eh(a.end),u=o.controlReplayPeriodMenuOpen,p=e==="custom"?"Eigen periode":r.some(g=>g.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function M1(e){let t=fu(e.severity);return`
    ...
    `}function $1(e,t){let r=fu(e.severity),n=t&&t.id===e.id,a=dh(e.kind),i=uh(e);return`
      <button
    ...
    `}function Sh(e){if(!e)return"";let t=fu(e.severity),r=qr(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=ph(e),l=uh(e),c=Array.isArray(e.checks)?e.checks:r.checks;return`
    ...
    `}function H1(e,t){let r=ma(),a=e.filter(c=>!c.timelineHidden).slice(0,80),i=String(o.decisionLogError||"").trim(),l=!a.length&&!o.decisionLog&&!i;return`
    ...
    `}function j1(){return te({modalId:"initial-load",titleId:"oq-loading-modal-title",kicker:"OpenQuatt",title:"OpenQuatt laden",backdropClass:"oq-helper-modal-backdrop--loading",modalClass:"oq-helper-modal--reconnect oq-helper-modal--loading",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `})}function W1(){return o.appView==="overview"?Gf():o.appView==="control"?Th():o.appView==="energy"?kf():o.appView==="diagnosis"?jf():o.appView==="results"?Ef():F1()}function V1(){let e=dc();return`
