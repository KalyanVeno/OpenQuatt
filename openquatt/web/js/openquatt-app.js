...
`)),r.toString()}function yl(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=wy(l);if(!c)return;let u=[...a,c],p=qp(u,t);if(a.length&&p.length>ly){r.push({keys:n,body:i}),n=[l],a=[c],i=qp(a,t);return}n=[...n,l],a=u,i=p}),a.length&&r.push({keys:n,body:i}),r}async function yy(e){let t=bl(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await kn(sy,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function vy(){let e=bl(),r=await kn(cy,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function Sy(){let e=bl(),r=await kn(uy,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function qy(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.hour_start_epoch_s??"",i.source??"",i.attention_count??"",e?.meta?.flash_write_count??""].join("|")}async function ky(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<dy)return!1;o.decisionLogFetchPromise=(async()=>{let n=await Sy(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=qy(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function Ey(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{bn.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=Pp(r,o.entities[r],n))})}async function Ty(e){if(!e.length)return{ok:!0,message:""};try{let t=await vy();return Ey(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function z(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(h=>a||bn.has(h)||!hy(h,n));if(!i.length)return;let l=i.filter(h=>bn.has(h)),c=i.filter(h=>!bn.has(h)),p=(await Ty(l)).ok?c:[...c,...l];if(!p.length){kp(),Rn(Zt()),o.busyAction||(Sp(),o.controlError="");return}let m=Number(r.concurrency),w=Number.isFinite(m)&&m>0?Math.floor(m):Kt,y=yl(p,t),b=Math.max(1,Math.min(w,Kt)),g=[];for(let h=0;h<y.length;h+=b){let v=y.slice(h,h+b),q=await Promise.allSettled(v.map(async E=>({chunk:E,payload:await yy(E)})));g.push(...q)}g.some(h=>h.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let d="";g.forEach(h=>{if(h.status!=="fulfilled"){let A=h.reason.message||String(h.reason);d||(d=A);return}let{chunk:v,payload:q}=h.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);v.keys.forEach(A=>{if(Object.prototype.hasOwnProperty.call(E,A)){o.optionalMissingEntities&&delete o.optionalMissingEntities[A],o.entities[A]=Pp(A,o.entities[A],E[A]);return}let R=O[A];R?.optional?k.has(A)&&by(A,n):d||(d=`${R?.name||A} ontbreekt in bulk response`)})}),kp(),Rn(Zt()),d?(Rp(d),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${d}`):o.busyAction||(Sp(),o.controlError="")}function Pp(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function kp(){o.complete=Da(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=ud(),o.appView==="diagnosis"&&!yt()&&Pt(yo(),{syncMode:"replace",forceSync:!0}),o.appView||Pt(An()||yo(),{syncMode:"replace",forceSync:!0})}function Cy(){let e=pl();return o.appView==="settings"?[...new Set([...e,...ml()])]:o.appView==="energy"||o.appView==="results"?Pn(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?Pn(o.appView,{forceFast:!0}):[...new Set(e)]}function xy(e=[]){let t=new Set(e);return(o.appView==="settings"?ml():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...Pn(o.appView,{includeBulk:!0}),...Ut])]:o.appView==="energy"||o.appView==="results"?[...new Set([...Pn(o.appView,{forceFast:!0}),...Ut])]:[...new Set(["setupComplete","strategy",...at])]).filter(n=>!t.has(n))}function Ep(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=zu}async function Se(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||Wa()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){ty(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<vn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&Ep(t,e),p=t-Number(o.lastStaticEntitySyncAt||0)>=Uu||o.updateInstallBusy||o.updateInstallPhaseHint?Ut:[],m=o.quickStartModalOpen&&o.currentStep==="flow-source"?wt:[],w=o.quickStartModalOpen&&o.currentStep==="setup"?Gt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Oo:[],b=tl()?Xt():[],g=n?[...ws,...at,"setupComplete",...p]:i?[...Pn(a,{forceFast:l,includeBulk:c}),...p]:r==="settings"?[...new Set([...ml(),...b,...p])]:c?["setupComplete",...p,...at,"strategy",...Lo,...nt,...le()?we.map(d=>d.key):$t]:["setupComplete",...at,"strategy",...p];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let d=o.deviceReconnectMode,h=gy(t,e)?await fy():{ok:!0,message:""};if(!h.ok){Rp(h.message),n||f();return}if(await z([...new Set([...g,...w,...m,...y])],n?"state":r==="settings"||w.length?"all":"state",{concurrency:l&&i?zt:Kt}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),p.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&Cp();let v=d!==o.deviceReconnectMode,q=l&&i,E=q?!1:a==="overview"||a==="control"||a==="diagnosis"?await ol():!1,k=q?!1:o.appView==="results"?await Ba():!1,A=a==="control"?await ky({force:e.forceDecisionLog===!0}):!1,R=q?!1:tl()?(await Promise.all([ep(),Ba({metaOnly:!0})])).some(Boolean):!1,P=q||!kd()?!1:await Ko(),V=q||!Ed()?!1:await zo(),F=q||!fp()?!1:await Yo({force:o.systemModal==="mqtt-sensors"}),W=Yd();if(q&&!o.nativeOpen&&Ap(gl(a)),v){f();return}if(E&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){f();return}if(k&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){f();return}if(A&&o.appView==="control"){rl()||f();return}if(R&&o.appView==="settings"){f();return}if(P&&(o.systemModal==="login"||Yt())){f();return}if(V&&o.appView==="settings"){f();return}if(o.systemModal==="mqtt-sensors"){za()!==o.mqttSensorsModalRenderSignature&&f();return}if(F&&o.appView==="settings"){f();return}let T=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(W!==o.headerRenderSignature)if(T&&Js())o.headerRenderSignature=W;else{f();return}else Js();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){Cn()!==o.settingsRenderSignature&&f();return}if(T)return;if(o.appView==="settings"){let D=Cn();if(D!==o.settingsRenderSignature&&!o.focusedField){f();return}if(!Zd()){o.focusedField||f();return}o.focusedField&&(o.settingsRenderSignature=D);return}if(o.appView==="diagnosis"){ap()||f();return}if(o.appView==="control"){rl()||f();return}if(o.appView==="energy"){op()||f();return}if(o.appView==="results"){rp()||f();return}np()||f()}catch(d){n||(o.controlError=`Helperstatus kon niet worden geladen. ${d.message}`,f())}finally{o.entitySyncInFlight=!1;let d=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,d&&!o.nativeOpen&&window.setTimeout(()=>{Se(d)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!d&&Ep(Date.now())&&window.setTimeout(()=>{Se({forceBulk:!0})},Xw)}}function Dt(){return{}}function My(){let e=String(C("hpGeneration")||"").trim();return e||""}function Zo(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function $y(){if(!Array.isArray($r))return"";if($r.some(t=>S(t)))return"duo";let e=o.optionalMissingEntities||{};return $r.every(t=>e[t])?"single":""}function vl(e){let t=Zo(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Ay(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function oe(){let e=Zo(C("installationTopology"));if(e==="single"||e==="duo")return vl(e);let t=Zo(Dt().installation);if(t==="single"||t==="duo")return vl(t);let r=$y();return r?vl(r):Ay()}function _t(){let e=oe(),t=My();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Xe(){return"OpenQuatt"}function Nn(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function st(e=lt()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Xo(e=oe()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function _r(){let e=String(C("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Dt().hardwareProfile||e).trim().toLowerCase()}function lt(){return Nn(C("connectionText")||Dt().connection)}function Np(){let e=lt();return e==="wifi"?"eth":e==="eth"?"wifi":""}function Dp(){let e=oe();return e==="single"?"duo":e==="duo"?"single":""}function qo(e=oe(),t=lt()){let r=Xo(e),n=_r();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${st(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${st(t)}`:n==="waveshare"?`Waveshare ${r} ${st(t)}`:`${Xe()} ${r} ${st(t)}`}function Sl(){let e=x("timeValid"),t=String(C("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(S("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function Ua(){return S("timeValid")&&!x("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${Sl()}`}function Hy(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Ry(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Lr(){let e=Ry(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Hy(o.entities.uptime);if(n==="d")return Bo(e*1440);if(n==="h")return Bo(e*60);if(n==="s")return Bo(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(Dt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Bo((Date.now()-r)/6e4)}function Or(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(Dt().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function ko({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let y=e();if(!y)return null;let b={scrollTop:y.scrollTop};if((i||l)&&(b.scrollHeight=y.scrollHeight),l){let g=y.scrollHeight-y.scrollTop-y.clientHeight;b.stickToBottom=g<c}return a&&(b.identity=a(y)),b}function p(y){if(!y)return;let b=e();if(!b||a&&a(b)!==y.identity)return;if(y.stickToBottom){b.scrollTop=b.scrollHeight;return}let g=i?b.scrollHeight-Number(y.scrollHeight||0):0;b.scrollTop=Math.max(0,Number(y.scrollTop||0)+g)}function m(y,b=!0){if(!y)return;let g=Number(t()||0)+1;r(g);let d=()=>{Number(t()||0)!==g||!n()||p(y)};if(b){window.requestAnimationFrame(d);return}d()}function w(){r(Number(t()||0)+1)}return{capture:u,invalidate:w,queue:m,restore:p}}function Ae(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:C(e)}function Eo({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `:""}function qe(e,t,r,n,a="",i=""){return`<article class="oq-helper-surface oq-settings-field${a?` ${a}`:""}" data-oq-settings-field="${s(e)}"><div class="oq-settings-field-head"><h3>${s(t)}</h3>${To(e,t,r)}</div><div class="oq-settings-field-control">${n}</div>${i}</article>`}function Ne(e,t,r,n,a=""){return qe(e,t,r,`<div class="oq-settings-static-value">${s(n)}</div>`,a)}function Fe({label:e,value:t,note:r="",action:n="",className:a="",dataAttribute:i="data-oq-diagnostics-row",dataValue:l=""}){let c=`oq-settings-system-row${n?" oq-settings-system-row--with-action":""}${a?` ${s(a)}`:""}`,u=i&&l?` ${i}="${s(l)}"`:"";return!n&&!r?`<div class="${c}"${u}><span class="oq-settings-system-row-label">${s(e)}</span><strong class="oq-settings-system-row-value">${s(t)}</strong></div>`:`<div class="${c}"${u}><div class="oq-settings-system-row-copy"><p class="oq-settings-system-row-label">${s(e)}</p><strong class="oq-settings-system-row-value">${s(t)}</strong>${r?`<p class="oq-settings-system-row-note">${s(r)}</p>`:""}</div>${n}</div>`}function He(e,t={}){let r=typeof t=="number"?{decimals:t}:t||{},n=o.entities[e];if(!n)return"\u2014";let a=Number(n.value);if(!Number.isNaN(a)){let c=Number.isInteger(a)?0:Number.isFinite(r.decimals)?r.decimals:1,u=a.toFixed(Math.max(0,c));return r.trimTrailingZeros&&u.includes(".")&&(u=u.replace(/\.?0+$/,"")),`${u}${n.uom?` ${n.uom}`:""}`}let i=String(n.state??n.value??"").trim(),l=i.toLowerCase();return!i||l==="nan"||l==="unknown"||l==="unavailable"?"\u2014":i}function er(e,t="\u2014"){let r=o.entities[e];if(!r)return t;let n=String(r.state??r.value??"").trim();return!n||n==="0"||n==="\u2014"?t:n}function eo(e,t="",r=2){let n=Number(e);return Number.isFinite(n)?`${n.toFixed(Math.max(0,r))}${t?` ${t}`:""}`:"\u2014"}function Te(e,t=2){return He(e,{decimals:t})}function tr(e,t="IDLE"){let r=er(e,t),n=String(r??"").trim();return!n||n==="0"||n==="UNKNOWN"||n==="UNAVAILABLE"||n==="NAN"?t:n}function Ga(){let e=er("commissioningStatus",""),t=x("cm100Active"),r=String(e||"").trim().toUpperCase();return(t||r==="CM100 READY"||r==="CM100 STOPPED"||r.includes("DONE")||r.includes("FAILED")||r.includes("ABORT")||r.includes("APPLIED")||r.includes("REFUSED"))&&(o.pendingCommissioningCm100Start=!1),r&&r!=="0"?r==="IDLE"&&o.pendingCommissioningCm100Start?"CM100 REQUESTED":r:o.pendingCommissioningCm100Start?"CM100 REQUESTED":t?"CM100 READY":"IDLE"}function Ee(e){let t=String(e||"").trim();return t?{None:"Geen",Manual:"Handmatig",Disabled:"Niet gebruiken","HA input + Manual":"HA-invoer + handmatig","MQTT + Manual":"MQTT + handmatig","OT thermostat + Manual":"OT-thermostaat + handmatig","CIC + Manual":"CIC + handmatig","CIC + HA input + Manual":"CIC + HA-invoer + handmatig",Balanced:"Gebalanceerd",Stable:"Stabiel",Responsive:"Direct",Calm:"Rustig",Custom:"Aangepast",[qa]:"Stooklijn",[Sa]:"Power House","Dew point required":"Dauwpuntmeting vereist","Dew point":"Dauwpunt","Dew point (MQTT)":"Dauwpunt (MQTT)","Dew point (HA)":"Dauwpunt (HA)","Allow without dew point":"Dauwpuntsbenadering","Allow without dew point, use fallback":"Dauwpuntsbenadering","Allow without dew point, use dew point approximation":"Dauwpuntsbenadering","Allow without dew point, user responsibility":"Expliciet toestaan",Fallback:"Dauwpuntsbenadering","Fallback blocked":"Dauwpuntsbenadering geblokkeerd","User responsibility":"Expliciet toegestaan",Local:"Lokaal",CIC:"CIC","HA input":"HA-invoer","CIC + HA input":"CIC + HA-invoer","OT thermostat":"OT-thermostaat","Outdoor unit":"Buitenunit",Auto:"Auto","CIC or HA input":"CIC of HA-invoer","Flowmeter HP1":"Flowmeter HP1","Flowmeter HP2":"Flowmeter HP2","Local aggregate HP1/HP2":"Gecombineerde flow HP1/HP2"}[t]||t:""}function Ir({key:e,option:t,currentValue:r,busy:n,copy:a="",meta:i="",image:l="",imageAlt:c="",infoTitle:u="",infoCopy:p="",infoId:m=""}){let w=t===r,y=`
    ...
    >
      <span class="oq-settings-choice-head">
        <span class="oq-settings-choice-title">${s(Ee(t))}</span>
        ${i?`<span class="oq-settings-choice-meta"><span class="oq-settings-choice-meta-text">${s(i)}</span></span>`:""}
      </span>
      ${l?`<span class="oq-settings-choice-media"><img src="${s(l)}" alt="${s(c||Ee(t))}" loading="lazy" decoding="async"></span>`:""}
      ${a?`<span class="oq-settings-choice-copy">${s(a)}</span>`:""}
    </button>
  `;if(!p)return y;let b=u||Ee(t),g=m||`${e}-${t}`;return`
    <article class="oq-settings-choice-card-shell${w?" is-active":""}${l?" oq-settings-choice-card-shell--with-image":""}">
      ${y}
      ${To(g,b,p)}
    </article>
  `}function to(e={}){return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Co(e,t,r,n=""){if(!S(e))return"";let a=o.entities[e]||{},i=String(C(e)||""),l=to(a);return qe(e,t,r,`<label class="oq-settings-control oq-settings-control--select"><select class="oq-helper-select" data-oq-field="${s(e)}" ${o.loadingEntities?"disabled":""}>${l.map(c=>`<option value="${s(c)}" ${c===i?"selected":""}>${s(Ee(c))}</option>`).join("")}</select><span class="oq-settings-select-caret" aria-hidden="true"></span></label>`,n)}function Py(e,t,r="Aan",n="Uit"){return`<span class="oq-settings-toggle-state${t?" is-on":""}" data-oq-switch-pill="${s(e)}" data-on-label="${s(r)}" data-off-label="${s(n)}">${s(t?r:n)}</span>`}function qt(e,t,r,n,a="Aan",i="Uit",l=!0){let c=r?a:i,u=r?"off":"on";return`
  ...
  `;return qe(e,t,r,p,a)}function de(e,t,r,n="",a={}){if(!S(e))return"";let i=ze(e),l=Ae(e),c=a.unitOverride||i.uom||"",u=a.showUnit!==!1&&!!c,p=u&&a.unitMode!=="outside",m=Eo({key:e,value:l,meta:i,controlClass:`oq-helper-control${u&&!p?" oq-helper-control--split":""}${p?" oq-helper-control--suffix":""}`,unitMarkup:u?p?`<span class="oq-helper-unit-chip">${s(c)}</span>`:`<span class="oq-helper-unit">${s(c)}</span>`:""});return qe(e,t,r,m,n,a.footerMarkup||"")}function ct(e,t,r,n="",a={}){if(!S(e))return"";let i=ze(e),l=ye(e,C(e)),c=a.minLabel||`${i.min}${i.uom||""}`,u=a.maxLabel||`${i.max}${i.uom||""}`,p=a.valueLabel||Y(e,l);return qe(e,t,r,`<label class="oq-helper-slider-field"><div class="oq-helper-slider-meta"><span>${s(c)}</span><strong>${s(p)}</strong><span>${s(u)}</span></div><input class="oq-helper-range" type="range" data-oq-field="${s(e)}" min="${i.min}" max="${i.max}" step="${i.step}" value="${l}" ${o.loadingEntities?"disabled":""}></label>`,n)}function kl(e,t,r,n={}){if(!S(e))return"";let a=ze(e),i=Ae(e),l=n.compact===!0,c=n.embedded===!0,u=n.infoId||e,p=n.showCopy!==!1;return`
...
`).trimEnd()}catch{}return t}function cv(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function uv(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function dv(e){for(;o.webServerLogEntries.length>Ip;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function pv(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function Nl(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function mv(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",Zp(r))}}function Zp(e){let t=jp(e.receivedAt),r=Oy(e.receivedAt);return`
  ...
  `}function bv(){let e=Cl();if(!e)return"";let t=Wp(e),r=jy(e),n=o.loadingEntities||!!o.busyAction;return`
    ${Fe({dataValue:"debugLevel",label:"Logger level",value:r||"Onbekend",note:"Past het runtime logniveau aan voor nieuwe firmwaremeldingen.",action:`<label class="oq-webserver-log-level-control" aria-label="Logger level">
...
`)}async function yv(){let e=wv();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",f();return}try{if(!await ho(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&f()}var vv={"open-webserver-log-modal":()=>ev(),"clear-webserver-log-output":()=>tv(),"copy-webserver-log-output":()=>yv()};function Xp(e){return ce(vv,e)}function em(){let e=nr(),t=o.busyAction==="clear-webserver-log-history",r=!!o.busyAction||o.webServerLogHistoryLoading||!e&&!o.nativeOpen&&!o.webServerLogCsrfToken;return te({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
      ...
      <button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="close-system-modal">Gereed</button>
    `})}function Sv(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Dl(e){return Sv().includes(e)}function Xa(){let e=_r(),t=oe(),r=lt(),n=Np();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:S("firmwareUpdateTarget")&&Dl("alternate connection")&&S("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:st(r),targetLabel:st(n),currentBuildLabel:qo(t,r),targetBuildLabel:qo(t,n)}}function ei(){let e=_r(),t=oe(),r=Dp(),n=lt(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:S("firmwareUpdateTarget")&&Dl("alternate topology")&&S("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Xo(t),targetLabel:Xo(r),currentBuildLabel:qo(t,n),targetBuildLabel:qo(r,n)}}function ti(e,t){let r=_r(),n=oe(),a=lt(),i=Zo(e),l=Nn(t),c=i&&i!==n,u=l&&l!==a,p=c&&u?"alternate topology and connection":c?"alternate topology":u?"alternate connection":"current build",m=r==="heatpump_controller_q"&&["single","duo"].includes(n)&&["single","duo"].includes(i)&&["wifi","eth"].includes(a)&&["wifi","eth"].includes(l),w=S("firmwareUpdateTarget"),y=Dl(p),b=S("installFirmwareUpdateTarget");return{available:m,canSwitch:m&&p!=="current build"&&w&&y&&b,targetEntityAvailable:w,targetOptionAvailable:y,installActionAvailable:b,currentTopology:n,currentConnection:a,targetTopology:i,targetConnection:l,targetOption:p,currentBuildLabel:qo(n,a),targetBuildLabel:qo(i,l)}}function ir(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function jr(){let e=_r(),t=oe(),r=lt(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${st(r)}`}}function Dn(e=ir(),t=jr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function $o(){if(ut())return"Controleren";let e=Et();if(e)return e.phaseLabel;if(Ol())return"Bijgewerkt";if(ar())return"Bezig";if(lr())return"Beschikbaar";let t=nm();if(t!==null&&t<=0)return"Actueel";let r=Dt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Fl()?"Actueel":fe()?"Nog niet gecontroleerd":"\u2014"}function fe(){return o.entities.firmwareUpdate||null}function oi(){let e=fe();return e?String(e.state??e.value??"").trim().toLowerCase():""}function qv(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function _n(){return qv().toLowerCase()}function rm(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function _l(){let e=String(o.updateInstallTargetVersion||"").trim(),t=ue();return!e||!t?!1:Ln(t,e)>=0}function kv(e=fe()||{}){let t=no(e),r=ue(e);return!t||!r?!1:Ln(r,t)>=0}function Ll(){return(_l()||kv())&&!ut()&&!Fr()&&!lr()}function Ol(){return(o.updateInstallCompleted||Ll())&&!ut()&&!Et()&&!lr()}function sr(){$e({updateInstallBusy:!1,updateInstallTargetVersion:"",updateInstallPhaseHint:"",updateInstallProgressHint:Number.NaN,updateInstallStatusPollObserved:!1,updateInstallMode:"",updateInstallTargetConnection:"",updateInstallTargetTopology:""}),Hv()}function Lt(){o.updateInstallPhaseHint="starting",o.updateInstallProgressHint=0,o.updateInstallStatusPollObserved=!1}function Ce(){$e({updateManualUploadFile:null,updateManualUploadFileName:"",updateManualUploadError:""})}function tt(e={}){$e({...e.clearPr?{updateTestFirmwarePr:""}:{},updateTestFirmwareConfirmed:!1,updateTestFirmwareError:"",updateTestFirmwareBuild:null})}function Ev(){let e=_n(),t=rm(),r=e!=="rebooting"||!o.updateInstallBusy||o.updateInstallStatusPollObserved;if((e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting")&&r){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(o.updateInstallMode==="normal"&&_l()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function Fr(){let e=_n();return e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting"}function Tv(){let e=_n();return e==="error"?"De firmware-installatie op het device is mislukt. Controleer de netwerkverbinding en probeer opnieuw.":e==="aborted"?"De firmware-installatie is door het device afgebroken. Probeer de installatie opnieuw.":""}function Et(){Ev();let e=_n(),t=e==="starting"||e==="retrying"||e==="uploading"||e==="rebooting"&&(!o.updateInstallBusy||o.updateInstallStatusPollObserved),r=t?e:o.updateInstallPhaseHint,n=rm(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!Fr()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="retrying"?{phaseLabel:"Opnieuw proberen",percent:0,copy:"De eerste verbinding voor de firmwaredownload mislukte. OpenQuatt probeert het automatisch nog \xE9\xE9n keer."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Xe()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${st(o.updateInstallTargetConnection)}-build wordt nu naar ${Xe()} verzonden.`:o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?`De ${qo(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Xe()} verzonden.`:`Firmware wordt nu naar ${Xe()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Xe()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${st(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"||o.updateInstallMode==="build-switch"?`Opstellingswissel naar ${Xo(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Xe()}.`}}function no(e=fe()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function ue(e=fe()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function Cv(e=fe()||{}){let t=ue(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:Ln(t,r)>0}function Il(e=fe()||{},t=Mo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=no(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function tm(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function Ln(e,t){let r=tm(e),n=tm(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function ar(){if(Ll())return!1;let e=oi();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function ut(){let e=oi();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function lr(){let e=oi();if(!Il())return!1;let t=nm();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Dt().updateAvailable===!0}function Fl(){let e=oi();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Cv()}function jl(){let e=fe()||{},t=ue(e)||"\u2014",r=Il(e)?no(e):"",n=r?Ln(r,t):null;return!ut()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function nm(){let{current:e,latest:t}=jl();return e==="\u2014"||t==="\u2014"?null:Ln(t,e)}function am(e=Mo()){let t=String(e||"").trim().toLowerCase();return ss[t]||ss.main}function xv(){let e=String((fe()||{}).release_url||"").trim(),t=am();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function Mv(){return Xe()}function Mo(){return String(C("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function $v(){return jl().latest!=="\u2014"}function om(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Av(e,t=fe()||{}){let r=om(e);return r?om(`${t.title||""} ${t.summary||""}`).includes(r):!0}function im(e){return new Promise(t=>window.setTimeout(t,e))}function Wr(e=fo){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,Ld(),typeof kt=="function"&&kt(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&Is()},e)}function Hv(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||Is()}function sm(){return o.deviceReconnectMode?te({modalId:"reconnect",titleId:"oq-reconnect-modal-title",kicker:"Systeem",title:wd(),modalClass:"oq-helper-modal--reconnect",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `}):""}function On(e=Mo()){let t=fe()||{},r=ue(t);o.entities.firmwareUpdate={...t,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:am(e),current_version:r}}async function In(e={}){let t=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await im(r===0?900:1200),await z(Gt,"all",{forceMissing:!0});let n=Il(),a=!t||Av(t),i=$v(),l=ut(),c=$o();if(n&&a&&(i||!l&&c!=="Nog niet gecontroleerd"))return!0}return!1}async function cr(e={}){let t=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await im(a===0?r:n);try{let i=o.entities.firmwareUpdateStatus;await z(Gt,"all",{forceMissing:!0});let l=_n();o.entities.firmwareUpdateStatus!==i&&(o.updateInstallStatusPollObserved=!0);let c=Tv();if(c){let u=new Error(c);throw u.firmwareInstallTerminal=!0,u}if(l==="rebooting"&&o.updateInstallStatusPollObserved&&wo("ota"),f(),o.updateInstallMode==="connection-switch"){let u=Nn(o.updateInstallTargetConnection);if(u&&lt()===u&&!Fr()&&!ar())return!0}else if(o.updateInstallMode==="topology-switch"){let u=Zo(o.updateInstallTargetTopology);if(u&&oe()===u&&!Fr()&&!ar())return!0}else if(o.updateInstallMode==="build-switch"){let u=Zo(o.updateInstallTargetTopology),p=Nn(o.updateInstallTargetConnection);if(u&&p&&oe()===u&&lt()===p&&!Fr()&&!ar())return!0}else if(_l()||Ll()||Fl()&&!Fr()&&!ar())return!0}catch(i){if(i?.firmwareInstallTerminal)throw i;t||(o.controlNotice="Wachten tot het device opnieuw is opgestart...",f(),t=!0)}}return!1}function Rv(){let e=Mo(),t=Et();if(t)return t.copy;if(Ol()){let r=o.updateInstallCompletedVersion||ue()||Mo();return`${Xe()} draait nu op ${r}.`}return ar()?`OTA-update wordt voorbereid voor ${Xe()}. Het device kan kort herstarten.`:ut()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:lr()?"Er staat een nieuwere firmware klaar.":Fl()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function lm(){return!!(o.firmwareAdvancedOpen||o.firmwareConnectionSwitchOpen||o.firmwareTopologySwitchOpen||o.updateManualUploadOpen||o.updateTestFirmwareOpen)}function Za(e,t,r,n,a=!1){return`
      <button
    ...
    `}function Pv(e,t,r,n){if(!lm())return"";let i=!!(Et()||o.updateInstallBusy||ut());return`
    ...
    `}function Nv(){let e=Xa();if(!e||!o.firmwareConnectionSwitchOpen)return"";let r=!!(Et()||o.updateInstallBusy||ut()),n=!!o.firmwareConnectionSwitchConfirmed,a=e.targetConnection==="eth",i=!e.canSwitch,l=a?"Sluit eerst de netwerkkabel aan. Na de herstart verdwijnt Wi-Fi uit deze firmware.":"Na de herstart verdwijnt Ethernet uit deze firmware. Als er geen Wi-Fi-gegevens bekend zijn, start het OpenQuatt fallback access point.",c=i?'<p class="oq-helper-modal-note oq-helper-modal-note--muted">Verbindingswissel wordt geladen. Open deze modal opnieuw of wacht een moment als de knop disabled blijft.</p>':"";return`
    ...
    `}function _v(){if(!o.updateTestFirmwareOpen)return"";let t=!!(Et()||o.updateInstallBusy||ut()),r=ir(),n=jr(),a=Dn(r,n),i=!!(n.available&&S("firmwareTestOtaUrl")&&S("firmwareTestOtaMd5Url")&&S("installFirmwareTestOta")),l=!!(r&&i),c=o.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,p=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Lv(){if(!o.updateManualUploadOpen)return"";let t=!!(Et()||o.updateInstallBusy||ut()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function cm(){if(!o.updateModalOpen)return"";let e=fe(),t=o.entities.firmwareUpdateChannel||null,{current:r,latest:n}=jl(),a=ut(),i=ar(),l=lr(),c=Rv(),u=Et(),p=Ol(),m=xv(),w=p?"Firmware-update afgerond":u||i?"Firmware-update bezig":a?"Controleren op firmware-update":Mv(),y=t?Array.isArray(t.option)?t.option:Array.isArray(t.options)?t.options:[]:[],b=Xa(),g=ei(),d=!!(b&&!p),h=!!(g&&!p);return te({id:"firmware-update",titleId:"oq-update-modal-title",kicker:"OTA-update",title:w,copy:c,backdropClass:a||i||u?"is-busy":"",className:"oq-helper-modal--firmware oq-helper-modal--scrollable",closeAction:"close-update-modal",closeLabel:"Sluit update-popup",body:`
          ...
          ${Pv(d,b,h,g)}`})}function Ot(){return o.debugRecordingDeviceStatus?Math.max(0,Number(o.debugRecordingDeviceStatus.sample_count||0)):Array.isArray(o.debugRecordingSamples)?o.debugRecordingSamples.length:0}function It(e=o.debugRecordingDeviceStatus){return e?.rolling===!0||String(e?.mode||"").toLowerCase()==="rolling"}function ni(e=o.debugRecordingDeviceStatus){return It(e)&&e?.frozen===!0&&!e?.active}function ur(e){let t=Math.max(0,Math.round(Number(e||0)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return r>0?`${r}u ${String(n).padStart(2,"0")}m`:n>0?`${n}m ${String(a).padStart(2,"0")}s`:`${a}s`}function dm(){return o.debugRecordingDeviceStatus?Math.max(0,Number(o.debugRecordingDeviceStatus.retained_duration_s||0)*1e3):pm()}function pm(){if(o.debugRecordingDeviceStatus)return Math.max(0,Number(o.debugRecordingDeviceStatus.elapsed_s||0)*1e3);if(!o.debugRecordingStartedAt)return 0;let e=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return Math.max(0,e-Number(o.debugRecordingStartedAt||e))}function Fn(){return o.debugRecordingDeviceStatus&&o.debugRecordingDeviceStatus.available===!1?"Niet beschikbaar":ni()?"Rolling gestopt":o.debugRecordingActive&&It()?"Rolling actief":o.debugRecordingActive?"Bezig met opnemen":Ot()>0?"Voltooid":"Niet gestart"}function ai(){return ni()?"Rolling debug is gestopt. De recente samples blijven bewaard tot je downloadt, kopieert, hervat of een nieuwe opname start.":o.debugRecordingActive&&It()?"Rolling debug bewaart continu de recente samples. Download of kopieer maakt een momentopname; rolling blijft daarna doorlopen.":o.debugRecordingActive?"De opname loopt in apparaatgeheugen. Je kunt deze pagina sluiten en later het bestand downloaden.":Ot()>0?"De opname is klaar. Download het supportbestand en voeg dit toe aan je supportverzoek.":o.debugRecordingDeviceStatus&&o.debugRecordingDeviceStatus.available===!1?"Debugopname in apparaatgeheugen is niet beschikbaar op deze firmware.":"Neem tijdelijk supportgegevens op voor analyse. De opname wordt lokaal in het apparaatgeheugen opgeslagen. Er wordt niets automatisch verzonden."}function mm(){return ni()?"Gestopt":o.debugRecordingActive&&It()?`Rolling \xB7 ${ur(dm())}`:o.debugRecordingActive?`Loopt \xB7 ${ur(gm())}`:Ot()>0?"Klaar":Fn()}function Ov(){let e=Number(o.debugRecordingSelectedMinutes||15);return $a.map(r=>Number(r.minutes)).includes(e)?e:Number($a[0]?.minutes||15)}function Iv(e){o.debugRecordingActive||(La({debugRecordingSelectedMinutes:Math.max(1,Number(e)||15),debugRecordingNotice:"",debugRecordingError:""}),f())}function gm(){return It()?0:o.debugRecordingDeviceStatus?Math.max(0,Number(o.debugRecordingDeviceStatus.remaining_s||0)*1e3):o.debugRecordingActive?Math.max(0,Number(o.debugRecordingEndsAt||0)-Date.now()):0}function Fv(){if(o.debugRecordingDeviceStatus){if(It()){let a=Math.max(1,Number(o.debugRecordingDeviceStatus.sample_capacity||0));return Math.max(0,Math.min(100,Ot()/a*100))}let r=Math.max(1,Number(o.debugRecordingDeviceStatus.duration_s||0)),n=Math.max(0,Number(o.debugRecordingDeviceStatus.elapsed_s||0));return!o.debugRecordingActive&&Ot()>0?100:Math.max(0,Math.min(100,n/r*100))}if(!o.debugRecordingActive||!o.debugRecordingStartedAt||!o.debugRecordingEndsAt)return Ot()>0?100:0;let e=Math.max(1,Number(o.debugRecordingEndsAt)-Number(o.debugRecordingStartedAt)),t=Math.max(0,Date.now()-Number(o.debugRecordingStartedAt));return Math.max(0,Math.min(100,t/e*100))}function fm(e=o.debugRecordingDeviceStatus){return String(e?.recording_id??e?.recording?.recording_id??"").trim()}function hm(e){if(e?.recording?.active)return;let t=fm(e);if(t){o.debugRecordingAcknowledgedId=t;try{window.localStorage.setItem("oq-debug-recording-acknowledged-id",t)}catch{}}}function Wl(){let e=o.debugRecordingDeviceStatus,t=Math.max(0,Number(e?.sample_count||0));if(!e||e.available===!1||!e.active&&t===0)return"";let r=!!e.active;if(!r&&fm(e)===o.debugRecordingAcknowledgedId)return"";let n=It(e),a=ur(Math.max(0,Number(e.retained_duration_s||0))*1e3),i=ur(Math.max(0,Number(e.remaining_s||0))*1e3),l=r?n?`Rolling debug \xB7 ${a}`:`Debug loopt \xB7 ${i}`:n?"Rolling gestopt":"Debug klaar",c=r?n?`Rolling debug loopt, laatste ${a} beschikbaar`:`Debugopname loopt, nog ${i}`:n?"Rolling debug gestopt; recente buffer klaar om te downloaden":"Debugopname klaar om te downloaden";return`
    <button
  ...
  `}function jv(){if(!o.root)return;if(o.interfacePanelOpen){f();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=Wl();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function Wv(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Fn()),r&&(r.textContent=ai())}function Vl(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function Vv(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${Vl(e)}</span>`}function ao(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${Vl(e)}</span>`}function Bv(){return[...jo]}function Kv(){let e=[];return jo.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function zv(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function ii(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function dr(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function Ao(e){return`${Pe()}/openquatt/debug-recording/${e}`}function Vr(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Bl(){Vr({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function Uv(){let e=await window.fetch(Ao("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),t}function jn(e=2e3){dr(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{si({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function si(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",f());try{await Uv(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),jn()}catch(t){Bl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?f():(jv(),Wv())}}async function bm(){let e=yl(jo,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch(Ao(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==jo.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${jo.length})`);return t}async function Gv(e){let t=Math.max(1,Number(e)||15);ii(),dr(),La({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),f();try{await bm();let r=await window.fetch(Ao(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Vr(n),jn()}catch(r){Bl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,f()}}async function Qv(){ii(),dr(),La({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),f();try{await bm();let e=await window.fetch(Ao("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Vr(t),jn()}catch(e){Bl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,f()}}async function Yv(){let e=await window.fetch(Ao("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Vr(t),dr(),t}async function Jv(){ii(),o.debugRecordingBusy=!0,o.debugRecordingError="",f();try{await Yv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,f()}}async function Zv(e={}){ii(),dr(),o.debugRecordingBusy=!0,o.debugRecordingError="",f();try{let t=await window.fetch(Ao("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Vr(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,f()}}function um(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(ju/1e3),sample_count:Ot(),column_count:jo.length,storage:"browser"},columns:Bv(),units:Kv(),initial:zv(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function ri(e){return JSON.stringify(e)}function Xv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([ri(um())]).size}catch{return ri(um()).length}}function eS(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function tS(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function oS(){if(Ot()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",f();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&It();f();try{let t=await window.fetch(Ao("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,As(tS(r),ri(r),"application/json"),hm(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,f()}}async function rS(){if(Ot()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",f();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&It();f();try{let t=await window.fetch(Ao("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await ho(ri(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");hm(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,f()}}var nS={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",f(),si()),"start-debug-recording":e=>Gv(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>Qv(),"select-debug-recording-duration":e=>Iv(e.dataset.debugMinutes||15),"stop-debug-recording":()=>Zv(),"freeze-debug-recording":()=>Jv(),"download-debug-recording":()=>oS(),"copy-debug-recording":()=>rS()};function wm(e,t){return ce(nS,e,t)}function ym(){let e=o.debugRecordingActive,t=It(),r=ni(),n=Ot(),a=o.debugRecordingBusy,i=eS(Xv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Ov(),u=gm(),p=dm(),m=Fv(),w=n>0,y=`${m.toFixed(1)}%`,b=[{icon:"status",label:"Status",value:Fn()},{icon:"clock",label:t?"Retentie":"Duur",value:ur(t?p:pm())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],g=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return te({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:ai(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
    ...
    `}function xm(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return te({id:"system",titleId:"oq-api-security-modal-title",kicker:"Toegang",title:bS(),copy:wS(),className:"oq-helper-modal--wide",closeAction:"close-system-modal",closeLabel:"Sluit API-beveiliging popup",body:`
      ...
      ${t}
    `}function SS(){let e=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",offsetKey:"hp1WaterInOffset",finalKey:"hp1WaterIn"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",offsetKey:"hp1WaterOutOffset",finalKey:"hp1WaterOut"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",offsetKey:"hp2WaterInOffset",finalKey:"hp2WaterIn"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",offsetKey:"hp2WaterOutOffset",finalKey:"hp2WaterOut"}].filter(r=>S(r.offsetKey)&&S(r.finalKey));if(!e.length)return"";let t=r=>{let n=ze(r.offsetKey),a=zn(r.rawKey,r.finalKey,r.offsetKey),i=Re(Ae(r.offsetKey)),l=Number.isFinite(a)&&Number.isFinite(i)?eo(a+i,n.uom||"\xB0C",2):Te(r.finalKey,2);return`
        <article class="oq-settings-hp-offset-row" data-oq-settings-field="${s(r.offsetKey)}" data-oq-hp-offset-raw-key="${s(r.rawKey)}" data-oq-hp-offset-final-key="${s(r.finalKey)}">
          ...
          <div class="oq-settings-hp-offset-equation" aria-label="${s(`${r.label} correctie`)}">
            <div class="oq-settings-hp-offset-readout">
              <span>Raw</span>
              <strong data-oq-hp-offset-raw>${s(Number.isFinite(a)?eo(a,n.uom||"\xB0C",2):Te(r.rawKey,2))}</strong>
            </div>
    ...
    `}function kS({status:e,running:t,resultReady:r,startDisabled:n,abortDisabled:a,applyDisabled:i,busy:l,controlsAvailable:c}){let u=String(e||"").toUpperCase(),p=u.includes("FAILED")||u.includes("REFUSED")||u.includes("ABORT"),m=u.includes("APPLIED"),w=S("hp2WaterIn")||S("hp2WaterOut")||S("hp2WaterInRaw")||S("hp2WaterOutRaw"),y=H("hpWaterCalibrationStableProgress"),b=H("hpWaterCalibrationStableRequired"),g=H("hpWaterCalibrationRemaining"),d=Math.round(H("hpWaterCalibrationPhase")),h=t&&(d===1||u.includes("MIXING")),v=t&&!h,q=300,E=60,k=Number.isFinite(g)?Math.max(0,q-g):NaN,A=Number.isFinite(k)?Math.max(0,E-k):NaN,R=h&&Number.isFinite(k)?Math.max(0,Math.min(100,k/E*100)):v&&Number.isFinite(y)&&Number.isFinite(b)&&b>0?Math.max(0,Math.min(100,y/b*100)):t&&Number.isFinite(g)?Math.max(0,Math.min(100,100-g/q*100)):r?100:0,P=r&&S("hpWaterCalibrationResultSpreadBefore")?Te("hpWaterCalibrationResultSpreadBefore",2):Te("hpWaterCalibrationSpread",2),V=h?"Water mengen":Number.isFinite(y)&&Number.isFinite(b)&&b>0?y>0?`${Math.round(Math.max(0,y))} / ${Math.round(b)} s binnen grenzen`:"Nog niet binnen grenzen":"Wachten op stabiel venster",F=r?3:t?2:1,W=m?"Offsets toegepast":r?`Meting klaar - spreiding ${P}`:t?h?`Water mengen${Number.isFinite(A)&&A>0?` - meting start over ${Math.round(A)} s`:""}`:`Meting bezig - ${Number.isFinite(g)&&g>0?`max. ${Math.round(g)} s resterend`:V}`:p?"Meting niet voltooid":"Voorbereiding",T=m?"De voorgestelde offsets zijn opgeslagen.":r?"Controleer de voorgestelde offsets en pas ze toe.":t?h?"De waterpomp circuleert zonder compressor zodat de watertemperaturen eerst kunnen mengen.":"De firmware stopt zodra het laatste meetvenster binnen de spreiding- en driftgrenzen valt.":p?er("hpWaterCalibrationStatus","Controleer de voorwaarden en start opnieuw."):w?"Start alleen wanneer compressor en boiler uit zijn. HP1 en HP2 water in/out worden samen naar een relatieve referentie gebracht.":"Start alleen wanneer compressor en boiler uit zijn. Bij single setup wordt HP1 water in/out onderling gelijkgetrokken; supply blijft diagnose.",D=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",liveKey:"hp1WaterIn",resultRawKey:"hpWaterCalibrationResultHp1InRawAvg",offsetKey:"hp1WaterInOffset",suggestedKey:"hp1WaterInOffsetSuggested"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",liveKey:"hp1WaterOut",resultRawKey:"hpWaterCalibrationResultHp1OutRawAvg",offsetKey:"hp1WaterOutOffset",suggestedKey:"hp1WaterOutOffsetSuggested"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",liveKey:"hp2WaterIn",resultRawKey:"hpWaterCalibrationResultHp2InRawAvg",offsetKey:"hp2WaterInOffset",suggestedKey:"hp2WaterInOffsetSuggested"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",liveKey:"hp2WaterOut",resultRawKey:"hpWaterCalibrationResultHp2OutRawAvg",offsetKey:"hp2WaterOutOffset",suggestedKey:"hp2WaterOutOffsetSuggested"}].filter(L=>S(L.liveKey)||S(L.rawKey)||S(L.offsetKey)),N=(L,j)=>{let Z=F>L;return`
      ...
      `},_=L=>`
        <article class="oq-settings-hp-calibration-live-card">
          <span>${s(L.label)}</span>
          <strong>${s(Te(L.liveKey,2))}</strong>
        </article>
      `,J=L=>{let j=H(L.resultRawKey),Z=Number.isFinite(j)?j:zn(L.rawKey,L.liveKey,L.offsetKey),De=H(L.suggestedKey),ft=Number.isFinite(Z)&&Number.isFinite(De)?eo(Z+De,o.entities[L.suggestedKey]?.uom||"\xB0C",2):"\u2014";return`
        <tr>
          <th scope="row">${s(L.label)}</th>
          <td>${s(Number.isFinite(Z)?eo(Z,o.entities[L.liveKey]?.uom||"\xB0C",2):"\u2014")}</td>
          <td>${s(Te(L.offsetKey,2))}</td>
          <td><span class="oq-settings-hp-calibration-offset-pill">${s(Te(L.suggestedKey,2))}</span></td>
          <td>${s(ft)}</td>
        </tr>
      `};return`
    ...
    `}function Hm(){let e=S("boilerCvAssistEnabled")&&x("boilerCvAssistEnabled"),t=Ga(),r=x("cm100Active"),n=String(t||"").trim().toUpperCase(),a=pr(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,u=!!o.pendingCommissioningCm100Start,p=c||i||a,m=c||!i,w=tr("boilerPowerTestStatus","IDLE"),y=di(w,"boiler"),b=x("boilerPowerTestActive"),g=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",d=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),h=!!o.pendingBoilerPowerTestStart,v=o.commissioningTaskLock==="boiler",q=pr(w),k=!xe(w)&&(b||h||v||zr(w))&&!q,A=He("boilerRatedHeatPower"),R=He("boilerHeatPower"),P=H("boilerHeatPower"),V=P>0?R:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:R;P>0&&(o.commissioningBoilerHeatPowerDisplay=R);let F=tr("flowAutotuneStatus","IDLE"),W=di(F,"autotune"),T=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",D=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),N=!!o.pendingFlowAutotuneStart,_=o.commissioningTaskLock==="autotune",J=pr(F),j=!xe(F)&&(N||_||zr(F))&&!J,Z=tr("airPurgeStatus","IDLE"),De=di(Z,"purge"),ft=x("airPurgeActive"),xt=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",co=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),ht=!!o.pendingAirPurgeStart,_e=o.commissioningTaskLock==="purge",he=!xe(Z)&&(ft||ht||_e||zr(Z)),Tr=/DONE/.test(String(Z||"").toUpperCase()),$=!!(co||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),M=He("airPurgeRemaining",{decimals:0}),B=H("airPurgePhase"),K=B===1?"Rustig":B===2?"Pulsen":B===3?"Stabiliseren":De.phase,X=tr("manualFlowStatus","IDLE"),ae=x("manualFlowActive"),ie=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",ke=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),ge=!!o.pendingManualFlowStart,pe=o.commissioningTaskLock==="manual-flow",Le=!xe(X)&&(ae||ge||pe||zr(X)),Qe=tr("manualHpStatus","IDLE"),Ke=x("manualHpActive"),Mt=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",uo=!!(o.entities.manualHpStart||o.entities.manualHpAbort),rt=!!o.pendingManualHpStart,Oe=o.commissioningTaskLock==="manual-hp",se=!xe(Qe)&&(Ke||rt||Oe||zr(Qe)),Cu=/SAFETY STOP/.test(String(Qe||"").toUpperCase()),xu=/STOPPING/.test(String(Qe||"").toUpperCase()),po=tr("hpWaterCalibrationStatus","IDLE"),hb=di(po,"hp-water-calibration"),Mu=x("hpWaterCalibrationActive"),wa=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",$u=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),ya=!!o.pendingHpWaterCalibrationStart,mo=o.commissioningTaskLock==="hp-water-calibration",Ye=!xe(po)&&(Mu||ya||mo||zr(po)),va=/DONE|APPLIED/.test(String(po||"").toUpperCase()),Au=/APPLIED/.test(String(po||"").toUpperCase()),bb=He("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),wb=He("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),Hu=/DONE|APPLIED/.test(String(w||"").toUpperCase()),Ru=/DONE|APPLIED/.test(String(F||"").toUpperCase()),ns=i?q?"Wachten op CM100":k?y.phase:Hu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",as=i?J?"Wachten op CM100":j?W.phase:Ru?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",is=i?he?De.phase:Tr?"Klaar":"Klaar om te starten":"Wachten op CM100",Pu=i?Le?"Actief":"Klaar om te starten":"Wachten op CM100",Nu=i?se?xu?"Bezig met stoppen":Cu?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",Du=i?Ye?hb.phase:Au?"Offsets toegepast":va?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",_u=!i||g||!d||j||he||Le||se||Ye||k||_||_e||pe||Oe||mo||h,yb=g||!(k||v||h),vb=g||_u||!Hu||j||he||Ye,Lu=!i||T||!D||k||he||Le||se||Ye||j||v||_e||pe||Oe||mo||N,Sb=T||!(j||_||N),qb=T||Lu||!Ru||k||he||Ye,kb=!i||xt||!co||k||j||Le||se||Ye||he||v||_||pe||Oe||mo||ht,Eb=xt||!(he||_e||ht),Tb=!i||ie||!ke||k||j||he||se||Ye||Le||v||_||_e||Oe||mo||ge,Cb=ie||!(Le||pe||ge),xb=!i||Mt||!uo||k||j||he||Le||Ye||se||v||_||_e||pe||mo||rt,Mb=Mt||!(se||Oe||rt),$b=!i||wa||!$u||k||j||he||Le||se||Ye||v||_||_e||pe||Oe||ya,Ab=wa||!(Ye||mo||ya),Hb=wa||Ye||!va||Au;u&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),h&&(b||xe(w))&&(o.pendingBoilerPowerTestStart=!1),v&&xe(w)&&(o.commissioningTaskLock=""),N&&xe(F)&&(o.pendingFlowAutotuneStart=!1),_&&xe(F)&&(o.commissioningTaskLock=""),ht&&(ft||xe(Z))&&(o.pendingAirPurgeStart=!1),_e&&xe(Z)&&(o.commissioningTaskLock=""),ge&&(ae||xe(X))&&(o.pendingManualFlowStart=!1),pe&&(ae||xe(X))&&(o.commissioningTaskLock=""),rt&&(Ke||xe(Qe))&&(o.pendingManualHpStart=!1),Oe&&(Ke||xe(Qe))&&(o.commissioningTaskLock=""),ya&&(Mu||xe(po))&&(o.pendingHpWaterCalibrationStart=!1),mo&&xe(po)&&(o.commissioningTaskLock="");let Rb=a?"Wachten op CM100":t,Pb=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",Nb=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:Du,available:!!($u||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:Du,statusCopy:Ye?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":va?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:kS({status:po,running:Ye,resultReady:va,startDisabled:$b,abortDisabled:Ab,applyDisabled:Hb,busy:wa,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:Pu,available:!!(ke||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:Pu,statusCopy:Le?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?or({active:Le,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:ie||Tb,stopDisabled:ie||Cb}):""}
            </div>
          `,metrics:`
          ...
          `}),modalActions:`
          ${o.entities.manualFlowApplyHeating?et("manualFlowApplyHeating","Overnemen voor verwarmen","oq-helper-button oq-helper-button--ghost",ie):""}
          ${o.entities.manualFlowApplyCooling?et("manualFlowApplyCooling","Overnemen voor koelen","oq-helper-button oq-helper-button--ghost",ie):""}
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:Nu,available:!!(uo||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Ur({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:Nu,statusCopy:se?xu?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":Cu?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
            ${o.entities.manualHpStart||o.entities.manualHpAbort?or({active:se,startKey:"manualHpStart",stopKey:"manualHpAbort",startLabel:"Bediening starten",stopLabel:"Bediening stoppen",startDisabled:Mt||xb,stopDisabled:Mt||Mb}):""}
          `,controls:`
          ...
          `,metrics:`
            ...
            <div class="oq-settings-manual-hp-results">
              ...
              ${S("hp2Compressor")?Ne("hp2Compressor","Warmtepomp 2 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",Am("hp2Compressor","hp2Freq"),"oq-settings-field--compact"):""}
            </div>
            ${Ne("manualHpGuardStatus","Bewaking","Toont waarom een handmatig verzoek tijdelijk niet of nog niet volledig wordt toegepast.",C("manualHpGuardStatus")||"Vrijgegeven","oq-settings-field--compact oq-settings-field--full")}
            <div class="oq-settings-manual-hp-statuses">
              ${Ne("hp1Failures","Warmtepomp 1 statusmelding","Actuele melding die de warmtepomp zelf rapporteert.",vo(I("hp1Failures","None")),"oq-settings-field--compact")}
              ${S("hp2Failures")?Ne("hp2Failures","Warmtepomp 2 statusmelding","Actuele melding die de warmtepomp zelf rapporteert.",vo(I("hp2Failures","None")),"oq-settings-field--compact"):""}
            </div>
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:as,available:!0,openDisabled:pr(as),cardMarkup:Ur({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:as,statusCopy:J?"Wacht totdat CM100 actief is voordat je autotune start.":j?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ...
            ${o.entities.flowAutotuneApply?et("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",T||qb):""}
          `,metrics:`
            ${Ne("flowKpSuggested","Voorgestelde Kp","Kp bepaalt hoe sterk de regeling meteen corrigeert.",bb,"oq-settings-field--compact")}
            ${Ne("flowKiSuggested","Voorgestelde Ki","Ki corrigeert kleine afwijkingen langzaam weg.",wb,"oq-settings-field--compact")}
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:ns,available:e,openDisabled:pr(ns),cardMarkup:Ur({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s(A)}`,status:ns,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":k?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ...
            ${o.entities.boilerPowerTestApply?et("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",g||vb):""}
          `,metrics:`
          ...
          `})}].filter(Db=>Db.available);return{cm100Status:Rb,cm100StartDisabled:p,cm100StopDisabled:m,serviceStatusCopy:Pb,tasks:Nb}}function ES(e){return Fe({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:Xt(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,Ro(),o.appView==="results"&&fr({force:!0}).then(()=>f())})},"select-energy-history-import-file":()=>oq(),"clear-energy-history-import-file":()=>{eg(),f()},"import-energy-history-file":()=>rq(),"export-energy-history":()=>WS(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",f();let e=ki({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&f()});return Ro([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",f()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",f()},"open-storage-energy":()=>{o.settingsStoragePage="energy",f()},"back-storage-overview":()=>{o.settingsStoragePage="overview",f()},"download-settings-backup":()=>uq(),"open-settings-backup-import":()=>{o.settingsBackupMqttPassword="",o.settingsBackupRestoreResult=null,o.settingsBackupError="",o.systemModal="settings-backup-import",f()},"confirm-settings-backup-restore":()=>fq()};function pg(e,t){return ce(hq,e,t)}function cc(e,t,r="",n=!1){return`
      ...
      <article class="oq-settings-storage-row oq-settings-storage-row--select" data-oq-settings-field="${s(e)}">
        ...
        <label class="oq-settings-storage-select">
          <select class="oq-helper-select" data-oq-field="${s(e)}" ${o.loadingEntities?"disabled":""}>
            ${i.map(c=>`<option value="${s(c)}" ${c===l?"selected":""}>${s(Ee(c))}</option>`).join("")}
          </select>
          <span class="oq-settings-select-caret" aria-hidden="true"></span>
        </label>
      </article>
    ...
    `:""}function yg(){if(!S("trendHistoryEnabled")&&!S("decisionLogHistoryEnabled")&&!S("lifetimeEnergyHistoryEnabled"))return"";let e=x("trendHistoryEnabled"),t=e&&x("trendHistoryFlashEnabled"),r=S("lifetimeEnergyHistoryEnabled"),n=r&&x("lifetimeEnergyHistoryEnabled"),a=S("decisionLogHistoryEnabled"),i=a&&x("decisionLogHistoryEnabled"),l=bg(),c=t?Ve("trendHistoryFlashAvailable","Alleen live"):"Alleen live",u=r?gg(Ve("lifetimeEnergyHistoryAvailable","Geen data")):"Geen data";return re("Diagnose","Gegevens bewaren","Bepaal welke gegevens OpenQuatt bewaart voor grafieken, resultaten en hulp bij problemen.",`
    ...
    `}function vg(){let e=["diagnosis","decision-log","energy"].includes(o.settingsStoragePage)?o.settingsStoragePage:"overview",t=S("trendHistoryEnabled")&&x("trendHistoryEnabled"),r=t&&S("trendHistoryFlashEnabled")&&x("trendHistoryFlashEnabled"),n=S("decisionLogHistoryEnabled"),a=n&&x("decisionLogHistoryEnabled"),i=bg(),l=hg(i.storedEvents),c=S("lifetimeEnergyHistoryEnabled"),u=c&&x("lifetimeEnergyHistoryEnabled"),p=c?Ve("lifetimeEnergyHistoryAvailable","Geen data"):"Geen data",m=gg(p),w=S("lifetimeEnergyHistoryClear")&&!["Geen data","\u2014"].includes(p),y=t&&S("trendHistoryFlush"),b=a&&S("decisionLogHistoryFlush"),g=S("lifetimeEnergyHistoryCapture"),d=wg(),h=String(o.energyHistoryRaw||"").includes("@hour_retention|"),v=h&&!d.hourPartitionAvailable,q=h?v?"Alleen live":fg(d.hourStoredDayCount,"Geen uurdata"):"Laden...",E=h&&!v?xi(d.hourStorageKb):"\u2014",k=h&&!v?Xn(d.hourWriteCount):"\u2014",A=h&&!v?Ci(d.hourLastWriteTimestampS):"Geen data",R=e==="overview"?"":`
      ...
      </button>`,P=(_,J,L,j,Z,De)=>`
      <button class="oq-settings-storage-hub-item${De?" is-on":""}" type="button" data-oq-action="${s(_)}">
    ...
    `}function Rq(){return mn.filter(e=>S(Me(e,"Status"))||S(Me(e,"Load"))||S(_o(e,"cooling",0)))}function Pq(e){return Re(Ae(e))}function Nq(e){let t=[];return ka.forEach(r=>{let n=-1/0;xr.forEach(a=>{let i=_o(e,r,a),l=Pq(i);(!Number.isFinite(l)||l<0||l>120||l<n)&&t.push(`${r==="cooling"?"C":"H"}F${a}`),Number.isFinite(l)&&(n=l)})}),{valid:t.length===0,invalid:t}}function Dq(e){let t=String(C(`hp${e}Mode`)||"").trim(),r=Re(C(`hp${e}Freq`)),n=t&&t!=="Onbekend"&&t!=="Unknown",a=Number.isFinite(r),i=n&&/standby|stand-by/i.test(t),l=a&&r<=.5,c=n?i?a?l?"Standby en compressor uit.":`Compressor draait op ${r.toFixed(0)} Hz.`:"Compressorfrequentie is onbekend.":`ODU staat in ${t}.`:"ODU status is onbekend.";return{mode:n?t:"Onbekend",freq:Number.isFinite(r)?`${r.toFixed(0)} Hz`:"Onbekend",safe:i&&l,reason:c}}function _q(e){let t=String(e||"").toUpperCase();return!e||t==="UNKNOWN"||t==="UNAVAILABLE"?"Nog geen readback of apply-status ontvangen.":t.includes("APPLIED")?"Runtime registers zijn geschreven en via readback bevestigd. Een ODU powercycle zet de originele tabel terug.":t.includes("GUARD_READ_REQUESTED")?"Firmware leest actuele ODU mode en compressorfrequentie voordat er geschreven wordt.":t.includes("WRITE_QUEUED")||t.includes("WRITE_CONFIRMED")?"Runtime write loopt; wacht op bevestigde readback voordat je de waarden vertrouwt.":t.includes("FAILED")?"Firmware kon de runtime tabel niet volledig bevestigen. Laad opnieuw voordat je verder test.":t.includes("LOADED")?"Readback is in de velden geladen. Controleer de waarden voordat je schrijft.":t.includes("BLOCKED")?"Firmware heeft de actie geblokkeerd; controleer enable, standby en compressorstatus.":t.includes("LOAD_REQUESTED")?"Readback is aangevraagd bij de ODU.":"Laatste status van de experimentele runtime tabel."}function Pg(e,t){return S(e)?Eo({key:e,value:Ae(e),meta:ze(e),controlClass:"oq-helper-control oq-helper-control--suffix oq-settings-odu-runtime-control",inputClass:"oq-helper-input oq-helper-input--compact-number oq-settings-odu-runtime-input",inputAttributes:`data-oq-odu-runtime-tab-index="${t}"`,unitMarkup:'<span class="oq-helper-unit-chip">Hz</span>'}):'<span class="oq-settings-odu-runtime-missing">-</span>'}function Lq(e){let t=xr.length;return`
      ...
      <article class="oq-settings-odu-runtime-panel">
        <div class="oq-settings-odu-runtime-panel-head">
          ...
          <div class="oq-settings-odu-runtime-actions">
            ${S(r)?et(r,o.busyAction===r?"Lezen...":"Uit ODU laden","oq-helper-button oq-helper-button--ghost",p):""}
      ${S(t)?qt(t,`HP${e} writes vrijgeven`,u,p,"Enable","Locked"):""}
            ${S(n)?et(n,o.busyAction===n?"Schrijven...":"Runtime toepassen","oq-helper-button oq-helper-button--warning",m):""}
          </div>
        </div>
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(C("hpGeneration")||""),n=to(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function Ig(){let e=_t(),t=o.entities.hpGeneration||{},r=S("hpGeneration")&&to(t).length>0;return!e&&!r?"":re("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Ec(e="oq-settings-grid oq-settings-boiler-simple-grid"){if(!S("boilerCvAssistEnabled"))return"";let t=x("boilerCvAssistEnabled"),r=S("boilerRatedHeatPower"),n=ze("boilerRatedHeatPower"),a=Ae("boilerRatedHeatPower"),i=o.loadingEntities||o.busyAction==="switch-boilerCvAssistEnabled",c=r?Eo({key:"boilerRatedHeatPower",value:a,meta:n,controlClass:"oq-helper-control oq-helper-control--suffix oq-settings-boiler-power-control",unitMarkup:'<span class="oq-helper-unit-chip">W</span>'}):`
      ...
      `)}function Wg(){let e=$o(),t=Ua(),r=o.busyAction==="restartAction";return re("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ${Fe({dataValue:"uptime",label:"Uptime",value:Lr()})}
          ${Fe({dataValue:"ip",label:"IP-adres",value:Or()})}
          ...
          ${Fe({dataValue:"webserverLog",label:"Logboek",value:Ja(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
          ...
          ${Fe({dataValue:"debugRecording",label:"Debugopname",value:Fn(),note:ai(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
              data-oq-action="open-debug-recording-modal"
            >
          ...
          ${Fe({dataValue:"espTemp",label:"ESP-temp",value:$i()})}
          ...
          ${S("statusLedsEnabled")?`
            ${Fe({dataValue:"statusLeds",label:"Status-LEDs",value:x("statusLedsEnabled")?"Aan":"Uit",note:"Schakelt de gele netwerk-LED en rode storings-LED op de Q-edition controller.",action:qt("statusLedsEnabled","Status-LEDs",x("statusLedsEnabled"),o.loadingEntities||o.busyAction==="switch-statusLedsEnabled")})}
          `:""}
        </div>
  ...
  `}function Ri({collapsible:e=!1,idPrefix:t="oq-usage",open:r=!1}={}){let n=s(t),a=`${n}-included-title`,i=`${n}-excluded-title`,l=`
    ...
    <p class="oq-usage-network-note">${me("server","oq-usage-network-note-icon")} De MQTT-broker kan, zoals iedere internetdienst, technisch wel het bron-IP-adres zien.</p>
  `;return e?`
  ...
  `}function jq(){let e=oe(),t=lt(),r=`${e}:${t}`,n=o.quickStartSetupDraft||r,[a,i]=n.split(":");return{...ti(a,i),currentKey:r,selectedKey:n,changes:n!==r,targetIsDuo:a==="duo",targetIsEthernet:i==="eth"}}function Wq(){let e=jq(),t=Et(),r=!!(t||o.updateInstallBusy),n=[["single:wifi","Single \xB7 Wi-Fi","E\xE9n warmtepomp via het draadloze netwerk."],["single:eth","Single \xB7 Ethernet","E\xE9n warmtepomp via een vaste netwerkkabel."],["duo:wifi","Duo \xB7 Wi-Fi","Twee warmtepompen via het draadloze netwerk."],["duo:eth","Duo \xB7 Ethernet","Twee warmtepompen via een vaste netwerkkabel."]],a=[e.targetIsDuo?"De tweede warmtepomp is aangesloten en hoort bij deze controller.":"Deze controller wordt voor \xE9\xE9n warmtepomp gebruikt.",e.targetIsEthernet?"De netwerkkabel is aangesloten.":"De Wi-Fi-gegevens zijn beschikbaar op de controller."];return`
    ...
    `}function xc(e){let t=String(e||"").trim();return t?{Ready:"Gereed","Waiting for room request":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","Cooling enabled, waiting for room temperature above cooling setpoint":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","No dew point source":"Geen dauwpuntbron","OpenQuatt paused":"OpenQuatt gepauzeerd","Cooling disabled":"Koeling uitgeschakeld","Cooling minimum unavailable":"Minimale koel-aanvoer onbekend","Flow too low":"Flow te laag","Fallback active":"Dauwpuntsbenadering actief","Fallback active (+0.5\xB0C warm night)":"Dauwpuntsbenadering actief (+0,5\xB0C warme nacht)","Fallback active (+1.0\xB0C very warm night)":"Dauwpuntsbenadering actief (+1,0\xB0C zeer warme nacht)","Fallback active (+1.5\xB0C tropical night)":"Dauwpuntsbenadering actief (+1,5\xB0C tropische nacht)","User responsibility (no dew point or fallback)":"Expliciet toegestaan (geen dauwpunt of benadering)","Fallback cooling active":"Dauwpuntsbenadering actief","Fallback corrected by warm night":"Dauwpuntsbenadering gecorrigeerd door warme nacht","Fallback blocked by tropical night":"Dauwpuntsbenadering geblokkeerd door tropische nacht"}[t]||t:""}function ef(){let e=!S("coolingRoomRequestRequired")||x("coolingRoomRequestRequired"),t=[de("coolingMinimumSupplyTemp","Minimale koel-aanvoer","Ondergrens voor het koeldoel. OpenQuatt gebruikt de hoogste waarde van deze instelling en de dauwpuntveilige grens."),ct("coolingDemandMax","Maximale koelsterkte","Bepaalt hoe krachtig OpenQuatt mag koelen. Lager geeft langere, rustigere runs; hoger geeft meer koelvermogen bij warm weer.","",{minLabel:"Rustig",maxLabel:"Krachtig",valueLabel:`${Y("coolingDemandMax")} max`}),de("coolingRestartDelta","Herstartmarge watertemperatuur","Na het bereiken van het koel-aanvoerdoel start de watercyclus pas opnieuw zodra de aanvoer deze marge boven het doel ligt."),de("coolingSafetyMargin","Dauwpunt veiligheidsmarge","Extra marge boven het geselecteerde dauwpunt voor de minimale veilige watertemperatuur.")].filter(Boolean),r=[S("coolingRoomRequestRequired")?_p("coolingRoomRequestRequired","Koelvraag via kamerthermostaat","Aan: OpenQuatt wacht op echte koelvraag vanuit de kamer. Uit: koeltoestemming geldt direct als koelvraag.","Koelvraag start en stopt met de marges hieronder.","Koeltoestemming geldt direct als koelvraag. De start- en stopmarge worden nu niet gebruikt.","oq-settings-field--span-2"):"",e?de("coolingRequestOnDelta","Koelvraag start boven setpoint","Koelvraag wordt actief zodra de kamer warmer is dan setpoint plus deze marge."):"",e?de("coolingRequestOffDelta","Koelvraag stopt boven setpoint","Koelvraag valt weer af zodra de kamer koeler is dan setpoint plus deze marge."):""].filter(Boolean),n=r.length>0,a=S("coolingWithoutDewPointMode"),i=[S("coolingGuardMode")?ta("Route",Ee(I("coolingGuardMode","Onbekend"))):"",S("coolingEffectiveMinSupplyTemp")?ta("Actieve ondergrens",I("coolingEffectiveMinSupplyTemp","\u2014")):""].filter(Boolean),l=i.length?qe("coolingGuardStatus","Actuele beveiliging","Laat zien welke route koeling nu begrenst en welke ondergrens daadwerkelijk geldt.",`<div class="oq-settings-cooling-facts">${i.join("")}</div>`,"oq-settings-field--span-2 oq-settings-field--cooling-status"):"",c=[S("outsideTempSelected")?ta("Actuele buitentemperatuur",I("outsideTempSelected","\u2014")):"",S("coolingFallbackNightMinOutdoorTemp")?ta("Nachtminimum buitentemperatuur",I("coolingFallbackNightMinOutdoorTemp","\u2014")):"",S("coolingFallbackMinSupplyTemp")?ta("Berekende minimum watertemperatuur",I("coolingFallbackMinSupplyTemp","\u2014")):""].filter(Boolean),u=c.length?`<div class="oq-settings-cooling-fallback-metrics">${c.join("")}</div>`:"",p=a||c.length>0,w=I("coolingGuardMode","").toLowerCase().includes("fallback");if(!t.length&&!n&&!a&&!l&&!p)return"";let y={"Dew point required":"Gebruik alleen een betrouwbare dauwpuntmeting. Zonder meting blijft koeling uit.","Allow without dew point":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use fallback":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use dew point approximation":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, user responsibility":"Negeer dauwpunt en benadering; alleen de ingestelde minimale koel-aanvoer geldt."};return re("Koeling","Koelingsinstellingen","Stel hier in wanneer koelvraag ontstaat, hoe koud het water mag worden en hoeveel het water mag opwarmen voor herstart.",`
        ...
        `:""}
      `)}function tf(){let e=S("otEnabled"),t=S("cicPollingEnabled")||S("cicFeedUrl"),r=S("cicCompatibilityMode"),n=S("otLinkProblem")||S("cicDataStale")||S("cicJsonFeedOk");if(!e&&!t&&!r&&!n)return"";let a=Nt("cicPollingEnabled"),i=Nt("otEnabled"),l=({label:g,value:d,active:h=!1})=>`
      <div class="oq-settings-integration-diagnostic-item${h?" is-warning":""}">
        <dt>${s(g)}</dt>
        <dd>${s(d)}</dd>
      </div>
    `,c=(g,d,h="Actief",v="Normaal",q={})=>{if(!S(g))return"";let E=ee(g);return l({label:d,value:E?h:v,active:q.warningWhenActive?E:!1})},u=(g,d,h={})=>{let v=h.fallbackKey||"";return!S(g)&&!(v&&S(v))?"":l({label:d,value:He(S(g)?g:v,h)})},p=(g,d)=>{let h=d.filter(Boolean).join("");return h?`
    ...
    `:"";return re("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
      ...
      `},D=({label:$="HA-invoer",valueKey:M="",validKey:B="",value:K=""})=>{if(!M||!B||!S(M)||!S(B))return[];let X=ee(B);return[T({label:$,key:M,value:K,status:X?"Geldig":"Ongeldig",statusTone:X?"valid":"invalid",statusTitle:X?"Home Assistant geeft dit signaal geldig door. OpenQuatt mag deze HA-invoer gebruiken.":"Home Assistant geeft dit signaal niet geldig door. OpenQuatt gebruikt deze HA-invoer dan niet als bron."})]},N=({label:$="MQTT",valueKey:M="",validKey:B="",value:K="",topicKey:X=""})=>{if(!M||!B||!S(M)||!S(B))return[];if(!m(X||c[M]))return[];let ae=ee(B),ie=ae?"MQTT heeft een geldige, recente waarde ontvangen. OpenQuatt mag deze MQTT-invoer gebruiken.":"MQTT heeft nog geen geldige recente waarde ontvangen. OpenQuatt gebruikt deze MQTT-invoer dan niet als bron.";return[T({label:$,key:M,value:ae?K:"\u2014",status:sl(B),statusTone:ae?"valid":"invalid",statusTitle:ie})]},_=({title:$,icon:M="",content:B="",rows:K=[],copy:X="",className:ae=""})=>{let ie=K.filter(Boolean).join("");return!B&&!ie&&!X?"":`
        <section class="oq-settings-source-group${ae?` ${s(ae)}`:""}">
          <h5>
            ${M?`<span class="oq-settings-source-group-icon">${me(M,"oq-settings-source-group-icon-svg")}</span>`:""}
            <span>${s($)}</span>
          </h5>
      ...
      `},J=($,M={})=>{if(!S($))return{markup:"",warning:""};let B=o.entities[$]||{},K=String(C($)||""),X=to(B),ae=new Set(M.hiddenOptions||[]),ie=K&&ae.has(K),ke=X.filter(Ke=>!ae.has(Ke)&&b(Ke,M)),ge=K&&!b(K,M),pe=y(K)&&!u||K==="HA input"&&M.keepUnavailableCurrent!==!0,Le=(ie&&!ke.includes(K)?[K,...ke]:ge&&!pe&&!ke.includes(K)?[K,...ke]:ke).map(Ke=>{let Mt=h(Ke,M);return`<option value="${s(Ke)}" ${Ke===K?"selected":""}>${s(Mt)}</option>`}).join(""),Qe=ge&&pe?'<option value="" selected disabled>Kies een beschikbare bron</option>':"";return{markup:`
        ...
        `,warning:ie?"Huidige bron is legacy; kies een nieuwe bron.":ge?`Huidige bron niet beschikbaar: ${g(K,M)}`:""}},L=({key:$,title:M,icon:B="",select:K,secondarySelect:X=null,secondarySelects:ae=null,activeRows:ie=[],measurementRows:ke=[],activeCopy:ge="",rows:pe=[]})=>{let bt=K&&K.when!==!1?J(K.key,K):{markup:"",warning:""},Qe=(Array.isArray(ae)?ae:X?[X]:[]).filter(se=>se&&se.when!==!1).map(se=>J(se.key,se)).filter(se=>se.markup),Ke=Qe.map(se=>se.markup).join(""),Mt=Qe.map(se=>se.warning).find(Boolean)||"",uo=pe.filter(Boolean).join(""),rt=`${bt.markup}${Ke}`,Oe=bt.warning||Mt,dn=[_({title:"Configuratie",icon:"settings",className:"oq-settings-source-group--config",content:rt?`
            <div class="oq-settings-source-controls">
              ${rt}
            </div>
            ${Oe?`<p class="oq-settings-source-warning">${s(Oe)}</p>`:""}
          `:""}),_({title:"Actief",icon:"target",rows:ie,copy:ge,className:"oq-settings-source-group--active"}),_({title:"Metingen",icon:"activity",rows:ke,className:"oq-settings-source-group--measurements"})].filter(Boolean).join("");return!dn&&!rt&&!uo?"":`
    ...
    `:"";return re("Integratie","MQTT inputbronnen","Beheer de brokerverbinding voor externe MQTT-bronwaarden.",`
      ...
      `)}function sf(){let e=Ie.has(o.settingsGroup)?o.settingsGroup:be[0].id;return`
    ...
    `}function lf(){let e=Ie.has(o.settingsGroup)?o.settingsGroup:be[0].id;return`
...
`)}function Nk(e,t=""){if(!e.length)return`
      ...
      `;let r=Rk(e),n=Hk(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,c)=>{let u=r.left+r.barSlot*c+r.barSlot/2,p=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],m=r.height-r.bottom,w=p.map(d=>{let h=Qr(l,d.key);if(h<=0)return"";let v=h/r.axisMax*r.plotHeight;return m-=v,`
        ...
        `}).join(""),b=e.length<=12||c===0||c===e.length-1||c%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||Ft(l.dateKey))}</text>`:"",g=Pk(l);return`
    ...
    `}function _k(e,t,r,n){let a=Be(r)==="day"?Number(n):Number.NaN,i=Number.isFinite(a)?e.find(l=>l.dateKey===a):null;return i?[i]:t}function Hc(){let e=oa(),t=Be(o.energyHistoryView),r=Fi(e,t),n=Sk(e,t,r),a=_k(e,n.buckets,t,r.selectedValue),i=qk(a);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:i,activeView:t}}function Sf(e=Hc()){return ve({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,summary:e.summary})}function qf(e=Hc()){let t=e.summary,r=Ek(t),n=e.buckets[0]?.dateKey?Ft(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?Ft(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
    ...
    `}function Lk(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=$c();return ot(t,df(n),pf(n))}function Ok(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=Hc(),a=Ck();return(a?!1:ot(t,Sf(n),qf(n)))||a}So({patchEnergyDom:Lk,patchResultsDom:Ok});function Ik({label:e,value:t,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(t)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function ia(e,t=!1){return e.map(r=>Ik({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?it(r.key):r.value,status:t})).join("")}function _c(e){return`
      <div class="oq-overview-sectionhead">
        <h3>${s(e)}</h3>
      </div>
    `}function Lc(){let e=Zt();return`
    ...
    `}function Ic(e,t,r,n){return`${r?jk(n):""}${Fk(e,t)}`}function $f(e,t,r,n,a){if(!e)return;let i=ve({mode:t,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&(Wt(e,Ic(t,r,n,a)),e.dataset.renderSignature=i)}function ji(e,t,r=""){return`
    ...
    `}function Vk(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}var Bk=Bo;function Kk(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function zk(e){let t=Kk(I("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Uk(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=Bk(r),a=S("timeValid")&&x("timeValid")?zk(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Gk(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Af(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>S(e))||""}function Qk(){return["hp1WaterIn","hp2WaterIn"].find(e=>S(e))||""}function Hf(e=I("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function Vt(){return Hf()}function Rf(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function Fc(){return Vt()?"Koeling":le()?"Stooklijn":"Power House"}function Yk(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=H(t);if(!Number.isNaN(r))return r}return Number.NaN}function Pf(){let e=Yk(),t=H("phouseHouse"),r=H("totalHeat"),n=H("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:U(e,0,"W"),houseText:U(t,0,"W"),correctionText:Gk(a),capacityText:it("hpCapacity"),statusTitle:i,statusCopy:l}}function Nf(){let e=H("curveSupplyTarget"),t=H("supplyTemp"),r=Af(),n=r?H(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:it("curveSupplyTarget"),supplyText:it("supplyTemp"),deltaText:Vk(a),capacityText:it("hpCapacity"),statusTitle:l,statusCopy:c}}function Df(){let e=H("supplyTemp"),t=I("coolingGuardMode",""),r=I("coolingFallbackNightMinOutdoorTemp","\u2014"),n=H("coolingSupplyError"),a=H("coolingDemandRaw"),i=x("coolingPermitted"),l=x("coolingRequestActive"),c=I("coolingBlockReason","Onbekend"),u=xc(c),p=Rf(c,l),m="Wacht op koelvraag",w="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return p?(m="Wacht op koelvraag",w="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(m="Houdt doel vast",w="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(m="Trekt aanvoer omlaag",w="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(m="Benadert koeldoel",w="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(m="Koelt rustig door",w="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(m="Koeling gereed",w="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(m="Koeling geblokkeerd",w=`Blokkade: ${u}.`),{targetText:it("coolingSupplyTarget"),supplyText:it("supplyTemp"),safeFloorText:it("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:it("coolingDemandRaw"),statusTitle:m,statusCopy:w,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:p}}function Wi(){if(Vt()){let t=Df(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(le()){let t=Nf();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=Pf();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function jc(e){return Mf({className:"oq-overview-system",title:e.title,copy:e.copy,signature:ve(e),body:`
      ...
      `})}function Jk(){if(!x("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(Vt()){let n=Df(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(cf())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(le()?Nf():Pf()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Zk(){return x("openquattEnabled")?Vt()?x("coolingPermitted")?x("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:I("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:x("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:x("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Wc(e,t){let r=Jk(),n=Zk();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Vc(e,t){let r=Wc(e,t);return`
    ...
    `}function fE(){let e=io(),t=ac();return`
    ...
    `}function jf(){let e=yt(),r=Of().length>0;return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
    ...
    `}function DE(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(Wt(e,Uf(t)),!0):(Wt(r,`
      <h3>Warmtepompen</h3>
      ${Bf(t)}
    ...
    `}function x1(){let e=gt(),t=ga(),r=ZE(),n=Do().filter(m=>!m.quick&&!m.custom),a=XE(),i=e1(a),l=eh(a.start),c=eh(a.end),u=o.controlReplayPeriodMenuOpen,p=e==="custom"?"Eigen periode":r.some(m=>m.id===e)?"Kies periode":t.shortLabel;return`
        ...
        <div class="oq-working-status-grid">
          <section class="oq-working-status-main${r?"":" oq-working-status-main--wide"}">
            ...
            <div class="oq-working-reason-list">
              ${t.checks.map(d=>`<span>${me("target","oq-working-reason-icon")} ${s(d)}</span>`).join("")}
            </div>
          </section>
          ${r?`
