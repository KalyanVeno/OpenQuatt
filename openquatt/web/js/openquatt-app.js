...
`)),r.toString()}function ol(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=Mw(l);if(!c)return;let u=[...a,c],d=Yd(u,t);if(a.length&&d.length>yw){r.push({keys:n,body:i}),n=[l],a=[c],i=Yd(a,t);return}n=[...n,l],a=u,i=d}),a.length&&r.push({keys:n,body:i}),r}async function Aw(e){let t=el(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await gn(ww,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function Hw(){let e=el(),r=await gn(vw,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function Rw(){let e=el(),r=await gn(Sw,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function Pw(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.attention_count??""].join("|")}async function Nw(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<qw)return!1;o.decisionLogFetchPromise=(async()=>{let n=await Rw(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=Pw(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function Dw(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{an.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=sp(r,o.entities[r],n))})}async function _w(e){if(!e.length)return{ok:!0,message:""};try{let t=await Hw();return Dw(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function B(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||an.has(b)||!xw(b,n));if(!i.length)return;let l=i.filter(b=>an.has(b)),c=i.filter(b=>!an.has(b)),d=(await _w(l)).ok?c:[...c,...l];if(!d.length){Jd(),kn(Ut()),o.busyAction||(Qd(),o.controlError="");return}let g=Number(r.concurrency),y=Number.isFinite(g)&&g>0?Math.floor(g):Vt,w=ol(d,t),f=Math.max(1,Math.min(y,Vt)),m=[];for(let b=0;b<w.length;b+=f){let S=w.slice(b,b+f),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await Aw(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let p="";m.forEach(b=>{if(b.status!=="fulfilled"){let x=b.reason.message||String(b.reason);p||(p=x);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(x=>{if(Object.prototype.hasOwnProperty.call(E,x)){o.optionalMissingEntities&&delete o.optionalMissingEntities[x],o.entities[x]=sp(x,o.entities[x],E[x]);return}let P=O[x];P?.optional?k.has(x)&&$w(x,n):p||(p=`${P?.name||x} ontbreekt in bulk response`)})}),Jd(),kn(Ut()),p?(ip(p),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${p}`):o.busyAction||(Qd(),o.controlError="")}function sp(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function Jd(){o.complete=$a(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=Vu(),o.appView==="diagnosis"&&!gt()&&$t(co(),{syncMode:"replace",forceSync:!0}),o.appView||$t(Sn()||co(),{syncMode:"replace",forceSync:!0})}function Ow(){let e=Qs();return o.appView==="settings"?[...new Set([...e,...Ys()])]:o.appView==="energy"||o.appView==="results"?En(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?En(o.appView,{forceFast:!0}):[...new Set(e)]}function Lw(e=[]){let t=new Set(e);return(o.appView==="settings"?Ys():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...En(o.appView,{includeBulk:!0}),...Bt])]:o.appView==="energy"||o.appView==="results"?[...new Set([...En(o.appView,{forceFast:!0}),...Bt])]:[...new Set(["setupComplete","strategy",...nt])]).filter(n=>!t.has(n))}function Zd(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=Eu}async function be(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||Da()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){pw(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<dn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&Zd(t,e),d=t-Number(o.lastStaticEntitySyncAt||0)>=Tu||o.updateInstallBusy||o.updateInstallPhaseHint?Bt:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?mt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Ao:[],w=Ls()?Vo():[],f=n?[...rs,...nt,"setupComplete",...d]:i?[...En(a,{forceFast:l,includeBulk:c}),...d]:r==="settings"?[...new Set([...Ys(),...w,...d])]:c?["setupComplete",...d,...nt,"strategy",...Mo,...rt,...ie()?ge.map(m=>m.key):Et]:["setupComplete",...nt,"strategy",...d];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let m=o.deviceReconnectMode,p=Tw(t,e)?await Cw():{ok:!0,message:""};if(!p.ok){ip(p.message),n||h();return}if(await B([...new Set([...f,...g,...y])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?jt:Vt}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),d.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&ep();let b=m!==o.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="control"||a==="diagnosis"?await Is():!1,E=S?!1:o.appView==="results"?await Oa():!1,k=a==="control"?await Nw({force:e.forceDecisionLog===!0}):!1,x=S?!1:Ls()?(await Promise.all([$d(),Oa({metaOnly:!0})])).some(Boolean):!1,P=S||!td()?!1:await Lo(),D=S||!od()?!1:await Io(),V=S||!jd()?!1:await Bo({force:o.systemModal==="mqtt-sensors"}),A=Ed();if(S&&!o.nativeOpen&&np(Js(a)),b){h();return}if(q&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){h();return}if(E&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){h();return}if(k&&o.appView==="control"){Fs()||h();return}if(x&&o.appView==="settings"){h();return}if(P&&(o.systemModal==="login"||Kt())){h();return}if(D&&o.appView==="settings"){h();return}if(o.systemModal==="mqtt-sensors"){Ia()!==o.mqttSensorsModalRenderSignature&&h();return}if(V&&o.appView==="settings"){h();return}let K=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(A!==o.headerRenderSignature)if(K&&Ns())o.headerRenderSignature=A;else{h();return}else Ns();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){bn()!==o.settingsRenderSignature&&h();return}if(K)return;if(o.appView==="settings"){let H=bn();if(H!==o.settingsRenderSignature&&!o.focusedField){h();return}if(!Cd()){o.focusedField||h();return}o.focusedField&&(o.settingsRenderSignature=H);return}if(o.appView==="diagnosis"){Pd()||h();return}if(o.appView==="control"){Fs()||h();return}if(o.appView==="energy"){Ad()||h();return}if(o.appView==="results"){Hd()||h();return}Rd()||h()}catch(m){n||(o.controlError=`Helperstatus kon niet worden geladen. ${m.message}`,h())}finally{o.entitySyncInFlight=!1;let m=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,m&&!o.nativeOpen&&window.setTimeout(()=>{be(m)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!m&&Zd(Date.now())&&window.setTimeout(()=>{be({forceBulk:!0})},uw)}}function At(){return{}}function Iw(){let e=String(T("hpGeneration")||"").trim();return e||""}function Tn(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function Fw(){if(!Array.isArray(wr))return"";if(wr.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return wr.every(t=>e[t])?"single":""}function rl(e){let t=Tn(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Ww(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function ne(){let e=Tn(T("installationTopology"));if(e==="single"||e==="duo")return rl(e);let t=Tn(At().installation);if(t==="single"||t==="duo")return rl(t);let r=Fw();return r?rl(r):Ww()}function Ht(){let e=ne(),t=Iw();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Qe(){return"OpenQuatt"}function nl(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function it(e=mo()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function zo(e=ne()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function Cn(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(At().hardwareProfile||e).trim().toLowerCase()}function mo(){return nl(T("connectionText")||At().connection)}function lp(){let e=mo();return e==="wifi"?"eth":e==="eth"?"wifi":""}function cp(){let e=ne();return e==="single"?"duo":e==="duo"?"single":""}function Er(e=ne(),t=mo()){let r=zo(e),n=Cn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${it(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${it(t)}`:n==="waveshare"?`Waveshare ${r} ${it(t)}`:`${Qe()} ${r} ${it(t)}`}function al(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function Fa(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${al()}`}function Vw(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function jw(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Tr(){let e=jw(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=Vw(o.entities.uptime);if(n==="d")return Oo(e*1440);if(n==="h")return Oo(e*60);if(n==="s")return Oo(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(At().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Oo((Date.now()-r)/6e4)}function Cr(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(At().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function Uo({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let w=e();if(!w)return null;let f={scrollTop:w.scrollTop};if((i||l)&&(f.scrollHeight=w.scrollHeight),l){let m=w.scrollHeight-w.scrollTop-w.clientHeight;f.stickToBottom=m<c}return a&&(f.identity=a(w)),f}function d(w){if(!w)return;let f=e();if(!f||a&&a(f)!==w.identity)return;if(w.stickToBottom){f.scrollTop=f.scrollHeight;return}let m=i?f.scrollHeight-Number(w.scrollHeight||0):0;f.scrollTop=Math.max(0,Number(w.scrollTop||0)+m)}function g(w,f=!0){if(!w)return;let m=Number(t()||0)+1;r(m);let p=()=>{Number(t()||0)!==m||!n()||d(w)};if(f){window.requestAnimationFrame(p);return}p()}function y(){r(Number(t()||0)+1)}return{capture:u,invalidate:y,queue:g,restore:d}}function $e(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function go({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function il(e,t,r="",n=""){let a=t?r:n;return a?`<p data-oq-switch-copy="${s(e)}" data-on-copy="${s(r)}" data-off-copy="${s(n)}">${s(a)}</p>`:""}function up(e,t,r,n="",a="",i=""){if(!v(e))return"";let l=!!T(e),c=o.loadingEntities||o.busyAction===`switch-${e}`;return we(e,t,r,`
  ...
  `}function ll(e,t,r,n=""){if(!v(e))return"";let a=pn(T(e));return we(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function oe(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var mp=250;function gp(){return[]}function $r(){return typeof window>"u"?!1:gp().length>0}function Kw(){return`${We()}/events`}function zw(){return`${We()}/openquatt/logs/recent`}function ja(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Ba(){return o.nativeOpen?"Niet beschikbaar":$r()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function Uw(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function fp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return Uw(t)}function Gw(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Qw(){return o.nativeOpen?"Niet beschikbaar":$r()?ja()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":ja()?"Buffer aan":"Buffer uit"}function Yw(){return ja()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function cl(){return o.entities?.debugLevel||null}function hp(e=cl()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function Jw(e=cl()){let t=String(e?.value??e?.state??"").trim(),r=hp(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function bp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function Zw(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function wp(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(Zw);let n=[];for(let a of r){let i=n[n.length-1]||null;bp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-mp)}function ul(e,t={}){let r=by(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:wy(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function Xw(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function ey(){let e=gp(),t=e.length;return e.map((r,n)=>ul(r,{receivedAt:Xw(n,t),seq:n+1}))}function ty(){let e=hl();e&&(e.scrollTop=e.scrollHeight)}var dl=Uo({getScroller:hl,getToken:()=>o.webServerLogScrollRestoreToken,setToken:e=>{o.webServerLogScrollRestoreToken=e},isActive:()=>o.systemModal==="webserver-logs",preserveGrowth:!0,stickToBottom:!0}),Jt=dl.capture,Zt=dl.queue;function oy(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}var yp=Uo({getScroller:oy,getToken:()=>o.cm100CommissioningScrollRestoreToken,setToken:e=>{o.cm100CommissioningScrollRestoreToken=e},isActive:()=>o.systemModal==="cm100-commissioning",preserveGrowth:!0,stickToBottom:!0}),vp=yp.capture,pl=yp.queue;function ry(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}var Sp=Uo({getScroller:ry,getToken:()=>o.serviceTaskModalScrollRestoreToken,setToken:e=>{o.serviceTaskModalScrollRestoreToken=e},isActive:()=>String(o.systemModal||"").startsWith("service-task-")}),qp=Sp.capture,ml=Sp.queue;function ny(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}var kp=Uo({getScroller:ny,getToken:()=>o.historyStorageModalScrollRestoreToken,setToken:e=>{o.historyStorageModalScrollRestoreToken=e},isActive:()=>o.systemModal==="history-storage"}),Ep=kp.capture,gl=kp.queue;async function Mr(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||Jt(),r=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=r,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let n=await window.fetch(zw(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==r)return;let i=iy(a);o.webServerLogHistoryLoaded=!0,i.length>0&&(wp(i,{prepend:!0}),o.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),o.webServerLogRecentAnchorAt=Date.now())}catch(n){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(h(),Zt(t))}}function ay(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?ul(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function iy(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>ay(r,n+1)).filter(r=>r!==null)}function sy(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function ly(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>bp(e,r))}function cy(){$r()&&o.webServerLogEntries.length===0&&ft({webServerLogEntries:ey()}),ft({webServerLogCopyMessage:"",webServerLogCopyError:""}),o.systemModal="webserver-logs",h(),B(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=Jt();h(),Zt(e)}),ty(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&Mr()}function Ar(){ft({webServerLogEntries:[],webServerLogError:"",webServerLogHistoryError:"",webServerLogHistoryLoading:!1,webServerLogHistoryLoaded:!1,webServerLogCopyMessage:"",webServerLogCopyError:"",webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1,webServerLogRecentTail:[],webServerLogRecentAnchorAt:0}),dl.invalidate(),o.systemModal==="webserver-logs"&&h()}function uy(){let e=Jt();Rt(),ft({webServerLogEnabled:null,webServerLogConnected:!1}),Ar(),o.systemModal==="webserver-logs"&&Mr({scrollState:e})}function fl(){if($r()){Rt();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs")){Rt();return}if(o.webServerLogEnabled===!1){Rt();return}o.webServerLogSource||dy()}function dy(){if($r()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",h();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",h();return}try{let e=new window.EventSource(Kw());o.webServerLogSource=e,e.addEventListener("open",py),e.addEventListener("ping",my),e.addEventListener("log",fy),e.onerror=gy}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",Rt(),h()}}function Rt(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function py(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Jt():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",h(),Zt(e)}Nd({clearOutput:Ar,closeStream:Rt,resetRecoveryState:uy});function my(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Jt():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",h(),Zt(e))}function gy(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?Jt():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",Rt(),h(),Zt(e)}function fy(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=Jt(),r=hy(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>ul(u)).filter(u=>!sy(u)&&!ly(u));if(i.length===0)return;wp(i);let l=vy(),c=hl();yy(l),Sy(i,l),o.webServerLogEnabled=!0,c&&t&&Zt(t,!1)}function hy(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).trimEnd()}catch{}return t}function by(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function wy(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function yy(e){for(;o.webServerLogEntries.length>mp;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function vy(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function hl(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function Sy(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",Tp(r))}}function Tp(e){let t=fp(e.receivedAt),r=Gw(e.receivedAt);return`
  ...
  `}function Ty(){let e=cl();if(!e)return"";let t=hp(e),r=Jw(e),n=o.loadingEntities||o.busyAction==="save-debugLevel";return`
...
`)}async function xy(){let e=Cy();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",h();return}try{if(!await io(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&h()}var $y={"open-webserver-log-modal":()=>cy(),"clear-webserver-log-output":()=>Ar(),"copy-webserver-log-output":()=>xy()};function Cp(e){return se($y,e)}function xp(){let e=$r();return ee({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
    ...
    `}function Fy(e,t,r,n){if(!_p())return"";let i=!!(Xt()||o.updateInstallBusy||lt());return`
    ...
    `}function By(){if(!o.updateManualUploadOpen)return"";let t=!!(Xt()||o.updateInstallBusy||lt()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Op(){if(!o.updateModalOpen)return"";let e=ye(),t=o.entities.firmwareUpdateChannel||null,{current:r,latest:n}=ql(),a=lt(),i=Hr(),l=Zo(),c=Iy(),u=Xt(),d=yl(),g=Ny(),y=d?"Firmware-update afgerond":u||i?"Firmware-update bezig":a?"Controleren op firmware-update":Dy(),w=t?Array.isArray(t.option)?t.option:Array.isArray(t.options)?t.options:[]:[],f=za(),m=Ua(),p=!!(f&&!d),b=!!(m&&!d);return ee({id:"firmware-update",titleId:"oq-update-modal-title",kicker:"OTA-update",title:y,copy:c,backdropClass:a||i||u?"is-busy":"",className:"oq-helper-modal--firmware oq-helper-modal--scrollable",closeAction:"close-update-modal",closeLabel:"Sluit update-popup",body:`
          ${d?`
  ...
  `}function Gy(){if(!o.root)return;if(o.interfacePanelOpen){h();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=kl();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function Qy(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Pn()),r&&(r.textContent=Za())}function El(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function Yy(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${El(e)}</span>`}function eo(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${El(e)}</span>`}function Jy(){return[...Po]}function Zy(){let e=[];return Po.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function Xy(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function Xa(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function er(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function vo(e){return`${We()}/openquatt/debug-recording/${e}`}function Dr(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Tl(){Dr({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function ev(){let e=await window.fetch(vo("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Dr(t),t}function Nn(e=2e3){er(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{ei({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function ei(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",h());try{await ev(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),Nn()}catch(t){Tl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?h():(Gy(),Qy())}}async function Kp(){let e=ol(Po,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch(vo(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==Po.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${Po.length})`);return t}async function tv(e){let t=Math.max(1,Number(e)||15);Xa(),er(),Aa({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await Kp();let r=await window.fetch(vo(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();Dr(n),Nn()}catch(r){Tl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,h()}}async function ov(){Xa(),er(),Aa({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await Kp();let e=await window.fetch(vo("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();Dr(t),Nn()}catch(e){Tl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function rv(){let e=await window.fetch(vo("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return Dr(t),er(),t}async function nv(){Xa(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{await rv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function av(e={}){Xa(),er(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{let t=await window.fetch(vo("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();Dr(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,h()}}function Lp(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(vu/1e3),sample_count:Pt(),column_count:Po.length,storage:"browser"},columns:Jy(),units:Zy(),initial:Xy(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function Ya(e){return JSON.stringify(e)}function iv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([Ya(Lp())]).size}catch{return Ya(Lp()).length}}function sv(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function lv(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function cv(){if(Pt()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Nt();h();try{let t=await window.fetch(vo("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,fs(lv(r),Ya(r),"application/json"),Bp(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,h()}}async function uv(){if(Pt()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Nt();h();try{let t=await window.fetch(vo("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await io(Ya(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");Bp(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,h()}}var dv={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",h(),ei()),"start-debug-recording":e=>tv(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>ov(),"select-debug-recording-duration":e=>zy(e.dataset.debugMinutes||15),"stop-debug-recording":()=>av(),"freeze-debug-recording":()=>nv(),"download-debug-recording":()=>cv(),"copy-debug-recording":()=>uv()};function zp(e,t){return se(dv,e,t)}function Up(){let e=o.debugRecordingActive,t=Nt(),r=Ja(),n=Pt(),a=o.debugRecordingBusy,i=sv(iv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Ky(),u=Vp(),d=Ip(),g=Uy(),y=n>0,w=`${g.toFixed(1)}%`,f=[{icon:"status",label:"Status",value:Pn()},{icon:"clock",label:t?"Retentie":"Duur",value:Xo(t?d:Fp())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],m=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return ee({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:Za(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
        ...
        <div class="oq-debug-recording-actions">
          ...
          `}
          <button class="oq-helper-button oq-helper-button--ghost" type="button" data-oq-action="download-debug-recording" ${!y||a?"disabled":""}>${eo("download")}${e&&t?"Download tot nu toe":"Download supportbestand"}</button>
          <button class="oq-helper-button oq-helper-button--ghost" type="button" data-oq-action="copy-debug-recording" ${!y||a?"disabled":""}>${eo("copy")}${e&&t?"Kopieer tot nu toe":"Kopieer data"}</button>
          ${m?`
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:Vo(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,jr(),o.appView==="results"&&nr({force:!0}).then(()=>h())})},"select-energy-history-import-file":()=>nS(),"clear-energy-history-import-file":()=>{km(),h()},"import-energy-history-file":()=>aS(),"export-energy-history":()=>Bv(),"open-history-storage-modal":()=>{o.systemModal="history-storage",h();let e=di({forceMissing:!0,forceTrendHistory:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&h()});return jr([1e3,3e3,7e3]),e},"download-settings-backup":()=>mS(),"open-settings-backup-import":()=>{o.systemModal="settings-backup-import",h()},"confirm-settings-backup-restore":()=>gS()};function Pm(e,t){return se(fS,e,t)}function jl(e,t,r="",n=!1){return`
    ...
    `}function CS(){if(!o.root)return!1;let e=o.root.querySelector(".oq-helper-status-grid");if(!e)return!!o.root.querySelector(".oq-helper-hub");let t=Bm();if(e.querySelectorAll("[data-oq-header-status]").length!==t.length)return e.outerHTML=Kn(),!0;for(let[n,a,i,l]of t){let c=e.querySelector(`[data-oq-header-status="${n}"]`);if(!c)return e.outerHTML=Kn(),!0;let u=jm(n),d=!!(l||u);if(c.tagName.toLowerCase()!==(d?"button":"div"))return e.outerHTML=Kn(),!0;let g=c.querySelector(".oq-helper-status-label"),y=c.querySelector(".oq-helper-status-value");if(!g||!y)return e.outerHTML=Kn(),!0;g.textContent!==a&&(g.textContent=a);let w=zm(n),f=w?`<span class="oq-helper-status-value-text">${s(i)}</span><span class="oq-helper-status-badge" aria-label="Update beschikbaar" title="Update beschikbaar"></span>`:s(i);y.innerHTML!==f&&(y.innerHTML=f),d?c.setAttribute("data-oq-action",u):c.removeAttribute("data-oq-action"),c.classList.toggle("oq-helper-status-item--button",d),c.classList.toggle("oq-helper-status-item--attention",w)}return!0}function xS(){return""}function Yl(){return""}function Um(){let e=o.nativeOpen?"native":"app",t=Km();return o.interfacePanelOpen?`
    ...
    `;return we("phResponseProfile","Power House responsprofiel","Kies hoe rustig of direct Power House mag reageren op veranderingen in je woning.",n,"oq-settings-field--span-2")}function oc(){if(!v("curveControlProfile"))return"";let e=String(T("curveControlProfile")||""),t=o.loadingEntities||o.busyAction==="save-curveControlProfile",n=`
    ...
    `;return we("curveControlProfile","Regelprofiel","Kies of de stooklijn vooral comfortabel, gebalanceerd of rustig moet reageren.",n,"oq-settings-field--span-2")}function HS(){let e=(N,Z=0)=>{let I=R(N);return Number.isNaN(I)?Z:Math.max(0,I)},r=e("phComfortBelow",.1),n=e("phComfortAbove",.3),a=e("phKp",3e3),i=20-r,l=20+n,c=620,u=184,d=46,g=24,y=18,w=40,f=96,m=c-d-g,p=Math.min(20-1.2,i-.35),b=Math.max(20+1.2,l+.35),S=N=>d+(N-p)/Math.max(.01,b-p)*m,q=S(p),E=S(b),k=S(i),x=S(20),P=S(l),D=Math.abs(i-20)>.001,V=Math.abs(l-20)>.001,A=y+24,K=u-w,H=f-44,F=(N,Z,I,W="")=>{let St=Math.max(q+4,Math.min(E-110-4,N-55)),pt=N-14,Ft=H,Xe=28,De=f-H+16;return`
      ...
      `},_=[`M ${q.toFixed(1)} ${A.toFixed(1)}`,`L ${k.toFixed(1)} ${f.toFixed(1)}`,`L ${P.toFixed(1)} ${f.toFixed(1)}`,`L ${E.toFixed(1)} ${K.toFixed(1)}`].join(" ");return`
      <div class="oq-ph-concept-card">
        <div class="oq-ph-concept-visual">
          ...
          <svg class="oq-ph-concept-svg" viewBox="0 0 ${c} ${u}" role="img" aria-label="Grafiek voor Power House tuning">
            ...
            ${V?F(P,"Comfort boven setpoint",z(l,1,"\xB0C"),"above"):""}
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=Qt(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function ng(){let e=Ht(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&Qt(t).length>0;return!e&&!r?"":oe("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function sg(){let e=wo(),t=Fa(),r=o.busyAction==="restartAction";return oe("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
    ...
    `}function JS(){return o.currentStep==="generation"?dg():o.currentStep==="boiler"?v("boilerCvAssistEnabled")?KS():ug():o.currentStep==="flow-source"?VS():o.currentStep==="thermostat-source"?jS():o.currentStep==="flow"?zS():o.currentStep==="heating"?US():o.currentStep==="water"?GS():o.currentStep==="silent"?QS():o.currentStep==="confirm"?YS():ug()}function ar(){return fr.filter(e=>!e.optionalEntity||v(e.optionalEntity))}function _t(e){let t=ar().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function ZS(e){let t=wi(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function XS(e=!1){return ar().map((t,r)=>{let n=ZS(r);return`
        <button
      ...
      `}).join("")}function eq(){let e=ar();return e.find(t=>t.id===o.currentStep)||e[0]||fr[0]}function wi(){return Math.max(0,ar().findIndex(e=>e.id===o.currentStep))}function wg(e){let t=ar(),r=Math.min(t.length-1,Math.max(0,wi()+e));o.currentStep=t[r]?.id||fr[0].id}function to(e={}){let t=wi(),r=ar(),n=t>0?r[t-1]:null,a=t<r.length-1?r[t+1]:null;return`
    ...
    `}function tq(){let e=wi(),t=ar();return`
...
`)}function Hq(e,t=""){if(!e.length)return`
    ...
    `}function Oc({bind:e,x:t,y:r,width:n,value:a,label:i,ariaLabel:l="",align:c="start"}){let u=l||`${i} temperatuur ${a}`,d=c==="end",g=c==="center",y=g?"middle":d?"end":"start",w=g?t+n/2:d?t+n-2:t+2;return`
      <g
    ...
    `}function yk({tooltip:e,...t}){return`${Oc(t)}${ea({bind:t.bind,...e})}`}function vk({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
      ...
      <div class="${s(e.boardClass)}" data-oq-hp-board="${s(e.title)}">
        <div class="oq-hp-tech-shell">
          <div class="oq-hp-tech-visual">
            ...
            ${Object.entries(e.pipes).map(([d,g])=>_c(d.replace(/[A-Z]/g,y=>`-${y.toLowerCase()}`),g.tone,g.d,g.animated,g.flowVariant)).join("")}
    ...
    `}function Lc(){if(!Wc())return"";let e=Hi();return o.hpVisualMode!=="schematic"?Mk(e):`
    ...
    `}function Hk(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(Ot(e,pf(t)),!0):(Ot(r,`
      <h3>Warmtepompen</h3>
      ${cf(t)}
    ...
    `}function q1(){let e=dt(),t=aa(),r=Gk(),n=To().filter(g=>!g.quick&&!g.custom),a=Qk(),i=Yk(a),l=yf(a.start),c=yf(a.end),u=o.controlReplayPeriodMenuOpen,d=e==="custom"?"Eigen periode":r.some(g=>g.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function k1(e){let t=Zc(e.severity);return`
    ...
    `}function E1(e,t){let r=Zc(e.severity),n=t&&t.id===e.id,a=Af(e.kind),i=Mf(e);return`
      <button
    ...
    `}function Wf(e){if(!e)return"";let t=Zc(e.severity),r=ur(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=Hf(e),l=Mf(e),c=Array.isArray(e.checks)?e.checks:r.checks;return`
    ...
    `}function C1(e,t){let r=aa(),a=e.filter(c=>!c.timelineHidden).slice(0,80),i=String(o.decisionLogError||"").trim(),l=!a.length&&!o.decisionLog&&!i;return`
    ...
    `}function x1(e){let t=ur(e.primaryReason),r=Hf({primaryReason:e.primaryReason,source:e.hp1Running&&e.hp2Running?"HP1 + HP2":e.hp1Running?"HP1":e.hp2Running?"HP2":"Geen bron"}),n=!!e.coolingProtection,a=!!e.coolingCapped,i=e.primaryReason==="restart_wait",l=e.primaryReason==="buffer_stop",c=e.primaryReason==="sticky_protection",u=l?"Koelregeling":"Bescherming",d=l?"Water al koud genoeg":n?i?"Wacht op veilige herstart":"Koeling tijdelijk beperkt":a?"Koeling met ingesteld maximum":c?"Geen comfortvraag actief":"Geen beperking actief",g=l?"Dit is normale regeling. De koelvraag blijft actief, maar de warmtepomp hoeft nu geen extra koude aan het water toe te voegen.":n?i?"De koelvraag blijft aanwezig. De warmtepomp start opnieuw zodra de veilige marge voldoende is hersteld.":"De aanvoer blijft boven de veilige grens. Daarom koelt het systeem tijdelijk minder hard.":a?"Dit is normale koeling binnen de ingestelde softwaregrens. Dauwpunt en waterflow blijven wel gewoon bewaakt.":c?"Alleen de pomp draait kort. De warmtepompen blijven uit en er worden geen compressorstarts geteld.":"Ontdooien, minimum rusttijd, dauwpunt en waterflow blijven bewaakt. Ze verschijnen hier zodra ze gedrag begrenzen.",y=l?[["Koelvraag actief","info","snowflake"],["Water koud genoeg","normal","droplet"],["Automatische herstart","context","activity"]]:n?[["Dauwpunt bewaakt","limited","droplet"],[`Max. niveau ${e.cooling.allowedMax}`,"info","target"],[`Nu niveau ${e.cooling.limitedDemand}`,"context","bar-chart"]]:a?[[`Ingesteld max. ${e.cooling.allowedMax}`,"info","target"],[`Nu niveau ${e.cooling.limitedDemand}`,"normal","bar-chart"],["Marge bewaakt","context","shield"]]:c?[["Korte pomprun","normal","shield"],["Geen koelvraag","context","snowflake"],["Geen warmtepompstart","info","activity"]]:[["Ontdooien vrij","normal","snowflake"],["Rusttijd vrij","normal","activity"],["Waterflow bewaakt","info","waves"]],w=[["Aanvoer",e.supplyTemp],["Buiten",e.outsideTemp],["Flow",e.flow],["Strategie",e.strategyLabel]];return(e.cooling.requestActive||n||a)&&(w.push(["Dauwpunt",e.cooling.dewPoint]),w.push(["Veilige min.",e.cooling.safeSupply])),`
        ...
        <div class="oq-working-status-grid">
          <section class="oq-working-status-main${r?"":" oq-working-status-main--wide"}">
            ...
            <div class="oq-working-reason-list">
              ${t.checks.map(f=>`<span>${Re("target","oq-working-reason-icon")} ${s(f)}</span>`).join("")}
            </div>
          </section>
          ${r?`
    ...
    `}function N1(){return ee({modalId:"initial-load",titleId:"oq-loading-modal-title",kicker:"OpenQuatt",title:"OpenQuatt laden",backdropClass:"oq-helper-modal-backdrop--loading",modalClass:"oq-helper-modal--reconnect oq-helper-modal--loading",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `})}function D1(){return o.appView==="overview"?mf():o.appView==="control"?Kf():o.appView==="energy"?Wg():o.appView==="diagnosis"?af():o.appView==="results"?Vg():P1()}function _1(){let e=Ql();return`
    ...
    `}function O1(){let e=typeof document<"u"?document.activeElement:null;return!e||typeof e.matches!="function"?null:e.matches("select[data-oq-dev-control]")?e:null}function L1(e){if(!e||o.deferDevControlSelectRender)return;o.deferDevControlSelectRender=!0;let t=()=>{e.removeEventListener("blur",t),e.removeEventListener("change",t),o.deferDevControlSelectRender=!1,window.setTimeout(()=>zf(),0)};e.addEventListener("blur",t,{once:!0}),e.addEventListener("change",t,{once:!0})}function I1(){if(o.nativeOpen||o.appView!=="settings"||o.renderedAppView!=="settings"||o.renderedSettingsGroup!==o.settingsGroup)return null;let e=document.scrollingElement||document.documentElement,t=Number(window.scrollY||e?.scrollTop||0);return!Number.isFinite(t)||t<=0?null:{group:o.settingsGroup,left:Number(window.scrollX||e?.scrollLeft||0),top:t}}function F1(e){if(!e)return;let t=(o.settingsPageScrollRestoreToken||0)+1;o.settingsPageScrollRestoreToken=t;let r=()=>{if(t!==o.settingsPageScrollRestoreToken||o.nativeOpen||o.appView!=="settings"||o.settingsGroup!==e.group)return;let n=document.scrollingElement||document.documentElement;if(!n)return;let a=Math.max(0,n.scrollHeight-n.clientHeight),i=Math.min(e.top,a);window.scrollTo({left:e.left,top:i,behavior:"auto"})};window.requestAnimationFrame(()=>{r(),window.requestAnimationFrame(r),window.setTimeout(r,80)})}function zf(){if(!o.root)return;let e=O1();if(e){L1(e);return}let t=o.systemModal==="webserver-logs"?Jt():null,r=o.systemModal==="cm100-commissioning"?vp():null,n=String(o.systemModal||"").startsWith("service-task-")?qp():null,a=o.systemModal==="history-storage"?Ep():null,i=o.quickStartModalOpen?bg():null,l=I1();if(o.nativeOpen){o.root.innerHTML=`
        ${Yl()}
        ${Gm()}
      `,Ss(o.root),o.renderedAppView="native",o.renderedSettingsGroup="",o.settingsRenderSignature="",o.headerRenderSignature=gi(),Sr({mqttSensorsModalRenderSignature:""}),yn(),_n(),fl(),ds(),us(),Zt(t),pl(r),ml(n),gl(a),lc(i);return}let c=D1(),u=o.loadingEntities?`${c}${N1()}`:c,d=o.appView==="overview"||o.appView==="control"||o.appView==="energy"||o.appView==="diagnosis"||o.appView==="results";o.root.innerHTML=`
      ${Yl()}
