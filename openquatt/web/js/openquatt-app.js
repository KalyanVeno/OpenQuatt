...
`)),r.toString()}function ol(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=Pw(l);if(!c)return;let u=[...a,c],d=Xd(u,t);if(a.length&&d.length>kw){r.push({keys:n,body:i}),n=[l],a=[c],i=Xd(a,t);return}n=[...n,l],a=u,i=d}),a.length&&r.push({keys:n,body:i}),r}async function Nw(e){let t=el(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body},n=await gn(qw,r,t,`bulk entities request timed out after ${t}ms`);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function _w(){let e=el(),r=await gn(Ew,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`service status request timed out after ${e}ms`);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function Dw(){let e=el(),r=await gn(Tw,{cache:"no-store",headers:{"Cache-Control":"no-store"}},e,`decision log request timed out after ${e}ms`);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function Lw(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.hour_start_epoch_s??"",i.source??"",i.attention_count??"",e?.meta?.flash_write_count??""].join("|")}async function Ow(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<Cw)return!1;o.decisionLogFetchPromise=(async()=>{let n=await Dw(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=Lw(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function Iw(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{an.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=up(r,o.entities[r],n))})}async function Fw(e){if(!e.length)return{ok:!0,message:""};try{let t=await _w();return Iw(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function K(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(b=>a||an.has(b)||!Aw(b,n));if(!i.length)return;let l=i.filter(b=>an.has(b)),c=i.filter(b=>!an.has(b)),d=(await Fw(l)).ok?c:[...c,...l];if(!d.length){ep(),kn(Kt()),o.busyAction||(Zd(),o.controlError="");return}let g=Number(r.concurrency),y=Number.isFinite(g)&&g>0?Math.floor(g):Ft,w=ol(d,t),f=Math.max(1,Math.min(y,Ft)),m=[];for(let b=0;b<w.length;b+=f){let S=w.slice(b,b+f),q=await Promise.allSettled(S.map(async E=>({chunk:E,payload:await Nw(E)})));m.push(...q)}m.some(b=>b.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let p="";m.forEach(b=>{if(b.status!=="fulfilled"){let $=b.reason.message||String(b.reason);p||(p=$);return}let{chunk:S,payload:q}=b.value,E=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach($=>{if(Object.prototype.hasOwnProperty.call(E,$)){o.optionalMissingEntities&&delete o.optionalMissingEntities[$],o.entities[$]=up($,o.entities[$],E[$]);return}let P=O[$];P?.optional?k.has($)&&Rw($,n):p||(p=`${P?.name||$} ontbreekt in bulk response`)})}),ep(),kn(Kt()),p?(cp(p),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${p}`):o.busyAction||(Zd(),o.controlError="")}function up(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function ep(){o.complete=xa(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=Ku(),o.appView==="diagnosis"&&!ft()&&xt(uo(),{syncMode:"replace",forceSync:!0}),o.appView||xt(Sn()||uo(),{syncMode:"replace",forceSync:!0})}function Ww(){let e=Qs();return o.appView==="settings"?[...new Set([...e,...Ys()])]:o.appView==="energy"||o.appView==="results"?En(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?En(o.appView,{forceFast:!0}):[...new Set(e)]}function jw(e=[]){let t=new Set(e);return(o.appView==="settings"?Ys():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...En(o.appView,{includeBulk:!0}),...jt])]:o.appView==="energy"||o.appView==="results"?[...new Set([...En(o.appView,{forceFast:!0}),...jt])]:[...new Set(["setupComplete","strategy",...tt])]).filter(n=>!t.has(n))}function tp(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=xu}async function be(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||Na()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){hw(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<dn)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&tp(t,e),d=t-Number(o.lastStaticEntitySyncAt||0)>=$u||o.updateInstallBusy||o.updateInstallPhaseHint?jt:[],g=o.quickStartModalOpen&&o.currentStep==="flow-source"?gt:[],y=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?Ro:[],w=Os()?zt():[],f=n?[...rs,...tt,"setupComplete",...d]:i?[...En(a,{forceFast:l,includeBulk:c}),...d]:r==="settings"?[...new Set([...Ys(),...w,...d])]:c?["setupComplete",...d,...tt,"strategy",...Ao,...et,...ae()?ge.map(m=>m.key):kt]:["setupComplete",...tt,"strategy",...d];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let m=o.deviceReconnectMode,p=Mw(t,e)?await Hw():{ok:!0,message:""};if(!p.ok){cp(p.message),n||h();return}if(await K([...new Set([...f,...g,...y])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?Wt:Ft}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),d.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&rp();let b=m!==o.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="control"||a==="diagnosis"?await Is():!1,E=S?!1:o.appView==="results"?await Da():!1,k=a==="control"?await Ow({force:e.forceDecisionLog===!0}):!1,$=S?!1:Os()?(await Promise.all([Ad(),Da({metaOnly:!0})])).some(Boolean):!1,P=S||!nd()?!1:await Fo(),N=S||!ad()?!1:await Wo(),V=S||!zd()?!1:await Ko({force:o.systemModal==="mqtt-sensors"}),A=xd();if(S&&!o.nativeOpen&&sp(Js(a)),b){h();return}if(q&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){h();return}if(E&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){h();return}if(k&&o.appView==="control"){Fs()||h();return}if($&&o.appView==="settings"){h();return}if(P&&(o.systemModal==="login"||Vt())){h();return}if(N&&o.appView==="settings"){h();return}if(o.systemModal==="mqtt-sensors"){Oa()!==o.mqttSensorsModalRenderSignature&&h();return}if(V&&o.appView==="settings"){h();return}let F=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(A!==o.headerRenderSignature)if(F&&Ns())o.headerRenderSignature=A;else{h();return}else Ns();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){bn()!==o.settingsRenderSignature&&h();return}if(F)return;if(o.appView==="settings"){let H=bn();if(H!==o.settingsRenderSignature&&!o.focusedField){h();return}if(!Md()){o.focusedField||h();return}o.focusedField&&(o.settingsRenderSignature=H);return}if(o.appView==="diagnosis"){Dd()||h();return}if(o.appView==="control"){Fs()||h();return}if(o.appView==="energy"){Pd()||h();return}if(o.appView==="results"){Nd()||h();return}_d()||h()}catch(m){n||(o.controlError=`Helperstatus kon niet worden geladen. ${m.message}`,h())}finally{o.entitySyncInFlight=!1;let m=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,m&&!o.nativeOpen&&window.setTimeout(()=>{be(m)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!m&&tp(Date.now())&&window.setTimeout(()=>{be({forceBulk:!0})},gw)}}function Mt(){return{}}function Vw(){let e=String(T("hpGeneration")||"").trim();return e||""}function Tn(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function Bw(){if(!Array.isArray(wr))return"";if(wr.some(t=>v(t)))return"duo";let e=o.optionalMissingEntities||{};return wr.every(t=>e[t])?"single":""}function rl(e){let t=Tn(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function Kw(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function ne(){let e=Tn(T("installationTopology"));if(e==="single"||e==="duo")return rl(e);let t=Tn(Mt().installation);if(t==="single"||t==="duo")return rl(t);let r=Bw();return r?rl(r):Kw()}function Ht(){let e=ne(),t=Vw();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Ue(){return"OpenQuatt"}function nl(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function rt(e=go()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Uo(e=ne()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function Cn(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Mt().hardwareProfile||e).trim().toLowerCase()}function go(){return nl(T("connectionText")||Mt().connection)}function dp(){let e=go();return e==="wifi"?"eth":e==="eth"?"wifi":""}function pp(){let e=ne();return e==="single"?"duo":e==="duo"?"single":""}function Er(e=ne(),t=go()){let r=Uo(e),n=Cn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${rt(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${rt(t)}`:n==="waveshare"?`Waveshare ${r} ${rt(t)}`:`${Ue()} ${r} ${rt(t)}`}function al(){let e=C("timeValid"),t=String(T("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(v("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function Ia(){return v("timeValid")&&!C("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${al()}`}function zw(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Uw(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function Tr(){let e=Uw(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=zw(o.entities.uptime);if(n==="d")return Io(e*1440);if(n==="h")return Io(e*60);if(n==="s")return Io(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(Mt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Io((Date.now()-r)/6e4)}function Cr(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(Mt().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function Go({getScroller:e,getToken:t,setToken:r,isActive:n=()=>!0,getIdentity:a=null,preserveGrowth:i=!1,stickToBottom:l=!1,nearBottomThreshold:c=48}){function u(){let w=e();if(!w)return null;let f={scrollTop:w.scrollTop};if((i||l)&&(f.scrollHeight=w.scrollHeight),l){let m=w.scrollHeight-w.scrollTop-w.clientHeight;f.stickToBottom=m<c}return a&&(f.identity=a(w)),f}function d(w){if(!w)return;let f=e();if(!f||a&&a(f)!==w.identity)return;if(w.stickToBottom){f.scrollTop=f.scrollHeight;return}let m=i?f.scrollHeight-Number(w.scrollHeight||0):0;f.scrollTop=Math.max(0,Number(w.scrollTop||0)+m)}function g(w,f=!0){if(!w)return;let m=Number(t()||0)+1;r(m);let p=()=>{Number(t()||0)!==m||!n()||d(w)};if(f){window.requestAnimationFrame(p);return}p()}function y(){r(Number(t()||0)+1)}return{capture:u,invalidate:y,queue:g,restore:d}}function $e(e){return Object.prototype.hasOwnProperty.call(o.inputDrafts,e)?o.inputDrafts[e]:T(e)}function fo({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function il(e,t,r="",n=""){let a=t?r:n;return a?`<p data-oq-switch-copy="${s(e)}" data-on-copy="${s(r)}" data-off-copy="${s(n)}">${s(a)}</p>`:""}function mp(e,t,r,n="",a="",i=""){if(!v(e))return"";let l=!!T(e),c=o.loadingEntities||o.busyAction===`switch-${e}`;return we(e,t,r,`
  ...
  `}function Jo({active:e,startKey:t,stopKey:r,startLabel:n,stopLabel:a,startClass:i="oq-helper-button oq-helper-button--primary",stopClass:l="oq-helper-button oq-helper-button--ghost",startDisabled:c=!1,stopDisabled:u=!1}){return Ge(e?r:t,e?a:n,e?l:i,e?u:c)}function fp(e,t,r,n,a=""){if(!v(e))return"";let i=o.entities[e]||{},l=String(T(e)||""),c=Gt(i),u=o.loadingEntities||o.busyAction===`save-${e}`,d=`
    <div class="oq-settings-choice-grid">
      ${c.map(g=>{let y=n[g]||"",w=typeof y=="string"?y:y.copy||"",f=typeof y=="string"?"":y.image||"",m=typeof y=="string"?"":y.alt||"";return xr({key:e,option:g,currentValue:l,busy:u,copy:w,image:f,imageAlt:m})}).join("")}
    </div>
  `;return we(e,t,r,d,a)}function le(e,t,r,n="",a={}){if(!v(e))return"";let i=je(e),l=$e(e),c=a.unitOverride||i.uom||"",u=a.showUnit!==!1&&!!c,d=u&&a.unitMode!=="outside",g=fo({key:e,value:l,meta:i,controlClass:`oq-helper-control${u&&!d?" oq-helper-control--split":""}${d?" oq-helper-control--suffix":""}`,unitMarkup:u?d?`<span class="oq-helper-unit-chip">${s(c)}</span>`:`<span class="oq-helper-unit">${s(c)}</span>`:""});return we(e,t,r,g,n,a.footerMarkup||"")}function nt(e,t,r,n="",a={}){if(!v(e))return"";let i=je(e),l=fe(e,T(e)),c=a.minLabel||`${i.min}${i.uom||""}`,u=a.maxLabel||`${i.max}${i.uom||""}`,d=a.valueLabel||Y(e,l);return we(e,t,r,`<label class="oq-helper-slider-field"><div class="oq-helper-slider-meta"><span>${s(c)}</span><strong>${s(d)}</strong><span>${s(u)}</span></div><input class="oq-helper-range" type="range" data-oq-field="${s(e)}" min="${i.min}" max="${i.max}" step="${i.step}" value="${l}" ${o.loadingEntities?"disabled":""}></label>`,n)}function sl(e,t,r,n={}){if(!v(e))return"";let a=je(e),i=$e(e),l=n.compact===!0,c=n.embedded===!0,u=n.infoId||e,d=n.showCopy!==!1;return`
    <article class="oq-settings-mini-field${l?" oq-settings-mini-field--compact":""}${c?" oq-settings-mini-field--embedded":""}">
      ...
      ${fo({key:e,value:i,meta:a,controlClass:"oq-helper-control oq-helper-control--suffix",inputClass:"oq-helper-input oq-helper-input--compact-number",unitMarkup:a.uom?`<span class="oq-helper-unit-chip">${s(a.uom)}</span>`:""})}
    </article>
  `}function ll(e,t,r,n=""){if(!v(e))return"";let a=pn(T(e));return we(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function oe(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var hp=250;function bp(){return[]}function $r(){return typeof window>"u"?!1:bp().length>0}function Qw(){return`${Le()}/events`}function Yw(){return`${Le()}/openquatt/logs/recent`}function ja(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Va(){return o.nativeOpen?"Niet beschikbaar":$r()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function Jw(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function wp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return Jw(t)}function Zw(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function Xw(){return o.nativeOpen?"Niet beschikbaar":$r()?ja()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":ja()?"Buffer aan":"Buffer uit"}function ey(){return ja()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function cl(){return o.entities?.debugLevel||null}function yp(e=cl()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function ty(e=cl()){let t=String(e?.value??e?.state??"").trim(),r=yp(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function vp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function oy(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function Sp(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(oy);let n=[];for(let a of r){let i=n[n.length-1]||null;vp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-hp)}function ul(e,t={}){let r=Sy(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:qy(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function ry(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function ny(){let e=bp(),t=e.length;return e.map((r,n)=>ul(r,{receivedAt:ry(n,t),seq:n+1}))}function ay(){let e=hl();e&&(e.scrollTop=e.scrollHeight)}var dl=Go({getScroller:hl,getToken:()=>o.webServerLogScrollRestoreToken,setToken:e=>{o.webServerLogScrollRestoreToken=e},isActive:()=>o.systemModal==="webserver-logs",preserveGrowth:!0,stickToBottom:!0}),Yt=dl.capture,Jt=dl.queue;function iy(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}var qp=Go({getScroller:iy,getToken:()=>o.cm100CommissioningScrollRestoreToken,setToken:e=>{o.cm100CommissioningScrollRestoreToken=e},isActive:()=>o.systemModal==="cm100-commissioning",preserveGrowth:!0,stickToBottom:!0}),kp=qp.capture,pl=qp.queue;function sy(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}var Ep=Go({getScroller:sy,getToken:()=>o.serviceTaskModalScrollRestoreToken,setToken:e=>{o.serviceTaskModalScrollRestoreToken=e},isActive:()=>String(o.systemModal||"").startsWith("service-task-")}),Tp=Ep.capture,ml=Ep.queue;function ly(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}var Cp=Go({getScroller:ly,getToken:()=>o.historyStorageModalScrollRestoreToken,setToken:e=>{o.historyStorageModalScrollRestoreToken=e},isActive:()=>o.systemModal==="history-storage"}),xp=Cp.capture,gl=Cp.queue;async function Mr(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||Yt(),r=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=r,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let n=await window.fetch(Yw(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==r)return;let i=uy(a);o.webServerLogHistoryLoaded=!0,i.length>0&&(Sp(i,{prepend:!0}),o.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),o.webServerLogRecentAnchorAt=Date.now())}catch(n){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(h(),Jt(t))}}function cy(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?ul(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function uy(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>cy(r,n+1)).filter(r=>r!==null)}function dy(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function py(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>vp(e,r))}function my(){$r()&&o.webServerLogEntries.length===0&&ht({webServerLogEntries:ny()}),ht({webServerLogCopyMessage:"",webServerLogCopyError:""}),o.systemModal="webserver-logs",h(),K(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=Yt();h(),Jt(e)}),ay(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&Mr()}function Hr(){ht({webServerLogEntries:[],webServerLogError:"",webServerLogHistoryError:"",webServerLogHistoryLoading:!1,webServerLogHistoryLoaded:!1,webServerLogCopyMessage:"",webServerLogCopyError:"",webServerLogHistoryRequestToken:o.webServerLogHistoryRequestToken+1,webServerLogRecentTail:[],webServerLogRecentAnchorAt:0}),dl.invalidate(),o.systemModal==="webserver-logs"&&h()}function gy(){let e=Yt();At(),ht({webServerLogEnabled:null,webServerLogConnected:!1}),Hr(),o.systemModal==="webserver-logs"&&Mr({scrollState:e})}function fl(){if($r()){At();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs")){At();return}if(o.webServerLogEnabled===!1){At();return}o.webServerLogSource||fy()}function fy(){if($r()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",h();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",h();return}try{let e=new window.EventSource(Qw());o.webServerLogSource=e,e.addEventListener("open",hy),e.addEventListener("ping",by),e.addEventListener("log",yy),e.onerror=wy}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",At(),h()}}function At(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function hy(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Yt():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",h(),Jt(e)}Ld({clearOutput:Hr,closeStream:At,resetRecoveryState:gy});function by(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Yt():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",h(),Jt(e))}function wy(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?Yt():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",At(),h(),Jt(e)}function yy(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=Yt(),r=vy(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(u=>u.trim()!=="");if(n.length===0)return;let i=n.map(u=>ul(u)).filter(u=>!dy(u)&&!py(u));if(i.length===0)return;Sp(i);let l=Ey(),c=hl();ky(l),Ty(i,l),o.webServerLogEnabled=!0,c&&t&&Jt(t,!1)}function vy(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).trimEnd()}catch{}return t}function Sy(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function qy(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let u=l[c];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function ky(e){for(;o.webServerLogEntries.length>hp;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function Ey(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function hl(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function Ty(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",$p(r))}}function $p(e){let t=wp(e.receivedAt),r=Zw(e.receivedAt);return`
  ...
  `}function My(){let e=cl();if(!e)return"";let t=yp(e),r=ty(e),n=o.loadingEntities||o.busyAction==="save-debugLevel";return`
...
`)}async function Ay(){let e=Hy();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",h();return}try{if(!await so(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&h()}var Ry={"open-webserver-log-modal":()=>my(),"clear-webserver-log-output":()=>Hr(),"copy-webserver-log-output":()=>Ay()};function Mp(e){return ie(Ry,e)}function Hp(){let e=$r();return te({id:"system",titleId:"oq-webserver-log-modal-title",kicker:"Diagnostiek",title:"OpenQuatt log",copy:e?"Hier zie je voorbeeldmeldingen uit de lokale preview.":"Hier zie je recente meldingen van OpenQuatt. Handig als je wilt terugzoeken wat er net gebeurde.",className:"oq-helper-modal--wide oq-helper-modal--scrollable oq-webserver-log-modal",closeAction:"close-system-modal",closeLabel:"Sluit logboek",body:`
      ...
      <button class="oq-helper-button oq-helper-button--primary" type="button" data-oq-action="close-system-modal">Gereed</button>
    `})}function Py(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function Pp(e){return Py().includes(e)}function Ka(){let e=Cn(),t=ne(),r=go(),n=dp();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:v("firmwareUpdateTarget")&&Pp("alternate connection")&&v("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:rt(r),targetLabel:rt(n),currentBuildLabel:Er(t,r),targetBuildLabel:Er(t,n)}}function za(){let e=Cn(),t=ne(),r=pp(),n=go(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:v("firmwareUpdateTarget")&&Pp("alternate topology")&&v("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Uo(t),targetLabel:Uo(r),currentBuildLabel:Er(t,n),targetBuildLabel:Er(r,n)}}function Zo(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function Rr(){let e=Cn(),t=ne(),r=go(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${rt(r)}`}}function $n(e=Zo(),t=Rr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function yo(){if(at())return"Controleren";let e=Zt();if(e)return e.phaseLabel;if(yl())return"Bijgewerkt";if(Ar())return"Bezig";if(Xo())return"Beschikbaar";let t=_p();if(t!==null&&t<=0)return"Actueel";let r=Mt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Sl()?"Actueel":ye()?"Nog niet gecontroleerd":"\u2014"}function ye(){return o.entities.firmwareUpdate||null}function Ua(){let e=ye();return e?String(e.state??e.value??"").trim().toLowerCase():""}function Ny(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Ga(){return Ny().toLowerCase()}function Np(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function bl(){let e=String(o.updateInstallTargetVersion||"").trim(),t=ue();return!e||!t?!1:Mn(t,e)>=0}function _y(e=ye()||{}){let t=vo(e),r=ue(e);return!t||!r?!1:Mn(r,t)>=0}function wl(){return(bl()||_y())&&!at()&&!xn()&&!Xo()}function yl(){return(o.updateInstallCompleted||wl())&&!at()&&!Zt()&&!Xo()}function Pr(){xe({updateInstallBusy:!1,updateInstallTargetVersion:"",updateInstallPhaseHint:"",updateInstallProgressHint:Number.NaN,updateInstallMode:"",updateInstallTargetConnection:"",updateInstallTargetTopology:""}),jy()}function He(){xe({updateManualUploadFile:null,updateManualUploadFileName:"",updateManualUploadError:""})}function Qe(e={}){xe({...e.clearPr?{updateTestFirmwarePr:""}:{},updateTestFirmwareConfirmed:!1,updateTestFirmwareError:"",updateTestFirmwareBuild:null})}function Dy(){let e=Ga(),t=Np();if(e==="starting"||e==="uploading"||e==="rebooting"){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(bl()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function xn(){let e=Ga();return e==="starting"||e==="uploading"||e==="rebooting"}function Zt(){Dy();let e=Ga(),t=e==="starting"||e==="uploading"||e==="rebooting",r=t?e:o.updateInstallPhaseHint,n=Np(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!xn()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Ue()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${rt(o.updateInstallTargetConnection)}-build wordt nu naar ${Ue()} verzonden.`:o.updateInstallMode==="topology-switch"?`De ${Er(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Ue()} verzonden.`:`Firmware wordt nu naar ${Ue()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Ue()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${rt(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${Uo(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Ue()}.`}}function vo(e=ye()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function ue(e=ye()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function Ly(e=ye()||{}){let t=ue(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:Mn(t,r)>0}function vl(e=ye()||{},t=wo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=vo(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function Ap(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function Mn(e,t){let r=Ap(e),n=Ap(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function Ar(){if(wl())return!1;let e=Ua();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function at(){let e=Ua();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Xo(){let e=Ua();if(!vl())return!1;let t=_p();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Mt().updateAvailable===!0}function Sl(){let e=Ua();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Ly()}function ql(){let e=ye()||{},t=ue(e)||"\u2014",r=vl(e)?vo(e):"",n=r?Mn(r,t):null;return!at()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function _p(){let{current:e,latest:t}=ql();return e==="\u2014"||t==="\u2014"?null:Mn(t,e)}function Dp(e=wo()){let t=String(e||"").trim().toLowerCase();return zi[t]||zi.main}function Oy(){let e=String((ye()||{}).release_url||"").trim(),t=Dp();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function Iy(){return Ue()}function wo(){return String(T("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function Fy(){return ql().latest!=="\u2014"}function Rp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Wy(e,t=ye()||{}){let r=Rp(e);return r?Rp(`${t.title||""} ${t.summary||""}`).includes(r):!0}function Lp(e){return new Promise(t=>window.setTimeout(t,e))}function Hn(e=Do){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,gd(),typeof At=="function"&&At(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&qs()},e)}function jy(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||qs()}function Op(){return o.deviceReconnectMode?te({modalId:"reconnect",titleId:"oq-reconnect-modal-title",kicker:"Systeem",title:Xu(),modalClass:"oq-helper-modal--reconnect",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `}):""}function An(e=wo()){let t=ye()||{},r=ue(t);o.entities.firmwareUpdate={...t,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:Dp(e),current_version:r}}async function Rn(e={}){let t=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await Lp(r===0?900:1200),await K(ln,"all",{forceMissing:!0});let n=vl(),a=!t||Wy(t),i=Fy(),l=at(),c=yo();if(n&&a&&(i||!l&&c!=="Nog niet gecontroleerd"))return!0}return!1}async function Nr(e={}){let t=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await Lp(a===0?r:n);try{if(await K(ln,"all",{forceMissing:!0}),Ga()==="rebooting"&&co("ota"),h(),o.updateInstallMode==="connection-switch"){let i=nl(o.updateInstallTargetConnection);if(i&&go()===i&&!xn()&&!Ar())return!0}else if(o.updateInstallMode==="topology-switch"){let i=Tn(o.updateInstallTargetTopology);if(i&&ne()===i&&!xn()&&!Ar())return!0}else if(bl()||wl()||Sl()&&!xn()&&!Ar())return!0}catch{t||(o.controlNotice="Wachten tot het device opnieuw is opgestart...",h(),t=!0)}}return!1}function Vy(){let e=wo(),t=Zt();if(t)return t.copy;if(yl()){let r=o.updateInstallCompletedVersion||ue()||wo();return`${Ue()} draait nu op ${r}.`}return Ar()?`OTA-update wordt voorbereid voor ${Ue()}. Het device kan kort herstarten.`:at()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:Xo()?"Er staat een nieuwere firmware klaar.":Sl()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function Ip(){return!!(o.firmwareAdvancedOpen||o.firmwareConnectionSwitchOpen||o.firmwareTopologySwitchOpen||o.updateManualUploadOpen||o.updateTestFirmwareOpen)}function Ba(e,t,r,n,a=!1){return`
      <button
    ...
    `}function By(e,t,r,n){if(!Ip())return"";let i=!!(Zt()||o.updateInstallBusy||at());return`
    ...
    `}function Ky(){let e=Ka();if(!e||!o.firmwareConnectionSwitchOpen)return"";let r=!!(Zt()||o.updateInstallBusy||at()),n=!!o.firmwareConnectionSwitchConfirmed,a=e.targetConnection==="eth",i=!e.canSwitch,l=a?"Sluit eerst de netwerkkabel aan. Na de herstart verdwijnt Wi-Fi uit deze firmware.":"Na de herstart verdwijnt Ethernet uit deze firmware. Als er geen Wi-Fi-gegevens bekend zijn, start het OpenQuatt fallback access point.",c=i?'<p class="oq-helper-modal-note oq-helper-modal-note--muted">Verbindingswissel wordt geladen. Open deze modal opnieuw of wacht een moment als de knop disabled blijft.</p>':"";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
          <strong>Verbinding wisselen</strong>
          <span>Installeer dezelfde ${s(wo())}-build voor de andere netwerkverbinding.</span>
        </div>
    ...
    `}function Uy(){if(!o.updateTestFirmwareOpen)return"";let t=!!(Zt()||o.updateInstallBusy||at()),r=Zo(),n=Rr(),a=$n(r,n),i=!!(n.available&&v("firmwareTestOtaUrl")&&v("firmwareTestOtaMd5Url")&&v("installFirmwareTestOta")),l=!!(r&&i),c=o.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,d=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Gy(){if(!o.updateManualUploadOpen)return"";let t=!!(Zt()||o.updateInstallBusy||at()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
  ...
  `}function Zy(){if(!o.root)return;if(o.interfacePanelOpen){h();return}let e=o.root.querySelector(".oq-helper-hub--collapsed .oq-helper-hub-head-actions");if(!e)return;let t=e.querySelector(".oq-debug-recording-header-status"),r=kl();if(!r){t?.remove();return}if(t){t.outerHTML=r;return}e.insertAdjacentHTML("afterbegin",r)}function Xy(){if(!o.root)return;let e=o.root.querySelector('[data-oq-diagnostics-row="debugRecording"]');if(!e)return;let t=e.querySelector(".oq-settings-system-row-value"),r=e.querySelector(".oq-settings-system-row-note");t&&(t.textContent=Pn()),r&&(r.textContent=Ja())}function El(e){let t={activity:'<svg viewBox="0 0 24 24" focusable="false"><path d="M3 12h4l2-7 4 14 2-7h6"/></svg>',status:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="4"/></svg>',clock:'<svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3 2"/></svg>',samples:'<svg viewBox="0 0 24 24" focusable="false"><path d="M4 16h3l2-7 4 9 2-5h5"/></svg>',changes:'<svg viewBox="0 0 24 24" focusable="false"><path d="M18 8a7 7 0 1 0 1 7"/><path d="M18 4v4h-4"/></svg>',file:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 3h7l4 4v14H7z"/><path d="M14 3v5h5"/></svg>',storage:'<svg viewBox="0 0 24 24" focusable="false"><ellipse cx="12" cy="6" rx="7" ry="3"/><path d="M5 6v6c0 1.7 3.1 3 7 3s7-1.3 7-3V6"/><path d="M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6"/></svg>',play:'<svg viewBox="0 0 24 24" focusable="false"><path d="M8 5v14l11-7z"/></svg>',stop:'<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7h10v10H7z"/></svg>',download:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 4v10"/><path d="m8 10 4 4 4-4"/><path d="M5 19h14"/></svg>',copy:'<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="10" height="10" rx="2"/><path d="M6 14H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1"/></svg>',check:'<svg viewBox="0 0 24 24" focusable="false"><path d="m5 13 4 4L19 7"/></svg>',alert:'<svg viewBox="0 0 24 24" focusable="false"><path d="M12 8v5"/><path d="M12 17h.01"/><path d="M10.3 4.7 2.8 18a2 2 0 0 0 1.7 3h15a2 2 0 0 0 1.7-3L13.7 4.7a2 2 0 0 0-3.4 0z"/></svg>'};return t[e]||t.status}function ev(e){return`<span class="oq-debug-recording-icon" aria-hidden="true">${El(e)}</span>`}function Xt(e){return`<span class="oq-debug-recording-button-icon" aria-hidden="true">${El(e)}</span>`}function tv(){return[..._o]}function ov(){let e=[];return _o.forEach((t,r)=>{let n=o.entities?.[t]||{},a=String(n.uom??n.unit_of_measurement??"").trim();a&&e.push([r,a])}),e}function rv(e){return Array.isArray(e)?e.map((t,r)=>t===null?null:[r,t]).filter(Boolean):[]}function Za(){o.debugRecordingTimer&&(window.clearTimeout(o.debugRecordingTimer),o.debugRecordingTimer=null)}function tr(){o.debugRecordingDevicePollTimer&&(window.clearTimeout(o.debugRecordingDevicePollTimer),o.debugRecordingDevicePollTimer=null)}function So(e){return`${Le()}/openquatt/debug-recording/${e}`}function _r(e){let t=e&&typeof e=="object"?e:{};o.debugRecordingDeviceStatus=t,o.debugRecordingActive=!!t.active,o.debugRecordingStartedAt=t.active||Number(t.sample_count||0)>0?Date.now()-Math.max(0,Number(t.elapsed_s||0)*1e3):0,o.debugRecordingEndsAt=t.active?Date.now()+Math.max(0,Number(t.remaining_s||0)*1e3):0,o.debugRecordingLastSampleAt=Number(t.sample_count||0)>0?Date.now():0}function Tl(){_r({ok:!1,available:!1,active:!1,mode:"manual",rolling:!1,frozen:!1,storage:"unavailable",interval_s:0,duration_s:0,elapsed_s:0,remaining_s:0,sample_count:0,sample_capacity:0,estimated_size:0,buffer:"unavailable"})}async function nv(){let e=await window.fetch(So("status"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return _r(t),t}function Nn(e=2e3){tr(),o.debugRecordingActive&&(o.debugRecordingDevicePollTimer=window.setTimeout(()=>{Xa({silent:!0})},Math.max(0,Number(o.systemModal==="debug-recording"?e:5e3)||0)))}async function Xa(e={}){e.silent||(o.debugRecordingBusy=!0,o.debugRecordingError="",h());try{await nv(),!o.debugRecordingActive&&e.silent&&(o.debugRecordingNotice="Debugopname is afgerond."),Nn()}catch(t){Tl(),o.debugRecordingError=`Status kon niet worden opgehaald. ${t.message||String(t)}`}finally{e.silent||(o.debugRecordingBusy=!1),!e.silent||o.systemModal==="debug-recording"?h():(Zy(),Xy())}}async function Gp(){let e=ol(_o,"state"),t=null;for(let r=0;r<e.length;r+=1){let n=await window.fetch(So(`configure?reset=${r===0?"1":"0"}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store","Content-Type":"application/x-www-form-urlencoded"},body:e[r].body});if(!n.ok)throw new Error(`configuratie HTTP ${n.status}`);t=await n.json()}if(Number(t?.entity_field_count||0)!==_o.length)throw new Error(`onvolledige debugset (${Number(t?.entity_field_count||0)}/${_o.length})`);return t}async function av(e){let t=Math.max(1,Number(e)||15);Za(),tr(),Ma({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await Gp();let r=await window.fetch(So(`start?duration_s=${encodeURIComponent(t*60)}`),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!r.ok)throw new Error(`HTTP ${r.status}`);let n=await r.json();_r(n),Nn()}catch(r){Tl(),o.debugRecordingError=`Debugopname kon niet worden gestart. ${r.message||String(r)}`}finally{o.debugRecordingBusy=!1,h()}}async function iv(){Za(),tr(),Ma({debugRecordingBusy:!0,debugRecordingError:"",debugRecordingNotice:"",debugRecordingSamples:[],debugRecordingEvents:[],debugRecordingInitialValues:null,debugRecordingLastValues:null,debugRecordingDeviceBundle:null,debugRecordingLastSampleAt:0,debugRecordingSequence:0}),h();try{await Gp();let e=await window.fetch(So("start?rolling=1"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();_r(t),Nn()}catch(e){Tl(),o.debugRecordingError=`Rolling debug kon niet worden gestart. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function sv(){let e=await window.fetch(So("freeze"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!e.ok)throw new Error(`HTTP ${e.status}`);let t=await e.json();return _r(t),tr(),t}async function lv(){Za(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{await sv(),o.debugRecordingNotice="Rolling debug is gestopt. De recente buffer blijft bewaard."}catch(e){o.debugRecordingError=`Rolling debug kon niet worden gestopt. ${e.message||String(e)}`}finally{o.debugRecordingBusy=!1,h()}}async function cv(e={}){Za(),tr(),o.debugRecordingBusy=!0,o.debugRecordingError="",h();try{let t=await window.fetch(So("stop"),{method:"POST",cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();_r(r),o.debugRecordingNotice=e.completed?"Debugopname is afgerond.":"Debugopname is gestopt."}catch(t){o.debugRecordingError=`Debugopname kon niet worden gestopt. ${t.message||String(t)}`}finally{o.debugRecordingBusy=!1,h()}}function Wp(e={}){let t=o.debugRecordingActive?Date.now():Number(o.debugRecordingLastSampleAt||Date.now());return{format:"openquatt-debug-v2",schema_version:2,kind:"openquatt_debug_recording",encoding:"column-delta-json-v2",exported_at:new Date().toISOString(),source:e.source||{},recording:{started_at:o.debugRecordingStartedAt?new Date(Number(o.debugRecordingStartedAt)).toISOString():"",ended_at:t?new Date(t).toISOString():"",active:!!o.debugRecordingActive,duration_s:o.debugRecordingStartedAt?Math.round(Math.max(0,t-Number(o.debugRecordingStartedAt))/1e3):0,interval_s:Math.round(ku/1e3),sample_count:Rt(),column_count:_o.length,storage:"browser"},columns:tv(),units:ov(),initial:rv(o.debugRecordingInitialValues),samples:o.debugRecordingSamples||[],events:o.debugRecordingEvents||[],...e.logs?{logs:e.logs}:{}}}function Qa(e){return JSON.stringify(e)}function uv(){let e=Number(o.debugRecordingDeviceStatus?.estimated_size||0);if(e>0)return e;try{return new Blob([Qa(Wp())]).size}catch{return Qa(Wp()).length}}function dv(e){let t=Math.max(0,Number(e)||0);return t>=1024*1024?`${(t/1024/1024).toFixed(1)} MB`:t>=1024?`${(t/1024).toFixed(1)} kB`:`${Math.round(t)} B`}function pv(e){let t=e?.exported_at||(e?.exported_at_ms?new Date(Number(e.exported_at_ms)).toISOString():new Date().toISOString()),r=String(t).replace(/[:.]/g,"-").replace(/T/,"_").replace(/Z$/,"Z");return`${String(e?.source?.installation||"OpenQuatt").replace(/\s+/g,"-").toLowerCase()}-debug-recording-${r}.oqdebug.json`}async function mv(){if(Rt()===0){o.debugRecordingError="Er is nog geen debugopname om te downloaden.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Pt();h();try{let t=await window.fetch(So("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();o.debugRecordingDeviceBundle=r,fs(pv(r),Qa(r),"application/json"),Up(r),o.debugRecordingNotice=e?"Momentopname gedownload. Rolling debug loopt door.":"Supportbestand gedownload."}catch{o.debugRecordingError="Download mislukt. Probeer opnieuw of kopieer de data."}finally{o.debugRecordingBusy=!1,h()}}async function gv(){if(Rt()===0){o.debugRecordingError="Er is nog geen debugopname om te kopi\xEBren.",h();return}o.debugRecordingBusy=!0,o.debugRecordingError="";let e=o.debugRecordingActive&&Pt();h();try{let t=await window.fetch(So("download"),{cache:"no-store",headers:{"Cache-Control":"no-store"}});if(!t.ok)throw new Error(`HTTP ${t.status}`);let r=await t.json();if(o.debugRecordingDeviceBundle=r,!await so(Qa(r)))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");Up(r),o.debugRecordingNotice=e?"Momentopname gekopieerd. Rolling debug loopt door.":"Supportbestand gekopieerd."}catch{o.debugRecordingError="Kopi\xEBren mislukt. Probeer opnieuw of download het supportbestand."}finally{o.debugRecordingBusy=!1,h()}}var fv={"open-debug-recording-modal":()=>(o.systemModal="debug-recording",o.debugRecordingError="",o.debugRecordingNotice="",h(),Xa()),"start-debug-recording":e=>av(e.dataset.debugMinutes||15),"start-rolling-debug-recording":()=>iv(),"select-debug-recording-duration":e=>Yy(e.dataset.debugMinutes||15),"stop-debug-recording":()=>cv(),"freeze-debug-recording":()=>lv(),"download-debug-recording":()=>mv(),"copy-debug-recording":()=>gv()};function Qp(e,t){return ie(fv,e,t)}function Yp(){let e=o.debugRecordingActive,t=Pt(),r=Ya(),n=Rt(),a=o.debugRecordingBusy,i=dv(uv()),l=Array.isArray(o.debugRecordingDeviceBundle?.events)?o.debugRecordingDeviceBundle.events.length:Array.isArray(o.debugRecordingEvents)?o.debugRecordingEvents.length:0,c=Qy(),u=Kp(),d=jp(),g=Jy(),y=n>0,w=`${g.toFixed(1)}%`,f=[{icon:"status",label:"Status",value:Pn()},{icon:"clock",label:t?"Retentie":"Duur",value:er(t?d:Vp())},{icon:"samples",label:"Samples",value:String(n)},{icon:"changes",label:"Statuswijzigingen",value:String(l)},{icon:"file",label:"Geschatte grootte",value:`\xB1 ${i}`},{icon:"storage",label:"Opslag",value:o.debugRecordingDeviceStatus?.available===!1?"Niet beschikbaar":"Apparaatgeheugen"}],m=o.debugRecordingError?{kind:"error",icon:"alert",text:o.debugRecordingError}:o.debugRecordingNotice?{kind:"success",icon:"check",text:o.debugRecordingNotice}:null;return te({id:"system",titleId:"oq-debug-recording-modal-title",kicker:"Diagnostiek",title:"Debugopname",copy:Ja(),className:"oq-debug-recording-modal",closeAction:"close-system-modal",closeLabel:"Sluit debugopname",body:`
        ...
        <article class="oq-settings-hp-offset-row" data-oq-settings-field="${s(r.offsetKey)}" data-oq-hp-offset-raw-key="${s(r.rawKey)}" data-oq-hp-offset-final-key="${s(r.finalKey)}">
          ...
          <div class="oq-settings-hp-offset-equation" aria-label="${s(`${r.label} correctie`)}">
            ...
            <label class="oq-settings-hp-offset-input">
              <span>Correctie</span>
              ${fo({key:r.offsetKey,value:$e(r.offsetKey),meta:n,controlClass:"oq-helper-control oq-helper-control--suffix",inputClass:"oq-helper-input oq-helper-input--compact-number",unitMarkup:n.uom?`<span class="oq-helper-unit-chip">${s(n.uom)}</span>`:""})}
            </label>
            <span class="oq-settings-hp-offset-operator">=</span>
    ...
    `}function lm(){let e=v("boilerCvAssistEnabled")&&C("boilerCvAssistEnabled"),t=Fa(),r=C("cm100Active"),n=String(t||"").trim().toUpperCase(),a=or(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,u=!!o.pendingCommissioningCm100Start,d=c||i||a,g=c||!i,y=Yo("boilerPowerTestStatus","IDLE"),w=ri(y,"boiler"),f=C("boilerPowerTestActive"),m=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",p=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),b=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=or(y),k=!Ee(y)&&(f||b||S||Or(y))&&!q,$=Me("boilerRatedHeatPower"),P=Me("boilerHeatPower"),N=R("boilerHeatPower"),V=N>0?P:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:P;N>0&&(o.commissioningBoilerHeatPowerDisplay=P);let A=Yo("flowAutotuneStatus","IDLE"),F=ri(A,"autotune"),H=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",L=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),D=!!o.pendingFlowAutotuneStart,_=o.commissioningTaskLock==="autotune",Z=or(A),j=!Ee(A)&&(D||_||Or(A))&&!Z,ee=Yo("airPurgeStatus","IDLE"),Ne=ri(ee,"purge"),vt=C("airPurgeActive"),ut=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",It=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),Je=!!o.pendingAirPurgeStart,_e=o.commissioningTaskLock==="purge",x=!Ee(ee)&&(vt||Je||_e||Or(ee)),B=/DONE/.test(String(ee||"").toUpperCase()),Q=!!(It||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),G=Me("airPurgeRemaining",{decimals:0}),se=R("airPurgePhase"),ve=se===1?"Rustig":se===2?"Pulsen":se===3?"Stabiliseren":Ne.phase,ce=Yo("manualFlowStatus","IDLE"),Te=C("manualFlowActive"),pe=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",dt=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),St=!!o.pendingManualFlowStart,de=o.commissioningTaskLock==="manual-flow",We=!Ee(ce)&&(Te||St||de||Or(ce)),Ze=Yo("manualHpStatus","IDLE"),pt=C("manualHpActive"),mt=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",Mo=!!(o.entities.manualHpStart||o.entities.manualHpAbort),Se=!!o.pendingManualHpStart,qt=o.commissioningTaskLock==="manual-hp",Xe=!Ee(Ze)&&(pt||Se||qt||Or(Ze)),lu=/SAFETY STOP/.test(String(Ze||"").toUpperCase()),cu=/STOPPING/.test(String(Ze||"").toUpperCase()),ro=Yo("hpWaterCalibrationStatus","IDLE"),Dh=ri(ro,"hp-water-calibration"),uu=C("hpWaterCalibrationActive"),ua=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",du=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),da=!!o.pendingHpWaterCalibrationStart,no=o.commissioningTaskLock==="hp-water-calibration",Ke=!Ee(ro)&&(uu||da||no||Or(ro)),pa=/DONE|APPLIED/.test(String(ro||"").toUpperCase()),pu=/APPLIED/.test(String(ro||"").toUpperCase()),Lh=Me("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),Oh=Me("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),mu=/DONE|APPLIED/.test(String(y||"").toUpperCase()),gu=/DONE|APPLIED/.test(String(A||"").toUpperCase()),Vi=i?q?"Wachten op CM100":k?w.phase:mu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Bi=i?Z?"Wachten op CM100":j?F.phase:gu?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Ki=i?x?Ne.phase:B?"Klaar":"Klaar om te starten":"Wachten op CM100",fu=i?We?"Actief":"Klaar om te starten":"Wachten op CM100",hu=i?Xe?cu?"Bezig met stoppen":lu?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",bu=i?Ke?Dh.phase:pu?"Offsets toegepast":pa?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",wu=!i||m||!p||j||x||We||Xe||Ke||k||_||_e||de||qt||no||b,Ih=m||!(k||S||b),Fh=m||wu||!mu||j||x||Ke,yu=!i||H||!L||k||x||We||Xe||Ke||j||S||_e||de||qt||no||D,Wh=H||!(j||_||D),jh=H||yu||!gu||k||x||Ke,Vh=!i||ut||!It||k||j||We||Xe||Ke||x||S||_||de||qt||no||Je,Bh=ut||!(x||_e||Je),Kh=!i||pe||!dt||k||j||x||Xe||Ke||We||S||_||_e||qt||no||St,zh=pe||!(We||de||St),Uh=!i||mt||!Mo||k||j||x||We||Ke||Xe||S||_||_e||de||no||Se,Gh=mt||!(Xe||qt||Se),Qh=!i||ua||!du||k||j||x||We||Xe||Ke||S||_||_e||de||qt||da,Yh=ua||!(Ke||no||da),Jh=ua||Ke||!pa||pu;u&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),b&&(f||Ee(y))&&(o.pendingBoilerPowerTestStart=!1),S&&Ee(y)&&(o.commissioningTaskLock=""),D&&Ee(A)&&(o.pendingFlowAutotuneStart=!1),_&&Ee(A)&&(o.commissioningTaskLock=""),Je&&(vt||Ee(ee))&&(o.pendingAirPurgeStart=!1),_e&&Ee(ee)&&(o.commissioningTaskLock=""),St&&(Te||Ee(ce))&&(o.pendingManualFlowStart=!1),de&&(Te||Ee(ce))&&(o.commissioningTaskLock=""),Se&&(pt||Ee(Ze))&&(o.pendingManualHpStart=!1),qt&&(pt||Ee(Ze))&&(o.commissioningTaskLock=""),da&&(uu||Ee(ro))&&(o.pendingHpWaterCalibrationStart=!1),no&&Ee(ro)&&(o.commissioningTaskLock="");let Zh=a?"Wachten op CM100":t,Xh=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",eb=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:bu,available:!!(du||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Ir({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:bu,statusCopy:Ke?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":pa?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:Nv({status:ro,running:Ke,resultReady:pa,startDisabled:Qh,abortDisabled:Yh,applyDisabled:Jh,busy:ua,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:fu,available:!!(dt||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Ir({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:fu,statusCopy:We?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?Jo({active:We,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:pe||Kh,stopDisabled:pe||zh}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:hu,available:!!(Mo||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Ir({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:hu,statusCopy:Xe?cu?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":lu?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
            ${o.entities.manualHpStart||o.entities.manualHpAbort?Jo({active:Xe,startKey:"manualHpStart",stopKey:"manualHpAbort",startLabel:"Bediening starten",stopLabel:"Bediening stoppen",startDisabled:mt||Uh,stopDisabled:mt||Gh}):""}
          `,controls:`
            <div class="oq-settings-manual-hp-controls">
              <div class="oq-settings-manual-hp-unit">
                ${bo("manualHp1Mode","Warmtepomp 1 werkmodus","Start in Standby. Verwarmen of koelen kan pas worden gekozen zodra voldoende flow is gemeten.","oq-settings-field--compact")}
              ...
              ${v("hp2ExcludedA")?`
                <div class="oq-settings-manual-hp-unit">
                  ${bo("manualHp2Mode","Warmtepomp 2 werkmodus","Start in Standby. Verwarmen of koelen kan pas worden gekozen zodra voldoende flow is gemeten.","oq-settings-field--compact")}
          ...
          `,metrics:`
            ...
            <div class="oq-settings-manual-hp-statuses">
              ${Pe("hp1Failures","Warmtepomp 1 statusmelding","Actuele melding die de warmtepomp zelf rapporteert.",po(W("hp1Failures","None")),"oq-settings-field--compact")}
              ${v("hp2Failures")?Pe("hp2Failures","Warmtepomp 2 statusmelding","Actuele melding die de warmtepomp zelf rapporteert.",po(W("hp2Failures","None")),"oq-settings-field--compact"):""}
            </div>
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:Bi,available:!0,openDisabled:or(Bi),cardMarkup:Ir({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:Bi,statusCopy:Z?"Wacht totdat CM100 actief is voordat je autotune start.":j?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ...
            ${o.entities.flowAutotuneApply?Ge("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",H||jh):""}
          `,metrics:`
          ...
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:Vi,available:e,openDisabled:or(Vi),cardMarkup:Ir({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s($)}`,status:Vi,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":k?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ...
            ${o.entities.boilerPowerTestApply?Ge("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",m||Fh):""}
          `,metrics:`
          ...
          `})}].filter(tb=>tb.available);return{cm100Status:Zh,cm100StartDisabled:d,cm100StopDisabled:g,serviceStatusCopy:Xh,tasks:eb}}function _v(e){return Oe({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
Alle bewaarde dagtotalen worden verwijderd. Dit heeft geen invloed op de werking van je warmtepomp.`))return e("lifetimeEnergyHistoryClear",{successNotice:"Energiehistorie is gewist.",errorPrefix:"Energiehistorie kon niet worden gewist",refreshKeys:zt(),refreshDelayMs:500}).then(()=>{o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryLastFetchAt=0,Eo(),o.appView==="results"&&ar({force:!0}).then(()=>h())})},"select-energy-history-import-file":()=>uS(),"clear-energy-history-import-file":()=>{Cm(),h()},"import-energy-history-file":()=>dS(),"export-energy-history":()=>Yv(),"open-history-storage-modal":()=>{o.settingsStoragePage="overview",o.systemModal="history-storage",h();let e=ui({forceMissing:!0,forceTrendHistory:!0,forceDecisionLog:!0,forceEnergyHistory:!0}).finally(()=>{o.systemModal==="history-storage"&&h()});return Eo([1e3,3e3,7e3]),e},"open-storage-diagnosis":()=>{o.settingsStoragePage="diagnosis",h()},"open-storage-decision-log":()=>{o.settingsStoragePage="decision-log",h()},"open-storage-energy":()=>{o.settingsStoragePage="energy",h()},"back-storage-overview":()=>{o.settingsStoragePage="overview",h()},"download-settings-backup":()=>yS(),"open-settings-backup-import":()=>{o.systemModal="settings-backup-import",h()},"confirm-settings-backup-restore":()=>vS()};function Dm(e,t){return ie(SS,e,t)}function Vl(e,t,r="",n=!1){return`
    ...
    `}function Bl(e){let t=e.filter(Boolean);return t.length?`
    ...
    `:""}function Vm(){if(!v("trendHistoryEnabled")&&!v("decisionLogHistoryEnabled")&&!v("lifetimeEnergyHistoryEnabled"))return"";let e=C("trendHistoryEnabled"),t=e&&C("trendHistoryFlashEnabled"),r=v("lifetimeEnergyHistoryEnabled"),n=r&&C("lifetimeEnergyHistoryEnabled"),a=v("decisionLogHistoryEnabled"),i=a&&C("decisionLogHistoryEnabled"),l=Wm(),c=t?Fe("trendHistoryFlashAvailable","Alleen live"):"Alleen live",u=r?Om(Fe("lifetimeEnergyHistoryAvailable","Geen data")):"Geen data";return oe("Diagnose","Gegevens bewaren","Bepaal welke gegevens OpenQuatt bewaart voor grafieken, resultaten en hulp bij problemen.",`
        <article class="oq-settings-storage-summary">
          ...
          <div class="oq-settings-storage-summary-metrics" aria-label="Opslagstatus">
            ${v("trendHistoryEnabled")?Vl("Diagnose",t?c:e?"Alleen live":"Uit",t?"Blijft bewaard na herstart":"Tijdelijk",e):""}
            ${a?Vl("Beslislog",i?Fm(l.storedHours):"Alleen sinds herstart",i?"Maximaal 7 dagen":"Tijdelijk",i):""}
            ${r?Vl("Energie",u,n?"Blijft bewaard na herstart":"Uit",n):""}
          </div>
    ...
    `}function Bm(){let e=["diagnosis","decision-log","energy"].includes(o.settingsStoragePage)?o.settingsStoragePage:"overview",t=v("trendHistoryEnabled")&&C("trendHistoryEnabled"),r=t&&v("trendHistoryFlashEnabled")&&C("trendHistoryFlashEnabled"),n=v("decisionLogHistoryEnabled"),a=n&&C("decisionLogHistoryEnabled"),i=Wm(),l=Fm(i.storedHours),c=v("lifetimeEnergyHistoryEnabled"),u=c&&C("lifetimeEnergyHistoryEnabled"),d=c?Fe("lifetimeEnergyHistoryAvailable","Geen data"):"Geen data",g=Om(d),y=v("lifetimeEnergyHistoryClear")&&!["Geen data","\u2014"].includes(d),w=t&&v("trendHistoryFlush"),f=a&&v("decisionLogHistoryFlush"),m=v("lifetimeEnergyHistoryCapture"),p=jm(),b=String(o.energyHistoryRaw||"").includes("@hour_retention|"),S=b&&!p.hourPartitionAvailable,q=b?S?"Alleen live":Im(p.hourStoredDayCount,"Geen uurdata"):"Laden...",E=b&&!S?gi(p.hourStorageKb):"\u2014",k=b&&!S?fi(p.hourWriteCount):"\u2014",$=b&&!S?mi(p.hourLastWriteTimestampS):"Geen data",P=e==="overview"?"":`
      ...
      <div class="oq-settings-storage-hub">
        ...
        ${n?N("open-storage-decision-log","Beslislog","Keuzes van de controller","Uursamenvattingen van starts, stops, bronwissels en bescherming.",a?`${l} \xB7 max. 7 dagen`:"Alleen sinds herstart",a):""}
      ...
      <p class="oq-settings-storage-footnote"><strong>Goed om te weten:</strong> gegevens die worden bewaard, blijven beschikbaar na een herstart. Tijdelijke gegevens bestaan alleen zolang de controller online is.</p>`;return e==="diagnosis"?(H="Diagnosegegevens",L="Beheer technische meetreeksen voor diagnosegrafieken en hulp bij problemen.",D=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
      ...
      </section>${Bl([V])}`):e==="decision-log"?(H="Beslisloghistorie",L="Bewaar compacte uursamenvattingen van controllerkeuzes, maximaal zeven dagen.",D=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
          ${pi("decisionLogHistoryEnabled","Beslisloghistorie bewaren","Bewaar ieder uur een compacte samenvatting van belangrijke beslissingen.","De laatste zeven dagen blijven beschikbaar na een herstart of update.","De actuele beslislog blijft tijdelijk beschikbaar; bestaande historie blijft staan.","Blijft bewaard na herstart")}
          ${f?`<div class="oq-settings-storage-inline-action"><div><h4>Beslislog nu opslaan</h4><p>Sla het lopende uur alvast op v\xF3\xF3r een update of herstart.</p></div>${Kn("decisionLogHistoryFlush","Nu opslaan","flush-decision-log-history",{disabled:!a,busyLabel:"Opslaan..."})}</div>`:""}
        </div>
      </section>${Bl([A])}
      ${v("decisionLogHistoryClear")?`<details class="oq-settings-storage-advanced"><summary>Geavanceerd</summary><div class="oq-settings-storage-inline-action oq-settings-storage-inline-action--danger"><div><h4>Beslisloghistorie wissen</h4><p>Verwijder alle bewaarde uursamenvattingen. De actuele beslislog blijft staan.</p></div>${Kn("decisionLogHistoryClear","Historie wissen","clear-decision-log-history",{disabled:Number(i.storedHours||0)<=0,buttonClass:"oq-helper-button oq-helper-button--warning",busyLabel:"Wissen..."})}</div></details>`:""}`):e==="energy"&&(H="Energiehistorie",L="Beheer dagtotalen en uurdetail voor de Resultatenpagina.",D=`${P}<section class="oq-settings-storage-domain oq-settings-storage-domain--single">
        <div class="oq-settings-storage-domain-rows">
      ...
      `})}function hi(){return[o.interfacePanelOpen?"open":"closed",o.nativeOpen?"native":"app",o.appView,o.complete?"complete":"incomplete",o.overviewTheme,o.hpVisualMode,U("installationTopology"),U("hardwareProfileText"),U("connectionText"),o.firmwareAdvancedOpen?"firmware-advanced-open":"firmware-advanced-closed",o.firmwareConnectionSwitchOpen?"connection-open":"connection-closed",o.firmwareTopologySwitchOpen?"topology-open":"topology-closed",o.updateManualUploadOpen?"upload-open":"upload-closed",o.updateTestFirmwareOpen?"test-open":"test-closed",o.updateTestFirmwareError,U("hpGeneration"),U("projectVersionText"),U("releaseChannelText"),Kl()].join("|")}function Kl(){let e=Math.max(Number(o.lastEntityResponseAt||0),Number(o.lastEntitySyncAt||0)),t=Number(o.deviceReconnectStartedAt||0);return o.entitySyncFailureCount>0&&!o.deviceReconnectMode?"Bezig":e>0&&(!o.deviceReconnectMode||e>=t)?"Verbonden":o.deviceReconnectMode?lo()?"Verbonden":o.deviceReconnectMode==="reconnect"?"Offline":"Bezig":v("status")&&!C("status")?"Offline":"Bezig"}function zl(){return String(T("projectVersionText")||"").trim()||"\u2014"}function MS(){let e=zl();return e&&e!=="\u2014"?e:yo()}function bi(){let e=o.entities.espInternalTemp;if(!e)return"\u2014";let t=R("espInternalTemp");return Number.isNaN(t)?W("espInternalTemp"):z(t,1,e.uom||" \xB0C")}function HS(){let e=[["Netwerkstatus",Kl()],["IP-adres",Cr()]],t=String(T("wifiSsid")||"").trim();t&&e.push(["WiFi SSID",t]);let r=o.entities.wifiSignal;if(r){let n=R("wifiSignal");Number.isNaN(n)||e.push(["WiFi signaal",z(n,0,r.uom||" dBm")])}return e}function Gm(e){return e==="version"?"open-update-modal":e==="connectivity"?"open-connectivity-modal":e==="debugRecording"?"open-debug-recording-modal":e==="webserverLog"?"open-webserver-log-modal":e==="login"?"open-login-modal":""}function Qm(){return[["installation","Installatie",Ht()],["uptime","Uptime",Tr()],["connectivity","Connectiviteit",Kl()],["time","Tijd",al()],["version","Versie",MS(),!!ye()],["debugRecording","Debugopname",Bp(),!0],["webserverLog","Logboek",Va(),!0]]}function Ym(){return Xo()}function Jm(e){return e==="version"&&Ym()}function zn(){return`
    ...
    `}function AS(){if(!o.root)return!1;let e=o.root.querySelector(".oq-helper-status-grid");if(!e)return!!o.root.querySelector(".oq-helper-hub");let t=Qm();if(e.querySelectorAll("[data-oq-header-status]").length!==t.length)return e.outerHTML=zn(),!0;for(let[n,a,i,l]of t){let c=e.querySelector(`[data-oq-header-status="${n}"]`);if(!c)return e.outerHTML=zn(),!0;let u=Gm(n),d=!!(l||u);if(c.tagName.toLowerCase()!==(d?"button":"div"))return e.outerHTML=zn(),!0;let g=c.querySelector(".oq-helper-status-label"),y=c.querySelector(".oq-helper-status-value");if(!g||!y)return e.outerHTML=zn(),!0;g.textContent!==a&&(g.textContent=a);let w=Jm(n),f=w?`<span class="oq-helper-status-value-text">${s(i)}</span><span class="oq-helper-status-badge" aria-label="Update beschikbaar" title="Update beschikbaar"></span>`:s(i);y.innerHTML!==f&&(y.innerHTML=f),d?c.setAttribute("data-oq-action",u):c.removeAttribute("data-oq-action"),c.classList.toggle("oq-helper-status-item--button",d),c.classList.toggle("oq-helper-status-item--attention",w)}return!0}function RS(){return""}function Ul(){return""}function Zm(){let e=o.nativeOpen?"native":"app",t=Ym();return o.interfacePanelOpen?`
    ...
    `}function Ql(e="oq-settings-grid"){return`
      <div class="${s(e)}">
        ${bo("strategy","Verwarmingsstrategie","Kies tussen automatisch regelen met Power House of regelen met een stooklijn.")}
      </div>
    `}function Yl(e="oq-settings-grid"){let t=[le("flowSetpoint","Gewenste flow verwarmen","De flow die OpenQuatt zoveel mogelijk probeert vast te houden buiten koeling."),le("coolingFlowSetpoint","Gewenste flow koelen","De flow die OpenQuatt gebruikt tijdens actieve koeling.")].filter(Boolean).join("");return`
      <div class="${s(e)}">
        ${bo("flowControlMode","Regelmodus","Kies tussen automatische flowregeling en een vaste pompstand.")}
    ...
    `;return we("phResponseProfile","Power House responsprofiel","Kies hoe rustig of direct Power House mag reageren op veranderingen in je woning.",n,"oq-settings-field--span-2")}function Xl(){if(!v("curveControlProfile"))return"";let e=String(T("curveControlProfile")||""),t=o.loadingEntities||o.busyAction==="save-curveControlProfile",n=`
    ...
    `;return we("curveControlProfile","Regelprofiel","Kies of de stooklijn vooral comfortabel, gebalanceerd of rustig moet reageren.",n,"oq-settings-field--span-2")}function DS(){let e=(_,Z=0)=>{let I=R(_);return Number.isNaN(I)?Z:Math.max(0,I)},r=e("phComfortBelow",.1),n=e("phComfortAbove",.3),a=e("phKp",3e3),i=20-r,l=20+n,c=620,u=184,d=46,g=24,y=18,w=40,f=96,m=c-d-g,p=Math.min(20-1.2,i-.35),b=Math.max(20+1.2,l+.35),S=_=>d+(_-p)/Math.max(.01,b-p)*m,q=S(p),E=S(b),k=S(i),$=S(20),P=S(l),N=Math.abs(i-20)>.001,V=Math.abs(l-20)>.001,A=y+24,F=u-w,H=f-44,L=(_,Z,I,j="")=>{let vt=Math.max(q+4,Math.min(E-110-4,_-55)),ut=_-14,It=H,Je=28,_e=f-H+16;return`
        <g class="oq-ph-concept-hotspot" tabindex="0" role="img" aria-label="${s(`${Z} ${I}`)}">
          <rect class="oq-ph-concept-hit" x="${ut}" y="${It}" width="${Je}" height="${_e}" rx="10"></rect>
    ...
    `:""}function tc(e,t,r){let n=[bo(t,"Stand A","Kies hier welke compressorstand je wilt uitsluiten."),bo(r,"Stand B","Kies hier nog een compressorstand die je wilt overslaan.")].filter(Boolean).join("");return n?`
    ...
    `:""}function tg(){let e=NS();return oe("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
      ...
      `;return oe("Regeling","Verwarmingsstrategie","Kies hier hoe OpenQuatt je verwarming regelt. De instellingen hieronder passen zich automatisch aan.",`
        ...
        ${Zl()}
        ${e}
      `)}function oc(){let r={top:22,right:18,bottom:38,left:34},n=560-r.left-r.right,a=240-r.top-r.bottom,i=ge[0].outdoor,l=ge[ge.length-1].outdoor,c=f=>r.left+(f-i)/(l-i)*n,u=f=>r.top+(70-f)/50*a,d=[20,30,40,50,60,70].map(f=>{let m=u(f);return`
    ...
    `}function To(e){let t=R(e);return Number.isNaN(t)?"\u2014":String(Math.max(0,Math.round(t)))}function BS(e){let t=R(e);if(Number.isNaN(t))return"Nog niet gemeten";if(t<1)return"Zojuist";if(t<60)return`${Math.round(t)} min geleden`;let r=Math.floor(t/60),n=Math.round(t%60);return`${r}u ${n}m geleden`}function ng(e){let t=R(e);return Number.isNaN(t)||t<=0?"Tijdstip onbekend":new Intl.DateTimeFormat("nl-NL",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}).format(new Date(t*1e3))}function KS(e){if(!e.cyclingAlertLatched)return"";let t=X("compressorCyclingAlertAlternating"),r=To("compressorCyclingAlertHp1Peak2h"),n=To("compressorCyclingAlertHp1Peak72h"),a=v("compressorCyclingAlertHp2Peak2h")?To("compressorCyclingAlertHp2Peak2h"):"",i=v("compressorCyclingAlertHp2Peak72h")?To("compressorCyclingAlertHp2Peak72h"):"";return`
    ...
    `:"";return oe("Bewaking","Installatiebewaking","Lokale diagnose voor compressorstarts, hydrauliek en verbindingen. Hiervoor is geen Home Assistant nodig.",`
      ...
      `)}function rc(){if(!v("hpGeneration"))return"";let e={V1:{copy:"Voor Quatt V1 en Quatt V1 + V1.5 combinaties.",image:$a,alt:"Quatt Hybrid V1 en V1.5",infoTitle:"V1",infoCopy:`Model: AMM4
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=Gt(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function cg(){let e=Ht(),t=o.entities.hpGeneration||{},r=v("hpGeneration")&&Gt(t).length>0;return!e&&!r?"":oe("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function nc(e="oq-settings-grid oq-settings-boiler-simple-grid"){if(!v("boilerCvAssistEnabled"))return"";let t=C("boilerCvAssistEnabled"),r=v("boilerRatedHeatPower"),n=je("boilerRatedHeatPower"),a=$e("boilerRatedHeatPower"),i=o.loadingEntities||o.busyAction==="switch-boilerCvAssistEnabled",c=r?fo({key:"boilerRatedHeatPower",value:a,meta:n,controlClass:"oq-helper-control oq-helper-control--suffix oq-settings-boiler-power-control",unitMarkup:'<span class="oq-helper-unit-chip">W</span>'}):`
      ...
      `)}function pg(){let e=yo(),t=Ia(),r=o.busyAction==="restartAction";return oe("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
      ...
      `)}function mg(){let e=[tc("Warmtepomp 1","hp1ExcludedA","hp1ExcludedB"),tc("Warmtepomp 2","hp2ExcludedA","hp2ExcludedB")].filter(Boolean).join("");return oe("Installatie","Compressorinstellingen","Stel hier de minimale draaitijd in en bepaal per warmtepomp welke compressorstanden je wilt overslaan.",`
        <div class="oq-settings-subpanel">
          <div class="oq-settings-subpanel-head">
    ...
    `}function GS(){let e=yi(),t=o.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function ZS(){return`
      <section class="oq-helper-panel">
        ...
        ${ae()?`
            <div class="oq-settings-grid oq-settings-grid--quickstart">${Xl()}</div>
            <div class="oq-settings-curve-shell">
              ${oc()}
            </div>
            ${Gl()}
    ...
    `}function oq(){return o.currentStep==="generation"?hg():o.currentStep==="boiler"?v("boilerCvAssistEnabled")?YS():fg():o.currentStep==="flow-source"?US():o.currentStep==="thermostat-source"?GS():o.currentStep==="flow"?JS():o.currentStep==="heating"?ZS():o.currentStep==="water"?XS():o.currentStep==="silent"?eq():o.currentStep==="confirm"?tq():fg()}function ir(){return fr.filter(e=>!e.optionalEntity||v(e.optionalEntity))}function _t(e){let t=ir().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function rq(e){let t=vi(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function nq(e=!1){return ir().map((t,r)=>{let n=rq(r);return`
        <button
    ...
    `}function iq(){let e=vi(),t=ir();return`
    ...
    `:"";return oe("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
      ...
      `},D=(M,x={})=>{if(!v(M))return{markup:"",warning:""};let B=o.entities[M]||{},Q=String(T(M)||""),G=Gt(B),se=new Set(x.hiddenOptions||[]),ve=Q&&se.has(Q),ce=G.filter(de=>!se.has(de)&&y(de,x)),Te=Q&&!y(Q,x),pe=Q==="HA input"&&x.keepUnavailableCurrent!==!0,St=(ve&&!ce.includes(Q)?[Q,...ce]:Te&&!pe&&!ce.includes(Q)?[Q,...ce]:ce).map(de=>{let gr=m(de,x);return`<option value="${s(de)}" ${de===Q?"selected":""}>${s(gr)}</option>`}).join("");return{markup:`
          <label class="oq-settings-source-select">
            <span class="oq-settings-source-select-head">
              <span>${s(x.label||"Bron")}</span>
              ${x.infoCopy?ho(x.infoId||M,x.infoTitle||x.label||"Bron",x.infoCopy):""}
            </span>
            <select class="oq-helper-select" data-oq-field="${s(M)}" ${o.loadingEntities?"disabled":""}>
              ${St}
            </select>
          </label>
        `,warning:ve?"Huidige bron is legacy; kies een nieuwe bron.":Te?`Huidige bron niet beschikbaar: ${w(Q,x)}`:""}},_=({key:M,title:x,icon:B="",select:Q,secondarySelect:G=null,secondarySelects:se=null,activeRows:ve=[],measurementRows:ce=[],activeCopy:Te="",rows:pe=[]})=>{let dt=Q&&Q.when!==!1?D(Q.key,Q):{markup:"",warning:""},de=(Array.isArray(se)?se:G?[G]:[]).filter(Se=>Se&&Se.when!==!1).map(Se=>D(Se.key,Se)).filter(Se=>Se.markup),gr=de.map(Se=>Se.markup).join(""),We=de.map(Se=>Se.warning).find(Boolean)||"",Ze=pe.filter(Boolean).join(""),pt=`${dt.markup}${gr}`,mt=dt.warning||We,Mo=[L({title:"Configuratie",icon:"settings",className:"oq-settings-source-group--config",content:pt?`
    ...
    `:"";return oe("Integratie","MQTT inputbronnen","Beheer de brokerverbinding voor externe MQTT-bronwaarden.",`
      ...
      `)}function $g(){let e=[["login","Login",Ln(),On(),"open-login-modal"],["api","ESPHome API encryption",Dr(),Lr(),"open-api-security-modal"]];return oe("Toegang","Toegang & Beveiliging","Pas hier de web-login of de ESPHome API-sleutel aan. Deze wijziging wordt actief na herstart.",`
      ...
      `)}function Mg(){let e=De.has(o.settingsGroup)?o.settingsGroup:me[0].id;return`
    ...
    `}function Hg(){let e=De.has(o.settingsGroup)?o.settingsGroup:me[0].id;return`
    ...
    `:""}function lc(){let e=qa.map(pq).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function Pg(e=lc()){return he(e)}function Ng(e=lc()){return`
    ...
    `}var _g=[{id:"day",label:"Dag"},{id:"week",label:"Week"},{id:"month",label:"Maand"},{id:"year",label:"Jaar"},{id:"all",label:"Alles"}],mq=new Set(["day","week","month","year"]);function Be(e){let t=String(e||"").trim();return _g.some(r=>r.id===t)?t:"day"}function Dg(e){let t=Be(e);o.energyHistoryView!==t&&(Ct({energyHistoryView:t,energyHistoryLastFetchAt:0}),h(),Lg())}function Lg(){typeof ar=="function"&&ar({force:!0}).then(e=>{e&&h()})}function Yr(e){return mq.has(Be(e))}function qi(){let e=new Date;return e.getFullYear()*1e4+(e.getMonth()+1)*100+e.getDate()}function Og(){return ai(o.energyHistoryRaw)}function cc(){let e=String(o.energyHistoryRaw||""),t=null;return e.split(/\r?\n/).forEach(r=>{let n=Ol(r);n&&(t=n.dateKey)}),t}function gq(e=[],t=!0){let r=cc(),n=Og(),a=(Array.isArray(e)?e:[]).map(i=>Number(i?.dateKey)).filter(Number.isFinite);return Number.isFinite(Number(n.newestDateKey))&&a.push(Number(n.newestDateKey)),Number.isFinite(Number(r))&&a.push(Number(r)),t&&ki().forEach(i=>{let l=Number(i?.dateKey);Number.isFinite(l)&&a.push(l)}),a.length?Math.max(...a):qi()}function sr(e){let t=R(e);return!Number.isFinite(t)||t<0?null:Math.round(t*1e3)}function fq(){let e=cc()||qi(),t=re(e);if(!t)return null;let r={sequence:Number.MAX_SAFE_INTEGER-1,dateKey:e,year:t.year,month:t.month,day:t.day,partial:!0,source:"sensors",electricalInputWh:sr("electricalEnergyDaily"),heatingInputWh:sr("heatingElectricalEnergyDaily"),coolingInputWh:sr("coolingElectricalEnergyDaily"),heatpumpHeatOutputWh:sr("heatpumpThermalEnergyDaily"),heatpumpCoolingOutputWh:sr("heatpumpCoolingEnergyDaily"),boilerHeatOutputWh:sr("boilerThermalEnergyDaily"),systemHeatOutputWh:sr("systemThermalEnergyDaily")};return Fn.some(n=>Number.isFinite(r[n]))?r:null}function Qn(){let e=new Map;String(o.energyHistoryRaw||"").split(/\r?\n/).forEach(i=>{let l=wm(i)||Ol(i);if(!l)return;let c=e.get(l.dateKey);(!c||l.sequence>=c.sequence)&&e.set(l.dateKey,l)});let r=fq();r&&e.set(r.dateKey,r);let n=new Set(e.keys()),a=new Map;return ki().forEach(i=>{if(n.has(i.dateKey))return;let l=a.get(i.dateKey);if(!l){let c=re(i.dateKey);if(!c)return;l=lr({dateKey:c.key,year:c.year,month:c.month,day:c.day,label:Nt(c.key),sortKey:c.key,source:"hour-summary"}),l.tooltipLabel=`${Nt(i.dateKey)} \xB7 uurdata sinds herstart`,a.set(i.dateKey,l)}cr(l,i)}),a.forEach((i,l)=>{e.set(l,i)}),[...e.values()].sort((i,l)=>i.dateKey-l.dateKey)}function ki(){let e=new Map;return String(o.energyHistoryRaw||"").split(/\r?\n/).forEach(r=>{let n=ym(r);if(!n)return;let a=`${n.dateKey}:${n.hour}`,i=e.get(a);(!i||n.sequence>=i.sequence)&&e.set(a,n)}),[...e.values()].sort((r,n)=>r.sortKey-n.sortKey)}function hq(e){return ki().filter(t=>t.dateKey===Number(e))}function Qr(e,t){return e.reduce((r,n)=>r+Wr(n,t),0)}function Ig(e){return["heatpumpHeatOutputWh","heatpumpCoolingOutputWh","boilerHeatOutputWh"].reduce((t,r)=>t+Wr(e,r),0)}function bq(e){return Wr(e,"electricalInputWh")+Ig(e)}function Gn(e,t){let r=Number(e),n=Number(t);return!Number.isFinite(r)||!Number.isFinite(n)||n<=0?"\u2014":(r/n).toFixed(2)}function st(e,t=1){let r=Number(e);return Number.isFinite(r)?Math.abs(r)>=999500?`${(r/1e6).toFixed(2)} MWh`:Math.abs(r)<1e3?`${Math.round(r)} Wh`:`${(r/1e3).toFixed(t)} kWh`:"\u2014"}function lr({dateKey:e,year:t,month:r,day:n,hour:a=null,label:i,tooltipLabel:l="",sortKey:c,source:u="bucket"}){return{sequence:0,dateKey:e,year:t,month:r,day:n,hour:a,label:i,tooltipLabel:l,sortKey:c??e,partial:!1,source:u,electricalInputWh:0,heatingInputWh:0,coolingInputWh:0,heatpumpHeatOutputWh:0,heatpumpCoolingOutputWh:0,boilerHeatOutputWh:0,systemHeatOutputWh:0}}function cr(e,t){return Fn.forEach(r=>{e[r]+=Wr(t,r)}),e.partial=e.partial||!!t?.partial,e.sequence=Math.max(Number(e.sequence||0),Number(t?.sequence||0)),e}function wq(e){let t=new Map;return e.forEach(r=>{t.set(r.dateKey,r)}),t}function Fg(e,t){let r=Be(e);if(r==="day"){let n=Rl(t)||re(t);return n?String(n.key):""}if(r==="week"){let n=bm(t);return n?String(n.key):""}if(r==="month"){let n=hm(t);return n?String(n.key):""}if(r==="year"){let n=Number(t);return Number.isInteger(n)&&n>=2020&&n<=2200?String(n):""}return""}function Ei(e,t){let r=Be(t),n=re(gq(e,!0)),a=Og(),i=ki(),l=[...e.map(f=>f.dateKey),...i.map(f=>f.dateKey)].filter(f=>Number.isFinite(Number(f)));Number.isFinite(Number(a.oldestDateKey))&&l.push(Number(a.oldestDateKey)),Number.isFinite(Number(a.newestDateKey))&&l.push(Number(a.newestDateKey));let c=l.length?Math.min(...l.map(Number)):n?.key,d=(c?re(c):n)?.date||n?.date||new Date,g=n?.date||new Date,y=n?.key||qi(),w=y;return r==="week"?(y=_l(d),w=_l(g)):r==="month"?(y=si(d),w=si(g)):r==="year"?(y=d.getFullYear(),w=g.getFullYear()):(y=Ie(d),w=Ie(g)),Number(y)>Number(w)&&(y=w),{min:String(y),max:String(w)}}function Wg(e,t){let r=Number(e);return Number.isFinite(r)?r<Number(t.min)?String(t.min):r>Number(t.max)?String(t.max):String(e):String(t.max)}function yq(e,t,r=Ei(e,t)){let n=Be(t),a=o.energyHistoryPeriodSelection?.[n],i=Fg(n,a);return Wg(i||r.max,r)}function vq(e,t){let r=Be(e),n=[],a=0;if(r==="week"){let i=String(t.max);for(;Number(i)>=Number(t.min)&&a<6e3;){let l=re(i);n.push({value:i,label:Dl(i),group:l?String(Nl(l.date).year):""}),i=l?String(Ie(nr(l.date,-7))):"",a+=1}return n}if(r==="month"){let i=String(t.max);for(;Number(i)>=Number(t.min)&&a<1200;){let l=rr(i);if(!l)break;n.push({value:i,label:l.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"}),group:String(l.year)}),i=Pl(i,-1),a+=1}return n}if(r==="year")for(let i=Number(t.max);i>=Number(t.min);i-=1)n.push({value:String(i),label:String(i)});return n}function Ti(e,t){let r=Be(t);if(!Yr(r))return{view:r,selectedValue:"",minValue:"",maxValue:"",canPrevious:!1,canNext:!1,isNow:!0,options:[]};let n=Ei(e,r),a=yq(e,r,n);return{view:r,selectedValue:a,minValue:n.min,maxValue:n.max,canPrevious:Number(a)>Number(n.min),canNext:Number(a)<Number(n.max),isNow:Number(a)===Number(n.max),options:vq(r,n)}}function Sq(e,t){let r=Be(t);if(!Yr(r))return{from:"",to:"",hours:"0"};let n=Ti(e,r);if(r==="day")return{from:n.selectedValue,to:n.selectedValue,hours:"1"};if(r==="week"){let a=re(n.selectedValue);if(!a)return{from:"",to:"",hours:"0"};let i=Fr(a.date),l=nr(i,6);return{from:String(Ie(i)),to:String(Ie(l)),hours:"0"}}if(r==="month"){let a=rr(n.selectedValue);return a?{from:String(a.year*1e4+a.month*100+1),to:String(a.year*1e4+a.month*100+Al(a.year,a.month)),hours:"0"}:{from:"",to:"",hours:"0"}}if(r==="year"){let a=Number(n.selectedValue);return Number.isInteger(a)?{from:`${a}0101`,to:`${a}1231`,hours:"0"}:{from:"",to:"",hours:"0"}}return{from:"",to:"",hours:"0"}}function qq(){if(!String(o.energyHistoryRaw||"").trim())return"?meta=1";let e=Qn(),t=Sq(e,o.energyHistoryView||"day"),r=new URLSearchParams;t.from&&r.set("from",t.from),t.to&&r.set("to",t.to),r.set("hours",t.hours);let n=r.toString();return n?`?${n}`:""}vm(qq);function Yn(e,t){let r=Be(e);if(!Yr(r))return;let n=Qn(),a=Ei(n,r),i=Fg(r,t),l=Wg(i||a.max,a);Ct({energyHistoryPeriodSelection:{...o.energyHistoryPeriodSelection,[r]:l},energyHistoryLastFetchAt:0}),h(),Lg()}function jg(e,t){let r=Be(e);if(!Yr(r))return;let n=Qn(),a=Ti(n,r),i=Number(t)<0?-1:1,l=a.selectedValue;if(r==="day"){let c=re(a.selectedValue);l=c?String(Ie(nr(c.date,i))):l}else if(r==="week"){let c=re(a.selectedValue);l=c?String(Ie(nr(c.date,i*7))):l}else r==="month"?l=Pl(a.selectedValue,i):r==="year"&&(l=String(Number(a.selectedValue)+i));Yn(r,l)}function Vg(e){let t=Be(e);if(!Yr(t))return;let r=Qn(),n=Ei(r,t);Yn(t,n.max)}function kq(e,t,r=Ti(e,t)){let n=Be(t),a=wq(e);if(!e.length&&n==="all")return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};if(n==="day"){let c=re(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=hq(c.key);if(u.length){let f=new Map(u.map(p=>[p.hour,p])),m=[];for(let p=0;p<24;p+=1){let b=String(p),S=`${c.date.toLocaleDateString("nl-NL",{day:"numeric",month:"long"})} \xB7 ${String(p).padStart(2,"0")}:00 - ${String((p+1)%24).padStart(2,"0")}:00`,q=lr({dateKey:c.key,year:c.year,month:c.month,day:c.day,hour:p,label:b,tooltipLabel:S,sortKey:p,source:"hour"}),E=f.get(p);E&&cr(q,E),m.push(q)}return{buckets:m,title:"Dag",detail:`${c.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 uurdata sinds herstart`}}let d=a.get(c.key),g=cc()||qi(),y=c.key===g?"Vandaag":Nt(c.key),w=lr({dateKey:c.key,year:c.year,month:c.month,day:c.day,label:y,sortKey:c.key,source:"day"});return d&&cr(w,d),{buckets:[w],title:"Dag",detail:`${c.date.toLocaleDateString("nl-NL",{weekday:"long",day:"numeric",month:"long",year:"numeric"})} \xB7 dagtotaal`}}if(n==="week"){let c=re(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=Fr(c.date),d=[];for(let g=0;g<7;g+=1){let y=nr(u,g),w=Ie(y),f=re(w),m=lr({dateKey:w,year:f.year,month:f.month,day:f.day,label:Nt(w,"weekday"),sortKey:w}),p=a.get(w);p&&cr(m,p),d.push(m)}return{buckets:d,title:"Week",detail:Dl(r.selectedValue)}}if(n==="month"){let c=rr(r.selectedValue);if(!c)return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=Al(c.year,c.month),d=[];for(let g=1;g<=u;g+=1){let y=Hl(c.year,c.month,g),w=Ie(y),f=lr({dateKey:w,year:c.year,month:c.month,day:g,label:String(g),sortKey:w}),m=a.get(w);m&&cr(f,m),d.push(f)}return{buckets:d,title:"Maand",detail:c.date.toLocaleDateString("nl-NL",{month:"long",year:"numeric"})}}if(n==="year"){let c=Number(r.selectedValue);if(!Number.isInteger(c))return{buckets:[],title:"Geen data",detail:"Lifetime energiehistorie"};let u=[];for(let d=1;d<=12;d+=1){let g=c*1e4+d*100+1,y=lr({dateKey:g,year:c,month:d,day:1,label:Nt(g,"month"),sortKey:d,source:"month"});e.filter(w=>w.year===c&&w.month===d).forEach(w=>cr(y,w)),u.push(y)}return{buckets:u,title:"Jaar",detail:String(c)}}let i=new Map;e.forEach(c=>{i.has(c.year)||i.set(c.year,lr({dateKey:c.year*1e4+101,year:c.year,month:1,day:1,label:String(c.year),sortKey:c.year,source:"year"})),cr(i.get(c.year),c)});let l=[...i.values()].sort((c,u)=>c.sortKey-u.sortKey);return{buckets:l,title:"Alles",detail:l.length?`${l[0].label} - ${l[l.length-1].label}`:"Geen data"}}function Eq(e){let t=Qr(e,"heatpumpHeatOutputWh"),r=Qr(e,"heatpumpCoolingOutputWh"),n=Qr(e,"boilerHeatOutputWh");return{electricalInputWh:Qr(e,"electricalInputWh"),heatingInputWh:Qr(e,"heatingInputWh"),coolingInputWh:Qr(e,"coolingInputWh"),heatOutputWh:t,coolingOutputWh:r,boilerOutputWh:n,outputWh:t+r+n}}function Tq(e){let t=Number(e.heatOutputWh||0)+Number(e.coolingOutputWh||0),r=Number(e.boilerOutputWh||0),n=t+r;return!Number.isFinite(n)||n<=0?Number.NaN:t/n*100}function Cq(e){let t=Gn(e.heatOutputWh,e.heatingInputWh),r=Gn(e.coolingOutputWh,e.coolingInputWh),n=Number(e.heatOutputWh||0)>0&&t!=="\u2014",a=Number(e.coolingOutputWh||0)>0&&r!=="\u2014";return n&&a?{label:"COP / EER",value:`${t} / ${r}`}:a?{label:"Gemiddelde EER",value:r}:{label:"Gemiddelde COP",value:t}}function Si(e,t,r=""){return`
      <div class="oq-energy-history-stat">
        <span>${s(e)}</span>
        <strong>${s(t)}</strong>
        ${r?`<p>${s(r)}</p>`:""}
      </div>
    `}function sc(e,t,r){let n=[];r.forEach(i=>{let l=String(i.group||""),c=n[n.length-1];(!c||c.label!==l)&&(c={label:l,options:[]},n.push(c)),c.options.push(i)});let a=n.some(i=>i.label)?n.map(i=>i.label?`
    ...
    `}function Rg(e){return e.view==="day"?`
      ...
      `:e.view==="week"?sc(e,"Week",e.options):e.view==="month"?sc(e,"Maand",e.options):e.view==="year"?sc(e,"Jaar",e.options):`
      <div class="oq-energy-history-period-field oq-energy-history-period-field--static">
        <span>Periode</span>
        <strong>Volledig bereik</strong>
      </div>
    `}function xq(e){return Yr(e.view)?`
    ...
    `:`
      ...
      `}function $q(){let e=document.activeElement;return!!(e&&e.closest&&e.closest(".oq-energy-history-period"))}function Mq(e){return`
      <div class="oq-energy-history-view-tabs" role="tablist" aria-label="Energiehistorie weergave">
        ${_g.map(t=>{let r=t.id===e;return`
            <button
              type="button"
    ...
    `}function Hq(e){let t=Number(e.electricalInputWh||0),r=Number(e.heatOutputWh||0),n=Number(e.coolingOutputWh||0),a=Number(e.boilerOutputWh||0),i=a>0?"boiler":"boiler-zero",l=Math.max(1,t+r+n+a),c=d=>`${Math.max(0,Number(d||0)/l*100).toFixed(2)}%`,u=Tq(e);return`
    ...
    `}function Aq(e){let t=Math.max(1,Number(e||0)/1e3),r=Math.pow(10,Math.floor(Math.log10(t))),n=t/r;return(n<=1.5?1.5:n<=3?3:n<=6?6:10)*r*1e3}function Rq(e){let t=Number(e);return Number.isFinite(t)?t>=999500?`${Number((t/1e6).toFixed(1))}`:`${Number((t/1e3).toFixed(1))}`:""}function Pq(e){return e>=999500?"MWh":"kWh"}function Nq(e){let d=Math.max(1e3,...e.map(bq)),g=Aq(d),y=e.length?1218/e.length:1218,w=Math.max(6,Math.min(38,y*.68));return{width:1280,height:260,left:44,right:18,top:26,bottom:38,plotWidth:1218,plotHeight:196,axisMax:g,barSlot:y,barWidth:w,yOf:m=>26+(1-Math.min(1,Math.max(0,Number(m||0)/g)))*196}}function _q(e){let t=Gn(e.heatpumpHeatOutputWh,e.heatingInputWh),r=Gn(e.heatpumpCoolingOutputWh,e.coolingInputWh),n=Gn(Ig(e),e.electricalInputWh);return[e.tooltipLabel||e.label||Nt(e.dateKey),`Elektrisch: ${st(e.electricalInputWh,1)}`,`Warmtepomp warmte: ${st(e.heatpumpHeatOutputWh,1)}`,`Warmtepomp koeling: ${st(e.heatpumpCoolingOutputWh,1)}`,`Cv-ketel: ${st(e.boilerHeatOutputWh,1)}`,`COP verwarmen: ${t}`,`EER koelen: ${r}`,`Output / elektrisch: ${n}`].join(`
`)}function Dq(e,t=""){if(!e.length)return`
      ...
      `;let r=Nq(e),n=Pq(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,c)=>{let u=r.left+r.barSlot*c+r.barSlot/2,d=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],g=r.height-r.bottom,y=d.map(p=>{let b=Wr(l,p.key);if(b<=0)return"";let S=b/r.axisMax*r.plotHeight;return g-=S,`
        ...
        `}).join(""),f=e.length<=12||c===0||c===e.length-1||c%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||Nt(l.dateKey))}</text>`:"",m=_q(l);return`
      ...
      <svg class="oq-energy-history-chart oq-energy-history-chart--${s(Be(t))}" viewBox="0 0 ${r.width} ${r.height}" role="img" aria-label="Energiehistorie">
        ...
        ${a.map(l=>{let c=r.yOf(l);return`
            ...
            <text x="${r.left-10}" y="${c.toFixed(1)}" text-anchor="end" dominant-baseline="middle" class="oq-energy-history-axis-label">${s(Rq(l))}</text>
          `}).join("")}
        ${i}
      </svg>
    `}function Lq(e=null){let t=Number(e?.boilerOutputWh||0)>0?"boiler":"boiler-zero";return`
    ...
    `}function uc(){let e=Qn(),t=Be(o.energyHistoryView),r=Ti(e,t),n=kq(e,t,r),a=Eq(n.buckets);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:a,activeView:t}}function Bg(e=uc()){return he({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,currentValues:Fn.map(t=>e.buckets[e.buckets.length-1]?.[t]??null)})}function Kg(e=uc()){let t=e.summary,r=Cq(t),n=e.buckets[0]?.dateKey?Nt(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?Nt(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
      <section class="oq-energy-history" data-render-signature="${s(Bg(e))}">
        ...
        <div class="oq-energy-history-chart-wrap">
          ${Dq(e.buckets,e.activeView)}
          <div class="oq-energy-history-tooltip" aria-hidden="true"></div>
        </div>
        ${Lq(t)}
      </section>
    `}function dc(e){if(o.appView!=="results"||!o.root)return;let t=e.target.closest?.("[data-oq-energy-history-tip]"),r=t?.closest?.(".oq-energy-history-chart-wrap")||o.root.querySelector(".oq-energy-history-chart-wrap"),n=r?.querySelector(".oq-energy-history-tooltip");if(!t||!r||!n){n&&n.classList.remove("is-visible");return}let a=String(t.dataset.oqEnergyHistoryTip||"").split(/\n/).filter(Boolean);if(!a.length){n.classList.remove("is-visible");return}n.innerHTML=`
      <strong>${s(a[0])}</strong>
    ...
    `}function Oq(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=lc();return Ye(t,Pg(n),Ng(n))}function Iq(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=uc(),a=$q();return(a?!1:Ye(t,Bg(n),Kg(n)))||a}mo({patchEnergyDom:Oq,patchResultsDom:Iq});function Fq({label:e,value:t,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(t)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function Xn(e,t=!1){return e.map(r=>Fq({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?ot(r.key):r.value,status:t})).join("")}function fc(e){return`
      <div class="oq-overview-sectionhead">
        <h3>${s(e)}</h3>
      </div>
    `}function hc(){let e=Kt();return`
    ...
    `}function Gg(e){let t=String(e||"").trim();return{None:"geen bron",Manual:"handmatig",Disabled:"handmatig","HA input":"HA-invoer",MQTT:"MQTT","OT thermostat":"OpenTherm","HA input + Manual":"HA-invoer + handmatig","MQTT + Manual":"MQTT + handmatig","OT thermostat + Manual":"OpenTherm + handmatig"}[t]||t}function Jn(e,t){return!t||t==="geen bron"?e:`${e} Toestemming: ${t}.`}function bc(e,t){return t?"Actief":e==="Stand-by"?"Stand-by":e==="Onbekend"?"Onbekend":"Niet actief"}function Wq(e,t){let r=t?"active":"neutral",n=bc(e,t);return`<span class="oq-overview-chip oq-overview-chip--${s(r)}" data-oq-bind="panel-status">${s(n)}</span>`}function jq(e){return`
      <span
    ...
    `}function Bq(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}var Kq=Io;function zq(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function Uq(e){let t=zq(W("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Gq(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=Kq(r),a=v("timeValid")&&C("timeValid")?Uq(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Qq(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Xg(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>v(e))||""}function Yq(){return["hp1WaterIn","hp2WaterIn"].find(e=>v(e))||""}function ef(e=W("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function Lt(){return ef()}function tf(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function yc(){return Lt()?"Koeling":ae()?"Stooklijn":"Power House"}function Jq(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=R(t);if(!Number.isNaN(r))return r}return Number.NaN}function of(){let e=Jq(),t=R("phouseHouse"),r=R("totalHeat"),n=R("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:z(e,0,"W"),houseText:z(t,0,"W"),correctionText:Qq(a),capacityText:ot("hpCapacity"),statusTitle:i,statusCopy:l}}function rf(){let e=R("curveSupplyTarget"),t=R("supplyTemp"),r=Xg(),n=r?R(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:ot("curveSupplyTarget"),supplyText:ot("supplyTemp"),deltaText:Bq(a),capacityText:ot("hpCapacity"),statusTitle:l,statusCopy:c}}function nf(){let e=R("supplyTemp"),t=W("coolingGuardMode",""),r=W("coolingFallbackNightMinOutdoorTemp","\u2014"),n=R("coolingSupplyError"),a=R("coolingDemandRaw"),i=C("coolingPermitted"),l=C("coolingRequestActive"),c=W("coolingBlockReason","Onbekend"),u=ic(c),d=tf(c,l),g="Wacht op koelvraag",y="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return d?(g="Wacht op koelvraag",y="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(g="Houdt doel vast",y="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(g="Trekt aanvoer omlaag",y="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(g="Benadert koeldoel",y="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(g="Koelt rustig door",y="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(g="Koeling gereed",y="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(g="Koeling geblokkeerd",y=`Blokkade: ${u}.`),{targetText:ot("coolingSupplyTarget"),supplyText:ot("supplyTemp"),safeFloorText:ot("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:ot("coolingDemandRaw"),statusTitle:g,statusCopy:y,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:d}}function xi(){if(Lt()){let t=nf(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(ae()){let t=rf();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=of();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function vc(e){return Jg({className:"oq-overview-system",title:e.title,copy:e.copy,signature:he(e),body:`
        ...
        <div class="oq-overview-metrics oq-overview-metrics--three-column">
          ${e.metrics.map(t=>Vq(t.label,t.value,t.tone,t.note)).join("")}
        </div>
      `})}function Zq(){if(!C("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(Lt()){let n=nf(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(Ag())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(ae()?rf():of()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Xq(){return C("openquattEnabled")?Lt()?C("coolingPermitted")?C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:W("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:C("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:C("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Sc(e,t){let r=Zq(),n=Xq();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function qc(e,t){let r=Sc(e,t);return`
    ...
    `}function kc(){let e=Lt();return[{key:"totalPower",label:"Elektrisch vermogen",tone:"blue",note:"hele systeem"},{key:e?"totalCoolingPower":"totalHeat",label:e?"Koelvermogen":"Verwarmingsvermogen",tone:"orange",note:"thermisch vermogen"},{key:e?"totalEer":"totalCop",label:e?"COP (EER)":"COP",tone:"green",note:"rendement"},{key:"flowSelected",label:"Flow",tone:"sky",note:"watercircuit"}]}function ek(){let e=C("openquattEnabled"),t=T("openquattResumeAt"),r=mn(t),n=(o.loadingEntities||o.entitySyncInFlight)&&!v("openquattResumeAt"),a=C("manualCoolingEnable"),i=v("coolingEnableSelected")?C("coolingEnableSelected"):a,l=Gg(W("coolingEnableEffectiveSource","")),c=String(T("coolingEnableSource")||"").trim(),u=Gg(T("coolingEnableSource")),d=String(T("silentModeOverride")||"Schedule"),g=!C("coolingPermitted"),y=C("coolingRequestActive"),w=ef(),f=W("coolingBlockReason",""),m=tf(f,y),p="Uit",b=c==="Disabled"?"Koeling is niet toegestaan: handmatig staat uit.":u&&u!=="geen bron"?`Koeling is niet toegestaan: ${u} geeft geen toestemming en handmatig staat uit.`:"Koeling is niet toegestaan.";i&&w?(p="Actief",b=Jn("Koeling draait nu.",l)):i&&m?(p="Aan",b=Jn("Koeling is toegestaan en wacht op kamertemperatuur boven het koel-setpoint.",l)):i&&g?(p="Geblokkeerd",b=Jn(ic(f||"Koeling wacht nog op veilige condities."),l)):i&&y?(p="Start bijna",b=Jn("Er is koelvraag. Koeling start zodra dat kan.",l)):i&&(p="Aan",b=Jn("Koeling is toegestaan en wacht op koelvraag.",l));let S="Uit",q="Stille modus staat uit.",E="neutral";return d==="On"?(S="Aan",q="Stille modus staat geforceerd aan, ook buiten het tijdvenster.",E="orange"):d==="Schedule"&&(S="Schema",C("silentActive")?(q="Stille modus staat nu aan via het tijdvenster.",E="violet"):q="Stille modus volgt het tijdvenster."),[{key:"openquattEnabled",label:"Openquatt regeling",status:e?"Actief":"Tijdelijk uit",copy:e?"Verwarmen en koelen worden automatisch geregeld.":r?"Verwarming en koeling zijn tijdelijk uitgeschakeld. Beveiligingen blijven actief.":"Verwarming en koeling zijn uitgeschakeld. Beveiligingen blijven actief.",tone:e?"green":"orange",kind:"openquatt-control",meta:e?[]:[n?{label:"Hervatten",value:"Laden\u2026",tone:"neutral",loading:!0}:{label:r?"Hervat automatisch":"Hervatten",value:r?yr(t,!0):"Handmatig",tone:r?"orange":"neutral"}]},{key:"manualCoolingEnable",label:"Koeling",status:p,copy:b,buttonLabel:a?"Handmatig uit":"Handmatig aan",nextState:a?"off":"on",tone:i?w?"blue":"sky":"neutral"},{key:"silentModeOverride",label:"Stille modus",status:S,copy:q,tone:E,kind:"select",selectedOption:d,settingsAction:!0,options:[{value:"Off",label:"Uit"},{value:"On",label:"Aan"},{value:"Schedule",label:"Schema"}]}].filter(k=>v(k.key))}function tk(e=[]){return e.length?`
    ...
    `:""}function Jr({className:e,action:t,label:r,busy:n=!1,loading:a=!1,attrs:i=""}){return`
      ...
      `:s(n?"Bezig...":r)}</button>
    `}function ok(e){if(e.kind==="openquatt-control"){let t=o.busyAction==="openquatt-regulation",r=(o.loadingEntities||o.entitySyncInFlight)&&!v("openquattResumeAt");return C("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${Jr({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:t})}</div>`:`
    ...
    `}function Ec(){let e=ek();return e.length?`
    ...
    `}function $i(){let e=Xg(),t=Yq();return Lt()?{title:"Koeltemperaturen",copy:"De belangrijkste temperaturen voor kamercomfort, koeldoel en dauwpuntveiligheid.",rows:[{label:"Kamertemperatuur",key:"roomTemp"},{label:"Kamer setpoint",key:"roomSetpoint"},{label:"Aanvoertemperatuur",key:"supplyTemp"},{label:"Koeldoel",key:"coolingSupplyTarget"},{label:"Veilige aanvoergrens",key:"coolingMinimumSafeSupplyTemp"},{label:"Dauwpunt",key:"coolingDewPointSelected"}]}:{title:"Temperaturen",copy:"De belangrijkste temperaturen voor comfort en regeling.",rows:[{label:"Kamertemperatuur",key:"roomTemp"},{label:"Kamer setpoint",key:"roomSetpoint"},{label:"Aanvoertemperatuur",key:"supplyTemp"},...t?[{label:"Retourtemperatuur",key:t}]:[],e?{label:"Buitentemperatuur",key:e}:{label:"Buitentemperatuur",key:"",value:"\u2014"}]}}function Tc(e=$i()){return he({...e,values:e.rows.map(t=>t.value||W(t.key))})}function Cc(){let e=$i();return Jg({className:"oq-overview-temps",title:e.title,copy:e.copy,signature:Tc(e),body:`
        <div class="oq-overview-temps-list">
          ${e.rows.map(t=>Ci(t.label,t.key,t.value||"")).join("")}
        </div>
      `})}var Qg=360,rk=100;function to(){let e=jn(o.trendWindowHours||Et);return e!==o.trendWindowHours&&jr(e),e}function sf(e=to()){return Math.max(1,Number(e)||24)*60*60*1e3}function nk(e=to()){let t=Number(e)||24;return t>=72&&t%24===0?`${t/24}d`:`${t}u`}function xc(e=to()){let t=Number(e)||24;if(t>=72&&t%24===0){let r=t/24;return`${r} ${r===1?"dag":"dagen"}`}return`${t} uur`}function pc(e){if(!Number.isFinite(e))return"\u2014";let t=new Date(e);if(Number.isNaN(t.getTime()))return"\u2014";let r={day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",r).format(t)}catch{return t.toLocaleString("nl-NL",r)}}function ak(e){let t=String(e||"").trim().split("|");if(t.length<5)return null;let r=Number(t[0]);if(!Number.isFinite(r))return null;let n=a=>{let i=Number(a);return Number.isFinite(i)?i:null};return{t:r,outside:n(t[1]),supply:n(t[2]),room:t.length>=8?n(t[3]):null,roomSetpoint:t.length>=8?n(t[4]):null,flow:t.length>=8?n(t[5]):null,input:t.length>=8?n(t[6]):n(t[3]),output:t.length>=8?n(t[7]):n(t[4])}}function gc(){return!1}function mc(e=to()){return[]}function lf(){let e=sf(),t=String(o.trendHistoryRaw||"").trim();if(!t)return gc()?mc():[];let r=t.split(/\r?\n/).map(ak).filter(Boolean),n=r.length?r[r.length-1].t:Number.NaN,a=Number.isFinite(o.trendHistoryNowMs)?o.trendHistoryNowMs:Number.isFinite(n)?n:Number.NaN;if(!Number.isFinite(a))return r.length?r.slice(-Qg):mc();let i=Math.max(0,a-e),l=r.filter(c=>c.t>=i).slice(-Qg);return l.length?l:gc()?mc(windowHours):[]}function $c(){let e=to(),t=xc(e),r=lf(),n=gc()&&r.length===0,a=Lt();return[{id:"temperatures",title:"Temperaturen",copy:`Buiten- en aanvoertemperatuur van de laatste ${t}.`,tone:"orange",samples:r,mock:n,windowHours:e,series:[{id:"outside",sampleKey:"outside",currentKey:"outsideTempSelected",label:"Buiten",tone:"orange",decimals:1,unit:" \xB0C"},{id:"supply",sampleKey:"supply",currentKey:"supplyTemp",label:"Aanvoer",tone:"blue",decimals:1,unit:" \xB0C"}]},{id:"power",title:"Vermogen",copy:`Elektrisch vermogen en verwarmingsvermogen van de laatste ${t}.`,tone:"green",samples:r,mock:n,windowHours:e,series:[{id:"input",sampleKey:"input",currentKey:"totalPower",label:"Elektrisch vermogen",tone:"green",decimals:0,unit:" W"},{id:"output",sampleKey:"output",currentKey:a?"totalCoolingPower":"totalHeat",label:a?"Koelvermogen":"Verwarmingsvermogen",tone:"sky",decimals:0,unit:" W"}]},{id:"rendement",title:"Rendement",copy:`COP van de laatste ${t}.`,tone:"slate",samples:r,mock:n,windowHours:e,series:[{id:"cop",label:"COP",tone:"slate",decimals:1,unit:"",currentKey:a?"totalEer":"totalCop",derive:i=>{let l=Number(i?.input),c=Number(i?.output);return!Number.isFinite(l)||!Number.isFinite(c)||l<rk?Number.NaN:c/l}}]},{id:"comfort",title:"Comfort",copy:`Kamertemperatuur en setpoint van de laatste ${t}.`,tone:"blue",samples:r,mock:n,windowHours:e,series:[{id:"roomTemp",sampleKey:"room",currentKey:"roomTemp",label:"Kamertemperatuur",tone:"blue",decimals:1,unit:" \xB0C"},{id:"roomSetpoint",sampleKey:"roomSetpoint",currentKey:"roomSetpoint",label:"Kamer setpoint",tone:"orange",decimals:1,unit:" \xB0C"}]},{id:"flow",title:"Flow",copy:`Flow van de laatste ${t}.`,tone:"sky",samples:r,mock:n,windowHours:e,series:[{id:"flow",sampleKey:"flow",currentKey:"flowSelected",label:"Flow",tone:"sky",decimals:0,unit:" L/h",axisMin:0,axisTickStep:250}]}]}function ik(e){let t=e.samples[e.samples.length-1]||null;return he({id:e.id,windowHours:e.windowHours,sampleCount:e.samples.length,firstTimestamp:e.samples[0]?.t||0,lastTimestamp:t?.t||0,trendSignature:o.trendHistorySignature||"",latestValues:t?[t.outside,t.supply,t.room,t.roomSetpoint,t.flow,t.input,t.output]:[]})}function Zn(e,t){if(!e||!t)return Number.NaN;let r=typeof e.derive=="function"?e.derive(t):t?.[e.sampleKey],n=Number(r);return Number.isFinite(n)?n:Number.NaN}function sk(e,t){let r=[];return e.forEach(n=>{t.forEach(a=>{let i=Zn(a,n);Number.isFinite(i)&&r.push(i)})}),r.length?{min:Math.min(...r),max:Math.max(...r)}:{min:0,max:1}}function lk(e){if(!Number.isFinite(e)||e<=0)return 1;let t=Math.floor(Math.log10(e)),r=e/10**t,n;return r<=1?n=1:r<=2?n=2:r<=5?n=5:n=10,n*10**t}function ck(e,t){let r=Number.isFinite(e?.min)?e.min:0,n=Number.isFinite(e?.max)?e.max:1,a=Math.max(n-r,1),i=Array.isArray(t)?t.map(w=>Number(w?.axisMin)).find(w=>Number.isFinite(w)):Number.NaN,l=Array.isArray(t)?t.map(w=>Number(w?.axisMax)).find(w=>Number.isFinite(w)):Number.NaN,c=Array.isArray(t)?t.map(w=>Number(w?.axisTickStep)).find(w=>Number.isFinite(w)&&w>0):Number.NaN,u=Math.max(1,Number.isFinite(c)?c:lk(a/4)),d=[];if(Number.isFinite(i)||Number.isFinite(l)){let w=Number.isFinite(i)?i:0,f=Number.isFinite(l)?l:Math.ceil(n/u)*u,m=Math.floor(w/u)*u,p=Math.ceil(f/u)*u;for(let b=m;b<=p+u*.5;b+=u)d.push(b)}else{let w=a/u,f=w<=1.8?3:w<=4.25?5:7,m=Math.floor(f/2),p=(r+n)/2,b=Math.round(p/u)*u;for(let S=-m;S<=m;S+=1)d.push(b+S*u)}let g=d[0],y=d[d.length-1];return{ticks:d,axisMin:g,axisMax:y,axisDecimals:0}}function cf(e,t,r={}){let n=Number(r.windowHours),a=Number.isFinite(n)?n:to(),i=sf(a),l=640,c=220,u=46,d=18,g=18,y=34,w=l-u-d,f=c-g-y,m=e[e.length-1],p=!!r.mockData,b=p?i:Number.isFinite(o.trendHistoryNowMs)?o.trendHistoryNowMs:m?m.t:0,S=p?0:b-i,q=Math.max(b-S,1),E=q,k=sk(e,t),$=k.min===k.max?{min:k.min-1,max:k.max+1}:{min:k.min-Math.max((k.max-k.min)*.12,1),max:k.max+Math.max((k.max-k.min)*.12,1)},P=ck(k,t),N=_=>u+(_-S)/q*w,V=_=>{if(!Number.isFinite(_))return Number.NaN;let Z=(_-$.min)/Math.max($.max-$.min,1);return g+(1-Math.min(1,Math.max(0,Z)))*f},A=[0,.5,1].map(_=>u+w*_),F=P.ticks.map(_=>V(_)),H=P.ticks.map((_,Z)=>({x:u-10,y:F[Z],text:z(_,P.axisDecimals)})),L=e.map(_=>{let Z=N(_.t),I=t.map(j=>{let ee=Zn(j,_);return Number.isFinite(ee)?{seriesId:j.id||j.sampleKey||j.label,tone:j.tone,label:j.label,decimals:j.decimals,unit:j.unit,value:ee,x:Z,y:V(ee)}:null});return{sample:_,x:Z,values:I}}),D=t.flatMap(_=>{let Z=[],I=[];return e.forEach(j=>{let ee=Zn(_,j);if(!Number.isFinite(ee)){I.length&&(Z.push(I),I=[]);return}I.push({x:N(j.t),y:V(ee)})}),I.length&&Z.push(I),Z.map(j=>j.length<2?{tone:_.tone,points:j,path:""}:{tone:_.tone,points:j,path:j.map((ee,Ne)=>`${Ne===0?"M":"L"} ${ee.x.toFixed(1)} ${ee.y.toFixed(1)}`).join(" ")})});return{width:l,height:c,left:u,right:d,top:g,bottom:y,plotWidth:w,plotHeight:f,latest:m,uptimeMs:E,endTime:b,startTime:S,span:q,windowHours:a,range:k,displayRange:$,gridXs:A,gridYs:F,yAxisLabels:H,points:L,tracks:D,series:t}}function Mi(){return he({windowHours:to(),trendSignature:o.trendHistorySignature||"",trendNowMs:Number.isFinite(o.trendHistoryNowMs)?o.trendHistoryNowMs:0,coolingActive:Lt()})}function uk(e){return $c().find(t=>t.id===e)||null}function dk(e,t){if(!e||!Array.isArray(e.points)||e.points.length===0)return-1;let r=0,n=Math.abs(e.points[0].x-t);return e.points.forEach((a,i)=>{let l=Math.abs(a.x-t);l<n&&(r=i,n=l)}),r}function pk(e,t){let r=uf(e,t);return`
    ...
    `}function uf(e,t){if(e?.currentKey&&v(e.currentKey)){let r=R(e.currentKey);if(Number.isFinite(r))return r}return Zn(e,t)}function Mc(e){e&&$c().forEach(t=>{let r=e.querySelector(`[data-oq-trend-card="${t.id}"]`),n=t.samples[t.samples.length-1]||null;r&&t.series.forEach(a=>{let l=r.querySelector(`[data-oq-trend-current="${a.id}"]`)?.querySelector("strong"),c=z(uf(a,n),a.decimals,a.unit);l&&l.textContent!==c&&(l.textContent=c)})})}function mk(e,t,r=!1,n=to()){let a=cf(e,t,{mockData:r,windowHours:n}),i=xc(n),l=pc(a.startTime),c=pc(a.startTime+a.span/2),u=pc(a.endTime),d=a.tracks.flatMap(g=>{if(g.points.length<2){let y=g.points[0];return y?`
          <circle
            cx="${y.x.toFixed(1)}"
            cy="${y.y.toFixed(1)}"
            r="3.8"
    ...
    `}function gk(e){let t=e.samples[e.samples.length-1]||null,r=xc(e.windowHours);return`
      <article class="oq-overview-trendcard oq-overview-trendcard--${s(e.tone)}" data-oq-trend-card="${s(e.id)}" data-render-signature="${s(ik(e))}">
        <div class="oq-overview-trendcard-head">
          ...
          <div class="oq-overview-trendcard-meta">
            <div class="oq-overview-trendcard-latest">
              ${e.series.map(n=>pk(n,t)).join("")}
            </div>
          </div>
        </div>
        ${mk(e.samples,e.series,e.mock,e.windowHours)}
    ...
    `}function hk(){let e=to(),t=Fl();return`
    ...
    `}function wk(){if(!o.root||o.appView!=="diagnosis")return!1;let e=o.root.querySelector(".oq-overview-board");if(!e)return!1;let t=e.querySelector(".oq-overview-trends");return t?(Ye(t,Mi(),Hi()),Mc(e),ea(e),!0):!1}function Hc(e){if(!e)return null;let t=e.__oqTrendHoverNodes;if(t&&t.chart&&t.hoverLayer&&t.hoverPanel)return t;let r=e.querySelector("[data-oq-trend-hover-layer]"),n={chart:e.querySelector(".oq-overview-trend-chart"),hoverLayer:r,hoverPanel:e.querySelector("[data-oq-trend-hover]"),hoverTime:e.querySelector("[data-oq-trend-hover-time]"),hoverNote:e.querySelector("[data-oq-trend-hover-note]"),hoverValues:e.querySelector("[data-oq-trend-hover-values]"),hoverLine:r?r.querySelector(".oq-overview-trend-hover-line"):null,hoverDots:{}};return r&&r.querySelectorAll("[data-oq-trend-hover-dot]").forEach(a=>{n.hoverDots[a.getAttribute("data-oq-trend-hover-dot")]=a}),e.__oqTrendHoverNodes=n,n}function Yg(e,t,r,n=Hc(e)){if(!e||!t||!Array.isArray(t.points)||t.points.length===0)return;let a=Math.max(0,Math.min(t.points.length-1,r)),i=t.points[a];if(!i||!n||!n.chart||!n.hoverLayer||!n.hoverPanel||!n.hoverTime||!n.hoverNote||!n.hoverValues)return;let l=String(a);if(!n.hoverPanel.hidden&&e.dataset.oqTrendHoverIndex===l)return;let c=Gq(i.sample.t,t.endTime);n.hoverPanel.hidden=!1,n.hoverLayer.removeAttribute("hidden"),n.hoverTime.textContent=c.value,n.hoverNote.textContent=c.note,n.hoverLine&&(n.hoverLine.setAttribute("x1",i.x.toFixed(1)),n.hoverLine.setAttribute("x2",i.x.toFixed(1)));let u=[];t.series.forEach(d=>{let g=Zn(d,i.sample),y=d.id||d.sampleKey||d.label,w=n.hoverDots[y];if(!Number.isFinite(g)){w&&w.setAttribute("display","none");return}let f=i.values.find(m=>m.seriesId===y);w&&f&&(w.removeAttribute("display"),w.setAttribute("cx",f.x.toFixed(1)),w.setAttribute("cy",f.y.toFixed(1))),u.push(`
        <div class="oq-overview-trend-hover-row oq-overview-trend-hover-row--${s(d.tone)}">
          <span>${s(d.label)}</span>
          <strong>${s(z(g,d.decimals,d.unit))}</strong>
        </div>
      `)}),n.hoverValues.innerHTML=u.join(""),e.dataset.oqTrendHoverIndex=l}function yk(e){if(!e)return;let t=Hc(e);t?.hoverPanel&&(t.hoverPanel.hidden=!0),t?.hoverLayer&&t.hoverLayer.setAttribute("hidden",""),delete e.dataset.oqTrendHoverIndex}function ea(e=o.root){if(!e)return;e.querySelectorAll("[data-oq-trend-card]").forEach(r=>{let n=r.dataset.renderSignature||"";if(r.__oqTrendBoundSignature===n)return;typeof r.__oqTrendCleanup=="function"&&r.__oqTrendCleanup(),r.__oqTrendHoverNodes=null,r.__oqTrendBoundSignature=n;let a=r.querySelector(".oq-overview-trend-chart");if(!a)return;let i=uk(r.dataset.oqTrendCard);if(!i)return;let l=cf(i.samples,i.series,{mockData:i.mock});r.__oqTrendModel=l;let c=Hc(r),u=0,d=null,g=()=>{let f=d;d=null,u=0;let m=a.getBoundingClientRect();if(!m.width||!m.height)return;let p=Number(f?.clientX);if(!Number.isFinite(p)){Yg(r,l,l.points.length-1,c);return}let S=Math.min(m.width,Math.max(0,p-m.left))/m.width*l.width,q=dk(l,S);Yg(r,l,q,c)},y=f=>{d=f,u||(u=window.requestAnimationFrame(g))},w=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,yk(r)};a.addEventListener("pointermove",y),a.addEventListener("pointerenter",y),a.addEventListener("pointerleave",w),a.addEventListener("focus",y),a.addEventListener("blur",w),a.addEventListener("touchstart",y,{passive:!0}),r.__oqTrendCleanup=()=>{u&&(window.cancelAnimationFrame(u),u=0),d=null,a.removeEventListener("pointermove",y),a.removeEventListener("pointerenter",y),a.removeEventListener("pointerleave",w),a.removeEventListener("focus",y),a.removeEventListener("blur",w),a.removeEventListener("touchstart",y)}})}mo({patchDiagnosisDom:wk});function Lc(e,t,r){let n=Ri(W(t.mode,"Unknown")),a=C(t.defrost),i=po(W(t.failures,"None")),l=_a(i),c=n==="Verwarmen"||n==="Koelen"||a;return{mode:n,defrostActive:a,failures:i,warningFailures:l,running:c,thermalKey:n==="Koelen"?t.cooling:t.heat,schematic:Mk(e,t,r,n,a,l,c)}}function mf(e,t=null){return`<h3>${s(e)}</h3>${t?`<button class="oq-overview-hp-card-action" type="button" data-oq-action="select-hp-layout" data-hp-layout="${s(t.layout)}">${_k(t.layout==="equal"?"minus":"plus")}<span>${s(t.label)}</span></button>`:""}`}function Rc(e,t,r,n){return`<div class="oq-overview-hp-status">${wc(e,t,r,n)}</div>`}function vk(e,t){return t?"ontdooit":e==="Verwarmen"?"verwarmt":e==="Koelen"?"koelt":e==="Stand-by"?"stand-by":"onbekend"}function gf(e){return!Array.isArray(e)||e.length===0?"":`<p class="oq-overview-hp-summary">${s(e.map(t=>`${t.title} ${vk(Ri(W(t.keys.mode,"Unknown")),C(t.keys.defrost))}`).join(", "))}</p>`}function Sk(e){let t=o.entities[e];if(!t)return"Positie: \u2014";let r=R(e);return Number.isNaN(r)?`Positie: ${W(e)}`:`Positie: ${z(r,0,t.uom||"")}`}function qk(e){return v(e)?`Positie: ${C(e)?"Koelen/Defrost":"Verwarmen"}`:"Positie: \u2014"}function Ri(e){let t=String(e||"").trim();return!t||t==="Unknown"?"Onbekend":t==="Standby"?"Stand-by":t==="Heating"?"Verwarmen":t==="Cooling"?"Koelen":t}function Pc(e,t,r,n=!0,a="default"){return`
    ...
    `}function kk(e,t,r){return e==="temperature"?`
        <svg
    ...
    `}function Nc({bind:e,x:t,y:r,width:n,value:a,label:i,ariaLabel:l="",align:c="start"}){let u=l||`${i} temperatuur ${a}`,d=c==="end",g=c==="center",y=g?"middle":d?"end":"start",w=g?t+n/2:d?t+n-2:t+2;return`
      <g
    ...
    `}function Ek({tooltip:e,...t}){return`${Nc(t)}${ta({bind:t.bind,...e})}`}function Tk({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
      ...
      <div class="${s(e.boardClass)}" data-oq-hp-board="${s(e.title)}">
        <div class="oq-hp-tech-shell">
          <div class="oq-hp-tech-visual">
            ...
            ${Object.entries(e.pipes).map(([d,g])=>Pc(d.replace(/[A-Z]/g,y=>`-${y.toLowerCase()}`),g.tone,g.d,g.animated,g.flowVariant)).join("")}
    ...
    `}function ff(e,t,r,n="normal",a=null){if(!v(t.power))return"";let i=Lc(e,t,r),{mode:l,defrostActive:c,running:u,thermalKey:d}=i,g=i.schematic;return o.hpVisualMode==="schematic"?`
        <section class="oq-overview-hp oq-overview-hp--${s(r)} oq-overview-hp--${s(n)}" data-oq-hp-panel="${s(e)}">
          <div class="oq-overview-hp-head">
            ...
            <div class="oq-overview-hp-head-side">
              ${Rc(l,u,g.warningActive,g.failureText)}
            </div>
          </div>
          ${Hk(g)}
        </section>
    ...
    `}function _c(){if(!Oc())return"";let e=Pi();return o.hpVisualMode!=="schematic"?Nk(e):`
      <section class="oq-overview-hp oq-overview-boiler" data-oq-boiler-panel data-render-signature="${s(Ic(e))}">
          ...
          <div class="oq-boiler-card-main">
            ...
            <div class="oq-boiler-mini-schematic">
              <svg class="oq-boiler-mini-svg" viewBox="0 0 420 132" role="img" aria-label="Schematische weergave CV-ketel of boiler">
                ...
                ${Pc("boiler-return","return","M24 88 H166 C180 88 190 80 196 68",!0,"water")}
                ${Pc("boiler-supply","supply","M224 52 C232 40 244 36 258 36 H396",!0,"water")}
                ...
                ${Nc({bind:"boiler-return",x:22,y:70,width:78,value:e.returnTempText,label:"Retour",ariaLabel:`Retour ${e.returnTempText}`,align:"start"})}
                ...
                ${Nc({bind:"boiler-supply",x:320,y:16,width:76,value:e.supplyTempText,label:"Aanvoer",ariaLabel:`Aanvoer ${e.supplyTempText}`,align:"end"})}
    ...
    `}function ra(){let e=typeof ne=="function"?ne():"";return ya.filter(t=>e==="single"&&t.title==="HP2"?!1:v(t.keys.power))}function Fc(e){return!Array.isArray(e)||e.length<2||o.hpVisualMode!=="schematic"?"equal":o.hpLayoutMode==="focus-hp1"||o.hpLayoutMode==="focus-hp2"?o.hpLayoutMode:"equal"}function Ai(e,t,r){return!Array.isArray(t)||t.length<2?"normal":r==="focus-hp1"?e===0?"focus":"muted":r==="focus-hp2"?e===1?"focus":"muted":"normal"}function Dc(e,t,r){return!Array.isArray(t)||t.length<2||o.hpVisualMode!=="schematic"?null:Ai(e,t,r)==="focus"?{layout:"equal",label:"Toon beide"}:{layout:e===0?"focus-hp1":"focus-hp2",label:"Vergroot"}}function hf(e){return!Array.isArray(e)||e.length!==1?Fc(e):Oc()?"equal":"single"}function _k(e="plus"){return`
    ...
    `}function Dk(e,t){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?(Dt(e,bf(t)),!0):(Dt(r,`
      <h3>Warmtepompen</h3>
      ${gf(t)}
    `),n.classList.toggle("is-active",o.hpVisualMode==="schematic"),a.classList.toggle("is-active",o.hpVisualMode==="compact"),!0)}function wf(){let e=yc(),t=ra(),r=Fc(t),n=hf(t),a=bf(t);return`
      <section class="oq-helper-panel oq-helper-panel--flush">
        <div class="oq-overview-board oq-overview-board--${s(o.overviewTheme)}">
          ${hc()}
          ${af(e)}
    ...
    `}function M1(){let e=ct(),t=sa(),r=Xk(),n=$o().filter(g=>!g.quick&&!g.custom),a=e1(),i=t1(a),l=Ef(a.start),c=Ef(a.end),u=o.controlReplayPeriodMenuOpen,d=e==="custom"?"Eigen periode":r.some(g=>g.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function H1(e){let t=Qc(e.severity);return`
    ...
    `}function A1(e,t){let r=Qc(e.severity),n=t&&t.id===e.id,a=_f(e.kind),i=Nf(e);return`
      <button
    ...
    `}function Bf(e){if(!e)return"";let t=Qc(e.severity),r=dr(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=Df(e),l=Nf(e),c=Array.isArray(e.checks)?e.checks:r.checks;return`
      <aside class="oq-working-detail oq-working-detail--${s(t.tone)}">
        <div>
          <span class="oq-working-eyebrow">Geselecteerd</span>
          <h3>${s(e.title)}${Yc(e)}</h3>
          <p>${s(e.summary)}</p>
        </div>
        ...
        <details class="oq-working-support" data-replay-support-item="${s(e.id)}"${o.controlReplaySupportDetailsItemId===e.id?" open":""}>
          ...
          <dl>
            <div><dt>Record</dt><dd>${s(_f(e.kind))}</dd></div>
    ...
    `}function Kf(){let e=mr(),t={HP1:!1,HP2:!1,boiler:!1,cooling:!1},r={HP1:0,HP2:0},n=i=>{let l=String(i||"").toUpperCase();return l==="BOTH"?["HP1","HP2"]:l==="HP1"||l==="HP2"?[l]:[]};return jf(Jc()).filter(i=>i&&!i._oq_hidden).sort((i,l)=>{let c=xo(i),u=xo(l);return(Number.isFinite(c)?c:Number.POSITIVE_INFINITY)-(Number.isFinite(u)?u:Number.POSITIVE_INFINITY)}).forEach(i=>{let l=xo(i);if(!Number.isFinite(l)||l>e.start)return;let c=String(i.event_type||""),u=Number(i._oq_context_cm??i.cm);c==="source_start"?n(i.subject).forEach(d=>{t[d]=!0,r[d]=u}):c==="source_stop"?n(i.subject).forEach(d=>{t[d]=!1,r[d]=0}):c==="boiler_assist_start"?t.boiler=!0:c==="boiler_assist_stop"&&(t.boiler=!1)}),t.cooling=["HP1","HP2"].some(i=>t[i]&&r[i]===5),{...t,sourceModes:r}}function D1(e){if(!e.some(f=>f.rawDecisionEvent))return null;let t=[{label:"HP1",tone:"running",segments:[]},{label:"HP2",tone:"running",segments:[]},{label:"CV-ketel",tone:"assist",segments:[]},{label:"Koeling",tone:"cooling",segments:[]},{label:"Ontdooien",tone:"defrost",segments:[]},{label:"Bescherming",tone:"limited",segments:[]}],r=Object.fromEntries(t.map(f=>[f.label,f])),n=(f,m,p,b,S=.5)=>{if(!r[f]||!Number.isFinite(m))return;let q=Math.max(0,Math.min(1440,Number(m))),E=Number.isFinite(p)?Math.max(q,Math.min(1440,Number(p))):q,k=Math.max(S,(E-q)/1440*100);r[f].segments.push({start:q/1440*100,width:k,tone:b})},a=(f,m,p,b=.5)=>{let S=Ot(m);n(f,S.start,S.end,p,b)},i=[...e].filter(f=>f.rawDecisionEvent).sort((f,m)=>Ot(f).start-Ot(m).start),l={HP1:null,HP2:null,"CV-ketel":null,Koeling:null},c=(f,m)=>{l[f]==null&&(l[f]=m)},u=(f,m,p="running",b=.8)=>l[f]==null?!1:(n(f,l[f],m,p,b),l[f]=null,!0),d=f=>{l.Koeling!=null&&l.HP1==null&&l.HP2==null&&u("Koeling",f,"cooling",.8)},g={},y=Kf();y.HP1&&c("HP1",0),y.HP2&&c("HP2",0),y.boiler&&c("CV-ketel",0),y.cooling&&c("Koeling",0),i.forEach(f=>{let m=Ot(f),p=String(f.realEventType||""),b=String(f.rawDecisionEvent?.subject||"").toUpperCase(),S=Number(f.rawDecisionEvent?._oq_context_cm??f.rawDecisionEvent?.cm),q=[];if((b==="HP1"||b==="BOTH")&&q.push("HP1"),(b==="HP2"||b==="BOTH")&&q.push("HP2"),p==="source_start")q.forEach(k=>c(k,m.start)),S===5&&c("Koeling",m.start);else if(p==="source_stop")q.forEach(k=>{u(k,m.start,"running")||a(k,f,"standby",.55)}),(S===5||l.Koeling!=null)&&d(m.start);else if(p==="topology_change")if(f.rawDecisionEvent?.to==="duo")c("HP1",m.start),c("HP2",m.start);else if(f.rawDecisionEvent?.to==="single"){let k=eu(f.rawDecisionEvent);k?(c(k,m.start),u(k==="HP1"?"HP2":"HP1",m.start,"running",.8)):u("HP2",m.start,"running",.8),d(m.start)}else f.rawDecisionEvent?.to==="idle"&&(u("HP1",m.start,"running",.8),u("HP2",m.start,"running",.8),u("Koeling",m.start,"cooling",.8));else if(p==="boiler_assist_start")c("CV-ketel",m.start);else if(p==="boiler_assist_stop")u("CV-ketel",m.start,"assist",.65)||a("CV-ketel",f,"standby",.65);else if(p==="candidate_blocked"||p==="flow_hold_start")a("Bescherming",f,"limited",.7);else if(p==="flow_hold_clear"){let k=Math.max(1,ia(f.rawDecisionEvent));n("Bescherming",Math.max(0,m.start-k),m.start,"limited",.7),f.rawDecisionEvent?.reason==="flow_postflow"&&u("Koeling",m.start,"cooling",.8)}if(p==="defrost_seen_start")g[b||"SYSTEM"]=m.start;else if(p==="defrost_seen_clear"&&g[b||"SYSTEM"]!=null)n("Ontdooien",g[b||"SYSTEM"],m.start,"defrost",.7),g[b||"SYSTEM"]=null;else if(p==="defrost_seen_clear"&&Number(f.rawDecisionEvent?.duration_s)>0){let k=Math.max(5,ia(f.rawDecisionEvent));n("Ontdooien",Math.max(0,m.start-k),m.start,"defrost",.7)}if(!(p==="candidate_blocked"||p==="flow_hold_start"||p==="flow_hold_clear")&&(f.severity==="limited"||f.severity==="attention"||p==="decision_blocked"||p==="decision_hold")&&a("Bescherming",f,f.severity==="attention"?"assist":"limited",.7),p==="sticky_pump_run"&&a("Bescherming",f,"safe",.6),p==="frost_protection_start")a("Bescherming",f,"limited",.8);else if(p==="frost_protection_clear"){let k=Math.max(1,ia(f.rawDecisionEvent));n("Bescherming",Math.max(0,m.start-k),m.start,"limited",.8)}});let w=Vf();return Object.entries(l).forEach(([f,m])=>{m!=null&&m<=w&&n(f,m,w,f==="CV-ketel"?"assist":f==="Koeling"?"cooling":"running",.8)}),Object.values(g).forEach(f=>{f!=null&&n("Ontdooien",f,Math.min(1440,f+7),"defrost",.7)}),t.filter(f=>f.segments.length)}function L1(e){let t=D1(e);return t||[]}function O1(e,t){let r=tu(),n=r/1440*100,a=sa(),i=Kc(r,a),l=L1(t),c=l.length?l.map(u=>_1(u.label,u.tone,u.segments)).join(""):Li("Nog geen grafiekdata","De grafiek gebruikt alleen echte beslislog-records. Nieuwe bronwissels, defrosts of begrenzingen verschijnen hier vanzelf.");return`
    ...
    `}function V1(){return te({modalId:"initial-load",titleId:"oq-loading-modal-title",kicker:"OpenQuatt",title:"OpenQuatt laden",backdropClass:"oq-helper-modal-backdrop--loading",modalClass:"oq-helper-modal--reconnect oq-helper-modal--loading",role:"status",ariaLive:"polite",bodyMarkup:`
      ...
      `})}function B1(){return o.appView==="overview"?wf():o.appView==="control"?Gf():o.appView==="energy"?zg():o.appView==="diagnosis"?df():o.appView==="results"?Ug():j1()}function K1(){let e=zl();return`
    ...
    `}function z1(){let e=typeof document<"u"?document.activeElement:null;return!e||typeof e.matches!="function"?null:e.matches("select[data-oq-dev-control]")?e:null}function U1(e){if(!e||o.deferDevControlSelectRender)return;o.deferDevControlSelectRender=!0;let t=()=>{e.removeEventListener("blur",t),e.removeEventListener("change",t),o.deferDevControlSelectRender=!1,window.setTimeout(()=>Qf(),0)};e.addEventListener("blur",t,{once:!0}),e.addEventListener("change",t,{once:!0})}function G1(){if(o.nativeOpen||o.appView!=="settings"||o.renderedAppView!=="settings"||o.renderedSettingsGroup!==o.settingsGroup)return null;let e=document.scrollingElement||document.documentElement,t=Number(window.scrollY||e?.scrollTop||0);return!Number.isFinite(t)||t<=0?null:{group:o.settingsGroup,left:Number(window.scrollX||e?.scrollLeft||0),top:t}}function Q1(e){if(!e)return;let t=(o.settingsPageScrollRestoreToken||0)+1;o.settingsPageScrollRestoreToken=t;let r=()=>{if(t!==o.settingsPageScrollRestoreToken||o.nativeOpen||o.appView!=="settings"||o.settingsGroup!==e.group)return;let n=document.scrollingElement||document.documentElement;if(!n)return;let a=Math.max(0,n.scrollHeight-n.clientHeight),i=Math.min(e.top,a);window.scrollTo({left:e.left,top:i,behavior:"auto"})};window.requestAnimationFrame(()=>{r(),window.requestAnimationFrame(r),window.setTimeout(r,80)})}function Qf(){if(!o.root)return;let e=z1();if(e){U1(e);return}let t=F1(),r=o.systemModal==="webserver-logs"?Yt():null,n=o.systemModal==="cm100-commissioning"?kp():null,a=String(o.systemModal||"").startsWith("service-task-")?Tp():null,i=o.systemModal==="history-storage"?xp():null,l=o.quickStartModalOpen?qg():null,c=G1();if(o.nativeOpen){o.root.innerHTML=`
        ${Ul()}
        ${Xm()}
      `,Ss(o.root),o.renderedAppView="native",o.renderedSettingsGroup="",o.settingsRenderSignature="",o.headerRenderSignature=hi(),Sr({mqttSensorsModalRenderSignature:""}),yn(),Dn(),fl(),ds(),us(),Jt(r),pl(n),ml(a),gl(i),ac(l);return}let u=B1(),d=o.loadingEntities?`${u}${V1()}`:u,g=o.appView==="overview"||o.appView==="control"||o.appView==="energy"||o.appView==="diagnosis"||o.appView==="results";o.root.innerHTML=`
      ${Ul()}
