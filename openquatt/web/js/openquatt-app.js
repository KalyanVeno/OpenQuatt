        ...
        <article class="oq-settings-mqtt-sensor-row${m?" is-open":""}${d?"":" is-disabled"}">
          ...
          >
            ...
            <span class="oq-settings-mqtt-sensor-value">
              ${d?yb(l):'<span class="oq-settings-mqtt-sensor-value-missing">\u2014</span>'}
            </span>
...
`)),r.toString()}function Ws(e,t){let r=[],n=[],a=[],i="";return e.forEach(l=>{let c=Xb(l);if(!c)return;let p=[...a,c],u=Ld(p,t);if(a.length&&u.length>Kb){r.push({keys:n,body:i}),n=[l],a=[c],i=Ld(a,t);return}n=[...n,l],a=p,i=u}),a.length&&r.push({keys:n,body:i}),r}async function ev(e){let t=Is(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body};if(typeof AbortController=="function"){let a=new AbortController,i=window.setTimeout(()=>a.abort(),t);try{let l=await fetch(Rd,{...r,signal:a.signal});if(!l.ok)throw new Error(`bulk entities HTTP ${l.status}`);return l.json()}catch(l){throw a.signal.aborted?new Error(`bulk entities request timed out after ${t}ms`):l}finally{window.clearTimeout(i)}}let n=await fetch(Rd,r);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function tv(){let e=Is(),t={cache:"no-store",headers:{"Cache-Control":"no-store"}};if(typeof AbortController=="function"){let n=new AbortController,a=window.setTimeout(()=>n.abort(),e);try{let i=await fetch(Pd,{...t,signal:n.signal});if(!i.ok)throw new Error(`service status HTTP ${i.status}`);return i.json()}catch(i){throw n.signal.aborted?new Error(`service status request timed out after ${e}ms`):i}finally{window.clearTimeout(a)}}let r=await fetch(Pd,t);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}async function ov(){let e=Is(),t={cache:"no-store",headers:{"Cache-Control":"no-store"}};if(typeof AbortController=="function"){let n=new AbortController,a=window.setTimeout(()=>n.abort(),e);try{let i=await fetch(Nd,{...t,signal:n.signal});if(!i.ok)throw new Error(`decision log HTTP ${i.status}`);return i.json()}catch(i){throw n.signal.aborted?new Error(`decision log request timed out after ${e}ms`):i}finally{window.clearTimeout(a)}}let r=await fetch(Nd,t);if(!r.ok)throw new Error(`decision log HTTP ${r.status}`);return r.json()}function rv(e={}){let t=Array.isArray(e.events)?e.events:[],r=Array.isArray(e.buckets)?e.buckets:[],n=t[0]||{},a=t[t.length-1]||{},i=r[r.length-1]||{};return[e?.meta?.event_count??t.length,e?.meta?.dropped_count??0,t.length,n.seq??"",a.seq??"",a.uptime_s??"",e?.meta?.boot_epoch_s??"",n.epoch_s??"",a.epoch_s??"",r.length,i.hour_start_uptime_s??"",i.attention_count??""].join("|")}async function nv(e={}){if(o.appView!=="control")return!1;let t=e.force===!0,r=Date.now();if(!t&&o.decisionLogFetchPromise)return o.decisionLogFetchPromise;if(!t&&(o.decisionLog||o.decisionLogError)&&r-Number(o.decisionLogLastFetchAt||0)<zb)return!1;o.decisionLogFetchPromise=(async()=>{let n=await ov(),a=Array.isArray(n?.events)?n.events:[];if(!n?.ok||!Array.isArray(n?.events))throw new Error("decision log response mist events");let i=rv(n),l=o.decisionLogError!==""||o.decisionLogSignature!==i||!o.decisionLog;return o.decisionLog={...n,events:a,buckets:Array.isArray(n.buckets)?n.buckets:[]},o.decisionLogError="",o.decisionLogSignature=i,o.decisionLogLastFetchAt=Date.now(),l})();try{return await o.decisionLogFetchPromise}catch(n){let a=n.message||String(n),i=!!o.decisionLog||o.decisionLogError!==a;return o.decisionLog=null,o.decisionLogError=a,o.decisionLogSignature="",o.decisionLogLastFetchAt=Date.now(),i}finally{o.decisionLogFetchPromise=null}}function av(e={}){let t=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(t).forEach(([r,n])=>{Gr.has(r)&&(o.optionalMissingEntities&&delete o.optionalMissingEntities[r],o.entities[r]=Ud(r,o.entities[r],n))})}async function iv(e){if(!e.length)return{ok:!0,message:""};try{let t=await tv();return av(t),o.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(t){return{ok:!1,message:t.message||String(t)}}}async function V(e,t="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(v=>a||Gr.has(v)||!Jb(v,n));if(!i.length)return;let l=i.filter(v=>Gr.has(v)),c=i.filter(v=>!Gr.has(v)),u=(await iv(l)).ok?c:[...c,...l];if(!u.length){Od(),dn(It()),o.busyAction||(Dd(),o.controlError="");return}let d=Number(r.concurrency),b=Number.isFinite(d)&&d>0?Math.floor(d):Lt,y=Ws(u,t),h=Math.max(1,Math.min(b,Lt)),g=[];for(let v=0;v<y.length;v+=h){let S=y.slice(v,v+h),q=await Promise.allSettled(S.map(async C=>({chunk:C,payload:await ev(C)})));g.push(...q)}g.some(v=>v.status==="fulfilled")&&(o.lastEntityResponseAt=Date.now());let m="";g.forEach(v=>{if(v.status!=="fulfilled"){let $=v.reason.message||String(v.reason);m||(m=$);return}let{chunk:S,payload:q}=v.value,C=q?.entities&&typeof q.entities=="object"?q.entities:{},k=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach($=>{if(Object.prototype.hasOwnProperty.call(C,$)){o.optionalMissingEntities&&delete o.optionalMissingEntities[$],o.entities[$]=Ud($,o.entities[$],C[$]);return}let N=O[$];N?.optional?k.has($)&&Zb($,n):m||(m=`${N?.name||$} ontbreekt in bulk response`)})}),Od(),dn(It()),m?(zd(m),o.deviceReconnectMode?o.controlError="":o.controlError=`Niet alle helpervelden konden worden ververst. ${m}`):o.busyAction||(Dd(),o.controlError="")}function Ud(e,t={},r={}){let n={...t||{},...r||{}};return O[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(t?.state??"").trim()&&(n.state=t.state),!String(r.value??"").trim()&&String(t?.value??"").trim()&&(n.value=t.value),!Array.isArray(r.option)&&Array.isArray(t?.option)&&(n.option=t.option),!Array.isArray(r.options)&&Array.isArray(t?.options)&&(n.options=t.options)),n}function Od(){o.complete=fa(),o.stage=o.complete===!0?"Gereed":o.complete===!1?"Quick Start":"Laden...",o.summary=Su(),o.appView==="diagnosis"&&!dt()&&pt(no(),{syncMode:"replace",forceSync:!0}),o.appView||pt(cn()||no(),{syncMode:"replace",forceSync:!0})}function sv(){let e=Ds();return o.appView==="settings"?[...new Set([...e,...Ls()])]:o.appView==="energy"||o.appView==="results"?pn(o.appView,{forceFast:!0}):o.appView==="overview"||o.appView==="diagnosis"?pn(o.appView,{forceFast:!0}):[...new Set(e)]}function lv(e=[]){let t=new Set(e);return(o.appView==="settings"?Ls():o.appView==="overview"||o.appView==="control"||o.appView==="diagnosis"?[...new Set([...pn(o.appView,{includeBulk:!0}),..._t])]:o.appView==="energy"||o.appView==="results"?[...new Set([...pn(o.appView,{forceFast:!0}),..._t])]:[...new Set(["setupComplete","strategy",...Ze])]).filter(n=>!t.has(n))}function _d(e=Date.now(),t={}){return t.forceBulk===!0?!0:e-Number(o.lastBulkEntitySyncAt||0)>=cu}async function ue(e={}){if(o.nativeOpen||o.loadingEntities||o.draggingCurveKey||o.busyAction||o.settingsInteractionLock)return;if(o.updateInstallBusy||wa()){o.pendingEntitySyncOptions=null;return}if(o.focusedField&&o.appView!=="settings")return;if(o.entitySyncInFlight){Fb(e);return}let t=Date.now();if(document.hidden&&t-Number(o.lastEntitySyncAttemptAt||0)<Xr)return;let r=o.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="control"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,c=!l&&!n&&_d(t,e),u=t-Number(o.lastStaticEntitySyncAt||0)>=uu||o.updateInstallBusy||o.updateInstallPhaseHint?_t:[],d=o.quickStartModalOpen&&o.currentStep==="flow-source"?ut:[],b=o.quickStartModalOpen&&o.currentStep==="thermostat-source"?xo:[],y=ks()?io():[],h=n?[...Ki,...Ze,"setupComplete",...u]:i?[...pn(a,{forceFast:l,includeBulk:c}),...u]:r==="settings"?[...new Set([...Ls(),...y,...u])]:c?["setupComplete",...u,...Ze,"strategy",...To,...Je,...ne()?ge.map(g=>g.key):qt]:["setupComplete",...Ze,"strategy",...u];o.entitySyncInFlight=!0,o.lastEntitySyncAttemptAt=t;try{let g=o.deviceReconnectMode,m=Qb(t,e)?await Yb():{ok:!0,message:""};if(!m.ok){zd(m.message),n||f();return}if(await V([...new Set([...h,...d,...b])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?Ot:Lt}),o.lastFastEntitySyncAt=Date.now(),c&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&(o.lastBulkEntitySyncAt=o.lastFastEntitySyncAt),u.length&&(o.lastStaticEntitySyncAt=o.lastFastEntitySyncAt),n)return;i&&!o.overviewMetadataHydrated&&!o.overviewMetadataHydrating&&Id();let v=g!==o.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="control"||a==="diagnosis"?await Cs():!1,C=S?!1:o.appView==="results"?await qa():!1,k=a==="control"?await nv({force:e.forceDecisionLog===!0}):!1,$=S?!1:ks()?(await Promise.all([ud(),qa({metaOnly:!0})])).some(Boolean):!1,N=S||!Au()?!1:await No(),D=S||!Ru()?!1:await ro(),B=S||!kd()?!1:await so({force:o.systemModal==="mqtt-sensors"}),H=id();if(S&&!o.nativeOpen&&Bd(Os(a)),v){f();return}if(q&&o.appView==="diagnosis"&&!o.root?.querySelector(".oq-overview-trends")){f();return}if(C&&o.appView==="results"&&!o.root?.querySelector(".oq-energy-history")){f();return}if(k&&o.appView==="control"){Es()||f();return}if($&&o.appView==="settings"){f();return}if(N&&(o.systemModal==="login"||Ft())){f();return}if(D&&o.appView==="settings"){f();return}if(o.systemModal==="mqtt-sensors"){Ca()!==o.mqttSensorsModalRenderSignature&&f();return}if(B&&o.appView==="settings"){f();return}let U=!!(o.updateModalOpen||o.systemModal||o.interfacePanelOpen);if(H!==o.headerRenderSignature)if(U&&ys())o.headerRenderSignature=H;else{f();return}else ys();if(o.appView==="settings"&&String(o.systemModal||"").startsWith("service-task-")){rn()!==o.settingsRenderSignature&&f();return}if(U)return;if(o.appView==="settings"){let A=rn();if(A!==o.settingsRenderSignature&&!o.focusedField){f();return}if(!ld()){o.focusedField||f();return}o.focusedField&&(o.settingsRenderSignature=A);return}if(o.appView==="diagnosis"){fd()||f();return}if(o.appView==="control"){Es()||f();return}if(o.appView==="energy"){pd()||f();return}if(o.appView==="results"){md()||f();return}gd()||f()}catch(g){n||(o.controlError=`Helperstatus kon niet worden geladen. ${g.message}`,f())}finally{o.entitySyncInFlight=!1;let g=o.pendingEntitySyncOptions;o.pendingEntitySyncOptions=null,g&&!o.nativeOpen&&window.setTimeout(()=>{ue(g)},0),l&&(a==="overview"||a==="control"||a==="diagnosis")&&!n&&!o.nativeOpen&&!g&&_d(Date.now())&&window.setTimeout(()=>{ue({forceBulk:!0})},Ob)}}function Tt(){return typeof window<"u"&&window.__OQ_DEV_META&&typeof window.__OQ_DEV_META=="object"?window.__OQ_DEV_META:{}}function cv(){let e=String(E("hpGeneration")||"").trim();return e||""}function mn(e){let t=String(e||"").trim().toLowerCase();return t==="single"||t.includes("quatt single")||t.includes("openquatt single")?"single":t==="duo"||t.includes("quatt duo")||t.includes("openquatt duo")?"duo":""}function uv(){if(!Array.isArray(cr))return"";if(cr.some(t=>w(t)))return"duo";let e=o.optionalMissingEntities||{};return cr.every(t=>e[t])?"single":""}function Vs(e){let t=mn(e);return(t==="single"||t==="duo")&&typeof o<"u"&&o&&typeof o=="object"&&(o.lastKnownInstallationTopology=t),t}function dv(){if(typeof o<"u"&&o&&typeof o=="object"){let e=String(o.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function te(){let e=mn(E("installationTopology"));if(e==="single"||e==="duo")return Vs(e);let t=mn(Tt().installation);if(t==="single"||t==="duo")return Vs(t);let r=uv();return r?Vs(r):dv()}function ft(){let e=te(),t=cv();return e==="single"?t?`Quatt Single ${t}`:"Quatt Single":e==="duo"?t?`Quatt Duo ${t}`:"Quatt Duo":t?`Quatt Hybrid ${t}`:"Quatt Hybrid"}function Fe(){return"OpenQuatt"}function Bs(e){let t=String(e||"").trim().toLowerCase();return t==="wifi"||t==="wi-fi"||t.includes("wifi")||t.includes("wi-fi")?"wifi":t==="eth"||t==="ethernet"||t.includes("ethernet")?"eth":""}function et(e=lo()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Fo(e=te()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function gn(){let e=String(E("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Tt().hardwareProfile||e).trim().toLowerCase()}function lo(){return Bs(E("connectionText")||Tt().connection)}function Gd(){let e=lo();return e==="wifi"?"eth":e==="eth"?"wifi":""}function Qd(){let e=te();return e==="single"?"duo":e==="duo"?"single":""}function fr(e=te(),t=lo()){let r=Fo(e),n=gn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${et(t)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${et(t)}`:n==="waveshare"?`Waveshare ${r} ${et(t)}`:`${Fe()} ${r} ${et(t)}`}function Ks(){let e=T("timeValid"),t=String(E("timeNowHhmm")||"").trim();if(t&&t!=="invalid")return t;if(w("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function Ta(){return w("timeValid")&&!T("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${Ks()}`}function Ea(e){if(!Number.isFinite(e)||e<0)return"\u2014";let t=Math.floor(e),r=Math.floor(t/1440),n=Math.floor(t%1440/60),a=t%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function pv(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function mv(e){let t=e?.state;if(t!==""&&t!==null&&t!==void 0){let a=Number(t);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function hr(){let e=mv(o.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=pv(o.entities.uptime);if(n==="d")return Ea(e*1440);if(n==="h")return Ea(e*60);if(n==="s")return Ea(e/60)}let t=String(o.entities.uptimeReadable?.state??o.entities.uptimeReadable?.value??"").trim();if(t&&t.toLowerCase()!=="unknown")return t;let r=Number(Tt().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":Ea((Date.now()-r)/6e4)}function br(){let e=String(o.entities.ipAddress?.state??o.entities.ipAddress?.value??"").trim();if(e)return e;let t=String(Tt().ipAddress||"").trim();return t||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function co({key:e,value:t,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `}function Gs(e,t,r,n=""){if(!w(e))return"";let a=en(E(e));return pe(e,t,r,`<label class="oq-settings-control oq-settings-control--time"><input class="oq-helper-input oq-helper-input--time" type="time" step="60" lang="nl-NL" inputmode="numeric" data-oq-field="${s(e)}" value="${s(a)}" ${o.loadingEntities?"disabled":""}><span class="oq-settings-time-icon" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><circle cx="10" cy="10" r="6.5" fill="none" stroke="currentColor" stroke-width="1.6" /><path d="M10 6.2 V10 L12.9 11.8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" /></svg></span></label>`,n||"oq-settings-field--time")}function oe(e,t,r,n,a=""){return`<section class="oq-settings-section"><div class="oq-settings-section-head"><div class="oq-settings-section-head-meta"><p class="oq-helper-label">${s(e)}</p>${a?`<div class="oq-settings-section-head-meta-badge">${a}</div>`:""}</div><h3>${s(t)}</h3><p>${s(r)}</p></div>${n}</section>`}var Xd=250;function ep(){if(typeof window>"u")return[];let e=window.__OQ_DEV_WEBSERVER_LOGS__,t=typeof e=="function"?e():e;return Array.isArray(t)?t.map(r=>String(r||"")).filter(r=>r.trim()!==""):[]}function yr(){return typeof window>"u"?!1:ep().length>0}function fv(){return`${Oe()}/events`}function hv(){return`${Oe()}/openquatt/logs/recent`}function Ma(){let e=o.entities?.webServerLogHistoryEnabled;if(!e)return!0;if(typeof e.value=="boolean")return e.value;let t=String(e.state??e.value??"").toLowerCase();return t==="on"||t==="true"||t==="1"}function Ha(){return o.nativeOpen?"Niet beschikbaar":yr()?"Voorbeeld":o.webServerLogEnabled===!1?"Niet beschikbaar":"Beschikbaar"}function bv(e){let t=Math.max(0,Math.floor(Number(e)/1e3)),r=Math.floor(t/3600),n=Math.floor(t%3600/60),a=t%60;return`${String(r).padStart(2,"0")}:${String(n).padStart(2,"0")}:${String(a).padStart(2,"0")}`}function tp(e){let t=Number(e)||0;if(t>9466848e5){let r=e instanceof Date?e:new Date(t),n={day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"};try{return new Intl.DateTimeFormat("nl-NL",n).format(r)}catch{return r.toLocaleString("nl-NL",n)}}return bv(t)}function vv(e){let t=Number(e)||0;if(t>9466848e5)return new Date(t).toLocaleString("nl-NL",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"});let r=Math.max(0,Math.floor(t/1e3)),n=Math.floor(r/3600),a=Math.floor(r%3600/60),i=r%60;return`Sinds opstart: ${n}u ${a}m ${i}s`}function yv(){return o.nativeOpen?"Niet beschikbaar":yr()?Ma()?"Voorbeeld buffer aan":"Voorbeeld buffer uit":Ma()?"Buffer aan":"Buffer uit"}function wv(){return Ma()?"Slaat de laatste firmwarelogs tijdelijk op in RAM. De viewer leest die buffer bij openen en blijft daarna live /events volgen.":"Geen tijdelijke buffer in RAM. De viewer toont alleen live /events."}function Qs(){return o.entities?.debugLevel||null}function op(e=Qs()){let t=Array.isArray(e?.option)?e.option:Array.isArray(e?.options)?e.options:[];return t.length?t:["NONE","ERROR","WARN","INFO","CONFIG","DEBUG"]}function Sv(e=Qs()){let t=String(e?.value??e?.state??"").trim(),r=op(e);return r.includes(t)?t:r.includes("INFO")?"INFO":r[0]||""}function rp(e,t=null){if(!e||!t)return!1;let r=Number(e.seq),n=Number(t.seq);if(Number.isFinite(r)&&Number.isFinite(n)&&r===n)return!0;let a=String(e.raw??e.text??"").trim(),i=String(t.raw??t.text??"").trim();if(!a||a!==i)return!1;let l=Number(e.receivedAt??e.ts??0),c=Number(t.receivedAt??t.ts??0);return Math.abs(l-c)<=2e3}function qv(e,t){let r=Number(e.receivedAt??e.ts??0),n=Number(t.receivedAt??t.ts??0);if(r!==n)return r-n;let a=Number(e.seq??0),i=Number(t.seq??0);return a!==i?a-i:String(e.raw??"").localeCompare(String(t.raw??""))}function np(e,{prepend:t=!1}={}){if(!Array.isArray(e)||e.length===0)return;let r=t?[...e,...o.webServerLogEntries]:[...o.webServerLogEntries,...e];r.sort(qv);let n=[];for(let a of r){let i=n[n.length-1]||null;rp(a,i)||n.push(a)}o.webServerLogEntries=n.slice(-Xd)}function Ys(e,t={}){let r=jv(e).trimEnd(),n=Number(t.receivedAt),a=Number(t.seq);return{raw:e,text:r,tone:Wv(e),receivedAt:Number.isFinite(n)?n:Date.now(),seq:Number.isFinite(a)?a:void 0}}function kv(e,t){let n=Math.max(0,t-e-1)*9e4;return Date.now()-n}function Cv(){let e=ep(),t=e.length;return e.map((r,n)=>Ys(r,{receivedAt:kv(n,t),seq:n+1}))}function Ev(){let e=Aa();e&&(e.scrollTop=e.scrollHeight)}function Bt(){let e=Aa();return e?{scrollHeight:e.scrollHeight,scrollTop:e.scrollTop,stickToBottom:Ra(e)}:null}function Tv(e){if(!e)return;let t=Aa();if(!t)return;if(e.stickToBottom){t.scrollTop=t.scrollHeight;return}let r=e.scrollTop+(t.scrollHeight-e.scrollHeight);t.scrollTop=Math.max(0,r)}function Kt(e,t=!0){if(!e)return;let r=Number(o.webServerLogScrollRestoreToken||0)+1;o.webServerLogScrollRestoreToken=r;let n=()=>{o.webServerLogScrollRestoreToken!==r||o.systemModal!=="webserver-logs"||Tv(e)};if(t){window.requestAnimationFrame(n);return}n()}function ap(){return o.root?o.root.querySelector("[data-oq-cm100-commissioning-scroller]"):null}function ip(){let e=ap();return e?{scrollHeight:e.scrollHeight,scrollTop:e.scrollTop,stickToBottom:Ra(e)}:null}function xv(e){if(!e)return;let t=ap();if(!t)return;if(e.stickToBottom){t.scrollTop=t.scrollHeight;return}let r=e.scrollTop+(t.scrollHeight-e.scrollHeight);t.scrollTop=Math.max(0,r)}function Js(e,t=!0){if(!e)return;let r=Number(o.cm100CommissioningScrollRestoreToken||0)+1;o.cm100CommissioningScrollRestoreToken=r;let n=()=>{o.cm100CommissioningScrollRestoreToken!==r||o.systemModal!=="cm100-commissioning"||xv(e)};if(t){window.requestAnimationFrame(n);return}n()}function sp(){return o.root?o.root.querySelector("[data-oq-service-task-scroller]"):null}function lp(){let e=sp();return e?{scrollTop:e.scrollTop}:null}function $v(e){if(!e)return;let t=sp();t&&(t.scrollTop=Math.max(0,e.scrollTop))}function Zs(e,t=!0){if(!e)return;let r=Number(o.serviceTaskModalScrollRestoreToken||0)+1;o.serviceTaskModalScrollRestoreToken=r;let n=()=>{o.serviceTaskModalScrollRestoreToken!==r||!String(o.systemModal||"").startsWith("service-task-")||$v(e)};if(t){window.requestAnimationFrame(n);return}n()}function cp(){return o.root?o.root.querySelector("[data-oq-history-storage-scroller]"):null}function up(){let e=cp();return e?{scrollTop:e.scrollTop}:null}function Mv(e){if(!e)return;let t=cp();t&&(t.scrollTop=Math.max(0,e.scrollTop))}function Xs(e,t=!0){if(!e)return;let r=Number(o.historyStorageModalScrollRestoreToken||0)+1;o.historyStorageModalScrollRestoreToken=r;let n=()=>{o.historyStorageModalScrollRestoreToken!==r||o.systemModal!=="history-storage"||Mv(e)};if(t){window.requestAnimationFrame(n);return}n()}async function wr(e={}){if(o.nativeOpen||typeof window.fetch!="function")return;let t=e.scrollState||Bt(),r=Number(o.webServerLogHistoryRequestToken||0)+1;o.webServerLogHistoryRequestToken=r,o.webServerLogHistoryLoading=!0,o.webServerLogHistoryError="";try{let n=await window.fetch(hv(),{headers:{"Cache-Control":"no-store"}});if(!n.ok)throw new Error(`HTTP ${n.status}`);let a=await n.json();if(o.systemModal!=="webserver-logs"||o.webServerLogHistoryRequestToken!==r)return;let i=Av(a);o.webServerLogHistoryLoaded=!0,i.length>0&&(np(i,{prepend:!0}),o.webServerLogRecentTail=i.slice(-4).map(l=>String(l.raw??l.text??"")),o.webServerLogRecentAnchorAt=Date.now())}catch(n){o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryError=n instanceof Error?n.message:"Recente logs konden niet worden opgehaald.")}finally{o.webServerLogHistoryRequestToken===r&&(o.webServerLogHistoryLoading=!1),o.systemModal==="webserver-logs"&&o.webServerLogHistoryRequestToken===r&&(f(),Kt(t))}}function Hv(e,t=0){if(!e||typeof e!="object")return null;let r=String(e.raw??"").trim()||String(e.message??"").trim();return r?Ys(r,{receivedAt:Number(e.ts??e.timestamp_ms??e.receivedAt??Date.now()),seq:Number(e.seq??t)}):null}function Av(e){return!e||typeof e!="object"?[]:e.enabled===!1?[]:(Array.isArray(e.entries)?e.entries:[]).map((r,n)=>Hv(r,n+1)).filter(r=>r!==null)}function Rv(e){if(!e||!Array.isArray(o.webServerLogRecentTail)||o.webServerLogRecentTail.length===0||Date.now()-Number(o.webServerLogRecentAnchorAt||0)>2500)return!1;let r=String(e.raw??e.text??"").trim();return r?o.webServerLogRecentTail.includes(r):!1}function Pv(e,t=o.webServerLogEntries){return!e||!Array.isArray(t)||t.length===0?!1:t.some(r=>rp(e,r))}function dp(){yr()&&o.webServerLogEntries.length===0&&(o.webServerLogEntries=Cv()),o.webServerLogCopyMessage="",o.webServerLogCopyError="",o.systemModal="webserver-logs",f(),V(["webServerLogHistoryEnabled","debugLevel"],"all",{forceFast:!0}).then(()=>{if(o.systemModal!=="webserver-logs")return;let e=Bt();f(),Kt(e)}),Ev(),(!o.webServerLogHistoryLoaded||o.webServerLogEntries.length===0)&&wr()}function Vo(){o.webServerLogEntries=[],o.webServerLogError="",o.webServerLogHistoryError="",o.webServerLogHistoryLoading=!1,o.webServerLogHistoryLoaded=!1,o.webServerLogScrollRestoreToken=Number(o.webServerLogScrollRestoreToken||0)+1,o.webServerLogCopyMessage="",o.webServerLogCopyError="",o.webServerLogHistoryRequestToken+=1,o.webServerLogRecentTail=[],o.webServerLogRecentAnchorAt=0,o.systemModal==="webserver-logs"&&f()}function Nv(){let e=Bt();xt(),o.webServerLogEnabled=null,o.webServerLogConnected=!1,Vo(),o.systemModal==="webserver-logs"&&wr({scrollState:e})}function el(){if(yr()){xt();return}if(!(o.mounted&&!o.nativeOpen&&o.systemModal==="webserver-logs")){xt();return}if(o.webServerLogEnabled===!1){xt();return}o.webServerLogSource||Dv()}function Dv(){if(yr()){o.webServerLogEnabled=!0,o.webServerLogConnected=!1,o.webServerLogError="",f();return}if(typeof window.EventSource!="function"){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="Deze browser ondersteunt geen live logstream.",f();return}try{let e=new window.EventSource(fv());o.webServerLogSource=e,e.addEventListener("open",Lv),e.addEventListener("ping",Ov),e.addEventListener("log",Fv),e.onerror=_v}catch(e){o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError=e instanceof Error?e.message:"De live logstream kon niet worden geopend.",xt(),f()}}function xt(){let e=o.webServerLogSource;if(e)try{e.close()}catch{}o.webServerLogSource=null,o.webServerLogConnected=!1}function Lv(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Bt():null;o.webServerLogEnabled=!0,o.webServerLogConnected=!0,o.webServerLogError="",f(),Kt(e)}hd({clearOutput:Vo,closeStream:xt,resetRecoveryState:Nv});function Ov(){if(!o.webServerLogSource||o.nativeOpen)return;let e=o.systemModal==="webserver-logs"?Bt():null;o.webServerLogEnabled=!0,o.webServerLogConnected||(o.webServerLogConnected=!0,o.webServerLogError="",f(),Kt(e))}function _v(){if(!o.webServerLogSource)return;let e=o.systemModal==="webserver-logs"?Bt():null;o.webServerLogEnabled=!1,o.webServerLogConnected=!1,o.webServerLogError="De live logstream kon niet worden geopend.",xt(),f(),Kt(e)}function Fv(e){if(!o.webServerLogSource||!e||typeof e.data!="string")return;let t=Bt(),r=Iv(e.data);if(!r)return;let n=r.split(/\r?\n/).filter(p=>p.trim()!=="");if(n.length===0)return;let i=n.map(p=>Ys(p)).filter(p=>!Rv(p)&&!Pv(p));if(i.length===0)return;np(i);let l=Bv(),c=Aa();Vv(l),Kv(i,l),o.webServerLogEnabled=!0,c&&t&&Kt(t,!1)}function Iv(e){let t=String(e??"").replace(/\r\n/g,`
`).replace(/\r/g,`
...
`).replace(/\r/g,`
`).trimEnd()}catch{}return t}function jv(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function Wv(e){let t=String(e??""),r=Array.from(t.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(c=>Number(c)).filter(c=>Number.isFinite(c));for(let c=l.length-1;c>=0;c-=1){let p=l[c];if(p===31||p===91)return"error";if(p===33||p===93)return"warning";if(p===32||p===92)return"info";if(p===36||p===96||p===34||p===35)return"debug";if(p===37||p===90||p===38||p===97)return"verbose"}}let n=t.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function Vv(e){for(;o.webServerLogEntries.length>Xd;)o.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function Bv(){return o.root?o.root.querySelector("[data-oq-webserver-log-output]"):null}function Aa(){return o.root?o.root.querySelector("[data-oq-webserver-log-scroller]"):null}function Ra(e){return e?e.scrollHeight-e.scrollTop-e.clientHeight<48:!1}function Kv(e,t){if(!(!t||e.length===0)){t.dataset.webServerLogEmpty==="true"&&(t.dataset.webServerLogEmpty="false",t.innerHTML="");for(let r of e)t.insertAdjacentHTML("beforeend",pp(r))}}function pp(e){let t=tp(e.receivedAt),r=vv(e.receivedAt);return`
  ...
  `}function zv(e=o.webServerLogEntries){return e.length?e.map(t=>pp(t)).join(""):`
  ...
  `}function Qv(){let e=Qs();if(!e)return"";let t=op(e),r=Sv(e),n=o.loadingEntities||o.busyAction==="save-debugLevel";return`
...
`)}async function mp(){let e=Yv();if(o.webServerLogCopyMessage="",o.webServerLogCopyError="",!e){o.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",f();return}try{if(!await eo(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");o.webServerLogCopyMessage=`${o.webServerLogEntries.length} logregel${o.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(t){o.webServerLogCopyError=t instanceof Error?t.message:"Kopi\xEBren naar het klembord is niet gelukt."}o.systemModal==="webserver-logs"&&f()}function gp(){let e=yr();return`
  ...
  `}function Jv(){let e=o.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function bp(e){return Jv().includes(e)}function Na(){let e=gn(),t=te(),r=lo(),n=Gd();return e!=="heatpump_controller_q"||t!=="single"&&t!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:w("firmwareUpdateTarget")&&bp("alternate connection")&&w("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:et(r),targetLabel:et(n),currentBuildLabel:fr(t,r),targetBuildLabel:fr(t,n)}}function Da(){let e=gn(),t=te(),r=Qd(),n=lo(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||t!=="single"&&t!=="duo"||!r||!a.includes(n)?null:{canSwitch:w("firmwareUpdateTarget")&&bp("alternate topology")&&w("installFirmwareUpdateTarget"),currentTopology:t,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Fo(t),targetLabel:Fo(r),currentBuildLabel:fr(t,n),targetBuildLabel:fr(r,n)}}function Bo(e=o.updateTestFirmwarePr){let t=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(t)?t:""}function qr(){let e=gn(),t=te(),r=lo(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||t!=="single"&&t!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${t}-${r}`,l=t==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${et(r)}`}}function hn(e=Bo(),t=qr()){if(!e||!t.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${t.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function go(){if(rt())return"Controleren";let e=zt();if(e)return e.phaseLabel;if(rl())return"Bijgewerkt";if(Sr())return"Bezig";if(Ko())return"Beschikbaar";let t=yp();if(t!==null&&t<=0)return"Actueel";let r=Tt();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||al()?"Actueel":he()?"Nog niet gecontroleerd":"\u2014"}function he(){return o.entities.firmwareUpdate||null}function La(){let e=he();return e?String(e.state??e.value??"").trim().toLowerCase():""}function Zv(){let e=o.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function Oa(){return Zv().toLowerCase()}function vp(){let e=o.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let t=Number(e.value??e.state);return Number.isNaN(t)?Number.NaN:Math.max(0,Math.min(100,t))}function tl(){let e=String(o.updateInstallTargetVersion||"").trim(),t=le();return!e||!t?!1:bn(t,e)>=0}function Xv(e=he()||{}){let t=fo(e),r=le(e);return!t||!r?!1:bn(r,t)>=0}function ol(){return(tl()||Xv())&&!rt()&&!fn()&&!Ko()}function rl(){return(o.updateInstallCompleted||ol())&&!rt()&&!zt()&&!Ko()}function kr(){o.updateInstallBusy=!1,o.updateInstallTargetVersion="",o.updateInstallPhaseHint="",o.updateInstallProgressHint=Number.NaN,o.updateInstallMode="",o.updateInstallTargetConnection="",o.updateInstallTargetTopology="",iy()}function Ae(){o.updateManualUploadFile=null,o.updateManualUploadFileName="",o.updateManualUploadError=""}function ot(e={}){e.clearPr&&(o.updateTestFirmwarePr=""),o.updateTestFirmwareConfirmed=!1,o.updateTestFirmwareError="",o.updateTestFirmwareBuild=null}function ey(){let e=Oa(),t=vp();if(e==="starting"||e==="uploading"||e==="rebooting"){o.updateInstallPhaseHint=e,Number.isNaN(t)||(o.updateInstallProgressHint=e==="rebooting"?Math.max(t,100):t);return}if(o.updateInstallBusy){if(tl()){o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100;return}o.controlNotice.includes("opnieuw is opgestart")&&(o.updateInstallPhaseHint="rebooting",o.updateInstallProgressHint=100)}}function fn(){let e=Oa();return e==="starting"||e==="uploading"||e==="rebooting"}function zt(){ey();let e=Oa(),t=e==="starting"||e==="uploading"||e==="rebooting",r=t?e:o.updateInstallPhaseHint,n=vp(),a=Number.isNaN(o.updateInstallProgressHint)?0:Math.round(o.updateInstallProgressHint),i=t&&!Number.isNaN(n)?Math.round(n):a;return!fn()&&!o.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:o.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":o.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":o.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Fe()} gedownload en ge\xEFnstalleerd.`:o.updateInstallMode==="connection-switch"?`De ${et(o.updateInstallTargetConnection)}-build wordt nu naar ${Fe()} verzonden.`:o.updateInstallMode==="topology-switch"?`De ${fr(o.updateInstallTargetTopology,o.updateInstallTargetConnection)}-build wordt nu naar ${Fe()} verzonden.`:`Firmware wordt nu naar ${Fe()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:o.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Fe()}.`:o.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${et(o.updateInstallTargetConnection)} is gestart.`:o.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${Fo(o.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Fe()}.`}}function fo(e=he()||{}){let t=String(e.latest_version||"").trim();if(t)return t;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function le(e=he()||{}){let t=String(o.entities.projectVersionText?.state||o.entities.projectVersionText?.value||"").trim();return t||String(e.current_version||"").trim()}function ty(e=he()||{}){let t=le(e),r=String(e.current_version||"").trim();return!t||!r||t===r?!1:bn(t,r)>0}function nl(e=he()||{},t=mo()){let r=String(t||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=fo(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function fp(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function bn(e,t){let r=fp(e),n=fp(t);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function Sr(){if(ol())return!1;let e=La();return o.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function rt(){let e=La();return o.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Ko(){let e=La();if(!nl())return!1;let t=yp();return t!==null?t>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Tt().updateAvailable===!0}function al(){let e=La();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||ty()}function il(){let e=he()||{},t=le(e)||"\u2014",r=nl(e)?fo(e):"",n=r?bn(r,t):null;return!rt()&&n!==null&&n<=0&&(r=""),{current:t,latest:r||"\u2014"}}function yp(){let{current:e,latest:t}=il();return e==="\u2014"||t==="\u2014"?null:bn(t,e)}function wp(e=mo()){let t=String(e||"").trim().toLowerCase();return Ni[t]||Ni.main}function oy(){let e=String((he()||{}).release_url||"").trim(),t=wp();return!e||t.includes("/dev-latest")&&!e.includes("/dev-latest")||!t.includes("/dev-latest")&&e.includes("/dev-latest")?t:e}function ry(){return Fe()}function mo(){return String(E("firmwareUpdateChannel")||o.entities.releaseChannelText?.state||o.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function ny(){return il().latest!=="\u2014"}function hp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function ay(e,t=he()||{}){let r=hp(e);return r?hp(`${t.title||""} ${t.summary||""}`).includes(r):!0}function Sp(e){return new Promise(t=>window.setTimeout(t,e))}function vn(e=Ao){let r=Date.now()+e;o.firmwareOtaQuietUntil=Math.max(Number(o.firmwareOtaQuietUntil||0),r),o.pendingEntitySyncOptions=null,Uu(),typeof xt=="function"&&xt(),o.firmwareOtaQuietTimer&&window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=window.setTimeout(()=>{o.firmwareOtaQuietTimer=null,o.firmwareOtaQuietUntil=0,!o.updateInstallBusy&&!o.nativeOpen&&ss()},e)}function iy(){o.firmwareOtaQuietTimer&&(window.clearTimeout(o.firmwareOtaQuietTimer),o.firmwareOtaQuietTimer=null),o.firmwareOtaQuietUntil=0,o.nativeOpen||ss()}function qp(){return o.deviceReconnectMode?`
    ...
    `}function ly(e,t,r,n){if(!kp())return"";let i=!!(zt()||o.updateInstallBusy||rt());return`
    ...
    `}function py(){if(!o.updateManualUploadOpen)return"";let t=!!(zt()||o.updateInstallBusy||rt()),r=String(o.updateManualUploadFileName||o.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Cp(){if(!o.updateModalOpen)return"";let e=he(),t=o.entities.firmwareUpdateChannel||null,{current:r,latest:n}=il(),a=rt(),i=Sr(),l=Ko(),c=sy(),p=zt(),u=rl(),d=oy(),b=u?"Firmware-update afgerond":p||i?"Firmware-update bezig":a?"Controleren op firmware-update":ry(),y=t?Array.isArray(t.option)?t.option:Array.isArray(t.options)?t.options:[]:[],h=Na(),g=Da(),m=!!(h&&!u),v=!!(g&&!u);return`
  ...
  `}function Wp(e){o.overviewTheme=e==="dark"?"dark":"light";try{window.localStorage.setItem("oq-overview-theme",o.overviewTheme)}catch{}}function Wa(e){o.interfacePanelOpen=e===!0}function ul(e){try{window.localStorage.setItem("oq-active-surface",e==="native"?"native":"app")}catch{}}function Cn(e,t={}){o.settingsGroup=Ne.has(e)?e:me[0].id;try{window.localStorage.setItem("oq-settings-group",o.settingsGroup)}catch{}t.syncUrl!==!1&&o.appView==="settings"&&Lo(t.syncMode||"replace")}function Vp(e){o.devPanelOpen=e===!0;try{window.localStorage.setItem("oq-dev-panel-open",o.devPanelOpen?"true":"false")}catch{}}function Bp(e){o.hpVisualMode=e==="compact"?"compact":"schematic";try{window.localStorage.setItem("oq-hp-visual-mode",o.hpVisualMode)}catch{}}function Kp(e){o.hpLayoutMode=e==="focus-hp1"||e==="focus-hp2"?e:"equal";try{window.localStorage.setItem("oq-hp-layout-mode",o.hpLayoutMode)}catch{}}function Ty(e){if(o.reducedMotion=!!e?.matches,o.reducedMotion){sn();return}an()}function xy(){let e=Gi();!e||o.motionPreferenceMedia===e||(o.motionPreferenceMedia=e,o.motionPreferenceListener=Ty,typeof e.addEventListener=="function"?e.addEventListener("change",o.motionPreferenceListener):typeof e.addListener=="function"&&e.addListener(o.motionPreferenceListener),o.reducedMotion=!!e.matches)}function $y(){return Object.keys(o.entities).length>0}function My(){return Ui+Math.floor(Math.random()*(fu-Ui+1))}function Hy(){return(document.hidden?Xr:lu)+My()}function zp(e=Hy()){o.pollTimer||o.nativeOpen||o.updateInstallBusy||wa()||(o.pollTimer=window.setTimeout(async()=>{o.pollTimer=null,await ue(),zp()},e))}function dl(){zp()}function pl(){o.pollTimer&&(window.clearTimeout(o.pollTimer),o.pollTimer=null)}zu({start:dl,stop:pl});function Ay(){o.nativeOpen||(pl(),dl(),document.hidden||ue({forceProbe:!0}))}function Va(e={}){if(En(),o.nativeOpen){pl(),sn(),o.nativeFrontendLoaded||Ba();return}if(an(),dl(),e.refresh!==!1){if(!$y()){jd();return}ue({forceFast:!0})}}function Ry(){let e=cn()||no(),t=e==="settings"?gs()||o.settingsGroup:"",r=e==="control"?ds()||"status":o.controlReplayTab,n=e==="control"?ps()||"last24":o.controlReplayWindow,a=e==="control"?ms():null;if(!(e===o.appView&&(e!=="settings"||t===o.settingsGroup)&&(e!=="control"||r===o.controlReplayTab&&n===o.controlReplayWindow&&(!a||a.start===o.controlReplayCustomStart&&a.end===o.controlReplayCustomEnd)))){if(o.appView=e,e==="control"&&(o.controlReplayTab=r,o.controlReplayWindow=n,o.controlReplayCustomStart=a?.start||"",o.controlReplayCustomEnd=a?.end||"",o.controlReplayPeriodMenuOpen=!1,o.controlReplayCustomPeriodOpen=!1,o.controlReplayCustomPeriodError=""),e==="settings"&&t){o.settingsGroup=t;try{window.localStorage.setItem("oq-settings-group",o.settingsGroup)}catch{}}f(),ue({forceFast:!0})}}function En(){o.nativeApp&&(o.nativeApp.classList.add("oq-native-app"),o.nativeApp.classList.toggle("oq-native-app--collapsed",!o.nativeOpen),o.nativeApp.setAttribute("aria-hidden",o.nativeOpen?"false":"true"))}function Up(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",jp,{once:!0}):jp(),window.addEventListener("pointermove",ed),window.addEventListener("pointerup",td),window.addEventListener("popstate",Ry),window.addEventListener("oq-mock-updated",Py),window.addEventListener("oq-dev-controls-changed",Ny),document.addEventListener("visibilitychange",Ay)}function Py(){o.mounted&&ue({forceDecisionLog:!0})}function Ny(){o.mounted&&f()}function jp(){Dy();let e=document.querySelector("esp-app");e||(e=document.createElement("esp-app"),document.body.appendChild(e)),o.nativeApp=e,o.nativeFrontendLoaded=Array.from(document.scripts).some(t=>t.src===Di),o.mounted||(Ly(e),o.mounted=!0,Va()),En(),o.nativeOpen||(ue(),kn({silent:!0}))}function Dy(){if(!document.head)return;let e=document.head.querySelector('meta[name="viewport"]');e||(e=document.createElement("meta"),e.name="viewport",document.head.appendChild(e)),e.setAttribute("content","width=device-width, initial-scale=1")}function Ly(e){let t=document.createElement("section");t.id="oq-helper-root",t.lang="nl-NL",document.documentElement&&!document.documentElement.lang&&(document.documentElement.lang="nl-NL"),e.parentNode.insertBefore(t,e),t.addEventListener("click",Yu),t.addEventListener("change",Qu),t.addEventListener("input",Ju),t.addEventListener("keydown",Zu),t.addEventListener("wheel",nd,{passive:!1}),t.addEventListener("focusin",ls),t.addEventListener("focusout",ls),t.addEventListener("mouseover",rd),t.addEventListener("mouseout",od),t.addEventListener("pointerdown",Xu),o.root=t,xy();let r=cn()||no(),n=r==="settings"?gs():"",a=r==="control"?ds():"",i=r==="control"?ps():"",l=r==="control"?ms():null;n&&Cn(n,{syncUrl:!1}),a&&(o.controlReplayTab=a),i&&(o.controlReplayWindow=i,o.controlReplayCustomStart=l?.start||"",o.controlReplayCustomEnd=l?.end||""),pt(r,{syncMode:"replace",forceSync:!0}),nn(),f()}function Oy(e){return new Promise((t,r)=>{if(!e){t();return}let n=Array.from(document.scripts).find(i=>i.src===e);if(n){if(n.dataset.loaded==="true"){t();return}n.addEventListener("load",()=>t(),{once:!0}),n.addEventListener("error",i=>r(i),{once:!0});return}let a=document.createElement("script");a.src=e,a.async=!1,a.addEventListener("load",()=>{a.dataset.loaded="true",t()},{once:!0}),a.addEventListener("error",i=>r(i),{once:!0}),document.head.appendChild(a)})}async function Ba(){if(!(o.nativeFrontendLoaded||o.nativeFrontendLoading)){o.nativeFrontendLoading=!0,o.nativeOpen&&f();try{await Oy(Di),o.nativeFrontendLoaded=!0}catch(e){o.controlError=`ESPHome fallback kon niet worden geladen. ${e.message||e}`,o.nativeOpen=!1,ul("app"),f(),Va()}finally{o.nativeFrontendLoading=!1,o.nativeOpen&&f()}}}function ml(){if(!o.root)return;let e=typeof window<"u"?window.__OQ_DEV_CONTROLS__:null;!e||typeof e.bind!="function"||e.bind(o.root)}function Tn(){let e=o.authStatus;return e?e.enabled?e.setup_window_active?"Instelvenster":"Beveiligd":"Onbeveiligd":"Laden..."}function _y(){return"Login"}function Fy(){let e=o.authStatus;return e?e.enabled?"De web-app vraagt nu een login voordat beheer beschikbaar is. Je kunt die hier aanpassen of uitzetten.":"De web-app staat open op je netwerk. Houd de herstelknop 5 seconden vast om een login toe te voegen.":"We halen de huidige loginstatus op."}function xn(){let e=o.authStatus;return e?e.enabled?e.setup_window_active?"Login actief. Tijdelijk instelvenster is open.":`Login actief${e.source?` via ${e.source}`:""}.`:e.setup_window_active?"Login uit. Tijdelijk instelvenster is open.":"Login uit. Webtoegang is open / onbeveiligd op het netwerk.":"Logingegevens laden..."}function Tr(){let e=o.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?"Herstart nodig":e.transport_active===!0?"Aan":e.transport_active===!1?"Uit":e.enabled?"Aan":"Uit":"Laden..."}function xr(){let e=o.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API beveiligd.":e.key?"Deze wijziging wordt actief na herstart. De sleutel blijft opgeslagen voor later gebruik.":"Deze wijziging wordt actief na herstart.":e.transport_active===!0?"API-encryptie staat aan. Gebruik dezelfde sleutel in Home Assistant.":"De native API staat nu open op je lokale netwerk.":"API-encryptie wordt geladen."}function Iy(){return"ESPHome API encryption"}function jy(){let e=o.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu alvast voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API nog beveiligd.":"Deze wijziging wordt actief na herstart. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":e.transport_active===!0?"De native API is beveiligd. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":"API-encryptie staat uit. Schakel in om een sleutel te bekijken, kopi\xEBren of vernieuwen.":"We halen de huidige API-beveiliging op."}function Wy(){let e=o.apiSecurityStatus;if(!e)return"Laden...";let t=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active);return t&&e.enabled===!0&&e.transport_active===!1?"Annuleer inschakelen":t&&e.enabled===!1&&e.transport_active===!0?"Annuleer uitschakelen":e.enabled?"Uitschakelen":"Inschakelen"}function Vy(){let e=o.apiSecurityStatus;return e?e.key?"Vernieuw sleutel":"Genereer sleutel":"Laden..."}function Gp(e,t,r="",n=!1){return`
    ...
    `}function Qp(){let e=o.apiSecurityStatus||{},t=e.enabled===!0,r=!!e.key,n=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active),a=r||e.transport_active===!0||n,i=o.apiSecurityNotice,l=o.apiSecurityError?`<div class="oq-helper-modal-note oq-helper-modal-note--error" aria-live="assertive">${s(o.apiSecurityError)}</div>`:"";return`
      <div class="oq-helper-modal-backdrop${o.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
        <section class="oq-helper-modal oq-helper-modal--wide" role="dialog" aria-modal="true" aria-labelledby="oq-api-security-modal-title">
          ...
          <p class="oq-helper-modal-copy">${s(jy())}</p>
      ...
      <div class="oq-helper-modal-backdrop${o.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
        <section class="oq-helper-modal" role="dialog" aria-modal="true" aria-labelledby="oq-login-modal-title">
          <div class="oq-helper-modal-head">
            <div>
              <p class="oq-helper-modal-kicker">Systeem</p>
              <h2 class="oq-helper-modal-title" id="oq-login-modal-title">${s(_y())}</h2>
            </div>
    ...
    `}function $n(e,t,r){let n=R(t),a=R(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=R(e);return Number.isFinite(i)?i:NaN}function gl(e="oq-settings-grid"){let t=By();return`
      <div class="${s(e)}">
    ...
    `}function Xp(){let e=w("boilerCvAssistEnabled")&&T("boilerCvAssistEnabled"),t=xa(),r=T("cm100Active"),n=String(t||"").trim().toUpperCase(),a=Go(t),i=!a&&(r||n==="CM100 READY"),l=o.commissioningTaskLock==="cm100",c=o.loadingEntities||o.busyAction==="commissioningCm100Start"||o.busyAction==="commissioningCm100Stop"||l,p=!!o.pendingCommissioningCm100Start,u=c||i||a,d=c||!i,b=jo("boilerPowerTestStatus","IDLE"),y=Ka(b,"boiler"),h=T("boilerPowerTestActive"),g=o.loadingEntities||o.busyAction==="boilerPowerTestStart"||o.busyAction==="boilerPowerTestAbort"||o.busyAction==="boilerPowerTestApply",m=!!(o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort||o.entities.boilerPowerTestApply),v=!!o.pendingBoilerPowerTestStart,S=o.commissioningTaskLock==="boiler",q=Go(b),k=!ke(b)&&(h||v||S||$r(b))&&!q,$=Te("boilerRatedHeatPower"),N=Te("boilerHeatPower"),D=R("boilerHeatPower"),B=D>0?N:k&&o.commissioningBoilerHeatPowerDisplay?o.commissioningBoilerHeatPowerDisplay:N;D>0&&(o.commissioningBoilerHeatPowerDisplay=N);let H=jo("flowAutotuneStatus","IDLE"),U=Ka(H,"autotune"),A=o.loadingEntities||o.busyAction==="flowAutotuneStart"||o.busyAction==="flowAutotuneAbort"||o.busyAction==="flowAutotuneApply",I=!!(o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort||o.entities.flowAutotuneApply),L=!!o.pendingFlowAutotuneStart,P=o.commissioningTaskLock==="autotune",Z=Go(H),j=!ke(H)&&(L||P||$r(H))&&!Z,X=jo("airPurgeStatus","IDLE"),We=Ka(X,"purge"),yt=T("airPurgeActive"),ct=o.loadingEntities||o.busyAction==="airPurgeStart"||o.busyAction==="airPurgeAbort",Nt=!!(o.entities.airPurgeStart||o.entities.airPurgeAbort),Ue=!!o.pendingAirPurgeStart,Re=o.commissioningTaskLock==="purge",x=!ke(X)&&(yt||Ue||Re||$r(X)),W=/DONE/.test(String(X||"").toUpperCase()),G=!!(Nt||o.entities.airPurgeStatus||o.entities.airPurgeReturnToAuto),ee=Te("airPurgeRemaining",{decimals:0}),ie=R("airPurgePhase"),be=ie===1?"Rustig":ie===2?"Pulsen":ie===3?"Stabiliseren":We.phase,re=jo("manualFlowStatus","IDLE"),xe=T("manualFlowActive"),ve=o.loadingEntities||o.busyAction==="manualFlowStart"||o.busyAction==="manualFlowAbort",ce=!!(o.entities.manualFlowStart||o.entities.manualFlowAbort),Dt=!!o.pendingManualFlowStart,de=o.commissioningTaskLock==="manual-flow",Pe=!ke(re)&&(xe||Dt||de||$r(re)),Ve=jo("manualHpStatus","IDLE"),Ge=T("manualHpActive"),Qe=o.loadingEntities||o.busyAction==="manualHpStart"||o.busyAction==="manualHpAbort",Co=!!(o.entities.manualHpStart||o.entities.manualHpAbort),ye=!!o.pendingManualHpStart,wt=o.commissioningTaskLock==="manual-hp",Ye=!ke(Ve)&&(Ge||ye||wt||$r(Ve)),Yn=/SAFETY STOP/.test(String(Ve||"").toUpperCase()),Jn=/STOPPING/.test(String(Ve||"").toUpperCase()),St=jo("hpWaterCalibrationStatus","IDLE"),Hi=Ka(St,"hp-water-calibration"),Zn=T("hpWaterCalibrationActive"),ar=o.loadingEntities||o.busyAction==="hpWaterCalibrationStart"||o.busyAction==="hpWaterCalibrationAbort"||o.busyAction==="hpWaterCalibrationApply",zc=!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort||o.entities.hpWaterCalibrationApply),Xn=!!o.pendingHpWaterCalibrationStart,Jt=o.commissioningTaskLock==="hp-water-calibration",Be=!ke(St)&&(Zn||Xn||Jt||$r(St)),ea=/DONE|APPLIED/.test(String(St||"").toUpperCase()),Uc=/APPLIED/.test(String(St||"").toUpperCase()),Eh=Te("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),Th=Te("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),Gc=/DONE|APPLIED/.test(String(b||"").toUpperCase()),Qc=/DONE|APPLIED/.test(String(H||"").toUpperCase()),Ai=i?q?"Wachten op CM100":k?y.phase:Gc?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Ri=i?Z?"Wachten op CM100":j?U.phase:Qc?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Pi=i?x?We.phase:W?"Klaar":"Klaar om te starten":"Wachten op CM100",Yc=i?Pe?"Actief":"Klaar om te starten":"Wachten op CM100",Jc=i?Ye?Jn?"Bezig met stoppen":Yn?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",Zc=i?Be?Hi.phase:Uc?"Offsets toegepast":ea?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",Xc=!i||g||!m||j||x||Pe||Ye||Be||k||P||Re||de||wt||Jt||v,xh=g||!(k||S||v),$h=g||Xc||!Gc||j||x||Be,eu=!i||A||!I||k||x||Pe||Ye||Be||j||S||Re||de||wt||Jt||L,Mh=A||!(j||P||L),Hh=A||eu||!Qc||k||x||Be,Ah=!i||ct||!Nt||k||j||Pe||Ye||Be||x||S||P||de||wt||Jt||Ue,Rh=ct||!(x||Re||Ue),Ph=!i||ve||!ce||k||j||x||Ye||Be||Pe||S||P||Re||wt||Jt||Dt,Nh=ve||!(Pe||de||Dt),Dh=!i||Qe||!Co||k||j||x||Pe||Be||Ye||S||P||Re||de||Jt||ye,Lh=Qe||!(Ye||wt||ye),Oh=!i||ar||!zc||k||j||x||Pe||Ye||Be||S||P||Re||de||wt||Xn,_h=ar||!(Be||Jt||Xn),Fh=ar||Be||!ea||Uc;p&&i&&(o.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(o.commissioningTaskLock=""),v&&(h||ke(b))&&(o.pendingBoilerPowerTestStart=!1),S&&ke(b)&&(o.commissioningTaskLock=""),L&&ke(H)&&(o.pendingFlowAutotuneStart=!1),P&&ke(H)&&(o.commissioningTaskLock=""),Ue&&(yt||ke(X))&&(o.pendingAirPurgeStart=!1),Re&&ke(X)&&(o.commissioningTaskLock=""),Dt&&(xe||ke(re))&&(o.pendingManualFlowStart=!1),de&&(xe||ke(re))&&(o.commissioningTaskLock=""),ye&&(Ge||ke(Ve))&&(o.pendingManualHpStart=!1),wt&&(Ge||ke(Ve))&&(o.commissioningTaskLock=""),Xn&&(Zn||ke(St))&&(o.pendingHpWaterCalibrationStart=!1),Jt&&ke(St)&&(o.commissioningTaskLock="");let Ih=a?"Wachten op CM100":t,jh=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",Wh=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:Zc,available:!!(zc||o.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:Mr({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:Zc,statusCopy:Be?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":ea?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:zy({status:St,running:Be,resultReady:ea,startDisabled:Oh,abortDisabled:_h,applyDisabled:Fh,busy:ar,controlsAvailable:!!(o.entities.hpWaterCalibrationStart||o.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:Yc,available:!!(ce||o.entities.manualFlowStatus),openDisabled:!i,cardMarkup:Mr({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:Yc,statusCopy:Pe?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
            <div class="oq-settings-manual-flow-control">
              ...
              ${o.entities.manualFlowStart||o.entities.manualFlowAbort?Wo({active:Pe,startKey:"manualFlowStart",stopKey:"manualFlowAbort",startLabel:"Waterpomp starten",stopLabel:"Waterpomp stoppen",startDisabled:ve||Ph,stopDisabled:ve||Nh}):""}
            </div>
          `,metrics:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:Jc,available:!!(Co||o.entities.manualHpStatus),openDisabled:!i,cardMarkup:Mr({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:Jc,statusCopy:Ye?Jn?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":Yn?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
            ${o.entities.manualHpStart||o.entities.manualHpAbort?Wo({active:Ye,startKey:"manualHpStart",stopKey:"manualHpAbort",startLabel:"Bediening starten",stopLabel:"Bediening stoppen",startDisabled:Qe||Dh,stopDisabled:Qe||Lh}):""}
          `,controls:`
          ...
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:Ri,available:!0,openDisabled:Go(Ri),cardMarkup:Mr({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:Ri,statusCopy:Z?"Wacht totdat CM100 actief is voordat je autotune start.":j?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
            ${o.entities.flowAutotuneStart||o.entities.flowAutotuneAbort?Wo({active:j,startKey:"flowAutotuneStart",stopKey:"flowAutotuneAbort",startLabel:"Autotune starten",stopLabel:"Autotune stoppen",startDisabled:A||eu,stopDisabled:A||Mh}):""}
            ${o.entities.flowAutotuneApply?ze("flowAutotuneApply","Toepassen","oq-helper-button oq-helper-button--ghost",A||Hh):""}
          `,metrics:`
            ${He("flowKpSuggested","Voorgestelde Kp","Kp bepaalt hoe sterk de regeling meteen corrigeert.",Eh,"oq-settings-field--compact")}
            ${He("flowKiSuggested","Voorgestelde Ki","Ki corrigeert kleine afwijkingen langzaam weg.",Th,"oq-settings-field--compact")}
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:Ai,available:e,openDisabled:Go(Ai),cardMarkup:Mr({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s($)}`,status:Ai,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":k?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ${o.entities.boilerPowerTestStart||o.entities.boilerPowerTestAbort?Wo({active:k,startKey:"boilerPowerTestStart",stopKey:"boilerPowerTestAbort",startLabel:"Boiler test starten",stopLabel:"Boiler test stoppen",startDisabled:g||Xc,stopDisabled:g||xh}):""}
            ${o.entities.boilerPowerTestApply?ze("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",g||$h):""}
          `,metrics:`
          ...
          `})},{key:"purge",title:"Ontluchten",label:"Ontluchten",summary:"Draait een vaste ontluchtingsrun van 5 minuten met rustige flow, pomp-pulsen en stabilisatie.",status:Pi,available:G,openDisabled:Go(Pi),cardMarkup:Mr({taskKey:"purge",title:"Ontluchten",copy:"Draait 5 minuten met rustige doorstroming, korte pomp-pulsen en een stabilisatiefase.",subcopy:"Na afloop kan OpenQuatt de service mode (CM100) afsluiten of actief laten.",status:Pi,statusCopy:x?"Ontluchten loopt vast 5 minuten door en stopt daarna automatisch.":i?"CM100 staat klaar. Start ontluchten wanneer het circuit open staat.":"Start CM100 eerst en voer daarna ontluchten uit.",progressTask:"purge",className:"oq-settings-commissioning-card--air-purge",actions:`
            ${o.entities.airPurgeStart||o.entities.airPurgeAbort?Wo({active:x,startKey:"airPurgeStart",stopKey:"airPurgeAbort",startLabel:"Ontluchten starten",stopLabel:"Ontluchten stoppen",startDisabled:ct||Ah,stopDisabled:ct||Rh}):""}
          `,metrics:`
          ...
          `})}].filter(Vh=>Vh.available);return{cm100Status:Ih,cm100StartDisabled:u,cm100StopDisabled:d,serviceStatusCopy:jh,tasks:Wh}}function Uy(e){return De({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
`));let u=`${n}|${p.length}|${p.slice(0,120)}|${p.slice(-120)}`,d=Number.isFinite(o.trendHistoryNowMs),y=Number.isFinite(c)?!d||o.trendHistoryNowMs!==c:d,h=p!==o.trendHistoryRaw||o.trendHistoryError!==""||o.trendHistorySignature!==u||y;return o.trendHistoryRaw=p,o.trendHistoryError="",o.trendHistorySignature=u,o.trendHistoryNowMs=Number.isFinite(c)?c:Number.NaN,o.trendHistoryLastFetchAt=Date.now(),h})();try{return await o.trendHistoryFetchPromise}catch(n){let a=`Trendhistorie kon niet worden geladen. ${n.message}`,i=o.trendHistoryError!==a;return o.trendHistoryError=a,o.trendHistoryRaw="",o.trendHistorySignature="",o.trendHistoryNowMs=Number.NaN,o.trendHistoryLastFetchAt=Date.now(),i}finally{o.trendHistoryFetchPromise=null}}async function yo(e={}){if(!w("lifetimeEnergyHistoryEnabled")&&!Rr()){let a=!!(o.energyHistoryRaw||o.energyHistoryError);return o.energyHistoryRaw="",o.energyHistoryError="",o.energyHistorySignature="",o.energyHistoryNowMs=Number.NaN,o.energyHistoryLastFetchAt=0,o.energyHistoryRequestQuery="",o.energyHistoryFetchQuery="",o.energyHistoryCsrfToken="",a}let t=e.force===!0,r=Date.now(),n=e.metaOnly===!0?"?meta=1":typeof Mn=="function"?Mn():"";if(!t&&o.energyHistoryFetchPromise&&o.energyHistoryFetchQuery===n)return o.energyHistoryFetchPromise;if(!t&&o.energyHistoryRequestQuery===n&&(o.energyHistoryRaw||o.energyHistoryError)&&r-Number(o.energyHistoryLastFetchAt||0)<ta)return!1;o.energyHistoryFetchQuery=n,o.energyHistoryFetchPromise=(async()=>{let a=async m=>{let v=await fetch(`${Oe()}/energy/history${m}`,{cache:"no-store"});if(!v.ok)throw new Error(`HTTP ${v.status}`);return v.text()},i=n,l=await a(i);if(e.metaOnly!==!0&&i.includes("meta=1")&&typeof Mn=="function"){let m=o.energyHistoryRaw;o.energyHistoryRaw=l;let v=Mn();o.energyHistoryRaw=m,v&&v!==i&&(i=v,l=await a(i))}let c=l.split(/\r?\n/),p=hm(l)||o.energyHistoryCsrfToken,u=Number.NaN;c.forEach(m=>{m.startsWith("@now|")&&(u=Number(m.slice(5)))});let d=`${l.length}|${l.slice(0,120)}|${l.slice(-120)}`,b=Number.isFinite(o.energyHistoryNowMs),h=Number.isFinite(u)?!b||o.energyHistoryNowMs!==u:b,g=l!==o.energyHistoryRaw||o.energyHistoryError!==""||o.energyHistorySignature!==d||h||p!==o.energyHistoryCsrfToken;return o.energyHistoryRaw=l,o.energyHistoryError="",o.energyHistorySignature=d,o.energyHistoryNowMs=Number.isFinite(u)?u:Number.NaN,o.energyHistoryLastFetchAt=Date.now(),o.energyHistoryRequestQuery=i,o.energyHistoryCsrfToken=p,g})();try{return await o.energyHistoryFetchPromise}catch(a){let i=`Energiehistorie kon niet worden geladen. ${a.message}`,l=o.energyHistoryError!==i;return o.energyHistoryError=i,o.energyHistoryRaw="",o.energyHistorySignature="",o.energyHistoryNowMs=Number.NaN,o.energyHistoryLastFetchAt=Date.now(),o.energyHistoryRequestQuery=n,o.energyHistoryCsrfToken="",l}finally{o.energyHistoryFetchPromise=null}}cd({refreshEnergyHistoryData:yo,refreshSettingsStorageState:Rn,refreshTrendHistoryData:yl,refreshTrendHistoryMetadata:lm,shouldRefreshSettingsStorageForCurrentSurface:bw});function Cm(){return F("controlModeLabel","").toLowerCase().includes("standby")}function Ht(e,t){e&&e.innerHTML!==t&&(e.innerHTML=t)}function nt(e,t,r){return!e||e.dataset.renderSignature===t?!1:(e.outerHTML=r,!0)}function Cw([e,t]){let r=Ji(t);if(!w(t)&&Number.isNaN(r))return"";let n=Yi(t)?Xe(t):F(t);return`
    ...
    `:""}function Tw(e){let t=e.groups.map(Ew).filter(Boolean).join("");return t?`
    ...
    `:""}function xw(e){let t=e.categories.map(Tw).filter(Boolean).join("");return t?`
    ...
    `:""}function kl(){let e=ua.map(xw).filter(Boolean),t=["oq-overview-energy-grid",e.length===1?"oq-overview-energy-grid--single":"",e.length===2?"oq-overview-energy-grid--two":""].filter(Boolean).join(" ");return{renderedColumns:e,gridClassName:t}}function Mm(e=kl()){return we(e)}function Hm(e=kl()){return`
    ...
    `}function Gw(e){return Lr(e.view)?`
    ...
    `:`
      ...
      `}function Qw(){let e=document.activeElement;return!!(e&&e.closest&&e.closest(".oq-energy-history-period"))}function Yw(e){return`
    ...
    `}function Jw(e){let t=Number(e.electricalInputWh||0),r=Number(e.heatOutputWh||0),n=Number(e.coolingOutputWh||0),a=Number(e.boilerOutputWh||0),i=a>0?"boiler":"boiler-zero",l=Math.max(1,t+r+n+a),c=u=>`${Math.max(0,Number(u||0)/l*100).toFixed(2)}%`,p=zw(e);return`
    ...
    `}function Zw(e){let t=Math.max(1,Number(e||0)/1e3),r=Math.pow(10,Math.floor(Math.log10(t))),n=t/r;return(n<=1.5?1.5:n<=3?3:n<=6?6:10)*r*1e3}function Xw(e){let t=Number(e);return Number.isFinite(t)?t>=999500?`${Number((t/1e6).toFixed(1))}`:`${Number((t/1e3).toFixed(1))}`:""}function eS(e){return e>=999500?"MWh":"kWh"}function tS(e){let u=Math.max(1e3,...e.map(_w)),d=Zw(u),b=e.length?1218/e.length:1218,y=Math.max(6,Math.min(38,b*.68));return{width:1280,height:260,left:44,right:18,top:26,bottom:38,plotWidth:1218,plotHeight:196,axisMax:d,barSlot:b,barWidth:y,yOf:g=>26+(1-Math.min(1,Math.max(0,Number(g||0)/d)))*196}}function oS(e){let t=Pn(e.heatpumpHeatOutputWh,e.heatingInputWh),r=Pn(e.heatpumpCoolingOutputWh,e.coolingInputWh),n=Pn(jm(e),e.electricalInputWh);return[e.tooltipLabel||e.label||Gt(e.dateKey),`Elektrisch: ${at(e.electricalInputWh,1)}`,`Warmtepomp warmte: ${at(e.heatpumpHeatOutputWh,1)}`,`Warmtepomp koeling: ${at(e.heatpumpCoolingOutputWh,1)}`,`Cv-ketel: ${at(e.boilerHeatOutputWh,1)}`,`COP verwarmen: ${t}`,`EER koelen: ${r}`,`Output / elektrisch: ${n}`].join(`
`)}function rS(e,t=""){if(!e.length)return`
      ...
      `;let r=tS(e),n=eS(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,c)=>{let p=r.left+r.barSlot*c+r.barSlot/2,u=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],d=r.height-r.bottom,b=u.map(m=>{let v=Ln(l,m.key);if(v<=0)return"";let S=v/r.axisMax*r.plotHeight;return d-=S,`
        ...
        `}).join(""),h=e.length<=12||c===0||c===e.length-1||c%3===0?`<text x="${p.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||Gt(l.dateKey))}</text>`:"",g=oS(l);return`
      ...
      <svg class="oq-energy-history-chart oq-energy-history-chart--${s(Ie(t))}" viewBox="0 0 ${r.width} ${r.height}" role="img" aria-label="Energiehistorie">
        ...
        ${a.map(l=>{let c=r.yOf(l);return`
            ...
            <text x="${r.left-10}" y="${c.toFixed(1)}" text-anchor="end" dominant-baseline="middle" class="oq-energy-history-axis-label">${s(Xw(l))}</text>
          `}).join("")}
        ${i}
      </svg>
    `}function nS(e=null){let t=Number(e?.boilerOutputWh||0)>0?"boiler":"boiler-zero";return`
    ...
    `}function Ml(){let e=On(),t=Ie(o.energyHistoryView),r=oi(e,t),n=Bw(e,t,r),a=Kw(n.buckets);return{records:e,buckets:n.buckets,viewModel:n,periodControl:r,summary:a,activeView:t}}function Bm(e=Ml()){return we({energyHistorySignature:o.energyHistorySignature||"",energyHistoryError:o.energyHistoryError||"",activeView:e.activeView,periodView:e.periodControl.view,periodValue:e.periodControl.selectedValue,periodMin:e.periodControl.minValue,periodMax:e.periodControl.maxValue,recordCount:e.records.length,bucketCount:e.buckets.length,latestDate:e.records[e.records.length-1]?.dateKey||0,currentValues:Za.map(t=>e.buckets[e.buckets.length-1]?.[t]??null)})}function Km(e=Ml()){let t=e.summary,r=Uw(t),n=e.buckets[0]?.dateKey?Gt(e.buckets[0].dateKey):"\u2014",a=e.buckets[e.buckets.length-1]?.dateKey?Gt(e.buckets[e.buckets.length-1].dateKey):"\u2014";return`
    ...
    `}function aS(){if(!o.root||o.appView!=="energy")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-overview-energy"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=kl();return nt(t,Mm(n),Hm(n))}function iS(){if(!o.root||o.appView!=="results")return!1;let e=o.root.querySelector(".oq-overview-board"),t=e?e.querySelector(".oq-energy-history"):null;if(!e||!t)return!1;let r=`oq-overview-board oq-overview-board--${o.overviewTheme}`;e.className!==r&&(e.className=r);let n=Ml(),a=Qw();return(a?!1:nt(t,Bm(n),Km(n)))||a}gr({patchEnergyDom:aS,patchResultsDom:iS});function Al(e,t,r="",n=!1){return`
    ...
    `}function sS(e,t,r,n=""){if(!w(e))return"";let a=o.entities[e],i=Array.isArray(a?.option)?a.option:[],l=String(E(e)||"");return i.length?`
    ...
    `:""}function lS(e){return w(e)||!!O[e]&&!o.optionalMissingEntities?.[e]}function Pl(e,t,r,n={}){if(!lS(e))return"";let a=w(e),i=a&&(o.loadingEntities||o.busyAction===e),l=n.disabled===!0||!a,c=n.buttonClass||"oq-helper-button oq-helper-button--ghost";return`
      <button
        class="${s(c)}"
        type="button"
        data-oq-action="${s(r)}"
    ...
    `}function pS(e){let t=e.filter(Boolean);return t.length?`
    ...
    `:""}function Zm(){if(!w("trendHistoryEnabled")&&!w("lifetimeEnergyHistoryEnabled"))return"";let e=T("trendHistoryEnabled"),t=e&&T("trendHistoryFlashEnabled"),r=w("lifetimeEnergyHistoryEnabled"),n=r&&T("lifetimeEnergyHistoryEnabled"),a=t?je("trendHistoryFlashAvailable","Alleen live"):"Alleen live",i=r?Gm(je("lifetimeEnergyHistoryAvailable","Geen data")):"Geen data";return oe("Diagnose","Gegevens bewaren","Bepaal welke gegevens OpenQuatt bewaart voor grafieken, resultaten en hulp bij problemen.",`
      ...
      `)}function mS(){if(!w("lifetimeEnergyHistoryEnabled"))return"";let e=o.energyHistoryImportRecords.length,t=new Set(o.energyHistoryImportHourRecords.map(c=>c.dateKey)).size,r=[];e>0&&r.push(`${e} dagrecords`),t>0&&r.push(`${t} uurdagen`),o.energyHistoryImportRange&&r.push(o.energyHistoryImportRange),o.energyHistoryImportSource&&r.push(o.energyHistoryImportSource),o.energyHistoryImportInvalidCount>0&&r.push(`${o.energyHistoryImportInvalidCount} regels niet gebruikt`);let n=!!o.energyHistoryImportFileName,a=e>0||t>0,i=Number(o.energyHistoryImportProgressPercent||0),l=o.energyHistoryImportBusy?`Importeren...${i>0?` (${i}%)`:""}`:"Importeren";return`
    ...
    `}function gS(){if(!w("lifetimeEnergyHistoryEnabled"))return"";let e=Ar(o.energyHistoryExportMode),t=An.map(n=>`
          ...
          <div class="oq-settings-storage-domain-grid">
            ...
            <section class="oq-settings-storage-domain oq-settings-storage-domain--energy">
              ...
              <div class="oq-settings-storage-domain-rows">
                ...
                ${sS("lifetimeEnergyHourRetention","Uurdetail bewaren","Kies hoelang OpenQuatt detail per uur mag bewaren voor de daggrafiek.","Geavanceerd")}
                ${p?`
    ...
    `}ad({getSignature:ni,patch:bS});function cg(){if(o.systemModal==="login")return Yp();if(o.systemModal==="api-security")return Qp();if(o.systemModal==="mqtt")return wd();if(o.systemModal==="mqtt-sensors")return Sd();if(o.systemModal==="connectivity"){let e=hS();return`
        <div class="oq-helper-modal-backdrop${o.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
      ...
      `}return""}function yS(e=!1){let t=ga();return t?`
    ...
    `:""}function Fl(){return`
      <div class="oq-settings-curve-grid">
        ...
        ${se("curveFallbackSupply","Fallback-aanvoertemperatuur zonder buitentemperatuur","Aanvoertemperatuur die gebruikt wordt als de buitentemperatuursensor niet beschikbaar is.","oq-settings-field--curve-fallback-card",{footerMarkup:yS()})}
      </div>
    ...
    `}function wS(e="oq-settings-grid"){let t=[se("flowKp","Flow PI Kp","Hoe sterk de regeling direct reageert op een afwijking."),se("flowKi","Flow PI Ki","Hoe snel de regeling kleine restfouten wegwerkt.")].filter(Boolean);return t.length?`
      <div class="${s(e)}">
        ${t.join("")}
      </div>
    ...
    `}function SS(){if(!w("phResponseProfile"))return"";let e=String(E("phResponseProfile")||""),t=o.loadingEntities||o.busyAction==="save-phResponseProfile",n=`
    ...
    `:""}function ug(){let e=wS();return oe("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
        ${jl()}
        ${e?`
          <div class="oq-settings-subpanel oq-settings-subpanel--nested">
            <div class="oq-settings-subpanel-head">
    ...
    `}function hg(){let e=kS();return e.length?`
    ...
    `}function gg(e,t){return w(`${t}CompressorStarts2h`)?`
      <div class="oq-settings-monitoring-compressor-unit">
        <div>
          <p>${s(e)}</p>
          <span>Laatste start: ${s(HS(`${t}CompressorLastStartAge`))}</span>
        </div>
        <dl>
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=o.entities.hpGeneration||{},r=String(E("hpGeneration")||""),n=Wt(t),a=o.loadingEntities||o.busyAction==="save-hpGeneration";return`
    ...
    `}function NS(){let e=li(),t=o.busyAction==="quickstart-thermostat-source",r=e.status==="Geldig"?" is-active":"",n=e.isRemoteProfile?`
    ...
    `}function Mg(){return!o.quickStartModalOpen||o.loadingEntities||o.complete===null||o.complete&&o.quickStartModalMode!=="generation"?"":o.quickStartModalMode==="generation"?`
      ...
      `:`
      <div class="oq-helper-modal-backdrop oq-helper-modal-backdrop--quickstart" data-oq-modal="quickstart-forced">
        <section class="oq-helper-modal oq-helper-modal--wide oq-helper-modal--quickstart" data-oq-quickstart-scroller data-oq-quickstart-step="${s(KS().id)}" role="dialog" aria-modal="true" aria-labelledby="oq-quickstart-modal-title">
          <div class="oq-helper-modal-head">
            <div>
    ...
    `}function Hg(){return o.root?o.root.querySelector("[data-oq-quickstart-scroller]"):null}function Ag(){let e=Hg();return e?{stepId:String(e.dataset.oqQuickstartStep||""),scrollHeight:e.scrollHeight,scrollTop:e.scrollTop,stickToBottom:Ra(e)}:null}function DS(e){if(!e)return;let t=Hg();if(!t||String(t.dataset.oqQuickstartStep||"")!==e.stepId)return;if(e.stickToBottom){t.scrollTop=t.scrollHeight;return}let r=e.scrollTop+(t.scrollHeight-e.scrollHeight);t.scrollTop=Math.max(0,r)}function Jl(e,t=!0){if(!e)return;let r=Number(o.quickStartScrollRestoreToken||0)+1;o.quickStartScrollRestoreToken=r;let n=()=>{o.quickStartScrollRestoreToken!==r||!o.quickStartModalOpen||DS(e)};if(t){window.requestAnimationFrame(n);return}n()}function Cg(){return`
      <section class="oq-helper-panel">
    ...
    `}function WS(){return o.currentStep==="generation"?Eg():o.currentStep==="boiler"?w("boilerCvAssistEnabled")?LS():Cg():o.currentStep==="flow-source"?PS():o.currentStep==="thermostat-source"?NS():o.currentStep==="flow"?OS():o.currentStep==="heating"?_S():o.currentStep==="water"?FS():o.currentStep==="silent"?IS():o.currentStep==="confirm"?jS():Cg()}function Xo(){return ir.filter(e=>!e.optionalEntity||w(e.optionalEntity))}function At(e){let t=Xo().findIndex(r=>r.id===e);return`Stap ${Math.max(0,t)+1}`}function VS(e){let t=ci(),r=e===t,n=o.complete===!0||e<t;return{tone:r?"current":n?"done":"upcoming",label:r?"Actief":n?"Gereed":"Volgend",current:r}}function BS(e=!1){return Xo().map((t,r)=>{let n=VS(r);return`
        <button
    ...
    `}function zS(){let e=ci(),t=Xo();return`
    ...
    `}function XS(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}function eq(e){if(!Number.isFinite(e)||e<0)return"\u2014";let t=Math.floor(e),r=Math.floor(t/1440),n=Math.floor(t%1440/60),a=t%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function tq(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function oq(e){let t=tq(F("timeNowHhmm",""));if(!Number.isFinite(t))return"";let r=Math.round(e),n=((t-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function rq(e,t){let r=Math.max(0,(Number(t)-Number(e))/6e4),n=eq(r),a=w("timeValid")&&T("timeValid")?oq(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function nq(e){let t=Number(e);return Number.isNaN(t)?"\u2014":`${t>0?"+":t<0?"-":""}${Math.abs(t).toFixed(0)} W`}function Bg(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>w(e))||""}function aq(){return["hp1WaterIn","hp2WaterIn"].find(e=>w(e))||""}function Kg(e=F("controlModeLabel","")){let t=String(e||"").toLowerCase();return t.includes("cm5")||t.includes("cooling")||t.includes("koeling")}function Rt(){return Kg()}function zg(e,t){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:t?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function sc(){return Rt()?"Koeling":ne()?"Stooklijn":"Power House"}function iq(){let e=["phouseReq","strategyRequestedPower"];for(let t of e){let r=R(t);if(!Number.isNaN(r))return r}return Number.NaN}function Ug(){let e=iq(),t=R("phouseHouse"),r=R("totalHeat"),n=R("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:e-t,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:K(e,0,"W"),houseText:K(t,0,"W"),correctionText:nq(a),capacityText:Xe("hpCapacity"),statusTitle:i,statusCopy:l}}function Gg(){let e=R("curveSupplyTarget"),t=R("supplyTemp"),r=Bg(),n=r?R(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(t)?Number.NaN:t-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",c="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",c="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",c="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",c="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",c="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:Xe("curveSupplyTarget"),supplyText:Xe("supplyTemp"),deltaText:XS(a),capacityText:Xe("hpCapacity"),statusTitle:l,statusCopy:c}}function Qg(){let e=R("supplyTemp"),t=F("coolingGuardMode",""),r=F("coolingFallbackNightMinOutdoorTemp","\u2014"),n=R("coolingSupplyError"),a=R("coolingDemandRaw"),i=T("coolingPermitted"),l=T("coolingRequestActive"),c=F("coolingBlockReason","Onbekend"),p=Xl(c),u=zg(c,l),d="Wacht op koelvraag",b="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return u?(d="Wacht op koelvraag",b="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(d="Houdt doel vast",b="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(d="Trekt aanvoer omlaag",b="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(d="Benadert koeldoel",b="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(d="Koelt rustig door",b="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(d="Koeling gereed",b="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(d="Koeling geblokkeerd",b=`Blokkade: ${p}.`),{targetText:Xe("coolingSupplyTarget"),supplyText:Xe("supplyTemp"),safeFloorText:Xe("coolingEffectiveMinSupplyTemp"),guardMode:t,fallbackNightMin:r,demandText:Xe("coolingDemandRaw"),statusTitle:d,statusCopy:b,permitted:i,requestActive:l,blockReason:p,waitingForRoomRequest:u}}function di(){if(Rt()){let t=Qg(),r=t.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:t.targetText,focusCopy:t.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:t.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${t.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:t.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(ne()){let t=Gg();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:t.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:t.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:t.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:t.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=Ug();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function lc(e){return Wg({className:"oq-overview-system",title:e.title,copy:e.copy,signature:we(e),body:`
        ...
        <div class="oq-overview-metrics oq-overview-metrics--three-column">
          ${e.metrics.map(t=>ZS(t.label,t.value,t.tone,t.note)).join("")}
        </div>
      `})}function sq(){if(!T("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(Rt()){let n=Qg(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(Cm())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let t=(ne()?Gg():Ug()).statusTitle;return{label:"Regeling nu",value:t,tone:t==="In balans"||t==="Dicht bij doel"?"green":t==="Nog aan het opbouwen"||t==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function lq(){return T("openquattEnabled")?Rt()?T("coolingPermitted")?T("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:F("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:T("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:T("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function cc(e,t){let r=sq(),n=lq();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:t,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function uc(e,t){let r=cc(e,t);return`
    ...
    `:""}function jr({className:e,action:t,label:r,busy:n=!1,loading:a=!1,attrs:i=""}){return`
      ...
      `:s(n?"Bezig...":r)}</button>
    `}function dq(e){if(e.kind==="openquatt-control"){let t=o.busyAction==="openquatt-regulation",r=(o.loadingEntities||o.entitySyncInFlight)&&!w("openquattResumeAt");return T("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${jr({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:t})}</div>`:`
    ...
    `}function pc(){let e=cq();return e.length?`
    ...
    `}function kq(e){let t=e.samples[e.samples.length-1]||null,r=fc(e.windowHours);return`
      <article class="oq-overview-trendcard oq-overview-trendcard--${s(e.tone)}" data-oq-trend-card="${s(e.id)}" data-render-signature="${s(fq(e))}">
        <div class="oq-overview-trendcard-head">
          ...
          <div class="oq-overview-trendcard-meta">
            <div class="oq-overview-trendcard-latest">
              ${e.series.map(n=>Sq(n,t)).join("")}
            </div>
          </div>
        </div>
        ${qq(e.samples,e.series,e.mock,e.windowHours)}
    ...
    `}function Eq(){let e=Yt(),t=hl();return`
    ...
    `}function Tq(){let e="overview-trends-history",t=o.settingsInfoOpen===e;return`
    ...
    `}function xq(){if(!o.root||o.appView!=="diagnosis")return!1;let e=o.root.querySelector(".oq-overview-board");if(!e)return!1;let t=e.querySelector(".oq-overview-trends");return t?(nt(t,mi(),gi()),bc(e),Bn(e),!0):!1}function vc(e){if(!e)return null;let t=e.__oqTrendHoverNodes;if(t&&t.chart&&t.hoverLayer&&t.hoverPanel)return t;let r=e.querySelector("[data-oq-trend-hover-layer]"),n={chart:e.querySelector(".oq-overview-trend-chart"),hoverLayer:r,hoverPanel:e.querySelector("[data-oq-trend-hover]"),hoverTime:e.querySelector("[data-oq-trend-hover-time]"),hoverNote:e.querySelector("[data-oq-trend-hover-note]"),hoverValues:e.querySelector("[data-oq-trend-hover-values]"),hoverLine:r?r.querySelector(".oq-overview-trend-hover-line"):null,hoverDots:{}};return r&&r.querySelectorAll("[data-oq-trend-hover-dot]").forEach(a=>{n.hoverDots[a.getAttribute("data-oq-trend-hover-dot")]=a}),e.__oqTrendHoverNodes=n,n}function jg(e,t,r,n=vc(e)){if(!e||!t||!Array.isArray(t.points)||t.points.length===0)return;let a=Math.max(0,Math.min(t.points.length-1,r)),i=t.points[a];if(!i||!n||!n.chart||!n.hoverLayer||!n.hoverPanel||!n.hoverTime||!n.hoverNote||!n.hoverValues)return;let l=String(a);if(!n.hoverPanel.hidden&&e.dataset.oqTrendHoverIndex===l)return;let c=rq(i.sample.t,t.endTime);n.hoverPanel.hidden=!1,n.hoverLayer.removeAttribute("hidden"),n.hoverTime.textContent=c.value,n.hoverNote.textContent=c.note,n.hoverLine&&(n.hoverLine.setAttribute("x1",i.x.toFixed(1)),n.hoverLine.setAttribute("x2",i.x.toFixed(1)));let p=[];t.series.forEach(u=>{let d=Wn(u,i.sample),b=u.id||u.sampleKey||u.label,y=n.hoverDots[b];if(!Number.isFinite(d)){y&&y.setAttribute("display","none");return}let h=i.values.find(g=>g.seriesId===b);y&&h&&(y.removeAttribute("display"),y.setAttribute("cx",h.x.toFixed(1)),y.setAttribute("cy",h.y.toFixed(1))),p.push(`
        <div class="oq-overview-trend-hover-row oq-overview-trend-hover-row--${s(u.tone)}">
          <span>${s(u.label)}</span>
          <strong>${s(K(d,u.decimals,u.unit))}</strong>
        </div>
      `)}),n.hoverValues.innerHTML=p.join(""),e.dataset.oqTrendHoverIndex=l}function $q(e){if(!e)return;let t=vc(e);t?.hoverPanel&&(t.hoverPanel.hidden=!0),t?.hoverLayer&&t.hoverLayer.setAttribute("hidden",""),delete e.dataset.oqTrendHoverIndex}function Bn(e=o.root){if(!e)return;e.querySelectorAll("[data-oq-trend-card]").forEach(r=>{let n=r.dataset.renderSignature||"";if(r.__oqTrendBoundSignature===n)return;typeof r.__oqTrendCleanup=="function"&&r.__oqTrendCleanup(),r.__oqTrendHoverNodes=null,r.__oqTrendBoundSignature=n;let a=r.querySelector(".oq-overview-trend-chart");if(!a)return;let i=yq(r.dataset.oqTrendCard);if(!i)return;let l=Xg(i.samples,i.series,{mockData:i.mock});r.__oqTrendModel=l;let c=vc(r),p=0,u=null,d=()=>{let h=u;u=null,p=0;let g=a.getBoundingClientRect();if(!g.width||!g.height)return;let m=Number(h?.clientX);if(!Number.isFinite(m)){jg(r,l,l.points.length-1,c);return}let S=Math.min(g.width,Math.max(0,m-g.left))/g.width*l.width,q=wq(l,S);jg(r,l,q,c)},b=h=>{u=h,p||(p=window.requestAnimationFrame(d))},y=()=>{p&&(window.cancelAnimationFrame(p),p=0),u=null,$q(r)};a.addEventListener("pointermove",b),a.addEventListener("pointerenter",b),a.addEventListener("pointerleave",y),a.addEventListener("focus",b),a.addEventListener("blur",y),a.addEventListener("touchstart",b,{passive:!0}),r.__oqTrendCleanup=()=>{p&&(window.cancelAnimationFrame(p),p=0),u=null,a.removeEventListener("pointermove",b),a.removeEventListener("pointerenter",b),a.removeEventListener("pointerleave",y),a.removeEventListener("focus",b),a.removeEventListener("blur",y),a.removeEventListener("touchstart",b)}})}gr({patchDiagnosisDom:xq});function Hc(e,t,r){let n=Ac(F(t.mode,"Unknown")),a=T(t.defrost),i=ao(F(t.failures,"None")),l=Sa(i),c=n==="Verwarmen"||n==="Koelen"||a;return{mode:n,defrostActive:a,failures:i,warningFailures:l,running:c,thermalKey:n==="Koelen"?t.cooling:t.heat,schematic:_q(e,t,r,n,a,l,c)}}function pf(e,t=null){return`<h3>${s(e)}</h3>${t?`<button class="oq-overview-hp-card-action" type="button" data-oq-action="select-hp-layout" data-hp-layout="${s(t.layout)}">${Bq(t.layout==="equal"?"minus":"plus")}<span>${s(t.label)}</span></button>`:""}`}function Sc(e,t,r,n){return`<div class="oq-overview-hp-status">${ic(e,t,r,n)}</div>`}function Mq(e,t){return t?"ontdooit":e==="Verwarmen"?"verwarmt":e==="Koelen"?"koelt":e==="Stand-by"?"stand-by":"onbekend"}function mf(e){return!Array.isArray(e)||e.length===0?"":`<p class="oq-overview-hp-summary">${s(e.map(t=>`${t.title} ${Mq(Ac(F(t.keys.mode,"Unknown")),T(t.keys.defrost))}`).join(", "))}</p>`}function Hq(e){let t=o.entities[e];if(!t)return"Positie: \xE2\u20AC\u201D";let r=R(e);return Number.isNaN(r)?`Positie: ${F(e)}`:`Positie: ${K(r,0,t.uom||"")}`}function Aq(e){return w(e)?`Positie: ${T(e)?"Koelen/Defrost":"Verwarmen"}`:"Positie: \xE2\u20AC\u201D"}function Ac(e){let t=String(e||"").trim();return!t||t==="Unknown"?"Onbekend":t==="Standby"?"Stand-by":t==="Heating"?"Verwarmen":t==="Cooling"?"Koelen":t}function qc(e,t,r,n=!0,a="default"){return`
    ...
    `}function Rq(e,t,r){return e==="temperature"?`
        <svg
    ...
    `}function Pq({tooltip:e,...t}){return`${kc(t)}${Un({bind:t.bind,...e})}`}function Nq({bind:e,ariaLabel:t,x:r,y:n,width:a,height:i,rx:l,tooltip:c}){return`
    ...
    `}function Cc(){if(!Rc())return"";let e=Si();return o.hpVisualMode!=="schematic"?Vq(e):`
    ...
    `}function k1(){let e=lt(),t=Qn(),r=Jq(),n=qo().filter(d=>!d.quick&&!d.custom),a=Zq(),i=Xq(a),l=af(a.start),c=af(a.end),p=o.controlReplayPeriodMenuOpen,u=e==="custom"?"Eigen periode":r.some(d=>d.id===e)?"Kies periode":t.shortLabel;return`
    ...
    `}function C1(e){let t=_c(e.severity);return`
    ...
    `}function E1(e,t){let r=_c(e.severity),n=t&&t.id===e.id,a=qf(e.kind),i=Sf(e);return`
      <button
    ...
    `}function Rf(e){if(!e)return"";let t=_c(e.severity),r=tr(e.reasonCode),n=e.reasonLabel||r.label,a=e.reasonSummary||r.summary,i=kf(e),l=Sf(e);return`
    ...
    `}function x1(e,t){let r=Qn(),a=e.filter(c=>!c.timelineHidden).slice(0,80),i=String(o.decisionLogError||"").trim(),l=!a.length&&!o.decisionLog&&!i;return`
    ...
    `}function K1(){return o.appView==="overview"?Of():o.appView==="control"?Lf():o.appView==="energy"?zm():o.appView==="diagnosis"?tf():o.appView==="results"?Um():V1()}function z1(){let e=Ol();return`
    ...
    `}function U1(){let e=typeof document<"u"?document.activeElement:null;return!e||typeof e.matches!="function"?null:e.matches("select[data-oq-dev-control]")?e:null}function G1(e){if(!e||o.deferDevControlSelectRender)return;o.deferDevControlSelectRender=!0;let t=()=>{e.removeEventListener("blur",t),e.removeEventListener("change",t),o.deferDevControlSelectRender=!1,window.setTimeout(()=>If(),0)};e.addEventListener("blur",t,{once:!0}),e.addEventListener("change",t,{once:!0})}function Q1(){if(o.nativeOpen||o.appView!=="settings"||o.renderedAppView!=="settings"||o.renderedSettingsGroup!==o.settingsGroup)return null;let e=document.scrollingElement||document.documentElement,t=Number(window.scrollY||e?.scrollTop||0);return!Number.isFinite(t)||t<=0?null:{group:o.settingsGroup,left:Number(window.scrollX||e?.scrollLeft||0),top:t}}function Y1(e){if(!e)return;let t=(o.settingsPageScrollRestoreToken||0)+1;o.settingsPageScrollRestoreToken=t;let r=()=>{if(t!==o.settingsPageScrollRestoreToken||o.nativeOpen||o.appView!=="settings"||o.settingsGroup!==e.group)return;let n=document.scrollingElement||document.documentElement;if(!n)return;let a=Math.max(0,n.scrollHeight-n.clientHeight),i=Math.min(e.top,a);window.scrollTo({left:e.left,top:i,behavior:"auto"})};window.requestAnimationFrame(()=>{r(),window.requestAnimationFrame(r),window.setTimeout(r,80)})}function If(){if(!o.root)return;let e=U1();if(e){G1(e);return}let t=o.systemModal==="webserver-logs"?Bt():null,r=o.systemModal==="cm100-commissioning"?ip():null,n=String(o.systemModal||"").startsWith("service-task-")?lp():null,a=o.systemModal==="history-storage"?up():null,i=o.quickStartModalOpen?Ag():null,l=Q1();if(o.nativeOpen){o.root.innerHTML=`
        ${_l()}
        ${lg()}
      `,o.renderedAppView="native",o.renderedSettingsGroup="",o.settingsRenderSignature="",o.headerRenderSignature=ni(),o.mqttSensorsModalRenderSignature="",sn(),En(),el(),ml(),Xi(),Zi(),Kt(t),Js(r),Zs(n),Xs(a),Jl(i);return}let c=K1(),p=o.loadingEntities?`${c}${B1()}`:c,u=o.appView==="overview"||o.appView==="control"||o.appView==="energy"||o.appView==="diagnosis"||o.appView==="results";o.root.innerHTML=`
      ${_l()}
