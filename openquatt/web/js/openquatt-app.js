...
`)),r.toString()}function gs(e,o){let r=[],n=[],a=[],i="";return e.forEach(l=>{let p=Hf(l);if(!p)return;let u=[...a,p],c=Uu(u,o);if(a.length&&c.length>Tf){r.push({keys:n,body:i}),n=[l],a=[p],i=Uu(a,o);return}n=[...n,l],a=u,i=c}),a.length&&r.push({keys:n,body:i}),r}async function Mf(e){let o=op(),r={method:"POST",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:e.body};if(typeof AbortController=="function"){let a=new AbortController,i=window.setTimeout(()=>a.abort(),o);try{let l=await fetch(Bu,{...r,signal:a.signal});if(!l.ok)throw new Error(`bulk entities HTTP ${l.status}`);return l.json()}catch(l){throw a.signal.aborted?new Error(`bulk entities request timed out after ${o}ms`):l}finally{window.clearTimeout(i)}}let n=await fetch(Bu,r);if(!n.ok)throw new Error(`bulk entities HTTP ${n.status}`);return n.json()}async function Rf(){let e=op(),o={cache:"no-store",headers:{"Cache-Control":"no-store"}};if(typeof AbortController=="function"){let n=new AbortController,a=window.setTimeout(()=>n.abort(),e);try{let i=await fetch(Ku,{...o,signal:n.signal});if(!i.ok)throw new Error(`service status HTTP ${i.status}`);return i.json()}catch(i){throw n.signal.aborted?new Error(`service status request timed out after ${e}ms`):i}finally{window.clearTimeout(a)}}let r=await fetch(Ku,o);if(!r.ok)throw new Error(`service status HTTP ${r.status}`);return r.json()}function Pf(e={}){let o=e?.entities&&typeof e.entities=="object"?e.entities:{};Object.entries(o).forEach(([r,n])=>{Dr.has(r)&&(t.optionalMissingEntities&&delete t.optionalMissingEntities[r],t.entities[r]=np(r,t.entities[r],n))})}async function Nf(e){if(!e.length)return{ok:!0,message:""};try{let o=await Rf();return Pf(o),t.lastEntityResponseAt=Date.now(),{ok:!0,message:""}}catch(o){return{ok:!1,message:o.message||String(o)}}}async function L(e,o="state",r={}){let n=Date.now(),a=r.forceMissing===!0,i=e.filter(v=>a||Dr.has(v)||!$f(v,n));if(!i.length)return;let l=i.filter(v=>Dr.has(v)),p=i.filter(v=>!Dr.has(v)),c=(await Nf(l)).ok?p:[...p,...l];if(!c.length){Gu(),Jr(Ot()),t.busyAction||(ju(),t.controlError="");return}let m=Number(r.concurrency),b=Number.isFinite(m)&&m>0?Math.floor(m):Mt,y=gs(c,o),w=Math.max(1,Math.min(b,Mt)),g=[];for(let v=0;v<y.length;v+=w){let S=y.slice(v,v+w),q=await Promise.allSettled(S.map(async C=>({chunk:C,payload:await Mf(C)})));g.push(...q)}g.some(v=>v.status==="fulfilled")&&(t.lastEntityResponseAt=Date.now());let f="";g.forEach(v=>{if(v.status!=="fulfilled"){let M=v.reason.message||String(v.reason);f||(f=M);return}let{chunk:S,payload:q}=v.value,C=q?.entities&&typeof q.entities=="object"?q.entities:{},x=new Set(Array.isArray(q?.missing)?q.missing:[]);S.keys.forEach(M=>{if(Object.prototype.hasOwnProperty.call(C,M)){t.optionalMissingEntities&&delete t.optionalMissingEntities[M],t.entities[M]=np(M,t.entities[M],C[M]);return}let V=R[M];V?.optional?x.has(M)&&Af(M,n):f||(f=`${V?.name||M} ontbreekt in bulk response`)})}),Gu(),Jr(Ot()),f?(rp(f),t.deviceReconnectMode?t.controlError="":t.controlError=`Niet alle helpervelden konden worden ververst. ${f}`):t.busyAction||(ju(),t.controlError="")}function np(e,o={},r={}){let n={...o||{},...r||{}};return R[e]?.domain==="select"&&(!String(r.state??"").trim()&&String(o?.state??"").trim()&&(n.state=o.state),!String(r.value??"").trim()&&String(o?.value??"").trim()&&(n.value=o.value),!Array.isArray(r.option)&&Array.isArray(o?.option)&&(n.option=o.option),!Array.isArray(r.options)&&Array.isArray(o?.options)&&(n.options=o.options)),n}function Gu(){t.complete=Xn(),t.stage=t.complete===!0?"Gereed":t.complete===!1?"Quick Start":"Laden...",t.summary=Pc(),t.appView==="diagnosis"&&!lt()&&ct(eo(),{syncMode:"replace",forceSync:!0}),t.appView||ct(Yr()||eo(),{syncMode:"replace",forceSync:!0})}function Of(){let e=ls();return t.appView==="settings"?[...new Set([...e,...cs()])]:t.appView==="energy"||t.appView==="results"?Xr(t.appView,{forceFast:!0}):t.appView==="overview"||t.appView==="diagnosis"?Xr(t.appView,{forceFast:!0}):[...new Set(e)]}function Df(e=[]){let o=new Set(e);return(t.appView==="settings"?cs():t.appView==="overview"||t.appView==="diagnosis"?[...new Set([...Xr(t.appView,{includeBulk:!0}),...Pt])]:t.appView==="energy"||t.appView==="results"?[...new Set([...Xr(t.appView,{forceFast:!0}),...Pt])]:[...new Set(["setupComplete","strategy",...ze])]).filter(n=>!o.has(n))}function zu(e=Date.now(),o={}){return o.forceBulk===!0?!0:e-Number(t.lastBulkEntitySyncAt||0)>=Sc}async function se(e={}){if(t.nativeOpen||t.loadingEntities||t.draggingCurveKey||t.busyAction||t.settingsInteractionLock)return;if(t.updateInstallBusy||aa()){t.pendingEntitySyncOptions=null;return}if(t.focusedField&&t.appView!=="settings")return;if(t.entitySyncInFlight){bf(e);return}let o=Date.now();if(document.hidden&&o-Number(t.lastEntitySyncAttemptAt||0)<Wr)return;let r=t.appView,n=e.prefetchView==="overview"&&!e.forceBulk&&r==="settings",a=n?"overview":r,i=a==="overview"||a==="diagnosis"||a==="energy"||a==="results",l=e.forceFast===!0&&!e.forceBulk,p=!l&&!n&&zu(o,e),c=o-Number(t.lastStaticEntitySyncAt||0)>=qc||t.updateInstallBusy||t.updateInstallPhaseHint?Pt:[],m=t.quickStartModalOpen&&t.currentStep==="flow-source"?st:[],b=t.quickStartModalOpen&&t.currentStep==="thermostat-source"?vo:[],y=Ji()?oo():[],w=n?[...Ei,...ze,"setupComplete",...c]:i?[...Xr(a,{forceFast:l,includeBulk:p}),...c]:r==="settings"?[...new Set([...cs(),...y,...c])]:p?["setupComplete",...c,...ze,"strategy",...yo,...Ge,...te()?me.map(g=>g.key):wt]:["setupComplete",...ze,"strategy",...c];t.entitySyncInFlight=!0,t.lastEntitySyncAttemptAt=o;try{let g=t.deviceReconnectMode,f=kf(o,e)?await xf():{ok:!0,message:""};if(!f.ok){rp(f.message),n||d();return}if(await L([...new Set([...w,...m,...b])],n?"state":r==="settings"?"all":"state",{concurrency:l&&i?Rt:Mt}),t.lastFastEntitySyncAt=Date.now(),p&&(a==="overview"||a==="diagnosis")&&!n&&(t.lastBulkEntitySyncAt=t.lastFastEntitySyncAt),c.length&&(t.lastStaticEntitySyncAt=t.lastFastEntitySyncAt),n)return;i&&!t.overviewMetadataHydrated&&!t.overviewMetadataHydrating&&Yu();let v=g!==t.deviceReconnectMode,S=l&&i,q=S?!1:a==="overview"||a==="diagnosis"?await Xi():!1,C=S?!1:t.appView==="results"?await sa():!1,x=S?!1:Ji()?(await Promise.all([qu(),sa({metaOnly:!0})])).some(Boolean):!1,M=S||!Bc()?!1:await ko(),V=S||!Kc()?!1:await Xt(),F=S||!Ou()?!1:await Ao(),ee=yu();if(S&&!t.nativeOpen&&ep(us(a)),v){d();return}if(q&&t.appView==="diagnosis"&&!t.root?.querySelector(".oq-overview-trends")){d();return}if(C&&t.appView==="results"&&!t.root?.querySelector(".oq-energy-history")){d();return}if(x&&t.appView==="settings"){d();return}if(M&&(t.systemModal==="login"||Nt())){d();return}if(V&&t.appView==="settings"){d();return}if(t.systemModal==="mqtt-sensors"){ca()!==t.mqttSensorsModalRenderSignature&&d();return}if(F&&t.appView==="settings"){d();return}let H=!!(t.updateModalOpen||t.systemModal||t.interfacePanelOpen);if(ee!==t.headerRenderSignature)if(H&&zi())t.headerRenderSignature=ee;else{d();return}else zi();if(t.appView==="settings"&&String(t.systemModal||"").startsWith("service-task-")){jr()!==t.settingsRenderSignature&&d();return}if(H)return;if(t.appView==="settings"){let G=jr();if(G!==t.settingsRenderSignature&&!t.focusedField){d();return}if(!wu()){t.focusedField||d();return}t.focusedField&&(t.settingsRenderSignature=G);return}if(t.appView==="diagnosis"){xu()||d();return}if(t.appView==="energy"){Eu()||d();return}if(t.appView==="results"){Cu()||d();return}ku()||d()}catch(g){n||(t.controlError=`Helperstatus kon niet worden geladen. ${g.message}`,d())}finally{t.entitySyncInFlight=!1;let g=t.pendingEntitySyncOptions;t.pendingEntitySyncOptions=null,g&&!t.nativeOpen&&window.setTimeout(()=>{se(g)},0),l&&(a==="overview"||a==="diagnosis")&&!n&&!t.nativeOpen&&!g&&zu(Date.now())&&window.setTimeout(()=>{se({forceBulk:!0})},hf)}}function Et(){return typeof window<"u"&&window.__OQ_DEV_META&&typeof window.__OQ_DEV_META=="object"?window.__OQ_DEV_META:{}}function Lf(){let e=String(T("hpGeneration")||"").trim();return e||""}function en(e){let o=String(e||"").trim().toLowerCase();return o==="single"||o.includes("quatt single")||o.includes("openquatt single")?"single":o==="duo"||o.includes("quatt duo")||o.includes("openquatt duo")?"duo":""}function Ff(){if(!Array.isArray(Zo))return"";if(Zo.some(o=>h(o)))return"duo";let e=t.optionalMissingEntities||{};return Zo.every(o=>e[o])?"single":""}function hs(e){let o=en(e);return(o==="single"||o==="duo")&&typeof t<"u"&&t&&typeof t=="object"&&(t.lastKnownInstallationTopology=o),o}function If(){if(typeof t<"u"&&t&&typeof t=="object"){let e=String(t.lastKnownInstallationTopology||"").trim().toLowerCase();if(e==="single"||e==="duo")return e}return""}function Z(){let e=en(T("installationTopology"));if(e==="single"||e==="duo")return hs(e);let o=en(Et().installation);if(o==="single"||o==="duo")return hs(o);let r=Ff();return r?hs(r):If()}function pt(){let e=Z(),o=Lf();return e==="single"?o?`Quatt Single ${o}`:"Quatt Single":e==="duo"?o?`Quatt Duo ${o}`:"Quatt Duo":o?`Quatt Hybrid ${o}`:"Quatt Hybrid"}function Fe(){return"OpenQuatt"}function fs(e){let o=String(e||"").trim().toLowerCase();return o==="wifi"||o==="wi-fi"||o.includes("wifi")||o.includes("wi-fi")?"wifi":o==="eth"||o==="ethernet"||o.includes("ethernet")?"eth":""}function Ye(e=ro()){return e==="wifi"?"Wi-Fi":e==="eth"?"Ethernet":"Onbekend"}function Mo(e=Z()){return e==="single"?"Single":e==="duo"?"Duo":"Onbekende opstelling"}function tn(){let e=String(T("hardwareProfileText")||"").trim().toLowerCase();return e&&e!=="unknown"&&e!=="onbekend"?e:String(Et().hardwareProfile||e).trim().toLowerCase()}function ro(){return fs(T("connectionText")||Et().connection)}function ap(){let e=ro();return e==="wifi"?"eth":e==="eth"?"wifi":""}function ip(){let e=Z();return e==="single"?"duo":e==="duo"?"single":""}function or(e=Z(),o=ro()){let r=Mo(e),n=tn();return n==="heatpump_controller_q"?`Heatpump Controller Q ${r} ${Ye(o)}`:n==="heatpump_listener"?`Heatpump Listener ${r} ${Ye(o)}`:n==="waveshare"?`Waveshare ${r} ${Ye(o)}`:`${Fe()} ${r} ${Ye(o)}`}function bs(){let e=k("timeValid"),o=String(T("timeNowHhmm")||"").trim();if(o&&o!=="invalid")return o;if(h("timeValid")&&!e)return"Geen tijdsync";try{return new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit"}).format(new Date)}catch{return new Date().toLocaleTimeString("nl-NL",{hour:"2-digit",minute:"2-digit"})}}function pa(){return h("timeValid")&&!k("timeValid")?"Geen tijdsync":`${new Intl.DateTimeFormat("nl-NL",{day:"numeric",month:"short",year:"numeric"}).format(new Date)} \xB7 ${bs()}`}function ua(e){if(!Number.isFinite(e)||e<0)return"\u2014";let o=Math.floor(e),r=Math.floor(o/1440),n=Math.floor(o%1440/60),a=o%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function _f(e){return String(e?.uom??e?.unit_of_measurement??"").trim().toLowerCase()}function Wf(e){let o=e?.state;if(o!==""&&o!==null&&o!==void 0){let a=Number(o);if(Number.isFinite(a))return a}let r=e?.value,n=Number(r);return Number.isFinite(n)?n:NaN}function rr(){let e=Wf(t.entities.uptime);if(Number.isFinite(e)&&e>=0){let n=_f(t.entities.uptime);if(n==="d")return ua(e*1440);if(n==="h")return ua(e*60);if(n==="s")return ua(e/60)}let o=String(t.entities.uptimeReadable?.state??t.entities.uptimeReadable?.value??"").trim();if(o&&o.toLowerCase()!=="unknown")return o;let r=Number(Et().bootedAt);return!Number.isFinite(r)||r<=0?"\u2014":ua((Date.now()-r)/6e4)}function nr(){let e=String(t.entities.ipAddress?.state??t.entities.ipAddress?.value??"").trim();if(e)return e;let o=String(Et().ipAddress||"").trim();return o||(typeof window<"u"?String(window.location.hostname||"").trim():"")||"\u2014"}function no({key:e,value:o,meta:r,controlClass:n,inputClass:a="oq-helper-input",inputAttributes:i="",unitMarkup:l=""}){return`
  ...
  `;return ce(e,o,r,c,a)}function re(e,o,r,n="",a={}){if(!h(e))return"";let i=Oe(e),l=Te(e),p=a.unitOverride||i.uom||"",u=a.showUnit!==!1&&!!p,c=u&&a.unitMode!=="outside",m=no({key:e,value:l,meta:i,controlClass:`oq-helper-control${u&&!c?" oq-helper-control--split":""}${c?" oq-helper-control--suffix":""}`,unitMarkup:u?c?`<span class="oq-helper-unit-chip">${s(p)}</span>`:`<span class="oq-helper-unit">${s(p)}</span>`:""});return ce(e,o,r,m,n,a.footerMarkup||"")}function Ze(e,o,r,n="",a={}){if(!h(e))return"";let i=Oe(e),l=ge(e,T(e)),p=a.minLabel||`${i.min}${i.uom||""}`,u=a.maxLabel||`${i.max}${i.uom||""}`,c=a.valueLabel||B(e,l);return ce(e,o,r,`<label class="oq-helper-slider-field"><div class="oq-helper-slider-meta"><span>${s(p)}</span><strong>${s(c)}</strong><span>${s(u)}</span></div><input class="oq-helper-range" type="range" data-oq-field="${s(e)}" min="${i.min}" max="${i.max}" step="${i.step}" value="${l}" ${t.loadingEntities?"disabled":""}></label>`,n)}function vs(e,o,r,n={}){if(!h(e))return"";let a=Oe(e),i=Te(e),l=n.compact===!0,p=n.embedded===!0,u=n.infoId||e,c=n.showCopy!==!1;return`
...
`).trimEnd()}catch{}return o}function hb(e){return String(e??"").replace(/\x1b\[[0-9;]*m/g,"")}function fb(e){let o=String(e??""),r=Array.from(o.matchAll(/\x1b\[([0-9;]*)m/g));for(let i=r.length-1;i>=0;i-=1){let l=r[i][1].split(";").map(p=>Number(p)).filter(p=>Number.isFinite(p));for(let p=l.length-1;p>=0;p-=1){let u=l[p];if(u===31||u===91)return"error";if(u===33||u===93)return"warning";if(u===32||u===92)return"info";if(u===36||u===96||u===34||u===35)return"debug";if(u===37||u===90||u===38||u===97)return"verbose"}}let n=o.match(/\[(E|W|I|D|V|VV)\]/i);if(!n)return"plain";let a=n[1].toUpperCase();return a==="E"?"error":a==="W"?"warning":a==="I"?"info":a==="D"?"debug":"verbose"}function bb(e){for(;t.webServerLogEntries.length>up;)t.webServerLogEntries.shift(),e&&e.firstElementChild&&e.removeChild(e.firstElementChild)}function yb(){return t.root?t.root.querySelector("[data-oq-webserver-log-output]"):null}function fa(){return t.root?t.root.querySelector("[data-oq-webserver-log-scroller]"):null}function ba(e){return e?e.scrollHeight-e.scrollTop-e.clientHeight<48:!1}function vb(e,o){if(!(!o||e.length===0)){o.dataset.webServerLogEmpty==="true"&&(o.dataset.webServerLogEmpty="false",o.innerHTML="");for(let r of e)o.insertAdjacentHTML("beforeend",Tp(r))}}function Tp(e){let o=dp(e.receivedAt),r=Uf(e.receivedAt);return`
  ...
  `}function wb(e=t.webServerLogEntries){return e.length?e.map(o=>Tp(o)).join(""):`
    ...
    <div class="oq-webserver-log-history-shell">
      ${Pe({dataValue:"webserverLogHistory",label:"RAM log history",value:r,note:n,action:`<button
  ...
  `}function Tb(){let e=Ss();if(!e)return"";let o=mp(e),r=Qf(e),n=t.loadingEntities||t.busyAction==="save-debugLevel";return`
    ${Pe({dataValue:"debugLevel",label:"Logger level",value:r||"Onbekend",note:"Past het runtime logniveau aan voor nieuwe firmwaremeldingen.",action:`<label class="oq-webserver-log-level-control" aria-label="Logger level">
...
`)}async function Ep(){let e=Eb();if(t.webServerLogCopyMessage="",t.webServerLogCopyError="",!e){t.webServerLogCopyError="Er zijn nog geen logregels om te kopi\xEBren.",d();return}try{if(!await Yt(e))throw new Error("Kopi\xEBren naar het klembord is niet gelukt.");t.webServerLogCopyMessage=`${t.webServerLogEntries.length} logregel${t.webServerLogEntries.length===1?"":"s"} gekopieerd.`}catch(o){t.webServerLogCopyError=o instanceof Error?o.message:"Kopi\xEBren naar het klembord is niet gelukt."}t.systemModal==="webserver-logs"&&d()}function Cp(){let e=ir();return`
  ...
  `}function Cb(){let e=t.entities.firmwareUpdateTarget||{};return Array.isArray(e.option)?e.option:Array.isArray(e.options)?e.options:[]}function $p(e){return Cb().includes(e)}function va(){let e=tn(),o=Z(),r=ro(),n=ap();return e!=="heatpump_controller_q"||o!=="single"&&o!=="duo"||r!=="wifi"&&r!=="eth"||!n?null:{canSwitch:h("firmwareUpdateTarget")&&$p("alternate connection")&&h("installFirmwareUpdateTarget"),currentConnection:r,targetConnection:n,currentLabel:Ye(r),targetLabel:Ye(n),currentBuildLabel:or(o,r),targetBuildLabel:or(o,n)}}function wa(){let e=tn(),o=Z(),r=ip(),n=ro(),a=e==="heatpump_controller_q"?["wifi","eth"]:["wifi"];return!["heatpump_controller_q","heatpump_listener","waveshare"].includes(e)||o!=="single"&&o!=="duo"||!r||!a.includes(n)?null:{canSwitch:h("firmwareUpdateTarget")&&$p("alternate topology")&&h("installFirmwareUpdateTarget"),currentTopology:o,targetTopology:r,currentConnection:n,targetConnection:n,currentLabel:Mo(o),targetLabel:Mo(r),currentBuildLabel:or(o,n),targetBuildLabel:or(r,n)}}function Do(e=t.updateTestFirmwarePr){let o=String(e||"").trim().replace(/^#?pr[-\s]*/i,"").replace(/^#/,"");return/^\d{1,6}$/.test(o)?o:""}function cr(){let e=tn(),o=Z(),r=ro(),a={waveshare:{slug:"waveshare",label:"Waveshare",connections:["wifi"]},heatpump_listener:{slug:"heatpump-listener",label:"Heatpump Listener",connections:["wifi"]},heatpump_controller_q:{slug:"heatpump-controller-q",label:"Heatpump Controller Q",connections:["wifi","eth"]}}[e];if(!a||o!=="single"&&o!=="duo"||!a.connections.includes(r))return{available:!1,label:"Onbekend target",error:"Deze firmware meldt geen herkenbaar hardware-, opstelling- of verbindingsprofiel."};let i=`openquatt-${a.slug}-${o}-${r}`,l=o==="duo"?"Duo":"Single";return{available:!0,artifactName:i,otaFileName:`${i}.firmware.ota.bin`,md5FileName:`${i}.firmware.ota.bin.md5`,label:`${a.label} ${l} ${Ye(r)}`}}function rn(e=Do(),o=cr()){if(!e||!o.available)return null;let n=`${`https://github.com/jeroen85/OpenQuatt/releases/download/pr-${e}`}/${o.otaFileName}`;return{otaUrl:n,md5Url:`${n}.md5`,releaseApiUrl:`https://api.github.com/repos/jeroen85/OpenQuatt/releases/tags/pr-${e}`}}function lo(){if(Xe())return"Controleren";let e=Vt();if(e)return e.phaseLabel;if(As())return"Bijgewerkt";if(lr())return"Bezig";if(Lo())return"Beschikbaar";let o=Hp();if(o!==null&&o<=0)return"Actueel";let r=Et();return typeof r.updateLabel=="string"&&r.updateLabel.trim()?r.updateLabel.trim():r.updateAvailable===!0?"Beschikbaar":r.updateAvailable===!1||Ms()?"Actueel":he()?"Nog niet gecontroleerd":"\u2014"}function he(){return t.entities.firmwareUpdate||null}function Sa(){let e=he();return e?String(e.state??e.value??"").trim().toLowerCase():""}function kb(){let e=t.entities.firmwareUpdateStatus;return e?String(e.state??e.value??"").trim():""}function qa(){return kb().toLowerCase()}function Ap(){let e=t.entities.firmwareUpdateProgress;if(!e)return Number.NaN;let o=Number(e.value??e.state);return Number.isNaN(o)?Number.NaN:Math.max(0,Math.min(100,o))}function xs(){let e=String(t.updateInstallTargetVersion||"").trim(),o=ae();return!e||!o?!1:nn(o,e)>=0}function xb(e=he()||{}){let o=co(e),r=ae(e);return!o||!r?!1:nn(r,o)>=0}function $s(){return(xs()||xb())&&!Xe()&&!on()&&!Lo()}function As(){return(t.updateInstallCompleted||$s())&&!Xe()&&!Vt()&&!Lo()}function ur(){t.updateInstallBusy=!1,t.updateInstallTargetVersion="",t.updateInstallPhaseHint="",t.updateInstallProgressHint=Number.NaN,t.updateInstallMode="",t.updateInstallTargetConnection="",t.updateInstallTargetTopology="",Nb()}function Ae(){t.updateManualUploadFile=null,t.updateManualUploadFileName="",t.updateManualUploadError=""}function Je(e={}){e.clearPr&&(t.updateTestFirmwarePr=""),t.updateTestFirmwareConfirmed=!1,t.updateTestFirmwareError="",t.updateTestFirmwareBuild=null}function $b(){let e=qa(),o=Ap();if(e==="starting"||e==="uploading"||e==="rebooting"){t.updateInstallPhaseHint=e,Number.isNaN(o)||(t.updateInstallProgressHint=e==="rebooting"?Math.max(o,100):o);return}if(t.updateInstallBusy){if(xs()){t.updateInstallPhaseHint="rebooting",t.updateInstallProgressHint=100;return}t.controlNotice.includes("opnieuw is opgestart")&&(t.updateInstallPhaseHint="rebooting",t.updateInstallProgressHint=100)}}function on(){let e=qa();return e==="starting"||e==="uploading"||e==="rebooting"}function Vt(){$b();let e=qa(),o=e==="starting"||e==="uploading"||e==="rebooting",r=o?e:t.updateInstallPhaseHint,n=Ap(),a=Number.isNaN(t.updateInstallProgressHint)?0:Math.round(t.updateInstallProgressHint),i=o&&!Number.isNaN(n)?Math.round(n):a;return!on()&&!t.updateInstallBusy?null:r==="rebooting"?{phaseLabel:"Herstarten",percent:Math.max(i,100),copy:t.updateInstallMode==="test-firmware"?"Testfirmware is geplaatst. Het device start opnieuw op en komt daarna vanzelf terug.":t.updateInstallMode==="connection-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna via de gekozen verbinding terug.":t.updateInstallMode==="topology-switch"?"Firmware is geplaatst. Het device start opnieuw op en komt daarna met de gekozen opstelling terug.":"Firmware is geplaatst. Het device start nu opnieuw op en komt daarna vanzelf terug."}:r==="uploading"?{phaseLabel:"Uploaden",percent:i,copy:t.updateInstallMode==="test-firmware"?`Testfirmware wordt nu door ${Fe()} gedownload en ge\xEFnstalleerd.`:t.updateInstallMode==="connection-switch"?`De ${Ye(t.updateInstallTargetConnection)}-build wordt nu naar ${Fe()} verzonden.`:t.updateInstallMode==="topology-switch"?`De ${or(t.updateInstallTargetTopology,t.updateInstallTargetConnection)}-build wordt nu naar ${Fe()} verzonden.`:`Firmware wordt nu naar ${Fe()} verzonden.`}:{phaseLabel:"Installeren",percent:i,copy:t.updateInstallMode==="test-firmware"?`Testfirmware-installatie is gestart voor ${Fe()}.`:t.updateInstallMode==="connection-switch"?`Verbindingswissel naar ${Ye(t.updateInstallTargetConnection)} is gestart.`:t.updateInstallMode==="topology-switch"?`Opstellingswissel naar ${Mo(t.updateInstallTargetTopology)} is gestart.`:`OTA-update is gestart voor ${Fe()}.`}}function co(e=he()||{}){let o=String(e.latest_version||"").trim();if(o)return o;let r=String(e.value||"").trim(),n=String(e.current_version||"").trim();return r&&r!==n&&/^v/i.test(r)?r:""}function ae(e=he()||{}){let o=String(t.entities.projectVersionText?.state||t.entities.projectVersionText?.value||"").trim();return o||String(e.current_version||"").trim()}function Ab(e=he()||{}){let o=ae(e),r=String(e.current_version||"").trim();return!o||!r||o===r?!1:nn(o,r)>0}function Hs(e=he()||{},o=so()){let r=String(o||"").trim().toLowerCase(),n=String(e.release_url||"").trim().toLowerCase(),a=co(e).toLowerCase();if(!r||r==="\u2014")return!0;if(r==="dev"){if(n){if(n.includes("/dev-latest"))return!0;if(a)return a.includes("-dev")}return a?a.includes("-dev"):!1}if(r==="main"){if(n){if(n.includes("/dev-latest"))return!1;if(a)return!a.includes("-dev")}return a?!a.includes("-dev"):!1}return!0}function kp(e){let r=String(e||"").trim().match(/^v?(\d+)\.(\d+)\.(\d+)(?:-([A-Za-z]+)(?:\.(\d+))?)?/);return r?{major:Number(r[1]),minor:Number(r[2]),patch:Number(r[3]),prereleaseTag:r[4]||"",prereleaseNumber:r[5]?Number(r[5]):null}:null}function nn(e,o){let r=kp(e),n=kp(o);if(!r||!n)return 0;if(r.major!==n.major)return r.major>n.major?1:-1;if(r.minor!==n.minor)return r.minor>n.minor?1:-1;if(r.patch!==n.patch)return r.patch>n.patch?1:-1;let a=!r.prereleaseTag,i=!n.prereleaseTag;return a!==i?a?1:-1:r.prereleaseTag!==n.prereleaseTag?r.prereleaseTag>n.prereleaseTag?1:-1:r.prereleaseNumber!==n.prereleaseNumber?(r.prereleaseNumber||0)>(n.prereleaseNumber||0)?1:-1:0}function lr(){if($s())return!1;let e=Sa();return t.updateInstallBusy||e==="installing"||e==="in_progress"||e==="updating"||e.includes("install")}function Xe(){let e=Sa();return t.updateCheckBusy||e==="checking"||e==="check"||e==="checking_for_update"||e.includes("checking")}function Lo(){let e=Sa();if(!Hs())return!1;let o=Hp();return o!==null?o>0:e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")?!1:e==="available"||e==="pending"||e.includes("available")?!0:Et().updateAvailable===!0}function Ms(){let e=Sa();return e==="installed"||e==="current"||e==="up_to_date"||e==="none"||e.includes("up to date")||e.includes("no update")||Ab()}function Rs(){let e=he()||{},o=ae(e)||"\u2014",r=Hs(e)?co(e):"",n=r?nn(r,o):null;return!Xe()&&n!==null&&n<=0&&(r=""),{current:o,latest:r||"\u2014"}}function Hp(){let{current:e,latest:o}=Rs();return e==="\u2014"||o==="\u2014"?null:nn(o,e)}function Mp(e=so()){let o=String(e||"").trim().toLowerCase();return mi[o]||mi.main}function Hb(){let e=String((he()||{}).release_url||"").trim(),o=Mp();return!e||o.includes("/dev-latest")&&!e.includes("/dev-latest")||!o.includes("/dev-latest")&&e.includes("/dev-latest")?o:e}function Mb(){return Fe()}function so(){return String(T("firmwareUpdateChannel")||t.entities.releaseChannelText?.state||t.entities.releaseChannelText?.value||"\u2014").trim()||"\u2014"}function Rb(){return Rs().latest!=="\u2014"}function xp(e){return String(e||"").toLowerCase().replace(/wi[\s-]?fi/g,"wifi").replace(/[^a-z0-9]+/g,"")}function Pb(e,o=he()||{}){let r=xp(e);return r?xp(`${o.title||""} ${o.summary||""}`).includes(r):!0}function Rp(e){return new Promise(o=>window.setTimeout(o,e))}function an(e=To){let r=Date.now()+e;t.firmwareOtaQuietUntil=Math.max(Number(t.firmwareOtaQuietUntil||0),r),t.pendingEntitySyncOptions=null,au(),typeof Ct=="function"&&Ct(),t.firmwareOtaQuietTimer&&window.clearTimeout(t.firmwareOtaQuietTimer),t.firmwareOtaQuietTimer=window.setTimeout(()=>{t.firmwareOtaQuietTimer=null,t.firmwareOtaQuietUntil=0,!t.updateInstallBusy&&!t.nativeOpen&&_i()},e)}function Nb(){t.firmwareOtaQuietTimer&&(window.clearTimeout(t.firmwareOtaQuietTimer),t.firmwareOtaQuietTimer=null),t.firmwareOtaQuietUntil=0,t.nativeOpen||_i()}function Pp(){return t.deviceReconnectMode?`
    ...
    `:""}function sn(e=so()){let o=he()||{},r=ae(o);t.entities.firmwareUpdate={...o,state:"CHECKING",value:"",latest_version:"",latestVersion:"",summary:"",release_url:Mp(e),current_version:r}}async function ln(e={}){let o=String(e.expectedBuildLabel||"").trim();for(let r=0;r<6;r+=1){await Rp(r===0?900:1200),await L(Fr,"all",{forceMissing:!0});let n=Hs(),a=!o||Pb(o),i=Rb(),l=Xe(),p=lo();if(n&&a&&(i||!l&&p!=="Nog niet gecontroleerd"))return!0}return!1}async function pr(e={}){let o=!1,r=Number.isFinite(Number(e.initialDelayMs))?Math.max(0,Number(e.initialDelayMs)):700,n=Number.isFinite(Number(e.pollDelayMs))?Math.max(250,Number(e.pollDelayMs)):1e3;for(let a=0;a<45;a+=1){await Rp(a===0?r:n);try{if(await L(Fr,"all",{forceMissing:!0}),qa()==="rebooting"&&Jt("ota"),d(),t.updateInstallMode==="connection-switch"){let i=fs(t.updateInstallTargetConnection);if(i&&ro()===i&&!on()&&!lr())return!0}else if(t.updateInstallMode==="topology-switch"){let i=en(t.updateInstallTargetTopology);if(i&&Z()===i&&!on()&&!lr())return!0}else if(xs()||$s()||Ms()&&!on()&&!lr())return!0}catch{o||(t.controlNotice="Wachten tot het device opnieuw is opgestart...",d(),o=!0)}}return!1}function Ob(){let e=so(),o=Vt();if(o)return o.copy;if(As()){let r=t.updateInstallCompletedVersion||ae()||so();return`${Fe()} draait nu op ${r}.`}return lr()?`OTA-update wordt voorbereid voor ${Fe()}. Het device kan kort herstarten.`:Xe()?`We controleren of er op kanaal ${e} een nieuwe firmware beschikbaar is.`:Lo()?"Er staat een nieuwere firmware klaar.":Ms()?`Je draait al de nieuwste firmware op kanaal ${e}.`:"Kies een kanaal en controleer of er een nieuwere firmware klaarstaat."}function Np(){return!!(t.firmwareAdvancedOpen||t.firmwareConnectionSwitchOpen||t.firmwareTopologySwitchOpen||t.updateManualUploadOpen||t.updateTestFirmwareOpen)}function ya(e,o,r,n,a=!1){return`
      <button
    ...
    `}function Db(e,o,r,n){if(!Np())return"";let i=!!(Vt()||t.updateInstallBusy||Xe());return`
    ...
    `}function Ib(){if(!t.updateTestFirmwareOpen)return"";let o=!!(Vt()||t.updateInstallBusy||Xe()),r=Do(),n=cr(),a=rn(r,n),i=!!(n.available&&h("firmwareTestOtaUrl")&&h("firmwareTestOtaMd5Url")&&h("installFirmwareTestOta")),l=!!(r&&i),p=t.updateTestFirmwareBuild||null,u=n.available?n.label:n.error,c=a?n.otaFileName:"Vul een PR-nummer in om de OTA-build te kiezen.";return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function _b(){if(!t.updateManualUploadOpen)return"";let o=!!(Vt()||t.updateInstallBusy||Xe()),r=String(t.updateManualUploadFileName||t.updateManualUploadFile?.name||"").trim();return`
      <div class="oq-firmware-advanced-detail">
        <div class="oq-firmware-advanced-detail-head">
    ...
    `}function Op(){if(!t.updateModalOpen)return"";let e=he(),o=t.entities.firmwareUpdateChannel||null,{current:r,latest:n}=Rs(),a=Xe(),i=lr(),l=Lo(),p=Ob(),u=Vt(),c=As(),m=Hb(),b=c?"Firmware-update afgerond":u||i?"Firmware-update bezig":a?"Controleren op firmware-update":Mb(),y=o?Array.isArray(o.option)?o.option:Array.isArray(o.options)?o.options:[]:[],w=va(),g=wa(),f=!!(w&&!c),v=!!(g&&!c);return`
    ...
    `}function kt(){return t.debugRecordingDeviceStatus?Math.max(0,Number(t.debugRecordingDeviceStatus.sample_count||0)):Array.isArray(t.debugRecordingSamples)?t.debugRecordingSamples.length:0}function xt(e=t.debugRecordingDeviceStatus){return e?.rolling===!0||String(e?.mode||"").toLowerCase()==="rolling"}function Ea(e=t.debugRecordingDeviceStatus){return xt(e)&&e?.frozen===!0&&!e?.active}function Fo(e){let o=Math.max(0,Math.round(Number(e||0)/1e3)),r=Math.floor(o/3600),n=Math.floor(o%3600/60),a=o%60;return r>0?`${r}u ${String(n).padStart(2,"0")}m`:n>0?`${n}m ${String(a).padStart(2,"0")}s`:`${a}s`}function Lp(){return t.debugRecordingDeviceStatus?Math.max(0,Number(t.debugRecordingDeviceStatus.retained_duration_s||0)*1e3):Fp()}function Fp(){if(t.debugRecordingDeviceStatus)return Math.max(0,Number(t.debugRecordingDeviceStatus.elapsed_s||0)*1e3);if(!t.debugRecordingStartedAt)return 0;let e=t.debugRecordingActive?Date.now():Number(t.debugRecordingLastSampleAt||Date.now());return Math.max(0,e-Number(t.debugRecordingStartedAt||e))}function cn(){return t.debugRecordingDeviceStatus&&t.debugRecordingDeviceStatus.available===!1?"Niet beschikbaar":Ea()?"Rolling gestopt":t.debugRecordingActive&&xt()?"Rolling actief":t.debugRecordingActive?"Bezig met opnemen":kt()>0?"Voltooid":"Niet gestart"}function Ca(){return Ea()?"Rolling debug is gestopt. De recente samples blijven bewaard tot je downloadt, kopieert, hervat of een nieuwe opname start.":t.debugRecordingActive&&xt()?"Rolling debug bewaart continu de recente samples. Download of kopieer maakt een momentopname; rolling blijft daarna doorlopen.":t.debugRecordingActive?"De opname loopt in apparaatgeheugen. Je kunt deze pagina sluiten en later het bestand downloaden.":kt()>0?"De opname is klaar. Download het supportbestand en voeg dit toe aan je supportverzoek.":t.debugRecordingDeviceStatus&&t.debugRecordingDeviceStatus.available===!1?"Debugopname in apparaatgeheugen is niet beschikbaar op deze firmware.":"Neem tijdelijk supportgegevens op voor analyse. De opname wordt lokaal in het apparaatgeheugen opgeslagen. Er wordt niets automatisch verzonden."}function Ip(){return Ea()?"Gestopt":t.debugRecordingActive&&xt()?`Rolling \xB7 ${Fo(Lp())}`:t.debugRecordingActive?`Loopt \xB7 ${Fo(Wp())}`:kt()>0?"Klaar":cn()}function Wb(){let e=Number(t.debugRecordingSelectedMinutes||15);return Gn.map(r=>Number(r.minutes)).includes(e)?e:Number(Gn[0]?.minutes||15)}function _p(e){t.debugRecordingActive||(t.debugRecordingSelectedMinutes=Math.max(1,Number(e)||15),t.debugRecordingNotice="",t.debugRecordingError="",d())}function Wp(){return xt()?0:t.debugRecordingDeviceStatus?Math.max(0,Number(t.debugRecordingDeviceStatus.remaining_s||0)*1e3):t.debugRecordingActive?Math.max(0,Number(t.debugRecordingEndsAt||0)-Date.now()):0}function Vb(){if(t.debugRecordingDeviceStatus){if(xt()){let a=Math.max(1,Number(t.debugRecordingDeviceStatus.sample_capacity||0));return Math.max(0,Math.min(100,kt()/a*100))}let r=Math.max(1,Number(t.debugRecordingDeviceStatus.duration_s||0)),n=Math.max(0,Number(t.debugRecordingDeviceStatus.elapsed_s||0));return!t.debugRecordingActive&&kt()>0?100:Math.max(0,Math.min(100,n/r*100))}if(!t.debugRecordingActive||!t.debugRecordingStartedAt||!t.debugRecordingEndsAt)return kt()>0?100:0;let e=Math.max(1,Number(t.debugRecordingEndsAt)-Number(t.debugRecordingStartedAt)),o=Math.max(0,Date.now()-Number(t.debugRecordingStartedAt));return Math.max(0,Math.min(100,o/e*100))}function Vp(e=t.debugRecordingDeviceStatus){return String(e?.recording_id??e?.recording?.recording_id??"").trim()}function Bp(e){if(e?.recording?.active)return;let o=Vp(e);if(o){t.debugRecordingAcknowledgedId=o;try{window.localStorage.setItem("oq-debug-recording-acknowledged-id",o)}catch{}}}function Ps(){let e=t.debugRecordingDeviceStatus,o=Math.max(0,Number(e?.sample_count||0));if(!e||e.available===!1||!e.active&&o===0)return"";let r=!!e.active;if(!r&&Vp(e)===t.debugRecordingAcknowledgedId)return"";let n=xt(e),a=Fo(Math.max(0,Number(e.retained_duration_s||0))*1e3),i=Fo(Math.max(0,Number(e.remaining_s||0))*1e3),l=r?n?`Rolling debug \xB7 ${a}`:`Debug loopt \xB7 ${i}`:n?"Rolling gestopt":"Debug klaar",p=r?n?`Rolling debug loopt, laatste ${a} beschikbaar`:`Debugopname loopt, nog ${i}`:n?"Rolling debug gestopt; recente buffer klaar om te downloaden":"Debugopname klaar om te downloaden";return`
    <button
    ...
    <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
      <section class="oq-helper-modal oq-debug-recording-modal" role="dialog" aria-modal="true" aria-labelledby="oq-debug-recording-modal-title">
        ...
        <div class="oq-debug-recording-actions">
          ...
          `}
          <button class="oq-helper-button oq-helper-button--ghost" type="button" data-oq-action="download-debug-recording" ${!b||a?"disabled":""}>${Bt("download")}${e&&o?"Download tot nu toe":"Download supportbestand"}</button>
          <button class="oq-helper-button oq-helper-button--ghost" type="button" data-oq-action="copy-debug-recording" ${!b||a?"disabled":""}>${Bt("copy")}${e&&o?"Kopieer tot nu toe":"Kopieer data"}</button>
          ${g?`
  ...
  `}function Xp(e){t.overviewTheme=e==="dark"?"dark":"light";try{window.localStorage.setItem("oq-overview-theme",t.overviewTheme)}catch{}}function xa(e){t.interfacePanelOpen=e===!0}function Ds(e){try{window.localStorage.setItem("oq-active-surface",e==="native"?"native":"app")}catch{}}function dn(e,o={}){t.settingsGroup=Re.has(e)?e:de[0].id;try{window.localStorage.setItem("oq-settings-group",t.settingsGroup)}catch{}o.syncUrl!==!1&&t.appView==="settings"&&Bi(o.syncMode||"replace")}function ed(e){t.devPanelOpen=e===!0;try{window.localStorage.setItem("oq-dev-panel-open",t.devPanelOpen?"true":"false")}catch{}}function td(e){t.hpVisualMode=e==="compact"?"compact":"schematic";try{window.localStorage.setItem("oq-hp-visual-mode",t.hpVisualMode)}catch{}}function od(e){t.hpLayoutMode=e==="focus-hp1"||e==="focus-hp2"?e:"equal";try{window.localStorage.setItem("oq-hp-layout-mode",t.hpLayoutMode)}catch{}}function ey(e){if(t.reducedMotion=!!e?.matches,t.reducedMotion){zr();return}Gr()}function ty(){let e=xi();!e||t.motionPreferenceMedia===e||(t.motionPreferenceMedia=e,t.motionPreferenceListener=ey,typeof e.addEventListener=="function"?e.addEventListener("change",t.motionPreferenceListener):typeof e.addListener=="function"&&e.addListener(t.motionPreferenceListener),t.reducedMotion=!!e.matches)}function oy(){return Object.keys(t.entities).length>0}function ry(){return ki+Math.floor(Math.random()*(xc-ki+1))}function ny(){return(document.hidden?Wr:wc)+ry()}function rd(e=ny()){t.pollTimer||t.nativeOpen||t.updateInstallBusy||aa()||(t.pollTimer=window.setTimeout(async()=>{t.pollTimer=null,await se(),rd()},e))}function Ls(){rd()}function Fs(){t.pollTimer&&(window.clearTimeout(t.pollTimer),t.pollTimer=null)}nu({start:Ls,stop:Fs});function ay(){t.nativeOpen||(Fs(),Ls(),document.hidden||se({forceProbe:!0}))}function $a(e={}){if(mn(),t.nativeOpen){Fs(),zr(),t.nativeFrontendLoaded||Aa();return}if(Gr(),Ls(),e.refresh!==!1){if(!oy()){Zu();return}se({forceFast:!0})}}function iy(){let e=Yr()||eo(),o=e==="settings"?Vi()||t.settingsGroup:"";if(!(e===t.appView&&(e!=="settings"||o===t.settingsGroup))){if(t.appView=e,e==="settings"&&o){t.settingsGroup=o;try{window.localStorage.setItem("oq-settings-group",t.settingsGroup)}catch{}}d(),se({forceFast:!0})}}function mn(){t.nativeApp&&(t.nativeApp.classList.add("oq-native-app"),t.nativeApp.classList.toggle("oq-native-app--collapsed",!t.nativeOpen),t.nativeApp.setAttribute("aria-hidden",t.nativeOpen?"false":"true"))}function nd(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Jp,{once:!0}):Jp(),window.addEventListener("pointermove",du),window.addEventListener("pointerup",mu),window.addEventListener("popstate",iy),window.addEventListener("oq-mock-updated",sy),window.addEventListener("oq-dev-controls-changed",ly),document.addEventListener("visibilitychange",ay)}function sy(){t.mounted&&se()}function ly(){t.mounted&&d()}function Jp(){cy();let e=document.querySelector("esp-app");e||(e=document.createElement("esp-app"),document.body.appendChild(e)),t.nativeApp=e,t.nativeFrontendLoaded=Array.from(document.scripts).some(o=>o.src===gi),t.mounted||(uy(e),t.mounted=!0,$a()),mn(),t.nativeOpen||(se(),pn({silent:!0}))}function cy(){if(!document.head)return;let e=document.head.querySelector('meta[name="viewport"]');e||(e=document.createElement("meta"),e.name="viewport",document.head.appendChild(e)),e.setAttribute("content","width=device-width, initial-scale=1")}function uy(e){let o=document.createElement("section");o.id="oq-helper-root",o.lang="nl-NL",document.documentElement&&!document.documentElement.lang&&(document.documentElement.lang="nl-NL"),e.parentNode.insertBefore(o,e),o.addEventListener("click",lu),o.addEventListener("change",su),o.addEventListener("input",cu),o.addEventListener("keydown",uu),o.addEventListener("wheel",fu,{passive:!1}),o.addEventListener("focusin",Wi),o.addEventListener("focusout",Wi),o.addEventListener("mouseover",hu),o.addEventListener("mouseout",gu),o.addEventListener("pointerdown",pu),t.root=o,ty();let r=Yr()||eo(),n=r==="settings"?Vi():"";n&&dn(n,{syncUrl:!1}),ct(r,{syncMode:"replace",forceSync:!0}),Ur(),d()}function py(e){return new Promise((o,r)=>{if(!e){o();return}let n=Array.from(document.scripts).find(i=>i.src===e);if(n){if(n.dataset.loaded==="true"){o();return}n.addEventListener("load",()=>o(),{once:!0}),n.addEventListener("error",i=>r(i),{once:!0});return}let a=document.createElement("script");a.src=e,a.async=!1,a.addEventListener("load",()=>{a.dataset.loaded="true",o()},{once:!0}),a.addEventListener("error",i=>r(i),{once:!0}),document.head.appendChild(a)})}async function Aa(){if(!(t.nativeFrontendLoaded||t.nativeFrontendLoading)){t.nativeFrontendLoading=!0,t.nativeOpen&&d();try{await py(gi),t.nativeFrontendLoaded=!0}catch(e){t.controlError=`ESPHome fallback kon niet worden geladen. ${e.message||e}`,t.nativeOpen=!1,Ds("app"),d(),$a()}finally{t.nativeFrontendLoading=!1,t.nativeOpen&&d()}}}function Is(){if(!t.root)return;let e=typeof window<"u"?window.__OQ_DEV_CONTROLS__:null;!e||typeof e.bind!="function"||e.bind(t.root)}function gn(){let e=t.authStatus;return e?e.enabled?e.setup_window_active?"Instelvenster":"Beveiligd":"Onbeveiligd":"Laden..."}function dy(){return"Login"}function my(){let e=t.authStatus;return e?e.enabled?"De web-app vraagt nu een login voordat beheer beschikbaar is. Je kunt die hier aanpassen of uitzetten.":"De web-app staat open op je netwerk. Houd de herstelknop 5 seconden vast om een login toe te voegen.":"We halen de huidige loginstatus op."}function hn(){let e=t.authStatus;return e?e.enabled?e.setup_window_active?"Login actief. Tijdelijk instelvenster is open.":`Login actief${e.source?` via ${e.source}`:""}.`:e.setup_window_active?"Login uit. Tijdelijk instelvenster is open.":"Login uit. Webtoegang is open / onbeveiligd op het netwerk.":"Logingegevens laden..."}function mr(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?"Herstart nodig":e.transport_active===!0?"Aan":e.transport_active===!1?"Uit":e.enabled?"Aan":"Uit":"Laden..."}function gr(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API beveiligd.":e.key?"Deze wijziging wordt actief na herstart. De sleutel blijft opgeslagen voor later gebruik.":"Deze wijziging wordt actief na herstart.":e.transport_active===!0?"API-encryptie staat aan. Gebruik dezelfde sleutel in Home Assistant.":"De native API staat nu open op je lokale netwerk.":"API-encryptie wordt geladen."}function gy(){return"ESPHome API encryption"}function hy(){let e=t.apiSecurityStatus;return e?e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active?e.enabled===!0&&e.transport_active===!1?"API-encryptie wordt ingeschakeld na herstart. Kopieer de sleutel nu alvast voor Home Assistant.":e.enabled===!1&&e.transport_active===!0?"API-encryptie wordt uitgeschakeld na herstart. Tot die tijd blijft de native API nog beveiligd.":"Deze wijziging wordt actief na herstart. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":e.transport_active===!0?"De native API is beveiligd. Je kunt de sleutel hier bekijken, kopi\xEBren of vernieuwen.":"API-encryptie staat uit. Schakel in om een sleutel te bekijken, kopi\xEBren of vernieuwen.":"We halen de huidige API-beveiliging op."}function fy(){let e=t.apiSecurityStatus;if(!e)return"Laden...";let o=!!(e.pending_restart||typeof e.enabled=="boolean"&&typeof e.transport_active=="boolean"&&e.enabled!==e.transport_active);return o&&e.enabled===!0&&e.transport_active===!1?"Annuleer inschakelen":o&&e.enabled===!1&&e.transport_active===!0?"Annuleer uitschakelen":e.enabled?"Uitschakelen":"Inschakelen"}function by(){let e=t.apiSecurityStatus;return e?e.key?"Vernieuw sleutel":"Genereer sleutel":"Laden..."}function ad(e,o,r="",n=!1){return`
    ...
    `}function fn(e,o,r){let n=A(o),a=A(r);if(Number.isFinite(n)&&Number.isFinite(a))return n-a;let i=A(e);return Number.isFinite(i)?i:NaN}function _s(e="oq-settings-grid"){let o=yy();return`
      <div class="${s(e)}">
      ...
      ${o}
    `}function yy(){let e=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",offsetKey:"hp1WaterInOffset",finalKey:"hp1WaterIn"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",offsetKey:"hp1WaterOutOffset",finalKey:"hp1WaterOut"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",offsetKey:"hp2WaterInOffset",finalKey:"hp2WaterIn"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",offsetKey:"hp2WaterOutOffset",finalKey:"hp2WaterOut"}].filter(r=>h(r.offsetKey)&&h(r.finalKey));if(!e.length)return"";let o=r=>{let n=Oe(r.offsetKey),a=fn(r.rawKey,r.finalKey,r.offsetKey),i=xe(Te(r.offsetKey)),l=Number.isFinite(a)&&Number.isFinite(i)?Lt(a+i,n.uom||"\xB0C",2):ve(r.finalKey,2);return`
        <article class="oq-settings-hp-offset-row" data-oq-settings-field="${s(r.offsetKey)}" data-oq-hp-offset-raw-key="${s(r.rawKey)}" data-oq-hp-offset-final-key="${s(r.finalKey)}">
          ...
          <div class="oq-settings-hp-offset-equation" aria-label="${s(`${r.label} correctie`)}">
            <div class="oq-settings-hp-offset-readout">
              <span>Raw</span>
              <strong data-oq-hp-offset-raw>${s(Number.isFinite(a)?Lt(a,n.uom||"\xB0C",2):ve(r.rawKey,2))}</strong>
            </div>
            <span class="oq-settings-hp-offset-operator">+</span>
            <label class="oq-settings-hp-offset-input">
              <span>Correctie</span>
              ${no({key:r.offsetKey,value:Te(r.offsetKey),meta:n,controlClass:"oq-helper-control oq-helper-control--suffix",inputClass:"oq-helper-input oq-helper-input--compact-number",unitMarkup:n.uom?`<span class="oq-helper-unit-chip">${s(n.uom)}</span>`:""})}
            </label>
            <span class="oq-settings-hp-offset-operator">=</span>
    ...
    `}function wy({status:e,running:o,resultReady:r,startDisabled:n,abortDisabled:a,applyDisabled:i,busy:l,controlsAvailable:p}){let u=String(e||"").toUpperCase(),c=u.includes("FAILED")||u.includes("REFUSED")||u.includes("ABORT"),m=u.includes("APPLIED"),b=h("hp2WaterIn")||h("hp2WaterOut")||h("hp2WaterInRaw")||h("hp2WaterOutRaw"),y=A("hpWaterCalibrationStableProgress"),w=A("hpWaterCalibrationStableRequired"),g=A("hpWaterCalibrationRemaining"),f=Math.round(A("hpWaterCalibrationPhase")),v=o&&(f===1||u.includes("MIXING")),S=o&&!v,q=300,C=60,x=Number.isFinite(g)?Math.max(0,q-g):NaN,M=Number.isFinite(x)?Math.max(0,C-x):NaN,V=v&&Number.isFinite(x)?Math.max(0,Math.min(100,x/C*100)):S&&Number.isFinite(y)&&Number.isFinite(w)&&w>0?Math.max(0,Math.min(100,y/w*100)):o&&Number.isFinite(g)?Math.max(0,Math.min(100,100-g/q*100)):r?100:0,F=r&&h("hpWaterCalibrationResultSpreadBefore")?ve("hpWaterCalibrationResultSpreadBefore",2):ve("hpWaterCalibrationSpread",2),ee=v?"Water mengen":Number.isFinite(y)&&Number.isFinite(w)&&w>0?y>0?`${Math.round(Math.max(0,y))} / ${Math.round(w)} s binnen grenzen`:"Nog niet binnen grenzen":"Wachten op stabiel venster",H=r?3:o?2:1,G=m?"Offsets toegepast":r?`Meting klaar - spreiding ${F}`:o?v?`Water mengen${Number.isFinite(M)&&M>0?` - meting start over ${Math.round(M)} s`:""}`:`Meting bezig - ${Number.isFinite(g)&&g>0?`max. ${Math.round(g)} s resterend`:ee}`:c?"Meting niet voltooid":"Voorbereiding",X=m?"De voorgestelde offsets zijn opgeslagen.":r?"Controleer de voorgestelde offsets en pas ze toe.":o?v?"De waterpomp circuleert zonder compressor zodat de watertemperaturen eerst kunnen mengen.":"De firmware stopt zodra het laatste meetvenster binnen de spreiding- en driftgrenzen valt.":c?Ro("hpWaterCalibrationStatus","Controleer de voorwaarden en start opnieuw."):b?"Start alleen wanneer compressor en boiler uit zijn. HP1 en HP2 water in/out worden samen naar een relatieve referentie gebracht.":"Start alleen wanneer compressor en boiler uit zijn. Bij single setup wordt HP1 water in/out onderling gelijkgetrokken; supply blijft diagnose.",ue=[{label:"HP1 water in",rawKey:"hp1WaterInRaw",liveKey:"hp1WaterIn",resultRawKey:"hpWaterCalibrationResultHp1InRawAvg",offsetKey:"hp1WaterInOffset",suggestedKey:"hp1WaterInOffsetSuggested"},{label:"HP1 water uit",rawKey:"hp1WaterOutRaw",liveKey:"hp1WaterOut",resultRawKey:"hpWaterCalibrationResultHp1OutRawAvg",offsetKey:"hp1WaterOutOffset",suggestedKey:"hp1WaterOutOffsetSuggested"},{label:"HP2 water in",rawKey:"hp2WaterInRaw",liveKey:"hp2WaterIn",resultRawKey:"hpWaterCalibrationResultHp2InRawAvg",offsetKey:"hp2WaterInOffset",suggestedKey:"hp2WaterInOffsetSuggested"},{label:"HP2 water uit",rawKey:"hp2WaterOutRaw",liveKey:"hp2WaterOut",resultRawKey:"hpWaterCalibrationResultHp2OutRawAvg",offsetKey:"hp2WaterOutOffset",suggestedKey:"hp2WaterOutOffsetSuggested"}].filter(N=>h(N.liveKey)||h(N.rawKey)||h(N.offsetKey)),ie=(N,I)=>{let z=H>N;return`
      ...
      `,Q=N=>{let I=A(N.resultRawKey),z=Number.isFinite(I)?I:fn(N.rawKey,N.liveKey,N.offsetKey),We=A(N.suggestedKey),ht=Number.isFinite(z)&&Number.isFinite(We)?Lt(z+We,t.entities[N.suggestedKey]?.uom||"\xB0C",2):"\u2014";return`
        <tr>
          <th scope="row">${s(N.label)}</th>
          <td>${s(Number.isFinite(z)?Lt(z,t.entities[N.liveKey]?.uom||"\xB0C",2):"\u2014")}</td>
    ...
    `}function ud(){let e=h("boilerCvAssistEnabled")&&k("boilerCvAssistEnabled"),o=da(),r=k("cm100Active"),n=String(o||"").trim().toUpperCase(),a=_o(o),i=!a&&(r||n==="CM100 READY"),l=t.commissioningTaskLock==="cm100",p=t.loadingEntities||t.busyAction==="commissioningCm100Start"||t.busyAction==="commissioningCm100Stop"||l,u=!!t.pendingCommissioningCm100Start,c=p||i||a,m=p||!i,b=Po("boilerPowerTestStatus","IDLE"),y=Ha(b,"boiler"),w=k("boilerPowerTestActive"),g=t.loadingEntities||t.busyAction==="boilerPowerTestStart"||t.busyAction==="boilerPowerTestAbort"||t.busyAction==="boilerPowerTestApply",f=!!(t.entities.boilerPowerTestStart||t.entities.boilerPowerTestAbort||t.entities.boilerPowerTestApply),v=!!t.pendingBoilerPowerTestStart,S=t.commissioningTaskLock==="boiler",q=_o(b),x=!we(b)&&(w||v||S||hr(b))&&!q,M=Ce("boilerRatedHeatPower"),V=Ce("boilerHeatPower"),F=A("boilerHeatPower"),ee=F>0?V:x&&t.commissioningBoilerHeatPowerDisplay?t.commissioningBoilerHeatPowerDisplay:V;F>0&&(t.commissioningBoilerHeatPowerDisplay=V);let H=Po("flowAutotuneStatus","IDLE"),G=Ha(H,"autotune"),X=t.loadingEntities||t.busyAction==="flowAutotuneStart"||t.busyAction==="flowAutotuneAbort"||t.busyAction==="flowAutotuneApply",ue=!!(t.entities.flowAutotuneStart||t.entities.flowAutotuneAbort||t.entities.flowAutotuneApply),ie=!!t.pendingFlowAutotuneStart,O=t.commissioningTaskLock==="autotune",Q=_o(H),I=!we(H)&&(ie||O||hr(H))&&!Q,z=Po("airPurgeStatus","IDLE"),We=Ha(z,"purge"),ht=k("airPurgeActive"),rt=t.loadingEntities||t.busyAction==="airPurgeStart"||t.busyAction==="airPurgeAbort",ft=!!(t.entities.airPurgeStart||t.entities.airPurgeAbort),nt=!!t.pendingAirPurgeStart,E=t.commissioningTaskLock==="purge",P=!we(z)&&(ht||nt||E||hr(z)),j=/DONE/.test(String(z||"").toUpperCase()),Y=!!(ft||t.entities.airPurgeStatus||t.entities.airPurgeReturnToAuto),ne=Ce("airPurgeRemaining",{decimals:0}),le=A("airPurgePhase"),Ne=le===1?"Rustig":le===2?"Pulsen":le===3?"Stabiliseren":We.phase,fe=Po("manualFlowStatus","IDLE"),He=k("manualFlowActive"),pe=t.loadingEntities||t.busyAction==="manualFlowStart"||t.busyAction==="manualFlowAbort",be=!!(t.entities.manualFlowStart||t.entities.manualFlowAbort),Ht=!!t.pendingManualFlowStart,je=t.commissioningTaskLock==="manual-flow",Se=!we(fe)&&(He||Ht||je||hr(fe)),Me=Po("manualHpStatus","IDLE"),at=k("manualHpActive"),bt=t.loadingEntities||t.busyAction==="manualHpStart"||t.busyAction==="manualHpAbort",ke=!!(t.entities.manualHpStart||t.entities.manualHpAbort),Uo=!!t.pendingManualHpStart,yt=t.commissioningTaskLock==="manual-hp",Ue=!we(Me)&&(at||Uo||yt||hr(Me)),Nn=/SAFETY STOP/.test(String(Me||"").toUpperCase()),On=/STOPPING/.test(String(Me||"").toUpperCase()),vt=Po("hpWaterCalibrationStatus","IDLE"),ci=Ha(vt,"hp-water-calibration"),Dn=k("hpWaterCalibrationActive"),Go=t.loadingEntities||t.busyAction==="hpWaterCalibrationStart"||t.busyAction==="hpWaterCalibrationAbort"||t.busyAction==="hpWaterCalibrationApply",nc=!!(t.entities.hpWaterCalibrationStart||t.entities.hpWaterCalibrationAbort||t.entities.hpWaterCalibrationApply),Ln=!!t.pendingHpWaterCalibrationStart,Gt=t.commissioningTaskLock==="hp-water-calibration",Ve=!we(vt)&&(Dn||Ln||Gt||hr(vt)),Fn=/DONE|APPLIED/.test(String(vt||"").toUpperCase()),ac=/APPLIED/.test(String(vt||"").toUpperCase()),ah=Ce("flowKpSuggested",{decimals:5,trimTrailingZeros:!0}),ih=Ce("flowKiSuggested",{decimals:5,trimTrailingZeros:!0}),ic=/DONE|APPLIED/.test(String(b||"").toUpperCase()),sc=/DONE|APPLIED/.test(String(H||"").toUpperCase()),ui=i?q?"Wachten op CM100":x?y.phase:ic?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",pi=i?Q?"Wachten op CM100":I?G.phase:sc?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",di=i?P?We.phase:j?"Klaar":"Klaar om te starten":"Wachten op CM100",lc=i?Se?"Actief":"Klaar om te starten":"Wachten op CM100",cc=i?Ue?On?"Bezig met stoppen":Nn?"Veiligheidsstop":"Actief":"Klaar om te starten":"Wachten op CM100",uc=i?Ve?ci.phase:ac?"Offsets toegepast":Fn?"Klaar om toe te passen":"Klaar om te starten":"Wachten op CM100",pc=!i||g||!f||I||P||Se||Ue||Ve||x||O||E||je||yt||Gt||v,sh=g||!(x||S||v),lh=g||pc||!ic||I||P||Ve,dc=!i||X||!ue||x||P||Se||Ue||Ve||I||S||E||je||yt||Gt||ie,ch=X||!(I||O||ie),uh=X||dc||!sc||x||P||Ve,ph=!i||rt||!ft||x||I||Se||Ue||Ve||P||S||O||je||yt||Gt||nt,dh=rt||!(P||E||nt),mh=!i||pe||!be||x||I||P||Ue||Ve||Se||S||O||E||yt||Gt||Ht,gh=pe||!(Se||je||Ht),hh=!i||bt||!ke||x||I||P||Se||Ve||Ue||S||O||E||je||Gt||Uo,fh=bt||!(Ue||yt||Uo),bh=!i||Go||!nc||x||I||P||Se||Ue||Ve||S||O||E||je||yt||Ln,yh=Go||!(Ve||Gt||Ln),vh=Go||Ve||!Fn||ac;u&&i&&(t.pendingCommissioningCm100Start=!1),l&&(i||/READY|STOPPED|DONE|FAILED|ABORT|APPLIED|REFUSED/.test(n))&&(t.commissioningTaskLock=""),v&&(w||we(b))&&(t.pendingBoilerPowerTestStart=!1),S&&we(b)&&(t.commissioningTaskLock=""),ie&&we(H)&&(t.pendingFlowAutotuneStart=!1),O&&we(H)&&(t.commissioningTaskLock=""),nt&&(ht||we(z))&&(t.pendingAirPurgeStart=!1),E&&we(z)&&(t.commissioningTaskLock=""),Ht&&(He||we(fe))&&(t.pendingManualFlowStart=!1),je&&(He||we(fe))&&(t.commissioningTaskLock=""),Uo&&(at||we(Me))&&(t.pendingManualHpStart=!1),yt&&(at||we(Me))&&(t.commissioningTaskLock=""),Ln&&(Dn||we(vt))&&(t.pendingHpWaterCalibrationStart=!1),Gt&&we(vt)&&(t.commissioningTaskLock="");let wh=a?"Wachten op CM100":o,Sh=a?"Service-stand wordt geopend. Wacht tot CM100 klaar staat.":i?"CM100 is actief en klaar voor service-taken.":"Start de service-stand voordat je een taak uitvoert.",qh=[{key:"hp-water-calibration",title:"Temperatuursensoren kalibreren",label:"Sensor kalibratie",summary:"Laat de waterpomp draaien zonder compressor en bepaal offsets voor HP1/HP2 water in/out.",status:uc,available:!!(nc||t.entities.hpWaterCalibrationStatus),openDisabled:!i,cardMarkup:fr({taskKey:"hp-water-calibration",title:"Temperatuursensoren kalibreren",copy:"Doorloop voorbereiding, meting en toepassen in vaste volgorde. De meting stopt eerder zodra de sensoren stabiel genoeg zijn.",subcopy:"De voorgestelde waarden worden pas actief wanneer je ze toepast; supply blijft een diagnosewaarde.",status:uc,statusCopy:Ve?"De pomp draait en de firmware wacht op een stabiel temperatuurbeeld.":Fn?"Controleer de voorgestelde offsets voordat je ze toepast.":i?"CM100 staat klaar. Start de meting wanneer compressor en boiler uit zijn.":"Start CM100 eerst.",progressTask:"hp-water-calibration",controls:wy({status:vt,running:Ve,resultReady:Fn,startDisabled:bh,abortDisabled:yh,applyDisabled:vh,busy:Go,controlsAvailable:!!(t.entities.hpWaterCalibrationStart||t.entities.hpWaterCalibrationAbort)}),className:"oq-settings-commissioning-card--hp-water-calibration"})},{key:"manual-flow",title:"Handmatige flowregeling",label:"Handmatige flow",summary:"Laat de waterpomp draaien op een tijdelijk flow-setpoint en luister naar het leidingwerk.",status:lc,available:!!(be||t.entities.manualFlowStatus),openDisabled:!i,cardMarkup:fr({taskKey:"manual-flow",title:"Handmatige flowregeling",copy:"Gebruik een tijdelijk flow-setpoint om het leidingwerk rustig te controleren. De normale instellingen wijzigen pas wanneer je een waarde bewust overneemt.",subcopy:"De bestaande PI-regeling blijft de pomp aansturen.",status:lc,statusCopy:Se?"De waterpomp draait. Pas het tijdelijke setpoint aan en controleer de gemeten flow.":i?"CM100 staat klaar. Kies een tijdelijk setpoint en start de waterpomp.":"Start CM100 eerst.",progressTask:"",controls:`
        ...
        `},{key:"manual-hp",title:"Handmatige warmtepompbediening",label:"Handmatige warmtepomp",summary:"Selecteer een werkmodus en vraag per warmtepomp een compressorstand aan binnen de bestaande bewaking.",status:cc,available:!!(ke||t.entities.manualHpStatus),openDisabled:!i,cardMarkup:fr({taskKey:"manual-hp",title:"Handmatige warmtepompbediening",copy:"Start eerst de service-taak zodat de waterpomp draait. Zodra voldoende flow is gemeten kun je per warmtepomp vanuit Standby naar verwarmen of koelen schakelen en daarna een compressorstand aanvragen.",subcopy:"Low-flow, maximale watertemperatuur, minimum draaitijd, minimum uit-tijd en veilige modusovergangen blijven actief. De koelvloer, silent-modus, dag/nacht-cap en normaal uitgesloten compressorstanden worden voor deze handmatige test bewust genegeerd.",status:cc,statusCopy:Ue?On?"De compressorvraag staat op 0. De waterpomp blijft draaien totdat de minimale draaitijd veilig is afgerond.":Nn?"De bewaking heeft de aangevraagde standen teruggezet naar 0. Controleer de oorzaak voordat je opnieuw opschaalt.":"De service-taak is actief. Een veiligheidsstop zet de aangevraagde standen terug naar 0; opnieuw opschalen vereist een bewuste handeling.":i?"CM100 staat klaar. Start de taak om handmatige warmtepompbediening vrij te geven.":"Start CM100 eerst.",progressTask:"",actions:`
          ...
          `,metrics:`
            ...
            <div class="oq-settings-manual-hp-results">
              ${$e("flowSelected","Gemeten flow","Actuele doorstroming in het watercircuit.",Ce("flowSelected"),"oq-settings-field--compact")}
              ${$e("hp1Compressor","Warmtepomp 1 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",cd("hp1Compressor","hp1Freq"),"oq-settings-field--compact")}
              ${h("hp2Compressor")?$e("hp2Compressor","Warmtepomp 2 actueel","Door de actuator werkelijk toegepaste compressorstand en gemeten compressorfrequentie.",cd("hp2Compressor","hp2Freq"),"oq-settings-field--compact"):""}
            </div>
            ${$e("manualHpGuardStatus","Bewaking","Toont waarom een handmatig verzoek tijdelijk niet of nog niet volledig wordt toegepast.",T("manualHpGuardStatus")||"Vrijgegeven","oq-settings-field--compact oq-settings-field--full")}
            <div class="oq-settings-manual-hp-statuses">
              ${$e("hp1Failures","Warmtepomp 1 statusmelding","Actuele melding die de warmtepomp zelf rapporteert.",to(D("hp1Failures","None")),"oq-settings-field--compact")}
              ${h("hp2Failures")?$e("hp2Failures","Warmtepomp 2 statusmelding","Actuele melding die de warmtepomp zelf rapporteert.",to(D("hp2Failures","None")),"oq-settings-field--compact"):""}
            </div>
          `})},{key:"autotune",title:"Flow autotune",label:"Autotune",summary:"Berekent een voorstel voor de flowregeling en kan Kp/Ki daarna toepassen.",status:pi,available:!0,openDisabled:_o(pi),cardMarkup:fr({taskKey:"autotune",title:"Flow autotune",copy:"Bereken een voorstel voor de flowregeling en pas dat daarna toe in de installatie-instellingen. Autotune duurt meestal ongeveer 5 tot 10 minuten.",subcopy:"Na toepassen worden de flow-instellingen bijgewerkt.",status:pi,statusCopy:Q?"Wacht totdat CM100 actief is voordat je autotune start.":I?"Autotune draait op dit moment.":i?"CM100 staat klaar. Start de autotune wanneer je wilt.":"Start CM100 eerst en voer daarna autotune uit.",progressTask:"autotune",actions:`
          ...
          `,metrics:`
            ${$e("flowKpSuggested","Voorgestelde Kp","Kp bepaalt hoe sterk de regeling meteen corrigeert.",ah,"oq-settings-field--compact")}
            ${$e("flowKiSuggested","Voorgestelde Ki","Ki corrigeert kleine afwijkingen langzaam weg.",ih,"oq-settings-field--compact")}
          `})},{key:"boiler",title:"Boiler power test",label:"Boiler test",summary:"Meet het effectieve boilervermogen bij stabiele flow en kan het resultaat toepassen.",status:ui,available:e,openDisabled:_o(ui),cardMarkup:fr({taskKey:"boiler",title:"Boiler power test",copy:"Meet het effectieve boilervermogen bij stabiele flow en schrijf daarna een afgerond voorstel weg naar de boilerinstelling. Boilertest duurt meestal ongeveer 5 tot 10 minuten.",subcopy:`Ingesteld boilervermogen: ${s(M)}`,status:ui,statusCopy:q?"Wacht totdat CM100 actief is voordat je de boiler-test start.":x?"De boiler-test draait op dit moment.":i?"CM100 staat klaar. Start de boiler-test wanneer je wilt.":"Start CM100 eerst en voer daarna de boilervermogentest uit.",progressTask:"boiler",actions:`
            ${t.entities.boilerPowerTestStart||t.entities.boilerPowerTestAbort?No({active:x,startKey:"boilerPowerTestStart",stopKey:"boilerPowerTestAbort",startLabel:"Boiler test starten",stopLabel:"Boiler test stoppen",startDisabled:g||pc,stopDisabled:g||sh}):""}
            ${t.entities.boilerPowerTestApply?Ke("boilerPowerTestApply","Toepassen","oq-helper-button oq-helper-button--ghost",g||lh):""}
          `,metrics:`
            ${$e("boilerHeatPower","Actueel vermogen","Live meting tijdens de boiler-test.",ee)}
            ${$e("boilerPowerTestResult","Gemeten testresultaat","Afgerond resultaat van de laatste boiler-test.",Ce("boilerPowerTestResult"))}
          `})},{key:"purge",title:"Ontluchten",label:"Ontluchten",summary:"Draait een vaste ontluchtingsrun van 5 minuten met rustige flow, pomp-pulsen en stabilisatie.",status:di,available:Y,openDisabled:_o(di),cardMarkup:fr({taskKey:"purge",title:"Ontluchten",copy:"Draait 5 minuten met rustige doorstroming, korte pomp-pulsen en een stabilisatiefase.",subcopy:"Na afloop kan OpenQuatt de service mode (CM100) afsluiten of actief laten.",status:di,statusCopy:P?"Ontluchten loopt vast 5 minuten door en stopt daarna automatisch.":i?"CM100 staat klaar. Start ontluchten wanneer het circuit open staat.":"Start CM100 eerst en voer daarna ontluchten uit.",progressTask:"purge",className:"oq-settings-commissioning-card--air-purge",actions:`
            ${t.entities.airPurgeStart||t.entities.airPurgeAbort?No({active:P,startKey:"airPurgeStart",stopKey:"airPurgeAbort",startLabel:"Ontluchten starten",stopLabel:"Ontluchten stoppen",startDisabled:rt||ph,stopDisabled:rt||dh}):""}
          `,metrics:`
            ${$e("airPurgeRemaining","Resterende tijd","Ontluchten loopt maximaal 5 minuten.",ne,"oq-settings-field--compact")}
            ${$e("airPurgePhase","Fase","Laat zien welk deel van het ontluchten nu actief is.",Ne,"oq-settings-field--compact")}
            ${$e("flowSelected","Actuele flow","Gemeten flow tijdens het ontluchten.",Ce("flowSelected"),"oq-settings-field--compact")}
            ${lp("airPurgeReturnToAuto","Na afloop","","Service mode (CM100) afsluiten","oq-settings-field--span-2 oq-settings-field--compact")}
          `})}].filter(Th=>Th.available);return{cm100Status:wh,cm100StartDisabled:c,cm100StopDisabled:m,serviceStatusCopy:Sh,tasks:qh}}function Sy(e){return Pe({dataAttribute:"data-oq-service-task",dataValue:e.key,className:"oq-settings-service-row",label:e.label,value:e.status,note:e.summary,action:`<button
          class="oq-helper-button oq-helper-button--ghost"
          type="button"
...
`)}function Hv(e,o=""){if(!e.length)return`
      ...
      `;let r=$v(e),n=xv(r.axisMax),a=[0,.25,.5,.75,1].map(l=>r.axisMax*l),i=e.map((l,p)=>{let u=r.left+r.barSlot*p+r.barSlot/2,c=[{key:"electricalInputWh",className:"input",label:"Verbruikte elektriciteit"},{key:"heatpumpHeatOutputWh",className:"heat",label:"Warmte door warmtepomp"},{key:"heatpumpCoolingOutputWh",className:"cooling",label:"Koeling warmtepomp"},{key:"boilerHeatOutputWh",className:"boiler",label:"Cv-ketel"}],m=r.height-r.bottom,b=c.map(f=>{let v=Tn(l,f.key);if(v<=0)return"";let S=v/r.axisMax*r.plotHeight;return m-=S,`
        ...
        `}).join(""),w=e.length<=12||p===0||p===e.length-1||p%3===0?`<text x="${u.toFixed(1)}" y="${r.height-18}" text-anchor="middle" class="oq-energy-history-axis-label">${s(l.label||Kt(l.dateKey))}</text>`:"",g=Av(l);return`
    ...
    `}function Wv(){if(!h("lifetimeEnergyHistoryEnabled"))return"";let e=yr(t.energyHistoryExportMode),o=vn.map(n=>`
    ...
    `}function pm(){let e=t.settingsBackupBusy,o=Qt.length,r=zt.length;return J("Beheer","Backup en restore","Sla een JSON-backup op van de instellingen die OpenQuatt in deze web-app beheert, en zet die later weer terug na een factory-bin update.",`
    ...
    `}function Kv(){if(!t.root)return!1;let e=t.root.querySelector(".oq-helper-status-grid");if(!e)return!!t.root.querySelector(".oq-helper-hub");let o=hm();if(e.querySelectorAll("[data-oq-header-status]").length!==o.length)return e.outerHTML=kn(),!0;for(let[n,a,i,l]of o){let p=e.querySelector(`[data-oq-header-status="${n}"]`);if(!p)return e.outerHTML=kn(),!0;let u=gm(n),c=!!(l||u);if(p.tagName.toLowerCase()!==(c?"button":"div"))return e.outerHTML=kn(),!0;let m=p.querySelector(".oq-helper-status-label"),b=p.querySelector(".oq-helper-status-value");if(!m||!b)return e.outerHTML=kn(),!0;m.textContent!==a&&(m.textContent=a);let y=bm(n),w=y?`<span class="oq-helper-status-value-text">${s(i)}</span><span class="oq-helper-status-badge" aria-label="Update beschikbaar" title="Update beschikbaar"></span>`:s(i);b.innerHTML!==w&&(b.innerHTML=w),c?p.setAttribute("data-oq-action",u):p.removeAttribute("data-oq-action"),p.classList.toggle("oq-helper-status-item--button",c),p.classList.toggle("oq-helper-status-item--attention",y)}return!0}function jv(){let e=typeof window<"u"?window.__OQ_DEV_CONTROLS__:null;return!e||typeof e.render!="function"?"":e.render()}function ul(){let e=jv();return e?t.devPanelOpen?`
    ...
    `}bu({getSignature:Ka,patch:Kv});function wm(){if(t.systemModal==="login")return sd();if(t.systemModal==="api-security")return id();if(t.systemModal==="mqtt")return Ru();if(t.systemModal==="mqtt-sensors")return Pu();if(t.systemModal==="connectivity"){let e=Bv();return`
      ...
      `;if(t.systemModal==="webserver-logs")return Cp();if(t.systemModal==="debug-recording")return Zp();if(t.systemModal==="openquatt-pause"){let e=k("openquattEnabled"),o=t.busyAction==="openquatt-regulation",r=h("openquattResumeAt"),n=t.loadingEntities||t.entitySyncInFlight,a=r||!n,i=Br(),l=Jo(T("openquattResumeAt")),p=Zn();return`
        <div class="oq-helper-modal-backdrop${t.overviewTheme==="dark"?" oq-helper-modal-backdrop--dark":""}" data-oq-modal="system">
    ...
    `:""}function pl(){return`
      <div class="oq-settings-curve-grid">
        ${me.map(e=>re(e.key,`Aanvoertemp. bij ${e.label}`,`Doelaanvoertemperatuur bij ${e.label} buitentemperatuur.`)).join("")}
    ...
    `;return ce("curveControlProfile","Regelprofiel","Kies of de stooklijn vooral comfortabel, gebalanceerd of rustig moet reageren.",n,"oq-settings-field--span-2")}function Qv(){let e=(O,Q=0)=>{let N=A(O);return Number.isNaN(N)?Q:Math.max(0,N)},r=e("phComfortBelow",.1),n=e("phComfortAbove",.3),a=e("phKp",3e3),i=20-r,l=20+n,p=620,u=184,c=46,m=24,b=18,y=40,w=96,g=p-c-m,f=Math.min(20-1.2,i-.35),v=Math.max(20+1.2,l+.35),S=O=>c+(O-f)/Math.max(.01,v-f)*g,q=S(f),C=S(v),x=S(i),M=S(20),V=S(l),F=Math.abs(i-20)>.001,ee=Math.abs(l-20)>.001,H=b+24,G=u-y,X=w-44,ue=(O,Q,N,I="")=>{let ht=Math.max(q+4,Math.min(C-110-4,O-55)),rt=O-14,ft=X,nt=28,E=w-X+16;return`
        <g class="oq-ph-concept-hotspot" tabindex="0" role="img" aria-label="${s(`${Q} ${N}`)}">
          <rect class="oq-ph-concept-hit" x="${rt}" y="${ft}" width="${nt}" height="${E}" rx="10"></rect>
          <circle class="oq-ph-concept-hit" cx="${O}" cy="${w}" r="14"></circle>
          <g class="oq-ph-concept-tooltip${I?` oq-ph-concept-tooltip--${I}`:""}" transform="translate(${ht} ${X})">
            <rect class="oq-ph-concept-tooltip-panel" width="110" height="36" rx="10"></rect>
            <text x="${110/2}" y="14" text-anchor="middle" class="oq-ph-concept-tooltip-kicker">${s(Q)}</text>
            <text x="${110/2}" y="27" text-anchor="middle" class="oq-ph-concept-tooltip-detail">${s(N)}</text>
          </g>
        </g>
      `},ie=[`M ${q.toFixed(1)} ${H.toFixed(1)}`,`L ${x.toFixed(1)} ${w.toFixed(1)}`,`L ${V.toFixed(1)} ${w.toFixed(1)}`,`L ${C.toFixed(1)} ${G.toFixed(1)}`].join(" ");return`
      <div class="oq-ph-concept-card">
        <div class="oq-ph-concept-visual">
          ...
          <svg class="oq-ph-concept-svg" viewBox="0 0 ${p} ${u}" role="img" aria-label="Grafiek voor Power House tuning">
            <rect x="${q.toFixed(1)}" y="${b}" width="${Math.max(20,x-q).toFixed(1)}" height="${(u-b-y).toFixed(1)}" rx="18" class="oq-ph-concept-band oq-ph-concept-band--below"></rect>
            <rect x="${x.toFixed(1)}" y="${b}" width="${Math.max(20,V-x).toFixed(1)}" height="${(u-b-y).toFixed(1)}" rx="18" class="oq-ph-concept-band oq-ph-concept-band--calm"></rect>
            <rect x="${V.toFixed(1)}" y="${b}" width="${Math.max(20,C-V).toFixed(1)}" height="${(u-b-y).toFixed(1)}" rx="18" class="oq-ph-concept-band oq-ph-concept-band--above"></rect>
            ...
            ${F?`<line x1="${x}" y1="${w-12}" x2="${x}" y2="${w+12}" class="oq-ph-concept-marker oq-ph-concept-marker--below"></line>`:""}
            ...
            ${F?`<circle cx="${x}" cy="${w}" r="5" class="oq-ph-concept-point oq-ph-concept-point--below"></circle>`:""}
            ...
            ${F?ue(x,"Comfort onder setpoint",_(i,1,"\xB0C"),"below"):""}
            ...
            ${F?`<text x="${x-5}" y="${u-14}" text-anchor="end" class="oq-ph-concept-tick-value">${s(_(i,1,"\xB0C"))}</text>`:""}
    ...
    `:""}function Sm(){let e=Gv();return J("Installatie","Flowregeling","Kies hoe de pomp wordt geregeld en stel de flow-instellingen direct als installatieparameter in. De autotune vind je later bij Service & commissioning.",`
      ...
      `;return J("Regeling","Verwarmingsstrategie","Kies hier hoe OpenQuatt je verwarming regelt. De instellingen hieronder passen zich automatisch aan.",`
        ...
        ${hl()}
        ${e}
      `)}function vl(){let r={top:22,right:18,bottom:38,left:34},n=560-r.left-r.right,a=240-r.top-r.bottom,i=me[0].outdoor,l=me[me.length-1].outdoor,p=w=>r.left+(w-i)/(l-i)*n,u=w=>r.top+(70-w)/50*a,c=[20,30,40,50,60,70].map(w=>{let g=u(w);return`
      ...
      `).join(""),b=me.map(w=>`${p(w.outdoor)},${u(ge(w.key,T(w.key)))}`).join(" "),y=me.map(w=>{let g=ge(w.key,T(w.key));return`
          <g>
            <circle
              cx="${p(w.outdoor)}"
              cy="${u(g)}"
    ...
    `:""}function $m(){let e=Ot();Jr(e);let o=Tt("cicPollingEnabled"),r=Tt("otEnabled"),n=[h("lowflowFaultActive")?kr({label:"Flow",value:U("lowflowFaultActive")?"Te lage flow gemeld":"Geen lage-flowmelding",active:U("lowflowFaultActive")}):"",h("flowMismatch")?kr({label:"Flowvergelijking duo",value:U("flowMismatch")?"Afwijking tussen warmtepompen":"Geen afwijking gemeld",active:U("flowMismatch")}):""].filter(Boolean).join(""),a=[h("cicDataStale")?kr({label:"CIC-data",value:o?U("cicDataStale")?"Verouderd":"Geen probleem gemeld":"Polling uitgeschakeld",active:o&&U("cicDataStale")}):"",h("otLinkProblem")?kr({label:"OpenTherm",value:r?U("otLinkProblem")?"Verbindingsprobleem":"Geen probleem gemeld":"Uitgeschakeld",active:r&&U("otLinkProblem")}):""].filter(Boolean).join(""),i=[h("hp1Failures")?kr({label:"Warmtepomp 1",value:ji("hp1Failures"),active:Zr("hp1Failures")}):"",h("hp2Failures")?kr({label:"Warmtepomp 2",value:ji("hp2Failures"),active:Zr("hp2Failures")}):""].filter(Boolean).join(""),l=A("compressorStarts2hWarningLimit"),p=A("compressorStarts72hWarningLimit"),u=U("compressorCyclingWarning2h")||U("compressorCyclingWarning72h")||U("alternatingCompressorStartsWarning")||e.cyclingAlertLatched,c=n?`
...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},o=t.entities.hpGeneration||{},r=String(T("hpGeneration")||""),n=Ft(o),a=t.loadingEntities||t.busyAction==="save-hpGeneration";return`
    ...
    `}function Am(){let e=pt(),o=t.entities.hpGeneration||{},r=h("hpGeneration")&&Ft(o).length>0;return!e&&!r?"":J("Basis","Quatt Hybrid-versie","Kies hier welke Quatt Hybrid je hebt. Deze keuze bepaalt de basis van de regeling.",`
      ...
      `)}function Rm(){let e=lo(),o=pa(),r=t.busyAction==="restartAction";return J("Diagnostiek","Systeemstatus","Snelle statusinformatie voor support, controle en onderhoud.",`
        <div class="oq-settings-system-summary">
          ${Pe({dataValue:"uptime",label:"Uptime",value:rr()})}
          ${Pe({dataValue:"ip",label:"IP-adres",value:nr()})}
          ${Pe({dataValue:"updates",label:"Updates",value:e,action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
          ...
          ${Pe({dataValue:"webserverLog",label:"Logboek",value:ha(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
          ...
          ${Pe({dataValue:"debugRecording",label:"Debugopname",value:cn(),note:Ca(),action:`<button
              class="oq-helper-button oq-helper-button--ghost"
              type="button"
              data-oq-action="open-debug-recording-modal"
            >
          ...
          ${Pe({dataValue:"datetime",label:"Datum/tijd",value:o})}
          ${Pe({dataValue:"espTemp",label:"ESP-temp",value:ja()})}
          ${Pe({dataValue:"restart",label:"Herstart OpenQuatt",value:"Opnieuw opstarten",note:"Dit onderbreekt de webinterface kort.",action:`<button
          ...
          ${h("statusLedsEnabled")?`
            ${Pe({dataValue:"statusLeds",label:"Status-LEDs",value:k("statusLedsEnabled")?"Aan":"Uit",note:"Schakelt de gele netwerk-LED en rode storings-LED op de Q-edition controller.",action:It("statusLedsEnabled","Status-LEDs",k("statusLedsEnabled"),t.loadingEntities||t.busyAction==="switch-statusLedsEnabled")})}
          `:""}
        </div>
    ...
    `}function ww(){let e=ye(D("hpGeneration")),o=te()?"Stooklijn":"Power House",r=v=>ye(D(v)),n=[],a=te()?[["Regelprofiel",r("curveControlProfile")],["Aanvoer bij -20\xB0C",B("curveM20")],["Aanvoer bij -10\xB0C",B("curveM10")],["Aanvoer bij 0\xB0C",B("curve0")],["Aanvoer bij 5\xB0C",B("curve5")],["Aanvoer bij 10\xB0C",B("curve10")],["Aanvoer bij 15\xB0C",B("curve15")],["Fallback-aanvoer",B("curveFallbackSupply")]]:[["Profiel",r("phResponseProfile")],["Rated maximum house power",B("housePower")],["Maximum heating outdoor temperature",B("houseOutdoorMax")],["Temperatuurreactie",B("phKp")],["Comfort onder setpoint",B("phComfortBelow")],["Comfort boven setpoint",B("phComfortAbove")]],i=String(T("flowControlMode")||""),l=xr(),p=[["Status",l.status],["Actuele flow",l.flowAvailable?`${Math.round(l.flowValue)} L/h`:"Nog geen actuele waarde"]],u=za(),c=[["Status",u.status],["Kamertemperatuur",Number.isFinite(u.roomTempValue)?`${u.roomTempValue.toFixed(1)} \xB0C`:"Nog geen actuele waarde"],["Kamer-setpoint",Number.isFinite(u.roomSetpointValue)?`${u.roomSetpointValue.toFixed(1)} \xB0C`:"Nog geen actuele waarde"]],m=[["Flowregeling",i==="Manual PWM"?"Vaste pompstand":"Gewenste flow"],i==="Manual PWM"?["Vaste pompstand",B("manualIpwm")]:["Gewenste flow",B("flowSetpoint")]],b=h("boilerCvAssistEnabled")?[["CV-ketel/boiler aanwezig",k("boilerCvAssistEnabled")?"Ja":"Nee"],...k("boilerCvAssistEnabled")?[["Boiler rated heat power",B("boilerRatedHeatPower")]]:[]]:[],y=[["Maximale watertemperatuur",B("maxWater")]],w=[["Start stille uren",Vr(T("silentStartTime"))||"\u2014"],["Einde stille uren",Vr(T("silentEndTime"))||"\u2014"],["Maximaal niveau tijdens stille uren",B("silentMax")],["Maximaal niveau overdag",B("dayMax")]],g=v=>`
    ...
    `}function Cl(e){let o=String(e||"").trim();return o?{Ready:"Gereed","Waiting for room request":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","Cooling enabled, waiting for room temperature above cooling setpoint":"Koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint","No dew point source":"Geen dauwpuntbron","OpenQuatt paused":"OpenQuatt gepauzeerd","Cooling disabled":"Koeling uitgeschakeld","Cooling minimum unavailable":"Minimale koel-aanvoer onbekend","Flow too low":"Flow te laag","Fallback active":"Dauwpuntsbenadering actief","Fallback active (+0.5\xB0C warm night)":"Dauwpuntsbenadering actief (+0,5\xB0C warme nacht)","Fallback active (+1.0\xB0C very warm night)":"Dauwpuntsbenadering actief (+1,0\xB0C zeer warme nacht)","Fallback active (+1.5\xB0C tropical night)":"Dauwpuntsbenadering actief (+1,5\xB0C tropische nacht)","User responsibility (no dew point or fallback)":"Expliciet toegestaan (geen dauwpunt of benadering)","Fallback cooling active":"Dauwpuntsbenadering actief","Fallback corrected by warm night":"Dauwpuntsbenadering gecorrigeerd door warme nacht","Fallback blocked by tropical night":"Dauwpuntsbenadering geblokkeerd door tropische nacht"}[o]||o:""}function Bm(){let e=!h("coolingRoomRequestRequired")||k("coolingRoomRequestRequired"),o=[re("coolingMinimumSupplyTemp","Minimale koel-aanvoer","Ondergrens voor het koeldoel. OpenQuatt gebruikt de hoogste waarde van deze instelling en de dauwpuntveilige grens."),Ze("coolingDemandMax","Maximale koelsterkte","Bepaalt hoe krachtig OpenQuatt mag koelen. Lager geeft langere, rustigere runs; hoger geeft meer koelvermogen bij warm weer.","",{minLabel:"Rustig",maxLabel:"Krachtig",valueLabel:`${B("coolingDemandMax")} max`}),re("coolingRestartDelta","Herstartmarge watertemperatuur","Na het bereiken van het koel-aanvoerdoel start de watercyclus pas opnieuw zodra de aanvoer deze marge boven het doel ligt."),re("coolingSafetyMargin","Dauwpunt veiligheidsmarge","Extra marge boven het geselecteerde dauwpunt voor de minimale veilige watertemperatuur.")].filter(Boolean),r=[h("coolingRoomRequestRequired")?sp("coolingRoomRequestRequired","Koelvraag via kamerthermostaat","Aan: OpenQuatt wacht op echte koelvraag vanuit de kamer. Uit: koeltoestemming geldt direct als koelvraag.","Koelvraag start en stopt met de marges hieronder.","Koeltoestemming geldt direct als koelvraag. De start- en stopmarge worden nu niet gebruikt.","oq-settings-field--span-2"):"",e?re("coolingRequestOnDelta","Koelvraag start boven setpoint","Koelvraag wordt actief zodra de kamer warmer is dan setpoint plus deze marge."):"",e?re("coolingRequestOffDelta","Koelvraag stopt boven setpoint","Koelvraag valt weer af zodra de kamer koeler is dan setpoint plus deze marge."):""].filter(Boolean),n=r.length>0,a=h("coolingWithoutDewPointMode"),i=[h("coolingGuardMode")?xn("Route",ye(D("coolingGuardMode","Onbekend"))):"",h("coolingEffectiveMinSupplyTemp")?xn("Actieve ondergrens",D("coolingEffectiveMinSupplyTemp","\u2014")):""].filter(Boolean),l=i.length?ce("coolingGuardStatus","Actuele beveiliging","Laat zien welke route koeling nu begrenst en welke ondergrens daadwerkelijk geldt.",`<div class="oq-settings-cooling-facts">${i.join("")}</div>`,"oq-settings-field--span-2 oq-settings-field--cooling-status"):"",p=[h("outsideTempSelected")?xn("Actuele buitentemperatuur",D("outsideTempSelected","\u2014")):"",h("coolingFallbackNightMinOutdoorTemp")?xn("Nachtminimum buitentemperatuur",D("coolingFallbackNightMinOutdoorTemp","\u2014")):"",h("coolingFallbackMinSupplyTemp")?xn("Berekende minimum watertemperatuur",D("coolingFallbackMinSupplyTemp","\u2014")):""].filter(Boolean),u=p.length?`<div class="oq-settings-cooling-fallback-metrics">${p.join("")}</div>`:"",c=a||p.length>0,b=D("coolingGuardMode","").toLowerCase().includes("fallback");if(!o.length&&!n&&!a&&!l&&!c)return"";let y={"Dew point required":"Gebruik alleen een betrouwbare dauwpuntmeting. Zonder meting blijft koeling uit.","Allow without dew point":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use fallback":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, use dew point approximation":"Gebruik dauwpunt waar mogelijk. Zonder meting geldt de conservatieve benadering hieronder.","Allow without dew point, user responsibility":"Negeer dauwpunt en benadering; alleen de ingestelde minimale koel-aanvoer geldt."};return J("Koeling","Koelingsinstellingen","Stel hier in wanneer koelvraag ontstaat, hoe koud het water mag worden en hoeveel het water mag opwarmen voor herstart.",`
        ...
        `:""}
      `)}function Km(){let e=h("otEnabled"),o=h("cicPollingEnabled")||h("cicFeedUrl"),r=h("cicCompatibilityMode"),n=h("otLinkProblem")||h("cicDataStale")||h("cicJsonFeedOk");if(!e&&!o&&!r&&!n)return"";let a=Tt("cicPollingEnabled"),i=Tt("otEnabled"),l=({label:g,value:f,active:v=!1})=>`
      <div class="oq-settings-integration-diagnostic-item${v?" is-warning":""}">
        <dt>${s(g)}</dt>
        <dd>${s(f)}</dd>
      </div>
    `,p=(g,f,v="Actief",S="Normaal",q={})=>{if(!h(g))return"";let C=U(g);return l({label:f,value:C?v:S,active:q.warningWhenActive?C:!1})},u=(g,f,v={})=>{let S=v.fallbackKey||"";return!h(g)&&!(S&&h(S))?"":l({label:f,value:Ce(h(g)?g:S,v)})},c=(g,f)=>{let v=f.filter(Boolean).join("");return v?`
    ...
    `:"";return J("Integratie","OpenTherm & CIC-polling","Configureer de directe thermostaatbus, externe CIC-feed en Quatt app-compatibiliteit.",`
        ...
        ${w}
      `)}function jm(){if(!Lr.some(E=>h(E)))return"";let o=Tt("cicPollingEnabled"),r=Tt("otEnabled"),n=(E={})=>E.haValueKey||(E.haKeys||[]).find($=>!/valid$/i.test($))||"",a=(E={})=>E.haValidKey||(E.haKeys||[]).find($=>/valid$/i.test($))||"",i=(E="",$="")=>!!E&&!!$&&h(E)&&h($)&&U($),l=(E={})=>i(n(E),a(E)),p={mqttCoolingDewPoint:"cooling_dew_point",mqttOutsideTemperature:"outside_temperature",mqttRoomTemperature:"room_temperature",mqttRoomSetpoint:"room_setpoint",mqttHeatingEnable:"heating_enable",mqttCoolingEnable:"cooling_enable"},u=(E={})=>E.mqttTopicKey||p[E.valueKey]||"",c=(E="")=>{if(!E)return!0;let $=t.mqttStatus?.input_enabled;return $&&typeof $=="object"&&Object.prototype.hasOwnProperty.call($,E)?$[E]!==!1:!0},m=E=>/\bMQTT\b/i.test(String(E||"")),b=(E,$={})=>E==="CIC"?o:E==="OT thermostat"?r:E==="HA input"?l($):E==="CIC or HA input"?o||l($):m(E)?c(u($)):E==="Flowmeter HP2"?h("hp2Flow"):E==="Local aggregate HP1/HP2"?h("flowLocal")||h("hp2Flow"):!0,y=(E,$={})=>E==="CIC"&&!o?"CIC-polling staat uit":E==="OT thermostat"&&!r?"OpenTherm staat uit":E==="HA input"&&!l($)?"HA-bron ongeldig":E==="CIC or HA input"&&!o&&!l($)?"CIC en HA ontbreken":m(E)&&!c(u($))?"MQTT-topic staat uit":E==="Flowmeter HP2"&&!h("hp2Flow")?"HP2-flow ontbreekt":E==="Local aggregate HP1/HP2"&&!h("flowLocal")&&!h("hp2Flow")?"Lokale flow ontbreekt":"",w=(E,$="Actief",P="Normaal")=>h(E)?U(E)?$:P:"",g=(E,$={})=>{let P=String(E||"").trim();return P?$.optionLabels?.[P]||ye(P):""},f=(E,$={})=>{let P=String(T(E)||"").trim();return P?g(P,$):""},v=E=>{let $=Ro(E,"");return $?ye($):""},S=E=>{let $=String(Ro(E,"")||"").trim();return!$||$==="None"?"":ye($)},q=(...E)=>E.find($=>String($||"").trim())||"",C=()=>{let E=f("waterSupplySource");if(String(T("waterSupplySource")||"")==="Local"&&h("localWaterSupplyTempSource")){let $=f("localWaterSupplyTempSource");return $?`${E} - ${$}`:E}return E},x=()=>{let E=String(T("flowSource")||"").trim();if(E==="Outdoor unit"){if(h("qFlowSource")){let $=String(T("qFlowSource")||"").trim(),P=String(T("hpGeneration")||"").trim();return $==="Local"||$==="Auto"&&P==="V1"?$==="Auto"?"Lokaal (auto)":"Lokaal":q(f("outdoorUnitFlowMode"),$==="Auto"?"Buitenunit (auto)":"Buitenunit")}return q(f("outdoorUnitFlowMode"),"Quatt-flow")}return ye(E)},M=()=>{let E=String(T("outsideTempSource")||"").trim();if(E==="MQTT"&&!c("outside_temperature"))return"MQTT-topic staat uit";if(E!=="Auto")return ye(E);let $=A("outsideTempLocalAggregated"),P=A("outsideTempHa"),j=A("mqttOutsideTemperature"),Y=!Number.isNaN($),ne=h("outsideTempHaValid")?U("outsideTempHaValid")&&!Number.isNaN(P):!Number.isNaN(P),le=c("outside_temperature")&&h("mqttOutsideTemperatureValid")&&U("mqttOutsideTemperatureValid")&&!Number.isNaN(j),Ne=[Y?{label:"Buitenunit",value:$}:null,ne?{label:"HA-invoer",value:P}:null,le?{label:"MQTT",value:j}:null].filter(Boolean);return Ne.length?Ne.reduce((fe,He)=>He.value<fe.value?He:fe,Ne[0]).label:"Auto"},V=E=>{if(!h(E))return NaN;let $=Number(T(E));if(Number.isFinite($))return $;let j=String(t.entities[E]?.state??"").trim().replace(",",".").match(/-?\d+(?:\.\d+)?/);return j?Number(j[0]):NaN},F=(E,$="")=>{if(!h(E))return!1;let P=V(E);return($?U($):!0)&&Number.isFinite(P)},ee=()=>{let E=String(T("coolingDewPointSource")||"").trim();if(E==="Home Assistant")return F("coolingDewPointHa","coolingDewPointHaValid")?"HA-invoer":"HA-invoer ontbreekt";if(E==="MQTT")return c("cooling_dew_point")?F("mqttCoolingDewPoint","mqttCoolingDewPointValid")?"MQTT":"MQTT ontbreekt of verouderd":"MQTT-topic staat uit";let $=F("coolingDewPointHa","coolingDewPointHaValid"),P=c("cooling_dew_point")&&F("mqttCoolingDewPoint","mqttCoolingDewPointValid");if($&&P){let j=V("coolingDewPointHa");return V("mqttCoolingDewPoint")>j?"MQTT":"HA-invoer"}return $?"HA-invoer":P?"MQTT":E?ye(E):"Auto"},H=({label:E,value:$="",key:P="",active:j=!1,status:Y="",statusTone:ne="",statusTitle:le=""})=>{let Ne=$||(P?Ce(P):"");if(!Ne&&!Y)return"";let fe=String(ne||"").replace(/[^a-z0-9_-]/gi,""),He=le||Y,pe=Y?`<em class="oq-settings-source-status${fe?` oq-settings-source-status--${s(fe)}`:""}" title="${s(He)}" aria-label="${s(`${Y}: ${He}`)}">${s(Y)}</em>`:"";return`
        <div class="oq-settings-source-row${j?" is-warning":""}${Y?" has-status":""}">
          <span>${s(E)}${pe}</span>
          <strong>${s(Ne)}</strong>
        </div>
      `},G=({label:E="HA-invoer",valueKey:$="",validKey:P="",value:j=""})=>{if(!$||!P||!h($)||!h(P))return[];let Y=U(P);return[H({label:E,key:$,value:j,status:Y?"Geldig":"Ongeldig",statusTone:Y?"valid":"invalid",statusTitle:Y?"Home Assistant geeft dit signaal geldig door. OpenQuatt mag deze HA-invoer gebruiken.":"Home Assistant geeft dit signaal niet geldig door. OpenQuatt gebruikt deze HA-invoer dan niet als bron."})]},X=({label:E="MQTT",valueKey:$="",validKey:P="",value:j="",topicKey:Y=""})=>{if(!$||!P||!h($)||!h(P))return[];if(!c(Y||p[$]))return[];let ne=U(P),le=ne?"MQTT heeft een geldige, recente waarde ontvangen. OpenQuatt mag deze MQTT-invoer gebruiken.":"MQTT heeft nog geen geldige recente waarde ontvangen. OpenQuatt gebruikt deze MQTT-invoer dan niet als bron.";return[H({label:E,key:$,value:ne?j:"\u2014",status:rs(P),statusTone:ne?"valid":"invalid",statusTitle:le})]},ue=({title:E,icon:$="",content:P="",rows:j=[],copy:Y="",className:ne=""})=>{let le=j.filter(Boolean).join("");return!P&&!le&&!Y?"":`
        <section class="oq-settings-source-group${ne?` ${s(ne)}`:""}">
          <h5>
            ${$?`<span class="oq-settings-source-group-icon">${it($,"oq-settings-source-group-icon-svg")}</span>`:""}
            <span>${s(E)}</span>
          </h5>
          ${P?`<div class="oq-settings-source-group-content">${P}</div>`:""}
          ${le?`<div class="oq-settings-source-rows">${le}</div>`:""}
          ${Y?`<p class="oq-settings-source-group-copy">${s(Y)}</p>`:""}
        </section>
      `},ie=(E,$={})=>{if(!h(E))return{markup:"",warning:""};let P=t.entities[E]||{},j=String(T(E)||""),ne=Ft(P).filter(pe=>b(pe,$)),le=j&&!b(j,$),Ne=j==="HA input"&&$.keepUnavailableCurrent!==!0,He=(le&&!Ne&&!ne.includes(j)?[j,...ne]:ne).map(pe=>{let be=g(pe,$);return`<option value="${s(pe)}" ${pe===j?"selected":""}>${s(be)}</option>`}).join("");return{markup:`
          <label class="oq-settings-source-select">
            <span class="oq-settings-source-select-head">
              <span>${s($.label||"Bron")}</span>
              ${$.infoCopy?ao($.infoId||E,$.infoTitle||$.label||"Bron",$.infoCopy):""}
            </span>
            <select class="oq-helper-select" data-oq-field="${s(E)}" ${t.loadingEntities?"disabled":""}>
              ${He}
            </select>
          </label>
        `,warning:le?`Huidige bron niet beschikbaar: ${y(j,$)}`:""}},O=({key:E,title:$,icon:P="",select:j,secondarySelect:Y=null,secondarySelects:ne=null,activeRows:le=[],measurementRows:Ne=[],activeCopy:fe="",rows:He=[]})=>{let pe=j&&j.when!==!1?ie(j.key,j):{markup:"",warning:""},Ht=(Array.isArray(ne)?ne:Y?[Y]:[]).filter(ke=>ke&&ke.when!==!1).map(ke=>ie(ke.key,ke)).filter(ke=>ke.markup),je=Ht.map(ke=>ke.markup).join(""),Mr=Ht.map(ke=>ke.warning).find(Boolean)||"",Se=He.filter(Boolean).join(""),Me=`${pe.markup}${je}`,at=pe.warning||Mr,bt=[ue({title:"Configuratie",icon:"settings",className:"oq-settings-source-group--config",content:Me?`
            <div class="oq-settings-source-controls">
              ${Me}
            </div>
            ${at?`<p class="oq-settings-source-warning">${s(at)}</p>`:""}
          `:""}),ue({title:"Actief",icon:"target",rows:le,copy:fe,className:"oq-settings-source-group--active"}),ue({title:"Metingen",icon:"activity",rows:Ne,className:"oq-settings-source-group--measurements"})].filter(Boolean).join("");return!bt&&!Me&&!Se?"":`
        <article class="oq-settings-source-card" data-oq-settings-field="${s(E||j.key)}">
    ...
    `:"";return J("Integratie","MQTT inputbronnen","Beheer de brokerverbinding voor externe MQTT-bronwaarden.",`
      ...
      `)}function zm(){let e=Re.has(t.settingsGroup)?t.settingsGroup:de[0].id;return`
    ...
    `}function Qm(){let e=Re.has(t.settingsGroup)?t.settingsGroup:de[0].id;return`
    ...
    `}function Sw(){if(!t.root||t.appView!=="settings")return!1;let e=t.root.querySelector(".oq-settings-group-nav"),o=t.root.querySelector(".oq-settings-group-stack");if(!e||!o)return!1;let r=Re.has(t.settingsGroup)?t.settingsGroup:de[0].id;if(r==="service")return!1;let n=e.querySelectorAll(".oq-settings-group-button");if(n.length!==de.length)return!1;n.forEach(g=>{let v=String(g.dataset.groupId||"")===r;g.classList.toggle("is-active",v),g.setAttribute("aria-pressed",v?"true":"false")}),o.querySelectorAll(".oq-settings-info").forEach(g=>{let f=String(g.dataset.oqSettingsInfo||""),v=t.settingsInfoOpen===f;g.classList.toggle("is-open",v);let S=g.querySelector(".oq-settings-info-popover");S&&(S.hidden=!v);let q=g.querySelector(".oq-settings-info-button");q&&q.setAttribute("aria-expanded",v?"true":"false")}),o.querySelectorAll("[data-oq-settings-field]").forEach(g=>{let f=String(g.dataset.oqSettingsField||"");if(!f)return;let v=g.querySelector(".oq-settings-static-value");if(v){let C=D(f);v.textContent!==C&&(v.textContent=C)}g.querySelectorAll("select[data-oq-field]").forEach(C=>{let x=String(C.dataset.oqField||f),M=String(T(x)||"");C.value!==M&&(C.value=M)}),g.querySelectorAll("input[data-oq-field]").forEach(C=>{let x=String(C.dataset.oqField||f),M=String(Te(x)||"");C.value!==M&&(C.value=M)});let S=g.querySelector(".oq-helper-slider-meta strong"),q=g.querySelector('input[type="range"][data-oq-field]');if(S&&q){let C=B(f,ge(f,T(f)));S.textContent!==C&&(S.textContent=C)}}),o.querySelectorAll("[data-select-key]").forEach(g=>{let f=String(g.dataset.selectKey||""),v=String(g.dataset.selectOption||""),S=String(T(f)||""),q=v===S;g.classList.toggle("is-active",q),g.setAttribute("aria-pressed",q?"true":"false"),f==="strategy"?g.disabled=t.loadingEntities||t.busyAction==="save-strategy":f==="hpGeneration"?g.disabled=t.loadingEntities||t.busyAction==="save-hpGeneration":f==="curveControlProfile"?g.disabled=t.loadingEntities||t.busyAction==="save-curveControlProfile":f==="phResponseProfile"&&(g.disabled=t.loadingEntities||t.busyAction==="save-phResponseProfile");let C=g.closest(".oq-settings-choice-card-shell");C&&C.classList.toggle("is-active",q)});let a=o.querySelector(".oq-settings-choice-card--static.oq-settings-choice-card--custom");if(a){let g=String(T("phResponseProfile")||"")==="Custom";a.classList.toggle("is-active",g),a.querySelectorAll("input[data-oq-field]").forEach(v=>{let S=String(v.dataset.oqField||""),q=String(Te(S)||"");v.value!==q&&(v.value=q)})}o.querySelectorAll('[data-oq-action="toggle-overview-control"][data-control-key]').forEach(g=>{let f=String(g.dataset.controlKey||""),v=!!T(f),S=String(g.dataset.onLabel||"Aan"),q=String(g.dataset.offLabel||"Uit"),C=String(g.dataset.switchTitle||f),x=v?S:q;g.dataset.controlState=v?"off":"on",g.classList.toggle("is-on",v),g.setAttribute("aria-checked",v?"true":"false"),g.setAttribute("aria-label",`${C}: ${x}`),g.disabled=t.loadingEntities||t.busyAction===`switch-${f}`}),o.querySelectorAll("[data-oq-switch-pill]").forEach(g=>{let f=String(g.dataset.oqSwitchPill||""),v=!!T(f),S=String(g.dataset.onLabel||"Aan"),q=String(g.dataset.offLabel||"Uit"),C=v?S:q;g.classList.toggle("is-on",v),g.textContent!==C&&(g.textContent=C)}),o.querySelectorAll("[data-oq-switch-copy]").forEach(g=>{let f=String(g.dataset.oqSwitchCopy||""),v=!!T(f),S=String(g.dataset.onCopy||""),q=String(g.dataset.offCopy||""),C=v?S:q;g.hidden=!C,g.textContent!==C&&(g.textContent=C)});let i=o.querySelector('button[data-oq-action="open-generation-modal"]')?.closest(".oq-settings-quickstart-status");if(i){let g=i.querySelector(".oq-settings-quickstart-status-value"),f=i.querySelector(".oq-settings-quickstart-status-copy"),v=i.querySelector('button[data-oq-action="open-generation-modal"]'),S=pt(),q=t.entities.hpGeneration||{},C=h("hpGeneration")&&Ft(q).length>0;if(g){let x=S||"Onbekend";g.textContent!==x&&(g.textContent=x)}if(f){let x="Pas dit aan als je een andere Quatt Hybrid hebt.";f.textContent!==x&&(f.textContent=x)}v&&(v.disabled=!C||t.loadingEntities||t.busyAction==="save-hpGeneration")}let l=o.querySelector('button[data-oq-action="open-cm100-commissioning-modal"]')?.closest(".oq-settings-quickstart-status");if(l){let g=l.querySelector(".oq-settings-quickstart-status-value"),f=l.querySelector(".oq-settings-quickstart-status-copy"),v=l.querySelector('button[data-oq-action="open-cm100-commissioning-modal"]'),S=da(),q=k("cm100Active");g&&g.textContent!==S&&(g.textContent=S);let C=q?"CM100 is actief en klaar voor commissioning.":"Open de modal om CM100 te starten en de taken hieronder te ontgrendelen.";f&&f.textContent!==C&&(f.textContent=C),v&&(v.disabled=t.loadingEntities)}let p=o.querySelector('button[data-oq-action="reset"]')?.closest(".oq-settings-quickstart-status");if(p){let g=p.querySelector(".oq-settings-quickstart-status-value"),f=p.querySelector(".oq-settings-quickstart-status-copy"),v=p.querySelector('button[data-oq-action="reset"]'),S=t.complete===!0?"Afgerond":t.complete===!1?"Open":"Laden...",q=t.complete===!0?"Quick Start is afgerond. Je kunt de status hier altijd weer openen met een reset.":t.complete===!1?"Quick Start staat nog open. Gebruik de resetknop om opnieuw te beginnen.":"De status van Quick Start wordt nog geladen.";g&&g.textContent!==S&&(g.textContent=S),f&&f.textContent!==q&&(f.textContent=q),v&&(v.disabled=t.busyAction==="reset")}let u=o.querySelectorAll("[data-oq-access-security-item]");u.length&&u.forEach(g=>{let f=String(g.dataset.oqAccessSecurityItem||""),v=g.querySelector(".oq-settings-quickstart-status-value"),S=g.querySelector(".oq-settings-quickstart-status-copy"),q=g.querySelector("button[data-oq-action]");if(f==="login"){let C=gn(),x=hn();v&&v.textContent!==C&&(v.textContent=C),S&&S.textContent!==x&&(S.textContent=x)}else if(f==="api"){let C=mr(),x=gr();v&&v.textContent!==C&&(v.textContent=C),S&&S.textContent!==x&&(S.textContent=x)}q&&(q.disabled=!1)});let c=o.querySelector(".oq-settings-system-summary");if(c){let g=c.querySelectorAll(".oq-settings-system-row"),f={uptime:rr(),ip:nr(),updates:lo(),datetime:pa(),espTemp:ja(),restart:"Opnieuw opstarten"};g.forEach(q=>{let C=q.querySelector(".oq-settings-system-row-value"),x=q.dataset.oqDiagnosticsRow||"";if(C&&Object.prototype.hasOwnProperty.call(f,x)){let M=f[x];C.textContent!==M&&(C.textContent=M)}});let v=c.querySelector('button[data-oq-action="open-update-modal"]');v&&(v.disabled=!1);let S=c.querySelector('button[data-oq-action="open-restart-confirm"]');if(S){let q=t.busyAction==="restartAction";S.disabled=q,S.textContent=q?"Herstarten...":"Herstarten"}}o.querySelectorAll(".oq-settings-hp-offset-row").forEach(g=>{let f=String(g.dataset.oqSettingsField||""),v=String(g.dataset.oqHpOffsetRawKey||""),S=String(g.dataset.oqHpOffsetFinalKey||"");if(!f||!v||!S)return;let q=Oe(f),C=fn(v,S,f),x=xe(Te(f)),M=Number.isFinite(C)&&Number.isFinite(x)?Lt(C+x,q.uom||"\xB0C",2):ve(S,2),V=g.querySelector("[data-oq-hp-offset-active]");if(V){let H=`${ve(S,2)} actief`;V.textContent!==H&&(V.textContent=H)}let F=g.querySelector("[data-oq-hp-offset-raw]");if(F){let H=Number.isFinite(C)?Lt(C,q.uom||"\xB0C",2):ve(v,2);F.textContent!==H&&(F.textContent=H)}let ee=g.querySelector("[data-oq-hp-offset-final]");ee&&ee.textContent!==M&&(ee.textContent=M)});let m=o.querySelector(".oq-settings-curve-shell"),b=te();if(!!m!==b)return!1;let y=!!a,w=String(T("phResponseProfile")||"")==="Custom";return y===w}vu({patch:Sw});function qw({label:e,value:o,tone:r,note:n,status:a=!1}){return`
      <article class="oq-overview-stat oq-overview-stat--${s(r)}${a?" oq-overview-stat--status":""}">
        <p>${s(e)}</p>
        <strong>${s(o)}</strong>
        <span>${s(n)}</span>
      </article>
    `}function Hn(e,o=!1){return e.map(r=>qw({...r,value:Object.prototype.hasOwnProperty.call(r,"key")?Qe(r.key):r.value,status:o})).join("")}function Al(e){return`
    ...
    `}function Rl(e,o,r,n){return`${r?Ew(n):""}${Tw(e,o)}`}function eg(e,o,r,n,a){if(!e)return;let i=Ee({mode:o,running:r,warningActive:n,failureText:a});e.dataset.renderSignature!==i&&($t(e,Rl(o,r,n,a)),e.dataset.renderSignature=i)}function Ya(e,o,r=""){return`
    ...
    `}function kw(e){return Number.isNaN(e)?"\u2014":`${e>0?"+":""}${e.toFixed(1)} \xB0C`}function xw(e){if(!Number.isFinite(e)||e<0)return"\u2014";let o=Math.floor(e),r=Math.floor(o/1440),n=Math.floor(o%1440/60),a=o%60;return r>0?`${r}d ${n}u`:n>0?`${n}u ${a}m`:`${a}m`}function $w(e){let r=String(e||"").trim().match(/^(\d{1,2}):(\d{2})$/);if(!r)return Number.NaN;let n=Number(r[1]),a=Number(r[2]);return Number.isNaN(n)||Number.isNaN(a)||n<0||n>23||a<0||a>59?Number.NaN:n*60+a}function Aw(e){let o=$w(D("timeNowHhmm",""));if(!Number.isFinite(o))return"";let r=Math.round(e),n=((o-r)%1440+1440)%1440,a=Math.floor(n/60),i=n%60;return`${String(a).padStart(2,"0")}:${String(i).padStart(2,"0")}`}function Hw(e,o){let r=Math.max(0,(Number(o)-Number(e))/6e4),n=xw(r),a=h("timeValid")&&k("timeValid")?Aw(r):"";return a?{value:a,note:`${n} geleden`}:{value:`${n} geleden`,note:"Geen tijdsync"}}function Mw(e){let o=Number(e);return Number.isNaN(o)?"\u2014":`${o>0?"+":o<0?"-":""}${Math.abs(o).toFixed(0)} W`}function tg(){return["outsideTempSelected","hp1OutsideTemp","hp2OutsideTemp"].find(e=>h(e))||""}function Rw(){return["hp1WaterIn","hp2WaterIn"].find(e=>h(e))||""}function og(e=D("controlModeLabel","")){let o=String(e||"").toLowerCase();return o.includes("cm5")||o.includes("cooling")||o.includes("koeling")}function fo(){return og()}function rg(e,o){let r=String(e||"").trim().toLowerCase();return r==="waiting for room request"||r==="wacht op kamervraag"||r==="cooling enabled, waiting for room temperature above cooling setpoint"||r==="koeling toegestaan, wacht op kamertemperatuur boven koel-setpoint"?!0:o?!1:r==="flow too low"||r==="flow te laag"||r==="flow unavailable"}function Pl(){return fo()?"Koeling":te()?"Stooklijn":"Power House"}function Pw(){let e=["phouseReq","strategyRequestedPower"];for(let o of e){let r=A(o);if(!Number.isNaN(r))return r}return Number.NaN}function ng(){let e=Pw(),o=A("phouseHouse"),r=A("totalHeat"),n=A("hpCapacity"),a=Number.isNaN(e)||Number.isNaN(o)?Number.NaN:e-o,i="Nog aan het opbouwen",l="Zodra alle vermogens beschikbaar zijn, zie je hier hoe de warmtevraag is opgebouwd.";return!Number.isNaN(e)&&!Number.isNaN(n)&&e>n+150?(i="Capaciteit begrenst",l="De gevraagde warmtevraag ligt boven wat de warmtepomp nu ongeveer kan leveren."):!Number.isNaN(e)&&!Number.isNaN(r)&&r<e-250?(i="Levert minder dan gevraagd",l="De actuele warmteafgifte blijft nog onder de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&r>e+250?(i="Levert meer dan gevraagd",l="De actuele warmteafgifte ligt nu boven de gevraagde warmtevraag."):!Number.isNaN(e)&&!Number.isNaN(r)&&(i="In balans",l="Gevraagde warmtevraag en actuele levering liggen nu dicht bij elkaar."),{requestedText:_(e,0,"W"),houseText:_(o,0,"W"),correctionText:Mw(a),capacityText:Qe("hpCapacity"),statusTitle:i,statusCopy:l}}function ag(){let e=A("curveSupplyTarget"),o=A("supplyTemp"),r=tg(),n=r?A(r):Number.NaN,a=Number.isNaN(e)||Number.isNaN(o)?Number.NaN:o-e,i=!!r&&Number.isNaN(n),l="Stuurt op buitentemperatuur",p="De doelaanvoer volgt de huidige buitentemperatuur en vergelijkt die met de actuele aanvoer.";return i?(l="Fallback actief",p="De buitentemperatuur ontbreekt, dus de regeling valt terug op de ingestelde fallback-aanvoer."):!Number.isNaN(a)&&a<-1?(l="Nog onder doel",p="De actuele aanvoertemperatuur ligt nog onder de doelaanvoer."):!Number.isNaN(a)&&a>1?(l="Boven doel",p="De actuele aanvoertemperatuur ligt nu boven de doelaanvoer."):Number.isNaN(a)||(l="Dicht bij doel",p="De actuele aanvoertemperatuur sluit nu goed aan op de doelaanvoer."),{targetText:Qe("curveSupplyTarget"),supplyText:Qe("supplyTemp"),deltaText:kw(a),capacityText:Qe("hpCapacity"),statusTitle:l,statusCopy:p}}function ig(){let e=A("supplyTemp"),o=D("coolingGuardMode",""),r=D("coolingFallbackNightMinOutdoorTemp","\u2014"),n=A("coolingSupplyError"),a=A("coolingDemandRaw"),i=k("coolingPermitted"),l=k("coolingRequestActive"),p=D("coolingBlockReason","Onbekend"),u=Cl(p),c=rg(p,l),m="Wacht op koelvraag",b="Zodra er koelvraag is, zie je hier hoe de regeling de aanvoer richting het koeldoel stuurt.";return c?(m="Wacht op koelvraag",b="Koeling is toegestaan en wacht tot de kamertemperatuur boven het koel-setpoint komt."):i?l?!Number.isNaN(a)&&a<=0?(m="Houdt doel vast",b="De koelvraag loopt nog, maar de compressor hoeft nu niet harder te werken."):!Number.isNaN(n)&&n>1?(m="Trekt aanvoer omlaag",b="De actuele aanvoertemperatuur ligt nog ruim boven het koeldoel."):!Number.isNaN(n)&&n>.2?(m="Benadert koeldoel",b="De regeling koelt nog door, maar zit al dicht bij de gewenste aanvoertemperatuur."):Number.isNaN(n)||(m="Koelt rustig door",b="De aanvoertemperatuur zit dicht bij het koeldoel en de regeling werkt nu op laag pitje."):(m="Koeling gereed",b="Koeling is toegestaan, maar wacht nog op actieve koelvraag vanuit de kamerregeling."):(m="Koeling geblokkeerd",b=`Blokkade: ${u}.`),{targetText:Qe("coolingSupplyTarget"),supplyText:Qe("supplyTemp"),safeFloorText:Qe("coolingEffectiveMinSupplyTemp"),guardMode:o,fallbackNightMin:r,demandText:Qe("coolingDemandRaw"),statusTitle:m,statusCopy:b,permitted:i,requestActive:l,blockReason:u,waitingForRoomRequest:c}}function Za(){if(fo()){let o=ig(),r=o.guardMode.toLowerCase(),n=r.includes("user responsibility"),a=r.includes("fallback");return{title:"Koelregeling",copy:n?"Koeling draait expliciet zonder dauwpuntmeting of dauwpuntsbenadering. De ingestelde minimale koel-aanvoer blijft gelden.":"Koeling laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die bij de veilige grens zit.",focusLabel:"Koeldoel",focusValue:o.targetText,focusCopy:o.statusCopy,metrics:[{label:"Actuele aanvoertemperatuur",value:o.supplyText,tone:"orange",note:"Wat nu door het systeem loopt."},{label:n?"Ingestelde grens":a?"Berekende grens":"Veilige aanvoergrens",value:o.safeFloorText,tone:"blue",note:n?"Geen dauwpuntmeting of benadering; dit is de ingestelde minimale koel-aanvoer.":a?`Conservatieve dauwpuntsbenadering. Nachtminimum: ${o.fallbackNightMin}.`:"Dauwpunt plus veiligheidsmarge."},{label:"Koelvraag",value:o.demandText,tone:"sky",note:"De huidige koelvraag van de regelaar."}]}}if(te()){let o=ag();return{title:"Stooklijnregeling",copy:"De stooklijn laat zien op welke aanvoertemperatuur de regeling nu mikt en hoe dicht die al benaderd wordt.",focusLabel:"Doelaanvoer",focusValue:o.targetText,focusCopy:"De aanvoertemperatuur waar de regeling nu naartoe werkt.",metrics:[{label:"Actuele aanvoertemperatuur",value:o.supplyText,tone:"orange",note:"Wat nu wordt geleverd."},{label:"Afwijking doelaanvoer",value:o.deltaText,tone:"blue",note:"Verschil met het doel."},{label:"Beschikbare warmtecapaciteit",value:o.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}let e=ng();return{title:"Vermogensbalans",copy:"Power House laat zien waar de warmtevraag nu vandaan komt en of de warmtepomp dat kan volgen.",focusLabel:"Gevraagd vermogen",focusValue:e.requestedText,focusCopy:"De warmtevraag waar Power House nu naartoe stuurt.",metrics:[{label:"Berekende huisvraag",value:e.houseText,tone:"blue",note:"Op basis van woning en buitentemperatuur."},{label:"Kamercorrectie",value:e.correctionText,tone:"orange",note:"Extra bijsturing rond setpoint."},{label:"Beschikbare warmtecapaciteit",value:e.capacityText,tone:"sky",note:"Bij huidige buitentemperatuur."}]}}function Nl(e){return Xm({className:"oq-overview-system",title:e.title,copy:e.copy,signature:Ee(e),body:`
      ...
      `})}function Nw(){if(!k("openquattEnabled"))return{label:"Regeling nu",value:"Regeling tijdelijk uit",tone:"orange"};if(fo()){let n=ig(),a=n.waitingForRoomRequest?"neutral":n.permitted?n.statusTitle==="Koelt rustig door"||n.statusTitle==="Houdt temperatuur vast"?"green":n.statusTitle==="Koeling gereed"?"neutral":"sky":"orange";return{label:"Regeling nu",value:n.statusTitle,tone:a}}if(Od())return{label:"Regeling nu",value:"Stand-by",tone:"neutral"};let o=(te()?ag():ng()).statusTitle;return{label:"Regeling nu",value:o,tone:o==="In balans"||o==="Dicht bij doel"?"green":o==="Nog aan het opbouwen"||o==="Stuurt op buitentemperatuur"?"neutral":"orange"}}function Ow(){return k("openquattEnabled")?fo()?k("coolingPermitted")?k("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:D("coolingBlockReason","Koeling geblokkeerd"),tone:"orange"}:k("silentActive")?{label:"Systeem",value:"Stille uren actief",tone:"neutral"}:k("stickyActive")?{label:"Systeem",value:"Pompbescherming actief",tone:"neutral"}:{label:"Systeem",value:"Normaal",tone:"neutral"}:{label:"Systeem",value:"Vorstbeveiliging blijft actief",tone:"neutral"}}function Ol(e,o){let r=Nw(),n=Ow();return[{label:"Strategie",value:e,tone:"orange",note:"regelstrategie"},{label:"Controlmode",value:o,tone:"orange",note:"actieve modus"},{label:"Regeling",value:r.value,tone:"orange",note:"wat OpenQuatt nu doet"},{label:"Systeem",value:n.value,tone:"orange",note:"actieve randvoorwaarde"}]}function Dl(e,o){let r=Ol(e,o);return`
    ...
    `}function Ll(){let e=fo();return[{key:"totalPower",label:"Elektrisch vermogen",tone:"blue",note:"hele systeem"},{key:e?"totalCoolingPower":"totalHeat",label:e?"Koelvermogen":"Verwarmingsvermogen",tone:"orange",note:"thermisch vermogen"},{key:e?"totalEer":"totalCop",label:e?"COP (EER)":"COP",tone:"green",note:"rendement"},{key:"flowSelected",label:"Flow",tone:"sky",note:"watercircuit"}]}function Dw(){let e=k("openquattEnabled"),o=T("openquattResumeAt"),r=Br(o),n=(t.loadingEntities||t.entitySyncInFlight)&&!h("openquattResumeAt"),a=k("manualCoolingEnable"),i=h("coolingEnableSelected")?k("coolingEnableSelected"):a,l=Ym(D("coolingEnableEffectiveSource","")),p=String(T("coolingEnableSource")||"").trim(),u=Ym(T("coolingEnableSource")),c=String(T("silentModeOverride")||"Schedule"),m=!k("coolingPermitted"),b=k("coolingRequestActive"),y=og(),w=D("coolingBlockReason",""),g=rg(w,b),f="Uit",v=p==="Disabled"?"Koeling is niet toegestaan: handmatig staat uit.":u&&u!=="geen bron"?`Koeling is niet toegestaan: ${u} geeft geen toestemming en handmatig staat uit.`:"Koeling is niet toegestaan.";i&&y?(f="Actief",v=$n("Koeling draait nu.",l)):i&&g?(f="Aan",v=$n("Koeling is toegestaan en wacht op kamertemperatuur boven het koel-setpoint.",l)):i&&m?(f="Geblokkeerd",v=$n(Cl(w||"Koeling wacht nog op veilige condities."),l)):i&&b?(f="Start bijna",v=$n("Er is koelvraag. Koeling start zodra dat kan.",l)):i&&(f="Aan",v=$n("Koeling is toegestaan en wacht op koelvraag.",l));let S="Uit",q="Stille modus staat uit.",C="neutral";return c==="On"?(S="Aan",q="Stille modus staat geforceerd aan, ook buiten het tijdvenster.",C="orange"):c==="Schedule"&&(S="Schema",k("silentActive")?(q="Stille modus staat nu aan via het tijdvenster.",C="violet"):q="Stille modus volgt het tijdvenster."),[{key:"openquattEnabled",label:"Openquatt regeling",status:e?"Actief":"Tijdelijk uit",copy:e?"Verwarmen en koelen worden automatisch geregeld.":r?"Verwarming en koeling zijn tijdelijk uitgeschakeld. Beveiligingen blijven actief.":"Verwarming en koeling zijn uitgeschakeld. Beveiligingen blijven actief.",tone:e?"green":"orange",kind:"openquatt-control",meta:e?[]:[n?{label:"Hervatten",value:"Laden\u2026",tone:"neutral",loading:!0}:{label:r?"Hervat automatisch":"Hervatten",value:r?Jo(o,!0):"Handmatig",tone:r?"orange":"neutral"}]},{key:"manualCoolingEnable",label:"Koeling",status:f,copy:v,buttonLabel:a?"Handmatig uit":"Handmatig aan",nextState:a?"off":"on",tone:i?y?"blue":"sky":"neutral"},{key:"silentModeOverride",label:"Stille modus",status:S,copy:q,tone:C,kind:"select",selectedOption:c,settingsAction:!0,options:[{value:"Off",label:"Uit"},{value:"On",label:"Aan"},{value:"Schedule",label:"Schema"}]}].filter(x=>h(x.key))}function Lw(e=[]){return e.length?`
    ...
    `}function Fw(e){if(e.kind==="openquatt-control"){let o=t.busyAction==="openquatt-regulation",r=(t.loadingEntities||t.entitySyncInFlight)&&!h("openquattResumeAt");return k("openquattEnabled")?`<div class="oq-overview-controlpanel-actions">${$r({className:"oq-overview-controlpanel-toggle",action:"open-openquatt-pause-modal",label:"Tijdelijk uitschakelen",busy:o})}</div>`:`
          <div class="oq-overview-controlpanel-actions oq-overview-controlpanel-actions--split">
            ...
            ${$r(r?{className:"oq-overview-controlpanel-segment oq-overview-controlpanel-segment--loading",action:"",label:"Hervatopties laden\u2026",loading:!0}:{className:"oq-overview-controlpanel-segment",action:"open-openquatt-pause-modal",label:Br()?"Moment wijzigen":"Automatisch hervatten"})}
          </div>
    ...
    `}function Jw(){let e=Ut(),o=Vs();return`
    ...
    `}function Rn({bind:e,modifier:o,x:r,y:n,width:a,kicker:i,detail:l,detailBind:p="",icon:u="heater",direction:c="down"}){let b=r+26,y=n+22,w=p?` data-oq-bind="${s(p)}"`:"",g="";if(c==="up"){let f=r+Math.round(a*.52);g=`M${f-6} ${n} L${f} ${n-8} L${f+6} ${n} Z`}else if(c==="left"){let f=n+Math.round(22);g=`M${r} ${f-6} L${r-8} ${f} L${r} ${f+6} Z`}else if(c==="right"){let f=n+Math.round(22);g=`M${r+a} ${f-6} L${r+a+8} ${f} L${r+a} ${f+6} Z`}else{let f=r+Math.round(a*.52);g=`M${f-6} ${n+44} L${f} ${n+44+8} L${f+6} ${n+44} Z`}return`
      <g
    ...
    `}function iS({tooltip:e,...o}){return`${zl(o)}${Rn({bind:o.bind,...e})}`}function sS({bind:e,ariaLabel:o,x:r,y:n,width:a,height:i,rx:l,tooltip:p}){return`
    ...
    `}function yS(e,o){if(!e)return!1;let r=e.querySelector(".oq-overview-hp-tools-copy"),n=e.querySelector('[data-hp-visual="schematic"]'),a=e.querySelector('[data-hp-visual="compact"]');return!r||!n||!a?($t(e,wg(o)),!0):($t(r,`
      <h3>Warmtepompen</h3>
      ${hg(o)}
