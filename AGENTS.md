...
- Before opening or updating a PR, perform a deliberate review of the complete diff against the target branch; do not limit the audit to the latest edit.
- Audit correctness, security and privacy, lifecycle and timing, failure handling, compatibility, migrations, upgrades, backup/restore, and fresh-install behavior where relevant.
- Cross-check configuration values against user-facing UI copy and documentation, inspect all relevant entry paths, and verify that generated files and focused regression tests cover the change.
- Resolve findings and run the appropriate checks before publishing the PR. Summarize the audit scope and validation in the PR body.
